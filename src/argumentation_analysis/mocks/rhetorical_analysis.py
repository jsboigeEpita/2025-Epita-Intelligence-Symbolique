...
class MockRhetoricalAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info("MockRhetoricalAnalyzer initialisé avec la config : %s", self.config)
    ...
    def analyze(self, text: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not isinstance(text, str):
            logger.warning("MockRhetoricalAnalyzer.analyze a reçu une entrée non textuelle.")
            return {"error": "Entrée non textuelle", "figures_de_style": [], "tonalite": "Neutre (Erreur)"}
        ...
        if "exemple de métaphore" in text.lower():
            figures_de_style.append({
                ...
                "extrait": text[text.lower().find("exemple de métaphore"):text.lower().find("exemple de métaphore")+30],
        ...
        if "question rhétorique" in text.lower():
            figures_de_style.append({
                ...
                "extrait": text[text.lower().find("question rhétorique"):text.lower().find("question rhétorique")+30],
        ...
        return {
            ...
            "score_engagement_simule": min(max(score_engagement, 0), 1), # clamp entre 0 et 1
