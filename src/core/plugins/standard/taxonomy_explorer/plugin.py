...
from argumentation_analysis.agents.core.informal.taxonomy_sophism_detector import get_global_detector, TaxonomySophismDetector
...
class TaxonomyExplorerPlugin(BasePlugin):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.critical(f"Échec de l'initialisation du TaxonomyExplorerPlugin: {e}", exc_info=True)
            raise

    def _load_families(self):
        """Charge les définitions des familles depuis le fichier YAML externe."""
        families_path = os.path.join(os.path.dirname(__file__), 'data', 'fallacy_families.yaml')
        self.logger.info(f"Chargement des familles de sophismes depuis : {families_path}")
        ...
        for pk, row in df.iterrows():
            ...
            for family_id, family_info in self.families.items():
                ...
                for pattern in family_info.patterns:
                    ...
                    if pattern_lower in name: score += 0.8
                    if pattern_lower in nom_vulgarise: score += 0.9
                    if pattern_lower in description: score += 0.3
    ...
    async def find_fallacies_by_family(self, family_name: str) -> List[Dict[str, Any]]:
        ...
        if family_name not in self.families: return []
    ...
    async def get_fallacy_details(self, fallacy_key: int) -> Optional[Dict[str, Any]]:
        ...
        if details.get('error'): return None
    ...
    async def detect_and_classify(self, text: str, max_fallacies: int = 20) -> List[Dict[str, Any]]:
        ...
        for sophism in detected_sophisms:
            ...
            context_relevance = self._calculate_context_relevance(text, family_info) if family_info else 0.0
            family_score = self._calculate_family_pattern_score(text, family_info) if family_info else 0.0
            severity = self._calculate_family_severity(family_info, sophism['confidence']) if family_info else "Indéterminée"
            ...
            classified = ClassifiedFallacy(
                ...
                detection_method=f"taxonomy_family_{family_id}" if family_id else "taxonomy_unclassified"
            )
            classified_fallacies.append(classified.model_dump())
            
        classified_fallacies.sort(key=lambda x: (x['confidence'] + x['family_pattern_score']) / 2, reverse=True)
        return classified_fallacies

    async def get_family_statistics(self, classified_fallacies: List[Dict]) -> Dict[str, Any]:
        """Génère des statistiques par famille à partir d'une liste de sophismes classifiés."""
        family_stats = {}
        total_fallacies = len(classified_fallacies)
        if total_fallacies == 0: return {}
    ...
    def _calculate_family_pattern_score(self, text: str, family_info: FallacyFamily) -> float:
        text_lower = text.lower()
        score = sum(0.1 for keyword in family_info.keywords if keyword.lower() in text_lower)
        return min(score, 1.0)
    
    def _calculate_contextual_relevance(self, text: str, family_info: FallacyFamily) -> float:
        text_lower = text.lower()
        relevance = sum(0.2 for context in family_info.common_contexts if context.lower() in text_lower)
        return min(relevance, 1.0)
    
    def _calculate_family_severity(self, family_info: FallacyFamily, base_confidence: float) -> str:
        weighted_score = base_confidence * family_info.severity_weight
        if weighted_score >= 0.8: return "Critique"
        if weighted_score >= 0.6: return "Haute"
        if weighted_score >= 0.4: return "Moyenne"
        if weighted_score >= 0.2: return "Faible"
        return "Négligeable"
