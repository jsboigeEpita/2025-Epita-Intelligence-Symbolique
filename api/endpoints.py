...
informal_router = APIRouter(
    prefix="/api/v1/informal", tags=["Informal Fallacy Analysis"]
)
...
framework_router = APIRouter(
    prefix="/api/v1/framework", tags=["Dung's Argumentation Framework"]
)
...
async def analyze_text_endpoint(analysis_req: AnalysisRequest, fastapi_req: Request):
    ...
    logger.info(
        f"[{analysis_id}] Requête d'analyse reçue: '{analysis_req.text[:80]}...'"
    )
    ...
    return {"analysis_id": analysis_id, "status": "success", "results": results_payload}
...
async def health_check_endpoint():
    ...
    return StatusResponse(
        status="operational", service_status={"details": "API is healthy and running."}
    )
...
async def list_endpoints_endpoint(request_fastapi: Request):
    ...
    for route in request_fastapi.app.routes:
        if hasattr(route, "path") and route.path not in [
            ...
            url_list.append(
                EndpointDetail(
                    path=route.path, methods=methods, description=description
                )
            )
