...
class InformalAnalysisPlugin:
    ...
    def _internal_explore_hierarchy(self, current_pk: int, df: pd.DataFrame, max_children: int = 15) -> Dict[str, Any]:
        ...
        result["current_node"] = {
            ...
            "parent_pk": int(current_row['FK_Parent']) if pd.notna(current_row['FK_Parent']) else None,
        ...
        if children_count > 0:
            ...
            for _, child in children.iterrows():
                child_info = {
                    ...
                    "has_children": len(df[df['FK_Parent'] == child['PK']]) > 0
                }
    ...
    def _internal_get_node_details(self, pk: int, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Trouver le parent
        if pd.notna(row['FK_Parent']):
            parent_pk = int(row['FK_Parent'])
            parent = df[df['PK'] == parent_pk]
            if len(parent) > 0:
