...
def run_tests(test_type="all", verbose=False, coverage_report=False):
    ...
    if coverage_report:
        cov = coverage.Coverage(
            source=["argumentiation_analysis.agents.tools.analysis"],
    ...
    if test_type in ["all", "unit"]:
        ...
        for test_file in unit_tests:
            ...
            if test_path.exists():
                module_name = f"argumentiation_analysis.tests.tools.{test_file[:-3]}"
                try:
    ...
    if test_type in ["all", "integration"]:
        ...
        if test_path.exists():
            module_name = f"argumentiation_analysis.tests.tools.{integration_test_file[:-3]}"
            try:
                tests = loader.loadTestsFromName(module_name)
                suite.addTests(tests)
    ...
    if test_type in ["all", "performance"]:
        ...
        if test_path.exists():
            module_name = f"argumentiation_analysis.tests.tools.{performance_test_file[:-3]}"
            try:
                tests = loader.loadTestsFromName(module_name)
                suite.addTests(tests)
