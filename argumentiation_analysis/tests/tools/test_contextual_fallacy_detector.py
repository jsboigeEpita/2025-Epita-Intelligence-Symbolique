...
class TestContextualFallacyDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.transformers_patcher = patch('argumentiation_analysis.agents.tools.analysis.new.contextual_fallacy_detector.HAS_TRANSFORMERS', False)
    ...
    def test_initialization(self):
        ...
        self.assertIsNotNone(self.detector.fallacy_taxonomy)
        self.assertIsNotNone(self.detector.context_profiles)
        self.assertIsNotNone(self.detector.detection_models)
        self.assertEqual(len(self.detector.detection_history), 0)

    def test_detect_contextual_fallacies(self):
        """Teste la méthode detect_contextual_fallacies."""
        result = self.detector.detect_contextual_fallacies(self.test_arguments, self.test_context)
        
        # Vérifier la structure du résultat
        self.assertIn("context_analysis", result)
        self.assertIn("detected_fallacies", result)
        self.assertIn("fallacy_relationships", result)
        self.assertIn("context_specific_insights", result)
        self.assertIn("detection_confidence", result)
        self.assertIn("detection_timestamp", result)
        ...
        self.assertGreaterEqual(len(result["detected_fallacies"]), 0)
        
        # Vérifier que l'historique de détection a été mis à jour
        self.assertEqual(len(self.detector.detection_history), 1)
        self.assertEqual(self.detector.detection_history[0]["type"], "contextual_fallacy_detection")
