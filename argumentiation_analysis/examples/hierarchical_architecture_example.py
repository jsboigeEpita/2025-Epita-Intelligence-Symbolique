...
async def example_complete_analysis():
    ...
    try:
        ...
        # Traiter la tâche
        logger.info(f"2. Traitement de la tâche d'analyse informelle: {informal_task['id']}")
        informal_result = await manager.process_tactical_task(informal_task)
        logger.info(f"Résultat de l'analyse informelle: {json.dumps(informal_result, indent=2)}")
        ...
        logger.info(f"Résultat de l'analyse formelle: {json.dumps(pl_result, indent=2)}")
