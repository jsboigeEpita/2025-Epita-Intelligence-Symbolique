...
def _download_file_with_progress(file_url: str, target_path: pathlib.Path, description: str):
    ...
    try:
        ...
        if response.status_code == 404:
             ...
             return False, False # Non présent, non-téléchargé
        ...
        if target_path.exists() and target_path.stat().st_size > 0:
            ...
            return True, True # Présent, téléchargé
        else:
            logger.error(f"❓ Téléchargement de '{target_path.name}' semblait terminé mais fichier vide ou absent.")
            if target_path.exists(): target_path.unlink(missing_ok=True) # Nettoyer fichier vide
            return False, False # Non présent, non-téléchargé
...
def download_tweety_jars(
    ...
    Args:
        version (str): Version de Tweety à télécharger.
        target_dir (str): Répertoire où placer les JARs principaux.
        native_subdir (str): Sous-répertoire dans target_dir pour les binaires natifs.

    Returns:
        bool: True si le JAR Core et au moins quelques modules sont présents (téléchargés ou existants), False sinon.
    """
    ...
    NATIVE_LIBS_DIR.mkdir(exist_ok=True) # Créer aussi le dossier natif

    CORE_JAR_NAME = f"org.tweetyproject.tweety-full-{version}-with-dependencies.jar"

    # Modules Tweety requis (basé sur l'ancien notebook, ajustez si besoin)
    ...
    if not core_present:
        ...
        return False # Inutile de continuer sans le core

    # 3. Télécharger JARs Modules
    ...
    for module in tqdm(REQUIRED_MODULES, desc="Modules JARs"):
        ...
        elif url_accessible: # Ne lister comme manquant que si on aurait dû pouvoir le télécharger
    ...
    if not native_binaries:
        ...
        for name in tqdm(native_binaries, desc="Binaires Natifs"):
             ...
             if present:
                 ...
                 # Rendre exécutable sur Linux/Mac après téléchargement
                 if new_dl and system != "Windows":
                     try:
                         target_path = NATIVE_LIBS_DIR / name
                         current_permissions = target_path.stat().st_mode
                         target_path.chmod(current_permissions | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH) # Add execute permissions
             ...
             elif url_accessible: # Là encore, ne compter comme manquant que si DL aurait dû être possible
    ...
    # Considérer réussi si le Core est là et au moins quelques modules
...
def initialize_jvm(
    ...
    """
    Vérifie/Télécharge les JARs Tweety puis initialise la JVM via JPype.

    Args:
        lib_dir_path (str): Chemin vers le répertoire contenant les JARs Tweety.
        native_lib_subdir (str): Sous-dossier optionnel pour les bibliothèques natives.
        tweety_version (str): Version de Tweety à assurer/télécharger.

    Returns:
        bool: True si la JVM a été démarrée ou était déjà démarrée, False sinon.
    """
    logger.info("\n--- Préparation et Initialisation de la JVM via JPype ---")

    # Étape 0: Télécharger/Vérifier les JARs Tweety
    ...
    # Étape 1: Démarrer la JVM (logique quasi-identique à avant)
    LIB_DIR = pathlib.Path(lib_dir_path)
    NATIVE_LIBS_DIR = LIB_DIR / native_lib_subdir
    jvm_ready = False

    # --- Vérification si déjà démarrée ---
    ...
    if java_home_to_set and not os.getenv("JAVA_HOME"):
        try:
         ...
         return False # Arrêter si pas de JAVA_HOME
    ...
    try:
        ...
        # Utiliser le LIB_DIR défini au début de cette fonction
        jar_list = sorted([str(p.resolve()) for p in LIB_DIR.glob("*.jar")])
        if not jar_list: # Vérification supplémentaire
        ...
        # Utiliser le NATIVE_LIBS_DIR défini au début de cette fonction
        ...
        # Ajout argument pour mémoire si besoin (optionnel)
        # jvm_args.append("-Xmx4g") # Exemple: 4GB max heap
