...
class ProjectArchitectureAnalyzer:
        ...
        for py_file in self.project_root.rglob('*.py'):
            if self._should_analyze_file(py_file):
                try:
                    ...
                    if module_info:
                        self.modules[str(py_file.relative_to(self.project_root))] = module_info
    ...
    def _calculate_metrics(self):
        ...
        self.metrics = {
            ...
            'agents_count': len([m for m in self.modules.values() if m.type == 'agent']),
            'tools_count': len([m for m in self.modules.values() if m.type == 'tool']),
            'orchestration_count': len([m for m in self.modules.values() if m.type == 'orchestration']),
            'entry_points_count': len([m for m in self.modules.values() if m.is_entry_point]),
            'avg_complexity': sum(m.complexity_score for m in self.modules.values()) / len(self.modules) if self.modules else 0,
