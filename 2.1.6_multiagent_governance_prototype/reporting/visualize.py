...
def plot_results(results, save_path=None):
    ...
    if history and rounds > 1:
        consensus = [h['votes'].count(h['winner']) / len(h['votes']) if h['votes'] else 0 for h in history]
...
def plot_coalitions(coalitions, save_path=None):
    ...
    plt.figure(figsize=(6, 6))
    nx.draw(G, with_labels=True, node_color=color_map, cmap=plt.cm.Set3, node_size=800, font_size=10)
...
def plot_manipulability_impact(results_list, metrics=('consensus_rate', 'fairness', 'satisfaction'), save_path=None):
    """
