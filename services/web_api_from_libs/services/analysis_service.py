...
class AnalysisService:
    ...
    def health_check(self):
        ...
        self.logger.info(
            f"✅ ComplexFallacyAnalyzer: {'Présent' if self.complex_analyzer else 'Absent'}"
        )
        self.logger.info(
            f"✅ ContextualFallacyAnalyzer: {'Présent' if self.contextual_analyzer else 'Absent'}"
        )
        self.logger.info(
            f"✅ FallacySeverityEvaluator: {'Présent' if self.severity_evaluator else 'Absent'}"
        )
        self.logger.info(
            f"✅ InformalAgent: {'Présent' if self.informal_agent else 'Absent'}"
        )
    ...
    async def _detect_fallacies(self, text: str, options) -> List[FallacyDetection]:
        ...
        try:
            ...
            # Analyse contextuelle si disponible
                ...
                result = self.contextual_analyzer.identify_contextual_fallacies(
                    text, ""
                )
                if result:
