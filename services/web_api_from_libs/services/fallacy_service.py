...
try:
    ...
    logging.warning(
        f"Impossible d'importer les analyseurs de sophismes (depuis web_api_from_libs): {e}"
    )
...
class FallacyService:
    ...
    def _initialize_analyzers(self):
        ...
        try:
            ...
            if not self.fallacy_detector:
                ...
                self.logger.warning(
                    "Aucun FallacyDetector fourni. Les analyseurs avancés sont désactivés."
                )
                return
    ...
    def _detect_with_enhanced_analyzer(
        ...
        try:
            if self.enhanced_analyzer:
                results = self.enhanced_analyzer.identify_contextual_fallacies(text, "")
    ...
    def _filter_and_deduplicate(
        self, fallacies: List[FallacyDetection], options
    ) -> List[FallacyDetection]:
        ...
        if options and options.categories:
            category_map = {
                info["name"]: key for key, info in self.fallacy_patterns.items()
            }
            filtered = [
                f
                for f in filtered
