...
class TestInterfacesCoexistence(unittest.TestCase):
    ...
    def test_02_key_files_existence(self):
        ...
        for file_name, file_path in self.key_files.items():
            with self.subTest(file=file_name):
                self.assertTrue(
                    file_path.exists(),
                    f"Le fichier {file_name} doit exister: {file_path}",
                )
    ...
    def test_03_port_configuration_separation(self):
        ...
        if self.key_files["simple_app"].exists():
            ...
            port_found = any(
                port in simple_content for port in ["3000", "5000", "PORT"]
            )
            self.assertTrue(
                port_found,
        ...
        if self.key_files["react_package"].exists():
            ...
            self.assertIn(
                "scripts", package_data, "package.json doit contenir des scripts"
            )
    ...
    def test_04_documentation_integration(self):
        ...
        self.assertGreater(
            len(existing_readmes), 0, "Au moins un fichier README doit exister"
        )
        ...
        if main_readme.exists():
            ...
            self.assertIn(
                "interface", readme_content, "Le README doit mentionner les interfaces"
            )
        ...
        if self.key_files["react_package"].exists():
            try:
                ...
                self.assertIn(
                    "dependencies",
                    package_data,
                    "package.json doit avoir des dépendances",
                )
        ...
        for script in scripts_to_check:
            if script.exists():
                with self.subTest(script=script.name):
                    ...
                    has_interface_mention = any(
                        mention in script_content for mention in interface_mentions
                    )

                    self.assertTrue(
                        has_interface_mention,
    ...
    def test_07_shared_resources_no_conflicts(self):
        ...
        if simple_templates.exists() and react_src.exists():
            ...
            simple_files = set(
                f.name for f in simple_templates.rglob("*") if f.is_file()
            )
            ...
            critical_conflicts = critical_conflicts.intersection(
            ...
            if critical_conflicts:
                ...
                print(
                    f"[INFO] Fichiers avec noms similaires détectés: {critical_conflicts}"
                )
                        ...
                        try:
                            ...
                            if "localhost" in api_content or "api/" in api_content:
                                ...
                                self.assertIn(
                                    "/",
                                    api_content,
                                    "Les appels API doivent être structurés",
                                )
        ...
        if flask_app.exists():
            ...
            try:
                ...
                self.assertEqual(
                    result.returncode,
                    0,
                    "L'app Flask doit être syntaxiquement correcte",
                )
            ...
            for script in essential_scripts:
                if script in scripts:
                    self.assertIsInstance(
                        scripts[script], str, f"Le script {script} doit être une chaîne"
                    )
                ...
                if config_file.suffix == ".json":
                    try:
                        with open(config_file, "r", encoding="utf-8") as f:
                            json.load(f)
                        self.assertTrue(
                            True, f"Configuration JSON {config_file.name} valide"
                        )
                    except json.JSONDecodeError:
...
def test_interfaces_integration_complete():
    ...
    suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestIntegrationEnvironment)
    )
