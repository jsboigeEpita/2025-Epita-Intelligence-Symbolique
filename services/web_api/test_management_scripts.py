...
class TestManagementScripts(unittest.TestCase):
    ...
    def test_02_health_check_script_basic(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.scripts["health_check"]), "--help"],
                capture_output=True,
                text=True,
                timeout=10,
            )
            ...
            self.assertEqual(
                result.returncode, 0, "Le script health_check doit afficher l'aide"
            )
            self.assertIn(
                "health_check",
                result.stdout.lower(),
                "L'aide doit mentionner health_check",
            )
    ...
    def test_03_health_check_execution(self):
        ...
        try:
            ...
            self.assertIn(
                result.returncode, [0, 1], "Le health check doit retourner 0 ou 1"
            )
            ...
            self.assertTrue(
                has_verification,
                f"La sortie doit mentionner une vérification. Sortie: {result.stdout[:200]}",
            )
    ...
    )  # Simuler réponse "non" pour l'ouverture navigateur
    def test_04_start_simple_script_help(self, mock_input):
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.scripts["start_simple"]), "--help"],
                capture_output=True,
                text=True,
                timeout=10,
            )
    ...
    def test_05_stop_all_script_help(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.scripts["stop_all"]), "--help"],
                capture_output=True,
                text=True,
                timeout=10,
            )
            ...
            self.assertEqual(
                result.returncode, 0, "Le script stop_all doit afficher l'aide"
            )
            self.assertIn(
                "stop", result.stdout.lower(), "L'aide doit mentionner l'arrêt"
            )
    ...
    def test_06_port_detection_functionality(self):
        ...
        self.assertIsInstance(
            processes_9999, list, "La détection de port doit retourner une liste"
        )
        ...
        for port in [80, 443, 3000]:
            ...
            self.assertIsInstance(
                processes, list, f"La détection du port {port} doit retourner une liste"
            )
...
class TestHealthCheckIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.health_script = (
            self.project_root / "services" / "web_api" / "health_check.py"
        )
...
def test_management_scripts_complete():
    ...
    suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestHealthCheckIntegration)
    )
