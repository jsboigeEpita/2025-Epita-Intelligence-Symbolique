    ...
    try:
        ...
        if service_manager:
            try:
                ...
                logger.info(f"Analyse {analysis_id} terminée avec succès par ServiceManager")
...
async def status():
    ...
    try:
        if service_manager:
            ...
            try:
                health_status = await service_manager.health_check() if hasattr(service_manager, 'health_check') else {'status': 'healthy'}
                service_details = await service_manager.get_service_status() if hasattr(service_manager, 'get_service_status') else {'components': ['ServiceManager']}
            except Exception as e:
            ...
            return jsonify({
                ...
                'services': {
                    ...
                    'fallacy_analyzers': 'available' if FALLACY_ANALYZERS_AVAILABLE else 'unavailable',
    ...
    if 'hierarchical' in results:
        ...
        if 'strategic' in hierarchical:
            ...
            if 'fallacy_analysis' in strategic:
                ...
                fallacy_analysis['fallacies_detected'].extend(fallacy_data.get('detected', []))
