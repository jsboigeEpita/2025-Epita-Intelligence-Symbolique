...
def test_api_endpoints():
    ...
    try:
        ...
        print(
            f"[OK] Status endpoint: {response.status_code} ({end_time - start_time:.3f}s)"
        )
    ...
    try:
        ...
        print(
            f"[OK] Examples endpoint: {response.status_code} ({end_time - start_time:.3f}s)"
        )

    except Exception as e:
        results["tests"]["examples"] = {"success": False, "error": str(e)}
    ...
    try:
        ...
        print(
            f"[OK] Analyze endpoint: {response.status_code} ({end_time - start_time:.3f}s)"
        )

    except Exception as e:
        results["tests"]["analyze"] = {"success": False, "error": str(e)}
...
if __name__ == "__main__":
    ...
    with open(
        f"../../../logs/web_api_calls_{timestamp}.json", "w", encoding="utf-8"
    ) as f:
