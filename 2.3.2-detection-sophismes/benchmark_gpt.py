...
def main():
    parser = argparse.ArgumentParser(description="Benchmark ChatGPT's fallacy classification on the test dataset.")
    ...
    parser.add_argument("--n_samples", type=int, default=0, help="Number of samples to test from the dataset. Set to 0 to test all samples.")
    ...
    print(f"Starting benchmark for {len(test_df)} samples. This may take a while and incur costs.")

    for index, row in test_df.iterrows():
        text = row['text']
        true_label_id = row['labels']
        true_fallacy_name = id_to_label[str(true_label_id)] # Get the string name of the true fallacy
        ...
        if gpt_response.get("fallacies_detected") and gpt_response.get("fallacies"):
            predicted_fallacy_types = [f.get('type', 'N/A') for f in gpt_response["fallacies"]]
            predicted_sophism_display = ", ".join(predicted_fallacy_types)
        
        print(f"Sample {index+1}: Expected: '{true_fallacy_name}', Predicted: '{predicted_sophism_display}'")
        ...
        if gpt_response.get("fallacies_detected") and gpt_response.get("fallacies"):
            ...
            for fallacy in gpt_response["fallacies"]:
                ...
                normalized_gpt_fallacy_type = gpt_fallacy_type.split(' (')[0].strip().lower() # Take only French part, lowercase
                normalized_true_fallacy_type = true_fallacy_name.split(' (')[0].strip().lower()

                if normalized_gpt_fallacy_type == normalized_true_fallacy_type:
                    gpt_predicted_id = true_label_id # If GPT predicted the correct one, assign its ID
                    found_match = True
                    break
            if not found_match:
