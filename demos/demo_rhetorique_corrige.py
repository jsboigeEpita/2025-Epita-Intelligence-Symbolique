...
"""
Démo corrigée du système d'analyse rhétorique unifié

Ce script teste les capacités fonctionnelles du système avec les bonnes APIs.
"""
...
logger = logging.getLogger("DemoRhetoriqueCorrige")
...
async def test_state_management(results: DemoResults):
    ...
    try:
        ...
        # Créer un état d'analyse avec le bon paramètre
        analysis_state = RhetoricalAnalysisState(initial_text=TEXTES_TEST["sophismes_basiques"])
        ...
        analysis_state.add_argument("arg1", "Test argument")
        
        # Tester l'ajout de sophismes
        analysis_state.add_fallacy("fallacy1", "ad_hominem", "Test fallacy")
...
async def test_semantic_kernel_integration(llm_service, results: DemoResults):
    ...
    try:
        ...
        # Créer une fonction simple de façon correcte
        ...
        kernel.add_function(
            ...
            function=analyze_text_function
        )
        
        # Exécuter la fonction
        result = await kernel.invoke(
            function_name="demo_plugin-analyze_text", 
            arguments=KernelArguments(text=TEXTES_TEST["sophismes_basiques"])
        )
...
async def test_rhetorical_tools(results: DemoResults):
    ...
    try:
        # Test des fonctions de traitement de texte (pas de classe)
        ...
        cleaned_text = normalize_text(test_text)
        tokens = tokenize_text(test_text)
        
        results.add_capability("Text Processing Functions", "working", {
...
async def test_java_configuration(results: DemoResults):
    """Test la configuration Java pour Tweety"""
    demo_name = "Configuration Java"
    logger.info(f"=== {demo_name} ===")
    demo_start = time.time()
    ...
    try:
        ...
        # Tenter d'initialiser la JVM
        jvm_status = initialize_jvm(lib_dir_path=LIBS_DIR)
        
        if jvm_status:
            results.add_capability("JVM Initialization", "working", {
                "libs_dir": str(LIBS_DIR)
            })
            logger.info("JVM initialisée avec succès")
        else:
            results.add_capability("JVM Initialization", "failed", {
                "libs_dir": str(LIBS_DIR),
                "reason": "initialize_jvm returned False"
            })
            logger.warning("JVM n'a pas pu être initialisée")
        
        # Vérifier si JPype est disponible
        try:
            import jpype
            if jpype.isJVMStarted():
                results.add_capability("JPype JVM Status", "working", {
                    "jvm_started": True
                })
            else:
                results.add_capability("JPype JVM Status", "failed", {
                    "jvm_started": False
                })
        except Exception as jpype_error:
            results.add_capability("JPype Import", "failed", {
                "error": str(jpype_error)
            })
...
async def test_environment_variables(results: DemoResults):
    """Test les variables d'environnement"""
    demo_name = "Variables d'Environnement"
    logger.info(f"=== {demo_name} ===")
    demo_start = time.time()
    ...
    try:
        ...
        results.add_capability("Environment Variables", "working", {
            "java_home": java_home,
            "has_path": bool(path)
        })
        
        if java_home:
            logger.info(f"JAVA_HOME défini: {java_home}")
        else:
            logger.warning("JAVA_HOME non défini")
        
        # Chercher des installations Java dans le PATH
        java_executables = []
        if path:
            for path_dir in path.split(os.pathsep):
                java_exe = os.path.join(path_dir, 'java.exe')
                if os.path.exists(java_exe):
                    java_executables.append(java_exe)
        
        results.add_capability("Java Executables Found", "working", {
            "count": len(java_executables),
            "paths": java_executables[:3]  # Limiter à 3 pour éviter la verbosité
        })
...
async def main():
    ...
    print("\n" + "="*60)
    print("  DEMO CORRIGEE DU SYSTEME D'ANALYSE RHETORIQUE UNIFIE")
    print("="*60)
    ...
    try:
        ...
        demos = [
            ...
            (test_java_configuration, (results,)),
        ...
        # Sauvegarder les résultats dans le bon répertoire
        results_file = project_root / "demos" / "rapport_demo_rhetorique_corrige.json"
