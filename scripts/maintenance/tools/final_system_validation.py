...
class OracleEnhancedValidator:
    ...
    def run_command_with_env(
        self, command: str, test_name: str
    ) -> Tuple[bool, str, str]:
    ...
    def test_oracle_imports(self) -> bool:
        ...
        if success and "Phase D OK" in stdout:
            self.log_result(
                "phase_d_extensions", True, "Extensions Phase D disponibles"
            )
            ...
            self.log_result(
                "phase_d_extensions",
                False,
                "Extensions Phase D non disponibles",
                stderr,
            )
    ...
    def test_sherlock_watson_functionality(self) -> bool:
        ...
        if success:
            self.log_result(
                "oracle_integration_test", True, "Tests d'intégration Oracle réussis"
            )
            ...
            self.log_result(
                "oracle_integration_test",
                False,
                "Tests d'intégration Oracle échoués",
                stderr,
            )
        ...
        if success:
            self.log_result(
                "cluedo_workflow_test", True, "Workflow Cluedo étendu fonctionnel"
            )
            ...
            self.log_result(
                "cluedo_workflow_test",
                False,
                "Workflow Cluedo étendu défaillant",
                stderr,
            )
        ...
        )  # Au moins 50% des tests de fonctionnalité doivent réussir
    ...
    def test_non_regression(self) -> bool:
        ...
        if phase_d_script.exists():
            success, stdout, stderr = self.run_command_with_env(
                "python test_phase_d_integration.py", "phase_d_integration"
            )
        ...
        for test_file in group_tests:
            ...
            if file_path.exists():
                success, stdout, stderr = self.run_command_with_env(
                    f"python {test_file}", f"group_test_{test_file}"
                )
                if success:
                    group_success += 1
                    self.log_result(
                        f"group_test_{test_file}", True, f"Test {test_file} réussi"
                    )
                else:
                    self.log_result(
                        f"group_test_{test_file}",
                        False,
                        f"Test {test_file} échoué",
                        stderr,
                    )

        self.results["integration_tests"]["group_tests_score"] = (
            group_success / len(group_tests) if group_tests else 0
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_result(
                    "documentation_links",
                    True,
                    "Aucun lien brisé détecté dans la documentation",
                )
                ...
                try:
                    ...
                    if valid_pct < 50:
                        self.results["warnings"].append(
                            f"📚 CRITIQUE: Seulement {valid_pct}% des liens de documentation sont valides"
                        )
                    elif valid_pct < 80:
                        self.results["warnings"].append(
                            f"📚 ATTENTION: {valid_pct}% des liens de documentation sont valides"
                        )
                except ValueError:
                    pass
            ...
            self.log_result(
                "documentation_links",
                False,
                "Timeout lors de l'analyse de documentation",
            )
            self.results["system_integrity"]["documentation_health"] = False
            return False
        except Exception as e:
    ...
    def generate_recommendations(self):
        ...
        if self.results["critical_tests"].get("imports", 0) < 0.8:
            recommendations.append(
                "[CRITIQUE] CRITIQUE: Corriger les problèmes d'imports Oracle Enhanced"
            )

        if self.results["critical_tests"].get("functionality", 0) < 0.5:
            recommendations.append(
                "[PRIORITE] HAUTE PRIORITÉ: Déboguer les fonctionnalités Sherlock-Watson-Moriarty"
            )

        if self.results["critical_tests"].get("documentation", 0) < 0.8:
            recommendations.append(
                "📚 IMPORTANT: Corriger les liens brisés dans la documentation"
            )

        if self.results["critical_tests"].get("integrity", 0) < 0.8:
            recommendations.append(
                "[IMPORTANT] IMPORTANTE: Restaurer l'intégrité de la structure du projet"
            )

        if self.results["critical_tests"].get("non_regression", 0) < 0.6:
            recommendations.append(
                "[PRIORITE] PRIORITÉ: Investiguer les régressions détectées"
            )
        ...
        if git_changes > 50:
            recommendations.append(
                f"[INFO] INFO: {git_changes} changements Git détectés - Préparer commit structuré"
            )
