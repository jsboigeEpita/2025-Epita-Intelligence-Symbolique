...
for module_name in modules_to_check:
    try:
        ...
        is_attribute_import = (
            len(parts) > 1 and parts[-1][0].islower()
        )  # Souvent les fonctions/attributs commencent par minuscule
        ...
        if is_attribute_import:
            ...
            parent_success, parent_message = test_module_import_by_name(
                parent_module_name
            )
            if parent_success:
                ...
                logging.info(
                    f"✅ Attribut '{attr_name}' du module '{parent_module_name}' accessible avec succès."
                )
                success_count += 1
            else:
                logging.error(
                    f"❌ Échec de l'import du module parent '{parent_module_name}' pour l'attribut '{attr_name}': {parent_message}"
                )
                failure_count += 1
        else:
        ...
        ImportError
    ) as e:  # Devrait être attrapé par test_module_import_by_name pour les modules
        logging.error(
            f"❌ Erreur lors de l'importation (ImportError) pour '{module_name}': {e}"
        )
        failure_count += 1
    except AttributeError as e:  # Spécifique à la logique getattr
        logging.error(
            f"❌ Attribut non trouvé (AttributeError) pour '{module_name}': {e}"
        )
        failure_count += 1
    except Exception as e_unexpected:  # Attraper d'autres erreurs inattendues
        logging.error(
            f"❌ Erreur inattendue pour '{module_name}': {e_unexpected}", exc_info=True
        )
        failure_count += 1
