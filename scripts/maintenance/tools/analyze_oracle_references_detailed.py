...
class OracleFileAnalyzer:
    ...
    def __init__(self, data_file: str):
        ...
        self.category_patterns = {
            ...
            FileCategory.TESTS_ORPHELINS: {
                "paths": [r"tests/.*test_.*\.py$", r".*test.*\.py$"],
    ...
    def analyze_code_quality(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        oracle_keywords = len(
            [k for k in keywords if k in ["oracle", "moriarty", "sherlock", "watson"]]
        )
        ...
        for category, patterns in self.category_patterns.items():
            ...
            if "keywords" in patterns:
                keyword_matches = sum(
                    ...
                    or kw.lower() in content_preview.lower()
                )
    ...
    def assess_recovery_potential(
        self, file_data: Dict[str, Any], category: FileCategory
    ) -> Tuple[str, int]:
        ...
        priority_matrix = {
            ...
            FileCategory.CODE_ACTIF: {"substantial": 8, "moderate": 6, "basic": 4},
            FileCategory.TESTS_ORPHELINS: {"substantial": 6, "moderate": 4, "basic": 2},
            FileCategory.SCRIPTS_TEMPORAIRES: {
                "substantial": 4,
                "moderate": 2,
                "basic": 1,
    ...
    def analyze_file(self, file_data: Dict[str, Any]) -> FileAnalysis:
        ...
        risk_assessment = (
            "FAIBLE" if not risk_factors else f"MOYEN ({', '.join(risk_factors)})"
        )
    ...
    def run_detailed_analysis(self) -> Dict[str, Any]:
        ...
        self.categorization_stats = {
            ...
            "priority_distribution": {
                f"priority_{i}": len(
                    [a for a in all_analyses if a.recovery_priority == i]
                )
                for i in range(1, 11)
            },
            "quality_distribution": {
                "substantial": len(
                    [
                        ...
                        if a.code_quality_metrics["maintainability"] == "high"
                    ]
                ),
                "moderate": len(
                    [
                        a
                        for a in all_analyses
                        if a.code_quality_metrics["maintainability"] == "medium"
                    ]
                ),
                "basic": len(
                    [
                        a
                        for a in all_analyses
                        if a.code_quality_metrics["maintainability"] == "low"
                    ]
                ),
            },
        }
        ...
        results = {
            "analysis_metadata": {
                ...
                "total_files_analyzed": len(all_analyses),
                "analysis_criteria": [
                    cat.value for cat in self.category_patterns.keys()
                ],
            },
            ...
            "high_priority_files": [
                ...
                for analysis in sorted(
                    all_analyses, key=lambda x: x.recovery_priority, reverse=True
                )[:20]
            ],
        }
...
def main():
    ...
    try:
        ...
        print("=" * 80)
        print(
            f"Total de fichiers analysés: {results['analysis_metadata']['total_files_analyzed']}"
        )
        print("\nDistribution par catégorie:")
        for category, count in results["categorization_summary"][
            "category_distribution"
        ].items():
