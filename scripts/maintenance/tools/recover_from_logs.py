...
class CodeRecovery:
        ...
        for _, event_type, *data in events:
            if event_type == "code":
                ...
                if self.promising_range[0] <= passed_count <= self.promising_range[1]:
                    ...
                    logging.info(
                        f"Created snapshot '{snapshot_name}' for test run with {passed_count} passed tests."
                    )
    ...
    def _collect_events(self, content: str) -> List[Tuple[int, str, Any]]:
        ...
        pytest_summary_regex = (
            r"={5,}\s*short test summary info\s*={5,}[\s\S]*?(?P<passed>\d+)\s+passed"
        )

        code_events = [
            (m.end(0), "code", m.group("tool"), m.group("xml_content"))
            for m in re.finditer(tool_tag_regex, content, re.DOTALL)
        ]
        test_events = [
            (m.start(0), "test", int(m.group("passed")))
            for m in re.finditer(pytest_summary_regex, content)
        ]

        all_events = sorted(code_events + test_events, key=lambda x: x[0])
        logging.info(
            f"Collected {len(all_events)} events ({len(code_events)} code, {len(test_events)} test)."
        )
        return all_events
    ...
    def _update_file_states(
        self, file_states: Dict[str, Dict[str, Any]], tool_name: str, xml_content: str
    ):
        """
        ...
        if not path:
            logging.warning(
                f"Could not find path in {tool_name} block: {xml_content[:100]}..."
            )
            return

        if tool_name in ["read_file", "write_to_file"]:
            content_match = re.search(
                r"<content>(.*?)</content>", xml_content, re.DOTALL
            )
            ...
            if path in file_states:
                diff_content_match = re.search(
                    r"<diff>(.*?)</diff>", xml_content, re.DOTALL
                )
                ...
                logging.warning(
                    f"Found apply_diff for '{path}' but no prior state. Ignoring."
                )
    ...
    def _create_snapshot(self, name: str, file_states: Dict[str, Dict[str, Any]]):
        ...
        for path_str, state in file_states.items():
            ...
            if state["diffs"]:
                ...
                logging.info(
                    f"  - Saved {path_str} with {len(state['diffs'])} diff(s)."
                )
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Recover code from development logs by creating snapshots based on test results."
    )
    parser.add_argument(
        "log_files",
        nargs="+",
        type=Path,
        help="Path to one or more log files to process.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./code_recovery_snapshots"),
    ...
    parser.add_argument(
        ...
        nargs=2,
        default=[5, 7],
        help="The range of passed tests [min, max] to consider 'promising' for a snapshot.",
    )
