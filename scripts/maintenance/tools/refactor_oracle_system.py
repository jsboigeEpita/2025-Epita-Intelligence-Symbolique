...
'''
    ...
    def _standardize_interfaces(self):
        ...
        # #2358 : ce script REGÉNÉRAIT interfaces.py depuis un gabarit gelé
        # portant des signatures mortes (query_type: str, méthodes sync) —
        # le rejouer aurait écrasé le contrat réel. Le module vivant est
        # désormais l'unique source de vérité : on le recopie tel quel
        # (idempotent), et son absence est une erreur bruyante.
        interface_path = self.oracle_dir / "interfaces.py"
        if not interface_path.exists():
            raise FileNotFoundError(
                f"{interface_path} introuvable : rien à standardiser, "
                "et ce script ne régénère plus le contrat depuis un gabarit."
            )
