...
def cleanup_processes():
    ...
    script_names_to_kill = [
        "uvicorn",
        "start_webapp.py",
        "unified_web_orchestrator.py",
        "temp_run_playwright_js.py",
    ]
    ...
    for p in psutil.process_iter(["pid", "name", "cmdline", "cwd"]):
        try:
            ...
            project_path_in_cmd = any(
                project_path_marker in os.path.normpath(L) for L in p.info["cmdline"]
            )
            cwd_in_project = process_cwd.startswith(project_path_marker)

            if (project_path_in_cmd or cwd_in_project) and any(
                script_name in cmdline_str for script_name in script_names_to_kill
            ):
