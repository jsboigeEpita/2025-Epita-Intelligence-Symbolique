...
class GitFilesInventory:
    ...
    def generate_recommendation(
        ...
        if status == "orphan":
            if file_path.startswith("test_"):
                if functional_test == "syntax_ok":
                    return (
                        "move",
                        f"Fichier de test fonctionnel - déplacer vers tests/validation_sherlock_watson/",
                    )
                else:
                    return (
                        "delete",
                        f"Fichier de test défaillant - supprimer (erreur: {functional_test})",
                    )
        ...
        if "/recovered/" in file_path:
            if functional_test == "syntax_ok":
                return (
                    "integrate",
                    "Code récupéré fonctionnel - intégrer dans la structure principale",
                )
            else:
                return "review", "Code récupéré à examiner avant intégration"
        ...
        if category == "oracle" or "sherlock" in file_path.lower():
            if functional_test == "syntax_ok":
                ...
                return (
                    "refactor",
                    f"Fichier Oracle/Sherlock avec erreurs - refactoriser ({functional_test})",
                )
            else:
                return "review", "Fichier Oracle/Sherlock à examiner"
    ...
    def run_inventory(self) -> None:
        ...
        print(
            f"[CHECK_MARK] Inventaire terminé - {len(self.files_analysis)} fichiers analysés"
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            for category, files in by_category.items():
                ...
                for file_analysis in files:
                    ...
                    f.write(
                        f"- Test fonctionnel: `{file_analysis.functional_test_result}`\n"
                    )
