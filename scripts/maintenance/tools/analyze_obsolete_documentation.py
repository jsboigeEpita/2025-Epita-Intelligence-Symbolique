...
def main():
    parser = argparse.ArgumentParser(
        description="Analyse de documentation obsolète Oracle Enhanced v2.1.0"
    )
    ...
    parser.add_argument(
        "--quick-scan",
        action="store_true",
        help="Analyse rapide (fichiers .md uniquement)",
    )
    parser.add_argument(
        "--full-analysis", action="store_true", help="Analyse complète (tous formats)"
    )
    ...
    if args.output_format == "json":
        output_file = (
            args.output
            or f"logs/obsolete_documentation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        with open(output_file, "w", encoding="utf-8") as f:
    ...
    print(
        f"   Pourcentage de liens valides: {100 - results['summary']['broken_percentage']:.1f}%"
    )
