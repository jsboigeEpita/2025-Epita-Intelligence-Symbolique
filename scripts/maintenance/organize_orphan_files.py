...
IGNORE_DIRS = {".git", "__pycache__", "node_modules", ".pytest_cache", "venv", "logs", "results"}
...
class OrphanFileAnalyzer:
    ...
    def scan_for_orphan_files(self) -> Dict[str, Any]:
        ...
        print("[SCAN] Analyse des fichiers orphelins Oracle/Sherlock/Watson/Moriarty...")
    ...
    def _analyze_file_content(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            content = ""
            if file_path.suffix in [".py", ".md", ".txt", ".yml", ".yaml", ".json", ".js", ".ts"]:
                try:
            ...
            if matches:
                ...
                analysis["integration_status"] = self._assess_integration_status(content, file_path)
    ...
    def _is_orphan_file(self, relative_path: Path) -> bool:
        ...
        if len(path_parts) == 1:
            ...
            config_files = {
                "conftest.py", ".env", ".env.example", ".gitignore", 
            ...
            if self._is_orphan_file(Path(file_path)):
                if self._has_recoverable_code(analysis):
                    ...
                    plan["code_recovery"].append({
                        "source": file_path,
                        "analysis": analysis,
                        "recommended_action": self._recommend_action(file_path, analysis)
                    })
                    plan["summary"]["code_to_integrate"] += 1
                else:
    ...
    def execute_organization_plan(self, dry_run: bool = True) -> Dict[str, Any]:
        ...
        if phase_d_source.exists():
            target = self.root_path / "argumentation_analysis/agents/core/oracle/phase_d_extensions.py"
        ...
        for recovery in self.organization_plan.get("code_recovery", []):
            ...
            if action == "move_to_scripts_orphaned":
                target_path = self.root_path / "scripts/orphaned" / Path(recovery["source"]).name
            elif action == "move_to_tests_orphaned":
                target_path = self.root_path / "tests/orphaned" / Path(recovery["source"]).name
            else:
...
def main():
    ...
    print("[START] Demarrage de l'analyse des fichiers orphelins Oracle/Sherlock/Watson/Moriarty")
    ...
    print(f"[OK] Simulation terminee: {len(execution_results['actions_completed'])} actions simulees")
    ...
    report_file = results_dir / f"orphan_files_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    report_file.write_text(report, encoding="utf-8")
    print(f"[SAVED] Rapport sauvegarde: {report_file}")
    
    # 6. Sauvegarder les données JSON
    json_file = results_dir / f"orphan_files_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    json_data = {
    ...
    print("="*80)
    print("1. [MOVE] Deplacer phase_d_extensions.py vers argumentation_analysis/agents/core/oracle/")
