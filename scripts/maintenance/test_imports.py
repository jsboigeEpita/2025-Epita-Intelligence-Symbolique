...
def test_import(module_name: str, attr_name: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        if attr_name:
            ...
            logging.info(
                f"✅ Attribut '{attr_name}' du module '{module_name}' importé avec succès."
            )
        else:
        ...
        logging.error(
            f"❌ Attribut '{attr_name}' non trouvé dans le module '{module_name}': {e}"
        )
        return False
...
def main() -> int:
    ...
    for module_name in modules_to_test:
        # `test_import` journalise déjà le message (succès ou échec) : un
        # second log ici ferait doublon. L'appel d'origine visait un helper
        # `test_module_import_by_name` qui n'existe pas (NameError, #2122).
