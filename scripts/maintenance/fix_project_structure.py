...
def main():
    parser = argparse.ArgumentParser(
        description="Résout les problèmes structurels du projet"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Ne pas effectuer les modifications"
    )
    parser.add_argument(
        "--skip-jars", action="store_true", help="Ne pas télécharger les JARs de test"
    )
    parser.add_argument(
        "--skip-imports",
        action="store_true",
        help="Ne pas mettre à jour les importations",
    )
    parser.add_argument(
        "--skip-paths",
        action="store_true",
        help="Ne pas mettre à jour les références aux chemins",
    )
    parser.add_argument(
        "--skip-tests", action="store_true", help="Ne pas exécuter les tests"
    )
    ...
    if return_code != 0:
        logging.error(
            f"❌ Erreur lors de l'installation du package (code: {return_code})."
        )
    ...
    if not args.skip_jars:
        ...
        if args.dry_run:
            ...
            run_shell_command(
                download_jars_cmd, description="Téléchargement des JARs de test"
            )
    ...
    if not args.skip_tests:
        test_imports_cmd = [
            sys.executable,
            str(script_dir / "utils/test_imports_utils.py"),
        ]
