...
class RealOrphanFilesProcessor:
    ...
    def categorize_file(self, file_path):
        ...
        categories = {
            ...
            'oracle_related': any(word in str(file_path).lower() for word in ['oracle', 'sherlock', 'watson'])
        }
    ...
    def get_recommendation(self, file_path, category):
        ...
        if category == 'logs':
            ...
            if 'backup' in str(file_path) and any(ext in str(file_path) for ext in ['.tar.gz', '.zip']):
                return 'delete', 'Archive temporaire non nécessaire sous contrôle de version'
            else:
            ...
            if 'validation' in str(file_path):
                return 'integrate', 'Test de validation - déplacer vers tests/validation/'
            else:
                return 'integrate', 'Fichier de test - intégrer dans la structure appropriée'
    ...
    def generate_reports(self):
        ...
        # Rapport JSON
        json_report_path = self.root_dir / "logs" / "complete_orphan_files_action_plan.json"
        ...
        with open(md_report_path, 'w', encoding='utf-8') as f:
            ...
            if plan['actions']['integrate']:
                f.write("## INTEGRATE ({} fichiers)\n\n".format(len(plan['actions']['integrate'])))
            ...
            for category, files in categories.items():
                f.write(f"### {category.replace('_', ' ').title()} ({len(files)} fichiers)\n")
                for file_info in files:
                    f.write(f"- `{file_info['path']}` → **{file_info['recommendation'].upper()}**\n")
                f.write("\n")
