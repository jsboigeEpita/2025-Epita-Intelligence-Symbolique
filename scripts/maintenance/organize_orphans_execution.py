...
def execute_orphan_organization():
    ...
    for doc_file in doc_files_to_clean:
        ...
        if file_path.exists():
            ...
            backup_path = base_path / "logs" / f"backup_{doc_file}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    summary_path = base_path / "logs" / f"orphan_organization_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
