...
class RecoveredCodeIntegrator:
    ...
    def analyze_file_content(self, file_path: str) -> Dict[str, Any]:
        ...
        if file_path.endswith(".py"):
            analysis["type"] = (
                "python_test" if "test_" in file_path else "python_script"
            )
        ...
        if analysis["type"].startswith("python"):
            try:
                ...
                analysis["oracle_related"] = any(
                    keyword.lower() in content.lower() for keyword in oracle_keywords
                )
                ...
                analysis["imports"] = import_lines[
                    :10
                ]  # Limite pour éviter trop de données
        ...
        if test_result["syntax_check"]:
            try:
                ...
                cmd = [
                    sys.executable,
                    "-c",
                    f"import sys; sys.path.insert(0, '.'); exec(open('{full_path}').read())",
                ]
            ...
            try:
                ...
                if result.returncode != 0:
                    test_result["errors"].append(
                        f"Pytest collect error: {result.stderr}"
                    )
            except Exception as e:
    ...
    def determine_integration_target(
    ...
    def integrate_file(
        self, source_path: str, target_dir: str, analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
...
'''
    ...
    def generate_integration_report(
        self, inventory: Dict, analyses: Dict, test_results: Dict, integrations: Dict
    ) -> str:
...
"""
        ...
        successful_tests = sum(
            1 for result in test_results.values() if result.get("syntax_check", False)
        )
        ...
        successful_integrations = sum(
            1
            for result in integrations.values()
...
def main():
    ...
    for file_path in all_files:
        ...
        integrations[file_path] = integrator.integrate_file(
            file_path, target_dir, analysis
        )
    ...
    test_report = integrator.generate_integration_report(
        inventory, analyses, test_results, integrations
    )
    ...
    mapping_path = (
        integrator.project_root / "logs/recovered_code_integration_mapping.json"
    )
    mapping_data = {
    ...
    successful_integrations = sum(
        1 for result in integrations.values() if result.get("status") == "integrated"
    )
    successful_tests = sum(
        1 for result in test_results.values() if result.get("syntax_check", False)
    )
    oracle_files = sum(
        1 for analysis in analyses.values() if analysis.get("oracle_related", False)
    )
