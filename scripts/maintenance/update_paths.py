...
IMPORT_PATHS_PATTERN = (
    r"from\s+argumentation_analysis\.paths\s+import\s+([^\n]+)"  # MODIFIÉ
)
...
def update_paths_in_directory(
    directory, extensions=None, exclude_dirs=None, dry_run=True
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Met à jour les références aux chemins dans les fichiers Python"
    )
    parser.add_argument(
        "--dir",
        type=str,
