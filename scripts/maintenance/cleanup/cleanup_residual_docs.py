...
# -*- coding: utf-8 -*-
"""
Script de nettoyage des fichiers de documentation résiduels

Ce script permet de:
1. Sauvegarder les fichiers de documentation résiduels dans un répertoire d'archives
...
Options:
    ...
    --list: Liste les fichiers résiduels sans effectuer d'actions
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("cleanup_residual_docs.log"),
    ],
)
logger = logging.getLogger(__name__)

# Liste des fichiers résiduels à supprimer
RESIDUAL_FILES = [
    # Rapports et documentations résiduels
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Script de nettoyage des fichiers de documentation résiduels"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="Liste les fichiers résiduels sans effectuer d'actions",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def backup_files(
    files: List[str], archive_path: str, dry_run: bool = False
...
def save_metadata(
    metadata: Dict[str, Dict[str, Any]], archive_path: str, dry_run: bool = False
) -> str:
    """
...
def verify_backup(metadata: Dict[str, Dict[str, Any]], dry_run: bool = False) -> bool:
    ...
    for file_path, file_meta in metadata.items():
        ...
        if archive_hash != file_meta["original_hash"]:
            ...
            logger.error(
                f"  Attendu: {file_meta['original_hash']}, Obtenu: {archive_hash}"
            )
            return False
...
def generate_report(
    ...
    report_content = f"""# Rapport de suppression des fichiers de documentation résiduels
...
```
python scripts/cleanup/cleanup_residual_docs.py --restore --archive-dir={archive_path}
```
"""
...
def restore_files(archive_dir: Optional[str] = None, dry_run: bool = False) -> bool:
    ...
    for file_path, file_meta in metadata.items():
        ...
        if not dry_run:
            ...
            logger.info(
                f"[DRY-RUN] Fichier qui serait restauré: {archive_path} -> {file_path}"
            )
...
def list_residual_files() -> None:
    """
    Liste les fichiers résiduels et leur statut.
    """
    logger.info("Liste des fichiers de documentation résiduels:")
...
def main() -> int:
    ...
    if not args.no_backup:
        ...
        try:
            metadata = backup_files(RESIDUAL_FILES, archive_path, dry_run=args.dry_run)
            ...
            if not args.dry_run:
                ...
                if not backup_valid and not args.force:
                    logger.error(
                        "La sauvegarde n'est pas valide. Utilisez --force pour forcer la suppression."
                    )
                    return 1
        ...
        logger.warning(
            "Mode sans sauvegarde activé. Les fichiers seront supprimés définitivement."
        )
        archive_path = None
        metadata = {}

    # Suppression des fichiers
    deleted_files = delete_files(RESIDUAL_FILES, dry_run=args.dry_run)
