...
class ReviewFileRefactorer:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if line.startswith("import ") or line.startswith("from "):
                try:
                    ...
                    if (
                        "oracle" in line.lower()
                        or "sherlock" in line.lower()
                        or "watson" in line.lower()
                    ):
                        ...
                        std_lib in line
                        for std_lib in ["os", "sys", "json", "datetime", "pathlib"]
                    ):
    ...
    def _find_oracle_references(self, content: str) -> List[Tuple[int, str]]:
        ...
        oracle_terms = [
            ...
            "Watson",
            "Moriarty",
            "dataset_access_manager",
            "moriarty_interrogator",
            "oracle_enhanced",
            "v2.1.0",
        ]
    ...
    def refactor_file(self, file_path: str, analysis: Dict) -> Dict:
        ...
        try:
            ...
            if analysis["oracle_references"]:
                refactored_content = self._update_oracle_imports(
            ...
            if line.strip().startswith(("import ", "from ")):
                if line.strip() not in seen_imports:
                    ...
                    result["changes_made"].append(
                        f"Supprimé import dupliqué: {line.strip()}"
                    )
            else:
                cleaned_lines.append(line)
    ...
    def test_refactored_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            test_result["overall_success"] = all(
                [test_result["syntax_valid"], test_result["import_test"]]
            )
    ...
    def make_final_decision(
        self, file_path: str, analysis: Dict, refactor_result: Dict, test_result: Dict
    ) -> str:
    ...
    def run_refactoring(self):
        ...
        for file_path in self.files_to_review:
            ...
            try:
                ...
                # 2. Refactorisation
                file_report["refactoring"] = self.refactor_file(
                    file_path, file_report["analysis"]
                )
                ...
                report["summary"]["decisions"][file_report["final_decision"]] = (
                    report["summary"]["decisions"].get(file_report["final_decision"], 0)
                    + 1
                )

                logger.info(
                    f"Décision pour {file_path}: {file_report['final_decision']}"
                )
    ...
    def _save_reports(self, report: Dict):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Refactorisations réussies**: {report['summary']['successful_refactoring']}\n"
            )
            ...
            for file_report in report["files_processed"]:
                ...
                if file_report.get("analysis"):
                    ...
                    f.write(
                        f"- Syntaxe valide: {analysis.get('syntax_valid', False)}\n"
                    )
                    f.write(f"- Complexité: {analysis.get('complexity_score', 0)}\n")
                    f.write(
                        f"- Références Oracle: {len(analysis.get('oracle_references', []))}\n"
                    )
