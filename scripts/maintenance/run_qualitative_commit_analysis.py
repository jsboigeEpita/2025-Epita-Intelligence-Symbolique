    ...
    async with semaphore:
        ...
        try:
            ...
            if analysis and not args.force:
                if analysis.get("status") == "skipped_no_diff":
                    logging.info(f"[{filename}] Previously skipped (no diff). Skipping.")
            ...
            for file_change in tqdm(files_to_process, desc=f"Analyzing files in {filename}", leave=False):
                if isinstance(file_change, str):
                    logging.warning(f"[{filename}] file_change is a string, not a dict. Assuming it's the file_path: {file_change}")
                    file_path = file_change
                    diff_content = "" # No diff content available in this case
                else:
                    file_path = file_change.get("filename")
                    if not file_path:
                        logging.warning(f"[{filename}] No file_path found in a file_change entry. Entry was: {file_change}. Skipping.")
                        continue
                ...
                try:
                    ...
                    # Stratégie de fractionnement (chunking) pour les diffs volumineux
                    # Estimation simple : 1 token ~ 4 caractères. On vise 120k tokens max.
                    MAX_CHUNK_SIZE = 120000 * 3
                    
                    if len(diff_content) > MAX_CHUNK_SIZE:
                        logging.warning(f"[{filename}/{file_path}] Diff content is very large ({len(diff_content)} chars). Splitting into chunks.")
                        diff_chunks = [diff_content[i:i+MAX_CHUNK_SIZE] for i in range(0, len(diff_content), MAX_CHUNK_SIZE)]
                    else:
                        diff_chunks = [diff_content]

                    llm_start_time = time.monotonic()
                    
                    # Traiter chaque morceau et agréger les résultats
                    file_analysis_chunks = []
                    for i, chunk in enumerate(diff_chunks):
                        logging.info(f"[{filename}/{file_path}] Processing chunk {i+1}/{len(diff_chunks)}")
                        try:
                            result = await kernel.invoke_prompt(
                                prompt,
                                arguments=KernelArguments(input=chunk),
                                settings=execution_settings
                            )
                            json_str_chunk = extract_json_from_markdown(str(result))
                            if json_str_chunk:
                                file_analysis_chunks.append(CommitAnalysis.model_validate_json(json_str_chunk))
                            else:
                                logging.error(f"[{filename}/{file_path}] Could not extract JSON from chunk {i+1} output.")
                        except Exception as chunk_e:
                            logging.error(f"[{filename}/{file_path}] Error processing chunk {i+1}: {chunk_e}")
                            continue
                    
                    # Agréger les résultats des morceaux
                    if not file_analysis_chunks:
                         raise ValueError("No analysis could be generated from any of the chunks.")
                    
                    # Pour cet exemple, on prend le premier résumé et on concatène les listes
                    final_detailed_summary = file_analysis_chunks[0].detailed_summary
                    final_debt_signals = [s for chunk_analysis in file_analysis_chunks for s in chunk_analysis.technical_debt_signals]
                    final_quality_leaps = [l for chunk_analysis in file_analysis_chunks for l in chunk_analysis.quality_leaps]

                    file_analysis = CommitAnalysis(
                        detailed_summary=final_detailed_summary,
                        technical_debt_signals=final_debt_signals,
                        quality_leaps=final_quality_leaps
                    )
                    
                    # On place le résultat en dehors du bloc try/except pour éviter la double capture
                    # et on utilise un dictionnaire pour le résultat final agrégé
                    raw_output = file_analysis.model_dump_json()
                    json_str = raw_output
            ...
            if aggregated_analysis["detailed_summary"]:
                ...
                logging.warning(f"[{filename}] No analysis was generated (likely no diffs). Marking as skipped and saving.")
                commit_data["qualitative_analysis"] = {"status": "skipped_no_diff"}
                output_file_path = os.path.join(args.commits_dir, filename)
                with open(output_file_path, 'w', encoding='utf-8') as f:
                    json.dump(commit_data, f, indent=4)
...
async def main():
    ...
    parser.add_argument("--force", action='store_true', help="Force overwrite of existing analysis.")
