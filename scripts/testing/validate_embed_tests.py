...
def setup_environment():
    ...
    try:
        ...
        print(
            f"Configuration du mock: {'[OK] Reussi' if setup_result else '[ERREUR] Echec'}"
        )
        return setup_result
...
def create_test_file(tmp_path, filename, data, passphrase):
    ...
    try:
        ...
        file_operations.save_extract_definitions(
            data, input_file, passphrase, embed_full_text=should_embed, config={}
        )
        return input_file
    except Exception as e:
...
def load_encrypted_file(file_path, passphrase):
    ...
    try:
        ...
        loaded_data = file_operations.load_extract_definitions(
            config_file=file_path, key=passphrase
        )
        return loaded_data
    except Exception as e:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            input_file = create_test_file(
                tmp_path, "input.enc", test_data, test_passphrase
            )
            if not input_file:
                return False
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            success = (
                ...
                and output_file.read_text() != "existing_content"
            )
...
def test_no_force_existing_file():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            success = (
                result.returncode == 1 and output_file.read_text() == "existing_content"
            )
...
def main():
    ...
    if passed_tests == len(tests):
        ...
        print(
            "[OK] La logique de chiffrement Fernet est harmonisee entre le mock et le script"
        )
        print(
            "[OK] Les fonctionnalites principales du script fonctionnent correctement"
        )
    else:
