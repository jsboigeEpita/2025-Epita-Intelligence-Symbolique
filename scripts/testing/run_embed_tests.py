...
def run_tests():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            tests_to_run = [
                (
                    "test_embed_script_success_no_text_initially",
                    test_embed_script_success_no_text_initially,
                ),
                (
                    "test_embed_script_text_already_present",
                    test_embed_script_text_already_present,
                ),
                (
                    "test_embed_script_force_overwrite",
                    test_embed_script_force_overwrite,
                ),
                (
                    "test_embed_script_output_exists_no_force",
                    test_embed_script_output_exists_no_force,
                ),
                (
                    "test_embed_script_passphrase_from_env",
                    test_embed_script_passphrase_from_env,
                ),
                (
                    "test_embed_script_missing_passphrase",
                    test_embed_script_missing_passphrase,
                ),
                (
                    "test_embed_script_input_file_not_found",
                    test_embed_script_input_file_not_found,
                ),
                (
                    "test_embed_script_incorrect_passphrase",
                    test_embed_script_incorrect_passphrase,
                ),
                (
                    "test_embed_script_source_fetch_error",
                    test_embed_script_source_fetch_error,
                ),
                (
                    "test_embed_script_empty_input_config",
                    test_embed_script_empty_input_config,
                ),
            ]
            ...
            for test_name, test_func in tests_to_run:
                ...
                try:
                    ...
                    if test_name == "test_embed_script_success_no_text_initially":
                        ...
                        with patch(
                            "argumentation_analysis.ui.utils.get_full_text_for_source"
                        ) as mock_get_text, patch(
                            "argumentation_analysis.ui.utils.load_from_cache",
                            return_value=None,
                        ), patch(
                            "argumentation_analysis.ui.utils.save_to_cache",
                            return_value=None,
                        ), patch(
                            "argumentation_analysis.ui.config.load_extract_sources"
                        ) as mock_load_extract_sources:
                            ...
                            mock_app_config_instance.get.side_effect = (
                                app_config_get_side_effect
                            )
                            mock_load_extract_sources.return_value = (
                                mock_app_config_instance
                            )

                            test_func(
                                tmp_path,
                                create_encrypted_config_file_func,
                                minimal_config_data_no_text_val,
                                test_passphrase_val,
                                (mock_get_text, mock_load_extract_sources),
                            )
