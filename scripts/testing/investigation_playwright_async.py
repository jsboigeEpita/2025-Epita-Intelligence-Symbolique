...
class NonBlockingPlaywrightInvestigator:
    def __init__(self, mode: str = "investigation"):
        ...
        self.report_file = self.temp_dir / f"investigation_async_report_{self.timestamp}.json"
    ...
    async def check_service(self, session: aiohttp.ClientSession, name: str, config: Dict) -> Dict[str, Any]:
    ...
    def start_service_if_needed(self, name: str, port: int, command: str) -> Optional[subprocess.Popen]:
    ...
    async def run_test_async(self, command: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"{status_emoji} Test terminé: {result['status']} ({duration:.2f}s)")
    ...
    async def generate_report(self):
        ...
        self.results["tests"]["success"] = sum(
            1 for r in self.results["tests"]["results"] 
            if r.get("status") == "SUCCESS"
        )
        
        if self.results["tests"]["total"] > 0:
            self.results["tests"]["success_rate"] = round(
                (self.results["tests"]["success"] / self.results["tests"]["total"]) * 100, 2
            )
        else:
    ...
    def display_summary(self):
        ...
        print(f"🧪 Tests: {self.results['tests']['success']}/{self.results['tests']['total']} réussis ({self.results['tests']['success_rate']}%)")
        
        for name, status in self.results["services"].items():
            emoji = {"OK": "✅", "WARNING": "⚠️", "ERROR": "❌"}.get(status.get("status", "ERROR"), "❓")
...
async def main():
    ...
    parser.add_argument("--mode", choices=["complet", "investigation", "rapide"], 
                       default="investigation", help="Mode d'exécution")
    parser.add_argument("--report", action="store_true", help="Ouvrir le rapport après exécution")
