    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            file_operations.save_extract_definitions(
                test_data, input_file, test_passphrase, embed_full_text=False, config={}
            )

            print(
                f"Fichier d'entree cree: {'[OK] Oui' if input_file.exists() else '[ERREUR] Non'}"
            )
            ...
            print(
                f"Fichier de sortie cree: {'[OK] Oui' if output_file.exists() else '[ERREUR] Non'}"
            )

            if output_file.exists():
                # Vérifier le contenu du fichier de sortie
                output_data = file_operations.load_extract_definitions(
                    config_file=output_file, key=test_passphrase
                )
                print(
                    f"Donnees de sortie: {len(output_data) if output_data else 0} elements"
                )
                if output_data and len(output_data) > 0:
                    has_full_text = "full_text" in output_data[0]
                    print(
                        f"Texte complet ajoute: {'[OK] Oui' if has_full_text else '[ERREUR] Non'}"
                    )
...
def main():
    ...
    print(
        f"Test execution script: {'[OK] REUSSI' if test2_success else '[ERREUR] ECHEC'}"
    )

    overall_success = test1_success and test2_success
    print(
        f"Resultat global: {'[OK] TOUS LES TESTS REUSSIS' if overall_success else '[ERREUR] CERTAINS TESTS ONT ECHOUE'}"
    )
