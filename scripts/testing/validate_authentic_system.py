...
def main():
    """Fonction principale."""
    parser = argparse.ArgumentParser(description="Outil de validation de l'authenticité du système.")
    parser.add_argument("--config", type=str, default="testing", help="Nom de la configuration a utiliser (e.g., testing, development).")
    parser.add_argument("--check-all", action="store_true", help="Verifier tous les composants.")
    parser.add_argument("--require-100-percent", action="store_true", help="Exiger 100%% d'authenticite.")
    parser.add_argument("--output", type=str, choices=['json', 'text'], default='text', help="Format de sortie.")
    ...
    if args.config not in valid_configs:
        print(f"Erreur : Configuration invalide '{args.config}'. Les configurations valides sont : {valid_configs}", file=sys.stderr)
    ...
    if args.output == 'json':
        ...
        print(f"  - 100% authentique : {'Oui' if results['is_100_percent_authentic'] else 'Non'}")
