...
Start-Job -ScriptBlock { conda run -n projet-is --no-capture-output --live-stream python argumentation_analysis/services/web_api/start_api.py --port 5003 } -Name "Backend"

# Lancer le frontend en arrière-plan
Write-Host "Démarrage du serveur frontend en arrière-plan..."
Start-Job -ScriptBlock { conda run -n projet-is --no-capture-output --live-stream npm start --prefix services/web_api/interface-web-argumentative } -Name "Frontend"
...
if (-not $backend_ok) {
    Write-Error "Le serveur backend n'a pas démarré sur le port 5000."
    Get-Job | Stop-Job
    Get-Job | Remove-Job
    exit 1
}
