...
def check_file_for_mocks(filepath):
    ...
    try:
        ...
        has_anti_simulation = any(re.search(pattern, content, re.IGNORECASE) for pattern in safe_patterns)
    ...
    for i, filepath in enumerate(files_lot2, 1):
        if os.path.exists(filepath):
            ...
            if is_clean:
                ...
                all_clean = False
                print(f"   Patterns détectés: {patterns[:3]}...")  # Show first 3 patterns
            print()
        else:
            print(f"{i}. {filepath}")
