...
class PlaywrightRunner:
    ...
    def _build_playwright_command(
        ...
        cmd = [
            ...
            "-v",
            "-p",
            "no:asyncio",  # Désactive le plugin asyncio pour éviter les conflits avec Playwright
            "-p",
            "pytest_playwright",  # Force le chargement du plugin Playwright
            "--capture=no",
        ...
        self.logger.info(
            f" chemins de test absolus (format POSIX) : {absolute_test_paths}"
        )
        ...
        if not config.get("headless", True):
            cmd.append("--headed")
        self.logger.info(
            f"Le mode Headless est configuré sur: {config.get('headless', True)}"
        )
    ...
    async def _execute_tests(
        ...
        self.logger.info(
            f"Exécution de la commande de test via run_in_activated_env_async: {' '.join(cmd)}"
        )
        ...
        try:
            ...
            stdout_task = asyncio.create_task(
                log_stream(process.stdout, self.logger.info, stdout_lines)
            )
            stderr_task = asyncio.create_task(
                log_stream(process.stderr, self.logger.warning, stderr_lines)
            )
            ...
            self.logger.info(
                f"Tests pytest terminés avec le code de retour : {process.returncode}"
            )

        except asyncio.TimeoutError:
            self.logger.error(
                f"Timeout ({timeout}s) atteint pendant l'exécution des tests."
            )
            ...
            self.logger.critical(
                f"Erreur ShellCommandError lors de l'exécution : {e}", exc_info=True
            )
            return subprocess.CompletedProcess(
                " ".join(cmd), returncode=e.returncode, stdout=e.stdout, stderr=e.stderr
            )
        except Exception as e:
            self.logger.critical(
                f"Erreur critique lors de l'exécution du sous-processus de test : {e}",
                exc_info=True,
            )
            return subprocess.CompletedProcess(
                " ".join(cmd),
    ...
    async def _analyze_results(self, result: subprocess.CompletedProcess) -> bool:
        ...
        if success:
            ...
            if stderr_content:
                ...
                self.logger.error(
                    "Voir les logs [PYTEST_STDERR] ci-dessus pour les détails complets."
                )
                self.logger.error("--- Fin des erreurs ---")
            else:
                self.logger.error(
                    "Aucune sortie d'erreur (stderr) n'a été capturée, mais le code de retour indique un échec."
                )
    ...
    def _extract_pytest_stats(self, stdout_lines: List[str]) -> Dict[str, Any]:
        ...
        stats = {
            "detail": "Parsing de sortie non effectué. Se baser sur success et return_code."
        }
        return stats
