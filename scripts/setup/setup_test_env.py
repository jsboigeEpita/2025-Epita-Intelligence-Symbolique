...
try:
    ...
    from argumentation_analysis.pipelines.setup_pipeline import (
        run_test_environment_setup_pipeline,
    )
except ImportError as e:
    print(
        f"Erreur d'import: {e}. Assurez-vous que le PYTHONPATH est correctement configuré ou que le script est exécuté depuis la racine du projet."
    )
    sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        / "requirements.txt",  # Ou un fichier requirements-test.txt spécifique
        help="Chemin vers le fichier requirements.txt.",
    )
    parser.add_argument(
        "--mock-jpype", action="store_true", help="Activer le mock pour JPype."
    )
    parser.add_argument(
        "--venv-path",
        type=Path,
    ...
    if not args.requirements_path.exists():
        logger.error(
            f"Le fichier requirements spécifié n'existe pas: {args.requirements_path}"
        )
        sys.exit(1)
    ...
    if args.config_path and not args.config_path.exists():
        logger.warning(
            f"Le fichier de configuration spécifié n'existe pas: {args.config_path}. Il sera ignoré."
        )
    ...
    if success:
        logger.info(
            "Configuration de l'environnement de test terminée avec succès via le pipeline."
        )
        ...
        logger.error(
            "La configuration de l'environnement de test via le pipeline a échoué."
        )
        sys.exit(1)
