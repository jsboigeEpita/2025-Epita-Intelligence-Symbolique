...
PATTERNS = [
    ...
    (
        r"jpype\.startJVM\((.*?)\)",
        r"# pyjnius initializes JVM automatically\n# Original: jpype.startJVM(\1)",
    ),
    (
        r"jp\.startJVM\((.*?)\)",
    ...
    (
        r"jpype\.shutdownJVM\(\)",
        r"# pyjnius handles JVM shutdown automatically\n# Original: jpype.shutdownJVM()",
    ),
    (
        r"jp\.shutdownJVM\(\)",
...
"""
        ...
        if not os.path.exists(init_file):
            with open(init_file, "w", encoding="utf-8") as f:
                f.write(
                    "# Ce répertoire contient des modules mock pour la compatibilité\n"
                )
...
def update_conftest():
    ...
    try:
        ...
        if "init_jpype_compatibility" in content:
            logger.info(
                f"Le fichier {conftest_file} contient déjà le code d'initialisation"
            )
            return False
...
"""
        ...
        if import_pos == -1:
            ...
            modified_content = (
                content[:end_of_imports] + import_code + content[end_of_imports:]
            )
...
def main():
    ...
    readme_file = os.path.join(
        PROJECT_ROOT, "scripts", "setup", "README_JPYPE_TO_PYJNIUS.md"
    )
