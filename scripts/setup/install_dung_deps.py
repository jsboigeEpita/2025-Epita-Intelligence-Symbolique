...
try:
    ...
    print(f"[INSTALL SCRIPT] ERREUR CRITIQUE lors de l'activation de l'environnement: {e}")
    sys.exit(1)
...
update_command = [sys.executable, "-m", "pip", "install", "--upgrade", "pip", "setuptools", "wheel"]
update_process = subprocess.run(update_command, env=env_vars, capture_output=True, text=True, check=False, encoding='utf-8')
