...
try:
    ...
    from argumentation_analysis.utils.core_utils.logging_utils import (
        setup_logging,
    )  # Pour configurer le logging centralisé
...
def main():
    ...
    if args.target_dir:
        ...
        test_resources_dir = (
            project_root_dir / "argumentation_analysis" / "tests" / "resources" / "libs"
        )
    ...
    if pipeline_successful:
        ...
        logger.info(
            "✅ Le pipeline de téléchargement des JARs s'est terminé avec succès."
        )
...
"""
        ...
        if not readme_file.exists() or args.force:  # Recréer si forcé ou non existant
            ...
            logger.info(
                f"Fichier README.md existe déjà dans {test_resources_dir}. Non modifié (utiliser --force pour écraser)."
            )
