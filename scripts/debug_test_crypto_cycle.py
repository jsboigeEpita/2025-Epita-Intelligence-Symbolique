...
try:
    ...
    if project_root_assumed not in sys.path:
        ...
        print("Le chemin racine présumé du projet est déjà dans PYTHONPATH, mais l'import a échoué.")
        print(f"PYTHONPATH actuel: {sys.path}")
        sys.exit(1)
...
def test_encryption_decryption_cycle():
    ...
    try:
        ...
        print(f"Données chiffrées (base64): {base64.b64encode(encrypted_bytes_val).decode('utf-8')}")
...
if __name__ == "__main__":
    ...
    if decrypted_result is not None:
        if isinstance(decrypted_result, str):
            ...
            try:
                ...
                final_decrypted_str = f"Impossible de décoder les bytes: {decrypted_result!r}"
    ...
    if encrypted:
        print(f"Données chiffrées (b64) : {base64.b64encode(encrypted).decode('utf-8') if isinstance(encrypted, bytes) else 'N/A (non byte)'}")
    else:
