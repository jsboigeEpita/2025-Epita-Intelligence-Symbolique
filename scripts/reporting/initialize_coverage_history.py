...
def create_initial_history(coverage_data, history_file):
    ...
    previous_date = (datetime.datetime.now() - datetime.timedelta(days=30)).strftime(
        "%Y-%m-%d"
    )
...
def main():
    ...
    if coverage_data:
        ...
        history_file_path = Path(
            history_file
        )  # Convertir en Path pour la nouvelle fonction
        if create_initial_coverage_history(
            coverage_data, history_file_path
        ):  # Utiliser la fonction importée
            print("Historique de couverture initialisé avec succès.")
        else:
            print(
                f"Échec de l'initialisation de l'historique de couverture dans {history_file_path}."
            )
        print(
            "Vous pouvez maintenant exécuter le script visualize_test_coverage.py pour générer les graphiques."
        )
    else:
