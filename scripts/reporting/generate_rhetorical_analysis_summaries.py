...
from datetime import (
    datetime,
)  # Conservé pour le timestamp dans main (si encore utilisé)
...
def main():
    ...
    logger.info(
        "Démarrage de la génération des synthèses d'analyses rhétoriques via le pipeline..."
    )
    ...
    logger.info(
        f"Pipeline de génération de synthèses d'analyses rhétoriques terminé. Vérifiez le répertoire : {output_dir}"
    )
