...
sys.stdout = io.TextIOWrapper(
    sys.stdout.buffer, encoding=_stdout_encoding, errors="replace", line_buffering=True
)

_stderr_encoding = sys.stderr.encoding if sys.stderr.encoding else "utf-8"
sys.stderr = io.TextIOWrapper(
    sys.stderr.buffer, encoding=_stderr_encoding, errors="replace", line_buffering=True
)
...
logger = logging.getLogger(
    __name__
)  # Utilisation de __name__ pour le logger du script lanceur
...
# from argumentation_analysis.core.utils.file_utils import load_json_file, load_text_file, load_csv_file, save_markdown_to_html # Utilisé par le pipeline
# from argumentation_analysis.core.utils.reporting_utils import generate_markdown_report_for_corpus, generate_overall_summary_markdown # Utilisé par le pipeline
from argumentation_analysis.utils.data_processing_utils import (
    group_results_by_corpus,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Génère un rapport d'analyse complet qui synthétise tous les résultats des tests précédents"
    )
    ...
    try:
        ...
        if not base_results_file:
            ...
            if results_dir.exists():
                result_files = sorted(
                    list(results_dir.glob("rhetorical_analysis_*.json")),
                ...
                if result_files:
                    ...
                    logger.info(
                        f"Utilisation du fichier de résultats de base le plus récent: {base_results_file}"
                    )
        ...
        if not advanced_results_file:
            ...
            if results_dir.exists():
                result_files = sorted(
                    list(results_dir.glob("advanced_rhetorical_analysis_*.json")),
                    key=lambda x: x.stat().st_mtime,
                    reverse=True,
                )
                if result_files:
                    ...
                    logger.info(
                        f"Utilisation du fichier de résultats avancés le plus récent: {advanced_results_file}"
                    )
        ...
        if not performance_report_file:
            ...
            if performance_dir.exists():
                report_files = list(
                    performance_dir.glob("rapport_performance.md")
                )  # Devrait en trouver un au plus
                if report_files:  # Prend le premier trouvé s'il y en a
                    performance_report_file = str(report_files[0])
                    logger.info(
                        f"Utilisation du fichier de rapport de performance: {performance_report_file}"
                    )
            if not performance_report_file:  # Si toujours pas trouvé
                logger.info(
                    "Aucun fichier de rapport de performance trouvé par défaut."
                )
        ...
        if not performance_metrics_file:
            ...
            if performance_dir.exists():
                metrics_files = list(
                    performance_dir.glob("performance_metrics.csv")
                )  # Devrait en trouver un au plus
                if metrics_files:  # Prend le premier trouvé
                    performance_metrics_file = str(metrics_files[0])
                    logger.info(
                        f"Utilisation du fichier de métriques de performance: {performance_metrics_file}"
                    )
            if not performance_metrics_file:  # Si toujours pas trouvé
                logger.info(
                    "Aucun fichier de métriques de performance trouvé par défaut."
                )
