...
def main():
    ...
    if not base_results_file:
        logger.error(
            "Aucun fichier de résultats de base spécifié et aucun fichier de résultats trouvé."
        )
        sys.exit(1)
    ...
    if not advanced_results_file:
        logger.error(
            "Aucun fichier de résultats avancés spécifié et aucun fichier de résultats trouvé."
        )
        sys.exit(1)
    ...
    base_results_data = load_json_file(
        base_results_path
    )  # MODIFIÉ: Appel de la fonction importée
    advanced_results_data = load_json_file(
    ...
    if isinstance(base_results_data, list):
        ...
        logger.warning(
            f"Les résultats de base chargés depuis {base_results_path} ne sont pas une liste, mais de type {type(base_results_data)}. Traitement comme liste vide."
        )
    ...
    if isinstance(advanced_results_data, list):
        ...
        logger.warning(
            f"Les résultats avancés chargés depuis {advanced_results_path} ne sont pas une liste, mais de type {type(advanced_results_data)}. Traitement comme liste vide."
        )
    ...
    generate_performance_comparison_markdown_report(
        base_metrics, advanced_metrics, report_file
    )

    logger.info(
        f"✅ Comparaison des performances terminée. Résultats sauvegardés dans {output_dir}"
    )
