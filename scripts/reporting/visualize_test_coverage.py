...
def generate_module_comparison_chart(history_file, output_dir):
    ...
    module_avg_coverage = {
        module: sum(coverages) / len(coverages)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Visualiser l'évolution de la couverture des tests."
    )
    parser.add_argument(
        "--xml",
        type=str,
        default="argumentation_analysis/tests/coverage.xml",
        help="Chemin vers le fichier coverage.xml",
    )
    parser.add_argument(
        "--history",
        type=str,
        default="results/coverage_history.json",
    ...
    if coverage_data:
        ...
        if not args.update_only:
            ...
            generate_coverage_trend_chart(
                history_file_path, output_dir_path, module_colors=COLORS
            )
            generate_module_comparison_chart(
                history_file_path, output_dir_path, module_colors=COLORS
            )
            generate_coverage_improvement_chart(
                history_file_path, output_dir_path, module_colors=COLORS
            )
    else:
