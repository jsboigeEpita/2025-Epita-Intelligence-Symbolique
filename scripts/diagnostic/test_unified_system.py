...
async def test_unified_pipeline():
    ...
    try:
        ...
        text = "L'intelligence artificielle transforme notre société de manière profonde."
        result = await pipeline.analyze_text_unified(text)
        
        print(f"[OK] Pipeline unifié - Analyse terminée")
        print(f"     Modes: {', '.join(result['metadata']['analysis_config']['analysis_modes'])}")
        print(f"     Temps: {result['metadata']['processing_time_ms']:.1f}ms")
        print(f"     Fallacies détectés: {len(result.get('informal_analysis', {}).get('fallacies', []))}")
...
async def test_real_llm_orchestrator():
    ...
    try:
        ...
        print(f"     Messages: {len(result.get('conversation_log', {}).get('messages', []))}")
...
async def main():
    ...
    if passed == total:
        print("[SUCCESS] Tous les composants du système d'analyse rhétorique fonctionnent!")
        return 0
    else:
