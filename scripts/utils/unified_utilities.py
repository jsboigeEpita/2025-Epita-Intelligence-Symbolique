...
class UnifiedCorpusManager:
    ...
    def _get_passphrase(self) -> str:
        ...
        passphrase = (
            self.config.passphrase or 
            os.getenv("TEXT_CONFIG_PASSPHRASE") or 
            "Propaganda"  # Fallback par défaut
        )
    ...
    def _load_crypto_utils(self):
        ...
        try:
            from argumentation_analysis.utils.core_utils.crypto_utils import (
                load_encryption_key, decrypt_data_with_fernet, encrypt_data_with_fernet
            )
            return load_encryption_key, decrypt_data_with_fernet, encrypt_data_with_fernet
        except ImportError:
            self.logger.warning("Modules de chiffrement non disponibles - utilisation de mocks")
    ...
    def save_corpus(self, corpus_data: List[Dict[str, Any]], backup: bool = None) -> bool:
        """
    ...
    def get_corpus_info(self) -> CorpusInfo:
        ...
        for i, source in enumerate(corpus_data):
            ...
            source_content_length = sum(len(extract.get('full_text', '')) for extract in extracts)
        ...
        file_size = self.encrypted_file_path.stat().st_size if self.encrypted_file_path.exists() else 0
        last_modified = datetime.fromtimestamp(
            self.encrypted_file_path.stat().st_mtime
        ).isoformat() if self.encrypted_file_path.exists() else "N/A"
        ...
        return CorpusInfo(
            ...
            encryption_status="encrypted" if self.encrypted_file_path.exists() else "not_found",
    ...
    def list_extracts(self) -> Dict[str, Any]:
        ...
        for source_idx, source in enumerate(corpus_data):
            ...
            for extract_idx, extract in enumerate(extracts):
                ...
                extract_meta = {
                    ...
                    "extract_name": extract.get("extract_name", f"Extrait_{extract_idx}"),
                    "start_marker": extract.get("start_marker", ""),
                    "end_marker": extract.get("end_marker", ""),
                    "has_content": "full_text" in extract and bool(extract.get("full_text")),
                    "content_length": len(extract.get("full_text", "")) if extract.get("full_text") else 0,
        ...
        metadata["total_extracts"] = sum(src["extract_count"] for src in metadata["sources"])
        
        self.logger.info(f"Métadonnées extraites: {metadata['total_sources']} sources, {metadata['total_extracts']} extraits")
        
        return metadata

    def find_extract_by_id(self, extract_id: str) -> Optional[Tuple[Dict, Dict, int, int]]:
        """
        ...
        try:
            ...
            self.logger.error(f"Format d'ID invalide: {extract_id}. Utilisez le format 'source_extract' (ex: '0_1')")
            return None
    ...
    def find_extract_by_name(self, extract_name: str) -> Optional[Tuple[Dict, Dict, int, int]]:
        """
    ...
    def extract_specific_content(self, extract_id: Optional[str] = None, extract_name: Optional[str] = None) -> Optional[ExtractInfo]:
        """
        ...
        self.logger.info(f"Longueur du contenu: {extract_info.content_length} caractères")
        
        return extract_info

    def integrate_new_source(self, source_file_path: str, validate: bool = None) -> bool:
        """
        ...
        try:
            ...
            total_extracts = sum(len(source.get('extracts', [])) for source in updated_corpus)
            new_extracts = len(new_source.get('extracts', []))
            
            self.logger.info(f"Corpus mis à jour: {total_sources} sources, {total_extracts} extraits (+{new_extracts} nouveaux)")
    ...
    def _load_new_source(self, source_file_path: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"Nouvelle source chargée: {len(new_source_data.get('extracts', []))} extraits")
    ...
    def _validate_integration(self, new_source: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if extract_count == expected_count:
                self.logger.info(f"Validation réussie - Source: '{last_source_name}' avec {extract_count} extraits")
                return True
            else:
                self.logger.error(f"Validation échouée - Attendu: {expected_count}, Trouvé: {extract_count}")
                return False
    ...
    def validate_corpus_integrity(self) -> Dict[str, Any]:
                ...
                if isinstance(corpus_data, list):
                    ...
                    for i, source in enumerate(corpus_data):
                        if not isinstance(source, dict):
                            corrupted_sources += 1
                            validation_results["warnings"].append(f"Source {i} n'est pas un dictionnaire")
                            continue
                        
                        extracts = source.get('extracts', [])
                        if not isinstance(extracts, list):
                            corrupted_sources += 1
                            validation_results["warnings"].append(f"Source {i}: extracts n'est pas une liste")
                            continue
                        ...
                        for j, extract in enumerate(extracts):
                            if not isinstance(extract, dict):
                                validation_results["warnings"].append(f"Source {i}, extrait {j}: format invalide")
                            elif not extract.get('full_text'):
                                validation_results["warnings"].append(f"Source {i}, extrait {j}: contenu vide")
                    
                    validation_results["content_stats"] = {
                        "total_sources": total_sources,
                        "total_extracts": total_extracts,
                        "corrupted_sources": corrupted_sources,
                        "integrity_score": (total_sources - corrupted_sources) / total_sources if total_sources > 0 else 0
                    }
                    
                else:
                    validation_results["structure_valid"] = False
                    validation_results["errors"].append("Structure racine n'est pas une liste")
        ...
        validation_results["overall_valid"] = (
            ...
            validation_results["decryption_success"] and
            validation_results["json_valid"] and
    ...
    def _optimize_corpus_structure(self, corpus_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for pattern in temp_patterns:
                for temp_file in pattern:
                    if temp_file.stat().st_mtime < (datetime.now().timestamp() - 86400):  # Plus de 24h
                        temp_file.unlink()
                        cleaned_count += 1
            
            self.logger.info(f"Nettoyage: {cleaned_count} fichiers temporaires supprimés")
            return True
...
class UnifiedUtilityFormatter:
    ...
    def format_corpus_info(info: CorpusInfo, format_type: OutputFormat) -> str:
        ...
        if format_type == OutputFormat.JSON:
            ...
            output.append(f"Longueur totale du contenu: {info.total_content_length:,} caractères")
            ...
            for source in info.sources_summary:
                output.append(f"[{source['index']}] {source['name']} ({source['type']})")
                output.append(f"    Extraits: {source['extract_count']}, Contenu: {source['content_length']:,} caractères")
            ...
            for source in info.sources_summary:
                writer.writerow([
                    source['index'], source['name'], source['type'],
                    source['extract_count'], source['content_length']
                ])
...
"""
    ...
    @staticmethod
    def format_extract_list(metadata: Dict[str, Any], format_type: OutputFormat, detailed: bool = False) -> str:
        ...
        if format_type == OutputFormat.JSON:
            ...
            for source in metadata["sources"]:
                ...
                output.append(f"   Type: {source['source_type']}, Schema: {source['schema']}")
                output.append(f"   Extraits: {source['extract_count']}")
                
                if detailed and source["extracts"]:
                    for extract in source["extracts"]:
                        output.append(f"   [{extract['extract_id']}] {extract['extract_name']}")
                        output.append(f"      - Debut: '{extract['start_marker'][:50]}{'...' if len(extract['start_marker']) > 50 else ''}'")
                        output.append(f"      - Fin: '{extract['end_marker'][:50]}{'...' if len(extract['end_marker']) > 50 else ''}'")
                        output.append(f"      - Contenu: {'OUI' if extract['has_content'] else 'NON'} ({extract['content_length']} caracteres)")
                elif source["extracts"]:
                    extract_ids = [extract['extract_id'] for extract in source["extracts"]]
            ...
            # En-têtes
            writer.writerow(["Source_Index", "Source_Name", "Source_Type", "Extract_ID", 
                           "Extract_Name", "Content_Length", "Has_Content"])
            
            for source in metadata["sources"]:
                for extract in source["extracts"]:
                    writer.writerow([
                        source['source_index'], source['source_name'], source['source_type'],
                        extract['extract_id'], extract['extract_name'], 
                        extract['content_length'], extract['has_content']
                    ])
    ...
    @staticmethod
    def format_extract_content(extract_info: ExtractInfo, format_type: OutputFormat, show_content: bool = True) -> str:
        ...
        if format_type == OutputFormat.JSON:
            ...
            output.append(f"- Marqueur début: '{extract_info.extract_info['start_marker']}'")
            output.append(f"- Marqueur fin: '{extract_info.extract_info['end_marker']}'")
...
def create_utility_factory(mode: str = "list", **kwargs) -> UnifiedCorpusManager:
    ...
    mode_configs = {
        "list": UtilityConfiguration(
            mode=UtilityMode.LIST,
            output_format=OutputFormat.TEXT,
            detailed=False
        ),
        "extract": UtilityConfiguration(
            mode=UtilityMode.EXTRACT,
            output_format=OutputFormat.TEXT,
            show_content=True
        ),
        ...
        "validate": UtilityConfiguration(
            mode=UtilityMode.VALIDATE,
            output_format=OutputFormat.JSON
        ),
        "maintenance": UtilityConfiguration(
            mode=UtilityMode.MAINTENANCE,
            backup_before_modify=True
        ),
        "info": UtilityConfiguration(
            mode=UtilityMode.INFO,
            output_format=OutputFormat.TEXT
        )
    }
...
Exemples:
    ...
    parser.add_argument("--mode", choices=["list", "extract", "integrate", "validate", "maintenance", "info"], 
                       default="list", help="Mode d'opération")
    parser.add_argument("--format", choices=["text", "json", "csv", "html"], default="text", help="Format de sortie")
    ...
    parser.add_argument("--show-content", action="store_true", help="Afficher le contenu textuel")
    parser.add_argument("--dry-run", action="store_true", help="Simulation sans modification")
    
    # Arguments spécifiques au mode extract
    parser.add_argument("--extract-id", help="ID de l'extrait (format: 'source_extract')")
    ...
    parser.add_argument("--no-validate", action="store_true", help="Ne pas valider après intégration")
    ...
    try:
        ...
        if args.mode == "list":
            ...
            extract_info = manager.extract_specific_content(args.extract_id, args.extract_name)
            if extract_info:
