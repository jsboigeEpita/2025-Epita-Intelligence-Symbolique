...
try:
    ...
    print(
        "Assurez-vous que le PYTHONPATH est correctement configuré ou que le projet est installé.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyse des références aux répertoires dans le code."
    )
    parser.add_argument(
        "--dir",
        type=str,
        default=str(
            Path(project_root_dir) / "src" / "argumentation_analysis"
        ),  # MODIFIÉ: Cible src/argumentation_analysis
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            project_root_dir, "results", "directory_usage_report.json"
        ),
    ...
    if not patterns_to_search:
        ...
        logger.error(
            "Aucun motif à rechercher, même après application des valeurs par défaut. Vérifier la logique de parsing_utils ou les motifs par défaut."
        )
    ...
    logger.info(
        f"Analyse des références aux motifs {list(patterns_to_search.keys())} dans {args.dir}..."
    )
    ...
    for pattern_name, data in results.items():
        ...
        logger.info(
            f"  Nombre de fichiers contenant des références: {len(data['files'])}"
        )
            ...
            for example in data["examples"]:
                ...
                try:
                    relative_file_path = os.path.relpath(
                        example["file"], project_root_dir
                    )
                except (
                    ValueError
                ):  # Peut arriver si les chemins sont sur des lecteurs différents (peu probable ici)
                    relative_file_path = example["file"]
                logger.info(
                    f"    {relative_file_path} (ligne {example['line']}): {example['content']}"
                )
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la génération du rapport JSON : {e}", exc_info=True
        )
        sys.exit(1)
