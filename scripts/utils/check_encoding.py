...
try:
    from argumentation_analysis.utils.dev_tools.encoding_utils import (
        check_project_python_files_encoding,
        logger,
    )
    import logging
...
def main():
    ...
    """
    print(
        f"Lancement de la vérification de l'encodage des fichiers Python du projet (racine: {project_root})..."
    )
    ...
    if non_utf8_files:
        print("\n---------------------------------------------------------------------")
        print(
            f"ATTENTION: {len(non_utf8_files)} fichier(s) Python ne sont pas (ou n'ont pas pu être vérifiés comme étant) encodés en UTF-8:"
        )
