...
if not script_logger.handlers:
    handler = logging.StreamHandler(
        sys.stdout
    )  # Utiliser stdout pour les messages du script
...
if not formatting_logger.handlers:
    ...
    module_formatter = logging.Formatter(
        "%(asctime)s [%(levelname)s] %(name)s (MODULE): %(message)s", datefmt="%H:%M:%S"
    )
...
def main():
    ...
    if (
        args.autopep8_args is not None
        and len(args.autopep8_args) == 1
        and args.autopep8_args[0] == ""
    ):
    ...
    success = format_python_file_with_autopep8(
        args.file_path, autopep8_args=autopep8_custom_args
    )
