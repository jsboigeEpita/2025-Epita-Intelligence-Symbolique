...
if not script_logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s [%(levelname)s] %(name)s (SCRIPT): %(message)s", datefmt="%H:%M:%S"
    )
...
encoding_logger.setLevel(
    logging.INFO
)  # Assurez-vous que le logger du module est au bon niveau
...
def main():
    parser = argparse.ArgumentParser(
        description="Corrige l'encodage d'un fichier vers UTF-8."
    )
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Activer les logs de niveau DEBUG."
    )
    ...
    success = fix_file_encoding(
        ...
        source_encodings=args.source_encodings,
    )
