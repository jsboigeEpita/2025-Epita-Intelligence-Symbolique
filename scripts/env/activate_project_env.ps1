...
if (Test-Path $envFilePath) {
            ...
            if ($parts.Length -eq 2) {
                ...
                if ($key -eq "JAVA_HOME") {
                    if ($value.StartsWith("./") -or $value.StartsWith(".\\")) {
                        $resolvedValue = (Resolve-Path (Join-Path -Path $projectRoot -ChildPath $value.Substring(2)) -ErrorAction SilentlyContinue).Path # Utiliser projectRoot
                        if ($resolvedValue) {
                            $value = $resolvedValue
                        } else {
                            Write-Warning "Impossible de résoudre le chemin relatif pour JAVA_HOME: $($parts[1].Trim()) depuis $projectRoot. Utilisation de la valeur brute."
                        }
                    }
                }
    ...
    # Mettre à jour $condaEnvName avec la valeur de .env si elle existe
    if (-not [string]::IsNullOrEmpty($env:CONDA_ENV_NAME)) {
        $condaEnvName = $env:CONDA_ENV_NAME
        Write-Host "[INFO] Utilisation de CONDA_ENV_NAME depuis .env: '$condaEnvName'"
    } else {
        Write-Warning "[WARN] CONDA_ENV_NAME n'est pas défini dans .env ou n'a pas pu être chargé. Utilisation de la valeur par défaut '$condaEnvName'."
    }
} else {
    ...
    # Si .env n'est pas trouvé, $condaEnvName reste à sa valeur par défaut "projet-is"
