...
async def main():
    ...
    complex_scenario_list = data_generator.generate_multiple_scenarios(
    ...
    logger.info(
        f"Texte d'entrée généré (premiers 200 caractères): {input_text[:200]}..."
    )
    ...
    try:
        ...
        analysis_result = await analysis_runner.run_analysis_v2(
            text_content=input_text, llm_service=llm_service
        )
        ...
        if isinstance(analysis_result, dict):
            if (
                ...
                logger.error(
                    f"L'analyse locale a échoué ou n'a pas retourné de résultat valide: {analysis_result.get('message', 'Raison inconnue')}"
                )
                result_to_save = analysis_result
        else:
            logger.error(
                f"Le résultat de l'analyse n'est pas un dictionnaire: {type(analysis_result)}"
            )
            result_to_save = {
        ...
        logger.error(
            f"Une erreur est survenue lors de l'exécution du pipeline d'analyse local: {e}",
            exc_info=True,
        )
