...
async def orchestrate_with_existing_tools():
    ...
    try:
        ...
        logger.info(
            f"Texte chargé: {extract_info['title']} ({extract_info['length']} caractères)"
        )
        ...
        safe_print(
            f"Durée: {(datetime.now() - tracker.start_time).total_seconds():.2f}s"
        )
        ...
        if isinstance(fallacies_data, dict):
            ...
            if "fallacies" in fallacies_data:
                ...
                safe_print(
                    f"Sophismes trouvés: {len(fallacy_list) if fallacy_list else 0}"
                )
