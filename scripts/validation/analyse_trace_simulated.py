...
class ConversationMetrics:
    ...
    def moyenne_globale(self) -> float:
        scores = [
            self.naturalite_score,
            self.pertinence_score,
            self.progression_logique_score,
            self.personnalite_distincte_score,
            self.fluidite_transitions_score,
            self.dosage_revelations_score,
            self.satisfaction_resolution_score,
        ]
...
def generate_realistic_conversation() -> Dict[str, Any]:
    ...
    oracle_statistics = {
        ...
        "agent_interactions": {"total_turns": 10, "oracle_queries": 3},
        "recent_revelations": [
    ...
    return {
        ...
        "workflow_info": {"strategy": "balanced", "execution_time_seconds": 45.3},
    }
...
def analyze_current_conversation_quality() -> ConversationMetrics:
    ...
    metrics.personnalite_distincte_score = evaluate_personality_distinction(
        conversation_history
    )

    # 5. Fluidité des transitions (0-10)
    metrics.fluidite_transitions_score = evaluate_transition_fluidity(
        conversation_history
    )

    # 6. Dosage des révélations (0-10)
    metrics.dosage_revelations_score = evaluate_revelation_dosage(
        conversation_history, workflow_result
    )

    # 7. Satisfaction de résolution (0-10)
    metrics.satisfaction_resolution_score = evaluate_resolution_satisfaction(
        workflow_result
    )
...
def evaluate_naturalness(conversation_history: List[Dict]) -> float:
    ...
    if avg_length > 150:  # Messages très longs
        score -= 1.5
        logger.info(
            f"Pénalité naturalité: messages trop longs (moyenne: {avg_length:.0f} caractères)"
        )
    ...
    technical_count = sum(
        all_text.lower().count(pattern) for pattern in technical_patterns
    )

    if technical_count > 5:
        score -= 1.0
        logger.info(
            f"Pénalité naturalité: langage trop technique ({technical_count} occurrences)"
        )
...
def evaluate_relevance(conversation_history: List[Dict]) -> float:
    ...
    sherlock_suggestions = sum(
        1
        for msg in sherlock_msgs
        if "suggère" in msg.lower() or "suggestion" in msg.lower()
    )
    ...
    moriarty_mechanical = sum(
        1
        for msg in moriarty_msgs
        if msg.startswith("**RÉFUTATION**") or msg.startswith("**AUCUNE RÉFUTATION**")
    )
...
def evaluate_logical_progression(workflow_result: Dict) -> float:
    ...
    logger.info(
        "Problème progression: enquête trop linéaire, manque de rebondissements"
    )
    score -= 0.5
...
def evaluate_personality_distinction(conversation_history: List[Dict]) -> float:
    ...
    for msg in conversation_history:
        ...
        if agent == "Sherlock":
            if any(
                word in content
                for word in ["méthodiquement", "stratégie", "élimination"]
            ):
                agent_styles[agent]["methodical"] += 1
            if any(
                word in content
                for word in ["conclusion", "je propose", "nouvelle suggestion"]
            ):
    ...
    watson_questions = sum(
        1
        for msg in conversation_history
        if msg["sender"] == "Watson" and "?" in msg["message"]
    )
    if watson_questions > 2:
        logger.info(
            "PROBLÈME MAJEUR: Watson trop passif, que des questions, pas de personnalité distinctive"
        )
        score -= 1.0
    ...
    moriarty_total = sum(
        1 for msg in conversation_history if msg["sender"] == "Moriarty"
    )

    if moriarty_total > 0 and moriarty_mechanical / moriarty_total > 0.8:
        logger.info(
            "PROBLÈME MAJEUR: Moriarty trop robotique, manque de mystère et de personnalité"
        )
        score -= 1.5
...
def evaluate_transition_fluidity(conversation_history: List[Dict]) -> float:
    ...
    for i, msg in enumerate(conversation_history):
        if msg["sender"] == "Watson" and not any(
            word in msg["message"].lower() for word in ["donc", "ainsi", "j'observe"]
        ):
            watson_abrupt += 1

    if watson_abrupt > 1:
        logger.info(
            "PROBLÈME: Watson transitions très abruptes, ne fait pas référence au contexte"
        )
        score -= 1.0
...
def evaluate_revelation_dosage(
    conversation_history: List[Dict], workflow_result: Dict
) -> float:
    """Évalue le dosage des révélations Moriarty (0-10)."""
    score = 6.0  # Score de base correct

    moriarty_revelations = [
        msg
        for msg in conversation_history
    ...
    mechanical_revelations = sum(
        1 for msg in moriarty_revelations if msg["message"].startswith("**RÉFUTATION**")
    )

    if mechanical_revelations == len(moriarty_revelations):
        logger.info(
            "PROBLÈME: Toutes les révélations Moriarty sont mécaniques, manque de variété"
        )
        score -= 1.5
...
def evaluate_resolution_satisfaction(workflow_result: Dict) -> float:
    ...
    execution_time = workflow_result.get("workflow_info", {}).get(
    ...
    logger.info(
        "PROBLÈME: Résolution correcte mais manque de suspense et de complexité narrative"
    )
    score -= 0.5
...
def generate_optimization_roadmap(
    improvements: List[Dict], metrics: ConversationMetrics
) -> Dict[str, Any]:
...
def main():
    ...
    print(
        f"  • Pertinence agents: {metrics.pertinence_score:.1f}/10 {'[X] FAIBLE' if metrics.pertinence_score < 6 else '[V] OK'}"
    )
    print(
        f"  • Progression logique: {metrics.progression_logique_score:.1f}/10 {'[X] FAIBLE' if metrics.progression_logique_score < 6 else '[V] OK'}"
    )
    print(
        f"  • Personnalités distinctes: {metrics.personnalite_distincte_score:.1f}/10 {'[X] CRITIQUE' if metrics.personnalite_distincte_score < 5 else '[X] FAIBLE' if metrics.personnalite_distincte_score < 6 else '[V] OK'}"
    )
    print(
        f"  • Fluidité transitions: {metrics.fluidite_transitions_score:.1f}/10 {'[X] FAIBLE' if metrics.fluidite_transitions_score < 6 else '[V] OK'}"
    )
    print(
        f"  • Dosage révélations: {metrics.dosage_revelations_score:.1f}/10 {'[X] FAIBLE' if metrics.dosage_revelations_score < 6 else '[V] OK'}"
    )
    print(
