...
async def main():
    ...
    try:
        result = await analyze_text(text=text_input, mode="orchestration")
        ...
        if (
            ...
            and result.get("metadata", {}).get("pipeline_mode") == "orchestration"
        ):
            logger.info(
                "VALIDATION RÉUSSIE : Le nouveau chemin a été emprunté et a terminé avec succès."
            )
        else:
            logger.warning(
                "VALIDATION PARTIELLE/ÉCHOUÉE : Vérifiez les logs et le résultat ci-dessus."
            )

    except Exception as e:
        logger.error(
            f"Une erreur est survenue durant l'exécution du pipeline: {e}",
            exc_info=True,
        )
