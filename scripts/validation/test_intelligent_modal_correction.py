...
class ModalCorrectionTester:
        ...
        for i, test_case in enumerate(self.test_errors, 1):
            ...
            try:
                ...
                if detected_type == test_case["type"]:
                    ...
                    print(
                        f"    ❌ Type incorrect: attendu {test_case['type']}, obtenu {detected_type}"
                    )
                ...
                results["failed_detections"] += 1
                results["details"].append(
                    {"test_case": test_case, "error": str(e), "status": "error"}
                )
    ...
    def test_bnf_generation(self) -> Dict[str, Any]:
        ...
        generation_rate = (
            results["successful_generations"] / results["total_tests"] * 100
        )
        ...
        for i, test_case in enumerate(self.test_errors, 1):
            ...
            try:
                ...
                formatted_feedback = analyze_tweety_error(
                    test_case["message"], attempt_number=i, context={"test": True}
                )
                ...
                sections_found = sum(
                    1 for section in required_sections if section in formatted_feedback
                )
    ...
    def test_confidence_calculation(self) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(self.test_errors, 1):
            ...
            try:
                ...
                results["details"].append(
                    {"test_case": test_case, "confidence": confidence, "status": status}
                )
        ...
        if results["valid_confidences"] > 0:
            results["average_confidence"] = (
                total_confidence / results["valid_confidences"]
            )
...
"""
        ...
        for test_name, results in all_results.items():
            if "total_tests" in results:
                successful = (
                    results.get("successful_detections", 0)
                    + results.get("successful_generations", 0)
                    + results.get("successful_formats", 0)
                    + results.get("valid_confidences", 0)
                )

                total = results["total_tests"] * len(
                    [k for k in results.keys() if "successful" in k or "valid" in k]
                )
                if total == 0:
                ...
                if "average_confidence" in results:
                    report += (
                        f"• Confiance moyenne: {results['average_confidence']:.1%}\n"
                    )
        ...
        if overall_success:
            report += (
                "🎉 Le système de correction intelligente fonctionne correctement.\n"
            )
...
async def main():
    ...
    try:
        ...
        success_rate = (
            success_count / total_test_categories * 100
            if total_test_categories > 0
            else 0
        )
