...
mock_cards_path = (
    project_root / "argumentation_analysis" / "data" / "mock_taxonomy_cards.csv"
)
...
if df_real is not None:
    ...
    if "carte" in df_real.columns:
        ...
        condition_not_empty_str = (
            df_real["carte"].astype(str).str.strip() != ""
        )  # Convertit en str, supprime les espaces et vérifie si non vide
        ...
        try:
            ...
            print(
                f"SUCCESS: Le fichier mock '{mock_cards_path.name}' a été créé avec succès."
            )
            print(
                f"INFO: Nombre de lignes dans '{mock_cards_path.name}': {len(df_cards)}"
            )
            ...
            try:
                ...
                print(
                    f"ERROR: Impossible de lire le fichier mock créé pour vérification: {e_read}"
                )
            ...
            if len(df_cards) < target_sample_size:
                ...
                print(
                    f"INFO: Moins de {target_sample_size} lignes dans '{mock_cards_path.name}' ({len(df_cards)} lignes), toutes les lignes seront utilisées pour '{mock_small_path.name}'."
                )
            else:
                df_small = df_cards.sample(n=target_sample_size, random_state=42)
                print(
                    f"INFO: Échantillon de {len(df_small)} lignes sélectionné pour '{mock_small_path.name}'."
                )
            ...
            try:
                ...
                print(
                    f"SUCCESS: Le fichier mock '{mock_small_path.name}' a été créé avec succès."
                )
                print(
                    f"INFO: Nombre de lignes dans '{mock_small_path.name}': {len(df_small)}"
                )
                ...
                try:
                    ...
                    print(
                        f"ERROR: Impossible de lire le fichier mock '{mock_small_path.name}' créé pour vérification: {e_read_small}"
                    )

            except Exception as e_save_small:
                print(
                    f"ERROR: Échec de la sauvegarde du fichier mock '{mock_small_path.name}': {e_save_small}"
                )
            ...
            if isinstance(df_real, pd.DataFrame) and "carte" in df_real.columns:
                ...
                print(
                    f"DEBUG: Exemples de valeurs dans 'carte' (premières 10 non-NaN): {df_real['carte'].dropna().head(10).tolist()}"
                )
            else:
                print(
                    "DEBUG: df_real n'est pas un DataFrame ou la colonne 'carte' est manquante avant le filtrage."
                )

    else:
        print(
            f"ERROR: La colonne 'carte' n'existe pas dans le fichier {real_taxonomy_path}"
        )
        print(f"DEBUG: Colonnes disponibles: {df_real.columns.tolist()}")
else:
    print(
        f"ERROR: Échec du chargement du fichier de taxonomie réelle: {real_taxonomy_path}"
    )
