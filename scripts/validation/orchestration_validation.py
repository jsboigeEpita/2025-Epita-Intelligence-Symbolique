...
class OrchestrationValidator:
    ...
    async def test_orchestrator_imports(self):
        ...
        try:
            ...
            self.log_test_result(
                test_name, False, {"error": str(e), "error_type": type(e).__name__}
            )
        ...
        try:
            ...
            for name, adapter_class in adapters.items():
                try:
                    ...
                    self.logger.warning(
                        f"Adaptateur {name} non instanciable: {adapter_error}"
                    )
        ...
        try:
            ...
            if llm_service:
                ...
                self.log_test_result(
                    test_name,
                    False,
                    {"error": "Service LLM non cree", "service_returned": None},
                )
    ...
    async def test_group_chat_functionality(self):
        ...
        try:
            from argumentation_analysis.orchestration.group_chat import (
    ...
    def calculate_metrics(self):
        ...
        successful_tests = sum(
            1 for test in self.results["tests"].values() if test["success"]
        )
    ...
    async def run_all_tests(self):
        ...
        for test in tests:
            try:
                ...
                self.logger.error(
                    f"Erreur lors de l'execution du test {test.__name__}: {e}"
                )
                traceback.print_exc()
        ...
        self.logger.info(
            f"[OK] Validation terminee - Taux de reussite: {self.results['success_rate']:.1f}%"
        )
