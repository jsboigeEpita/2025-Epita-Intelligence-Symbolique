...
async def validate_simple(
    ...
    simple_results = {
        "components_available_count": sum(
            1 for comp_status in available_components.values() if comp_status
        ),
    ...
    try:
        ...
        if available_components.get("unified_config", False):
            try:
                ...
                basic_tests_results["import_unified_config"][
                    "details"
                ] = f"ImportError: {e}"
            ...
            basic_tests_results["import_unified_config"][
                "details"
            ] = "UnifiedConfig non marqué comme disponible."
        ...
        if available_components.get(
            ...
            try:
                ...
                basic_tests_results["import_orchestrators"][
                    "details"
                ] = f"ImportError: {e}"
            ...
            basic_tests_results["import_orchestrators"][
                "details"
            ] = "Orchestrateurs non marqués comme disponibles."
        ...
        if available_components.get("conversation_orchestrator", False):
            try:
                ...
                if result and isinstance(result, str):
                    ...
                    logger.info(
                        "  ✓ Test basic_orchestration (ConversationOrchestrator micro): Réussi"
                    )
                else:
                    basic_tests_results["basic_orchestration_conversation_micro"][
                        "status"
                ...
                logger.error(
                    f"  ✗ Test basic_orchestration (ConversationOrchestrator micro): Échoué import ({e})"
                )
            except Exception as e:
                basic_tests_results["basic_orchestration_conversation_micro"][
                    "status"
                ...
                logger.error(
                    f"  ✗ Test basic_orchestration (ConversationOrchestrator micro): Erreur ({e})"
                )
        else:
            basic_tests_results["basic_orchestration_conversation_micro"][
                "status"
            ...
            basic_tests_results["basic_orchestration_conversation_micro"][
                "details"
            ] = "ConversationOrchestrator non marqué comme disponible."
            logger.info(
                "  - Test basic_orchestration (ConversationOrchestrator micro): Skipped (non disponible)"
            )

        simple_results["basic_tests"] = basic_tests_results

        # Status final basé sur les tests effectués
        successful_tests_count = sum(
            1
            for test_result in basic_tests_results.values()
            if test_result["status"] == "success"
        )
        total_relevant_tests = sum(
            1
            for test_result in basic_tests_results.values()
        ...
        if total_relevant_tests == 0:  # Aucun test n'a pu être effectué
            ...
            logger.warning(
                "  Validation simple: Aucun test pertinent n'a pu être exécuté."
            )
            ...
            logger.warning(
                "  ~ Validation simple: Certains tests ont échoué ou ont des erreurs."
            )
        else:  # Aucun test n'a réussi
            simple_results["status"] = "FAILED"
            logger.error(
                "  ✗ Validation simple: Tous les tests pertinents ont échoué ou ont des erreurs."
            )
        ...
        logger.error(
            f"  ✗ Erreur majeure durant la validation simple: {e}", exc_info=True
        )
