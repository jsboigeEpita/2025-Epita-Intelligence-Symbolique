    ...
    try:
        with zipfile.ZipFile(jar_path, 'r') as jar_file:
            for file_path in jar_file.namelist():
                if file_path.startswith(package_path + '/') and file_path.endswith(".class"):
        ...
        if found_classes:
            print(f"Classes trouvées dans le package '{package_name}' du fichier '{os.path.basename(jar_path)}':")
            for c in sorted(found_classes):
                print(f"  - {c}")
        else:
            print(f"Aucune classe trouvée pour le package '{package_name}' dans '{os.path.basename(jar_path)}'.")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python list_classes_in_jar.py <chemin_vers_le_jar> <nom_du_package>")
        print("Exemple: python list_classes_in_jar.py libs/tweety/tweety.jar org.tweetyproject.logics.fol.reasoner")
        sys.exit(1)
