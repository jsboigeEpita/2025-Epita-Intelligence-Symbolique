...
def log_status(message, status="INFO"):
    ...
    prefixes = {"INFO": "[INFO]", "OK": "[OK]  ", "WARN": "[WARN]", "ERROR": "[ERROR]"}
...
def test_conda():
    ...
    try:
        ...
        result = run_in_activated_env(
            ["conda", "--version"], "projet-is", check_errors=False
        )
        ...
        result = run_in_activated_env(
            ["conda", "env", "list", "--json"], "projet-is", check_errors=False
        )
        if result.returncode == 0:
...
def test_gpt4o():
    """Test configuration gpt-5.6-luna"""
    log_status("Test 4: Configuration gpt-5.6-luna")
    ...
    if model_id != "gpt-5.6-luna":
        log_status(f"Modele configure: {model_id} (attendu: gpt-5.6-luna)", "WARN")
    else:
        log_status("Modele gpt-5.6-luna configure", "OK")
