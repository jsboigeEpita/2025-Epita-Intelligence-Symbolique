...
async def run_demo():
    ...
    agent = SherlockJTMSAgent(
        kernel=project_context.kernel, settings=project_context.settings
    )
    # Remplacer l'appel obsolète par l'appel à une méthode principale comme analyze_clues
    clues = [
        {
            "type": "arme",
            "description": "Couteau ensanglanté trouvé dans la cuisine.",
            "reliability": 0.9,
        },
        {
            "type": "témoignage",
            "description": "Un cri a été entendu vers 22h.",
            "reliability": 0.6,
        },
    ]
