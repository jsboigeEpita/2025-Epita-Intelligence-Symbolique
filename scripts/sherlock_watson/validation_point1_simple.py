...
Donne un indice cryptique qui aide sans révéler directement la solution."""
        ...
        try:
            ...
            logger.info(f"🎭 Moriarty - Réponse fournie: {len(response_content)} caractères")
...
class SimpleWatsonAgent:
    ...
    def __init__(self, api_key: str, model: str = "gpt-4o-mini"):
        ...
        self.model = model
        self.conversation_history = []
        logger.info(f"👨‍⚕️ Agent Watson initialisé avec {model} (mode dégradé sans Tweety)")
...
Effectue un raisonnement logique structuré et propose tes conclusions."""
        ...
        try:
            ...
            logger.info(f"🧠 Watson - Raisonnement effectué: {len(reasoning)} caractères")
            return reasoning
...
async def run_cluedo_demo_authentic():
    ...
    case_info = {
        ...
        "armes": ["Pointeur Laser", "Clavier Mécanique", "Serveur en Rack", "Cable Ethernet"],
...
Quelle est votre conclusion finale sur l'identité du coupable, l'arme et le lieu ?"""
    ...
    results = {
        "session_info": {
            ...
            "agents": ["Sherlock (ChatGPT)", "Watson (ChatGPT dégradé)", "Moriarty (ChatGPT Oracle)"]
        },
