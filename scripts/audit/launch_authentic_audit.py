...
def check_prerequisites():
    ...
    if python_version.major >= 3 and python_version.minor >= 8:
        ...
        print(f"[ERROR] Python {python_version.major}.{python_version.minor} (3.8+ requis)")
        return False
    
    # Vérifier les modules critiques
    required_modules = [
        'openai', 'semantic_kernel', 'asyncio', 'json', 'hashlib', 'uuid'
    ]
...
def setup_environment():
    ...
    if not real_api_key:
        ...
        masked_key = real_api_key[:7] + "*" * 20 + real_api_key[-4:] if len(real_api_key) > 11 else "sk-***"
