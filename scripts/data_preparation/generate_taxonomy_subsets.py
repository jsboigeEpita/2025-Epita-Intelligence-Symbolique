...
def load_and_prepare_taxonomy(file_path: str) -> pd.DataFrame:
    ...
    if 'PK' in df.columns and df['PK'].isnull().any():
        raise ValueError("La colonne 'PK' contient des valeurs nulles, ce qui est interdit.")
    ...
    if 'PK' in df.columns:
        ...
        raise ValueError("La colonne 'PK' est manquante et ne peut être définie comme index.")

    print(f"Taxonomie chargée avec {len(df)} entrées.")
    return df

def get_direct_children(df: pd.DataFrame, parent_path: str, parent_depth: int) -> pd.DataFrame:
    """
