    ...
    try:
        ...
        if not match:
            ...
            try:
                ...
                return (
                    False,
                    f"La réponse n'est pas un JSON valide et n'est pas dans un bloc de code. Reçu: {analysis_result_str}",
                )
        else:
        ...
        detected_ids = {
            str(item.get("fallacy_type", "")).lower().strip()
        ...
        return (
            False,
            f"Erreur de parsing ou de comparaison: {e}. Réponse reçue: {analysis_result_str}",
        )
...
async def main():
    ...
    print(
        f"{Colors.BOLD}--- Début du test de débogage pour 'Concept Volé' avec le nouveau workflow ---{Colors.ENDC}"
    )
    ...
    try:
        ...
        workflow_plugin = FallacyWorkflowPlugin(
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}Résultat brut du workflow:{Colors.ENDC}\n{final_result_str}"
        )

        # Le résultat est maintenant une chaîne JSON, la fonction de comparaison est adaptée
        success, details = compare_sophisms(
            final_result_str, scenario["expected_sophisms"]
        )
