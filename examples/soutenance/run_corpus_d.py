#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Soutenance demo — Corpus D (src_idx=0, ~99K chars, EN).

One-shot end-to-end SCDA pipeline run on corpus_dense_D.
Produces a state JSON snapshot and a readable summary.

Usage:
    conda activate projet-is
    python examples/soutenance/run_corpus_d.py

Expected output (targets — first run, conservative):
    arguments_found : 15 ± 5
    fallacies_found : 10 ± 5
    unique_formal_categories : ≥ 3

All outputs go to ``outputs/soutenance_demo/corpus_dense_D/`` (gitignored).
"""
...
async def run_demo() -> None:
    ...
    print(f"  (expected ~30 min wall-clock)")
