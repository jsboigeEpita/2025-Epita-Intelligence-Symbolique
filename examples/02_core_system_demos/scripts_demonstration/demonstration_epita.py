...
class EpitaValidator:
    ...
    def __init__(self):
        ...
        self.mock_indicators = ["simulation", "mock", "données factices"]
    ...
    def validate_with_dataset(
        self, dataset: CustomTestDataset, module_func, mode: str
    ) -> ValidationResult:
        ...
        try:
            ...
            with contextlib.redirect_stdout(
                ...
                try:
                    if hasattr(module_func, "__call__"):
                        ...
                        result = (
                            module_func()
                            if not module_func.__code__.co_argcount
                            else module_func(dataset.content)
                        )
                    else:
                        result = True
            ...
            custom_data_processed = (
                dataset.marker in output
                or dataset.content_hash in output
                or any(
                    expected.lower() in output.lower()
                    for expected in dataset.expected_indicators
                )
            )
...
def charger_et_executer_module(
    nom_module: str, mode_interactif: bool = False, **kwargs
) -> bool:
    ...
    try:
        ...
        if not module_path.exists():
            print(
                f"{Colors.FAIL}{Symbols.CROSS} Module {nom_module} non trouvé{Colors.ENDC}"
            )
            return False
        ...
        if target_func:
            ...
            print(
                f"{Colors.WARNING}Fonction de démonstration appropriée non trouvée dans {nom_module}{Colors.ENDC}"
            )
            return False

    except Exception as e:
        print(
            f"{Colors.FAIL}{Symbols.CROSS} Erreur lors de l'exécution de {nom_module}: {e}{Colors.ENDC}"
        )
        return False
    ...
    while True:
        ...
        try:
            ...
            if choix.isdigit():
                ...
                cat_selectionnee = next(
                    (
                        (cid, cinfo)
                        for cid, cinfo in categories.items()
                        if cinfo.get("id") == num_choix
                    ),
                    None,
                )
                ...
                if cat_selectionnee:
                    ...
                    nom_module = (
                        "demo_analyse_argumentation"
                        if cat_id == "agents_logiques"
                        else cat_info.get("module", "")
                    )
                    ...
                    if confirmer_action(f"Exécuter la démonstration '{nom_cat}' ?"):
                        succes = charger_et_executer_module(
                            nom_module, mode_interactif=True, **kwargs
                        )

                        if succes:
                            logger.success(
                                f"{Symbols.CHECK} Démonstration '{nom_cat}' terminée avec succès !"
                            )
                        else:
                            logger.error(
                                f"{Symbols.CROSS} Échec de la démonstration '{nom_cat}'"
                            )
                ...
                print(
                    f"{Colors.FAIL}Veuillez entrer un numéro (1-6) ou 'q'{Colors.ENDC}"
                )
                pause_interactive()
    ...
    for cat_id, cat_info in categories.items():
        ...
        if module_name:
            try:
                ...
                succes = charger_et_executer_module(
                    module_name, mode_interactif=False, **kwargs
                )
                if succes:
...
def mode_metrics_only(config: Dict[str, Any]) -> None:
    ...
    print(
        f"{Colors.GREEN}{Symbols.CHECK} Taux de succès des tests : {taux_succes}%{Colors.ENDC}"
    )
...
{Colors.ENDC}""")
    ...
    logger.info(
        f"{Symbols.ROCKET} Début de l'exécution complète - {len(categories_triees)} catégories à traiter"
    )
    logger.info(
        f"Paramètres : agent_type={kwargs.get('agent_type')}, taxonomy_path={kwargs.get('taxonomy_path')}"
    )
    ...
    for i, (cat_id, cat_info) in enumerate(categories_triees, 1):
        ...
        print(
            f"{Colors.CYAN}{cat_info.get('icon', '•')} CATÉGORIE {i}/{total_categories} : {nom_cat}{Colors.ENDC}"
        )
        ...
        try:
            ...
            succes = charger_et_executer_module(
                nom_module, mode_interactif=False, **kwargs
            )
        except Exception as e:
            erreur = e
        ...
        if succes:
            ...
            logger.success(
                f"{symbol} Catégorie '{nom_cat}' terminée avec succès en {cat_duration:.2f}s"
            )
        else:
            categories_echouees += 1
            logger.error(
                f"{symbol} Échec de la catégorie '{nom_cat}' après {cat_duration:.2f}s"
            )
        ...
        resultats_detailles.append(
            {
                ...
                "index": i,
                "erreur": str(erreur) if erreur else "",
            }
        )
    ...
    taux_reussite = (
        (categories_reussies / total_categories) * 100 if total_categories > 0 else 0
    )

    print(f"\n{Colors.BOLD}{'=' * 80}{Colors.ENDC}")
    print(
        f"{Colors.CYAN}{Colors.BOLD}           RAPPORT FINAL - EXÉCUTION COMPLÈTE{Colors.ENDC}"
    )
    print(f"{'=' * 80}")
    ...
    for resultat in resultats_detailles:
        ...
        print(
            f"   {status_symbol} {resultat['index']:2d}. {resultat['categorie']:<30} "
    ...
    print(
        f"   [PYTHON] Architecture : {config.get('config', {}).get('architecture', 'Python + Java (JPype)')}"
    )
    print(f"   [VERSION] Version : {config.get('config', {}).get('version', '2.0.0')}")
    print(
        f"   [TARGET] Taux succès tests : {config.get('config', {}).get('taux_succes_tests', 99.7)}%"
    )
    ...
    if categories_echouees == 0:
        ...
        final_message = (
            f"[WARNING] EXÉCUTION TERMINÉE AVEC {categories_echouees} ÉCHEC(S)"
        )
...
{Colors.ENDC}""")
    ...
    for cat_id, cat_info in categories_triees:
        ...
        print(
            f"{Colors.CYAN}[{timestamp_start_cat} | SEARCH] VALIDATION MODULE: {nom_cat}{Colors.ENDC}"
        )
        print(f"{'=' * 60}")
        ...
        for dataset in datasets:
            ...
            try:
                ...
                if module_path.exists():
                    ...
                    if demo_func:
                        result = validator.validate_with_dataset(
                            dataset, demo_func, nom_cat
                        )
                        ...
                        if result.success:
                            print(
                                f"   {Colors.GREEN}[OK] Exécution: SUCCÈS{Colors.ENDC}"
                            )
                        else:
                            print(
                                f"   {Colors.FAIL}[FAIL] Exécution: ÉCHEC{Colors.ENDC}"
                            )

                        if result.custom_data_processed:
                            print(
                                f"   {Colors.GREEN}[DOC] Données custom: TRAITÉES{Colors.ENDC}"
                            )
                        else:
                            print(
                                f"   {Colors.WARNING}[DOC] Données custom: NON DÉTECTÉES{Colors.ENDC}"
                            )

                        if result.real_processing_detected:
                            print(
                                f"   {Colors.GREEN}[TOOL] Traitement réel: DÉTECTÉ{Colors.ENDC}"
                            )
                        else:
                            print(
                                f"   {Colors.WARNING}[TOOL] Traitement réel: NON DÉTECTÉ{Colors.ENDC}"
                            )

                        if result.mock_detected:
                            print(
                                f"   {Colors.FAIL}[MOCK] Mocks détectés: OUI{Colors.ENDC}"
                            )
                        else:
                            print(
                                f"   {Colors.GREEN}[MOCK] Mocks détectés: NON{Colors.ENDC}"
                            )

                        timestamp_end_ds = datetime.now().strftime("%H:%M:%S")
                        print(
                            f"   [{timestamp_end_ds} | TIME] Temps d'exécution: {result.execution_time:.3f}s"
                        )

                        if result.error:
                            print(
                                f"   {Colors.FAIL}[ERROR] Erreur: {result.error}{Colors.ENDC}"
                            )
                    else:
                        print(
                            f"   {Colors.WARNING}[WARN] Aucune fonction de démo trouvée{Colors.ENDC}"
                        )
                else:
                    print(
                        f"   {Colors.FAIL}[FAIL] Module non trouvé: {module_path}{Colors.ENDC}"
                    )
    ...
    print(f"\n{Colors.BOLD}{'=' * 80}{Colors.ENDC}")
    print(
        f"{Colors.CYAN}{Colors.BOLD}           RAPPORT FINAL - VALIDATION DONNÉES CUSTOM{Colors.ENDC}"
    )
    print(f"{'=' * 80}")
    ...
    if all_results:
        ...
        real_processing_tests = sum(
            1 for r in all_results if r.real_processing_detected
        )
        ...
        print(
            f"   Tests réussis: {success_tests}/{total_tests} ({success_tests/total_tests*100:.1f}%)"
        )
        print(
            f"   Traitement réel détecté: {real_processing_tests}/{total_tests} ({real_processing_tests/total_tests*100:.1f}%)"
        )
        print(
            f"   Données custom traitées: {custom_data_tests}/{total_tests} ({custom_data_tests/total_tests*100:.1f}%)"
        )
        print(
            f"   Mocks détectés: {mock_detected_tests}/{total_tests} ({mock_detected_tests/total_tests*100:.1f}%)"
        )

        print(f"\n{Colors.BOLD}[TARGET] ÉVALUATION CAPACITÉS:{Colors.ENDC}")
        if custom_data_tests > total_tests * 0.7:
            print(
                f"   {Colors.GREEN}[OK] EXCELLENTE acceptation des données custom{Colors.ENDC}"
            )
        elif custom_data_tests > total_tests * 0.4:
            print(
                f"   {Colors.WARNING}[WARN] MODÉRÉE acceptation des données custom{Colors.ENDC}"
            )
        else:
            print(
                f"   {Colors.FAIL}[FAIL] FAIBLE acceptation des données custom{Colors.ENDC}"
            )
        ...
        if real_processing_tests > total_tests * 0.6:
            ...
            print(
                f"   {Colors.WARNING}[WARN] MOCKS ou simulations détectés{Colors.ENDC}"
            )

        # Sauvegarder le rapport détaillé
        rapport_path = (
            Path("logs")
            / f"validation_epita_custom_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
{Colors.ENDC}""")
    ...
    for cat_id, cat_info in sorted(categories.items(), key=lambda x: x[1]["id"]):
        ...
        try:
            ...
            if module_path.exists():
                ...
                demo_func = getattr(module, "run_demo_rapide", None) or getattr(
                    module, "run_demo_interactive", None
                )

                if demo_func:
                    result = validator.validate_with_dataset(
                        custom_dataset, demo_func, nom_cat
                    )
                    ...
                    print(
                        f"   {status} | {data_processed} | {real_processing} | [TIME] {result.execution_time:.3f}s"
                    )
                else:
                    print(
                        f"   {Colors.WARNING}[WARN] Fonction de démo non trouvée{Colors.ENDC}"
                    )
            else:
    ...
    if results:
        ...
        processing_rate = (
            sum(1 for r in results if r.custom_data_processed) / len(results) * 100
        )
        real_rate = (
            sum(1 for r in results if r.real_processing_detected) / len(results) * 100
        )
        ...
        if processing_rate > 70:
            print(
                f"   {Colors.GREEN}[TARGET] CONCLUSION: Les données custom sont bien acceptées et traitées{Colors.ENDC}"
            )
        elif processing_rate > 30:
            print(
                f"   {Colors.WARNING}[TARGET] CONCLUSION: Acceptation partielle des données custom{Colors.ENDC}"
            )
        else:
            print(
                f"   {Colors.FAIL}[TARGET] CONCLUSION: Les données custom ne semblent pas être traitées{Colors.ENDC}"
            )
...
Modes disponibles :
  ...
  --legacy           Exécution du script original (compatibilité)
    ...
    parser.add_argument(
        "--validate-custom",
        action="store_true",
        help="Mode validation avec données dédiées",
    )
    parser.add_argument(
        "--custom-data",
        type=str,
        metavar="TEXT",
        help="Test avec des données custom spécifiques",
    )
...
def main():
    ...
    if not valider_environnement():
        print(
            f"{Colors.FAIL}Environnement non valide. Exécutez depuis la racine du projet.{Colors.ENDC}"
        )
        sys.exit(1)
    ...
    kwargs = {"agent_type": args.agent_type, "taxonomy_path": args.taxonomy_path}
    ...
    if args.validate_custom:
        ...
        categories = sorted(
            config.get("categories", {}).items(), key=lambda x: x[1]["id"]
        )
