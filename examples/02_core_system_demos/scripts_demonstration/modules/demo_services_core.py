...
def demo_reparation_marqueurs(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    if succes:
        ...
        print(
            f'{Colors.GREEN}Réparé: "Si P alors Q" (correction automatique){Colors.ENDC}'
        )
...
def demo_verification_extraits(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    tests_verification = ["tests/unit/argumentation_analysis/test_verify_extracts.py"]
...
def demo_communication_services(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    if succes:
        ...
        print(
            f'  Service A: {Colors.GREEN}Request(analyze_text, "Si P alors Q"){Colors.ENDC}'
        )
        print(f"           ↓")
        print(f"  Service B: {Colors.BLUE}Processing...{Colors.ENDC}")
        print(f"           ↓")
        print(
            f"  Service A: {Colors.GREEN}Response(rules=[P→Q], facts=[], conclusions=[]){Colors.ENDC}"
        )
...
def demo_etat_partage(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    if succes:
        ...
        print(
            f"  {Colors.CYAN}Agent 1{Colors.ENDC} ←→ {Colors.BLUE}State Manager{Colors.ENDC} ←→ {Colors.CYAN}Agent 2{Colors.ENDC}"
        )
        print(f"               ↕")
...
def run_demo_interactive() -> bool:
    ...
    resultats_modules["Réparation Marqueurs"] = demo_reparation_marqueurs(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Vérification Extraits"] = demo_verification_extraits(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Services Communication"] = demo_communication_services(
        logger, config
    )
    pause_interactive()
