    ...
    def process_custom_data(self, content: str, mode: str) -> Dict[str, Any]:
        ...
        logical_elements = {
            ...
            "conclusions": re.findall(
                r"donc|par conséquent|ainsi", content, re.IGNORECASE
            ),
            "connectors": re.findall(
                r"si.*alors|puisque|car|parce que", content, re.IGNORECASE
            ),
        }
        ...
        if logical_elements["connectors"]:
            arg_strength = min(
                1.0, arg_strength + 0.3 * len(logical_elements["connectors"])
            )
...
class AdaptiveAnalyzer:
    ...
    def analyze_modal_logic(self, content: str) -> Dict[str, Any]:
        ...
        modal_operators = []
        if re.search(
            r"nécessairement|obligatoirement|forcément", content, re.IGNORECASE
        ):
        ...
        return {
            ...
            "temporal_elements": re.findall(
                r"avant|après|pendant|toujours|jamais", content, re.IGNORECASE
            ),
