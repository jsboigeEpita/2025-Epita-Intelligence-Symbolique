...
from modules.custom_data_processor import (
    CustomDataProcessor,
    AdaptiveAnalyzer,
...
def process_custom_data_cas_usage(
    custom_data: str = None, logger: DemoLogger = None
) -> Dict[str, Any]:
    """
    ...
    if logger:
        logger.info(
            f"{Symbols.BRAIN} [CUSTOM_DATA_CAS_USAGE] Traitement des données custom pour cas d'usage..."
        )
    ...
    if custom_data:
        ...
        if logger:
            logger.success(
                f"{Symbols.CHECK} [CUSTOM_DATA_CAS_USAGE] Analyse adaptative terminée: {len(custom_data)} caractères traités"
            )
            logger.info(
                f"  • Scénarios détectés: {len(use_case_elements['scenarios_detected'])}"
            )
            logger.info(
                f"  • Patterns workflow: {len(use_case_elements['workflow_patterns'])}"
            )
            logger.info(
                f"  • Aspects collaboration: {len(use_case_elements['collaboration_aspects'])}"
            )
    else:
        ...
        if logger:
            logger.warning(
                f"{Symbols.WARNING} [CUSTOM_DATA_CAS_USAGE] Mode fallback - aucune donnée custom fournie"
            )
...
def demo_cluedo_sherlock_watson(
    logger: DemoLogger, config: Dict[str, Any], custom_data: str = None
) -> bool:
    ...
    if custom_data:
        ...
        logger.info(
            f"{Symbols.BRAIN} Données custom traitées: {custom_result.get('content_hash', 'N/A')[:8]}..."
        )
        logger.info(
            f"  • Scénarios détectés: {len(custom_result.get('use_case_elements', {}).get('scenarios_detected', []))}"
        )
    ...
    if succes:
        logger.success(
            f"{Symbols.FIRE} Système Cluedo Sherlock-Watson : 100% OPÉRATIONNEL !"
        )
        print(
        ...
        print(
            f'{Colors.BLUE}Watson: "Nous avons 3 suspects : Colonel Moutarde, Miss Scarlett, Professeur Plum"'
        )
        print(
            f'{Colors.GREEN}Sherlock: "Intéressant. L\'arme était le chandelier dans la bibliothèque."'
        )
        print(
            f'{Colors.BLUE}Watson: "Mais qui était présent dans la bibliothèque à ce moment ?"'
        )
        print(
            f'{Colors.GREEN}Sherlock: "Élémentaire ! Seul le Colonel avait accès à cette pièce..."{Colors.ENDC}'
        )
...
def demo_personnalites_distinctes(
    ...
    if succes:
        ...
        print(
            f'{Colors.BLUE}Watson: "Pourrions-nous examiner les preuves ensemble ?"{Colors.ENDC}'
        )
...
def demo_workflows_rhetoriques(
    logger: DemoLogger, config: Dict[str, Any], custom_data: str = None
) -> bool:
    ...
    print(
        f'  {Colors.CYAN}Preuves{Colors.ENDC}: "Accès à la bibliothèque", "Motif", "Aucun alibi"'
    )
...
def demo_collaboration_multi_agents(
    logger: DemoLogger, config: Dict[str, Any], custom_data: str = None
) -> bool:
...
def run_demo_interactive() -> bool:
    ...
    resultats_modules["Cluedo Sherlock-Watson"] = demo_cluedo_sherlock_watson(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Personnalités Distinctes"] = demo_personnalites_distinctes(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Workflows Rhétoriques"] = demo_workflows_rhetoriques(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Collaboration Multi-Agents"] = demo_collaboration_multi_agents(
        logger, config
    )
    pause_interactive()
    ...
    resultats_modules["Cas d'Usage Éducatifs"] = demo_cas_usage_educatifs(
        logger, config
    )
    ...
    for module, succes in resultats_modules.items():
        status = (
            f"{Colors.GREEN}{Symbols.CHECK}"
            if succes
            else f"{Colors.FAIL}{Symbols.CROSS}"
        )
...
def run_demo_rapide(**kwargs) -> bool:
    ...
    if succes:
        ...
        print(
            f"  • {Colors.GREEN}Résolution Cluedo{Colors.ENDC}: Sherlock-Watson collaborent pour résoudre mystères"
        )
        print(
            f"  • {Colors.BLUE}Workflows rhétoriques{Colors.ENDC}: Argumentation structurée et raisonnement"
        )
        print(
            f"  • {Colors.WARNING}Collaboration multi-agents{Colors.ENDC}: Dialogue naturel et synchronisation"
        )
        print(
            f"  • {Colors.CYAN}Scénarios complets{Colors.ENDC}: Applications pratiques bout-en-bout"
        )
