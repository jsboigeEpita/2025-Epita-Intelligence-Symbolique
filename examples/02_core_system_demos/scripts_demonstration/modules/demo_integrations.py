...
def process_custom_data_integration(
    custom_content: str, logger: DemoLogger
) -> Dict[str, Any]:
    ...
    logger.info(
        f"✅ Potentiel d'intégration: {integration_analysis.get('integration_potential', 'unknown')}"
    )
...
def demo_integration_operational(
...
def demo_interface_tactique_strategique(
    logger: DemoLogger, config: Dict[str, Any]
) -> bool:
    ...
    if succes:
        logger.success(
            f"{Symbols.FIRE} Interface stratégique-tactique opérationnelle !"
        )
        print(
        ...
        print(
            f'  {Colors.CYAN}Tactique{Colors.ENDC}: "Analyser indices", "Éliminer suspects"'
        )
        print(f"       ↓ (exécution)")
        print(
            f'  {Colors.GREEN}Opérationnel{Colors.ENDC}: "Parser texte", "Appliquer règles"'
        )
...
def demo_interface_tactique_operationnel(
    logger: DemoLogger, config: Dict[str, Any]
) -> bool:
...
def demo_jpype_tweety_integration(
    logger: DemoLogger, config: Dict[str, Any], custom_data: str = None
) -> bool:
    ...
    if custom_data:
        logger.info(
            f"{Symbols.ROCKET} Analyse RÉELLE d'intégration custom (anti-simulation)"
        )
        ...
        if integration_analysis:
            ...
            print(
                f"  • Potentiel: {integration_analysis.get('integration_potential', 'unknown')}"
            )
            ...
            for category, patterns in integrations.items():
                if patterns:
                    print(
                        f"  • {category.replace('_', ' ').title()}: {', '.join(patterns)}"
                    )
        else:
            print(
    ...
    if custom_data and integration_results:
        ...
        print(
            f"  • Simulation utilisée: ❌ {integration_analysis.get('mock_used', False)}"
        )
    ...
    logger.success(
        f"{Symbols.FIRE} Intégration JPype-Tweety {'avec données custom' if custom_data else ''} analysée !"
    )
    return True
...
def demo_apis_externes(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    for api, description in apis_supportees.items():
        print(
            f"  {Colors.GREEN}•{Colors.ENDC} {Colors.BOLD}{api}{Colors.ENDC}: {description}"
        )
...
def demo_adaptation_protocoles(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    print(
        f"  • {Colors.BLUE}Bridge Pattern{Colors.ENDC}: Abstraction d'implémentations"
    )
    print(
        f"  • {Colors.WARNING}Facade Pattern{Colors.ENDC}: Simplification d'interfaces complexes"
    )
    ...
    print(
        f"  Agent A ← JSON ← {Colors.BLUE}Adaptateur{Colors.ENDC} ← XML ← Agent B{Colors.ENDC}"
    )
...
def run_demo_interactive(custom_data: str = None) -> bool:
    ...
    if custom_data:
        ...
        print(
            f"  • Hash préliminaire: {CustomDataProcessor().compute_content_hash(custom_data)[:8]}..."
        )
    else:
    ...
    resultats_modules["Intégration Opérationnelle"] = demo_integration_operational(
        logger, config
    )
    ...
    resultats_modules["JPype-Tweety"] = demo_jpype_tweety_integration(
        logger, config, custom_data
    )
    ...
    resultats_modules["Adaptation Protocoles"] = demo_adaptation_protocoles(
        logger, config
    )
