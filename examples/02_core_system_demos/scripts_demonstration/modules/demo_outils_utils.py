...
def demo_utilitaires_core(logger: DemoLogger, config: Dict[str, Any]) -> bool:
    ...
    if succes:
        ...
        for util, desc in utilitaires_core.items():
            print(
                f"  {Colors.GREEN}•{Colors.ENDC} {Colors.BOLD}{util}{Colors.ENDC}: {desc}"
            )
...
def run_demo_interactive() -> bool:
    ...
    resultats_modules["Générateurs de Données"] = demo_generateurs_donnees(
        logger, config
    )
    ...
    resultats_modules["Outils Développement"] = demo_outils_developpement(
        logger, config
    )

    # 4. Métriques et visualisation
    afficher_progression(4, total_etapes, "Métriques & visualisation")
    resultats_modules["Métriques & Visualisation"] = demo_metriques_visualisation(
        logger, config
    )
