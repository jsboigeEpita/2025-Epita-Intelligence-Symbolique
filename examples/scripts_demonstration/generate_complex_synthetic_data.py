...
class ComplexSyntheticDataGenerator:
    ...
    def generate_complex_scenario(self, complexity: str, scenario_id: str) -> Dict[str, Any]:
        ...
        for i in range(config['args_per_scenario']):
            ...
            argument = {
                ...
                'epistemic_metadata': {
                    ...
                    'temporal_validity': random.choice(['permanent', 'contextual', 'temporal']),
                    'modal_status': random.choice(['necessary', 'possible', 'contingent'])
                },
                'pragmatic_context': {
                    'speech_act': random.choice(['assertion', 'question', 'command', 'promise']),
                    'implicature': random.choice(['scalar', 'conversational', 'conventional', 'none']),
        ...
        for i in range(config['contradictions']):
            ...
            contradiction = {
                ...
                'resolution_strategies': [
                    random.choice(['reject_premise', 'modify_conclusion', 'context_differentiation', 'temporal_separation'])
        ...
        scenario = {
            ...
            'complexity_metrics': {
                ...
                'epistemic_complexity': len([item for sublist in selected_issues.values() for item in sublist]),
    ...
    def generate_multiple_scenarios(self, complexity: str, num_scenarios: int) -> List[Dict[str, Any]]:
        ...
        report = {
            'generation_summary': {
                ...
                'output_files': {
                    ...
                    'by_complexity': {level: str(output_path / f"scenarios_{level}_{timestamp}.json") 
                                    for level in by_complexity.keys()}
                }
            },
            'quality_metrics': {
                'avg_arguments_per_scenario': sum(len(s['arguments']) for s in scenarios) / len(scenarios),
                'avg_contradictions_per_scenario': sum(len(s['contradictions']) for s in scenarios) / len(scenarios),
                'avg_coherence_score': sum(s['global_coherence_score'] for s in scenarios) / len(scenarios),
...
def main():
    parser = argparse.ArgumentParser(description='Génération de données synthétiques complexes')
    parser.add_argument('--complexity', choices=['low', 'medium', 'high'], default='high',
                       help='Niveau de complexité des données')
    parser.add_argument('--scenarios', choices=['single', 'multiple', 'comprehensive'], default='multiple',
                       help='Type de scénarios à générer')
    parser.add_argument('--output-dir', default='data/synthetic_complex',
                       help='Répertoire de sortie')
    parser.add_argument('--count', type=int, default=5,
                       help='Nombre de scénarios à générer')
    ...
    if args.scenarios == 'single':
        scenarios = [generator.generate_complex_scenario(args.complexity, f"single_{uuid.uuid4().hex[:8]}")]
    elif args.scenarios == 'multiple':
        scenarios = generator.generate_multiple_scenarios(args.complexity, args.count)
    else:  # comprehensive
        scenarios = []
        for level in ['low', 'medium', 'high']:
            level_scenarios = generator.generate_multiple_scenarios(level, args.count // 3 + 1)
    ...
    print(f"   Complexite epistemologique moyenne: {sum(s['complexity_metrics']['epistemic_complexity'] for s in scenarios) / len(scenarios):.1f}")
