...
def simulate_speech_to_text(audio_description: str) -> str:
    ...
    return sample_speeches.get(
        audio_description, sample_speeches["climate_debate"]
    ).strip()
...
def process_audio_pipeline(audio_description: str, show_steps: bool = True):
    ...
    if show_steps:
        ...
        print(
            f"Unique fallacy types: {analysis_result['summary']['unique_fallacy_types']}"
        )
...
def main():
    ...
    for audio_type, description in test_cases:
        ...
        results.append(
            {"audio_type": audio_type, "description": description, "result": result}
        )
    ...
    total_fallacies = sum(
        r["result"]["fallacy_analysis"]["summary"]["total_fallacies"] for r in results
    )
