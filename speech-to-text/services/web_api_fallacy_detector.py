...
class WebAPIFallacyDetector:
    ...
    def detect_fallacies_web_api(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Successfully detected {len(result.get('fallacies', []))} fallacies"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self._create_error_response(
                    f"Analysis API error: {response.status_code}"
                )
    ...
    def _standardize_analysis_response(
        ...
        fallacy_response = self._standardize_response(
            {"fallacies": fallacies, "text": api_response.get("text", "")}
        )
    ...
    def _generate_recommendations(
        self, fallacy_types: list, total_fallacies: int
    ) -> list:
        """
        Generate recommendations based on detected fallacies
        ...
        if total_fallacies == 0:
            recommendations.append(
                "Excellent logical structure with no fallacies detected"
            )
            return recommendations

        # General recommendation
        recommendations.append(
            "Consider reviewing the logical structure of your arguments"
        )
...
def main():
    ...
    if not detector.check_api_health():
        ...
        print(
            "💡 Run: cd argumentation_analysis/services/web_api && python start_api.py"
        )
        return False
