...
class FallacyDetectionService:
        ...
        if skip_advanced:
            ...
            try:
                ...
                self.informal_agent = InformalAnalysisAgent(
                    kernel=kernel,
                    agent_name="fallacy_detection_service"
                )
        ...
        try:
            ...
            if self.use_advanced_services and self.informal_agent:
                try:
                    ...
                    return self._format_result(result, text, processing_time, "advanced_services", options)
                except Exception as e:
            ...
            processing_time = time.time() - start_time
            return self._format_result(result, text, processing_time, "pattern_matching", options)
            ...
            try:
                ...
                if loop.is_running():
                    with ThreadPoolExecutor() as executor:
                        future = executor.submit(asyncio.run, self._async_advanced_analysis(text))
    ...
    def _pattern_matching_analysis(self, text: str) -> Dict[str, Any]:
        ...
        patterns = {
            ...
            "Appeal to Authority": {
                "keywords": ["expert dit", "scientifique affirme", "selon les spécialistes"],
                "confidence": 0.65
            }
        }
    ...
    def _parse_semantic_analysis(self, analysis_text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in ['fallacy', 'sophisme', 'erreur logique']):
                fallacies.append({
    ...
    def _format_result(self, result: Dict[str, Any], text: str, processing_time: float, 
                      method: str, options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, fallacies: List[Dict[str, Any]], total_fallacies: int) -> List[str]:
        ...
        if "False Dilemma" in fallacy_types:
            recommendations.append("🔍 Look for alternative options beyond binary choices.")
        
        if "Hasty Generalization" in fallacy_types:
            recommendations.append("📊 Be careful with generalizations - seek more evidence.")
        
        if "Appeal to Authority" in fallacy_types:
            recommendations.append("🧐 Question authority claims - ask for concrete evidence.")
