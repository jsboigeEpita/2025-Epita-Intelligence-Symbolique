...
def detect_fallacies():
    ...
    try:
        ...
        if not data or "text" not in data:
            return (
                jsonify(
                    {"status": "error", "error_message": "Field 'text' is required"}
                ),
                400,
            )
        ...
        if not text or not text.strip():
            return (
                jsonify({"status": "error", "error_message": "Text cannot be empty"}),
                400,
            )
        ...
        return (
            jsonify(
                {"status": "error", "error_message": f"Internal server error: {str(e)}"}
            ),
            500,
        )
...
def method_not_allowed(error):
    ...
    return (
        jsonify(
            {"status": "error", "error_message": "Method not allowed for this endpoint"}
        ),
        405,
    )
...
if __name__ == "__main__":
    ...
    print("\n📖 API Documentation:")
    print(
        "   • POST /api/fallacies expects JSON: {'text': 'your text', 'options': {...}}"
    )
    print(
        "   • Returns structured analysis with fallacies, summary, and recommendations"
    )
