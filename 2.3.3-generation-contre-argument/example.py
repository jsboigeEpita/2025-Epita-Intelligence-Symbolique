...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Exemple d'utilisation de l'agent de génération de contre-arguments."
    )
    parser.add_argument(
        "--api-key",
    ...
    config = {
        ...
        "tweety_jar_path": os.path.join(
            os.path.dirname(__file__), "counter_agent", "tweety-full.jar"
        ),
    }
    ...
    for test_arg in test_arguments:
        ...
        print(
            f"- Stratégie rhétorique: {result['counter_argument'].rhetorical_strategy}"
        )
    ...
    print(
        f"- Temps de génération moyen: {summary['average_generation_time']:.3f} secondes"
    )
