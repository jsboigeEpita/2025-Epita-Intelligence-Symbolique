...
class PerformanceMetrics:
    ...
    def record_metrics(
        ...
        self.metrics_history["logical_strength"].append(
            evaluation_result.logical_strength
        )
        ...
        self.counter_type_metrics[counter_type.value].append(
        ...
        logger.debug(
            f"Métriques enregistrées pour {counter_type.value}, score: {evaluation_result.overall_score:.2f}"
        )
    ...
    def get_summary_metrics(self) -> Dict[str, Any]:
        ...
        average_metrics = {
            ...
            "average_logical_strength": np.mean(
                self.metrics_history["logical_strength"]
            ),
    ...
    def export_metrics_report(self) -> str:
        ...
        report = (
            "Rapport de performances de l'agent de génération de contre-arguments\n"
        )
        report += "================================================================\n\n"
        ...
        if summary["worst_counter_type"]:
            ...
            report += (
                f"(score: {summary['worst_counter_type']['average_score']:.3f})\n\n"
            )
...
class MetricsTracker:
    ...
    def stop_tracking(
        self, evaluation_result: EvaluationResult, success: bool = True
    ) -> float:
        """
        ...
        self.metrics.record_metrics(
            evaluation_result, self.current_counter_type, execution_time, success
        )

        logger.debug(
            f"Fin du suivi pour {self.current_counter_type.value}, temps: {execution_time:.3f}s"
        )
