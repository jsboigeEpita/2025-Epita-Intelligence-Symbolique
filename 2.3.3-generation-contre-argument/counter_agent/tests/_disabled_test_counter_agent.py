...
class TestVulnerabilityAnalysis(unittest.TestCase):
    ...
    def test_identify_generalisation_vulnerability(self):
        ...
        self.assertTrue(
            generalisation_found, "Une généralisation abusive aurait dû être détectée"
        )
    ...
    def test_identify_premise_vulnerability(self):
        ...
        self.assertTrue(
            premise_vuln_found, "Une vulnérabilité de prémisse aurait dû être détectée"
        )
...
class TestCounterArgumentGeneration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Analyse préalable
        self.argument = self.agent.analyze_argument(
            self.argument_text, self.mock_taxonomy
        )
    ...
    def test_direct_refutation_generation(self):
        ...
        counter_arg = self.agent.generate_counter_argument(
            self.argument, CounterArgumentType.DIRECT_REFUTATION, self.vulnerabilities
        )

        # Vérifications
        self.assertEqual(
            counter_arg.counter_type, CounterArgumentType.DIRECT_REFUTATION
        )
    ...
    def test_counter_example_generation(self):
        ...
        counter_arg = self.agent.generate_counter_argument(
            self.argument, CounterArgumentType.COUNTER_EXAMPLE, self.vulnerabilities
        )
    ...
    def test_premise_challenge_generation(self):
        ...
        counter_arg = self.agent.generate_counter_argument(
            self.argument, CounterArgumentType.PREMISE_CHALLENGE, self.vulnerabilities
        )

        # Vérifications
        self.assertEqual(
            counter_arg.counter_type, CounterArgumentType.PREMISE_CHALLENGE
        )
        self.assertIsNotNone(counter_arg.counter_content)
        self.assertGreater(len(counter_arg.counter_content), 10)
...
class TestCounterArgumentEvaluation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Argument utilisé pour les tests
        self.argument_text = (
            "Tous les cygnes sont blancs. Donc il n'existe pas de cygnes noirs."
        )
    ...
    def test_evaluation_metrics(self):
        ...
        evaluation = self.agent.evaluate_counter_argument(
            self.argument, self.counter_arg
        )
