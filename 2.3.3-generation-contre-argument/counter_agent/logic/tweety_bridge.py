...
class TweetyBridge:
    ...
    def __init__(self, tweety_jar_path: Optional[str] = None):
        ...
        if tweety_jar_path is None:
            ...
            possible_paths = [
                os.path.join(
                    os.path.dirname(__file__), "..", "..", "libs", "tweety-full.jar"
                ),
                os.path.join(
                ...
                os.path.join(
                    ...
                    "..",
                    "..",
                    "2025-Epita-Intelligence-Symbolique-sujet-2.3.3",
                    "libs",
                    "tweety-full.jar",
                ),
            ]
        ...
        if tweety_jar_path is None or not os.path.exists(tweety_jar_path):
            logger.warning(
                "Fichier JAR de TweetyProject non trouvé. La validation formelle ne sera pas disponible."
            )
        ...
        if not jpype.isJVMStarted():
            ...
            try:
                jpype.startJVM(
                    ...
                    f"-Djava.class.path={self.tweety_jar_path}",
                )
    ...
    def _import_tweety_classes(self):
        ...
        try:
            ...
            from org.tweetyproject.arg.dung.syntax import (
                DungTheory,
                Argument as TweetyArgument,
                Attack,
            )
    ...
    def validate_counter_argument(
        self, original_arg: Argument, counter_arg: CounterArgument
    ) -> Dict[str, Any]:
        """
        ...
        if not self.tweety_available:
            logger.warning(
                "TweetyProject n'est pas disponible. Validation simplifiée utilisée."
            )
        ...
        try:
            ...
            self._add_attack_based_on_type(
                theory, counter_arg, original_argument, counter_argument
            )
            ...
            original_in_grounded = self._is_in_extension(
                original_argument, grounded_extension
            )
            counter_in_grounded = self._is_in_extension(
                counter_argument, grounded_extension
            )

            original_in_complete = any(
                self._is_in_extension(original_argument, ext)
                for ext in complete_extensions
            )
            counter_in_complete = any(
                self._is_in_extension(counter_argument, ext)
                for ext in complete_extensions
            )
            ...
            logger.info(
                f"Validation formelle terminée: is_valid_attack={is_valid_attack}, original_survives={original_survives}, counter_succeeds={counter_succeeds}"
            )
            ...
            return {
                ...
                "extensions": {
                    ...
                    "complete": [
                        self._extension_to_string(ext) for ext in complete_extensions
                    ],
                },
            }
    ...
    def assess_argument_strength(
        self, original_arg: Argument, counter_args: List[CounterArgument]
    ) -> float:
        """
        ...
        if not self.tweety_available or not counter_args:
            logger.warning(
                "TweetyProject n'est pas disponible ou pas de contre-arguments. Évaluation simplifiée utilisée."
            )
        ...
        try:
            ...
            if complete_extensions:
                acceptance_rate = sum(
                    1
                    for ext in complete_extensions
                    if self._is_in_extension(original_argument, ext)
                ) / len(complete_extensions)
            else:
                acceptance_rate = 0.0
            ...
            logger.error(
                f"Erreur lors de l'évaluation de la force avec TweetyProject: {e}"
            )
            return self._fallback_strength_assessment(original_arg, counter_args)
    ...
    def _add_attack_based_on_type(
        self, theory, counter_arg: CounterArgument, original_argument, counter_argument
    ):
        """
    ...
    def _fallback_validation(
        ...
        original_survives = counter_arg.strength in [
            ArgumentStrength.WEAK,
            ArgumentStrength.MODERATE,
        ]
        counter_succeeds = counter_arg.strength in [
            ArgumentStrength.MODERATE,
            ArgumentStrength.STRONG,
            ArgumentStrength.DECISIVE,
        ]
        logical_consistency = True

        logger.info(
            f"Validation fallback terminée: is_valid_attack={is_valid_attack}, original_survives={original_survives}, counter_succeeds={counter_succeeds}"
        )
    ...
    def _fallback_strength_assessment(
        ...
        strong_counter_args = sum(
            1
            for ca in counter_args
            if ca.strength in [ArgumentStrength.STRONG, ArgumentStrength.DECISIVE]
        )
        ...
        strength_factor = (
            1.0 - (strong_counter_args / total_counter_args)
