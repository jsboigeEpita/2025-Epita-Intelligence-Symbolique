...
class LLMGenerator:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Vous êtes un expert en analyse d'arguments. Votre tâche est d'analyser la structure d'un argument.",
                    },
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Vous êtes un expert en analyse d'arguments. Votre tâche est d'identifier les vulnérabilités dans un argument.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
            ...
            content = response.choices[0].message.content
            logger.debug(
                f"Réponse LLM pour l'identification des vulnérabilités: {content[:100]}..."
            )
            ...
            logger.error(
                f"Erreur lors de l'identification des vulnérabilités avec LLM: {e}"
            )
            return []
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Vous êtes un expert en argumentation. Votre tâche est de générer un contre-argument pertinent.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.7,
            ...
            content = response.choices[0].message.content
            logger.debug(
                f"Réponse LLM pour la génération de contre-argument: {content[:100]}..."
            )
            ...
            logger.error(
                f"Erreur lors de la génération de contre-argument avec LLM: {e}"
            )
            return {
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Vous êtes un expert en évaluation d'arguments. Votre tâche est d'évaluer la qualité d'un contre-argument.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
            ...
            content = response.choices[0].message.content
            logger.debug(
                f"Réponse LLM pour l'évaluation de contre-argument: {content[:100]}..."
            )
            ...
            logger.error(
                f"Erreur lors de l'évaluation de contre-argument avec LLM: {e}"
            )
            return {
                "scores": {
                    "relevance": 0.3,
    ...
    async def analyze_argument_async(self, argument_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            content = response.choices[0].message.content
            logger.debug(
                f"Réponse LLM pour l'analyse d'argument (async): {content[:100]}..."
            )
    ...
    async def generate_counter_argument_async(
        ...
        try:
            ...
            content = response.choices[0].message.content
            logger.debug(
                f"Réponse LLM pour la génération de contre-argument (async): {content[:100]}..."
            )
            ...
            logger.error(
                f"Erreur lors de la génération de contre-argument avec LLM (async): {e}"
            )
            return {
                "counter_argument": f"Erreur lors de la génération: {str(e)}",
                "target_component": "error",
