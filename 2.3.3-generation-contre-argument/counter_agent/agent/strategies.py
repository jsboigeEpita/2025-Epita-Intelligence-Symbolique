...
class RhetoricalStrategies:
    ...
    def get_strategy_prompt(self, strategy: RhetoricalStrategy) -> str:
        ...
        return prompts.get(
            strategy,
            "Utilisez la stratégie la plus appropriée pour ce type d'argument.",
        )

    def apply_strategy(
        self, strategy: RhetoricalStrategy, argument: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_from_template(
        self, strategy_info: Dict[str, Any], argument: Argument, context: Dict[str, Any]
    ) -> str:
        """Génère un contre-argument à partir d'un template."""
        templates = strategy_info.get("templates", [])
        if not templates:
            return self._fallback_counter_argument(
                argument, CounterArgumentType.DIRECT_REFUTATION
            )
        ...
        if "{expert}" in template:
            filled_template = filled_template.replace(
                "{expert}", "les experts du domaine"
            )

        if "{reason}" in template:
            filled_template = filled_template.replace(
                "{reason}", "les données empiriques contredisent cette affirmation"
            )

        if "{counter_evidence}" in template:
            filled_template = filled_template.replace(
                "{counter_evidence}", "les preuves disponibles suggèrent le contraire"
            )
        ...
        if "{statistical_evidence}" in template:
            filled_template = filled_template.replace(
                "{statistical_evidence}", self._generate_statistical_counter(argument)
            )
        ...
        if "{alternative_scenario}" in template:
            filled_template = filled_template.replace(
                "{alternative_scenario}", "le contexte était différent"
            )
