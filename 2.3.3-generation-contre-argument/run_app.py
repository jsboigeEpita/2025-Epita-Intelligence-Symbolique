...
if "OPENAI_API_KEY" not in os.environ:
    print(
        "⚠️  Attention: La variable d'environnement OPENAI_API_KEY n'est pas définie."
    )
    print(
        "    Les fonctionnalités utilisant l'API OpenAI ne fonctionneront pas correctement."
    )
...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("counter_agent.log")],
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Lance l'interface web de l'agent de génération de contre-arguments."
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
        help="Hôte sur lequel exécuter l'application (défaut: 127.0.0.1)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=5000,
        help="Port sur lequel exécuter l'application (défaut: 5000)",
    )
    ...
    parser.add_argument(
        "--api-key",
        help="Clé API OpenAI (remplace la variable d'environnement OPENAI_API_KEY)",
    )
