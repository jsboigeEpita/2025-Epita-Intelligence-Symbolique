...
def main():
    parser = argparse.ArgumentParser(description='Agent d\'argumentation abstraite de Dung')
    subparsers = parser.add_subparsers(dest='command', help='Commandes disponibles')
    
    # Commande pour créer un framework interactif
    create_parser = subparsers.add_parser('create', help='Créer un framework interactivement')
    create_parser.add_argument('--enhanced', action='store_true', help='Utiliser l\'agent amélioré')
    
    # Commande pour générer un framework aléatoire
    random_parser = subparsers.add_parser('random', help='Générer un framework aléatoire')
    random_parser.add_argument('--size', type=int, default=5, help='Nombre d\'arguments')
    random_parser.add_argument('--prob', type=float, default=0.3, help='Probabilité d\'attaque')
    ...
    analyze_parser = subparsers.add_parser('analyze', help='Analyser un framework depuis un fichier')
    analyze_parser.add_argument('file', help='Fichier JSON du framework')
    analyze_parser.add_argument('--enhanced', action='store_true', help='Utiliser l\'agent amélioré')
    ...
    examples_parser.add_argument('--list', action='store_true', help='Lister les exemples disponibles')
    ...
    convert_parser.add_argument('--format', choices=['json', 'tgf', 'dot'], default='json', help='Format de sortie')
...
def handle_examples(args):
    ...
    if args.list:
        ...
        for name, agent in examples.items():
            ...
            print(f"- {name}: {props['num_arguments']} arguments, {props['num_attacks']} attaques")
        return
