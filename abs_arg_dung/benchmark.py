...
class ArgumentationBenchmark:
    ...
    def benchmark_semantics_consistency(self, num_frameworks=20):
        ...
        consistency_rate = (
            (total_checks - inconsistencies) / total_checks if total_checks > 0 else 1.0
        )
    ...
    def benchmark_scalability(self, max_size=25, step=5):
        ...
        for size in sizes:
            ...
            try:
                ...
                print(
                    f"Taille {size:2d}: {computation_time:.3f}s, "
    ...
    def benchmark_framework_properties(self, num_samples=50):
        ...
        for i in range(num_samples):
            ...
            density = (
                props["num_attacks"] / (props["num_arguments"] ** 2)
                if props["num_arguments"] > 0
                else 0
            )
        ...
        for key, data in summary.items():
            print(
                f"{key}: μ={data['mean']:.2f} σ={data['std']:.2f} "
                f"[{data['min']}, {data['max']}]"
            )
