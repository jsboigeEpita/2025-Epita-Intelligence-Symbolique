...
class DungAgent:
    ...
    def add_attack(self, source_name: str, target_name: str):
        if source_name in self._arguments and target_name in self._arguments:
            self.af.add(
                self.Attack(self._arguments[source_name], self._arguments[target_name])
            )
            self._invalidate_cache()
        else:
            print(
                f"Erreur : Un ou plusieurs arguments ('{source_name}', '{target_name}') n'existent pas."
            )
    ...
    def _compute_extensions_if_needed(self):
        ...
        if not self._cache_valid:
            ...
            self._cached_extensions = {
                ...
                "admissible": self._format_extensions(
                    self.admissible_reasoner.getModels(self.af)
                ),
                "ideal": sorted(
                    [
                        str(arg.getName())
                        for arg in self.ideal_reasoner.getModel(self.af)
                    ]
                ),
                "semi_stable": self._format_extensions(
                    self.semi_stable_reasoner.getModels(self.af)
                ),
            }
    ...
    def _format_extensions(self, java_collection) -> list:
        return [
            sorted([str(arg.getName()) for arg in extension])
            for extension in java_collection
        ]
    ...
    def visualize_graph(
        self, extension_to_highlight: list = None, title_suffix: str = ""
    ):
        import matplotlib.pyplot as plt
        import networkx as nx
        ...
        if extension_to_highlight is not None:
            node_colors = [
                "lightgreen" if node in extension_to_highlight else "lightcoral"
                for node in G.nodes()
            ]
    ...
    def get_semantics_relationships(self) -> dict:
        ...
        relationships = {
            ...
            "theoretical_checks": {
                ...
                "all_preferred_are_complete": all(
                    pref in complete for pref in preferred
                ),
                "all_stable_are_preferred": all(
                    stable_ext in preferred for stable_ext in stable
                ),
            },
        }
    ...
    def print_all_arguments_status(self):
        ...
        for arg_name, status in all_status.items():
            if "error" in status:
                ...
                print(
                    f"  - Accepté de manière crédule: {status['credulously_accepted']}"
                )
                print(
                    f"  - Accepté de manière sceptique: {status['skeptically_accepted']}"
                )
                print(
                    f"  - Accepté dans l'extension fondée: {status['grounded_accepted']}"
                )
                print(
                    f"  - Accepté dans une extension stable: {status['stable_accepted']}"
                )
    ...
    def get_framework_properties(self) -> dict:
        ...
        G.add_edges_from(
            [(a.getAttacker().getName(), a.getAttacked().getName()) for a in attacks]
        )
...
if __name__ == "__main__":
    ...
    try:
        if not jpype.isJVMStarted():
            ...
            if not libs_dir.exists():
                raise FileNotFoundError(
                    f"Le répertoire des librairies Tweety est introuvable: {libs_dir}"
                )
        ...
        print(
            "Assurez-vous que JAVA_HOME est configuré et que les JARs Tweety sont dans libs/tweety."
        )
