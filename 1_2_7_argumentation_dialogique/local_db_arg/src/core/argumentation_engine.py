...
class ArgumentationEngine:
    ...
    def _load_argumentation_schemes(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "expert_opinion": {
                "premises": ["Expert E says P", "E is expert in domain D", "P is in domain D"],
                "conclusion": "P",
                "strength": 0.8
            },
            "analogy": {
                "premises": ["Case A has property X", "Case B is similar to A", "X is relevant"],
            ...
            "consensus": {
                "premises": ["Majority of experts agree on P", "Experts are qualified", "No systematic bias"],
                "conclusion": "P is likely true",
                "strength": 0.85
            },
            "empirical_evidence": {
                "premises": ["Data shows P", "Data is reliable", "Sample is representative"],
            ...
            "precautionary_principle": {
                "premises": ["Risk R is possible", "R has severe consequences", "Prevention is possible"],
                "conclusion": "Prevention should be taken",
                "strength": 0.7
            },
            "moral_argument": {
                "premises": ["Action A affects group G", "G has rights", "A violates rights"],
                "conclusion": "Action A is morally wrong",
                "strength": 0.8
            },
            "historical_precedent": {
                "premises": ["Situation S occurred before", "S led to outcome O", "Current situation similar to S"],
    ...
    def generate_argument(self, target: Proposition, scheme: str = "modus_ponens") -> Optional[Argument]:
        ...
        related_props = [p for p in self.kb.get_all_propositions() 
                        if target.content.lower() in p.content.lower() or 
                           p.content.lower() in target.content.lower()]
    ...
    def evaluate_argument(self, arg: Argument) -> float:
        ...
        premise_support = sum(1 for p in arg.premises if self.kb.entails(p)) / len(arg.premises) if arg.premises else 0
    ...
    def _are_contradictory(self, prop1: Proposition, prop2: Proposition) -> bool:
        ...
        return (prop1.content == f"¬{prop2.content}" or 
                prop2.content == f"¬{prop1.content}" or
                ("not" in prop1.content.lower() and prop2.content.lower() in prop1.content.lower()) or
                ("not" in prop2.content.lower() and prop1.content.lower() in prop2.content.lower()))
