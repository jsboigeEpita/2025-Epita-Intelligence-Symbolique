...
class DialogueAgent:
    ...
    def _create_move(
        self,
        act: SpeechAct,
        content: Union[str, Proposition, Argument],
    ...
    def process_move(self, move: DialogueMove) -> Optional[DialogueMove]:
        ...
        if response:
            ...
            self.logger.info(
                f"Réponse générée: {response.act.value} - {response.content}"
            )
    ...
    def _is_loop_detected(self) -> bool:
        ...
        if len(my_moves) >= 2:
            ...
            return my_moves[-1].act == my_moves[-2].act and str(
                my_moves[-1].content
            ) == str(my_moves[-2].content)
    ...
    def _break_loop_response(self) -> Optional[DialogueMove]:
        ...
        if props:
            ...
            return self._create_move(
                SpeechAct.CLAIM, f"Considérons ceci: {prop.content}"
            )
        else:
            return self._create_move(
                SpeechAct.UNDERSTAND,
                "Peut-être devrions-nous aborder cela différemment",
            )
    ...
    def _collaborative_response(self, move: DialogueMove) -> Optional[DialogueMove]:
        ...
        if move.act == SpeechAct.QUESTION:
            ...
            if relevant_props:
                ...
                return self._create_move(
                    SpeechAct.CLAIM, f"Selon mes connaissances: {best_prop.content}"
                )
            else:
                return self._create_move(
            ...
            for prop in self.kb.get_all_propositions():
                if any(word in prop.content.lower() for word in topic_words):
                    if (
                        "not" in str(move.content).lower()
                        or "pas" in str(move.content).lower()
                    ):
            ...
            if supporting_props:
                ...
                return self._create_move(
                    SpeechAct.SUPPORT, f"Je suis d'accord. {best_support.content}"
                )
            elif conflicting_props:
                best_conflict = max(conflicting_props, key=lambda p: p.confidence)
                return self._create_move(
                    SpeechAct.CHALLENGE, f"J'ai des réserves. {best_conflict.content}"
                )
            else:
                return self._create_move(
                    SpeechAct.QUESTION, "Pouvez-vous m'expliquer votre raisonnement?"
                )

        elif move.act == SpeechAct.SUPPORT:
            return self._create_move(
                SpeechAct.UNDERSTAND, "Merci pour cette information utile"
            )
            ...
            if relevant_props:
                ...
                return self._create_move(
                    SpeechAct.ARGUE, f"Voici mon argument: {evidence.content}"
                )
            else:
                return self._create_move(
                    SpeechAct.QUESTION, "Qu'est-ce qui vous fait douter?"
                )

        elif move.act == SpeechAct.ARGUE:
            return self._create_move(
                SpeechAct.UNDERSTAND, "C'est un point de vue intéressant"
            )
    ...
    def _skeptical_response(self, move: DialogueMove) -> Optional[DialogueMove]:
        ...
        if move.act == SpeechAct.QUESTION:
            ...
            return self._create_move(
                SpeechAct.QUESTION, random.choice(counter_questions)
            )
            ...
            if counter_props:
                ...
                return self._create_move(
                    SpeechAct.CHALLENGE, f"Mais que dire de ceci: {counter.content}?"
                )
            else:
                challenges = [
            ...
            return self._create_move(
                SpeechAct.CHALLENGE,
                "Ce soutien me semble insuffisant. Avez-vous d'autres arguments?",
            )

        elif move.act == SpeechAct.ARGUE:
            # Cherche des contre-arguments dans sa base
            relevant_props = [
                p for p in self.kb.get_all_propositions() if p.confidence > 0.7
            ]
            if relevant_props:
                counter_arg = random.choice(relevant_props)
                return self._create_move(
                    SpeechAct.REFUTE, f"Je conteste avec ceci: {counter_arg.content}"
                )
            else:
                return self._create_move(
                    SpeechAct.CHALLENGE,
                    "Cet argument n'est pas assez solide pour me convaincre",
                )
            ...
            if random.random() < 0.3:  # 30% de chance de concéder
                return self._create_move(
                    SpeechAct.CONCEDE, "Vous marquez un point intéressant"
                )
            else:
                return self._create_move(
                    SpeechAct.QUESTION,
                    "Pouvez-vous être plus précis dans votre challenge?",
                )
    ...
    def get_dialogue_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "duration": (
                self.dialogue_history[-1].timestamp - self.dialogue_history[0].timestamp
            ).total_seconds(),
