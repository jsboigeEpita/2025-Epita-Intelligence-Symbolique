...
def create_rich_demo_agents(system: MultiAgentDialogueSystem):
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            print(
                "Fichier de configuration enrichie non trouvé, utilisation des agents de base"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Système de Dialogue Argumentatif Walton-Krabbe"
    )
    parser.add_argument("--demo", action="store_true", help="Lance une démonstration")
    parser.add_argument(
        "--rich-demo", action="store_true", help="Lance une démonstration enrichie"
    )
    parser.add_argument(
        "--config", type=str, help="Chemin vers un fichier de configuration"
    )
    parser.add_argument(
        "--topic", type=str, help="Sujet de dialogue pour la démo rapide"
    )
        ...
        try:
            ...
            if len(system.agents) >= 2:
                ...
                dialogue_id = system.create_dialogue(
                    agents_list[0], agents_list[1], topic, DialogueType.INQUIRY
                )
