...
def test_module_import_by_path(module_file_path: Path, module_name_override: Optional[str] = None) -> Tuple[bool, str]:
    """
    ...
    Returns:
        Tuple[bool, str]: Un tuple contenant:
            ...
            - Un message décrivant le résultat.
    """
    if not isinstance(module_file_path, Path) or not module_file_path.is_file() or module_file_path.suffix != '.py':
    ...
    try:
        if str(module_dir) not in sys.path:
            ...
            path_inserted = True
            logger.debug(f"Temporairement ajouté '{module_dir}' à sys.path pour importer '{actual_module_name}'.")
        ...
        if path_inserted and str(module_dir) in sys.path :
            ...
            try:
                ...
                logger.warning(f"Tentative de retirer '{module_dir}' de sys.path, mais non trouvé. sys.path actuel: {sys.path}")
            ...
            logger.debug(f"Le chemin '{module_dir}' semble avoir été ajouté à sys.path par une autre opération, non retiré par cet utilitaire.")
