...
class EducationalConfiguration:
    ...
    def get_difficulty_config(self) -> Dict[str, Any]:
        ...
        configs = {
            ...
            "L2": {
                "complexity": 0.5,
                "concepts": ["sophismes", "logique_prop"],
                "explanation": "medium",
            },
            "L3": {
                "complexity": 0.7,
...
class EducationalConversationLogger:
    ...
    def log_agent_message(
        self, agent: str, message: str, phase: str = "", context: Dict = None
    ):
        ...
        msg = ConversationMessage(
            ...
            context=context
            or {"phase": phase, "educational_level": self.config.student_level},
        )
        ...
        logger.info(
            f"[CONVERSATION-{phase.upper()}] {agent}: {translated_message[:100]}..."
        )

    def log_tool_interaction(
        self, agent: str, tool: str, args: str, result: str, duration_ms: float
    ):
        ...
        msg = ConversationMessage(
            ...
            context={
                "tool": tool,
                "performance": "normal" if duration_ms < 5000 else "slow",
            },
        )
...
class EducationalProjectManager:
    ...
    async def _finalize_educational_analysis(self, results: Dict[str, Any]):
        ...
        successful_agents = len(
            [
                r
                for r in results["agents_results"].values()
                if r.get("status") == "success"
            ]
        )
        self.metrics.pedagogical_effectiveness = (
            successful_agents / len(self.agents) if self.agents else 0
        )
        ...
        results["conversations"] = [
            msg.__dict__ for msg in self.conversation_logger.conversations
        ]
...
class EducationalShowcaseSystem:
    ...
    def _setup_educational_logging(self):
        ...
        logger.info(
            f"Système éducatif initialisé - Mode: {self.config.mode.value}, Niveau: {self.config.student_level}"
        )
    ...
    async def initialize_system(self) -> bool:
        ...
        try:
            ...
            if self.config.enable_real_llm:
                ...
                self.llm_service = create_llm_service(service_id="educational_showcase")
                if self.llm_service:
            ...
            agents_ready = await self.project_manager.initialize_educational_agents(
                self.llm_service
            )
            if agents_ready:
                agent_count = len(self.project_manager.agents)
                print(
                    f"   [OK] {agent_count} agents specialises prets pour le niveau {self.config.student_level}"
                )
            else:
    ...
    async def run_educational_demo(
        ...
        if source_corpus:
            ...
            if not text_content:
                print(
                    "   [WARN] Impossible de charger le texte depuis le corpus. Utilisation d'un texte de fallback."
                )
        ...
        results["session_metrics"] = {
            ...
            "educational_effectiveness": results.get("metrics", {}).get(
        ...
        print(
            f"   - Efficacite pedagogique: {results['session_metrics']['educational_effectiveness']:.0%}"
        )
    ...
    async def _load_text_from_encrypted_corpus(self, corpus_name: str) -> Optional[str]:
        ...
        try:
            ...
            if not source.extracts:
                logger.error(
                    f"La source '{source.source_name}' ne contient aucun extrait."
                )
                return None
            ...
            if not text:
                logger.error(
                    f"L'extrait '{extract.extract_name}' ne contient pas de texte."
                )
                return None
            ...
            logger.info(
                f"Texte sélectionné ({len(text)} cars) depuis la source: {metadata.get('source_name')}"
            )
            return text
        except Exception as e:
            logger.error(
                f"Erreur lors du chargement depuis le corpus chiffré: {e}",
                exc_info=True,
            )
            return None
    ...
    def _select_appropriate_text(self) -> str:
        ...
        text_key = level_mapping.get(
            self.config.student_level, "L2_logique_propositionnelle"
        )
        selected_text = sample_texts.get(
            text_key, sample_texts["L2_logique_propositionnelle"]
        )
    ...
    def _generate_educational_report(
        self, text_analyzed: str, results: Dict[str, Any]
    ) -> str:
        ...
        # Texte analysé
        report_lines.extend(["## Texte Analyse", "", "```", text_analyzed, "```", ""])
        ...
        if agents_results:
            report_lines.extend(["## Analyses des Agents Specialises", ""])
            ...
            for agent_name, agent_result in agents_results.items():
                ...
                if agent_result.get("error"):
                    ...
                    if agent_name == "informal":
                        ...
                        consistency = agent_result.get(
                            "consistency", agent_result.get("modal_consistency", False)
                        )
                        queries_count = agent_result.get(
                            "queries_count", agent_result.get("modal_queries", 0)
                        )
                        report_lines.extend(
                            [
        ...
        if conversations and self.config.enable_conversation_capture:
            ...
            important_conversations = [
                ...
                if conv.get("message_type") == "conversation"
                and not conv.get("message", "").startswith("[TOOL]")
            ][
                :10
        ...
        report_lines.extend(["## Recommandations Pedagogiques", ""])
    ...
    async def save_educational_session(
        ...
        if output_file:
            ...
            report_file = (
                REPORTS_DIR
                / f"educational_report_{self.config.student_level}_{timestamp}.md"
            )
        ...
        data_file = (
            LOGS_DIR
            / f"educational_session_{self.config.student_level}_{timestamp}.json"
        )
        with open(data_file, "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2, ensure_ascii=False, default=str)

        conversations_file = (
            LOGS_DIR / f"conversations_{self.config.student_level}_{timestamp}.json"
        )
...
Exemples d'utilisation:
  ...
  python educational_showcase_system.py --demo-modes
    ...
    parser.add_argument(
        ...
        default="L3",
        help="Niveau etudiant (defaut: L3)",
    )
    parser.add_argument(
        "--mode",
        choices=[mode.value for mode in EducationalMode],
        default="intermediaire",
        help="Mode d'apprentissage (defaut: intermediaire)",
    )
    parser.add_argument(
        "--lang",
        choices=["fr", "en", "es"],
    ...
    parser.add_argument(
        "--source-corpus",
        type=str,
        help="Nom du fichier de corpus chiffre a utiliser (ex: extract_sources.json.gz.enc)",
    )
    parser.add_argument(
        "--output-file",
        type=str,
        help="Chemin du fichier de sortie pour la trace/rapport markdown",
    )
    parser.add_argument(
        "--demo-modes",
        action="store_true",
    ...
    parser.add_argument("--verbose", action="store_true", help="Logging detaille")
    ...
    if args.demo_modes:
        ...
        async def run_custom_session():
            ...
            if await system.initialize_system():
                ...
                report_file = await system.save_educational_session(
