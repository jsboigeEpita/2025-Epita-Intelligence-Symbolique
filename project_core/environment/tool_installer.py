...
def ensure_tools_are_installed(
    ...
    try:
        ...
        for tool_name in tools_to_ensure:
            ...
            if tool_env_var and tool_env_var in installed_tools:
                ...
                local_logger.success(
                    f"{tool_name.upper()} est configuré. {tool_env_var}={tool_path}"
                )
                ...
                local_logger.error(
                    f"L'installation de {tool_name.upper()} a échoué ou n'a pas retourné de chemin."
                )
                all_tools_ok = False
        ...
        local_logger.error(
            f"Une erreur critique est survenue lors de l'appel à setup_tools: {e}",
            exc_info=True,
        )
        return False
...
if __name__ == "__main__":
    ...
    main_logger.info(
        "--- Démonstration de l'installation des outils (JDK et Node.js) ---"
    )
    ...
    success = ensure_tools_are_installed(
        tools_to_ensure=["jdk", "node"], logger_instance=main_logger
    )
    ...
    if success:
        ...
        main_logger.error("\n--- RÉSULTAT DE LA DÉMONSTRATION ---")
        main_logger.error(
            "La démonstration a rencontré des problèmes. Veuillez vérifier les logs."
        )
