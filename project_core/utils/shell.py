...
def run_sync(
    ...
    try:
        ...
        raise ShellCommandError(
            f"Command not found: {command[0]}", -1, "", str(e)
        ) from e
    except subprocess.TimeoutExpired as e:
        logger.error(f"Command timed out after {timeout}s: {command_str}")
        raise ShellCommandError(
            f"Command timed out: {command_str}", -1, e.stdout or "", e.stderr or ""
        ) from e
        ...
        raise ShellCommandError(
            f"An unexpected error occurred: {e}", -1, "", str(e)
        ) from e
...
def _find_conda_executable():
    ...
    try:
        result = subprocess.run(
            ["where", "conda"], capture_output=True, text=True, check=True, shell=True
        )
...
def _get_env_python_executable(env_name: str) -> str:
    ...
    if env_name == "default" or env_name == os.path.basename(sys.prefix):
        logger.info(
            f"Using current Python executable for env '{env_name}': {sys.executable}"
        )
            ...
            for env_path in envs:
                if os.path.basename(env_path) == env_name:
                    ...
                    if os.path.exists(py_exe):
                        logger.info(
                            f"Found Python executable for conda env '{env_name}': {py_exe}"
                        )
                        return py_exe
        except (subprocess.SubprocessError, json.JSONDecodeError, KeyError) as e:
            logger.warning(
                f"Could not list or parse conda environments: {e}. Falling back."
            )
...
def run_in_activated_env(
    ...
    """
    logger.info(
        f"Attempting to run sync command in activated env '{env_name}': {' '.join(command)}"
    )
    try:
        ...
        raise ShellCommandError(
            f"Failed to find environment '{env_name}'.", -1, "", str(e)
        ) from e
...
async def run_in_activated_env_async(
    ...
    """
    logger.info(
        f"Attempting to run async command in activated env '{env_name}': {' '.join(command)}"
    )
    try:
        ...
        return await run_async(command=full_command, cwd=cwd, env=env)
    except FileNotFoundError as e:
        logger.error(f"Could not run in activated environment. Reason: {e}")
        raise ShellCommandError(
            f"Failed to find environment '{env_name}'.", -1, "", str(e)
