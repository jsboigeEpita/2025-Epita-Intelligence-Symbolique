...
class MsvcBuildStrategy(BaseStrategy):
    ...
    def execute(self, package: str, options: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if vcvars_path:
            self.logger.info("La stratégie de compilation MSVC automatique n'est pas encore implémentée.")
            self.logger.info("Pour une installation manuelle, utilisez une 'x64 Native Tools Command Prompt' et exécutez les commandes suivantes:")
            self.logger.info(f'conda activate {self.manager_env.get_conda_env_name_from_dotenv()}')
