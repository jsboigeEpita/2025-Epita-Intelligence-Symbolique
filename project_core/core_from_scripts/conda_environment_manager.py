...
logging.basicConfig(
    level=logging.INFO,
    format="[ENV_MGR] [%(asctime)s] - %(name)s - %(levelname)s - %(message)s",
)
...
class EnvironmentManager:
    ...
    def _get_conda_env_path(self, env_name: str) -> Optional[Path]:
        ...
        try:
            ...
            self.logger.error(
                f"Erreur lors de la recherche de l'environnement Conda: {e}"
            )
            return None
    ...
    def run_command(self, command: List[str]) -> int:
        ...
        is_pytest_command = (
            ...
            and "pytest" in command[2]
        )

        if is_pytest_command:
            pytest_args = command[3:]
            self.logger.info(
                f"Détection de pytest. Exécution par programmation avec les arguments: {pytest_args}"
            )
            try:
                ...
                self.logger.info(
                    f"Pytest s'est terminé avec le code de sortie: {exit_code}"
                )
                return exit_code
            except Exception as e:
                self.logger.error(
                    f"Erreur CRITIQUE lors de l'exécution de pytest.main: {e}"
                )
        ...
        self.logger.info(
            f"Exécution de la commande via un sous-processus: {command_str}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"La commande s'est terminée avec le code de sortie: {result.returncode}"
            )
...
def main():
    ...
    if args.get_python_path:
        ...
        if args.setup_vars:
            logger.info(
                "Argument --setup-vars ignoré (obsolète mais conservé pour compatibilité)."
            )

        # NE PAS initialiser l'environnement applicatif complet pour les tests.
        # C'est la cause du conflit avec pytest.
        logger.info(
            "Bootstrap de l'application intentionnellement ignoré pour l'exécution d'une commande (mode test)."
        )
        ...
        logger.warning(
            "Aucune action spécifiée (ex: --get-python-path ou --run-command). Affichage de l'aide."
        )
        parser.print_help()
