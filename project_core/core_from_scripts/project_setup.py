...
class ProjectSetup:
    ...
    def setup_environment(
        ...
        if env_name == "test":
            ...
            self.logger.error(
                f"Nom d'environnement non reconnu : '{env_name}'. Les choix valides sont 'test', 'dev'."
            )
            return False
    ...
    def install_project(self, requirements_file: str = "requirements.txt") -> bool:
        ...
        self.logger.info(
            f"Étape 2/3 : Installation des dépendances depuis '{requirements_file}'..."
        )
        deps_installed = self.env_manager.fix_dependencies(
            requirements_file=requirements_file, strategy_name="aggressive"
        )
        if not deps_installed:
            self.logger.error(
                f"L'installation des dépendances depuis '{requirements_file}' a échoué. Installation annulée."
            )
            return False
        ...
        self.logger.info(
            "Étape 3/3 : Configuration du PYTHONPATH via le fichier .pth..."
        )
        path_set = self.set_project_path_file()
        if not path_set:
            self.logger.error(
                "La configuration du PYTHONPATH a échoué. L'installation est peut-être incomplète."
            )
            return False
    ...
    def set_project_path_file(self) -> bool:
        ...
        try:
            ...
            if not site_packages_paths:
                self.logger.warning(
                    "site.getsitepackages() n'a retourné aucun chemin. Tentative avec site.getusersitepackages()."
                )
                ...
                if user_site_packages and Path(user_site_packages).is_dir():
                    ...
                    self.logger.error(
                        "Impossible de déterminer le répertoire site-packages, même avec getusersitepackages."
                    )
                    return False
            ...
            if not site_packages.is_dir():
                self.logger.error(
                    f"Le répertoire site-packages trouvé n'existe pas ou n'est pas un répertoire : {site_packages}"
                )
                return False
            ...
            self.logger.info(
                f"Le fichier '{pth_file_path.name}' a été configuré avec succès."
            )
            return True
    ...
    def _setup_test_environment(self, with_mocks: bool = False) -> bool:
        ...
        """
        self.logger.info(
            "Orchestration de la configuration de l'environnement de test..."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Gestionnaire de Setup et Configuration du Projet."
    )
    # Arguments globaux
    ...
    parser_setup = subparsers.add_parser(
        "setup", help="Configurer un environnement de projet ('dev' ou 'test')."
    )
    parser_setup.add_argument(
        ...
        required=True,
        choices=["dev", "test"],
        help="Le type d'environnement à configurer.",
    )
    parser_setup.add_argument(
        "--force",
        action="store_true",
    ...
    parser_install = subparsers.add_parser(
        "install", help="Lancer une installation complète et propre du projet."
    )
