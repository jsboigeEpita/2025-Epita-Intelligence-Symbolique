...
logging.basicConfig(
    level=logging.INFO,
    stream=sys.stdout,
    format="[ENV_MGR] [%(asctime)s] - %(levelname)s - %(message)s",
)
...
class EnvironmentManager:
    ...
    def get_var_from_dotenv(self, var_name: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.warning(
                f"La variable '{var_name}' n'a pas été trouvée dans {self.target_env_file}"
            )
            return None
        except IOError as e:
            self.logger.error(
    ...
    def run_command_in_conda_env(self, command_parts: List[str]) -> int:
        ...
        if command_parts and "pytest" in command_parts[0].lower():
            self.logger.info(
                "Détection de 'pytest'. Configuration de l'environnement Java..."
            )
            java_home_path = self.get_java_home_from_dotenv()
            if java_home_path:
                # S'assurer que le chemin est absolu
                if not os.path.isabs(java_home_path):
                    java_home_path = os.path.normpath(
                        os.path.join(self.project_root, java_home_path)
                    )
                ...
                self.logger.warning(
                    "JAVA_HOME n'a pas été trouvé dans le .env. Les tests dépendants de Java pourraient échouer."
                )
        ...
        if conda_env_path_str:
            ...
            self.logger.info(
                f"Injection des chemins pour l'environnement Conda '{env_name}' dans le PATH."
            )
            path_var = env_vars.get("PATH", "")
            ...
            self.logger.error(
                f"Impossible de trouver le chemin de l'environnement Conda '{env_name}'. L'exécution de la commande risque d'échouer."
            )
        ...
        description = (
            f"Exécution de '{command_str[:70]}...' dans l'environnement courant"
        )
        ...
        if not env_path:
            self.logger.error(
                f"Impossible de trouver le chemin de l'environnement '{env_name}' pour construire le chemin python."
            )
            return 1
        ...
        final_command_parts = [
            part.replace("python", python_exe, 1) if part == "python" else part
    ...
    def switch_environment(self, target_name: str) -> bool:
        ...
        if not source_path.is_file():
            self.logger.error(
                f"Le fichier d'environnement source n'existe pas : {source_path}"
            )
            return False
        try:
            shutil.copy(source_path, self.target_env_file)
            self.logger.info(
                f"L'environnement a été basculé vers '{target_name}'. Fichier '{source_path}' copié vers '{self.target_env_file}'."
            )
            return True
        except IOError as e:
            self.logger.error(
                f"Erreur lors de la copie du fichier d'environnement : {e}"
            )
            return False
    ...
    def _load_strategies(self):
        ...
        if not self.strategies_dir.is_dir():
            self.logger.warning(
                f"Le répertoire des stratégies '{self.strategies_dir}' est introuvable."
            )
            return
        ...
        for _, module_name, _ in pkgutil.iter_modules([str(self.strategies_dir)]):
            ...
            try:
                module = importlib.import_module(
                    f".{module_name}", package=strategies_package
                )
                for attribute_name in dir(module):
                    ...
                    if (
                        ...
                        self.logger.debug(
                            f"Stratégie '{strategy_instance.name}' chargée depuis '{module_name}'."
                        )
            except Exception as e:
                self.logger.error(
                    f"Erreur lors du chargement de la stratégie depuis {module_name}: {e}"
                )
    ...
    def fix_dependencies(
        ...
        if not self._strategies:
            self.logger.warning(
                "Aucune stratégie de réparation n'a été chargée. Tentative d'installation directe via pip."
            )
            if requirements_file:
                if not (self.project_root / requirements_file).is_file():
                    self.logger.error(
                        f"Fichier de requirements introuvable: {requirements_file}"
                    )
                    return False
                ...
                result = self.conda_manager.run_in_conda_env(
                    shlex.split(command), env_name=env_name
                )
                return result.returncode == 0
            else:
                self.logger.error(
                    "Aucune stratégie et aucun fichier de requirements fourni. Impossible de continuer."
                )
                return False

        if packages and requirements_file:
            raise ValueError(
                "Les arguments 'packages' et 'requirements_file' sont mutuellement exclusifs."
            )
        ...
        if requirements_file:
            ...
            command = f"pip install -r {requirements_file}"
            self.logger.info(
                f"Exécution de la réparation depuis le fichier de requirements : {command}"
            )
            import shlex
        ...
        if packages:
            ...
            if not strategy:
                self.logger.error(
                    f"Stratégie inconnue : '{strategy_name}'. Stratégies disponibles : {', '.join(self._strategies.keys())}"
                )
                return False

            self.logger.info(
                f"Lancement de la stratégie de réparation '{strategy_name}' pour : {', '.join(packages)}"
            )
            for package in packages:
                if not strategy.execute(package):
                    self.logger.error(
                        f"Échec de la stratégie '{strategy_name}' pour le paquet '{package}'."
                    )
                    return False
            return True
    ...
    def _run_aggressive_strategy(self, packages: List[str]) -> bool:
        ...
        self.logger.info(
            f"Lancement de la stratégie de réparation AGRESSIVE pour : {', '.join(packages)}"
        )
        ...
        for package in packages:
            ...
            for strategy_name in strategy_order:
                ...
                if not strategy:
                    self.logger.warning(
                        f"Stratégie '{strategy_name}' non trouvée, ignorée."
                    )
                    continue

                if strategy.execute(package):
                    self.logger.info(
                        f"Réparation réussie pour '{package}' avec la stratégie '{strategy_name}'."
                    )
                    success = True
            ...
            if not success:
                self.logger.error(
                    f"Toutes les stratégies agressives ont échoué pour le paquet '{package}'."
                )
                return False
...
if __name__ == "__main__":
    ...
    run_parser = subparsers.add_parser(
        "run",
        help="Exécute une commande dans l'environnement Conda configuré via .env.",
    )
    run_parser.add_argument(
        "command_parts",
        nargs=argparse.REMAINDER,
        help="La commande à exécuter et ses arguments.",
    )

    # --- Commande pour basculer d'environnement ---
    switch_parser = subparsers.add_parser(
        "switch",
        help="Bascule vers un autre environnement .env en copiant le fichier de configuration.",
    )
    switch_parser.add_argument(
        "name",
        help="Le nom de l'environnement à activer (ex: dev, prod). Le fichier correspondant doit exister dans config/environments.",
    )

    # --- Commande pour obtenir le nom de l'environnement ---
    get_name_parser = subparsers.add_parser(
        "get-env-name",
        help="Affiche le nom de l'environnement Conda configuré dans .env.",
    )

    # --- Nouvelle commande pour la réparation des dépendances ---
    fix_parser = subparsers.add_parser(
        "fix", help="Répare les dépendances d'un ou plusieurs paquets."
    )
    ...
    fix_parser.add_argument(
        ...
        "--strategy",
        default="default",
        help="La stratégie de réparation à utiliser (ex: default, aggressive, simple, no-binary).",
    )
    ...
    if args.command == "run":
        if not args.command_parts:
            ...
            if len(command_to_run) == 1 and " " in command_to_run[0]:
                ...
                logger.info(
                    f"La commande '{command_str}' (normalisée) semble être une chaîne unique, division avec shlex."
                )
        ...
        if not manager.fix_dependencies(
            args.packages, args.requirements_file, args.strategy
        ):
            exit_code = 1
    else:
