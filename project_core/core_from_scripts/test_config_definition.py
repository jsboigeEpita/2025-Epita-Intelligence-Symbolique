...
class TestRunner:
    ...
    def _build_pytest_command(self, config: TestConfig, test_paths: List[str]) -> List[str]:
    ...
    def _get_test_paths_for_mode(self, config: TestConfig) -> List[str]:
        ...
        if config.component:
            ...
            for base_path in ["tests/unit", "tests/integration"]:
                component_path = os.path.join(base_path, f"*{config.component.lower()}*")
    ...
    def run_pytest(self, config: TestConfig) -> Dict[str, Any]:
        ...
        try:
            start_time = self.logger.log.__self__.get_time() if hasattr(self.logger.log.__self__, 'get_time') else None
    ...
    def _parse_pytest_output(self, output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if "passed" in line and "in" in line:
                ...
                stats["tests_run"] = stats["passed"] + stats["failed"] + stats["skipped"]
                break
        
        return stats
    
    def run_validation_suite(self, mode: str = "complete", generate_report: bool = False, 
        ...
        script_path = os.path.join(self.project_root, "test_sherlock_watson_synthetic_validation.py")
    ...
    def run_component_tests(self, component: str = None, authentic: bool = False, 
                          fast: bool = False, level: str = "all") -> Dict[str, Any]:
        """
        ...
        config = TestConfig(
            ...
            level=TestLevel(level) if level in [l.value for l in TestLevel] else TestLevel.STANDARD,
            component=component,
            authentic=authentic,
            fast=fast,
...
def run_pytest(test_path: str = "tests", verbose: bool = False, fast: bool = False,
              env_name: str = "projet-is") -> int:
...
def run_python_script(script_path: str, args: List[str] = None, 
                     env_name: str = "projet-is", logger: Logger = None) -> int:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrateur d'exécution des tests"
    )
    ...
    parser.add_argument(
        '--authentic',
        action='store_true',
        help='Mode authentique avec vrais services'
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=str,
        help='Fichier de sortie pour rapport JSON'
    )
