    ...
    def execute(self, package: str, options: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if package.lower() == "jpype1":
            ...
            for version in versions_to_try:
                ...
                self.logger.info(
                    f"Tentative de téléchargement et installation du wheel: {wheel_url}"
                )
                command = f'pip install "{wheel_url}"'
                if self.manager_env.run_command_in_conda_env(command) == 0:
                    self.logger.info(
                        f"[{package}] Succès de l'installation via le wheel précompilé '{wheel_name}'."
                    )
                    return True

        self.logger.warning(
            f"[{package}] Impossible de deviner un wheel pour le paquet '{package}'."
        )
        return False
