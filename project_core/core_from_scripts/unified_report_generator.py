...
try:
    ...
    print("Assurez-vous que le module argumentation_analysis.core.report_generation est disponible")
    sys.exit(1)
...
class ReportTemplate:
    ...
    def render(self, data: Dict[str, Any]) -> str:
        ...
        logger.warning("Utilisation de l'ancienne interface ReportTemplate - migration recommandée vers UnifiedReportGenerator")
...
class UnifiedReportGeneratorLegacy:
    ...
    def generate_report(self,
        ...
        logger.info(f"Génération rapport (compatibilité) - Format: {format_type}, Template: {template}")
        ...
        return self.unified_generator.generate_unified_report(analysis_data, metadata, config)
...
def create_legacy_generator(config_path: Optional[str] = None) -> UnifiedReportGeneratorLegacy:
...
def generate_component_report(component_name: str,
    ...
    config = ReportConfiguration(
        ...
        output_mode="both" if save_file else "console"
    )
...
Modes de compatibilité:
  python unified_report_generator.py --legacy --data results.json  # Utilise l'ancienne interface
    ...
    parser.add_argument("--data", required=True, help="Chemin vers le fichier de données JSON à analyser")
    parser.add_argument("--format", choices=["markdown", "json", "html", "console"],
                       default="markdown", help="Format de sortie du rapport")
    parser.add_argument("--template", default="default", help="Template à utiliser")
    parser.add_argument("--output-mode", choices=["file", "console", "both"],
                       default="file", help="Mode de sortie")
    ...
    parser.add_argument("--component", help="Nom du composant source (pour métadonnées)")
    ...
    try:
        ...
        if args.legacy:
            ...
            config = ReportConfiguration(
                ...
                output_mode=args.output_mode,
                output_directory=Path(args.output_path).parent if args.output_path else None
            )
            
            report_content = generator.generate_unified_report(analysis_data, metadata, config)
