...
class EnhancedRhetoricalResultVisualizer(RhetoricalResultVisualizer):
    ...
    def visualize_argument_network(self, state: Dict[str, Any], output_path: Optional[str] = None) -> str:
        ...
        """
        self.logger.info("Génération d'un graphe des arguments et des sophismes avec networkx")
        ...
        argument_nodes = [node for node, attrs in G.nodes(data=True) if attrs.get("type") == "argument"]
        nx.draw_networkx_nodes(G, pos, nodelist=argument_nodes, node_color="skyblue", node_size=500, alpha=0.8)
        
        # Dessiner les nœuds de sophismes
        fallacy_nodes = [node for node, attrs in G.nodes(data=True) if attrs.get("type") == "fallacy"]
        nx.draw_networkx_nodes(G, pos, nodelist=fallacy_nodes, node_color="salmon", node_size=500, alpha=0.8)
    ...
    def visualize_fallacy_distribution(self, state: Dict[str, Any], output_path: Optional[str] = None) -> str:
        """
        ...
        Returns:
            Chemin vers l'image générée ou message d'erreur
        """
        self.logger.info("Génération d'un diagramme circulaire de la distribution des sophismes")
        ...
        self.logger.info(f"Diagramme de distribution des sophismes sauvegardé dans {output_path}")
        return output_path
    
    def visualize_argument_quality(self, state: Dict[str, Any], output_path: Optional[str] = None) -> str:
        """
        ...
        arg_texts = [identified_arguments[arg_id][:30] + "..." if len(identified_arguments[arg_id]) > 30 else identified_arguments[arg_id] for arg_id in arg_ids]
        ...
        self.logger.info(f"Heatmap de qualité des arguments sauvegardée dans {output_path}")
        return output_path
    
    def visualize_rhetorical_results(self, results: Dict[str, Any], analysis_results: Dict[str, Any], output_dir: Optional[str] = None) -> Dict[str, str]:
        """
        ...
        if not output_dir:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_dir = str(Path(parent_dir) / "results" / "visualizations" / f"analysis_{timestamp}")
        ...
        argument_network_path = self.visualize_argument_network(
            state, 
            output_path=str(Path(output_dir) / "argument_network.png")
        )
        
        fallacy_distribution_path = self.visualize_fallacy_distribution(
            state, 
            output_path=str(Path(output_dir) / "fallacy_distribution.png")
        )
        
        argument_quality_path = self.visualize_argument_quality(
            state, 
            output_path=str(Path(output_dir) / "argument_quality.png")
        )
        ...
        for name, code in mermaid_visualizations.items():
            with open(str(Path(output_dir) / f"{name}.mmd"), "w", encoding="utf-8") as f:
                f.write(code)
    ...
    def generate_enhanced_html_report(self, state: Dict[str, Any], image_paths: Dict[str, str], output_path: Optional[str] = None) -> str:
        """
        ...
        for name, path in image_paths.items():
            if path and not path.startswith("Aucun"):
                ...
                html.append(f"            <img src='{os.path.basename(path)}' alt='{name}'>")
                html.append(f"        </div>")
                html.append(f"    </div>")
        ...
        html.append("        <ul>")
        html.append(f"            <li><strong>Nombre d'arguments identifiés:</strong> {len(state.get('identified_arguments', {}))}</li>")
        html.append(f"            <li><strong>Nombre de sophismes identifiés:</strong> {len(state.get('identified_fallacies', {}))}</li>")
        ...
        if state.get('identified_arguments') and state.get('identified_fallacies'):
            ...
            quality_score = max(0, 10 - (total_fallacies / total_args) * 5) if total_args > 0 else 0
            html.append(f"            <li><strong>Score de qualité global:</strong> {quality_score:.1f}/10</li>")
