...
def test_analyze_context_impact(evaluator_instance):
    ...
    assert (
        context_analysis["domain_type"] == "sciences"
    )  # ou "scientifique" selon l'implémentation
...
def test_calculate_fallacy_severity(evaluator_instance):
    ...
    severity_evaluation = evaluator_instance._calculate_fallacy_severity(
    ...
    assert (
        severity_evaluation["context_text"]
        == "Les experts affirment que ce produit est sûr."
    )
    assert (
        severity_evaluation["base_severity"] == 0.6
    )  # Valeur de base pour "Appel à l'autorité"
    ...
    assert severity_evaluation["severity_level"] in [
        "Faible",
        "Modéré",
        "Élevé",
        "Critique",
...
@pytest.mark.parametrize(
    "context", ["académique", "politique", "commercial", "juridique", "médical"]
)
def test_evaluate_fallacy_severity_with_different_contexts(
    evaluator_instance, test_arguments, context
):
