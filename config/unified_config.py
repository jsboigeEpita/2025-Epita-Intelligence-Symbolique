...
Objectif : Système authentique par défaut avec gpt-5.6-luna et OpenAI.
...
class UnifiedConfig:
    ...
    analysis_modes: List[str] = field(
        default_factory=lambda: ["fallacies", "coherence", "semantic"]
    )
    enable_advanced_tools: bool = True
    enable_jvm: bool = True
    require_real_llm: bool = (
        True  # Force l'utilisation de vrais LLM - AUTHENTIQUE PAR DÉFAUT
    )
    ...
    require_real_tweety: bool = (
        True  # Force Tweety JAR authentique - AUTHENTIQUE PAR DÉFAUT
    )
    require_full_taxonomy: bool = (
        True  # Force taxonomie 1000 nœuds - AUTHENTIQUE PAR DÉFAUT
    )
    validate_tool_calls: bool = (
        True  # Valide l'authenticité des tool calls - AUTHENTIQUE PAR DÉFAUT
    )
                ...
                if isinstance(value, str):
                    try:
                        ...
                        for member in f.type:
                            ...
                            raise ValueError(
                                f"'{value}' n'est pas une valeur valide pour {f.type.__name__}"
                            )
    ...
    def _validate_configuration(self):
        ...
        if self.mock_level != MockLevel.NONE and (
            self.require_real_gpt
            or self.require_real_tweety
            or self.require_full_taxonomy
        ):
            raise ValueError(
                "Configuration incohérente: mock_level doit être NONE si des composants réels sont requis."
            )
    ...
    def _normalize_agent_list(self):
        ...
        if self.logic_type in [LogicType.FOL, LogicType.FIRST_ORDER]:
            self.agents = [
                AgentType.FOL_LOGIC if a == AgentType.LOGIC else a for a in self.agents
            ]
...
def load_config_from_env() -> UnifiedConfig:
    ...
    config.require_real_gpt = (
        os.getenv("UNIFIED_REQUIRE_REAL_GPT", "true").lower() == "true"
    )
    config.require_real_tweety = (
        os.getenv("UNIFIED_REQUIRE_REAL_TWEETY", "true").lower() == "true"
    )
    config.use_mock_llm = os.getenv("UNIFIED_USE_MOCK_LLM", "false").lower() == "true"
    config.use_authentic_llm = (
        os.getenv("UNIFIED_USE_AUTHENTIC_LLM", "true").lower() == "true"
    )
    config.use_mock_services = (
        os.getenv("UNIFIED_USE_MOCK_SERVICES", "false").lower() == "true"
    )
    config.use_authentic_services = (
        os.getenv("UNIFIED_USE_AUTHENTIC_SERVICES", "true").lower() == "true"
    )
...
def validate_config(config: UnifiedConfig) -> List[str]:
    ...
    if config.use_mock_llm and config.use_authentic_llm:
        errors.append(
            "use_mock_llm et use_authentic_llm ne peuvent pas être tous les deux True"
        )
    ...
    if config.default_provider != "openai":
        errors.append(
            f"default_provider devrait être 'openai' pour l'authenticité, trouvé: {config.default_provider}"
        )
