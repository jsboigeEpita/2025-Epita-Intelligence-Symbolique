...
def test_custom_data_processor():
    ...
    try:
        ...
        if 'sophistries_detected' not in result:
            ...
            print(f"[OK] Analyse sophistique : {len(result['sophistries_detected'])} detections")
        
        # Verification du traitement adaptatif dans processing_metadata
        adaptive_analysis = result.get('processing_metadata', {}).get('adaptive_analysis', False)
        real_processing = result.get('processing_metadata', {}).get('real_processing', False)
        ...
        if validation_passed:
            ...
            print(f"- Traitement reel : {result.get('processing_metadata', {}).get('real_processing', 'N/A')}")
            print(f"- Analyse adaptive : {result.get('processing_metadata', {}).get('adaptive_analysis', 'N/A')}")
            print(f"- Marqueurs EPITA : {len(result.get('markers_found', []))}")
            print(f"- Detections sophistique : {len(result.get('sophistries_detected', []))}")
            print(f"- Mock utilise : {result.get('processing_metadata', {}).get('mock_used', 'N/A')}")
            return True
        else:
