...
def test_pipeline_enhanced():
    ...
    try:
        ...
        # Test du pipeline Enhanced
        run_enhanced_rhetoric_pipeline(
            extract_definitions=extracts,
            base_results=base_results,
            output_file=output_path,
            orchestrator_type="conversation",  # Utiliser l'orchestrateur conversationnel
            use_real_tools=False,  # Utiliser les mocks pour le test
            enhanced=True  # Mode Enhanced activé
        )
        ...
        assert isinstance(results, list), "Les résultats Enhanced doivent être une liste"
        assert len(results) > 0, "Les résultats Enhanced ne doivent pas être vides"
        
        # Vérifier les améliorations Enhanced
        first_result = results[0]
        assert "enhanced_analysis" in first_result, "L'analyse Enhanced manque"
