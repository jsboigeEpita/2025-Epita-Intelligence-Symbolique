...
class LocalLlm():
    ...
    def _prompt_model(self, model_name: str, prompt: str, max_tokens: int, use_internal_prompt: bool) -> str:
        full_prompt = prompt
    ...
    def __call__(self, model: ModelEnum, prompt: str, max_tokens: int = 300, use_internal_prompt: bool = True) -> str:
        if model.name not in self.models.keys():
            print(f"The requested model has not been loaded: {model.name}.", file=stderr)
        else:
            print(f"Prompting...")
            return self._prompt_model(model.name, prompt, max_tokens, use_internal_prompt)
