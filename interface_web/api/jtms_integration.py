...
class APIError(Exception):
    ...
    def __init__(
        self, message: str, error_code: str = "GENERIC_ERROR", status_code: int = 500
    ):
...
class JTMSIntegrationAPI:
    ...
    async def create_session(
        self, session_id: str = None, name: str = None, description: str = ""
    ) -> SessionResponse:
        """
        ...
        try:
            ...
            raise APIError(
                f"Erreur création session: {str(e)}", "SESSION_CREATION_ERROR"
            )
    ...
    async def get_sessions(self) -> List[SessionResponse]:
        ...
        try:
            ...
            if self.session_manager:
                ...
                for session_data in backend_sessions:
                    session = SessionResponse(
                        ...
                        last_modified=session_data.get(
                            "last_modified", datetime.now().isoformat()
                        ),
            ...
            raise APIError(
                f"Erreur récupération sessions: {str(e)}", "SESSIONS_FETCH_ERROR"
            )
    ...
    async def get_session(self, session_id: str) -> SessionResponse:
        ...
        try:
            if self.session_manager:
                ...
                if not session_data:
                    raise APIError(
                        f"Session {session_id} non trouvée", "SESSION_NOT_FOUND", 404
                    )
            ...
            raise APIError(
                f"Erreur récupération session: {str(e)}", "SESSION_FETCH_ERROR"
            )
    ...
    async def add_belief(
        ...
        session_id: str,
        belief_name: str,
        initial_status: BeliefStatus = BeliefStatus.UNKNOWN,
    ) -> BeliefResponse:
        """
        ...
        try:
            ...
            if self.jtms_service:
                await self.jtms_service.add_belief(
                    session_id, belief_name, initial_status.value
                )
    ...
    async def get_beliefs(self, session_id: str) -> List[BeliefResponse]:
        ...
        try:
            ...
            if self.jtms_service:
                ...
                for belief_data in backend_beliefs:
                    belief = BeliefResponse(
                        ...
                        session_id=session_id,
                        created_at=belief_data.get(
                            "created_at", datetime.now().isoformat()
                        ),
                        updated_at=belief_data.get(
                            "updated_at", datetime.now().isoformat()
                        ),
            ...
            raise APIError(
                f"Erreur récupération croyances: {str(e)}", "BELIEFS_FETCH_ERROR"
            )
    ...
    async def get_network_visualization(
        ...
        try:
            ...
            if self.jtms_service:
                try:
                    justifications = await self.jtms_service.get_justifications(
                        session_id
                    )
            ...
            statistics = {
                ...
                "valid_beliefs": sum(
                    1 for b in beliefs if b.status == BeliefStatus.VALID
                ),
                "invalid_beliefs": sum(
                    1 for b in beliefs if b.status == BeliefStatus.INVALID
                ),
                "unknown_beliefs": sum(
                    1 for b in beliefs if b.status == BeliefStatus.UNKNOWN
                ),
            ...
            raise APIError(
                f"Erreur génération visualisation: {str(e)}", "VISUALIZATION_ERROR"
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self.jtms_service:
                ...
                test_result = (
                    await self.jtms_service.health_check()
                    if hasattr(self.jtms_service, "health_check")
                    else True
                )
                health_status["services"]["jtms_service"] = (
...
def initialize_integration_api(
    jtms_service=None, session_manager=None, websocket_manager=None
):
    """Initialise l'API d'intégration avec les services fournis."""
    global integration_api
    integration_api = JTMSIntegrationAPI(
        jtms_service, session_manager, websocket_manager
    )
