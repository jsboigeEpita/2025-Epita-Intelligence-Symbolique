    ...
    try:
        if not JTMS_AVAILABLE:
            return render_template(
                "jtms/error.html", error="Services JTMS non disponibles"
            )
    ...
    try:
        ...
        examples = [
            ...
            },
            {
                ...
                "justifications": [
                    {
                        "in_list": ["weapon_found", "motive_exists"],
                        "out_list": ["alibi_verified"],
                        "conclusion": "suspect_guilty",
                    }
                ],
            },
        ]
...
def get_session_data(session_id):
    ...
    try:
        ...
        invalid_count = sum(
            1 for b in session_data.beliefs.values() if b.valid is False
        )
        unknown_count = sum(
            1
            for b in session_data.beliefs.values()
            if b.valid is None and not b.non_monotonic
        )
        nonmonotonic_count = sum(
            1 for b in session_data.beliefs.values() if b.non_monotonic
        )
...
def get_belief_details(belief_name):
    ...
    try:
        ...
        explanation = (
            jtms_service.explain_belief(belief_name, session_id)
            if hasattr(jtms_service, "explain_belief")
            else "Explication non disponible"
        )
            ...
            "justifications": [
                {
                    "in_list": [str(b) for b in j.in_list],
                    "out_list": [str(b) for b in j.out_list],
                    "valid": all(b.valid for b in j.in_list)
                    and not any(b.valid for b in j.out_list),
                }
                for j in belief.justifications
            ],
            "implications": [str(j.conclusion) for j in belief.implications],
...
def create_session():
    ...
    try:
        ...
        created_session_id = session_manager.create_session(
            session_id, session_name, description
        )
...
def jtms_not_found(error):
    ...
    return render_template("jtms/error.html", error="Page JTMS introuvable"), 404
...
def jtms_internal_error(error):
    ...
    return (
        render_template("jtms/error.html", error="Erreur interne du système JTMS"),
        500,
    )
