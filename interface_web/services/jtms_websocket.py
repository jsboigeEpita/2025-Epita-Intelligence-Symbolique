...
class JTMSWebSocketManager:
    ...
    async def register_client(self, client_id: str, websocket) -> WebSocketClient:
        ...
        welcome_message = WebSocketMessage(
            ...
            session_id="system",
            data={
                "message": "Connexion WebSocket JTMS établie",
                "client_id": client_id,
            },
    ...
    def broadcast_to_session(
        self, session_id: str, message_type: MessageType, data: Dict[str, Any]
    ):
    ...
    def broadcast_belief_added(
        self, session_id: str, belief_name: str, belief_data: Dict[str, Any]
    ):
        """Diffuse l'ajout d'une croyance."""
        self.broadcast_to_session(
            session_id,
            MessageType.BELIEF_ADDED,
            {"belief_name": belief_name, "belief_data": belief_data, "action": "add"},
        )

    def broadcast_belief_updated(
        self, session_id: str, belief_name: str, old_status: str, new_status: str
    ):
    ...
    def broadcast_justification_added(
        self, session_id: str, justification_data: Dict[str, Any]
    ):
        """Diffuse l'ajout d'une justification."""
        self.broadcast_to_session(
            session_id,
            MessageType.JUSTIFICATION_ADDED,
            {"justification_data": justification_data, "action": "add"},
        )
    ...
    def broadcast_network_updated(self, session_id: str, network_data: Dict[str, Any]):
        ...
        self.broadcast_to_session(
            ...
            {"network_data": network_data, "action": "full_update"},
        )

    def broadcast_consistency_result(
        self, session_id: str, is_consistent: bool, conflicts: List[Dict]
    ):
    ...
    async def _send_to_clients(self, message: WebSocketMessage):
        ...
        for client_id, client in self.clients.items():
            try:
                ...
                if message.session_id == "system" or client.is_subscribed_to_session(
                    message.session_id
                ):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                session_id = kwargs.get("session_id") or getattr(
                    args[0], "current_session_id", "default"
                )

                if operation_type == "add" and len(args) > 1:
                    belief_name = (
                        args[1] if len(args) > 1 else kwargs.get("belief_name")
                    )
                    if belief_name:
            ...
            def wrapper(*args, **kwargs):
                ...
                if len(args) > 1:
                    self.ws_manager.broadcast_justification_added(
                        session_id,
                        {"conclusion": args[1] if len(args) > 1 else "unknown"},
                    )
...
def setup_websocket_callbacks(jtms_service=None):
    ...
    def on_belief_change(
        belief_name: str, old_status: str, new_status: str, session_id: str
    ):
        """Callback appelé lors du changement d'une croyance."""
        websocket_manager.broadcast_belief_updated(
            session_id, belief_name, old_status, new_status
        )
