...
def demo_error_analysis():
    ...
    print(f"✅ Corrections proposées: {len(feedback.corrections)} corrections")
    print()
    
    # Générer le feedback constructif
    feedback_message = analyzer.generate_bnf_feedback_message(feedback, 1)
    
    print("📝 FEEDBACK BNF GÉNÉRÉ POUR L'AGENT:")
    print("-" * 50)
    # Afficher les premières lignes du feedback
    lines = feedback_message.split('\n')[:15]
    for line in lines:
        print(f"   {line}")
    print("   [...Message complet de correction disponible...]")
    print()
    
    print("🎯 IMPACT DE LA CORRECTION INTELLIGENTE:")
    print("-" * 50)
    print("✅ L'agent reçoit un feedback BNF spécifique au lieu d'une erreur générique")
    print("✅ Les règles de syntaxe Tweety sont expliquées clairement")
    print("✅ Des exemples de correction sont fournis")
    print("✅ Le prompt de la tentative suivante sera enrichi avec ce feedback")
    print("✅ L'apprentissage progressif remplace la répétition aveugle")
    print()
...
def demo_progressive_learning():
    """Démonstration de l'apprentissage progressif entre tentatives."""
    print("🔄 SIMULATION DE L'APPRENTISSAGE PROGRESSIF:")
    print("-" * 50)
    
    analyzer = TweetyErrorAnalyzer()
    
    # Simuler plusieurs erreurs différentes
    errors = [
        "Predicate 'constantanalyser_faits_rigueur' has not been declared",
        "JSON structure invalid: missing key 'modal_formulas'", 
    ...
    for attempt, error in enumerate(errors, 1):
        print(f"\nTentative {attempt}: {error}")
        
        feedback = analyzer.analyze_error(error, {"attempt": attempt})
