...
## Modifications Récentes (Mai 2025)

Cette section présente les modifications récentes apportées au projet, notamment les améliorations de structure, les optimisations et les corrections de bugs.

### Système de Gestion des Données Chiffrées

Le projet utilise désormais un système robuste de chiffrement pour protéger les données sensibles:

- **Fichiers chiffrés**: Les configurations et sources d'extraits sont stockées dans des fichiers `.gz.enc` chiffrés avec AES-256
- **Gestion des clés**: La clé de chiffrement est dérivée d'une phrase secrète stockée dans la variable d'environnement `TEXT_CONFIG_PASSPHRASE`
- **Scripts de gestion**:
  - [`create_complete_encrypted_config.py`](./argumentiation_analysis/create_complete_encrypted_config.py): Crée un fichier de configuration chiffré
  - [`load_complete_encrypted_config.py`](./argumentiation_analysis/load_complete_encrypted_config.py): Charge et déchiffre les configurations
  - [`cleanup_after_encryption.py`](./argumentiation_analysis/cleanup_after_encryption.py): Nettoie les fichiers temporaires après chiffrement

### Optimisation du Chargeur de Taxonomie

Le chargeur de taxonomie a été amélioré pour faciliter les tests et le développement:

- **Mode mock**: Utilise des données simulées pour éviter les téléchargements et dépendances externes
- **Contrôle par variable**: La variable globale `USE_MOCK` permet de basculer entre les modes mock et réel
- **Documentation améliorée**: Chaque fonction est maintenant documentée avec ses cas d'utilisation et limitations

Pour utiliser la version mock (recommandée pour les tests):
```python
from argumentiation_analysis.utils.taxonomy_loader import TaxonomyLoader

# La version mock est activée par défaut
loader = TaxonomyLoader()
taxonomy = loader.load_taxonomy()
```

Pour utiliser la version réelle (nécessite le fichier de taxonomie):
```python
import argumentiation_analysis.utils.taxonomy_loader as tax_loader

# Désactiver le mode mock
tax_loader.USE_MOCK = False

# Charger la taxonomie réelle
loader = tax_loader.TaxonomyLoader()
taxonomy = loader.load_taxonomy()
```

### Script de Nettoyage du Projet

Un nouveau script de nettoyage automatique a été ajouté pour maintenir la propreté du projet:

- **[`cleanup_project.py`](./cleanup_project.py)**: Script principal de nettoyage qui:
  - Supprime les fichiers temporaires Python (`.pyc`, `__pycache__`, etc.)
  - Nettoie les fichiers de logs obsolètes
  - Vérifie l'existence du répertoire `data`
  - Met à jour le fichier `.gitignore`
  - Vérifie que les fichiers sensibles ne sont pas suivis par Git

Pour exécuter le script de nettoyage:
```bash
...
- **[`CHANGELOG.md`](./CHANGELOG.md)** : Journal des modifications apportées au projet.
