...
from argumentation_analysis.core.jvm_setup import (
    find_valid_java_home,
    _build_effective_classpath,
)
from argumentation_analysis.paths import (
    LIBS_DIR,
    PROJECT_ROOT_DIR,
)  # Pour les chemins par défaut
...
def main():
    ...
    try:
        ...
        if not java_home_selected:
            print(
                "Chemin JAVA_HOME valide non trouvé. Vérifiez votre configuration Java."
            )
            return
        ...
        if _system == "Windows":
            ...
            if (
                not _potential_jvm_dll.is_file()
                and (
                    _java_home_path
                    / "Contents"
                    / "Home"
        ...
        if _potential_jvm_dll and _potential_jvm_dll.is_file():
            ...
            print(
                f"Chemin JVM spécifique non trouvé dans {java_home_selected}, utilisation du chemin par défaut de JPype: {jvm_dll_path}"
            )
        ...
        project_root_for_classpath = (
            script_dir.parent
        )  # Remonte de tests/ à la racine du projet
        ...
        if not combined_jar_list:
            print(
                "Classpath Tweety non construit ou vide. Assurez-vous que les JARs sont dans le répertoire libs/."
            )
            return
