...
def test_env(tmp_path, crypto_service_instance, sample_extract_data):
    ...
    encrypted_content = crypto_service_instance.encrypt_data(
        compressed_data
    )  # Uses key (bytes)
...
def test_load_definitions_no_file(test_env):
    ...
    definitions = load_extract_definitions(
        config_file=non_existent_file, b64_derived_key=test_env["key"].decode("utf-8")
    )
    assert definitions is not None
...
def test_load_definitions_encrypted_no_key(test_env):
    ...
    with pytest.raises(json.JSONDecodeError):
        load_extract_definitions(
            config_file=test_env["encrypted_definitions_file"], b64_derived_key=None
        )
...
def test_save_definitions_encrypted(test_env):
    ...
    save_extract_definitions(
        definitions_obj.to_dict_list(),
        config_file=new_encrypted_file,
        b64_derived_key=new_key.decode("utf-8"),
    )
    ...
    decrypted_data_bytes = test_env["crypto_service"].decrypt_data(
        encrypted_data_read, new_key
    )
...
def test_load_default_if_path_none(test_env):
    ...
    definitions = load_extract_definitions(
        config_file=non_existent_default_path, b64_derived_key=None
    )
    assert definitions is not None
...
def test_load_malformed_json(test_env):
    ...
    with pytest.raises(InvalidToken):
        load_extract_definitions(
            config_file=malformed_encrypted_file,
            b64_derived_key=test_env["key"].decode("utf-8"),
            raise_on_decrypt_error=True,
        )
