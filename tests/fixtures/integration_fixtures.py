    ...
    try:
        ...
        if not _REAL_JPYPE_MODULE.isJVMStarted():
            ...
            if not jvm_path_to_use:
                logger.warning("integration_fixtures.py: Tentative de fallback avec _REAL_JPYPE_MODULE.getDefaultJVMPath().")
                try:
                    jvm_path_to_use = _REAL_JPYPE_MODULE.getDefaultJVMPath()
                    logger.info(f"integration_fixtures.py: _REAL_JPYPE_MODULE.getDefaultJVMPath() a retourné: {jvm_path_to_use}")
                except _REAL_JPYPE_MODULE.JVMNotFoundException as e:
            ...
            if not os.path.isdir(tweety_libs_path):
                ...
                pytest.skip(f"Répertoire {tweety_libs_path} non trouvé.")
                return None
            ...
            if not os.path.isfile(full_jar_path):
                logger.error(f"integration_fixtures.py: JAR principal {full_jar_name} non trouvé dans {tweety_libs_path}")
                pytest.skip(f"JAR principal {full_jar_name} non trouvé dans {tweety_libs_path}.")
                return None
            
            tweety_jars = [full_jar_path] # Utiliser uniquement le JAR "full"
            ...
            try:
                logger.info("integration_fixtures.py: Ajout de l'option JVM '-Xcheck:jni' pour le débogage.")
                _REAL_JPYPE_MODULE.startJVM(jvm_path_to_use, classpath_arg, "-Xcheck:jni", convertStrings=False)
                logger.info("integration_fixtures.py: JVM démarrée avec succès (avec -Xcheck:jni).")
        ...
        if hasattr(_REAL_JPYPE_MODULE, 'config') and _REAL_JPYPE_MODULE.config is not None:
            ...
            logger.info(f"integration_fixtures.py: _REAL_JPYPE_MODULE.config.destroy_jvm mis à False.")
        else:
            logger.warning("integration_fixtures.py: _REAL_JPYPE_MODULE.config non disponible, impossible de régler destroy_jvm.")

        yield _REAL_JPYPE_MODULE
