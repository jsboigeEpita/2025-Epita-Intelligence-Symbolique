...
def generate_text_with_fallacies(fallacy_types=None, num_fallacies=3):
    ...
    selected_types = random.sample(
        fallacy_types, min(num_fallacies, len(fallacy_types))
    )
...
def generate_task(task_type="extraction", objective_id=None):
    ...
    task_templates = {
        ...
        "analysis": {
            ...
            "parameters": {"analysis_type": "fallacy", "output_format": "json"},
        },
        "report": {
            "id": task_id,
            ...
            "parameters": {"report_type": "summary", "output_format": "json"},
        },
    }
...
def generate_analysis_result(fallacy_types=None, num_fallacies=3):
    ...
    for fallacy_type in selected_types:
        ...
        fallacies.append({"type": fallacy_type, "text": text, "confidence": confidence})
...
def generate_enhanced_analysis_result(fallacy_types=None, num_fallacies=3):
    ...
    for fallacy in fallacies:
        ...
        severity_evaluation.append(
            {"type": fallacy_type, "severity": severity, "impact": impact}
        )

    # Générer l'analyse rhétorique
    rhetorical_analysis = {
        "tone": random.choice(
            ["persuasif", "informatif", "émotionnel", "agressif", "défensif"]
        ),
        "style": random.choice(
            ["formel", "informel", "académique", "journalistique", "polémique"]
        ),
