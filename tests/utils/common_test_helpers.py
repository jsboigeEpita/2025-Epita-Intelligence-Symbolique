...
def compare_json_objects(obj1, obj2, ignore_keys=None):
    ...
    if isinstance(obj1, dict) and isinstance(obj2, dict):
        if set(k for k in obj1 if k not in ignore_keys) != set(
            k for k in obj2 if k not in ignore_keys
        ):
            return False
        ...
        return all(
            compare_json_objects(o1, o2, ignore_keys) for o1, o2 in zip(obj1, obj2)
        )
...
def setup_test_environment():
    ...
    dependencies_available = {"numpy": False, "pandas": False, "jpype": False}
    ...
    return {"project_dir": parent_dir, "dependencies_available": dependencies_available}
...
def run_with_real_dependencies(func, *args, **kwargs):
    ...
    try:
        ...
        logger.warning(
            "Les dépendances numpy et pandas ne sont pas disponibles. Le test sera ignoré."
        )
