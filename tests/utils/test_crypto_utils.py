...
def test_derive_key_valid_passphrase():
    ...
    assert (
        key is not None
    ), "La clé ne devrait pas être None pour une phrase secrète valide."
    ...
    assert (
        len(decoded_key) == 32
    ), "La clé décodée devrait avoir une longueur de 32 bytes."
...
def test_derive_key_different_passphrases():
    ...
    assert (
        key1 != key2
    ), "Des phrases secrètes différentes devraient produire des clés différentes."
...
def test_derive_key_empty_passphrase():
    ...
    assert (
        key is None
    ), "La dérivation avec une phrase secrète vide devrait retourner None."
...
def test_derive_key_known_value():
    ...
    key = derive_encryption_key(TEST_PASSPHRASE_VALID)
    assert (
        key == EXPECTED_KEY_FOR_VALID_PASSPHRASE
...
EXPECTED_KEY_FOR_SIMPLE_PASSPHRASE = derive_encryption_key(
    TEST_PASSPHRASE_SIMPLE
)  # Pré-calculer pour comparaison
...
def test_load_key_no_source():
    ...
    with mock.patch(
        "argumentation_analysis.core.utils.crypto_utils.settings.passphrase", None
    ):
        key = load_encryption_key(passphrase_arg=None)
        assert key is None
...
def test_load_key_invalid_arg_valid_env():
    ...
    with mock.patch(
        ...
        assert (
            key is not None
        ), "Devrait utiliser la clé des settings si l'arg est invalide."
        assert key == EXPECTED_KEY_FOR_SIMPLE_PASSPHRASE
...
def test_load_key_arg_none_invalid_env():
    ...
    with mock.patch(
        ...
        assert (
            key is None
        ), "Devrait retourner None si les settings sont invalides et pas d'arg."
...
def test_fernet_encrypt_decrypt_cycle():
    ...
    assert (
        FERNET_KEY_B64_BYTES is not None
    ), "La dérivation de la clé b64 bytes pour Fernet ne devrait pas échouer."
    ...
    assert (
        len(decoded_key_for_check) == 32
    ), "La clé Fernet (b64 bytes) décodée doit faire 32 bytes."

    encrypted_data = encrypt_data_with_fernet(
        TEST_DATA_TO_ENCRYPT, FERNET_KEY_B64_BYTES
    )
    assert (
        encrypted_data is not None
    ), "Le chiffrement Fernet ne devrait pas retourner None avec une clé valide."
    assert (
        encrypted_data != TEST_DATA_TO_ENCRYPT
    ), "Les données chiffrées doivent être différentes des originales."

    decrypted_data = decrypt_data_with_fernet(encrypted_data, FERNET_KEY_B64_BYTES)
    assert (
        decrypted_data is not None
    ), "Le déchiffrement Fernet ne devrait pas retourner None avec une clé valide."
    assert (
        decrypted_data == TEST_DATA_TO_ENCRYPT
    ), "Les données déchiffrées doivent correspondre aux originales."
...
def test_fernet_decrypt_with_wrong_key():
    ...
    ), "Le déchiffrement avec une mauvaise clé Fernet devrait retourner None."


def test_fernet_encrypt_no_key():
    """Teste le chiffrement Fernet avec une clé None."""
    encrypted_data = encrypt_data_with_fernet(
        TEST_DATA_TO_ENCRYPT, None
    )  # Passe None comme b64_encoded_key_str
    assert (
        encrypted_data is None
    ), "Le chiffrement Fernet avec une clé None devrait retourner None."
...
def test_fernet_decrypt_no_key():
    ...
    decrypted_data = decrypt_data_with_fernet(
        encrypted_data, None
    )  # Passe None comme b64_encoded_key_bytes
    assert (
        decrypted_data is None
    ), "Le déchiffrement Fernet avec une clé None devrait retourner None."
...
def test_fernet_decrypt_tampered_data():
    ...
    assert (
        decrypted_data is None
    ), "Le déchiffrement Fernet avec des données altérées devrait retourner None (InvalidToken/Signature)."
