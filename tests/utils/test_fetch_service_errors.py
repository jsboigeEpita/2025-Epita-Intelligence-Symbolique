    ...
    def fetch_url(self, url, retry_count=0):
        ...
        try:
            ...
            if "Failed to resolve" in str(e):
                ...
                return (
                    None,
                    f"Erreur de connexion lors de la récupération de '{url}': {e}",
                )
        ...
        ) as e:  # SSLError est un sous-type de RequestException
...
def test_connection_timeout(fetch_service):
    ...
    with patch("requests.get") as mock_get:
        ...
        assert (
            error
            == f"Timeout après {fetch_service.max_retries} tentatives pour '{url}'"
        )
...
def test_ssl_certificate_error(fetch_service):
    ...
    with patch("requests.get") as mock_get:
        ssl_error_instance = requests.exceptions.SSLError(
...
def test_successful_fetch(fetch_service):
    ...
    with patch("requests.get") as mock_get:
        ...
        mock_get.assert_called_once_with(
            url,
            headers={"User-Agent": fetch_service.user_agent},
            timeout=fetch_service.timeout,
        )
...
def test_unknown_connection_error(fetch_service):
    ...
    with patch("requests.get") as mock_get:
        ...
        assert (
            error
            == f"Erreur de connexion lors de la récupération de '{url}': {original_error_message}"
        )
...
def test_unexpected_generic_exception(fetch_service):
    ...
    with patch("requests.get") as mock_get:
        ...
        assert (
            error
            == f"Erreur inattendue lors de la récupération de '{url}': {original_error_message}"
        )
