...
def start_react_interface():
    ...
    react_dir = Path(__file__).parent.parent.parent / "services" / "web_api" / "interface-web-argumentative"
...
def main():
    ...
    try:
        ...
        if api_process is not None or requests.get("http://localhost:5003/api/health", timeout=2).status_code == 200:
            # Démarrer l'interface React
            react_process = start_react_interface()
            processes.append(("Interface React", react_process))
            
            if react_process is not None or requests.get("http://localhost:3000/", timeout=2).status_code == 200:
                # Exécuter les tests
                success = run_tests()
            else:
