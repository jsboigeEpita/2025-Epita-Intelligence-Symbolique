...
class Phase3WebAPITester:
    ...
    def _verify_no_mocks(self):
        ...
        try:
            ...
            logger.info(f"✅ Service Web authentique accessible: {web_response.status_code}")
        except Exception as e:
            logger.error(f"❌ Service Web non accessible: {e}")
            raise RuntimeError("Service Web non accessible - Phase 3 nécessite des services authentiques")
            
        try:
            # Test simple du backend
            api_response = requests.get(f"{self.api_url}/health", timeout=5)
            logger.info(f"✅ Service API authentique accessible: {api_response.status_code}")
        except Exception as e:
            logger.warning(f"⚠️ Service API non accessible via /health, test avec /status: {e}")
            try:
                api_response = requests.get(f"{self.api_url}/status", timeout=5)
                logger.info(f"✅ Service API authentique accessible via /status: {api_response.status_code}")
            except Exception as e2:
                logger.warning(f"⚠️ Service API non accessible, continuer avec Web seulement: {e2}")
