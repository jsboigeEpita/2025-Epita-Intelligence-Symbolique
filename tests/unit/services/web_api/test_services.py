...
class TestAnalysisService:
    ...
    def analysis_service(self):
        ...
        with patch.dict(
            ...
            # #1859: llm_service is required by signature but unused at
            # runtime today — _initialize_components builds its own via
            # create_llm_service for the AgentFactory path and never reads
            # this parameter. A plain Mock is the honest depth: nothing it
            # could fake sits on the exercised path.
            return AnalysisService(llm_service=Mock())
    ...
    @pytest.mark.asyncio
    async def test_analyze_text_basic(self, analysis_service):
    ...
    async def test_analyze_text_with_options(self, analysis_service):
        ...
        request = AnalysisRequest(
            text="Argument avec sophisme potentiel", options=options
        )
    ...
    @pytest.mark.asyncio
    async def test_fallback_response(self, analysis_service):
...
class TestValidationService:
    ...
    def test_validate_invalid_premises(self):
        ...
        with pytest.raises(ValueError):
            ValidationRequest(
                premises=[], conclusion="Conclusion", argument_type="deductive"
            )
...
class TestFallacyService:
    ...
    def test_detect_fallacies_basic(self, fallacy_service):
        ...
        request = FallacyRequest(text="Tu ne peux pas avoir raison car tu es stupide.")
    ...
    def test_detect_fallacies_with_options(self, fallacy_service):
        ...
        options = FallacyOptions(
            severity_threshold=0.8, include_context=True, max_fallacies=5
        )
        request = FallacyRequest(
            text="Texte avec sophismes potentiels", options=options
        )
...
class TestServiceIntegration:
    ...
    def test_all_services_health_check(self):
        ...
        with patch.dict(
            ...
            for (
                service_instance
            ) in services:  # Renommé pour éviter conflit avec module 'service'
    ...
    async def test_service_error_handling(self):
        ...
        with patch.dict(
            ...
            service_instance = AnalysisService(llm_service=Mock())
            ...
            with patch.object(
                ...
                request = AnalysisRequest(text="Texte de test")
                response = await service_instance.analyze_text(request)
...
class TestModelValidation:
    ...
    def test_validation_request_validation(self):
        ...
        request = ValidationRequest(
            premises=["Prémisse 1", "Prémisse 2"], conclusion="Conclusion"
        )
