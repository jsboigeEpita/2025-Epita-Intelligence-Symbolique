...
def mcp_service_mock():
    ...
    with patch("argumentation_analysis.services.mcp_server.main.FastMCP") as mock_fast_mcp, patch(
        ...
        async def mock_analyze_text(*args, **kwargs):
            return AnalysisResponse(success=True, text_analyzed="Sample text")
        ...
        async def mock_validate_argument(*args, **kwargs):
            ...
            return ValidationResponse(
                ...
                result=ValidationResult(
                    is_valid=True, validity_score=1.0, soundness_score=1.0
                ),
            )
        ...
        async def mock_build_framework(*args, **kwargs):
            return FrameworkResponse(success=True, semantics_used="complete")

        mock_services_instance.analysis_service.analyze_text.side_effect = (
            mock_analyze_text
        )
        mock_services_instance.validation_service.validate_argument.side_effect = (
            mock_validate_argument
        )
        mock_services_instance.fallacy_service.detect_fallacies.side_effect = (
            mock_detect_fallacies
        )
        mock_services_instance.framework_service.build_framework.side_effect = (
            mock_build_framework
        )
...
def test_validate_argument_method(mcp_service_mock: MCPService):
    ...
    response = asyncio.run(
        mcp_service_mock.validate_argument(premises=["p1"], conclusion="c1")
    )
    assert response["success"] is True
