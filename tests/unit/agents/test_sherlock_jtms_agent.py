...
def sherlock_agent(mock_kernel, mock_settings):
    ...
    agent.validate_hypothesis_against_evidence = AsyncMock(
        return_value={
...
class TestSherlockJTMSAgent:
    ...
    async def test_form_hypothesis(self, sherlock_agent):
        ...
        clue1_result = await sherlock_agent.analyze_clues(
            [{"description": "Couteau", "location": "bibliotheque"}]
        )
        clue2_result = await sherlock_agent.analyze_clues(
            [{"description": "Bibliothèque fermée", "location": "bibliotheque"}]
        )
        evidence_ids = (
            clue1_result["new_evidence_ids"] + clue2_result["new_evidence_ids"]
        )
    ...
    async def test_explain_belief(self, sherlock_agent):
        ...
        clue_result = await sherlock_agent.analyze_clues(
            [{"description": "Traces de sang"}]
        )
        ...
        hyp_result = await sherlock_agent.formulate_hypothesis(
            "La victime a été déplacée.", evidence_ids=[evidence_id]
        )
        ...
        assert (
            f"supporting_evidence: ['{evidence_id}']"
            in str(sherlock_agent.get_all_beliefs()[hypothesis_id].to_dict())
            or evidence_id in explanation
        )
    ...
    async def test_validate_hypothesis_against_new_evidence(self, sherlock_agent):
        ...
        hyp_result = await sherlock_agent.formulate_hypothesis(
            "Contexte initial homme grand"
        )
        hypothesis_id = hyp_result["hypothesis_id"]

        # Nouvelle évidence qui supporte l'hypothèse
        supporting_evidence = {
            "description": "Témoin a vu un homme grand avec un chapeau sur la scène."
        }
        ...
        assert (
            validation_result["updated_strength"]["strength_score"]
            > hyp_result["confidence"]
        )
    ...
    async def test_investigate_cluedo_case(self, sherlock_agent):
        ...
        assert result is not None
        assert (
            "primary_hypothesis" in result
        ), f"Erreur de déduction: {result.get('error')}"
...
if __name__ == "__main__":
    ...
    async def run_basic_sherlock_tests():
        ...
        result2 = await agent.formulate_hypothesis(
            "Contexte de test", evidence_ids=[result["new_evidence_ids"][0]]
        )
