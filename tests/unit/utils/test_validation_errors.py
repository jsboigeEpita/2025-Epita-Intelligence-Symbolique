    ...
    def validate_source(self, source):
        ...
        if "source_type" in source:
            if source["source_type"] not in ["url", "file", "text"]:
                self.errors.append(
                    f"Erreur de validation: Type de source '{source['source_type']}' invalide. Valeurs acceptées: 'url', 'file', 'text'."
                )
        ...
        if "source_type" in source and source["source_type"] == "url":
            if "schema" not in source:
                self.errors.append(
                    "Erreur de validation: Champ 'schema' manquant pour une source de type 'url'."
                )
            elif source["schema"] not in ["http", "https"]:
                self.errors.append(
                    f"Erreur de validation: Schéma '{source['schema']}' invalide. Valeurs acceptées: 'http', 'https'."
                )

            if "host_parts" not in source:
                self.errors.append(
                    "Erreur de validation: Champ 'host_parts' manquant pour une source de type 'url'."
                )
            elif not isinstance(source["host_parts"], list):
                self.errors.append(
                    "Erreur de validation: Le champ 'host_parts' doit être une liste."
                )
            elif any(not part for part in source["host_parts"]):
                self.errors.append(
                    "Erreur de validation: Les parties d'hôte ne doivent pas contenir de chaînes vides."
                )

            if "path" not in source:
                self.errors.append(
                    "Erreur de validation: Champ 'path' manquant pour une source de type 'url'."
                )
            elif not source["path"].startswith("/"):
                self.errors.append(
                    "Erreur de validation: Le chemin doit commencer par '/'."
                )
        ...
        if "extracts" not in source:
            ...
            self.errors.append(
                "Erreur de validation: Le champ 'extracts' doit être une liste."
            )
        else:
            extract_names = set()
            for i, extract in enumerate(source["extracts"]):
                ...
                if "extract_name" not in extract:
                    self.errors.append(
                        f"Erreur de validation: Champ 'extract_name' manquant pour l'extrait #{i+1}."
                    )
                elif not isinstance(extract["extract_name"], str):
                    self.errors.append(
                        f"Erreur de validation: Le champ 'extract_name' doit être une chaîne de caractères pour l'extrait #{i+1}."
                    )
                else:
                    if extract["extract_name"] in extract_names:
                        self.errors.append(
                            f"Erreur de validation: Nom d'extrait '{extract['extract_name']}' dupliqué."
                        )
                    extract_names.add(extract["extract_name"])

                if "start_marker" not in extract:
                    self.errors.append(
                        f"Erreur de validation: Champ 'start_marker' manquant pour l'extrait '{extract.get('extract_name', f'#{i+1}')}."
                    )

                if "end_marker" not in extract:
                    self.errors.append(
                        f"Erreur de validation: Champ 'end_marker' manquant pour l'extrait '{extract.get('extract_name', f'#{i+1}')}."
                    )
                elif not extract["end_marker"]:
                    self.errors.append(
                        f"Erreur de validation: Marqueur de fin vide pour l'extrait '{extract.get('extract_name', f'#{i+1}')}."
                    )
...
def test_missing_fields():
    ...
    source = {
        ...
        "extracts": [{"extract_name": "Extrait incomplet", "start_marker": "Début"}],
    }
