...
class TestEnvironmentManager(unittest.TestCase):
    def setUp(self):
        ...
        with patch.object(EnvironmentManager, "_load_strategies"):
            ...
            self.manager = EnvironmentManager(
                project_root=self.mock_project_root, logger_instance=self.mock_logger
            )
    ...
    @patch(
        "project_core.core_from_scripts.environment_manager.EnvironmentManager.run_command_in_conda_env",
        return_value=0,
    )
    ...
    def test_fix_dependencies_with_packages(self):
        ...
        result = self.manager.fix_dependencies(
            packages=packages_to_fix, strategy_name="default"
        )
    ...
    def test_fix_dependencies_mutually_exclusive_args(self):
        ...
        with self.assertRaises(ValueError):
            self.manager.fix_dependencies(
                packages=["numpy"], requirements_file="req.txt"
            )
    ...
    def test_fix_dependencies_aggressive_strategy_success_on_first_try(self):
        ...
        result = self.manager.fix_dependencies(
            packages=["JPype1"], strategy_name="aggressive"
        )
    ...
    def test_fix_dependencies_aggressive_strategy_all_fail(self):
        ...
        result = self.manager.fix_dependencies(
            packages=["acme"], strategy_name="aggressive"
        )
        ...
        self.manager._strategies["wheel-install"].execute.assert_called_once_with(
            "acme"
        )
        if sys.platform == "win32":
            self.manager._strategies["msvc-build"].execute.assert_called_once_with(
                "acme"
            )
