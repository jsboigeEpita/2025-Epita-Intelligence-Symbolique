...
def test_rhetorical_analysis_state_manipulation(rhetorical_state):
    ...
    assert (
        rhetorical_state.identified_arguments["arg_1"]
        == "Ceci est un premier argument."
    )
...
def test_state_manager_plugin_write(state_manager_plugin):
    ...
    assert (
        state_manager_plugin._state.identified_arguments["arg_1"]
        == "Argument ajouté via le plugin."
    )
