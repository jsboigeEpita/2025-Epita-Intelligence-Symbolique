...
def orchestrator(mock_config, mock_kernel):
    ...
    with patch(
        "argumentation_analysis.orchestration.engine.main_orchestrator.CluedoExtendedOrchestrator",
        new=MagicMock(),
    ), patch(
...
class TestMainOrchestrator:
    ...
    async def test_run_analysis_selects_and_executes_correct_strategy(
        self, mock_select_strategy, orchestrator
    ):
        """
        ...
        with patch.object(
            orchestrator, "_execute_hierarchical_full", new_callable=AsyncMock
        ) as mock_execute:
            mock_execute.return_value = {
                "status": "success",
                "strategy_used": test_strategy.value,
            }
            ...
            # Assertions
            mock_select_strategy.assert_called_once_with(
                orchestrator.config, text_input, None, None
            )
    ...
    async def test_run_analysis_routes_to_all_strategies(
        self, mock_select_strategy, strategy, method_name, orchestrator
    ):
        """
        ...
        with patch.object(
            orchestrator, method_name, new_callable=AsyncMock
            ...
            result = await orchestrator.run_analysis(
                text_input, source_info, custom_config
            )

            # Assertions
            mock_select_strategy.assert_called_with(
                orchestrator.config, text_input, source_info, custom_config
            )
