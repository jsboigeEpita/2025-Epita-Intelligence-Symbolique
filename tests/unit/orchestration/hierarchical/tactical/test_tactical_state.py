...
class TestTacticalState(unittest.TestCase):
    ...
    def test_assign_task(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.state.task_assignments[self.test_task1["id"]], "agent-001"
        )
    ...
    def test_add_task_dependency(self):
        ...
        result = self.state.add_task_dependency(
            self.test_task1["id"], self.test_task2["id"]
        )
        self.assertTrue(result)
        self.assertIn(
            self.test_task2["id"], self.state.task_dependencies[self.test_task1["id"]]
        )
    ...
    def test_add_intermediate_result(self):
        ...
        result = self.state.add_intermediate_result(
            self.test_task1["id"], {"partial_result": "Sophisme détecté"}
        )
        self.assertTrue(result)
        self.assertEqual(
            self.state.intermediate_results[self.test_task1["id"]],
            {"partial_result": "Sophisme détecté"},
        )

        # Tester avec un ID de tâche invalide
        result = self.state.add_intermediate_result(
            "invalid-id", {"partial_result": "Test"}
        )
        self.assertFalse(result)
    ...
    def test_add_rhetorical_analysis_result(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.state.rhetorical_analysis_results["complex_fallacy_analyses"][
                self.test_task1["id"]
            ],
            self.test_rhetorical_result,
        )
        ...
        # Tester avec un ID de tâche invalide
        result = self.state.add_rhetorical_analysis_result(
            "invalid-id", "complex_fallacy_analyses", self.test_rhetorical_result
        )
        self.assertFalse(result)

        # Tester avec un type de résultat invalide
        result = self.state.add_rhetorical_analysis_result(
            self.test_task1["id"], "invalid_type", self.test_rhetorical_result
        )
        self.assertFalse(result)
    ...
    def test_get_rhetorical_analysis_result(self):
        ...
        result = self.state.get_rhetorical_analysis_result(
            "complex_fallacy_analyses", self.test_task1["id"]
        )
        ...
        # Tester avec un ID de tâche invalide
        result = self.state.get_rhetorical_analysis_result(
            "complex_fallacy_analyses", "invalid-id"
        )
        self.assertIsNone(result)
    ...
    def test_resolve_conflict(self):
        ...
        # Résoudre le conflit
        result = self.state.resolve_conflict(
            self.test_conflict["id"], self.test_resolution
        )
        self.assertTrue(result)
        self.assertTrue(self.state.identified_conflicts[0]["resolved"])
        self.assertEqual(
            self.state.identified_conflicts[0]["resolution"], self.test_resolution
        )
    ...
    def test_update_agent_utilization(self):
        ...
        self.assertEqual(
            self.state.tactical_metrics["agent_utilization"]["agent-001"], 0.75
        )

        # Tester avec une utilisation > 1.0
        self.state.update_agent_utilization("agent-001", 1.5)
        self.assertEqual(
            self.state.tactical_metrics["agent_utilization"]["agent-001"], 1.0
        )

        # Tester avec une utilisation < 0.0
        self.state.update_agent_utilization("agent-001", -0.5)
        self.assertEqual(
            self.state.tactical_metrics["agent_utilization"]["agent-001"], 0.0
        )
