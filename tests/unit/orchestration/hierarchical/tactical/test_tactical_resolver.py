...
class TestConflictResolver(unittest.TestCase):
    ...
    def test_detect_conflicts_with_arguments(self):
        ...
        self.tactical_state.tasks = {
            "completed": [{"id": "task-1", "objective_id": "obj-1"}],
            "pending": [],
            "in_progress": [],
            "failed": [],
        }
    ...
    def test_are_formal_analyses_contradictory(self):
        ...
        self.assertTrue(
            self.resolver._are_formal_analyses_contradictory(analysis1, analysis2)
        )
    ...
    def test_resolve_conflict_overlap(self):
        ...
        self.assertEqual(
            result["resolution"]["merged_result"]["conclusion"],
            "Le produit est très sûr",
        )
    ...
    def test_resolve_conflict_inconsistency(self):
        ...
        # Configurer l'état tactique pour le test
        conflict = {"id": "conflict-1", "type": "inconsistency"}
        self.tactical_state.identified_conflicts = [conflict]
    ...
    def test_resolve_conflict_ambiguity(self):
        ...
        # Configurer l'état tactique pour le test
        conflict = {"id": "conflict-1", "type": "ambiguity"}
        self.tactical_state.identified_conflicts = [conflict]
    ...
    def test_escalate_unresolved_conflicts(self):
        ...
        default_conflict = next(
            (c for c in conflicts if c["conflict_id"] == "conflict-2"), None
        )
        ...
        unresolved_conflict = next(
            (c for c in conflicts if c["conflict_id"] == "conflict-3"), None
        )
