...
class TestUnifiedConfig:
    ...
    def test_orchestration_mode_validation(self):
        ...
        # Test CONVERSATION
        config_conversation = UnifiedConfig(
    ...
    def test_invalid_combinations(self):
        ...
        with pytest.raises(ValueError, match="Configuration incohérente"):
            UnifiedConfig(mock_level=MockLevel.PARTIAL, require_real_gpt=True)

        with pytest.raises(ValueError, match="Configuration incohérente"):
            UnifiedConfig(mock_level=MockLevel.FULL, require_real_tweety=True)

        # Test 2: FOL_LOGIC agent sans JVM
        with pytest.raises(
            ValueError, match="FOL_LOGIC agent nécessite enable_jvm=True"
        ):
            UnifiedConfig(agents=[AgentType.FOL_LOGIC], enable_jvm=False)
    ...
    def test_service_configurations(self):
        ...
        config = UnifiedConfig(
            logic_type=LogicType.FOL, mock_level=MockLevel.NONE, timeout_seconds=120
        )
...
class TestConfigUtilities:
    ...
    def test_validate_config_errors(self):
        ...
        assert len(errors) > 0
        assert any(
            "Mock level != none mais require_real_gpt=True" in error for error in errors
        )
