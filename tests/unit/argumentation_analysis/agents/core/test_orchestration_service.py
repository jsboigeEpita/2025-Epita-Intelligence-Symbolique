...
class TestOrchestrationService(unittest.TestCase):
    ...
    def test_execute_plugin_with_missing_dependency_raises_error(self):
        ...
        with pytest.raises(
            PluginDependencyError,
            match="Dépendance non satisfaite : le plugin 'LoggerPlugin' requis par 'MainPluginWithDependency' est introuvable.",
        ):
...
class TestOrchestrationServiceContract:
    ...
    def test_get_plugin_with_invalid_type_raises_type_error(self, invalid_name):
        ...
        with pytest.raises(
            TypeError, match="Le nom du plugin doit être une chaîne de caractères."
        ):
            self.service.get_plugin(invalid_name)

    @pytest.mark.parametrize("invalid_name", [123, None, [], {"key": "value"}])
    def test_execute_plugin_with_invalid_name_type_raises_type_error(
        self, invalid_name
    ):
    ...
    @pytest.mark.parametrize(
        "not_a_plugin", [object(), {"name": "a dict"}, "just a string"]
    )
    def test_register_plugin_with_invalid_instance_type_raises_error(
        self, not_a_plugin
    ):
        """Vérifie que register_plugin lève une TypeError si l'objet n'est pas un BasePlugin."""
        with pytest.raises(
            TypeError, match="Le plugin doit être une instance de BasePlugin."
        ):
