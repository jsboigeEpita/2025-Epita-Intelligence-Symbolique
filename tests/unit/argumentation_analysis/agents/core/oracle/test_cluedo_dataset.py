...
class TestCluedoDataset:
    ...
    def test_validate_cluedo_suggestion_cannot_refute(self, cluedo_dataset):
        ...
        assert result.authorized is True
        assert (
            result.can_refute is False
        )  # Moriarty ne peut pas réfuter la vraie solution
    ...
    def test_reveal_card_owned(self, cluedo_dataset):
        ...
        revelation = cluedo_dataset.reveal_card(
            moriarty_card, "TestAgent", "Test de révélation", QueryType.CARD_INQUIRY
        )
    ...
    def test_different_reveal_policies(self, standard_elements):
        ...
        policies = [
            RevealPolicy.COOPERATIVE,
            RevealPolicy.COMPETITIVE,
            RevealPolicy.BALANCED,
            RevealPolicy.PROGRESSIVE,
        ]
        ...
        for policy in policies:
            dataset = CluedoDataset(
                solution_secrete=solution_secrete, cartes_distribuees=cartes_distribuees
            )
...
class TestCluedoDatasetIntegration:
    ...
    def test_reveal_policy_impact_on_game_flow(self, full_cluedo_setup):
        ...
        for policy in policies:
            ...
            revelation = dataset.reveal_card(
                moriarty_card, "TestAgent", "Test politique", QueryType.CARD_INQUIRY
            )
