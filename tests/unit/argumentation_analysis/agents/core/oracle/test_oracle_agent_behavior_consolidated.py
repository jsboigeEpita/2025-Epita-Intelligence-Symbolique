...
from argumentation_analysis.agents.core.oracle.dataset_access_manager import DatasetAccessManager, CluedoDatasetManager
from argumentation_analysis.agents.core.oracle.permissions import QueryType, PermissionRule, OracleResponse, ValidationResult
from argumentation_analysis.agents.core.oracle.cluedo_dataset import CluedoDataset, CluedoSuggestion, RevealPolicy
...
def mock_cluedo_dataset() -> CluedoDataset:
    ...
    dataset.validate_cluedo_suggestion = Mock(return_value=ValidationResult(can_refute=True, revealed_cards=[], suggestion_valid=True, authorized=True))
    dataset.reveal_card = Mock(return_value={"revealed_card": "Poignard", "revealing_agent": "Moriarty"})
    dataset._generate_strategic_clue = Mock(return_value={"clue_type": "strategic", "content": "Hint"})
    return dataset
...
def mock_dataset_manager(mock_cluedo_dataset: CluedoDataset) -> DatasetAccessManager:
    ...
    success_response = OracleResponse(authorized=True, message="Succès", data={"card": "Test"}, query_type=QueryType.CARD_INQUIRY)
...
class TestConsolidatedOracleAgentBehavior:
    ...
    @pytest.fixture
    def oracle_agent(self, mock_kernel: Kernel, mock_dataset_manager: DatasetAccessManager, sample_agent_config: dict) -> OracleBaseAgent:
        """Fournit une instance OracleBaseAgent initialisée."""
        return OracleBaseAgent(kernel=mock_kernel, dataset_manager=mock_dataset_manager, **sample_agent_config)
    ...
    async def test_execute_query_successfully(self, oracle_agent: OracleBaseAgent):
        ...
        result = await oracle_agent.oracle_tools.query_oracle_dataset(query_type="card_inquiry", query_params='{"card": "Test"}')
        assert "Succès" in result

    @pytest.mark.asyncio
    async def test_execute_query_permission_denied(self, oracle_agent: OracleBaseAgent, mock_dataset_manager: DatasetAccessManager):
        """Teste la gestion du refus de permission."""
        mock_dataset_manager.execute_oracle_query.return_value = OracleResponse(authorized=False, message="Accès refusé", data={}, query_type=QueryType.ADMIN_COMMAND)
        result = await oracle_agent.oracle_tools.query_oracle_dataset(query_type="admin_command", query_params='{"command": "reset"}')
        assert "Accès refusé" in result

    @pytest.mark.asyncio
    async def test_validate_permissions_for_another_agent(self, oracle_agent: OracleBaseAgent, mock_dataset_manager: DatasetAccessManager):
        """Teste la validation des permissions pour un autre agent."""
        # Test succès
        mock_dataset_manager.check_permission.return_value = True
        result_success = await oracle_agent.oracle_tools.validate_agent_permissions(target_agent="Watson", query_type="card_inquiry")
        assert "a les permissions" in result_success

        # Test échec
        mock_dataset_manager.check_permission.return_value = False
        result_failure = await oracle_agent.oracle_tools.validate_agent_permissions(target_agent="Watson", query_type="admin_command")
...
class SimpleCluedoOracleForDemo:
    ...
    def validate_suggestion(self, suspect: str, arme: str, lieu: str):
        ...
        if cards_to_reveal:
            return {"can_refute": True, "revealed_cards": cards_to_reveal, "message": f"Je possède {', '.join(cards_to_reveal)}."}
        else:
            return {"can_refute": False, "revealed_cards": [], "message": "Je ne peux rien révéler."}
