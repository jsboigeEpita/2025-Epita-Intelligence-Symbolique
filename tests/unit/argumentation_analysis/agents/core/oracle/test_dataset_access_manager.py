...
class TestQueryCache:
    ...
    def test_cache_operations(self, query_cache):
        ...
        cached_result = query_cache.get(
            "TestAgent", QueryType.CARD_INQUIRY, {"card": "knife"}
        )
...
class TestDatasetAccessManager:
    ...
    def dataset_manager(self, mock_dataset, mock_permission_manager):
        ...
        return DatasetAccessManager(
            dataset=mock_dataset, permission_manager=mock_permission_manager
        )

    def test_dataset_access_manager_initialization(
        self, dataset_manager, mock_dataset, mock_permission_manager
    ):
    ...
    def test_permission_validation(self, dataset_manager, mock_permission_manager):
        ...
        dataset_manager.permission_manager.add_permission_rule(
            PermissionRule("Sherlock", [QueryType.CARD_INQUIRY])
        )

        result_sherlock_authorized = dataset_manager.permission_manager.is_authorized(
        ...
        result_unknown_authorized = dataset_manager.permission_manager.is_authorized(
            "UnknownAgent", QueryType.SUGGESTION_VALIDATION
        )
        assert result_unknown_authorized is False

        result_sherlock_unauthorized_type = (
            dataset_manager.permission_manager.is_authorized(
    ...
    async def test_execute_query_success(
        ...
        mock_permission_manager.add_permission_rule(
            PermissionRule("Sherlock", [QueryType.CARD_INQUIRY])
        )
        ...
        result = await dataset_manager.execute_query(
            ...
            },  # Utiliser une carte du dataset réel
        )
    ...
    @pytest.mark.asyncio
    async def test_permission_denied_query(
        self, dataset_manager, mock_permission_manager
    ):
        ...
        result = await dataset_manager.execute_query(
            ...
            query_params={
                "suggestion": {"suspect": "Plum", "arme": "Poignard", "lieu": "Salon"}
            },
        )
    ...
    def test_generate_cache_key(self, dataset_manager):
        ...
        key1 = dataset_manager._generate_cache_key(
            "Agent1", QueryType.CARD_INQUIRY, {"card": "knife"}
        )
        ...
        key3 = dataset_manager._generate_cache_key(
            "Agent1", QueryType.CARD_INQUIRY, {"card": "rope"}
        )
    ...
    def test_apply_permission_filters(self, dataset_manager):
        ...
        test_result = QueryResult(
            success=True, data=test_data, query_type=QueryType.CARD_INQUIRY
        )

        # Test avec données filtrées
        filtered_result = dataset_manager._apply_permission_filters(
            "TestAgent", test_result  # agent_name  # QueryResult
        )
...
class TestCluedoDatasetManager:
    ...
    def mock_cluedo_dataset(self):
        ...
        return CluedoDataset(
            moriarty_cards=["Professeur Violet", "Chandelier", "Bureau"]
        )
    ...
    def test_cluedo_dataset_manager_initialization(
        self, cluedo_manager, mock_cluedo_dataset
    ):
...
class TestDatasetAccessManagerIntegration:
    ...
    def test_real_permission_validation(self, integration_manager):
        ...
        result_sherlock = integration_manager.permission_manager.is_authorized(
            "Sherlock", QueryType.CARD_INQUIRY
        )
        result_watson = integration_manager.permission_manager.is_authorized(
