...
from argumentation_analysis.core.strategies import (
    BalancedParticipationStrategy as BalancedStrategy,
)
...
class HierarchicalOrchestrator:
    async def analyze_text(self, text: str, analysis_type: str):
        logging.info(
            f"Mocked HierarchicalOrchestrator.analyze_text called with text (len: {len(text)}), type: {analysis_type}"
        )
...
@pytest.mark.skip(
    reason="Test de performance désactivé car il dépend de composants mockés et d'une ancienne architecture."
)
