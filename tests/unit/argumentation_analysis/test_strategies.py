...
class TestSimpleTerminationStrategy:
    ...
    def test_initialization(self, strategy_components):
    ...
    def test_should_terminate_max_steps(self, strategy_components):
    ...
    def test_should_terminate_conclusion(self, strategy_components):
    ...
    def test_reset(self, strategy_components):
...
class TestDelegatingSelectionStrategy:
    ...
    def test_initialization(self, delegating_strategy_components):
        """Teste l'initialisation correcte de la stratégie."""
    ...
    def test_next_with_empty_history(self, delegating_strategy_components):
        ...
        (
            ...
            _,
            empty_history,
        ) = delegating_strategy_components

        async def run_test():
            # Sans désignation, devrait retourner l'agent par défaut (PM)
            return await strategy.next(agents, empty_history)

        selected_agent = asyncio.run(run_test())
        assert selected_agent == pm_agent

    def test_next_with_designation(self, delegating_strategy_components):
    ...
    def test_next_with_invalid_designation(self, delegating_strategy_components):
    ...
    def test_next_after_user_message(self, delegating_strategy_components):
    ...
    def test_next_after_assistant_message(self, delegating_strategy_components):
    ...
    def test_reset(self, delegating_strategy_components):
        """Teste la réinitialisation de la stratégie."""
        state, strategy, _, _, _, _, _ = delegating_strategy_components
        # Désigner un agent
        state.designate_next_agent("PropositionalLogicAgent")
...
class TestBalancedParticipationStrategy:
    ...
    def test_initialization_default(self, balanced_strategy_components):
        ...
        assert (
            strategy._target_participation["ProjectManagerAgent"]
            > strategy._target_participation["PropositionalLogicAgent"]
        )
        assert (
            strategy._target_participation["ProjectManagerAgent"]
            > strategy._target_participation["InformalAnalysisAgent"]
        )
        ...
        assert (
            abs(total_participation - 1.0) < 1e-9
        )  # Utiliser une tolérance pour les flottants

    def test_initialization_custom(self, balanced_strategy_components):
    ...
    def test_next_with_designation(self, balanced_strategy_components):
    ...
    def test_next_with_invalid_designation(self, balanced_strategy_components):
        """Teste la sélection avec une désignation invalide."""
        (
            state,
            strategy,
    ...
    def test_balanced_participation(self, balanced_strategy_components):
        ...
        async def run_test():
            ...
            participation_counts = {
                "ProjectManagerAgent": 0,
                "PropositionalLogicAgent": 0,
                "InformalAnalysisAgent": 0,
            }
            ...
            for agent_name in participation_counts:
                assert (
                    participation_counts[agent_name] > 0
            ...
            for agent_name, count in participation_counts.items():
                ...
                assert (
                    abs(actual_rate - target_rate) <= 0.2
                ), f"Taux de participation pour {agent_name}: {actual_rate} vs cible {target_rate}"

            # Vérifier que les compteurs internes correspondent aux compteurs que nous avons suivis
            for agent_name in participation_counts:
                assert (
                    strategy._participation_counts[agent_name]
                    == participation_counts[agent_name]
                )
    ...
    def test_imbalance_budget_adjustment(self, balanced_strategy_components):
