...
class TestRealSystemUtils(unittest.TestCase):
    ...
    def test_ensure_directory_exists_real(self):
        ...
        try:
            ...
            self.assertFalse(
                test_dir.exists(), "Le répertoire ne devrait pas exister initialement"
            )
            ...
            self.assertTrue(
                result2, "Devrait retourner True même si le répertoire existe"
            )
    ...
    def test_get_project_root_real(self):
        ...
        try:
            ...
            self.assertGreater(
                len(existing_dirs), 0, "Devrait contenir au moins un dossier du projet"
            )
...
class TestRealFileOperations(unittest.TestCase):
    ...
    def test_real_file_operations(self):
        ...
        try:
            ...
            # Vérifications
            self.assertEqual(
                loaded_data, test_data, "Les données chargées devraient correspondre"
            )
    ...
    def test_real_directory_traversal(self):
        ...
        try:
            ...
            for file_path in all_files:
                self.assertTrue(
                    file_path.exists(), f"Le fichier {file_path} devrait exister"
                )
...
class TestRealTextProcessing(unittest.TestCase):
    ...
    def test_real_text_normalization(self):
        ...
        try:
            ...
            # Vérifications
            self.assertNotIn(
                "  ", normalized, "Ne devrait pas contenir d'espaces doubles"
            )
            self.assertFalse(
                normalized.startswith(" "), "Ne devrait pas commencer par un espace"
            )
            self.assertFalse(
                normalized.endswith(" "), "Ne devrait pas finir par un espace"
            )
    ...
    def test_real_logical_pattern_extraction(self):
        ...
        try:
            ...
            print(
                f"✅ Test extraction patterns réelle réussie - Règles: {rules}, Faits: {facts}"
            )
            return True
