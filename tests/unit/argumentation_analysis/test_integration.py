...
def mock_run_analysis(mocker):
    ...
    return mocker.patch(
        "argumentation_analysis.orchestration.analysis_runner_v2.AnalysisRunnerV2.run_analysis"
    )
...
class TestBasicIntegration:
    ...
    def test_component_interaction(self, basic_state):
        ...
        async def run_test():
            ...
            state.add_fallacy(
                "Faux raisonnement", "Confusion entre apparence et réalité", arg_id
            )
...
class TestSimulatedAnalysisFlow:
    ...
    def test_simulated_analysis_flow(self, mock_run_analysis, basic_state):
        ...
        fallacy1_id = state.add_fallacy(
            "Faux raisonnement", "Confusion entre apparence et réalité", arg1_id
        )
        fallacy2_id = state.add_fallacy(
            "Fausse analogie", "Mauvaise compréhension de la gravité", arg2_id
        )
        fallacy3_id = state.add_fallacy(
            "Ad hominem", "Attaque la crédibilité plutôt que l'argument", arg3_id
        )
