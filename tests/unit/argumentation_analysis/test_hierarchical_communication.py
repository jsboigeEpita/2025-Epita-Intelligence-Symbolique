...
class TestHierarchicalCommunication(unittest.TestCase):
    ...
    def test_bottom_up_communication(self):
        ...
        def tactical_agent():
            ...
            logger.info(
                f"Tactical agent received result.content: {result.content if result else 'None'}"
            )
    ...
    def test_full_hierarchical_workflow(self):
            ...
            for _ in range(10):  # Essayer plusieurs fois
                ...
                for message in messages:
                    if (
                        ...
                        and message.content.get("info_type") == "operational_update"
                    ):
        ...
        for _ in range(10):  # Essayer plusieurs fois
            messages = self.middleware.get_pending_messages(
                recipient_id="strategic-agent-1", channel_type=ChannelType.HIERARCHICAL
            )

            for message in messages:
                if (
                    message.type == MessageType.INFORMATION
                    and message.content.get("info_type") == "tactical_update"
                    and message.content.get("update_type") == "directive_received"
                ):
                    ack = message
                    break
        ...
        for _ in range(10):  # Essayer plusieurs fois
            ...
            for message in messages:
                if (
                    message.type == MessageType.INFORMATION
                    and message.content.get("info_type") == "tactical_update"
                    and message.content.get("update_type") == "task_progress"
                ):
                    progress = message
                    break
    ...
    def test_strategic_guidance(self):
        ...
        def strategic_agent():
            ...
            for _ in range(5):  # Essayer quelques fois avec un court délai
                ...
                if (
                    processed_msg and processed_msg.id == response.id
                ):  # Si on a traité la réponse elle-même
                    break
                time.sleep(0.01)
            ...
            for _ in range(10):  # Essayer plusieurs fois
                ...
                for message in messages:
                    if (
                        ...
                        and message.content.get("command_type") == "allocate_resources"
                    ):
                        allocation = message
                        break
    ...
    async def test_async_operational_assistance(self):
        ...
        async def tactical_agent():
            ...
            self.middleware.send_message(
                response
            )  # Note: send_message est synchrone ici
            await asyncio.sleep(0.1)  # Donner du temps pour le traitement du message
            # Tentative de forcer le traitement de la réponse par l'agent opérationnel
            for _ in range(5):
                processed_msg = await self.middleware.receive_message_async(
