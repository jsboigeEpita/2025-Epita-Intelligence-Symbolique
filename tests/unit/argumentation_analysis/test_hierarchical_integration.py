...
class TestHierarchicalIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Créer les managers
        self.strategic_manager = StrategicManager(strategic_state=self.strategic_state)
        ...
        self.extract_agent.get_capabilities.return_value = [
            "text_extraction",
            "preprocessing",
        ]
        self.extract_agent.can_process_task.return_value = True
        self.extract_agent.process_task = AsyncMock(
            return_value={
                "id": "result-op-task-1",
                "task_id": "op-task-1",
        ...
        self.informal_agent.get_capabilities.return_value = [
            "fallacy_detection",
            "argument_identification",
        ]
        self.informal_agent.can_process_task.return_value = True
        self.informal_agent.process_task = AsyncMock(
            return_value={
                "id": "result-op-task-2",
                "task_id": "op-task-2",
        ...
        self.pl_agent.get_capabilities.return_value = [
            "formal_logic",
            "validity_checking",
        ]
        self.pl_agent.can_process_task.return_value = True
        self.pl_agent.process_task = AsyncMock(
            return_value={
                "id": "result-op-task-3",
                "task_id": "op-task-3",
    ...
    def test_end_to_end_workflow(self):
        ...
        strategic_plan = {
            "phases": [
                ...
                {"id": "phase-3", "name": "Analyse formelle", "objectives": ["obj-3"]},
            ],
            "success_criteria": {
        ...
        tactical_directives = (
            self.strategic_tactical_interface.translate_objectives_to_directives(
                objectives
            )
        )
        ...
        operational_tasks = []
        for task in tasks:
            op_task = self.tactical_operational_interface.translate_task_to_command(
                task
            )
        ...
        for op_task in operational_tasks:
            ...
            elif "fallacy_detection" in op_task.get("techniques", [{}])[0].get(
                "name", ""
            ):
        ...
        for i, op_result in enumerate(operational_results):
            ...
            tac_result = self.tactical_operational_interface.process_operational_result(
                original_op_task, op_result
            )
        ...
        tactical_report = {
            ...
            "progress_by_objective": {
                "obj-1": {"total_tasks": 1, "completed_tasks": 1, "progress": 1.0},
                "obj-2": {"total_tasks": 1, "completed_tasks": 1, "progress": 1.0},
                "obj-3": {"total_tasks": 1, "completed_tasks": 1, "progress": 1.0},
            },
            "issues": [],
            "conflicts": {"total": 0, "resolved": 0},
            "metrics": {
                "agent_utilization": {
                    "ExtractAgent": 0.5,
                    "InformalAgent": 0.6,
...
class TestHierarchicalPerformance(unittest.TestCase):
    ...
    def test_performance_comparison(self):
        ...
        async def run_test():
            ...
            improvement = (
                (old_architecture_time - new_architecture_time)
                / old_architecture_time
                * 100
            )

            # Afficher les résultats
            print(
                f"Temps d'exécution de la nouvelle architecture: {new_architecture_time:.3f} secondes"
            )
            print(
                f"Temps d'exécution de l'ancienne architecture: {old_architecture_time:.3f} secondes"
            )
