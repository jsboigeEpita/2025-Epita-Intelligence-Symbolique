...
def crypto_service():
    ...
    return CryptoService(
        encryption_key=None
    )  # Sans chiffrement pour simplifier les tests
...
@pytest.fixture
def definition_service_with_crypto(
    crypto_service_with_key, temp_config_file, temp_fallback_file
):
...
def mock_json_load(mocker):
    ...
    return mocker.patch(
        "json.load", side_effect=json.JSONDecodeError("Erreur JSON de test", "", 0)
    )
...
class TestDefinitionService:
    ...
    def test_load_definitions_from_file(
        self, definition_service, sample_definitions_dict
    ):
    ...
    def test_load_definitions_from_fallback(
        self, definition_service, sample_definitions_dict
    ):
    ...
    def test_load_definitions_from_default(
        self,
        crypto_service,
        temp_config_file,
        temp_fallback_file,
        sample_definitions_dict,
    ):
    ...
    def test_load_definitions_encrypted(
        self, definition_service_with_crypto, sample_definitions
    ):
    ...
    def test_save_definitions_encrypted(
        self, definition_service_with_crypto, sample_definitions
    ):
        """Test de sauvegarde de définitions chiffrées."""
        # Sauvegarder les définitions
        success, error_message = definition_service_with_crypto.save_definitions(
            sample_definitions
        )
    ...
    def test_save_definitions_directory_error(
        self, mocker, definition_service, sample_definitions
    ):
        """Test de sauvegarde de définitions avec une erreur de création de répertoire."""
        # Simuler une erreur de création de répertoire
        mocker.patch(
            "pathlib.Path.mkdir",
            side_effect=OSError("Erreur de création de répertoire de test"),
        )
    ...
    def test_save_definitions_fallback(self, definition_service, sample_definitions):
        ...
        with patch("builtins.open", side_effect=mock_open_func):
            # Sauvegarder les définitions
            success, error_message = definition_service.save_definitions(
                sample_definitions
            )
    ...
    def test_export_definitions_to_json(
        self, definition_service, sample_definitions, tmp_path
    ):
        ...
        success, message = definition_service.export_definitions_to_json(
            sample_definitions, output_path
        )
    ...
    def test_export_definitions_error(
        self, mocker, definition_service, sample_definitions, tmp_path
    ):
    ...
    def test_import_definitions_from_json(
        self, definition_service, sample_definitions, tmp_path
    ):
