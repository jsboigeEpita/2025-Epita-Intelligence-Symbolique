...
def miner_custom_config() -> MockClaimMiner:
    ...
    config = {"min_claim_length": 5, "claim_keywords": ["point clé:", "important:"]}
...
def test_extract_claims_non_string_input(miner_default: MockClaimMiner, caplog):
    ...
    assert result == []
    assert (
        "MockClaimMiner.extract_claims a reçu une entrée non textuelle." in caplog.text
    )
...
def test_extract_claims_by_keyword_text_too_short_after_keyword(
    miner_default: MockClaimMiner, miner_custom_config: MockClaimMiner
):
...
def test_extract_claims_assertive_sentence_if_no_keywords(
    miner_default: MockClaimMiner,
):
    """Teste la logique de fallback sur Revendication Globale."""
    text = (
        "Ceci est une phrase assertive. Elle est assez longue. Une autre plus courte."
    )
    result = miner_default.extract_claims(text)
...
def test_extract_claims_assertive_sentence_filtered_by_length(
    miner_custom_config: MockClaimMiner,
):
