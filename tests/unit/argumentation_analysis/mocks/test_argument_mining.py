...
def test_mine_arguments_affirmation_simple_too_short(
    miner_custom_config: MockArgumentMiner,
):
...
def test_mine_arguments_explicit_premise_conclusion_too_short(
    miner_default: MockArgumentMiner, miner_custom_config: MockArgumentMiner
):
    ...
    text_custom_miner = (
        "Prémisse: Un. Conclusion: Deux."  # "Un." (len 3), "Deux." (len 5)
    )
...
def test_conclusion_delimitation_explicit(miner_default: MockArgumentMiner):
    ...
    text_no_period_end_of_string = (
        "Prémisse: P2 est une autre longue prémisse. Conclusion: C2 est la fin"
    )
    ...
    arg_c3 = next(
        filter(
            lambda x: x.get("premise") == "P3 est une prémisse assez longue.", result3
        ),
        None,
    )
