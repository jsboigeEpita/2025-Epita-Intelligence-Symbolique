...
def mock_logger():
    ...
    with mock.patch(
        "argumentation_analysis.mocks.fallacy_detection.logger"
    ) as mock_log:
        yield mock_log
...
def test_slicing_protection_in_ad_populum(detector: MockFallacyDetector):
    ...
    for fallacy in result:
        if fallacy["fallacy_type"] == "Ad Populum (Mock)":
            ...
            ), "Le contexte pour Ad Populum devrait être vide pour un texte très court."
    ...
    for fallacy in result_medium:
        if fallacy["fallacy_type"] == "Ad Populum (Mock)":
            ...
            assert (
                fallacy["context_text"] == medium_text_variant[50:150]
            )  # ou medium_text_variant[50:70]
