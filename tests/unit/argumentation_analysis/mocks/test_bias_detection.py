...
def test_initialization_default(detector_default: MockBiasDetector):
    ...
    assert (
        ...
        in detector_default.bias_patterns["Biais de Confirmation (Mock)"]
    )
...
def test_initialization_custom_config(detector_custom_config: MockBiasDetector):
    ...
    assert detector_custom_config.bias_patterns["Test Biais (Mock)"] == [
        r"pattern test A",
        r"pattern test B",
    ]
...
def test_detect_biases_non_string_input(detector_default: MockBiasDetector, caplog):
    ...
    assert result == []
    assert (
        "MockBiasDetector.detect_biases a reçu une entrée non textuelle." in caplog.text
    )
...
def test_detect_bias_confirmation(detector_default: MockBiasDetector):
    ...
    assert len(result) >= 1
    found_bias = any(
        b["bias_type"] == "Biais de Confirmation (Mock)"
        and b["detected_pattern"] == r"il est évident que"
        for b in result
    )
    assert found_bias
    # Vérifier le contexte (le mock prend 30 chars avant/après)
    bias_entry = next(
        b for b in result if b["bias_type"] == "Biais de Confirmation (Mock)"
    )
...
def test_detect_bias_generalisation(detector_default: MockBiasDetector):
    ...
    assert len(result) >= 1
    found_bias = any(
        b["bias_type"] == "Généralisation Hâtive (Mock)"
        and b["detected_pattern"] == r"toujours"
        for b in result
    )
    assert found_bias
    bias_entry = next(
        b for b in result if b["bias_type"] == "Généralisation Hâtive (Mock)"
    )
...
def test_detect_bias_autorite(detector_default: MockBiasDetector):
    ...
    assert len(result) >= 1
    found_bias = any(
        b["bias_type"] == "Biais d'Autorité (Mock)"
        and b["detected_pattern"] == r"selon l'expert"
        for b in result
    )
    assert found_bias
...
def test_detect_bias_faux_dilemme(detector_default: MockBiasDetector):
    ...
    assert len(result) >= 1
    found_bias = any(
        b["bias_type"] == "Faux Dilemme (Mock)"
        and b["detected_pattern"] == r"soit A ou soit B"
        for b in result
    )
    assert found_bias


def test_detect_biases_pattern_context_too_short(
    detector_custom_config: MockBiasDetector,
):
    ...
    short_detector = MockBiasDetector(
        config={
            "min_context_length": 30,
            "bias_patterns": {"Short (Mock)": ["shortpattern"]},
        }
    )
    text_short_pattern = "shortpattern"  # len 12
    result_short = short_detector.detect_biases(
        text_short_pattern
    )  # min_context_length 30
...
def test_detect_biases_no_exageration_if_other_bias_found(
    detector_default: MockBiasDetector,
):
    ...
    result = detector_default.detect_biases(text)
    assert len(result) >= 1
    has_confirmation_bias = any(
        b["bias_type"] == "Biais de Confirmation (Mock)" for b in result
    )
    has_exageration_bias = any(
        b["bias_type"] == "Exagération Potentielle (Mock)" for b in result
    )
    assert has_confirmation_bias
    assert not has_exageration_bias  # Le fallback ne devrait pas s'activer


def test_detect_biases_no_exageration_if_text_too_short(
    detector_default: MockBiasDetector,
):
...
def test_detect_biases_structure_and_indices(detector_default: MockBiasDetector):
    ...
    text = (
        "Selon l'expert, c'est la seule voie. Jamais une autre option n'a fonctionné."
    )
    ...
    for bias in result:
        ...
        assert (
            text[start:end].lower() == bias["detected_pattern"].lower()
            or bias["detected_pattern"].lower() in text[start:end].lower()
        )  # Pour les regex plus complexes
