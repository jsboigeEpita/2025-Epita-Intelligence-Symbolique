...
def detector_custom_config() -> MockEvidenceDetector:
    ...
    config = {"min_evidence_length": 10, "evidence_keywords": ["preuve:", "source:"]}
...
def test_detect_evidence_non_string_input(
    detector_default: MockEvidenceDetector, caplog
):
    ...
    assert result == []
    assert (
        "MockEvidenceDetector.detect_evidence a reçu une entrée non textuelle."
        in caplog.text
    )
...
def test_detect_evidence_by_keyword_success(detector_default: MockEvidenceDetector):
    ...
    assert (
        evidence["evidence_text"] == "les résultats sont concluants."
    )  # S'arrête au point
    ...
    assert evidence["source_indices"] == (
        keyword_start,
        text.find(".", keyword_start) + 1,
    )
...
def test_detect_evidence_by_keyword_no_period(detector_default: MockEvidenceDetector):
    ...
    assert evidence["type"] == "Preuve par Mot-Clé (Mock)"
    assert (
        evidence["evidence_text"] == "l'impact est significatif et continue"
    )  # Prend jusqu'à 150 chars ou fin
    assert evidence["keyword_used"] == "les données montrent que"


def test_detect_evidence_by_keyword_text_too_short_after_keyword(
    detector_default: MockEvidenceDetector, detector_custom_config: MockEvidenceDetector
):
    ...
    assert (
        result == []
    )  # Ne devrait rien trouver car trop court, et pas de fallback global dans ce mock
...
def test_detect_evidence_factual_digit_if_no_keywords(
    detector_default: MockEvidenceDetector,
):
...
def test_detect_evidence_factual_percent_if_no_keywords(
    detector_default: MockEvidenceDetector,
):
...
def test_detect_evidence_factual_filtered_by_length(
    detector_custom_config: MockEvidenceDetector,
):
...
def test_detect_evidence_by_citation_if_no_keywords_or_factual(
    detector_default: MockEvidenceDetector,
):
    """Teste preuve par citation si rien d'autre n'est trouvé."""
    text = (
        'Il a dit: "Ceci est une citation directe et pertinente." Et puis autre chose.'
    )
    result = detector_default.detect_evidence(text)
    assert len(result) == 1
    evidence = result[0]
...
def test_detect_evidence_citation_too_short(
    detector_custom_config: MockEvidenceDetector,
):
...
def test_detect_evidence_priority_order(detector_default: MockEvidenceDetector):
    ...
    text_keyword_and_factual = (
        "Selon l'étude, 50% des cas sont résolus. Il y a aussi 20 chiffres."
    )
    result1 = detector_default.detect_evidence(text_keyword_and_factual)
    assert len(result1) == 1
    assert result1[0]["type"] == "Preuve par Mot-Clé (Mock)"
    assert (
        result1[0]["evidence_text"] == "50% des cas sont résolus."
    )  # Texte après mot-clé

    # 2. Factuel a priorité sur Citation (si pas de mot-clé)
    text_factual_and_citation = (
        'Le rapport indique 75% de succès. Il a aussi affirmé: "C\'est vrai".'
    )
    ...
    assert (
        result3[0]["evidence_text"]
        == "Ceci est la seule preuve disponible ici et elle est assez longue pour être valide"
    )
