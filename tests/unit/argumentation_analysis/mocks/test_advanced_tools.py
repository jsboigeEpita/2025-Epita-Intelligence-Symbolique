...
def test_create_mock_advanced_rhetorical_tools_structure():
    ...
    assert isinstance(tools["complex_fallacy_analyzer"], MockEnhancedComplexFallacyAnalyzer)
    assert isinstance(tools["contextual_fallacy_analyzer"], MockEnhancedContextualFallacyAnalyzer)
    assert isinstance(tools["fallacy_severity_evaluator"], MockEnhancedFallacySeverityEvaluator)
    assert isinstance(tools["rhetorical_result_analyzer"], MockEnhancedRhetoricalResultAnalyzer)
...
def test_mock_enhanced_complex_fallacy_analyzer():
    ...
    assert result["note"] == "Analyse simulée - les outils réels ne sont pas disponibles"
...
def test_mock_tools_handle_none_or_empty_inputs():
    ...
    result_empty_text = contextual_analyzer.analyze_context("", {}) # Doit gérer str vide
    ...
    assert result_sev_none["overall_severity"] is not None # Doit retourner une structure valide
