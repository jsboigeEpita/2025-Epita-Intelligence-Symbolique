...
import pytest
import logging
from argumentation_analysis.mocks.emotional_tone_analysis import (
    MockEmotionalToneAnalyzer,
)
...
def test_initialization_custom_config(
    analyzer_custom_config: MockEmotionalToneAnalyzer,
):
...
def test_analyze_tone_non_string_input(
    analyzer_default: MockEmotionalToneAnalyzer, caplog
):
...
def test_analyze_tone_single_emotion_joy(analyzer_default: MockEmotionalToneAnalyzer):
    ...
    text = (
        "Je suis tellement heureux et content aujourd'hui, c'est une journée joyeuse !"
    )
    # "heureux", "content", "joyeux" sont des mots-clés pour Joie.
    # count = 3.
    # score = min(1.0, 0.3 * 3 + 0.1) = min(1.0, 1.0) = 1.0
    # Aucun mot-clé fort pour la joie dans la liste par défaut.
    result = analyzer_default.analyze_tone(text)
    assert result["emotions_scores"]["Joie (Mock)"] == pytest.approx(1.0)
    assert result["dominant_emotion"] == "Joie (Mock)"
    ...
    assert "joyeuse" in keywords_found


def test_analyze_tone_single_emotion_sadness_strong_keyword(
    analyzer_default: MockEmotionalToneAnalyzer,
):
    """Teste la détection de la tristesse avec un mot-clé fort."""
    text = "Il se sentait complètement déprimé et abattu."
    # "déprimé" et "abattu" sont des mots-clés pour Tristesse.
    # count = 2.
    # score_base = min(1.0, 0.3 * 2 + 0.1) = 0.7
    # "déprimé" est un mot-clé fort. Bonus de +0.2.
    # score_final = min(1.0, 0.7 + 0.2) = 0.9
    result = analyzer_default.analyze_tone(text)
    assert result["emotions_scores"]["Tristesse (Mock)"] == pytest.approx(0.9)
    assert result["dominant_emotion"] == "Tristesse (Mock)"


def test_analyze_tone_mixed_emotions_dominant_below_threshold(
    analyzer_default: MockEmotionalToneAnalyzer,
):
    """Teste émotions mixtes où la plus forte est sous le seuil d'intensité."""
    text = "Je suis un peu content mais aussi un peu triste."
    # "content" (Joie): count=1. score = 0.3*1+0.1 = 0.4
    # "triste" (Tristesse): count=1. score = 0.3*1+0.1 = 0.4
    # Aucun mot-clé fort.
    # Max score = 0.4, qui est < intensity_threshold (0.6)
    result = analyzer_default.analyze_tone(text)
    assert result["emotions_scores"]["Joie (Mock)"] == 0.4
    ...
    assert (
        "Mixte (dominant: Joie, score: 0.40)" in result["dominant_emotion"]
        or "Mixte (dominant: Tristesse, score: 0.40)" in result["dominant_emotion"]
    )


def test_analyze_tone_mixed_emotions_one_dominant_above_threshold(
    analyzer_default: MockEmotionalToneAnalyzer,
):
    ...
    # score_base_joie = min(1.0, 0.3*4 + 0.1) = 1.0
    # "ravi" est un mot-clé fort. Bonus +0.2.
    # score_final_joie = min(1.0, 1.0 + 0.2) = 1.0
    # Tristesse: "triste". count = 1. score = 0.3*1 + 0.1 = 0.4. Pas de mot-clé fort.
    result = analyzer_default.analyze_tone(text)
...
def test_analyze_tone_custom_config_keywords(
    analyzer_custom_config: MockEmotionalToneAnalyzer,
):
    """Teste avec les mots-clés de la configuration personnalisée."""
    text = "C'est super génial ! Mais quel dommage."
    # Bonheur (Custom): "super", "génial". count = 2.
    # score = min(1.0, 0.3*2 + 0.1) = 0.7. Pas de mot-clé fort dans la config custom.
    # Tristesse (Custom): "dommage". count = 1. score = 0.3*1 + 0.1 = 0.4
    ...
    assert result["dominant_emotion"] == "Bonheur (Custom)"
...
def test_analyze_tone_details_indices(analyzer_default: MockEmotionalToneAnalyzer):
    ...
    for detail in details:
        if detail["emotion"] == "Peur (Mock)" and detail["keyword"] == "peur":
            assert detail["indices"] == (
                text.lower().find("peur"),
                text.lower().find("peur") + len("peur"),
            )
            found_peur = True
        if detail["emotion"] == "Tristesse (Mock)" and detail["keyword"] == "triste":
            assert detail["indices"] == (
                text.lower().find("triste"),
                text.lower().find("triste") + len("triste"),
            )
            found_triste = True
    assert found_peur
    assert found_triste
