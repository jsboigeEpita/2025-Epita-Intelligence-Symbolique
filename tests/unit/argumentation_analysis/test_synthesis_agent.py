...
class MockInformalAgent:
    async def analyze_text(self, text: str) -> dict:
        ...
        return {
            "fallacies": fallacies,
            "structure": "Structure analysée (mock)",
            "devices": devices,
        }
...
class TestSynthesisAgent:
    ...
    @pytest.mark.asyncio
    async def test_synthesize_analysis_advanced_mode_not_implemented(
    ...
    async def test_unify_results(self, mocker, synthesis_agent):
        ...
        logic_result = LogicAnalysisResult(
            logical_validity=True, consistency_check=True
        )
        ...
        report = await synthesis_agent.unify_results(
            logic_result, informal_result, "Original text"
        )
    ...
    async def test_generate_report(self, synthesis_agent):
        ...
        report = UnifiedReport(
            ...
            contradictions_identified=["Contradiction"],
    ...
    def test_assess_overall_validity(self, synthesis_agent):
        assert not synthesis_agent._assess_overall_validity(
            LogicAnalysisResult(logical_validity=False), InformalAnalysisResult()
        )
        assert synthesis_agent._assess_overall_validity(
            LogicAnalysisResult(logical_validity=True),
            InformalAnalysisResult(fallacies_detected=[]),
        )

    def test_calculate_confidence_level(self, synthesis_agent):
        confidence = synthesis_agent._calculate_confidence_level(
            LogicAnalysisResult(), InformalAnalysisResult()
        )
        assert 0.0 <= confidence <= 1.0

    def test_identify_basic_contradictions(self, synthesis_agent):
        contradictions = synthesis_agent._identify_basic_contradictions(
            LogicAnalysisResult(logical_validity=True),
            InformalAnalysisResult(fallacies_detected=[{"type": "test"}]),
        )
        assert len(contradictions) > 0

    def test_generate_basic_recommendations(self, synthesis_agent):
        reco = synthesis_agent._generate_basic_recommendations(
            LogicAnalysisResult(),
            InformalAnalysisResult(fallacies_detected=[{"type": "test"}]),
        )
        assert len(reco) > 0
...
class TestSynthesisAgentIntegration:
    ...
    async def test_report_generation_integration(self, mocker, integration_agent):
        ...
        mock_report = UnifiedReport(
            ...
            logic_analysis=LogicAnalysisResult(),
            informal_analysis=InformalAnalysisResult(
                fallacies_detected=[{"type": "appel_au_sens_commun"}]
            ),
            executive_summary="Résumé",
