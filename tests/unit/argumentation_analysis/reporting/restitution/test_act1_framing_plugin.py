...
class TestWeaveFailLoud:
    ...
    def test_raised_call_is_not_recorded_as_mute(self):
        ...
        mute = run(build_act1_framing(_political_state(), llm_callable=_stub_llm("")))  # type: ignore[arg-type]
        assert raised.status == mute.status == "unavailable"
        assert raised.narrative == mute.narrative == ""
        assert mute.degraded["act1_framing"] == (
            "Cadrage indisponible — le LLM n'a rien produit (fail-loud, #1108)."
        )
        assert "TimeoutError" in raised.degraded["act1_framing"]
        assert "n'a rien produit" not in raised.degraded["act1_framing"]
        assert "sk-SENTINEL-2345" not in raised.degraded["act1_framing"]
