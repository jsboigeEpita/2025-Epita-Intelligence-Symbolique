...
class TestTacticalOperationalInterface(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_tactical_state.get_task_dependencies = MagicMock(
            return_value=["task-3"]
        )
    ...
    def test_translate_task(self):
        ...
        self.assertIsInstance(
            command["techniques"], list
        )  # La logique peut retourner une liste vide
    ...
    def test_determine_execution_parameters(self):
        ...
        task = {"estimated_duration": "short", "priority": "high"}
        ...
        task = {"estimated_duration": "medium", "priority": "medium"}
        result = self.interface._determine_execution_parameters(task)
        self.assertEqual(result["timeout"], 60)
        self.assertEqual(result["precision_target"], 0.8)

        # Tâche longue et priorité faible
        task = {"estimated_duration": "long", "priority": "low"}
        result = self.interface._determine_execution_parameters(task)
        self.assertEqual(result["timeout"], 60)
        self.assertEqual(result["precision_target"], 0.8)
    ...
    def test_determine_expected_outputs(self):
        ...
        task = {"description": "Identifier les arguments dans le texte"}
        ...
        # Tâche générique
        task = {"description": "Effectuer une analyse générale"}
        result = self.interface._determine_expected_outputs(task)
    ...
    def test_find_related_tasks(self):
        ...
        # Définir une tâche
        task = {"id": "task-1", "objective_id": "obj-1"}
            ...
            "issues": [
                {
                    ...
                    "severity": "medium",
                    "details": {"affected_arguments": ["arg-2"]},
                }
            ],
        }

        # Appeler la méthode à tester
        self.interface.process_operational_result(
    ...
    def test_translate_outputs(self):
        ...
        self.assertEqual(
            result, outputs
        )  # Dans l'implémentation actuelle, les outputs sont copiés tels quels
    ...
    def test_request_operational_status(self):
        ...
        expected_response = {"status": "ok", "tasks_in_progress": 2}
        self.mock_tactical_adapter.request_operational_info.return_value = (
            expected_response
        )

        # Appeler la méthode à tester
        result = self.interface.request_operational_status(
            "operational_agent", timeout=5.0
        )
