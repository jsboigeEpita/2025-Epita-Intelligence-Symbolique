...
class TestUnifiedConfig:
    ...
    def test_incompatible_combinations(self):
        ...
        config = UnifiedConfig(
            mock_level="none", use_real_tweety=False, use_real_llm=False
        )
...
"""
        ...
        assert config.config.get("custom_field") == "test_value" or config.config.get(
            "loaded_from_file"
        )  # Adjust based on mock
...
class TestConfigurationCLI:
    ...
    def test_cli_validation_invalid_combinations(self):
        ...
        try:
            ...
            invalid_args_ns.logic_type = (
                "invalid"  # This should cause validate_cli_args to fail
            )
