...
class TestDataClasses:
    ...
    def test_orchestration_flow(self):
        ...
        flow = OrchestrationFlow(
            agents_called=["SynthesisAgent", "LogicAgent_propositional", "InformalAgent"],
    ...
    def test_state_evolution(self):
        ...
        evolution = StateEvolution(
            ...
            belief_state_construction=[{"phase": "construction", "details": "Building belief state"}],
            progressive_enrichment=["Analysis PL simulée", "Analysis FOL simulée"],
            state_transitions=[("Initialisation", "Configuration"), ("Analyse", "Synthèse")]
        )
    ...
    def test_query_results(self):
        ...
        results = QueryResults(
            ...
            validation_results=[{"query": "p", "result": "ACCEPTED", "confidence": 0.9}],
    ...
    def test_informal_exploration(self):
        ...
        exploration = InformalExploration(
            ...
            fallacy_detection=[{"type": "ad_hominem", "confidence": 0.8, "severity": "high"}],
...
class TestTraceAnalyzer:
    ...
    def test_load_traces_success(self, trace_analyzer, sample_conversation_log, sample_report_json):
    ...
    def test_load_traces_no_files(self, trace_analyzer):
        ...
        success = trace_analyzer.load_traces("/nonexistent/path", "/another/nonexistent/path")
    ...
    def test_extract_metadata_from_conversation(self, trace_analyzer, sample_conversation_log):
    ...
    def test_extract_metadata_combined_sources(self, trace_analyzer, sample_conversation_log, sample_report_json):
    ...
    def test_extract_metadata_content_types(self, trace_analyzer):
        ...
        # Test fallback text
        trace_analyzer.raw_conversation_data = "Fichier source analysé : fallback_text.txt"
        metadata = trace_analyzer.extract_metadata()
        assert metadata.content_type == "fallback_text"
        
        # Test encrypted corpus
        trace_analyzer.raw_conversation_data = "Fichier source analysé : corpus_data.txt"
        metadata = trace_analyzer.extract_metadata()
        assert metadata.content_type == "encrypted_corpus"
    ...
    def test_analyze_orchestration_flow(self, trace_analyzer, sample_conversation_log):
        ...
        assert any("Démarrage des analyses logiques" in step[1] for step in flow.sequence_order)
        assert flow.total_execution_time == 250.5
        assert flow.success_status == "success"
    ...
    def test_analyze_informal_exploration_from_json(self, trace_analyzer, sample_report_json):
    ...
    def test_generate_comprehensive_report(self, trace_analyzer, sample_conversation_log, sample_report_json):
    ...
    def test_generate_comprehensive_report_sections(self, trace_analyzer, sample_conversation_log, sample_report_json):
    ...
    def test_generate_comprehensive_report_error_handling(self, trace_analyzer):
        ...
        with patch.object(trace_analyzer, 'extract_metadata', side_effect=Exception("Test error")):
            report = trace_analyzer.generate_comprehensive_report()
...
class TestTraceAnalyzerIntegration:
    ...
    def test_full_analysis_workflow(self):
            ...
            "formal_analysis": {
                ...
                "inference_results": {
                    ...
                    "results": [
                        ...
                        {"query": "forall x P(x)", "result": "REJECTED", "confidence": 0.75}
                    ]
                }
            },
            "informal_analysis": {
                "fallacies_detected": [
                    {"type": "ad_hominem", "confidence": 0.85, "location": "paragraph 2", "severity": "high"},
                    {"type": "strawman", "confidence": 0.72, "location": "paragraph 4", "severity": "medium"}
                ],
                "severity_distribution": {
                    "high": 1,
                    "medium": 1,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "propositional" in full_report or "first_order" in full_report or "modal" in full_report
    ...
    def test_large_data_handling(self):
        ...
        large_log = "\n".join([f"[INFO] Log line {i} with some content" for i in range(1000)])
        
        large_json = {
            "formal_analysis": {
                "logic_types": ["propositional"] * 10,
                "knowledge_base": {
                    "formulas": [f"formula_{i}" for i in range(100)]
                },
                "inference_results": {
                    "results": [{"query": f"q_{i}", "result": "ACCEPTED"} for i in range(50)]
                }
            },
            "informal_analysis": {
