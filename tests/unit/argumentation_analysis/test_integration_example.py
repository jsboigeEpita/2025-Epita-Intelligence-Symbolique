...
def test_verify_extracts_integration(mocker, integration_services, tmp_path):
    ...
    mocker.patch(
        "argumentation_analysis.utils.dev_tools.verification_utils.FetchService",
        return_value=mock_fetch_service,
    )
    mocker.patch(
        "argumentation_analysis.utils.dev_tools.verification_utils.ExtractService",
        return_value=mock_extract_service,
    )
    ...
    results = verify_all_extracts(integration_sample_definitions.to_dict_list())
