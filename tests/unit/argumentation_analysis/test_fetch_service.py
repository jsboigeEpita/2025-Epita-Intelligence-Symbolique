...
class TestFetchService:
    ...
    def test_fetch_text_from_cache(
        self, mock_get, fetch_service, sample_source_info, sample_text
    ):
    ...
    def test_fetch_text_force_refresh(
        self, mock_get, fetch_service, sample_source_info, sample_text
    ):
    ...
    def test_fetch_text_jina(
        self, mocker, mock_get, fetch_service, sample_source_info, sample_text
    ):
        ...
        # Récupérer le texte
        mock_fetch_jina = mocker.patch.object(
            fetch_service, "fetch_with_jina", return_value=sample_text
        )
        ...
        mock_fetch_jina.assert_called_once_with(
            fetch_service.reconstruct_url(
                jina_source_info["schema"],
                jina_source_info["host_parts"],
                jina_source_info["path"],
            )
        )

    def test_fetch_text_tika(
        self, mocker, mock_get, fetch_service, sample_source_info, sample_text
    ):
        ...
        # Récupérer le texte
        mock_fetch_tika = mocker.patch.object(
            fetch_service, "fetch_with_tika", return_value=sample_text
        )
        ...
        mock_fetch_tika.assert_called_once_with(
            fetch_service.reconstruct_url(
                tika_source_info["schema"],
                tika_source_info["host_parts"],
                tika_source_info["path"],
            )
        )

    def test_fetch_text_tika_plaintext(
        self, mocker, mock_get, fetch_service, sample_source_info, sample_text
    ):
        ...
        # Récupérer le texte
        mock_fetch_direct = mocker.patch.object(
            fetch_service, "fetch_direct_text", return_value=sample_text
        )
        text, message = fetch_service.fetch_text(tika_source_info)

        # Vérifier que le texte est récupéré directement
        assert text == sample_text
        mock_fetch_direct.assert_called_once_with(
            fetch_service.reconstruct_url(
                tika_source_info["schema"],
                tika_source_info["host_parts"],
                tika_source_info["path"],
    ...
    def test_fetch_direct_text(self, mock_get, fetch_service, sample_url, sample_text):
        ...
        mock_get.assert_called_once_with(
            sample_url, headers={"User-Agent": "ArgumentAnalysisApp/1.0"}, timeout=60
        )
    ...
    def test_fetch_with_jina_no_marker(
        self, mock_get, fetch_service, sample_url, sample_text
    ):
    ...
    def test_fetch_with_tika_url(
        self, mock_get, mock_put, fetch_service, sample_url, sample_text
    ):
        """Test de récupération de texte via Tika avec une URL."""
        # Simuler une réponse HTTP pour le téléchargement
        mock_get.return_value = MockResponse(
            text="binary content", content=b"binary content"
        )
    ...
    def test_fetch_with_tika_timeout(
        self, mock_get, mock_put, fetch_service, sample_url
    ):
    ...
    def test_fetch_with_tika_raw_cache(
        self,
        mock_get,
        mock_put,
        mock_read_bytes,
    ...
    def test_fetch_with_tika_raw_cache_error(
        self,
        mock_get,
        mock_put,
        mock_read_bytes,
