...
from argumentation_analysis.utils.error_estimation import (
    ...
    return [
        ...
        },
        {
            ...
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [
                        {"detected_fallacies": [{"fallacy_type": "AppealToAuthority"}]}
                    ]
                }
            },
        },
        {
            "source_name": "SourceB",
            "extract_name": "Extract_OnlyBase",
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [
                        {"detected_fallacies": [{"fallacy_type": "RedHerring"}]}
                    ]
                }
            },
        },
    ...
    return [
        {
            ...
            "analyses": {
                "contextual_fallacies": {
                    "contextual_fallacies": [
                        {"fallacy_type": "AdHominem", "confidence": 0.9},
                        {"fallacy_type": "HastyGeneralization", "confidence": 0.7},
                        {"fallacy_type": "AppealToEmotion", "confidence": 0.6},
                    ]
                },
                "complex_fallacies": {
                    "composite_severity": {"severity_level": "Modéré"}
                },
            },
        },
        {
            ...
            "analyses": {
                "contextual_fallacies": {
                    "contextual_fallacies": [
                        {"fallacy_type": "AppealToAuthority", "confidence": 0.4},
                        {"fallacy_type": "SlipperySlope", "confidence": 0.8},
                    ]
                },
                "complex_fallacies": {
                    "composite_severity": {"severity_level": "Faible"}
                },
            },
        },
        {
            "source_name": "SourceC",
            "extract_name": "Extract_OnlyAdvanced",
            "analyses": {
                "contextual_fallacies": {
                    "contextual_fallacies": [
                        {"fallacy_type": "FalseDilemma", "confidence": 0.9}
                    ]
                }
            },
        },
...
def test_estimate_false_positives_negatives_rates_success(
    ...
    error_rates = estimate_false_positives_negatives_rates(
        sample_base_results_for_error_rates, sample_advanced_results_for_error_rates
    )
    assert "base_contextual" in error_rates
    assert (
        pytest.approx(error_rates["base_contextual"]["false_positive_rate"], 0.01)
        == (1 / 3 + 0) / 2
    )
    assert (
        pytest.approx(error_rates["base_contextual"]["false_negative_rate"], 0.01)
        == (1 / 3 + 1 / 2) / 2
    )

    assert "advanced_contextual" in error_rates
    assert (
        pytest.approx(error_rates["advanced_contextual"]["false_positive_rate"], 0.01)
        == (0 / 1 + 1 / 2) / 2
    )  # Ajusté pour refléter la logique de la fonction
    assert error_rates["advanced_contextual"]["false_negative_rate"] == 0.0

    assert "advanced_complex" in error_rates
    assert (
        pytest.approx(error_rates["advanced_complex"]["false_positive_rate"], 0.01)
        == (0.1 + 0.2) / 2
    )
...
def test_estimate_false_positives_negatives_rates_no_common_extracts(
    ...
    error_rates = estimate_false_positives_negatives_rates(
        sample_base_results_for_error_rates, no_common_advanced_results
    )
...
@pytest.mark.use_real_numpy
def test_estimate_false_positives_negatives_rates_empty_inputs(
    sample_base_results_for_error_rates, sample_advanced_results_for_error_rates
):
    error_rates_empty_base = estimate_false_positives_negatives_rates(
        [], sample_advanced_results_for_error_rates
    )
    error_rates_empty_advanced = estimate_false_positives_negatives_rates(
    ...
    expected_empty_rates = {"false_positive_rate": 0.0, "false_negative_rate": 0.0}
    for rates_dict in [
        error_rates_empty_base,
        error_rates_empty_advanced,
        error_rates_both_empty,
    ]:
...
def test_estimate_false_positives_negatives_rates_missing_fallacy_data():
    ...
    error_rates = estimate_false_positives_negatives_rates(
        base_missing_fallacies, advanced_missing_fallacies
    )
