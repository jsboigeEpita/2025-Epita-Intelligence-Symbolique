...
from pathlib import (
    Path,
)  # Bien que non utilisé directement, souvent utile pour les fixtures
...
def sample_results_for_time_extraction() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "extract_name": "Extract_NoMainTS",
            "analyses": {"analysis_A": {"analysis_timestamp": "2023-01-01T12:00:05"}},
        },
        {
            "extract_name": "Extract_NoAnalysisTS",
            "timestamp": "2023-01-01T13:00:00",
            "analyses": {"analysis_A": {}},
        },
        {
            "extract_name": "Extract_InvalidMainTS",
            "timestamp": "invalid-date-format",
            "analyses": {"analysis_A": {"analysis_timestamp": "2023-01-01T14:00:05"}},
        },
        {
            "extract_name": "Extract_InvalidAnalysisTS",
            "timestamp": "2023-01-01T15:00:00",
            "analyses": {"analysis_A": {"analysis_timestamp": "invalid-date"}},
        },
        {"extract_name": "Extract_NoAnalysesKey", "timestamp": "2023-01-01T16:00:00"},
        {
            "extract_name": "Extract_AnalysesNotDict",
            "timestamp": "2023-01-01T17:00:00",
            "analyses": "not_a_dict",
        },
        {
            "timestamp": "2023-01-01T18:00:00",
            "analyses": {"analysis_X": {"analysis_timestamp": "2023-01-01T18:00:05"}},
        },
    ]
...
def test_extract_execution_time_success(
    ...
    assert (
        "Extract1" in execution_times
        and execution_times["Extract1"]["analysis_A"] == 5.0
        and execution_times["Extract1"]["analysis_B"] == 15.0
    )
    assert (
        "Extract2" in execution_times
        and execution_times["Extract2"]["analysis_A"] == 10.0
        and execution_times["Extract2"]["analysis_C"] == 30.0
    )
...
def test_extract_execution_time_missing_or_invalid_data(
    ...
    found_generated_name = any(
        key.startswith("Inconnu_") and execution_times[key].get("analysis_X") == 5.0
...
def sample_results_for_fallacy_counting() -> List[Dict[str, Any]]:
    return [
            ...
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [
                        ...
                        {"detected_fallacies": [{"type": "Hasty Generalization"}]},
                    ]
                },
                "complex_fallacies": {
        ...
        {
            "extract_name": "Extract_Fallacy_None",
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [{"detected_fallacies": []}],
                    "contextual_fallacies_count": 0,
                },
                "complex_fallacies": {
        ...
        {
            "extract_name": "Extract_Fallacy_MissingKeys",
            "analyses": {"contextual_fallacies": {}, "complex_fallacies": {}},
        },
        ...
        {"extract_name": "Extract_AnalysesNotDict", "analyses": "not_a_dict"},
        {
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [
                        {"detected_fallacies": [{"type": "Appeal to Emotion"}]}
                    ]
                }
            }
        },
        {
            "extract_name": "Extract_AdvContextual",
            "analyses": {
                "contextual_fallacies": {
                    "argument_results": [
                        {"detected_fallacies": [{"type": "Ad Hominem"}]}
                    ],
                    "contextual_fallacies_count": 5,
                },
                "complex_fallacies": {"individual_fallacies_count": 0},
            },
        },
    ]
...
def test_count_fallacies_success(
    ...
    results_subset = [
        ...
        sample_results_for_fallacy_counting[6],
    ]
    fallacy_counts = count_fallacies_in_results(results_subset)
    assert (
        fallacy_counts["Extract_Fallacy1"]["base_contextual"] == 3
        and fallacy_counts["Extract_Fallacy1"]["advanced_complex"] == 7
        and fallacy_counts["Extract_Fallacy1"]["advanced_contextual"] == 0
    )
    assert (
        fallacy_counts["Extract_AdvContextual"]["base_contextual"] == 1
        and fallacy_counts["Extract_AdvContextual"]["advanced_complex"] == 0
        and fallacy_counts["Extract_AdvContextual"]["advanced_contextual"] == 5
    )
...
def test_count_fallacies_none_and_missing(
    ...
    fallacy_counts = count_fallacies_in_results(results_subset)
    assert (
        fallacy_counts["Extract_Fallacy_None"]["base_contextual"] == 0
        and fallacy_counts["Extract_Fallacy_None"]["advanced_complex"] == 0
        and fallacy_counts["Extract_Fallacy_None"]["advanced_contextual"] == 0
    )
    assert (
        fallacy_counts["Extract_Fallacy_MissingKeys"]["base_contextual"] == 0
        and fallacy_counts["Extract_Fallacy_MissingKeys"]["advanced_complex"] == 0
        and fallacy_counts["Extract_Fallacy_MissingKeys"]["advanced_contextual"] == 0
    )
    assert fallacy_counts["Extract_NoAnalyses"] == {
        "base_contextual": 0,
        "advanced_complex": 0,
        "advanced_contextual": 0,
    }
    assert fallacy_counts["Extract_AnalysesNotDict"] == {
        "base_contextual": 0,
        "advanced_complex": 0,
        "advanced_contextual": 0,
    }
...
def test_count_fallacies_extract_name_missing(
    ...
    assert (
        generated_name_key.startswith("Inconnu_")
        and fallacy_counts[generated_name_key]["base_contextual"] == 1
    )
...
def sample_results_for_confidence_scores() -> List[Dict[str, Any]]:
    return [
        {
            "extract_name": "Extract_Conf1",
            "analyses": {
                "argument_coherence": {"overall_coherence": {"score": 0.85}},
                "rhetorical_results": {
                    "overall_analysis": {"rhetorical_quality": 0.75},
                    "coherence_analysis": {"overall_coherence": 0.90},
                },
                "fallacy_severity": {"overall_severity": 0.60},
            },
        },
        {
            "extract_name": "Extract_Conf_Zeros",
            "analyses": {
                "argument_coherence": {"overall_coherence": {}},
                "rhetorical_results": {"overall_analysis": {"rhetorical_quality": 0.0}},
            },
        },
        {
            "extract_name": "Extract_Conf_MissingStruct",
            "analyses": {
                "argument_coherence": "not_a_dict",
                "rhetorical_results": {"overall_analysis": "not_a_dict"},
            },
        },
...
def test_extract_confidence_scores_success(
    ...
    assert (
        scores1["base_coherence"] == 0.85
        and scores1["advanced_rhetorical"] == 0.75
        and scores1["advanced_coherence"] == 0.90
        and scores1["advanced_severity"] == 0.60
    )
...
def test_extract_confidence_scores_zeros_and_missing(
    ...
    confidence_scores = extract_confidence_scores_from_results(results_subset)
    assert (
        confidence_scores["Extract_Conf_Zeros"]["base_coherence"] == 0.0
        and confidence_scores["Extract_Conf_Zeros"]["advanced_rhetorical"] == 0.0
    )
...
def test_extract_confidence_scores_extract_name_missing(
    ...
    assert (
        generated_name_key.startswith("Inconnu_")
        and confidence_scores[generated_name_key]["base_coherence"] == 0.5
    )
...
def sample_results_for_richness_analysis() -> List[Dict[str, Any]]:
    return [
        {
            "extract_name": "Extract_Rich1",
            "analyses": {
                "contextual_fallacies": {
                    "contextual_factors": {"f1": "v1", "f2": "v2"},
                    "context_analysis": {
                        "context_type": "Debate",
                        "context_subtypes": ["Pol", "Pub"],
                        "audience_characteristics": ["Exp", "Lay"],
                        "formality_level": "Formal",
                    },
                },
                "rhetorical_results": {
                    "overall_analysis": {
                        "main_strengths": ["Clarity", "Evidence"],
                        "main_weaknesses": ["Tone"],
        ...
        {
            "extract_name": "Extract_Rich_Empty",
            "analyses": {
                "contextual_fallacies": {
                    "contextual_factors": {},
                    "context_analysis": {
                ...
                "rhetorical_results": {
                    "overall_analysis": {"main_strengths": [], "main_weaknesses": []}
                },
            },
        },
        {
            "extract_name": "Extract_Rich_MissingStruct",
            "analyses": {
                "contextual_fallacies": "not_a_dict",
                "rhetorical_results": {},
            },
        },
...
def test_analyze_contextual_richness_success(
    ...
    assert (
        scores1["base_contextual"] == 2.0
        and scores1["advanced_contextual"] == 6.0
        and scores1["advanced_rhetorical"] == 4.0
    )
...
def test_analyze_contextual_richness_extract_name_missing(
    ...
    assert (
        generated_name_key.startswith("Inconnu_")
        and richness_scores[generated_name_key]["base_contextual"] == 1.0
    )
...
def sample_results_for_coherence_relevance() -> List[Dict[str, Any]]:
    return [
        {
            "extract_name": "Extract_CohRel1",
            "analyses": {
                "argument_coherence": {
                    "recommendations": ["R1", "R2"],
                    "coherence_evaluations": {"e1": {}, "e2": {}, "e3": {}},
                },
                "rhetorical_results": {
                    "coherence_analysis": {
                    ...
                    "recommendations": {
                        "coherence_recommendations": ["AR1", "AR2", "AR3"]
                    },
                },
            },
        },
        {
            "extract_name": "Extract_CohRel_Empty",
            "analyses": {
                "argument_coherence": {
                    "recommendations": [],
                ...
                "rhetorical_results": {
                    "coherence_analysis": {},
                    "recommendations": {"coherence_recommendations": []},
                },
            },
        },
        {
            "extract_name": "Extract_CohRel_MissingStruct",
            "analyses": {
                "argument_coherence": "not_a_dict",
                "rhetorical_results": {
                    "coherence_analysis": "not_a_dict",
                    "recommendations": "not_a_dict",
                },
            },
        },
...
def test_evaluate_coherence_relevance_success(
    ...
    assert (
        scores1["base_coherence"] == 5.0
        and scores1["advanced_coherence"] == 4.0
        and scores1["advanced_recommendations"] == 3.0
    )
...
def test_evaluate_coherence_relevance_extract_name_missing(
    ...
    assert (
        generated_name_key.startswith("Inconnu_")
        and relevance_scores[generated_name_key]["base_coherence"] == 1.0
    )
...
def test_calculate_obj_complexity():
    ...
    assert (
        pytest.approx(_calculate_obj_complexity({"k1": "v1", "k2": {"nk1": "nv1"}}))
        == 2.0
    )
...
def sample_results_for_complexity() -> List[Dict[str, Any]]:
    return [
        {
            "extract_name": "Extract_Comp1",
            "analyses": {
                "simple_analysis": "just a string",
                "list_analysis": [1, 2, 3],
                "dict_analysis": {"k1": "v1", "k2": "v2"},
            },
        },
        {
            "extract_name": "Extract_Comp_Nested",
            "analyses": {
                "nested_list": ["a", ["b", ["c"]]],
                "nested_dict": {
                    "k1": "v1",
                    "k2": {"nk1": "nv1", "nk2": {"nnk1": "nnv1"}},
                },
            },
        },
        ...
        {
            "extract_name": "Extract_Comp_AnalysesNotDict",
            "analyses": "not_a_dict_or_list",
        },
    ]
...
def test_analyze_result_complexity_success(
    ...
    assert (
        scores1["simple_analysis"] == 0.0
        and pytest.approx(scores1["list_analysis"]) == 1.0
        and pytest.approx(scores1["dict_analysis"]) == 1.0
    )
    scores_nested = complexity_scores["Extract_Comp_Nested"]
    assert (
        pytest.approx(scores_nested["nested_list"]) == 2.75
        and pytest.approx(scores_nested["nested_dict"]) == 2.75
    )
