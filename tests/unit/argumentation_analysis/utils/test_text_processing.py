...
def test_split_no_delimiters_short_text():
    ...
    assert split_text_into_arguments("Texte assez long", min_arg_length=5) == expected_long_enough
...
def test_split_text_ends_without_delimiter_but_is_argument():
    ...
    expected = ["Ceci est un argument qui ne finit pas par un point mais est assez long"]
    assert split_text_into_arguments(text, min_arg_length=10) == expected
