...
def test_check_syntax_invalid_file(temp_python_file):
    ...
    assert any(
        "my_func()" in line for line in context
    ), "Le contexte devrait inclure la ligne avec l'erreur"
...
def test_check_tokens_with_indentation_error_token(temp_python_file):
    ...
    if syntax_ok:
        ...
        assert (
            tokens_ok is False
        ), "check_python_tokens aurait dû trouver une erreur de token."
        assert (
            "Erreur de tokenization" in msg or "tokens d'erreur ont été détectés" in msg
        )
        assert len(errors) > 0
        assert any(
            "indent" in err.get("message", "").lower()
            or "dedent" in err.get("message", "").lower()
            for err in errors
        )
    else:
        ...
        pytest.skip(
            "Syntaxe déjà invalide, test de TokenError spécifique non exécuté pour ce cas."
        )


def test_check_tokens_file_not_found():
    """Teste check_python_tokens avec un fichier inexistant."""
    tokens_ok, msg, errors = check_python_tokens(
        "fichier_inexistant_pour_test_tokens.py"
    )
