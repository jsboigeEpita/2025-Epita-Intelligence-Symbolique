...
from argumentation_analysis.utils.dev_tools.encoding_utils import (
    fix_file_encoding,
    logger as encoding_utils_logger,
)
...
def test_fix_encoding_custom_source_encodings(temp_file_for_encoding_test):
    ...
    result_success = fix_file_encoding(
        str(file_path), source_encodings=["koi8-r", "utf-8"]
    )
...
def test_fix_encoding_custom_target_encoding(temp_file_for_encoding_test):
    ...
    result = fix_file_encoding(
        str(file_path), target_encoding="latin-1", source_encodings=["utf-8"]
    )
    assert result is True
