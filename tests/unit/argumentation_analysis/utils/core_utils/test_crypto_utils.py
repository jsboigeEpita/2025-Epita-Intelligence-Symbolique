...
def test_derive_encryption_key_success():
    ...
    assert (
        key1 == key2
    ), "La dérivation de clé doit être déterministe pour la même passphrase et le même sel."
...
)  # Assure un environnement propre pour chaque test de variable d'env
...
def test_load_encryption_key_from_settings(mock_settings):
    ...
    mock_settings.passphrase_arg = (
        None  # Assurer que l'argument direct n'est pas utilisé
    )
...
def test_fernet_encryption_decryption_cycle():
    ...
    assert (
        derived_key_bytes is not None
    ), "La dérivation de clé a échoué avant le test Fernet."
    ...
    assert (
        decrypted == test_data
    ), "Les données déchiffrées ne correspondent pas aux originales."
...
def test_decrypt_with_wrong_key_fernet():
    ...
    assert (
        correct_key_bytes != wrong_key_bytes
    ), "Les clés correcte et incorrecte doivent être différentes."
    ...
    decrypted_with_wrong_key = decrypt_data_with_fernet(
        encrypted_with_correct_key, wrong_key_bytes
    )
    assert (
        decrypted_with_wrong_key is None
    ), "Le déchiffrement avec une mauvaise clé doit retourner None (InvalidToken)."
