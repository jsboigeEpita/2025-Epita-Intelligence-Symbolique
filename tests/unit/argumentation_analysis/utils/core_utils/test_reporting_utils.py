...
def sample_analysis_results() -> list:
    return [
        ...
        {"source_name": "Texte Beta", "error": "Erreur d'analyse majeure."},
        {
...
def test_generate_json_report_calls_save_json_report(
    tmp_path, sample_analysis_results, mocker
):
...
def test_generate_json_report_logs_error_on_save_failure(
    tmp_path, sample_analysis_results, mocker, caplog
):
...
def test_generate_specific_rhetorical_markdown_report_success(
    tmp_path, sample_analysis_results, mocker
):
    ...
    assert (
        "**Erreur lors de l'analyse :** `Erreur d'analyse majeure.`"
        in report_content_generated
    )
...
def test_generate_specific_rhetorical_markdown_report_empty_analysis(tmp_path, mocker):
    ...
    empty_results = [
        ...
        {
            "source_name": "Sans Analyse",
            "analysis": None,
        },  # Simuler un cas où analysis pourrait être None
    ]
...
def test_generate_specific_rhetorical_markdown_report_save_fails(
    tmp_path, sample_analysis_results, mocker, caplog
):
...
def sample_corpus_data_full() -> dict:
    return {
        ...
        "recommendations": [
            "Utiliser Agent X pour ce type de texte.",
            "Surveiller les faux positifs de B.",
        ],
    }
...
def test_generate_markdown_report_for_corpus_full_data(sample_corpus_data_full):
    ...
    report_lines = generate_markdown_report_for_corpus(
        corpus_name, sample_corpus_data_full
    )
...
def test_generate_markdown_report_for_corpus_minimal_data(sample_corpus_data_minimal):
    ...
    report_lines = generate_markdown_report_for_corpus(
        corpus_name, sample_corpus_data_minimal
    )
    report_str = "\n".join(report_lines)
...
def test_generate_markdown_report_for_corpus_type_error():
    ...
    with pytest.raises(
        TypeError,
        match="Le paramètre 'corpus_name' doit être une chaîne de caractères.",
    ):
...
def sample_all_average_scores() -> dict:
    return {
        "Corpus Alpha": {
            ...
            "contextual_richness": 0.70,
            "processing_time": 10.5,
        },
        "Corpus Beta": {
            "confidence_score": 0.92,
            "contextual_richness": 0.85,
            "processing_time": 8.2,
        },
        "Corpus Gamma": {
            "contextual_richness": 0.65,
            "custom_metric": 0.99,
        },  # Métrique manquante, métrique en plus
    }
...
def test_generate_overall_summary_markdown_success(sample_all_average_scores):
    ...
    assert (
        "| Corpus | Confidence Score | Contextual Richness | Custom Metric | Processing Time |"
        in report_str
    )
    # Vérifier une ligne de données
    assert (
        "| Corpus Alpha | 0.88 | 0.70 | 0.00 | 10.50 |" in report_str
    )  # custom_metric est 0.00 car manquante
    assert "| Corpus Beta | 0.92 | 0.85 | 0.00 | 8.20 |" in report_str
    assert (
        "| Corpus Gamma | 0.00 | 0.65 | 0.99 | 0.00 |" in report_str
    )  # confidence et processing_time manquantes
...
def test_generate_overall_summary_markdown_type_error():
    ...
    with pytest.raises(
        TypeError, match="Le paramètre 'all_average_scores' doit être un dictionnaire."
    ):
...
def test_generate_performance_comparison_markdown_report_runs(tmp_path, mocker):
    ...
    advanced_metrics = {
        ...
        "confidence_scores": {
            "advanced_rhetorical": 0.8,
            "advanced_coherence": 0.75,
            "advanced_severity": 0.9,
        },
        "richness_scores": {"advanced_contextual": 0.7, "advanced_rhetorical": 0.85},
    }

    generate_performance_comparison_markdown_report(
        base_metrics, advanced_metrics, output_file
    )
    ...
    assert (
        "Agent contextuel de base | 10 |" in report_content
    )  # Vérifie une partie des données
    assert (
        "L'agent rhétorique avancé fournit l'analyse contextuelle la plus riche."
        in report_content
    )  # Vérifie une partie de l'analyse
