...
from argumentation_analysis.core.utils.network_utils import download_file, network_breaker
...
def test_download_file_cleans_up_partial_on_request_exception(tmp_path, caplog):
    ...
    with patch('requests.get', side_effect=requests.exceptions.ConnectionError("Simulated network error")), \
         patch('os.remove') as mock_remove, \
         patch.object(Path, 'exists', return_value=True):
...
def test_download_file_cleans_up_partial_on_unexpected_exception(mock_requests_get, tmp_path, caplog):
    ...
    mock_response = mock_requests_get.return_value.__enter__.return_value
    mock_response.iter_content.side_effect = ValueError("Unexpected error during iteration")
...
def test_download_file_handles_os_error_on_remove_after_exception(tmp_path, caplog):
    ...
    with patch('requests.get', side_effect=requests.exceptions.Timeout("Simulated timeout")), \
         patch.object(Path, 'exists', return_value=True), \
         patch('os.remove', side_effect=OSError("Simulated permission error on remove")):
