...
def reset_logging_state():
    ...
    original_specific_levels = {
        name: logging.getLogger(name).level for name in all_specific_loggers
    }
    ...
    for handler_orig in original_handlers:
        if (
            handler_orig not in logging.root.handlers
        ):  # Évite d'ajouter si déjà restauré par ex.
...
def test_setup_logging_valid_levels(level_str):
    ...
    assert (
        logging.getLogger().getEffectiveLevel() == expected_level
...
def test_setup_logging_configures_handler_and_formatter(
    mock_stream_handler_class, caplog
):
    """
    ...
    mock_handler_instance.level = (
        logging.DEBUG
    )  # Correspond au log_level_str passé à setup_logging
    ...
    assert (
        mock_handler_instance in logging.getLogger().handlers
    ...
    assert isinstance(
        formatter_arg, logging.Formatter
    ), "L'argument de setFormatter devrait être un logging.Formatter"

    expected_format_str = "%(asctime)s [%(levelname)s] [%(name)s] %(message)s"
    expected_date_fmt_str = "%H:%M:%S"

    actual_format_str = (
        formatter_arg._style._fmt
        if hasattr(formatter_arg, "_style")
        else formatter_arg._fmt
    )

    assert (
        actual_format_str == expected_format_str
    ), f"Le format du formateur est incorrect. Attendu: '{expected_format_str}', Obtenu: '{actual_format_str}'"
    assert (
        formatter_arg.datefmt == expected_date_fmt_str
...
def test_setup_logging_quiets_third_party_libraries():
    ...
    for lib_name in libraries_to_quiet:
        assert (
            logging.getLogger(lib_name).getEffectiveLevel() == logging.WARNING
...
def test_setup_logging_project_specific_loggers_inherit_level():
    ...
    for logger_name in project_specific_loggers:
        assert (
            logging.getLogger(logger_name).getEffectiveLevel() == logging.DEBUG
    ...
    for logger_name in project_specific_loggers:
        assert (
            logging.getLogger(logger_name).getEffectiveLevel() == logging.ERROR
...
def test_setup_logging_removes_existing_handlers():  # mocker n'est plus nécessaire ici
    ...
    ), "Le logger racine devrait être sans handlers au début du test."

    # 2. Ajouter un handler factice
    dummy_handler = logging.StreamHandler(
        sys.stderr
    )  # Utiliser stderr pour le distinguer
    dummy_handler.set_name("dummy_handler_for_test")
    root_logger.addHandler(dummy_handler)
    assert (
        dummy_handler in root_logger.handlers
    ), "Le handler factice devrait être présent."
    ...
    assert (
        dummy_handler not in root_logger.handlers
