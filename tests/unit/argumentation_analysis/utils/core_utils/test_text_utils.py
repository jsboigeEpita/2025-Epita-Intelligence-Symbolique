...
def test_normalize_text_normalize_spaces():
    assert (
        normalize_text("Extra   spaces  here.") == "extra spaces here"
    )  # Corrected: strip removes trailing period if space before


def test_normalize_text_with_accents():
    assert (
        normalize_text("Voilà un résumé.") == "voila un resume"
    )  # Corrected: strip removes trailing period
...
def test_normalize_text_mixed_punctuation_and_spaces():
    assert (
        normalize_text("  Test,,, with   multiple issues!!  ")
...
def test_normalize_text_apostrophes_internal():
    assert (
        normalize_text("l'important c'est d'avoir l'apostrophe")
        == "l'important c'est d'avoir l'apostrophe"
    )


def test_normalize_text_apostrophes_external_and_multiple():
    assert (
        normalize_text("'début' et ''fin'' et 'isolée' et l'''exemple'''")
        == "debut et fin et isolee et l'exemple"
    )
...
def test_tokenize_text_simple_sentence():
    ...
    assert tokenize_text("This is a simple sentence.") == [
        "this",
        "is",
        "a",
        "simple",
