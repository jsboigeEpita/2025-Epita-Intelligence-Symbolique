...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "nom_tres_long_qui_devrait_etre_tronque_pour_respecter_la_limite_de_longueur_maximale_autorisee.log",
        ),  # Exemple de troncature (si max_len est plus petit)
        ...
        (
            "a.b.c.d.e.f.g.h.i.j.k.l.m.n.o.p.q.r.s.t.u.v.w.x.y.z.longextension",
            "a_b_c_d_e_f_g_h_i_j_k_l_m_n_o_p_q_r_s_t_u_v_w_x_y_z.longextension",
        ),  # Troncature du nom, pas de l'extension
    ],
)
...
def test_sanitize_filename_complex_extensions():
    ...
    assert (
        sanitize_filename("archive.zip.encrypted", max_len=255)
        == "archive_zip.encrypted"
    )
    assert (
        sanitize_filename("document.v1.2.final.docx", max_len=255)
        == "document_v1_2_final.docx"
    )
    # Test avec troncature
    assert (
        sanitize_filename("document.v1.2.final.docx", max_len=20)
        == "document_v1_2_f.docx"
    )
...
def test_save_and_load_text_file_success(tmp_path):
    ...
    # Sauvegarde
    assert (
        save_text_file(file_path, content_to_save) is True
    ), "La sauvegarde aurait dû réussir."
    ...
    assert (
        loaded_content == content_to_save
    ), "Le contenu chargé ne correspond pas au contenu sauvegardé."
...
def test_save_text_file_io_error(mocker, tmp_path):
    ...
    assert (
        save_text_file(file_path, content) is False
    ), "Devrait retourner False en cas d'IOError."
...
def sample_json_list_data() -> list:
    return [
        ...
        {"id": 2, "name": "Test 2", "value": None, "active": False},
    ]
...
def test_load_json_file_decode_error(tmp_path, caplog):
    ...
    with open(file_path, "w") as f:
        f.write(
            "{'bad_json': True,}"
        )  # JSON invalide (virgule en trop, quotes simples)
...
def test_load_json_file_double_encoded_string(tmp_path, sample_json_list_data, caplog):
    ...
    assert loaded_data == sample_json_list_data
    assert (
        f"Contenu de {str(file_path.resolve())} est une chaîne, tentative de re-parse JSON."
        in caplog.text
    )
...
def test_load_json_file_double_encoded_string_invalid_inner(tmp_path, caplog):
    ...
    double_encoded_content = json.dumps(
        invalid_inner_json_string
    )  # La chaîne externe est un JSON valide
...
def test_save_markdown_to_html_success(tmp_path, mocker):
    ...
    mock_markdown_converter = mocker.patch(
        "argumentation_analysis.utils.core_utils.file_utils.markdown.markdown"
    )
...
def test_check_path_exists_file_success(tmp_path):
    ...
    assert (
        check_path_exists(file_path, path_type="file") is True
...
def test_check_path_exists_not_found_raises_sysexit(tmp_path, mocker):
    ...
    check_path_exists(
        non_existent_path, path_type="file"
    )  # L'appel à sys.exit devrait se produire ici
...
def test_check_path_exists_invalid_type_param_raises_sysexit(tmp_path, mocker):
    ...
    some_path.write_text(
        "content"
    )  # Le chemin doit exister pour atteindre la vérification du type
...
@pytest.mark.parametrize(
    ...
    [
        (
            "archives",
            "data/raw/project_alpha/file.txt",
            2,
            "archives/raw/project_alpha/file.txt",
        ),  # Corrigé
        ...
        (
            ...
            -1,
            "archives/file.txt",
        ),  # preserve_levels négatif traité comme 0
        (
            "archives",
            "file.txt",
            2,
            "archives/file.txt",
        ),  # Moins de parents que preserve_levels
        ...
        (
            ...
            10,
            "base/a/b/c/d/e.f",
        ),  # preserve_levels > nombre de parents
    ],
)
def test_create_archive_path(
    base_archive_str, source_str, preserve_levels, expected_str, tmp_path
):
    ...
    result_path = create_archive_path(
        base_archive_dir, source_file_path, preserve_levels
    )
    ...
    assert (
        result_path.parent.exists()
    ), "Le répertoire parent de la destination aurait dû être créé."
...
def test_archive_file_source_not_found(tmp_path, caplog):
    ...
    assert (
        f"❌ Le fichier source {non_existent_source.resolve()} n'existe pas ou n'est pas un fichier."
        in caplog.text
    )
...
def test_archive_file_permission_error(tmp_path, mocker, caplog):
    ...
    mocker.patch(
        "shutil.move", side_effect=PermissionError("Simulated Permission Denied")
    )
...
def test_load_extracts_success_returns_list(tmp_path, sample_json_list_data, mocker):
    ...
    mocked_load_json = mocker.patch(
        "argumentation_analysis.utils.core_utils.file_loaders.load_json_file",
        return_value=sample_json_list_data,
    )
...
def test_load_extracts_returns_empty_list_on_load_json_file_none(
...
def test_load_extracts_returns_empty_list_on_load_json_file_not_list(
    tmp_path, sample_json_dict_data, mocker, caplog
):
    """Teste que load_extracts retourne une liste vide si load_json_file retourne un dict."""
    file_path = tmp_path / "extracts_dict.json"
    mocked_load_json = mocker.patch(
        "argumentation_analysis.utils.core_utils.file_loaders.load_json_file",
        return_value=sample_json_dict_data,
    )
    ...
    mocked_load_json.assert_called_once_with(file_path)
    assert (
        f"Les données chargées depuis {str(file_path.resolve())} ne sont pas une liste comme attendu pour des extraits."
        in caplog.text
    )


# Tests pour load_base_analysis_results
def test_load_base_analysis_results_success_returns_list(
    tmp_path, sample_json_list_data, mocker
):
...
def test_load_base_analysis_results_returns_empty_list_on_load_json_file_none(
    tmp_path, mocker, caplog
):
...
def test_load_base_analysis_results_returns_empty_list_on_load_json_file_not_list(
    tmp_path, sample_json_dict_data, mocker, caplog
):
    ...
    mocked_load_json.assert_called_once_with(file_path)
    assert (
        f"Les données chargées depuis {str(file_path.resolve())} pour les résultats d'analyse de base ne sont pas une liste."
        in caplog.text
    )
