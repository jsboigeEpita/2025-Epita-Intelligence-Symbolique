...
class TestLLMService:
    ...
    def test_create_llm_service_openai_authentic(self):
        ...
        if not self.api_key:
            pytest.skip(
                "La variable d'environnement OPENAI_API_KEY est requise pour ce test."
            )
        ...
        async def _run_async_test():
            try:
                ...
                prompt_function = kernel.add_function(
                    function_name="echo", plugin_name="test", prompt=prompt_template
                )

                prompt = "Bonjour, es-tu là ?"
                result = await kernel.invoke(
                    prompt_function, KernelArguments(input=prompt)
                )
                ...
                assert (
                    len(response_str) > 0
                ), "La réponse du LLM ne devrait pas être vide."
                logging.info(
                    f"Réponse authentique du LLM reçue : '{response_str[:50]}...'"
                )
