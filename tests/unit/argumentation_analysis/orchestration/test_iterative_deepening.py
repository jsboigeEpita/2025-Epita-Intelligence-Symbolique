...
class MockTaxonomy:
    ...
    def __init__(self, nodes=None):
        self.nodes = nodes or {}
        self._roots = [v for v in self.nodes.values() if v.get("depth") == 0]

    def get_root_nodes(self):
        return self._roots

    def get_children(self, pk: str):
        return [v for v in self.nodes.values() if v.get("parent_pk") == pk]
...
SAMPLE_TAXONOMY = MockTaxonomy(
    {
        ...
        "root1.1": {
            "PK": "root1.1",
            "depth": 1,
            "text_fr": "Abus personnel",
            "parent_pk": "root1",
        },
        "root1.1.1": {
            "PK": "root1.1.1",
            "depth": 2,
            "text_fr": "Empoisonnement du puits",
            "parent_pk": "root1.1",
        },
        "root2": {
            "PK": "root2",
            "depth": 0,
            "text_fr": "Appel à l'autorité",
            "parent_pk": None,
        },
    }
)
...
class TestProtocols:
    ...
    def test_kernel_creation(self):
        ...
        assert (
            len(functions) >= 3
        )  # explore_branch, confirm_fallacy, conclude_no_fallacy
...
class TestTimeoutProtection:
    ...
    async def test_timeout_returns_none(self):
        ...
        mock_llm.get_chat_message_contents = AsyncMock(
        ...
        result = await orch._llm_call_with_timeout(history, MagicMock(), kernel)
