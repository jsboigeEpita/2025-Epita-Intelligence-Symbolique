...
class TestPrebuiltWorkflows:
    ...
    def test_build_standard_workflow(self):
        """Standard workflow has 13 phases (incl. optional CamemBERT, hierarchical fallacy, nl_to_logic, pl, fol, dung, aspic)."""
        from argumentation_analysis.orchestration.unified_pipeline import (
...
class TestInvokeCallables:
    ...
    async def test_invoke_dung_extensions_error(self):
        """_invoke_dung_extensions returns Python fallback on exception."""
        from argumentation_analysis.orchestration.unified_pipeline import (
    ...
    async def test_invoke_dung_extensions_no_arguments(self):
        ...
        with patch.dict(
            ...
            result = await _invoke_dung_extensions("text", {})
        # Should have called with argument_placeholder (from _extract_arguments_from_context)
