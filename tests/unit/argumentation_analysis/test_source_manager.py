...
class TestSourceManager:
    ...
    def complex_config(self):
        ...
        return SourceConfig(
            source_type=SourceType.COMPLEX, passphrase="test_passphrase"
        )
    ...
    def test_load_sources_simple_type(self, mocker, source_manager_simple):
        ...
        mock_simple = mocker.patch.object(
            source_manager_simple,
            "_load_simple_sources",
    ...
    def test_load_sources_complex_type(self, mocker, source_manager_complex):
        ...
        mock_complex = mocker.patch.object(
            source_manager_complex,
            "_load_complex_sources",
            return_value=(None, "Success"),
        )
    ...
    def test_load_simple_sources_success(self, mocker, source_manager_simple):
        ...
        mocker.patch.object(
            source_manager_simple._legacy_manager,
            "load_sources",
            return_value=("definitions", "Success"),
        )
    ...
    def test_load_simple_sources_exception(self, mocker, source_manager_simple):
        ...
        mocker.patch.object(
            source_manager_simple._legacy_manager,
            "load_sources",
            side_effect=ValueError("Legacy error"),
        )
    ...
    def test_load_complex_sources_success(self, mocker, source_manager_complex):
        ...
        mocker.patch.object(
            source_manager_complex,
            "_load_complex_sources",
            return_value=("complex_definitions", "Complex Success"),
        )
    ...
    def test_load_complex_sources_env_passphrase(self, mocker, source_manager_complex):
        ...
        mocker.patch.dict(
            os.environ, {"TEXT_CONFIG_PASSPHRASE": "env_passphrase"}, clear=True
        )
        ...
        mock_legacy_load = mocker.patch.object(
            ...
            "load_sources",
            return_value=(None, "Loaded with env pass"),
        )
    ...
    def test_load_complex_sources_key_derivation_failure(
        ...
        mocker.patch.object(
            source_manager_complex._legacy_manager,
            "load_sources",
            side_effect=RuntimeError("Key derivation failure"),
        )
    ...
    def test_select_text_for_analysis_empty_sources(
        self, mocker, source_manager_simple
    ):
        ...
        text, description = source_manager_simple.select_text_for_analysis(
            mock_definitions
        )
    ...
    def test_select_text_for_analysis_simple_sources(
        self, mocker, source_manager_simple
    ):
        ...
        mock_select = mocker.patch.object(
            ...
            return_value=(
                "Texte d'analyse depuis source simple",
                "Source simple: Source de test",
            ),
        )
    ...
    def test_select_text_for_analysis_complex_sources(
        self, mocker, source_manager_complex
    ):
        ...
        text, description = source_manager_complex.select_text_for_analysis(
            mock_definitions
        )
    ...
    def test_select_text_for_analysis_complex_no_anonymization(self, mocker):
        ...
        config = SourceConfig(
            source_type=SourceType.COMPLEX, passphrase="test", anonymize_logs=False
        )
        manager = SourceManager(config)
        mock_definitions = mocker.MagicMock(spec=ExtractDefinitions)
        mock_definitions.sources = [mocker.MagicMock()]
    ...
    def test_select_text_for_analysis_short_complex_content(
        self, mocker, source_manager_complex
    ):
    ...
    def test_cleanup_sensitive_data_disabled(self):
        ...
        config = SourceConfig(source_type=SourceType.SIMPLE, auto_cleanup=False)
        manager = SourceManager(config)
    ...
    def test_context_manager_exit(self, mocker, source_manager_simple):
        ...
        mock_cleanup = mocker.patch.object(
            source_manager_simple, "cleanup_sensitive_data"
        )
...
class TestCreateSourceManager:
    ...
    def test_create_source_manager_complex(self):
        ...
        manager = create_source_manager(
            "complex", passphrase="test_pass", anonymize_logs=False, auto_cleanup=False
        )
    ...
    def test_full_simple_workflow(self, mocker):
        ...
        with create_source_manager("simple") as manager:
            ...
            mocker.patch.object(
                manager._legacy_manager,
                "load_sources",
                return_value=(mock_defs, "succès"),
            )
            mocker.patch.object(
                manager._legacy_manager,
                "select_text_for_analysis",
                return_value=("some text", "some description"),
            )
    ...
    def test_full_complex_workflow(self, mocker):
        ...
        with create_source_manager("complex", passphrase="test") as manager:
            ...
            mocker.patch.object(
                manager._legacy_manager,
                "select_text_for_analysis",
                return_value=("x" * 300, "some description [ANONYMISÉ]"),
            )
