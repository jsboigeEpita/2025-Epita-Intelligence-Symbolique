...
def _extract_zip(zip_path: pathlib.Path, extract_to_dir: pathlib.Path) -> Optional[pathlib.Path]:
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            top_level_dirs = list(set(item.split('/')[0] for item in zip_ref.namelist() if '/' in item))
        ...
        if top_level_dirs and len(top_level_dirs) == 1:
            ...
            if (extract_to_dir / extracted_root_name).is_dir():
                 logger.info(f"Répertoire racine de l'archive détecté: {extracted_root_name}")
        ...
        if expected_extracted_path.is_dir():
            logger.info(f"Répertoire racine attendu '{OCTAVE_EXTRACTED_DIR_NAME}' trouvé après extraction.")
            return expected_extracted_path

        logger.warning(f"Impossible de déterminer le répertoire racine exact après extraction dans '{extract_to_dir}'. "
        ...
        logger.error(f"L'archive ZIP '{zip_path}' n'a pas été trouvée pour l'extraction.")
        return None
...
def ensure_portable_octave(project_root_dir: pathlib.Path) -> Optional[pathlib.Path]:
    ...
    logger.info(f"Octave portable non trouvé à {octave_bin_path}. Tentative de mise en place...")
    ...
    if not octave_zip_path.is_file():
        logger.info(f"Archive ZIP d'Octave '{OCTAVE_ZIP_NAME}' non trouvée dans '{temp_dir}'. Téléchargement...")
    ...
    if octave_root_install_dir.exists():
        logger.info(f"Suppression de l'ancien répertoire d'installation Octave: {octave_root_install_dir}")
        try:
            shutil.rmtree(octave_root_install_dir)
        except Exception as e_rm:
            logger.error(f"Impossible de supprimer l'ancien répertoire {octave_root_install_dir}: {e_rm}")
    ...
    if not extracted_octave_base or not extracted_octave_base.is_dir():
        logger.error(f"Échec de l'extraction d'Octave ou répertoire de base non trouvé.")
        return None
    ...
    if extracted_octave_base.name != OCTAVE_EXTRACTED_DIR_NAME and \
       (extracted_octave_base / OCTAVE_EXTRACTED_DIR_NAME).is_dir():
        # Cas où _extract_zip a retourné le parent et le dossier est dedans
        octave_root_install_dir_final = extracted_octave_base / OCTAVE_EXTRACTED_DIR_NAME
        ...
        for item in portable_octave_parent_dir.iterdir():
            if item.is_dir() and item.name.startswith("octave-") and item.name.endswith(OCTAVE_ARCH):
                found_octave_dir = item
                break
        if found_octave_dir:
            octave_root_install_dir_final = found_octave_dir
            logger.info(f"Répertoire Octave détecté après extraction: {octave_root_install_dir_final}")
        else:
            logger.error(f"Impossible de localiser le répertoire racine d'Octave après extraction dans {portable_octave_parent_dir}")
            return None


    octave_bin_path_final = octave_root_install_dir_final / "mingw64" / "bin"

    if octave_bin_path_final.is_dir() and (octave_bin_path_final / "octave-cli.exe").is_file():
        logger.info(f"Octave portable mis en place avec succès. Chemin binaire: {octave_bin_path_final}")
        return octave_bin_path_final
    else:
        logger.error(f"Octave portable semble extrait mais le répertoire bin '{octave_bin_path_final}' ou 'octave-cli.exe' est invalide/manquant.")
        return None
...
if __name__ == '__main__':
    ...
    project_root = script_dir.parent.parent # Remonter de tests/utils à la racine du projet
    ...
    if octave_bin:
        ...
        if octave_cli_exe.is_file():
            import subprocess
            try:
                result = subprocess.run([str(octave_cli_exe), "--version"], capture_output=True, text=True, check=True, timeout=10)
                logger.info(f"Octave CLI version output:\n{result.stdout}")
            except Exception as e_cli:
                logger.error(f"Erreur lors de l'exécution de octave-cli --version: {e_cli}")
        else:
            logger.error(f"octave-cli.exe non trouvé à {octave_cli_exe}")
    else:
        logger.error("Test échoué: Impossible d'assurer la disponibilité d'Octave portable.")