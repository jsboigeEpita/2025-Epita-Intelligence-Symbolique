...
class TestBaseWorkflowState:
    ...
    def test_initialization(self, initial_context_data, workflow_id_data):
        ...
        state = BaseWorkflowState(
            initial_context=initial_context_data, workflow_id=workflow_id_data
        )
        ...
        assert isinstance(
            uuid.UUID(state_auto_id.workflow_id, version=4), uuid.UUID
        )  # Vérifie que c'est un UUIDv4 valide
    ...
    def test_get_tasks(self, initial_context_data):
        ...
        agent_B_completed_tasks = state.get_tasks(
            assignee="agent_B", status="completed"
        )
    ...
    def test_add_result(self, initial_context_data):
        ...
        added_result_1 = state.add_result(
            query_id, agent_source, content, result_id=result_id_provided
        )
        ...
        added_result_2 = state.add_result(
            query_id, agent_source, {"data": "other_result"}
        )
    ...
    def test_get_results(self, initial_context_data):
        ...
        query2_agent1_results = state.get_results(
            query_id=query2_id, agent_source=agent1
        )
    ...
    def test_add_log_message(self, initial_context_data):
        ...
        assert isinstance(
            uuid.UUID(log_entry["timestamp"], version=4), uuid.UUID
    ...
    def test_set_and_get_final_output(self, initial_context_data):
        ...
        # Test écrasement
        new_output_data = {
            "summary": "Workflow completed with errors.",
            "errors": ["error1"],
        }
...
class TestEnquetePoliciereState:
    ...
    def test_get_and_update_case_description(self, enquete_policiere_state_data):
        state = EnquetePoliciereState(**enquete_policiere_state_data)
        assert (
            state.get_case_description()
            == enquete_policiere_state_data["description_cas"]
        )
    ...
    def test_add_and_get_identified_elements(self, enquete_policiere_state_data):
        state = EnquetePoliciereState(**enquete_policiere_state_data)
        elem1 = state.add_identified_element(
            "témoin", "A vu une ombre s'enfuir.", "Inspecteur Lestrade"
        )
        elem2 = state.add_identified_element(
            "indice", "Une plume rare trouvée sur les lieux.", "Sherlock Holmes"
        )
    ...
    def test_query_log_management(self, enquete_policiere_state_data):
        ...
        query_id_1 = state.add_query_log_entry(
            queried_by_1, query_params_1, bs_target_1
        )
        ...
        query_id_2 = state.add_query_log_entry(
            queried_by_2, query_params_2, bs_target_2
        )
        ...
        holmes_alibis_entries = state.get_query_log_entries(
            queried_by=queried_by_1, belief_set_id_cible=bs_target_2
        )
    ...
    def test_hypotheses_management(self, enquete_policiere_state_data):
        ...
        hypo1 = state.add_hypothesis(
            "Le Colonel Moutarde est le coupable.", 0.7, hypothesis_id=hypo_id_provided
        )
        ...
        assert (
            updated_hypo1_again in investigation_hypotheses
        )  # hypo1 est "under_investigation"
...
class TestEnqueteCluedoState:
    ...
    @pytest.fixture
    def enquete_cluedo_state_data(
        self, initial_context_data, workflow_id_data, cluedo_elements
    ):
        return {
    ...
    def test_initialization_auto_generate_solution(
        self, enquete_cluedo_state_data, cluedo_elements
    ):
        with patch.object(
            EnqueteCluedoState, "_initialize_cluedo_belief_set"
        ) as mock_init_bs:
            state = EnqueteCluedoState(
                **enquete_cluedo_state_data, auto_generate_solution=True
            )

            assert (
                state.nom_enquete_cluedo
                == enquete_cluedo_state_data["nom_enquete_cluedo"]
            )
            ...
            assert (
                state.solution_secrete_cluedo["suspect"] in cluedo_elements["suspects"]
            )
    ...
    def test_initialization_value_error(self, enquete_cluedo_state_data):
        with pytest.raises(
            ValueError, match="Une solution secrète doit être fournie ou auto-générée."
        ):
            EnqueteCluedoState(
                **enquete_cluedo_state_data,
    ...
    def test_generate_random_solution(self, enquete_cluedo_state_data, cluedo_elements):
        ...
        with pytest.raises(
            ValueError, match="La liste des suspects ne peut pas être vide"
        ):
        ...
        with pytest.raises(
            ValueError, match="La liste des armes ne peut pas être vide"
        ):
        ...
        with pytest.raises(
            ValueError, match="La liste des lieux ne peut pas être vide"
        ):
            EnqueteCluedoState(**data_no_lieux, auto_generate_solution=True)

    def test_initialize_cluedo_belief_set(
        self, enquete_cluedo_state_data, cluedo_elements
    ):
        solution = {"suspect": "Professeur Violet", "arme": "Corde", "lieu": "Salon"}

        with patch.object(
            EnqueteCluedoState, "add_or_update_belief_set", new_callable=MagicMock
        ) as mock_add_bs:
            state = EnqueteCluedoState(
                **enquete_cluedo_state_data,
                solution_secrete_cluedo=solution,
            ...
            expected_excluded_suspects_desc = [
                f"{s} n'est pas le meurtrier."
                for s in cluedo_elements["suspects"]
                if s != solution["suspect"]
            ]
            assert sorted(state.belief_set_initial_watson["suspects_exclus"]) == sorted(
                expected_excluded_suspects_desc
            )
            assert (
                f"{solution['suspect']} n'est pas le meurtrier."
                not in state.belief_set_initial_watson["suspects_exclus"]
            )

            expected_excluded_armes_desc = [
                f"{a} n'est pas l'arme du crime."
                for a in cluedo_elements["armes"]
                if a != solution["arme"]
            ]
            assert sorted(state.belief_set_initial_watson["armes_exclues"]) == sorted(
                expected_excluded_armes_desc
            )
            assert (
                f"{solution['arme']} n'est pas l'arme du crime."
                not in state.belief_set_initial_watson["armes_exclues"]
            )

            expected_excluded_lieux_desc = [
                f"{l} n'est pas le lieu du crime."
                for l in cluedo_elements["lieux"]
                if l != solution["lieu"]
            ]
            assert sorted(state.belief_set_initial_watson["lieux_exclus"]) == sorted(
                expected_excluded_lieux_desc
            )
            assert (
                f"{solution['lieu']} n'est pas le lieu du crime."
                not in state.belief_set_initial_watson["lieux_exclus"]
            )
            ...
            expected_non_suspects_formulas = [
                f"Not(Coupable({s}))"
                for s in cluedo_elements["suspects"]
                if s != solution["suspect"]
            ]
    ...
    def test_get_solution_secrete(self, enquete_cluedo_state_data):
        ...
        with patch.object(
            EnqueteCluedoState, "_initialize_cluedo_belief_set"
        ):  # Mock pour éviter l'effet de bord
            state = EnqueteCluedoState(
                **enquete_cluedo_state_data,
                solution_secrete_cluedo=solution,
                auto_generate_solution=False,
