...
async def test_agent_initialization_simplified(jvm_session):
    ...
    try:
        ...
        logger.info(f"✅ TweetyBridge est prêt (JVM démarrée par jvm_session: {tweety_available})")
    ...
    logger.info("--- Test d'initialisation simplifié terminé avec succès dans le worker ---")
...
def main():
    ...
    try:
        ...
        if result == pytest.ExitCode.OK:
            ...
            logger.error(f"Les tests ont échoué dans le worker avec le code de sortie: {result}")
            sys.exit(int(result))
            
    except Exception as e:
        logger.critical(f"Une erreur critique est survenue dans le worker: {e}", exc_info=True)
        sys.exit(1)
