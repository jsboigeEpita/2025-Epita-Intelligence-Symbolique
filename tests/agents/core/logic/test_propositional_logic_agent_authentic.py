...
def authentic_pl_agent(tweety_bridge_fixture):
    ...
    if not tweety_bridge_fixture.initializer.is_jvm_ready():
        pytest.skip(
            "TweetyBridge not ready, skipping authentic propositional agent tests."
        )
    ...
    if llm_service_configured:
        print(
            f"[AUTHENTIC] PropositionalLogicAgent configuré avec service: {llm_service_id}"
        )
    else:
...
async def test_full_propositional_reasoning_workflow_authentic(authentic_pl_agent):
    ...
    if not (
        authentic_pl_agent["llm_service_configured"]
        and authentic_pl_agent["tweety_available"]
    ):
        pytest.skip(
            "Services LLM et TweetyBridge requis pour test intégration authentique"
        )
...
def test_formula_validation_performance_authentic(authentic_pl_agent):
    ...
    execution_time = time.time() - start_time
    print(
        f"[AUTHENTIC] Validation de {len(test_formulas)} formules en {execution_time:.2f}s"
    )
