...
class BehaviorComparator:
        ...
        try:
            ...
            if ORACLE_SYSTEM_AVAILABLE:
                ...
                real_result = await self._execute_basic_real_scenario(
                    real_kernel, test_scenario
                )

            execution_time = time.time() - start_time

            # Estimation du coût (approximative)
            estimated_tokens = len(real_result.get("content", "")) + len(
                test_scenario.get("prompt", "")
            )
            estimated_cost = (
                estimated_tokens * 0.000375 / 1000
    ...
    async def _simulate_mock_execution(self, orchestrator, scenario) -> Dict[str, Any]:
        ...
        if "moriarty" in scenario.get("prompt", "").lower():
            mock_content = (
                "Mock Moriarty: Je révèle automatiquement la carte Colonel Moutarde!"
            )
    ...
    async def _execute_real_gpt_scenario(
        self, orchestrator, scenario
    ) -> Dict[str, Any]:
        ...
        oracle_state = await orchestrator.setup_workflow(
            nom_enquete="Comparison Test", elements_jeu=elements_jeu
        )
    ...
    async def _execute_basic_real_scenario(self, kernel, scenario) -> Dict[str, Any]:
        ...
        response = await chat_service.get_chat_message_contents(
            chat_history=messages, settings=settings
        )

        content = (
            response[0].content if response and len(response) > 0 else "No response"
        )

        return {"content": content, "metadata": {"basic_real": True}}

    def compare_results(
        self, mock_result: Dict[str, Any], real_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            # Performance
            "execution_time_ratio": real_result["execution_time"]
            / max(mock_result["execution_time"], 0.001),
            "mock_faster": mock_result["execution_time"]
            < real_result["execution_time"],
            # Contenu
            "content_length_ratio": real_result["response_length"]
            / max(mock_result["response_length"], 1),
            "real_more_verbose": real_result["response_length"]
            > mock_result["response_length"],
            # Cohérence
            "content_similarity": self._calculate_content_similarity(
                mock_result["response_content"], real_result["response_content"]
            ),
            # Coût
            "cost_difference": real_result["cost"] - mock_result["cost"],
            "cost_per_char": real_result["cost"]
            / max(real_result["response_length"], 1),
            # Prévisibilité
            "predictability_difference": mock_result["predictable"]
            and not real_result["predictable"],
            # Qualité
            "mock_quality_score": self._assess_response_quality(
                mock_result["response_content"]
            ),
            "real_quality_score": self._assess_response_quality(
                real_result["response_content"]
            ),
        }

        comparison["quality_advantage"] = (
            comparison["real_quality_score"] - comparison["mock_quality_score"]
        )
    ...
    def _calculate_content_similarity(
        self, mock_content: str, real_content: str
    ) -> float:
    ...
    def _assess_response_quality(self, content: str) -> float:
        ...
        keyword_count = sum(
            1 for keyword in relevant_keywords if keyword in content.lower()
        )
...
@pytest.mark.skipif(
    not COMPARISON_TESTS_ENABLED, reason="Tests de comparaison désactivés"
)
class TestMockVsRealComparison:
    """Tests de comparaison Mock vs Réel."""

    @pytest.mark.asyncio
    async def test_individual_scenario_comparison(
        self, behavior_comparator, comparison_test_scenarios
    ):
        ...
        for scenario in comparison_test_scenarios:
            ...
            assert mock_result[
                "success"
            ], f"Le scénario mock '{scenario['name']}' a échoué: {mock_result.get('error')}"
            ...
            if REAL_GPT_AVAILABLE:
                ...
                assert real_result[
                    "success"
                ], f"Le scénario réel '{scenario['name']}' a échoué: {real_result.get('error')}"

                # Comparaison seulement si les deux réussissent
                if mock_result["success"] and real_result["success"]:
                    comparison = behavior_comparator.compare_results(
                    ...
                    assert comparison[
                        "comparison_valid"
                    ], f"Comparaison invalide pour {scenario['name']}"
                    assert (
                        comparison["execution_time_ratio"] > 0
                    ), "Ratio temps d'exécution invalide"
                    assert (
                        0 <= comparison["content_similarity"] <= 1
        ...
        avg_similarity = statistics.mean(
            [c["content_similarity"] for c in comparison_results]
        )
        avg_quality_advantage = statistics.mean(
            [c["quality_advantage"] for c in comparison_results]
        )
        mock_speed_advantage = sum(
        ...
        assert (
            mock_speed_advantage >= 0.8
        ), f"Mocks pas assez rapides: {mock_speed_advantage}"
    ...
    async def test_oracle_enhanced_consistency(self, behavior_comparator):
        ...
        for i in range(3):  # 3 exécutions
            ...
            if REAL_GPT_AVAILABLE:
                real_result = await behavior_comparator.test_real_behavior(
                    oracle_scenario
                )
        ...
        if len(mock_contents) > 1:
            ...
            mock_similarities = [
                behavior_comparator._calculate_content_similarity(
                    mock_contents[0], content
                )
                for content in mock_contents[1:]
            ]
            avg_mock_consistency = statistics.mean(mock_similarities)
            assert (
                avg_mock_consistency >= 0.8
            ), f"Mocks pas assez cohérents: {avg_mock_consistency}"
        ...
        if len(real_contents) > 1:
            ...
            real_similarities = [
                behavior_comparator._calculate_content_similarity(
                    real_contents[0], content
                )
                for content in real_contents[1:]
            ]
            avg_real_consistency = statistics.mean(real_similarities)
            assert (
                avg_real_consistency >= 0.3
            ), f"Réels incohérents: {avg_real_consistency}"
            ...
            for content in real_contents:
                oracle_relevance = sum(
                    1 for kw in oracle_keywords if kw in content.lower()
                )
                assert (
                    oracle_relevance >= 1
                ), f"Contenu réel pas assez Oracle: {content[:100]}"

    @pytest.mark.asyncio
    async def test_performance_vs_quality_tradeoff(
        self, behavior_comparator, comparison_test_scenarios
    ):
        ...
        for scenario in comparison_test_scenarios[:2]:  # Limité pour coût
            ...
            if mock_result["success"] and real_result["success"]:
                ...
                if comparison["mock_faster"]:
                    ...
                    tradeoff_analysis["speed_advantage_ratios"].append(
                ...
                if comparison["quality_advantage"] > 0:
                    ...
                    if real_result["cost"] > 0:
                        cost_per_quality = (
                            real_result["cost"] / comparison["quality_advantage"]
                        )
                        tradeoff_analysis["cost_per_quality_point"].append(
                            cost_per_quality
                        )
        ...
        if tradeoff_analysis["scenarios_tested"] > 0:
            mock_speed_rate = (
                tradeoff_analysis["mock_faster_count"]
                / tradeoff_analysis["scenarios_tested"]
            )
            real_quality_rate = (
                tradeoff_analysis["real_better_quality_count"]
                / tradeoff_analysis["scenarios_tested"]
            )
            ...
            if tradeoff_analysis["speed_advantage_ratios"]:
                avg_speed_advantage = statistics.mean(
                    tradeoff_analysis["speed_advantage_ratios"]
                )
                print(f"Ratio vitesse moyen: {avg_speed_advantage:.2f}x")

            if tradeoff_analysis["cost_per_quality_point"]:
                avg_cost_per_quality = statistics.mean(
                    tradeoff_analysis["cost_per_quality_point"]
                )
...
class TestComparisonMetrics:
    ...
    def test_content_similarity_calculation(self, behavior_comparator):
        ...
        for case in test_cases:
            ...
            if case["expected_similarity"] == 1.0:
                assert (
                    similarity == 1.0
                ), f"Textes identiques mal calculés: {similarity}"
            else:
                # Pour les autres cas, vérifier l'ordre de grandeur
                assert (
                    abs(similarity - case["expected_similarity"]) < 0.4
    ...
    def test_quality_assessment(self, behavior_comparator):
        ...
        quality_test_cases = [
            ...
            {"content": "Réponse très courte", "expected_high_quality": False},
        ]

        for case in quality_test_cases:
            quality_score = behavior_comparator._assess_response_quality(
                case["content"]
            )

            assert (
                0 <= quality_score <= 1
            ), f"Score qualité hors limites: {quality_score}"

            if case["expected_high_quality"]:
                assert (
                    quality_score >= 0.5
                ), f"Qualité élevée mal évaluée: {quality_score} pour '{case['content'][:50]}...'"
            else:
                assert (
                    quality_score <= 0.6
                ), f"Qualité faible mal évaluée: {quality_score} pour '{case['content'][:50]}...'"
