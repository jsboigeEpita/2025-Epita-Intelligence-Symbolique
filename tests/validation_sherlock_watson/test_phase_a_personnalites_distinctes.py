...
class PersonalityAnalyzer:
    ...
    def __init__(self):
        ...
        self.watson_proactive_patterns = [
            r"J'observe que", r"Logiquement.*implique", r"Cette déduction.*amène",
            r"L'analyse révèle", r"Je remarque", r"Il apparaît clairement",
            r"Mon analyse.*suggère", r"Je détecte"
        ]
        
        self.watson_passive_patterns = [
            r"Voulez-vous que je", r"Souhaitez-vous", r"Que faisons-nous",
            r"Dois-je", r"Faut-il que", r"Désirez-vous"
        ]
        
        # Patterns pour Moriarty (théâtralité)
        self.moriarty_theatrical_patterns = [
            r"Comme c'est.*intéressant", r"Permettez-moi de.*éclairer",
            r"Vous brûlez.*ou pas", r"Quelle perspicacité",
            r"délicieusement prévisible", r"sourire énigmatique",
            r"troubler vos certitudes", r"Ah, mon cher"
        ]
        ...
        self.sherlock_leadership_patterns = [
            r"Je pressens que", r"L'évidence.*clairement",
            r"Concentrons-nous sur", r"La logique.*inexorablement",
            r"Mes déductions.*révèlent", r"Il est évident que",
    ...
    def analyze_watson_response(self, response: str) -> Tuple[float, float, float]:
        ...
        proactive_matches = sum(1 for pattern in self.watson_proactive_patterns 
                               if re.search(pattern, response, re.IGNORECASE))
        passive_matches = sum(1 for pattern in self.watson_passive_patterns 
                             if re.search(pattern, response, re.IGNORECASE))
    ...
    def analyze_moriarty_response(self, response: str) -> Tuple[float, float, float]:
        ...
        theatrical_matches = sum(1 for pattern in self.moriarty_theatrical_patterns 
                                if re.search(pattern, response, re.IGNORECASE))
        mechanical_matches = sum(1 for pattern in self.moriarty_mechanical_patterns 
                                if re.search(pattern, response, re.IGNORECASE))
        
        total_sentences = len(re.split(r'[.!?]+', response))
        
        theatricality_score = min(10, (theatrical_matches / max(1, total_sentences)) * 15)
    ...
    def analyze_sherlock_response(self, response: str) -> Tuple[float, float, float]:
        ...
        leadership_matches = sum(1 for pattern in self.sherlock_leadership_patterns 
                                if re.search(pattern, response, re.IGNORECASE))
        
        # Recherche d'assertions confiantes
        confident_patterns = [r"je.*certain", r"il est clair", r"évidemment", 
                             r"sans aucun doute", r"ma conclusion"]
        confident_matches = sum(1 for pattern in confident_patterns 
                               if re.search(pattern, response, re.IGNORECASE))
...
class CluedoPersonalityTestRunner:
    ...
    def run_personality_tests(self) -> Dict[str, PersonalityMetrics]:
        ...
        for i, scenario in enumerate(self.test_scenarios, 1):
            ...
            for agent_name in ["Watson", "Moriarty", "Sherlock"]:
                ...
                if agent_name == "Watson":
                    proactivity, passive_ratio, distinctiveness = self.analyzer.analyze_watson_response(response)
                    ...
                    theatricality, mechanical_ratio, distinctiveness = self.analyzer.analyze_moriarty_response(response)
                    results[agent_name].theatricality_score += theatricality
                    results[agent_name].mechanical_responses_ratio += mechanical_ratio
                    results[agent_name].distinctiveness_score += distinctiveness
                    
                elif agent_name == "Sherlock":
                    leadership, confident_ratio, distinctiveness = self.analyzer.analyze_sherlock_response(response)
    ...
    def generate_validation_report(self, results: Dict[str, PersonalityMetrics]) -> Dict[str, Any]:
        ...
        # Métriques globales
        overall_distinctiveness = statistics.mean([m.distinctiveness_score for m in results.values()])
        
        # Validation des critères Phase A
        watson_proactive_success = results["Watson"].passive_questions_ratio < 0.20
        moriarty_theatrical_success = results["Moriarty"].mechanical_responses_ratio < 0.30
        ...
        report = {
            ...
            "resultats_watson": {
                ...
                "ratio_questions_passives": round(results["Watson"].passive_questions_ratio * 100, 1),
                "objectif_passivite": "< 20%",
                "objectif_atteint": watson_proactive_success,
                "distinctivite_score": round(results["Watson"].distinctiveness_score, 2)
            },
            
            "resultats_moriarty": {
                "theatralite_score": round(results["Moriarty"].theatricality_score, 2),
                "ratio_reponses_mecaniques": round(results["Moriarty"].mechanical_responses_ratio * 100, 1),
                "objectif_mecanique": "< 30%",
                "objectif_atteint": moriarty_theatrical_success,
                "distinctivite_score": round(results["Moriarty"].distinctiveness_score, 2)
            },
            
            "resultats_sherlock": {
                "leadership_score": round(results["Sherlock"].leadership_score, 2),
                "ratio_assertions_confiantes": round(results["Sherlock"].confident_assertions_ratio * 100, 1),
                "objectif_leadership": ">= 6.0/10",
                "objectif_atteint": sherlock_leadership_success,
                "distinctivite_score": round(results["Sherlock"].distinctiveness_score, 2)
            },
            
            "validation_phase_a": {
                "criteres_valides": sum([watson_proactive_success, moriarty_theatrical_success, 
                                       sherlock_leadership_success, distinctiveness_success]),
                "criteres_totaux": 4,
                "phase_a_reussie": all([watson_proactive_success, moriarty_theatrical_success,
                                       sherlock_leadership_success, distinctiveness_success]),
                "prochaine_etape": "Phase B - Naturalité du dialogue" if all([watson_proactive_success, moriarty_theatrical_success, sherlock_leadership_success, distinctiveness_success]) else "Ajustements Phase A nécessaires"
            }
        }
        
        # Affichage console
        print(f"\n>> SCORE PERSONNALITES DISTINCTES: {overall_distinctiveness:.1f}/10")
        print(f"   Objectif: 6.0/10 - {'[ATTEINT]' if distinctiveness_success else '[NON ATTEINT]'}")
        
        print(f"\n>> WATSON (Proactivite):")
        print(f"   • Score proactivite: {results['Watson'].proactivity_score:.1f}/10")
        print(f"   • Questions passives: {results['Watson'].passive_questions_ratio*100:.1f}% (objectif: <20%)")
        print(f"   • Resultat: {'[SUCCES]' if watson_proactive_success else '[ECHEC]'}")
        
        print(f"\n>> MORIARTY (Theatralite):")
        print(f"   • Score theatralite: {results['Moriarty'].theatricality_score:.1f}/10")
        print(f"   • Reponses mecaniques: {results['Moriarty'].mechanical_responses_ratio*100:.1f}% (objectif: <30%)")
        print(f"   • Resultat: {'[SUCCES]' if moriarty_theatrical_success else '[ECHEC]'}")
        
        print(f"\n>> SHERLOCK (Leadership):")
        print(f"   • Score leadership: {results['Sherlock'].leadership_score:.1f}/10")
        print(f"   • Assertions confiantes: {results['Sherlock'].confident_assertions_ratio*100:.1f}%")
        print(f"   • Resultat: {'[SUCCES]' if sherlock_leadership_success else '[ECHEC]'}")
        
        print(f"\n>> VALIDATION PHASE A: {'[REUSSIE]' if report['validation_phase_a']['phase_a_reussie'] else '[ECHEC]'}")
        print(f"   Criteres valides: {report['validation_phase_a']['criteres_valides']}/4")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"Erreur lors de l'exécution des tests Phase A: {e}", exc_info=True)
        print(f"\n>> ERREUR: {e}")
        exit(2)