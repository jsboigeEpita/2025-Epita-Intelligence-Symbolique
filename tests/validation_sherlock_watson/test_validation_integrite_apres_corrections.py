...
class TestValidationIntegriteApresCorrections:
    ...
    def test_simulate_other_player_response_maintenant_legitime(self):
        ...
        with patch.object(self.dataset, "get_autres_joueurs_cards") as mock_method:
            ...
            result = tools.simulate_other_player_response(
                "Docteur Olive,Poignard,Salon", "AutreJoueur"
            )
            ...
            assert (
                "simulation probabiliste" in result or "Simulation impossible" in result
            )
    ...
    def test_systeme_permissions_renforce_fonctionne(self):
        ...
        assert "get_autres_joueurs_cards" in sherlock_rule.conditions.get(
            "forbidden_methods", []
        )
    ...
    def test_fonctionnalites_legitimes_preservees(self):
        ...
        result = asyncio.run(
            self.dataset.process_query("TestAgent", QueryType.CLUE_REQUEST, {})
        )
    ...
    def test_oracle_enhanced_respecte_integrite(self):
        ...
        result = asyncio.run(
            self.dataset.process_query(
                "TestAgent", QueryType.SUGGESTION_VALIDATION, query_params
            )
        )
        ...
        if hasattr(result.data, "revealed_cards"):
            ...
            for card in result.data.revealed_cards:
                ...
                if card_name:
                    assert (
                        card_name in cartes_moriarty
                    ), f"Carte révélée non autorisée: {card_name}"
    ...
    def test_regles_cluedo_maintenant_respectees(self):
        ...
        try:
            ...
            logger.info(
                "[OK] RÈGLE 1: Accès aux cartes des autres joueurs bloqué - CONFORME"
            )
        ...
        logger.info(
            "[OK] RÈGLE 4: Séparation cartes Moriarty/autres par construction - CONFORME"
        )

        # Aucune violation ne doit être détectée
        assert (
            len(violations) == 0
        ), f"Violations détectées après corrections: {violations}"
...
def test_validation_complete_integrite_apres_corrections():
    ...
    logger.info(
        f"[OK] VALIDATION RÉUSSIE: {len(succes_tests)} tests d'intégrité passés"
    )
    for succes in succes_tests:
        logger.info(f"  [OK] {succes}")

    logger.info(
        "🎉 CONCLUSION: L'intégrité des règles du Cluedo a été restaurée avec succès !"
    )

    # Assertion pour que pytest reconnaisse le succès
    assert (
        len(succes_tests) == 7
    ), f"Expected 7 successful tests, got {len(succes_tests)}"
    return True
