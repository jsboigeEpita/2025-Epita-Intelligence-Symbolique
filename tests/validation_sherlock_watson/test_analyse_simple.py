...
def test_workflow_simple():
    ...
    try:
        ...
        assert (
            kernel is not None
        ), "Le kernel ne doit pas être None après l'initialisation"
        ...
        print(
            f"Tours total: {oracle_stats.get('agent_interactions', {}).get('total_turns', 0)}"
        )
        print(
            f"Interactions Oracle: {oracle_stats.get('workflow_metrics', {}).get('oracle_interactions', 0)}"
        )
        print(
            f"Cartes révélées: {oracle_stats.get('workflow_metrics', {}).get('cards_revealed', 0)}"
        )
        ...
        print(
            f"  Score global: {(naturalite_score + pertinence_score + progression_score) / 3:.1f}"
        )
        ...
        def clean_for_json(obj):
            ...
            if obj is None:
                ...
                for k, v in obj.items():
                    ...
                    clean_key = (
                        str(k)
                        if isinstance(k, property) or hasattr(k, "__dict__")
                        else k
                    )
...
def analyze_relevance(conversation_history, agent_counts):
    ...
    for msg in conversation_history:
        if msg.get("sender") == "Watson":
            ...
            if any(
                word in content
                for word in ["logique", "formule", "validation", "analyse"]
            ):
    ...
    for msg in conversation_history:
        if msg.get("sender") == "Moriarty":
            content = msg.get("message", "").lower()
            if any(
                word in content for word in ["révèle", "carte", "réfutation", "possess"]
            ):
                moriarty_reveals += 1
...
def analyze_progression(solution_analysis, oracle_stats):
    ...
    oracle_interactions = oracle_stats.get("workflow_metrics", {}).get(
...
def identify_improvements(naturalite, pertinence, progression):
    ...
    if naturalite < 6.0:
        improvements.append(
            "Améliorer la naturalité du dialogue (variété vocabulaire, longueur messages)"
        )
