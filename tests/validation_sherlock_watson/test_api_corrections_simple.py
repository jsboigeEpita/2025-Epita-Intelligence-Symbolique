...
def test_revelation_record_api():
    ...
    assert revelation.reason == "Moriarty possede Colonel Moutarde", f"Expected reason match, got {revelation.reason}"
...
def test_cluedo_dataset_api():
    ...
    assert hasattr(dataset, 'elements_jeu'), "Propriete elements_jeu manquante"
    ...
    assert hasattr(dataset, 'is_game_solvable_by_elimination'), "Methode is_game_solvable_by_elimination manquante"
...
def test_record_agent_turn():
    ...
    # Vérifications
    assert "Sherlock" in oracle_state.agent_turns, "Agent 'Sherlock' non trouve dans agent_turns"
...
async def test_validate_suggestion_async():
    ...
    try:
        ...
        print(f"[ERREUR] Erreur async validate_suggestion: {e}")
        raise

async def run_all_tests():
    """Lance tous les tests de validation."""
    print("=== DEMARRAGE DES TESTS DE VALIDATION API ===")
    ...
    try:
        ...
        print("\n=== TOUS LES TESTS DE COMPATIBILITE API REUSSIS ! ===")
        print("[SUCCESS] Les incompatibilites critiques ont ete corrigees")
        
    except Exception as e:
        print(f"\n[ECHEC] ECHEC DES TESTS: {e}")
