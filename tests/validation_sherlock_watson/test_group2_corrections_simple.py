...
def test_dataset_manager_check_permission():
    ...
    print(
        "Test 1: Verification de l'existence de check_permission sur DatasetAccessManager"
    )
    ...
    assert hasattr(
        dataset_manager, "check_permission"
    ), "La methode check_permission doit exister"

    # Tester la methode
    result_authorized = asyncio.run(
        dataset_manager.check_permission("Watson", QueryType.CARD_INQUIRY)
    )
    result_unauthorized = asyncio.run(
...
def test_mock_permission_setup():
    ...
    assert hasattr(
        mock_dataset_manager, "permission_manager"
    ), "Le mock doit avoir l'attribut permission_manager"
    assert hasattr(
        mock_dataset_manager, "check_permission"
    ), "Le mock doit avoir la methode check_permission"

    # Test des appels
    result = asyncio.run(
        mock_dataset_manager.check_permission("Watson", QueryType.CARD_INQUIRY)
    )
...
def test_oracle_tools_integration():
    ...
    assert hasattr(
        oracle_agent.oracle_tools, "check_agent_permission"
    ), "OracleTools doit avoir check_agent_permission"
    ...
    assert (
        "Watson a les permissions" in result
    ), f"Resultat attendu non trouve dans: {result}"
