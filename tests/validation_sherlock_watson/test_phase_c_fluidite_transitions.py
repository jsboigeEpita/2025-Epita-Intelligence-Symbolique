...
def test_phase_c_fluidite_complete():
    ...
    for test_num in range(1, 6):
        ...
        try:
            ...
            print(
                f"   - References contextuelles: {test_result['contextual_reference_rate']}%"
            )
            print(
                f"   - Reactions emotionnelles: {test_result['emotional_reaction_rate']}%"
            )
...
def simulate_fluidity_conversation(
    test_num: int, elements_jeu: Dict[str, List[str]]
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Simulation conversation {test_num} avec {len(conversation_scenarios)} tours"
    )
...
def simulate_contextual_analysis(
    oracle_state, agent_name: str, content: str, turn: int
):
    """
...
def analyze_aggregate_results(all_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    aggregate["success_percentage"] = (
        sum(success_criteria) / len(success_criteria)
    ) * 100
...
def print_phase_c_results(aggregate: Dict[str, Any]):
    ...
    print(
        f"   Cible atteinte: {'[OK] OUI' if fluidity['target_achieved'] else '[FAIL] NON'}"
    )
    ...
    print(
        f"   Cible atteinte: {'[OK] OUI' if refs['target_achieved'] else '[FAIL] NON'}"
    )
    ...
    print(
        f"   Taux moyen: {reactions['mean_rate']:.1f}% (cible: {reactions['target']}%)"
    )
    print(
        f"   Cible atteinte: {'[OK] OUI' if reactions['target_achieved'] else '[FAIL] NON'}"
    )
    ...
    print(
        f"   Amélioration fluidité (>5.0): {'[OK]' if aggregate['phase_c_success']['fluidity_improved'] else '[FAIL]'}"
    )
    print(
        f"   Références suffisantes (≥90%): {'[OK]' if aggregate['phase_c_success']['references_sufficient'] else '[FAIL]'}"
    )
    print(
        f"   Réactions suffisantes (≥70%): {'[OK]' if aggregate['phase_c_success']['reactions_sufficient'] else '[FAIL]'}"
    )
    print()

    print(
        f"🏆 RÉSULTAT GLOBAL: {'[OK] SUCCÈS' if aggregate['overall_success'] else '[FAIL] ÉCHEC'}"
    )
