...
try:
    ...
    from argumentation_analysis.agents.core.oracle.permissions import OracleResponse, QueryType, PermissionManager
...
def test_oracle_response_api():
    ...
    response = OracleResponse(authorized=True, message="Test reussi", query_type=QueryType.CARD_INQUIRY)
    assert response.success is True
    assert response.authorized is True
    response_fail = OracleResponse(authorized=False, message="Test echoue", error_code="TEST_ERROR")
...
def test_revelation_record_api():
    ...
    revelation = RevelationRecord(card_revealed="Colonel Moutarde", revelation_type="owned_card", message="Moriarty possede Colonel Moutarde", strategic_value=0.8)
