    ...
    try:
        ...
        for line in output.split("\n"):
            if any(
                ...
                for keyword in ["failed", "error", "passed", "="]
            ):
                print(line.strip())
...
test_groups = [
    (
        "Oracle Tests",
        "python -m pytest tests/unit/argumentation_analysis/agents/core/oracle/ --tb=no",
    ),
    (
        "Integration Tests",
        "python -m pytest tests/integration/test_oracle_integration.py --tb=no",
    ),
    (
        "Logic Tests (sans Tweety)",
        "python -m pytest tests/agents/core/logic/test_modal_logic_agent_authentic.py --tb=no",
    ),
]
