...
class PhaseBValidator:
    ...
    def analyser_naturalite_message(self, message: str) -> Dict[str, Any]:
        ...
        naturelles = sum(
            len(re.findall(pattern, message_clean))
            for pattern in self.patterns_naturels
        )
        techniques = sum(
            len(re.findall(pattern, message_clean))
        ...
        mecaniques = sum(
            len(re.findall(pattern, message_clean))
            for pattern in self.formules_mecaniques
        )
    ...
    def tester_naturalite_agents(self) -> Dict[str, NaturaliteMetrics]:
        ...
        for agent_name, messages in agents_data.items():
            ...
            print(
                f"  {status_naturalite} Naturalité cible (≥7.0): {score_naturalite:.2f}"
            )
            print(
                f"  {status_mecaniques} Zéro répétition mécanique: {total_mecaniques}"
            )
    ...
    def generer_comparaison_avant_apres(self, resultats: Dict[str, NaturaliteMetrics]):
        ...
        longueur_apres = sum(m.longueur_moyenne for m in resultats.values()) / len(
            resultats
        )
        naturalite_apres = sum(m.score_naturalite for m in resultats.values()) / len(
            resultats
        )
        techniques_apres = sum(
            m.expressions_techniques for m in resultats.values()
        ) / len(resultats)
        mecaniques_apres = sum(
            m.repetitions_mecaniques for m in resultats.values()
        ) / len(resultats)
        ...
        reduction_mecaniques = (
            100
            if avant_phase_b["repetitions_mecaniques"] > 0 and mecaniques_apres == 0
            else 0
        )
    ...
    def valider_objectifs_phase_b(
        ...
        # Calculs globaux
        longueur_moyenne_globale = sum(
            m.longueur_moyenne for m in resultats.values()
        ) / len(resultats)
        naturalite_moyenne_globale = sum(
            m.score_naturalite for m in resultats.values()
        ) / len(resultats)
        ...
        criteres = {
            ...
            "tous_agents_optimises": all(
                m.score_naturalite >= 6.5 for m in resultats.values()
            ),
        }

        # Affichage validation
        print(
            f"Longueur optimisee (<=120): {longueur_moyenne_globale:.1f} {'[OK]' if criteres['longueur_optimisee'] else '[NON]'}"
        )
        print(
            f"Naturalite cible (>=7.0): {naturalite_moyenne_globale:.2f} {'[OK]' if criteres['naturalite_cible'] else '[NON]'}"
        )
        print(
            f"Zero repetitions mecaniques: {mecaniques_total} {'[OK]' if criteres['zero_repetitions'] else '[NON]'}"
        )
        print(
            f"Tous agents optimises (>=6.5): {'[OK]' if criteres['tous_agents_optimises'] else '[NON]'}"
        )
    ...
    def generer_rapport_phase_b(
        self,
        resultats: Dict[str, NaturaliteMetrics],
        comparaison: Dict,
        ...
        # Calculs summary
        longueur_moyenne = sum(m.longueur_moyenne for m in resultats.values()) / len(
            resultats
        )
        naturalite_moyenne = sum(m.score_naturalite for m in resultats.values()) / len(
            resultats
        )
