...
def test_handle_task_result_objective_completion(task_coordinator, mock_tactical_state):
    ...
    def side_effect_update_status(tid, status, *args, **kwargs):
        if tid == tactical_task_id and status == "completed":
            ...
            mock_tactical_state.tasks["completed"].append({"id": tactical_task_id, "objective_id": objective_id})
    ...
    mock_tactical_state.add_intermediate_result.assert_called_once_with(tactical_task_id, result_data)
