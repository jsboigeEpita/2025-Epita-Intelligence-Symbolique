...
class TestTaskCoordinator(unittest.TestCase):
    ...
    def test_process_strategic_objectives(self):
        ...
        with patch.object(
            self.coordinator, "_decompose_objective_to_tasks"
        ) as mock_decompose:
            mock_decompose.side_effect = [
                ...
                [
                    {
                        "id": "task-obj-2-1",
                        "description": "Analyser sophismes",
                        "objective_id": "obj-2",
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    self.tactical_state.add_assigned_objective.assert_any_call(
                        objectives[0]
                    )
                    self.tactical_state.add_assigned_objective.assert_any_call(
                        objectives[1]
                    ...
                    self.assertEqual(
                        self.tactical_state.add_assigned_objective.call_count, 2
                    )
