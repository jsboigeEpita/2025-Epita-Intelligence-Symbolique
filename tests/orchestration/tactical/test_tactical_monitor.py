...
class TestProgressMonitor(unittest.TestCase):
    ...
    def test_update_task_progress_with_error(self):
        ...
        # Vérifier que la méthode update_task_progress de l'état tactique a été appelée
        self.tactical_state.update_task_progress.assert_called_once_with(
            "invalid-task", 0.5
        )
    ...
    def test_check_task_anomalies_stagnation(self):
        ...
        # Configurer l'état tactique pour le test
        self.tactical_state.tasks = {
            "in_progress": [{"id": "task-1", "description": "Tâche 1"}],
            "pending": [],
            "completed": [],
            "failed": [],
        }
        ...
        if anomalies:
            ...
            if stagnation_found:
                stagnation_anomaly = next(
                    a for a in anomalies if a["type"] == "stagnation"
                )
    ...
    def test_check_task_anomalies_regression(self):
        ...
        self.assertEqual(
            len(anomalies), 2
        )  # La méthode détecte à la fois une régression et une stagnation

        # Vérifier qu'une anomalie de régression est présente
        regression_anomaly = next(
            (a for a in anomalies if a["type"] == "regression"), None
        )
    ...
    def test_check_task_anomalies_blocked_dependency(self):
        ...
        self.tactical_state.tasks = {
            ...
            "pending": [],
            "completed": [],
            "failed": [{"id": "task-dep", "description": "Tâche dépendance"}],
        }
        ...
        self.assertEqual(
            len(anomalies), 2
        )  # La méthode détecte à la fois une dépendance bloquée et une stagnation

        # Vérifier qu'une anomalie de dépendance bloquée est présente
        blocked_anomaly = next(
            (a for a in anomalies if a["type"] == "blocked_dependency"), None
        )
    ...
    def test_generate_progress_report(self):
        ...
        # Configurer l'état tactique pour le test
        self.tactical_state.tasks = {
            "pending": [{"id": "task-obj-1-3", "objective_id": "obj-1"}],
            "in_progress": [{"id": "task-obj-2-1", "objective_id": "obj-2"}],
            "completed": [
                {"id": "task-obj-1-1", "objective_id": "obj-1"},
                {"id": "task-obj-1-2", "objective_id": "obj-1"},
            ],
            "failed": [{"id": "task-obj-2-2", "objective_id": "obj-2"}],
        }
        self.tactical_state.assigned_objectives = [
            {"id": "obj-1", "description": "Objectif 1"},
            {"id": "obj-2", "description": "Objectif 2"},
        ]
        self.tactical_state.task_progress = {"task-obj-2-1": 0.6}
        self.tactical_state.identified_conflicts = [
            {"id": "conflict-1", "description": "Conflit 1", "resolved": True},
            {"id": "conflict-2", "description": "Conflit 2", "resolved": False},
        ]
    ...
    def test_detect_critical_issues(self):
        ...
        self.tactical_state.get_task_dependencies = MagicMock(return_value=["task-dep"])
        self.tactical_state.task_progress = {"task-delayed": 0.2}
        ...
        failure_issue = next(
            (i for i in issues if i["type"] == "high_failure_rate"), None
        )
    ...
    def test_suggest_corrective_actions(self):
        ...
        blocked_action = next(
            (a for a in actions if a["action_type"] == "reassign_dependency"), None
        )
        self.assertIsNotNone(blocked_action)
        self.assertEqual(
            blocked_action["description"],
            "Réassigner la dépendance de la tâche task-blocked",
        )

        # Vérifier l'action pour la tâche en retard
        delayed_action = next(
            (a for a in actions if a["action_type"] == "allocate_resources"), None
        )
        self.assertIsNotNone(delayed_action)
        self.assertEqual(
            delayed_action["description"],
            "Allouer plus de ressources à la tâche task-delayed",
        )

        # Vérifier l'action pour le conflit
        conflict_action = next(
            (a for a in actions if a["action_type"] == "resolve_conflict"), None
        )
        self.assertIsNotNone(conflict_action)
        self.assertEqual(
            conflict_action["description"],
            "Résoudre le conflit entre les tâches task-1, task-2",
        )

        # Vérifier l'action pour le taux d'échec élevé
        failure_action = next(
            (a for a in actions if a["action_type"] == "review_strategy"), None
        )
        self.assertIsNotNone(failure_action)
        self.assertEqual(
            failure_action["description"], "Revoir la stratégie globale d'analyse"
        )
