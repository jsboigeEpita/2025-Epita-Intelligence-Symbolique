...
def enquete_cluedo_state_fixture() -> EnqueteCluedoState:
    return EnqueteCluedoState(
        ...
        solution_secrete_cluedo={"suspect": "Colonel Moutarde", "arme": "Poignard", "lieu": "Salon"}
    )

def test_plugin_initialization_with_enquete_policiere_state(
    kernel: sk.Kernel, 
    enquete_policiere_state_fixture: EnquetePoliciereState
):
...
def test_plugin_initialization_with_enquete_cluedo_state(
    kernel: sk.Kernel, 
    enquete_cluedo_state_fixture: EnqueteCluedoState
):
...
async def test_add_task_policiere(
    ...
    args = KernelArguments(description="Interroger le témoin principal", assignee="Watson")
    ...
    assert enquete_policiere_state_fixture.tasks[0]["description"] == "Interroger le témoin principal"
...
async def test_get_cluedo_game_elements(
    ...
    result = await kernel.invoke(kernel.plugins["CluedoState"]["get_cluedo_game_elements"])
    result_dict = json.loads(str(result))
...
async def test_designate_next_agent(
    ...
    result = await kernel.invoke(kernel.plugins["EnqueteState"]["designate_next_agent"], args)
    result_dict = json.loads(str(result))

    assert "message" in result_dict
    assert agent_name_to_designate in result_dict["message"]
    assert enquete_policiere_state_fixture.get_designated_next_agent() == agent_name_to_designate

    # Test get_designated_next_agent
    result_get = await kernel.invoke(kernel.plugins["EnqueteState"]["get_designated_next_agent"])
...
async def test_add_identified_element_policiere(
    ...
    args = KernelArguments(element_type="preuve", description="Plume de canari près de la boîte à biscuits", source="Observation directe")
    result = await kernel.invoke(kernel.plugins["EnqueteState"]["add_identified_element"], args)
    result_dict = json.loads(str(result))
    ...
    assert enquete_policiere_state_fixture.elements_identifies[0]["description"] == "Plume de canari près de la boîte à biscuits"
...
async def test_add_hypothesis_policiere(
    ...
    assert enquete_policiere_state_fixture.hypotheses_enquete[0]["text"] == "Le canari est le coupable"