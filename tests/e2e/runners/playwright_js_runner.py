...
class PlaywrightJSRunner:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger):
        ...
        self.screenshots_dir = self.project_root / config.get(
    ...
    async def run_tests(
        ...
        try:
            ...
            self.logger.error(
                f"Erreur exécution tests Playwright JS: {e}", exc_info=True
            )  # Ajout de exc_info
            return False
    ...
    async def _prepare_test_environment(self, config: Dict[str, Any]):
        ...
        env_vars = {
            "FRONTEND_URL": config[
                "base_url"
            ],  # Correction ici pour correspondre au code JS
    ...
    async def _collect_artifacts(self):
        ...
        try:
            ...
            if screenshots_path.exists():
                ...
                self.last_results["artifacts"]["screenshots"] = [
                    str(p) for p in screenshots
                ]

            # Traces
            traces = (
                list(screenshots_path.rglob("*.zip"))
                if screenshots_path.exists()
                else []
            )
            ...
            # Vidéos
            videos = (
                list(screenshots_path.rglob("*.webm"))
                if screenshots_path.exists()
                else []
            )
...
def create_playwright_js_runner(
