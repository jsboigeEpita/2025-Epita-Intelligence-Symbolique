...
class IntegrationWorkflowHelpers:
    ...
    def navigate_with_validation(self, tab_name: str, expected_element: str):
        ...
        if not tab_found:
            print(
                f"  [NAV-WARN] Aucun sélecteur d'onglet n'a fonctionné pour '{tab_name}'. Tentative de fallback."
            )
            # Fallback : utiliser le premier onglet disponible
            try:
                self.page.locator(
                    'nav button, nav a, .nav-link, [role="tab"]'
                ).first.click()
                tab_found = True
        ...
        try:
            ...
            print(
                f"  [NAV-FAIL] L'élément attendu '{expected_element}' n'est pas visible après la navigation."
            )
            ...
            print(
                "  [NAV-INFO] État du réseau 'idle' atteint malgré l'absence de l'élément."
            )
...
def test_full_argument_analysis_workflow(
    ...
    integration_helpers.navigate_with_validation(
        "reconstructor", '[data-testid="reconstructor-text-input"]'
    )
    ...
    expect(
        page.locator('[data-testid="reconstructor-results-container"]')
    ).to_be_visible(timeout=WORKFLOW_TIMEOUT)
    ...
    page.locator("textarea.premise-textarea").first.fill(
        "Les changements climatiques sont un défi majeur"
    )
    ...
    expect(page.locator(".results-section")).to_be_visible(timeout=WORKFLOW_TIMEOUT)
    ...
    assert (
        performance["full_workflow"] < 60
    ), "Le workflow complet ne doit pas dépasser 60 secondes"
...
def test_framework_based_validation_workflow(
    ...
    expect(
        results_section.locator('h4:has-text("Extensions trouvées")')
    ).to_be_visible()
...
@pytest.mark.integration
@pytest.mark.e2e
def test_cross_tab_data_persistence(
    page_with_console_logs: "Page",
    e2e_servers,
    integration_helpers: IntegrationWorkflowHelpers,
    complex_test_data: Dict[str, Any],
    ...
    tabs_data = [
        ...
        (
            ...
            complex_test_data["complex_logic_formula"]["formula"],
        ),
    ]
    ...
    performance = integration_helpers.get_performance_report()
    assert (
        performance["data_persistence"] < 60
    ), "Le test de persistance ne doit pas dépasser 60 secondes"
    ...
    assert (
        persistent_tabs >= len(tabs_data) // 2
    ), "Au moins la moitié des onglets doivent conserver leurs données"
...
@pytest.mark.e2e
@pytest.mark.slow
def test_performance_stress_workflow(
    page_with_console_logs: "Page",
    e2e_servers,
    integration_helpers: IntegrationWorkflowHelpers,
    complex_test_data: Dict[str, Any],
    ...
    assert (
        performance_report["stress_test"] < 120
    ), "Le stress test complet ne doit pas dépasser 2 minutes"
    assert (
        rapid_navigation_duration < 25
    ), "La navigation rapide ne doit pas dépasser 25 secondes"
    ...
    assert (
        success_rate >= 0.5
    ), f"Au moins 50% des opérations doivent réussir (taux actuel: {success_rate:.2%})"
