...
def test_successful_graph_visualization(page_with_console_logs: Page, e2e_servers):
    ...
    expect(page_with_console_logs.locator(".api-status.connected")).to_be_visible(
        timeout=15000
    )
    ...
    text_input = page_with_console_logs.locator(
        '[data-testid="logic-graph-text-input"]'
    )
    submit_button = page_with_console_logs.locator(
        '[data-testid="logic-graph-submit-button"]'
    )
    graph_container = page_with_console_logs.locator(
        '[data-testid="logic-graph-container"]'
    )
...
@pytest.mark.playwright
@pytest.mark.skip(
    reason="Le composant de graphe ne s'affiche pas, problème frontend à investiguer."
)
