...
def test_dung_framework_analysis_api(
    ...
    test_data = {
        "arguments": [
            ...
            {"id": "c", "content": "Argument C", "attacks": [], "supports": []},
        ],
        "options": {"semantics": "preferred"},
    }
    ...
    assert (
        response_data["argument_count"] == 3
    ), f"Attendu 3 arguments, mais obtenu {response_data['argument_count']}"

    assert "attack_count" in response_data, "La clé 'attack_count' est manquante."
    assert (
        response_data["attack_count"] == 2
    ), f"Attendu 2 attaques, mais obtenu {response_data['attack_count']}"
    ...
    preferred_extension = next(
        (ext for ext in extensions_list if ext.get("type") == "preferred"), None
    )
    assert (
        preferred_extension is not None
    ), "Aucune extension de type 'preferred' n'a été trouvée."
    ...
    assert (
        actual_extension_content == expected_extension_content
    ), f"Contenu de l'extension incorrect. Attendu: {expected_extension_content}, Obtenu: {actual_extension_content}"
