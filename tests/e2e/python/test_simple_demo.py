...
@pytest.mark.e2e
def test_app_loads_successfully(page: Page, e2e_servers):
    """
...
@pytest.mark.e2e
def test_api_connectivity(page: Page, e2e_servers):
    """
    ...
    try:
        ...
        try:
            ...
            api_indicators = page.locator(
                '.api-status, [class*="status"], [class*="api"]'
            ).all()
...
@pytest.mark.e2e
def test_navigation_tabs(page: Page, e2e_servers):
    """
    ...
    try:
        ...
        potential_tabs = page.locator(
            'button, [role="tab"], [data-testid*="tab"], .tab'
        ).all()
