...
def backend_url(request) -> str:
    ...
    if not url:
        pytest.fail(
            "The --backend-url command-line option is not defined. The test runner must provide it."
        )
    return url
...
def frontend_url(request) -> str:
    ...
    if not url:
        pytest.fail(
            "The --frontend-url command-line option is not defined. The test runner must provide it."
        )
    return url
