...
class TestVerificationUtils(unittest.TestCase):
    ...
    def test_verify_extract_valid(self, mock_load_source_text, mock_extract_text_with_markers):
        ...
        self.assertEqual(result["message"], "Extrait valide. Les deux marqueurs ont été trouvés.")
    ...
    def test_verify_all_extracts(self, mock_verify_extract):
        ...
        mock_verify_extract.assert_any_call(extract_definitions_list[0], extract_definitions_list[0]["extracts"][0])
    ...
    def test_generate_verification_report(self, mock_file_open, mock_mkdir):
        ...
        results_data: List[Dict[str, Any]] = [
            {"source_name": "Source A", "extract_name": "Extrait 1", "status": "valid", "message": "OK", "extracted_length": 100, "start_found": True, "end_found": True},
            {"source_name": "Source B", "extract_name": "Extrait 2", "status": "invalid", "message": "Marqueur début manquant", "start_found": False, "end_found": True},
            {"source_name": "Source C", "extract_name": "Extrait 3", "status": "warning", "message": "Trop court", "extracted_length": 5, "start_found": True, "end_found": True, "template_issue": False, "encoding_issues": False},
            {"source_name": "Source D", "extract_name": "Extrait 4", "status": "error", "message": "Chargement source impossible"},
        ]
        ...
        mock_file_open.assert_called_once_with(Path(output_file_path), 'w', encoding='utf-8')
        ...
        self.assertIn("Extraits valides: <strong class=\"valid\">1</strong>", html_content)
        self.assertIn("Extraits avec avertissements: <strong class=\"warning\">1</strong>", html_content)
        self.assertIn("Extraits invalides: <strong class=\"invalid\">1</strong>", html_content)
        ...
        self.assertIn("<td>Source B</td><td>Extrait 2</td><td>invalid</td>", html_content)
