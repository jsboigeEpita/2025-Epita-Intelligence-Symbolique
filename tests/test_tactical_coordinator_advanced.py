...
class TestTacticalCoordinatorAdvanced(unittest.TestCase):
    ...
    def test_handle_task_result(self):
        ...
        self.assertIn("task-2", [task["id"] for task in self.tactical_state.tasks["completed"]])
        
        # Vérifier que le résultat a été enregistré
        self.assertIn("task-2", self.tactical_state.intermediate_results)
        ...
        self.assertEqual(report["report_type"], "objective_completion")
        self.assertEqual(report["content"]["objective_id"], "obj-1")
        self.assertEqual(report["content"]["status"], "completed")
        self.assertEqual(report["recipient_id"], "strategic_manager")
        self.assertEqual(report["priority"], MessagePriority.HIGH)
    ...
    def test_apply_strategic_adjustments(self):
        ...
        with patch.object(self.coordinator, '_determine_appropriate_agent', return_value="extract_processor") as mock_determine:
            ...
            self.assertEqual(len(self.adapter.sent_status_updates), 2)  # 1 pour la tâche, 1 pour la ressource
            ...
            self.assertEqual(task_update["status"]["task_id"], "task-1")
