...
class TestIntegrationSuccess:
    ...
    def test_mock_fact_claim_extractor_success(self):
        ...
        claim_texts = [
            getattr(c, "claim_text", c.get("claim_text", "")) for c in claims
        ]
    ...
    async def test_mock_fact_verification_service_success(self):
        ...
        mock_claim = type(
            "MockClaim", (), {"claim_text": "85% des entreprises utilisent l'IA"}
        )()
    ...
    async def test_mock_fact_verification_multiple_claims(self):
        ...
        claims = [
            type("MockClaim", (), {"claim_text": f"Claim {i}"})() for i in range(3)
        ]
    ...
    def test_mock_taxonomy_manager_success(self):
        ...
        text = (
            "Tous les experts s'accordent à dire que cette solution génère de la peur."
        )
    ...
    async def test_mock_family_analyzer_comprehensive(self):
        ...
        text = (
            "En 2024, 85% des experts confirment que l'IA génère de la peur chez tous."
        )
    ...
    async def test_mock_orchestrator_no_claims(self):
        ...
        result = await orchestrator.quick_fact_check(
            "Texte sans affirmations factuelles."
        )
...
class TestIntegrationScenarios:
    ...
    async def test_scenario_success_end_to_end(self):
        ...
        request = type(
            "MockRequest", (), {"text": text, "analysis_depth": "comprehensive"}
        )()
    ...
    async def test_scenario_error_recovery(self):
        ...
        request = type(
            "MockRequest",
            (),
            {"text": "Text that will cause an error", "analysis_depth": "standard"},
        )()
...
class TestIntegrationPerformance:
    ...
    async def test_concurrent_analysis(self):
        ...
        for i, text in enumerate(texts):
            request = type(
                "MockRequest", (), {"text": text, "analysis_depth": "basic"}
            )()
...
class TestIntegrationValidation:
    ...
    async def test_integration_chain_complete(self):
        ...
        if hasattr(response, "comprehensive_result"):
            ...
            if hasattr(result, "factual_claims") and hasattr(
                result, "fact_check_results"
            ):
                # Au moins l'un des composants devrait avoir produit des résultats
                has_content = (
                    len(result.factual_claims) > 0
                    or len(result.fact_check_results) > 0
                    or len(getattr(result, "family_results", {})) > 0
                )
                assert has_content
        else:
            # Format dict
