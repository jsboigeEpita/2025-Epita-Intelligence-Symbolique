# Script PowerShell pour exécuter test_load_theory.py avec CLASSPATH configuré
$ErrorActionPreference = "Stop"

Write-Host "--- Début de l'exécution de run_test_load_theory.ps1 ---"
...
$TestScript = Join-Path -Path $PSScriptRoot -ChildPath "test_load_theory.py"
...
Try {
    ...
    If ($LASTEXITCODE -ne 0) {
        ...
        # Pas besoin de 'exit $LASTEXITCODE' ici si $ErrorActionPreference = "Stop" est actif et qu'une erreur est levée
    } Else {
    ...
    # PowerShell lèvera une erreur et s'arrêtera à cause de $ErrorActionPreference = "Stop"
}

Write-Host "--- Fin de l'exécution de run_test_load_theory.ps1 ---"