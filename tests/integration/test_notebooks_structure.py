...
class TestNotebooksStructure(unittest.TestCase):
    ...
    def _verify_notebook_structure(self, notebook_relative_path):
        ...
        self.assertTrue(os.path.exists(notebook_path), f"Le notebook {notebook_path} (chemin relatif: {notebook_relative_path}) n'existe pas. Vérifié depuis {os.getcwd()}")
        ...
        try:
            ...
            self.assertTrue(isinstance(notebook_content['cells'], list), "L'attribut 'cells' n'est pas une liste.")
            ...
            for i, cell in enumerate(notebook_content['cells']):
                self.assertIn('cell_type', cell, f"La cellule {i} ne contient pas de type")
                self.assertIn('source', cell, f"La cellule {i} ne contient pas de source")
    ...
    def test_logic_agents_tutorial_notebook(self):
        ...
        self._verify_notebook_structure(os.path.join('examples', 'notebooks', 'logic_agents_tutorial.ipynb'))
    ...
    def test_api_logic_tutorial_notebook(self):
        ...
        self._verify_notebook_structure(os.path.join('examples', 'notebooks', 'api_logic_tutorial.ipynb'))
