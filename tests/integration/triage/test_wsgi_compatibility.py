...
def test_wsgi_health_check_via_asgi(e2e_servers, backend_url):
    ...
    try:
        ...
        try:
            ...
            pytest.fail(
                f"La réponse de {health_endpoint} n'est pas un JSON valide. Contenu: {response.text}"
            )

        # Vérification 3: Le contenu JSON est conforme à ce qui est attendu
        assert (
            "status" in response_json
        ), "La clé 'status' est absente de la réponse JSON."
        assert (
            response_json["status"] == "operational"
        ), f"Le statut attendu était 'operational', mais nous avons reçu '{response_json.get('status')}'. Réponse complète: {response_json}"

    except httpx.RequestError as e:
        pytest.fail(
            f"Une erreur de requête est survenue lors de la connexion à {health_endpoint}: {e}"
        )
