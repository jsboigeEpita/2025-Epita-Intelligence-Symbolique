...
class TestFOLTweetyCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_fol_formula_tweety_compatibility(
    ...
    @pytest.mark.skipif(not TWEETY_AVAILABLE, reason="TweetyBridge non disponible")
    @pytest.mark.asyncio
    async def test_fol_predicate_declaration_validation(
        self, fol_agent_with_kernel, jvm_session
    ):
...
"""
    ...
    @pytest.mark.skipif(not TWEETY_AVAILABLE, reason="TweetyBridge non disponible")
    @pytest.mark.asyncio
    async def test_fol_quantifier_binding_validation(
        self, fol_agent_with_kernel, jvm_session
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_programmatic_belief_set_creation_and_consistency(
        self, fol_agent_with_kernel, jvm_session
    ):
        """
        ...
        assert (
            java_belief_set_obj is not None
        ), "La création du BeliefSet programmatique a retourné None."

        # Créer un objet BeliefSet de haut niveau pour les étapes suivantes
        belief_set = FirstOrderBeliefSet(
            content=java_belief_set_obj.toString(), java_object=java_belief_set_obj
        )

        # Vérifier que le type est correct
        assert "FolBeliefSet" in str(
            type(belief_set.java_object)
        ), f"L'objet retourné n'est pas un FolBeliefSet mais un {type(belief_set.java_object)}"

        # 4. Vérifier la cohérence du BeliefSet créé
        is_consistent, consistency_msg = await agent.is_consistent(belief_set)
        assert (
            is_consistent
        ), f"Le BeliefSet créé par programmation est incohérent: {consistency_msg}"
        ...
        logger.info(
            "✅ Le flux de création programmatique du BeliefSet est validé avec succès (sans LLM)."
        )
...
class TestRealTweetyFOLAnalysis:
    ...
    @pytest.mark.asyncio
    async def test_real_tweety_fol_syllogism_analysis(
        self, fol_agent_with_kernel, jvm_session
    ):
...
"""
        ...
        ), "Le belief set du syllogisme devrait être consistant."
    ...
    async def test_real_tweety_fol_inconsistency_detection(
        ...
        assert (
            java_belief_set_obj is not None
        ), "La création du BeliefSet n'a pas réussi."
        belief_set = FirstOrderBeliefSet(
            content=java_belief_set_obj.toString(), java_object=java_belief_set_obj
        )
        ...
        assert (
            not is_consistent
        ), f"Le BeliefSet devrait être incohérent, mais il est considéré comme consistant. Message: {consistency_msg}"

        assert (
            "inconsistent" in consistency_msg.lower()
            or "incohérent" in consistency_msg.lower()
        ), f"Le message de retour '{consistency_msg}' n'indique pas une incohérence comme attendu."

        logger.info(
            f"✅ Incohérence programmatique (P ∧ ¬P) correctement détectée. Message: {consistency_msg}"
        )

    @pytest.mark.asyncio
    async def test_real_tweety_fol_inference_generation(
        self, fol_agent_with_kernel, jvm_session
    ):
    ...
    async def test_fol_syntax_error_recovery(self, fol_agent_with_kernel):
        ...
        try:
            ...
            if belief_set.is_empty():
                ...
                logger.warning(
                    f"⚠️ Le LLM a halluciné une structure logique à partir d'un texte absurde : {belief_set.content}. Le test valide simplement que l'agent n'a pas crashé."
                )

            # Le test est considéré comme réussi si on arrive ici sans exception.
            logger.info(
                "✅ Le test de robustesse est réussi : l'agent a géré un texte absurde sans planter."
            )

        except Exception as e:
            pytest.fail(
                f"L'agent a levé une exception inattendue face à un texte absurde: {e}",
                pytrace=True,
            )
...
class TestFOLPerformanceVsModal:
    ...
    @pytest.mark.asyncio
    async def test_fol_vs_modal_performance_comparison(
        self, fol_agent_with_kernel, jvm_session
    ):
        ...
        belief_set = FirstOrderBeliefSet(
            content=java_obj.toString(), java_object=java_obj
        )
        ...
        assert (
            fol_time < 10.0
        ), "Une analyse simple ne devrait pas prendre plus de 10 secondes."
...
class TestFOLRealWorldIntegration:
    ...
    @pytest.mark.asyncio
    async def test_fol_complex_argumentation_analysis(
        self, fol_agent_with_kernel, jvm_session
    ):
