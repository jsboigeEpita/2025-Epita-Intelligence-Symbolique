...
class TestNewOrchestrator:
    @pytest.mark.asyncio
    async def test_orchestrator_runs_successfully(
...
class TestWorkflowComparison:
    ...
    def mock_conversation_2agents(self):
        ...
        return [
            ChatMessageContent(
                role="assistant",
                content="Sherlock: J'examine les indices...",
                name="Sherlock",
            ),
            ChatMessageContent(
                role="assistant",
                content="Watson: Logiquement, cela suggère...",
                name="Watson",
            ),
            ChatMessageContent(
                role="assistant",
                content="Sherlock: Je propose Colonel Moutarde, Poignard, Salon",
                name="Sherlock",
            ),
        ]
    ...
    def mock_conversation_3agents(self):
        ...
        return [
            ...
            ChatMessageContent(
                role="assistant",
                content="Moriarty: Je révèle posséder Professeur Violet",
                name="Moriarty",
            ),
            ChatMessageContent(
                role="assistant",
                content="Sherlock: Avec cette information...",
                name="Sherlock",
            ),
            ChatMessageContent(
                role="assistant",
                content="Watson: Donc c'est Colonel Moutarde, Poignard, Salon",
                name="Watson",
            ),
        ]
    ...
    @pytest.mark.asyncio
    async def test_agent_capabilities_comparison(
        self, mock_kernel, comparison_elements
    ):
        ...
        watson_2 = factory.create_watson_agent(
            ...
            constants=[
                name.replace(" ", "")
                for category in comparison_elements.values()
    ...
    async def test_conversation_length_comparison(
        self, mock_kernel, mock_conversation_2agents, mock_conversation_3agents
    ):
        ...
        conv_3_revelations = sum(
            1 for term in revelation_terms if term in conv_3_content.lower()
        )
        conv_2_revelations = sum(
            1 for term in revelation_terms if term in conv_2_content.lower()
        )
    ...
    @pytest.mark.asyncio
    async def test_resolution_efficiency_simulation(
        self, mock_kernel, comparison_elements
    ):
        ...
        total_time_2 = metrics_2agents["setup_time"] + (
            metrics_2agents["total_turns"] * metrics_2agents["average_turn_duration"]
        )
        total_time_3 = metrics_3agents["setup_time"] + (
            metrics_3agents["total_turns"] * metrics_3agents["average_turn_duration"]
        )
        ...
        assert (
            metrics_3agents["resolution_confidence"]
            > metrics_2agents["resolution_confidence"]
        )
    ...
    async def test_strategy_adaptation_comparison(
        ...
        assert len(set(approaches_3)) == len(
            strategies
        )  # Approches variées pour 3-agents
...
class TestPerformanceComparison:
    ...
    async def test_memory_usage_comparison(self, performance_elements):
        ...
        data_3 = (
            len(state_3.get_hypotheses())
            + len(state_3.get_tasks())
            + len(state_3.recent_revelations)
            + len(state_3.agent_turns)
        )
    ...
    def test_solution_quality_comparison(self, performance_elements):
        ...
        valid_solutions_2 = sum(
            1
            for sol in solutions_2
            if (
                sol["suspect"] in performance_elements["suspects"]
                and sol["arme"] in performance_elements["armes"]
                and sol["lieu"] in performance_elements["lieux"]
            )
        )
        valid_solutions_3 = sum(
            1
