...
class TestOracleWorkflowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_oracle_workflow_setup(
        self, oracle_orchestrator, integration_elements
    ):
        """Test la configuration complète du workflow Oracle."""
        # Configuration du workflow
        oracle_state = await oracle_orchestrator.setup_workflow(
            nom_enquete="Integration Test Case", elements_jeu=integration_elements
        )
    ...
    @pytest.mark.asyncio
    async def test_revelation_system_integration(
        self, oracle_orchestrator, integration_elements
    ):
    ...
    async def test_termination_conditions(
        ...
        mock_history = [
            ChatMessageContent(
                role="assistant", content="Test message", name="TestAgent"
            )
        ]

        # Test de terminaison par nombre de tours
        should_terminate = await termination_strategy.should_terminate(
            mock_agent, mock_history
        )
...
class TestOraclePerformanceIntegration:
    ...
    def test_memory_usage_oracle_state(self, performance_kernel):
        ...
        assert (
            len(oracle_state.recent_revelations) <= 10
        )  # Limite des révélations récentes
...
class TestOracleErrorHandlingIntegration:
    ...
    async def test_dataset_connection_failure(self, error_test_kernel):
        ...
        with patch.object(
            ...
            result = await oracle_state.query_oracle(
                agent_name="TestAgent", query_type="test_query", query_params={}
            )
    ...
    async def test_invalid_configuration_handling(self, error_test_kernel):
        ...
        try:
            ...
            if hasattr(oracle_state, "oracle_strategy"):
                ...
                assert oracle_state.oracle_strategy in [
                    "cooperative",
                    "competitive",
                    "balanced",
                    "progressive",
...
class TestOracleScalabilityIntegration:
    ...
    async def test_large_game_configuration(self):
        ...
        assert len(moriarty_cards) < len(large_elements["suspects"]) + len(
            large_elements["armes"]
        ) + len(large_elements["lieux"])
