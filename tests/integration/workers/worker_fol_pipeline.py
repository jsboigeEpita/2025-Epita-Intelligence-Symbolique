...
class TestFOLPipelineIntegration:
    ...
    async def test_fol_with_real_tweety_integration(self):
        ...
        try:
            ...
            valid_formulas = ["∀x(Homme(x) → Mortel(x))", "Homme(socrate)"]
            ...
            belief_set = FirstOrderBeliefSet(
                belief_set_str
            )  # Création manuelle pour ce test
    ...
    def test_fol_error_analysis_integration(self):
        ...
        feedback = error_analyzer.analyze_error(
            fol_error, context={"logic_type": "first_order", "agent": "FOLLogicAgent"}
        )
    ...
    async def test_fol_syntax_validation_integration(self, fol_agent_with_kernel):
        ...
        for formula in test_formulas:
            try:
                ...
                validation_results.append(
                    {"formula": formula, "valid": False, "error": str(e)}
                )
