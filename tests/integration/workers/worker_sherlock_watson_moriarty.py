...
class TestRealGPTIntegration:
    ...
    async def test_real_gpt_kernel_connection(self, real_gpt_kernel, rate_limiter):
        ...
        chat_service: ChatCompletionClientBase = real_gpt_kernel.get_service(
            "real_openai_gpt4o_mini"
        )
    ...
    @pytest.mark.asyncio
    async def test_real_gpt_sherlock_agent_creation(
        self, real_gpt_kernel, real_gpt_elements, rate_limiter
    ):
        ...
        oracle_state = await orchestrator.setup_workflow(
            nom_enquete="Test Real GPT Sherlock", elements_jeu=real_gpt_elements
        )
        ...
        case_description = (
            await orchestrator.sherlock_agent.get_current_case_description()
        )
    ...
    @pytest.mark.asyncio
    async def test_real_gpt_watson_analysis(
        self, real_gpt_kernel, real_gpt_elements, rate_limiter
    ):
        ...
        oracle_state = await orchestrator.setup_workflow(
            nom_enquete="Test Real GPT Watson", elements_jeu=real_gpt_elements
        )
        ...
        chat_history.add_user_message(
            "Analysez logiquement: Colonel Moutarde avec le Poignard dans le Salon"
        )
        ...
        assert (
            "analyse" in analysis_result.lower() or "logique" in analysis_result.lower()
        )
        assert "Colonel Moutarde" in analysis_result

    @pytest.mark.asyncio
    async def test_real_gpt_moriarty_revelation(
        self, real_gpt_kernel, real_gpt_elements, rate_limiter
    ):
        ...
        oracle_state = await orchestrator.setup_workflow(
            nom_enquete="Test Real GPT Moriarty", elements_jeu=real_gpt_elements
        )
    ...
    @pytest.mark.asyncio
    async def test_real_gpt_complete_workflow(
        self, real_gpt_kernel, real_gpt_elements, rate_limiter
    ):
...
class TestRealGPTPerformance:
    ...
    async def test_real_gpt_token_usage(self, real_gpt_kernel, rate_limiter):
        ...
        chat_history = ChatHistory()
        chat_history.add_user_message(
            "Expliquez brièvement le jeu Cluedo en 2 phrases."
        )
...
class TestRealGPTErrorHandling:
    ...
    async def test_real_gpt_timeout_handling(self, real_gpt_kernel, rate_limiter):
        ...
        try:
            ...
            assert any(
                ...
                for keyword in ["rate limit", "quota", "limit", "timeout"]
            )
    ...
    async def test_real_gpt_retry_logic(self, real_gpt_kernel, rate_limiter):
        ...
        try:
            ...
            assert any(
                keyword in error_str
                for keyword in ["rate", "limit", "timeout", "quota"]
            )
...
class TestRealGPTAuthenticity:
    ...
    async def test_real_vs_mock_behavior_comparison(
        self, real_gpt_kernel, rate_limiter
    ):
        """Compare le comportement réel vs mock."""
        await rate_limiter()

        real_chat_service: ChatCompletionClientBase = real_gpt_kernel.get_service(
            "real_openai_gpt4o_mini"
        )
    ...
    @pytest.mark.asyncio
    async def test_real_gpt_oracle_authenticity(
        self, real_gpt_kernel, real_gpt_elements, rate_limiter
    ):
        ...
        oracle_state = await orchestrator.setup_workflow(
            nom_enquete="Test Authenticité Oracle", elements_jeu=real_gpt_elements
        )
