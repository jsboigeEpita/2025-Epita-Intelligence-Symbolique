...
def authentic_extract_agent():
    ...
    try:
        ...
        pytest.fail(
            f"Échec de la configuration de l'agent d'extraction authentique: {e}"
        )
...
def test_extract_from_name_large_text(authentic_extract_agent, mock_load_source_text):
    ...
    async def run_test():
        ...
        target_sentence = (
            "La déclaration cruciale sur l'économie verte est enfouie ici."
        )
        text_with_target = (
        ...
        assert (
            target_sentence in result.extracted_text
        ), "Le texte extrait devrait contenir la phrase cible."
