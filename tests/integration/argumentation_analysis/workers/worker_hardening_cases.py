...
def app(jvm_session):  # Dépend de jvm_session pour l'ordre d'initialisation
    ...
    """
    logger.info(
        "--- Création de l'application Flask pour les tests d'intégration (worker) ---"
    )
...
def test_analyze_complex_argumentative_text(client):
    ...
    assert any(
        "ad hominem" in f_name or "appel à l'autorité" in f_name
...
def main():
    ...
    try:
        ...
        logger.critical(
            f"Erreur critique dans le worker Hardening Cases: {e}", exc_info=True
        )
        sys.exit(1)
