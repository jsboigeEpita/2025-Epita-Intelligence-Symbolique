...
SCRIPT_PATH = os.path.join(PROJECT_ROOT, "scripts", "sherlock_watson", "run_unified_investigation.py")
...
def test_cluedo_workflow_integration(setup_test_environment):
    ...
    assert len(result_files) == 1, f"Un seul fichier de résultat JSON était attendu, mais {len(result_files)} ont été trouvés."
    
    result_file_path = result_files[0]
    assert os.path.exists(result_file_path), "Le fichier de trace JSON n'a pas été créé."
