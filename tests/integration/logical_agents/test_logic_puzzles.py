...
class TestLogicalAgentHardening:
    ...
    def test_agent_identifies_contradiction(self, kernel, load_scenario):
        ...
        assert (
            consistency_report["is_consistent"] is False
    ...
    def test_agent_handles_ambiguity(self, kernel, load_scenario):
        ...
        async def run_test():
            ...
            evidence_ids = [
                agent._evidence_manager.add_evidence({"description": f})
                for f in load_scenario["facts"]
            ]

            # 3. Manually create two competing hypotheses to simulate ambiguity
            hypothesis1_id = agent._hypothesis_tracker.create_hypothesis(
                "Le Colonel Moutarde est le coupable."
            )
            hypothesis2_id = agent._hypothesis_tracker.create_hypothesis(
            ...
            assert (
                solution_report["confidence_score"] < 0.8
            ...
            assert (
                len(solution_report.get("alternative_hypotheses", [])) > 0
            ), "There should be at least one alternative hypothesis."

            # Verify that the alternative is the other hypothesis we created
            alt_ids = [
                h["hypothesis_id"] for h in solution_report["alternative_hypotheses"]
            ]
            primary_id = solution_report["primary_hypothesis"]["hypothesis_id"]

            assert (
                hypothesis1_id in alt_ids or hypothesis2_id in alt_ids
            ), "The alternative hypothesis list should contain one of our created hypotheses."
            assert (
                primary_id != alt_ids[0]
            ), "The primary and alternative hypotheses must be different."
