...
def test_argumentation_syntax_in_subprocess(run_in_jvm_subprocess):
    ...
    """
    worker_script_path = (
        Path(__file__).parent / "workers" / "worker_argumentation_syntax.py"
    )

    print(
        f"Lancement du worker pour les tests de syntaxe d'argumentation: {worker_script_path}"
    )
    run_in_jvm_subprocess(worker_script_path)
    print(
        "Le worker de syntaxe s'est terminé, le test principal est considéré comme réussi."
    )
