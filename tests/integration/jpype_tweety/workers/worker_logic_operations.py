...
def _test_load_logic_theory_from_file(logic_classes, base_dir):
    ...
    assert (
        theory_file_path.exists()
    ), f"Le fichier de théorie {theory_file_path} n'existe pas."
    parser = PlParser()
...
def _test_simple_pl_reasoner_queries(logic_classes, base_dir):
    PlParser, Proposition, SimplePlReasoner = (
        logic_classes["PlParser"],
        logic_classes["Proposition"],
        logic_classes["SimplePlReasoner"],
    )
    theory_file_path = base_dir / "sample_theory.lp"
    parser = PlParser()
    belief_set = parser.parseBeliefBaseFromFile(str(theory_file_path))
...
def _test_formula_syntax_and_semantics(logic_classes):
    PlParser, Proposition, Negation, Conjunction, Implication = (
        logic_classes["PlParser"],
        logic_classes["Proposition"],
        logic_classes["Negation"],
        logic_classes["Conjunction"],
        logic_classes["Implication"],
    )
    parser = PlParser()
    formula_str1 = "p && q"
