...
def test_dialogical_argumentation_in_subprocess(run_in_jvm_subprocess):
    ...
    """
    worker_script_path = (
        Path(__file__).parent / "workers" / "worker_dialogical_argumentation.py"
    )

    print(
        f"Lancement du worker pour les tests d'argumentation dialogique: {worker_script_path}"
    )
    run_in_jvm_subprocess(worker_script_path)
    print(
        "Le worker d'argumentation dialogique s'est terminé, le test principal est considéré comme réussi."
    )
