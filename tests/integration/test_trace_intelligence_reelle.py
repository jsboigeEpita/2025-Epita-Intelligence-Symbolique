...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'trace_reelle_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
...
def test_scenario_cluedo_simple():
    ...
    try:
        ...
        description_cas_test = (
            "Un cas de test pour valider l'initialisation de CluedoOracleState."
        )
        ...
        pytest.fail(
            f"L'instanciation de CluedoOracleState a échoué avec une exception inattendue: {e}"
        )
