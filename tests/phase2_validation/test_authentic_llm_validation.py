...
class TestPhase2AuthenticLLMValidation:
    ...
    def setup_method(self):
        ...
        logger.info(f"✅ Configuration authentique : mock_level={self.config.mock_level.value}")
    ...
    def test_config_mock_level_validation(self):
        ...
        with pytest.raises(ValueError, match="Configuration incohérente.*mock_level=partial"):
    ...
    def test_environment_authentic_configuration(self):
        ...
        if missing_vars:
            logger.warning(f"⚠️ Variables manquantes pour tests authentiques: {missing_vars}")
...
def validate_no_mocks_in_traceback():
    ...
    for frame_info in frame_infos:
        ...
        for indicator in mock_indicators:
            ...
            assert indicator not in function_name, f"Mock détecté dans fonction: {function_name}"
