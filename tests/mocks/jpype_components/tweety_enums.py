...
if not logger.hasHandlers():
    ...
    formatter = logging.Formatter(
        "[MOCK JPYPE TWEETY_ENUMS LOG] %(asctime)s - %(levelname)s - %(message)s"
    )
...
class MockJavaEnumInstance(MagicMock):
    ...
    def __init__(
        self, enum_class_name=None, enum_name=None, enum_ordinal=None, *args, **kwargs
    ):
        super().__init__(*args, **kwargs)
        if enum_class_name is None or enum_name is None or enum_ordinal is None:
            logger.warning(
                f"MockJavaEnumInstance initialisée sans arguments spécifiques (probablement par un mécanisme de mock interne). class_name: {enum_class_name}, name: {enum_name}, ordinal: {enum_ordinal}"
            )
    ...
    def equals(self, other):
        if not isinstance(other, MockJavaEnumInstance):
            return False
        return (
            self._enum_class_name == other._enum_class_name
            and self._enum_name == other._enum_name
        )
...
class MockEnumMetaclass(type(MockJClassCore)):
    def __init__(cls, name, bases, dct):
        ...
        if (
            hasattr(cls, "_initialize_enum_members")
            and cls.__name__ != "MockTweetyEnum"
        ):
            cls._initialize_enum_members()

        def values_method(klass):
            logger.debug(
                f"MockEnum {klass.__name__}.values() appelée. Membres: {list(klass._enum_members.values())}"
            )
...
class MockTweetyEnum(MockJClassCore, metaclass=MockEnumMetaclass):
    ...
    def __init__(self, name, jclass_provider_func=None):
        ...
        logger.debug(
            f"MockTweetyEnum (classe {self.__name__}) initialisée pour le nom Java {name}."
        )
    ...
    def _initialize_enum_members(cls):
        ...
        if cls.__name__ == "MockTweetyEnum":
            logger.warning(
                "MockTweetyEnum._initialize_enum_members appelée directement sur la classe de base. Cela ne devrait pas arriver."
            )
            return
        raise NotImplementedError(
            f"{cls.__name__} doit implémenter _initialize_enum_members."
        )
    ...
    def _add_member(cls, name, ordinal):
        ...
        if name in cls._enum_members:
            ...
            logger.warning(
                f"Tentative de redéfinition du membre d'enum {name} pour {cls.__name__}. Ignoré."
            )
            return cls._enum_members[name]

        java_class_name = getattr(cls, "MOCK_JAVA_CLASS_NAME", cls.__name__)

        instance = MockJavaEnumInstance(
            enum_class_name=java_class_name, enum_name=name, enum_ordinal=ordinal
        )
        cls._enum_members[name] = instance
        setattr(cls, name, instance)
        logger.debug(
            f"Membre d'enum ajouté à {cls.__name__} ({java_class_name}): {name} (ordinal {ordinal}) -> {instance}"
        )
        return instance
    ...
    def valueOf(cls, name_str):
        logger.debug(
            f"MockEnum {cls.__name__}.valueOf('{name_str}') appelée. Membres: {list(cls._enum_members.keys())}"
        )
        ...
        if actual_string in cls._enum_members:
            ...
            java_class_name = getattr(cls, "MOCK_JAVA_CLASS_NAME", cls.__name__)
            logger.error(
                f"Aucun membre d'enum avec le nom '{actual_string}' trouvé dans {java_class_name}. Membres: {list(cls._enum_members.keys())}"
            )
    ...
    def __call__(self, *args, **kwargs):
        raise TypeError(
            f"Les classes Enum Java ne peuvent pas être instanciées directement. Utilisez les membres statiques (ex: {self.__name__}.MEMBER_NAME)."
        )
...
logger.info(
    "Module jpype_components.tweety_enums initialisé avec les mocks d'énumérations Tweety."
)
