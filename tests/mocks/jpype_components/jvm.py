...
if not mock_logger.hasHandlers():
    ...
    formatter = logging.Formatter(
        "[MOCK JPYPE JVM LOG] %(asctime)s - %(levelname)s - %(message)s"
    )
...
def startJVM(jvmpath=None, *args, **kwargs):
    ...
    mock_logger.info(
        f"startJVM appelée. _jvm_started mis à True. Classpath: {kwargs.get('classpath')}"
    )
...
def getDefaultJVMPath():
    ...
    if sys.platform == "win32":
        return (
            os.path.join(
                os.environ.get("JAVA_HOME", "C:\\Program Files\\Java\\jdk-11"),
                "bin",
                "server",
                "jvm.dll",
            )
            if os.environ.get("JAVA_HOME")
            else "C:\\Program Files\\Java\\jdk-11\\bin\\server\\jvm.dll"
        )
