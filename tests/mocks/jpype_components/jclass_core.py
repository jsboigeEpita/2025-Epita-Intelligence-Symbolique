...
if not mock_logger.hasHandlers():
    ...
    formatter = logging.Formatter(
        "[MOCK JPYPE JCLASS_CORE LOG] %(asctime)s - %(levelname)s - %(message)s"
    )
...
class MockJClassCore:
    def __init__(self, name, jclass_provider_func=None):
        ...
        self._instance_specific_config = (
            {}
        )  # Pour stocker des configs spécifiques à une classe
        self._jclass_provider = jclass_provider_func  # Pour que les configurateurs puissent créer des JClass
        mock_logger.debug(
            f"MockJClassCore pour '{name}' initialisée (jclass_provider: {jclass_provider_func is not None})."
        )
        ...
        if self.class_name == "java.lang.System":
            ...
            def mock_get_property(property_name):
                mock_logger.info(
                    f"[MOCK] java.lang.System.getProperty('{property_name}') CALLED"
                )
                ...
                mock_logger.warning(
                    f"Appel à System.getProperty pour une propriété non explicitement mockée: '{property_name}'. Retourne None."
                )
                return None
        ...
        if self.class_name == "org.tweetyproject.logics.ml.syntax.ModalLogic":
            ...
            for logic_name in logics:
                ...
                mock_logic_enum_instance = MagicMock(
                    name=f"MockModalLogicEnum_{logic_name}"
                )
                mock_logic_enum_instance.name = (
                    lambda: logic_name
                )  # Simuler la méthode name() de l'enum Java
                mock_logic_enum_instance.toString = (
                    lambda: logic_name
                )  # Simuler la méthode toString()
                self._static_attributes[logic_name] = mock_logic_enum_instance
            mock_logger.debug(
                f"Attributs statiques pour les logiques modales configurés pour {self.class_name}."
            )

    def __getattr__(self, attr_name):
        mock_logger.debug(
            f"MockJClassCore('{self.class_name}').__getattr__('{attr_name}')"
        )
        if attr_name in self._static_attributes:
            return self._static_attributes[attr_name]
        raise AttributeError(
            f"MockJClassCore '{self.class_name}' has no static attribute '{attr_name}' or it's not defined in _static_attributes."
        )
    ...
    def __call__(self, *args, **kwargs):
        ...
        mock_logger.debug(
            f"MockJClassCore('{self.class_name}').__call__ avec args: {args}, kwargs: {kwargs}"
        )

        if self.class_name in tweety_enums.ENUM_MAPPINGS:
            raise TypeError(
                f"Les classes Enum Java ({self.class_name}) ne peuvent pas être instanciées directement. Utilisez les membres statiques (ex: {self.class_name}.MEMBER_NAME)."
            )

        if self.class_name.startswith("java.util.") and (
            "List" in self.class_name
            or "Set" in self.class_name
            or "Map" in self.class_name
            or "Collection" in self.class_name
            or "Queue" in self.class_name
            or "Deque" in self.class_name
        ):
            mock_logger.info(
                f"Instanciation d'une collection Java simulée: {self.class_name}"
            )
        ...
        instance_mock.toString = MagicMock(
            return_value=f"MockedToString_{self.class_name}({args}, {kwargs})"
        )

        def default_instance_equals(other_obj):
            if (
                not isinstance(other_obj, MagicMock)
                or not hasattr(other_obj, "class_name")
                or instance_mock.class_name != other_obj.class_name
            ):
                return False
            ...
            for i in range(len(self_args)):
                ...
                if hasattr(arg_self, "equals") and callable(arg_self.equals):
                    if not arg_self.equals(arg_other):
                        return False
                elif arg_self != arg_other:
                    return False
            return True

        instance_mock.equals = MagicMock(
            name=f"{self.class_name}_equals", side_effect=default_instance_equals
        )
        ...
        instance_mock.hashCode = MagicMock(
            name=f"{self.class_name}_hashCode", side_effect=default_instance_hash_code
        )
        ...
        if self._jclass_provider is None:
            mock_logger.warning(
                f"JClass provider non disponible pour MockJClassCore('{self.class_name}'), la configuration des reasoners/agents pourrait être limitée."
            )

        if self.class_name == "net.sf.tweety.arg.dung.reasoner.CompleteReasoner":
            if self._jclass_provider:
                tweety_reasoners.configure_complete_reasoner_mock(
                    instance_mock, dung_theory_instance_arg, self._jclass_provider
                )
            else:
                mock_logger.error(
                    "Impossible de configurer CompleteReasoner: JClass provider manquant."
                )
        elif self.class_name == "net.sf.tweety.arg.dung.reasoner.StableReasoner":
            if self._jclass_provider:
                tweety_reasoners.configure_stable_reasoner_mock(
                    instance_mock, dung_theory_instance_arg, self._jclass_provider
                )
            else:
                mock_logger.error(
                    "Impossible de configurer StableReasoner: JClass provider manquant."
                )
        elif (
            self.class_name
            ...
            if self._jclass_provider:
                tweety_reasoners.configure_preferred_reasoner_mock(
                    instance_mock, dung_theory_instance_arg, self._jclass_provider
                )
            else:
                mock_logger.error(
                    "Impossible de configurer SimplePreferredReasoner: JClass provider manquant."
                )
        elif self.class_name == "net.sf.tweety.arg.dung.reasoner.GroundedReasoner":
            if self._jclass_provider:
                tweety_reasoners.configure_grounded_reasoner_mock(
                    instance_mock, dung_theory_instance_arg, self._jclass_provider
                )
            else:
                mock_logger.error(
                    "Impossible de configurer GroundedReasoner: JClass provider manquant."
                )
