...
if not tweety_agents_logger.hasHandlers():
    ...
    formatter = logging.Formatter('[MOCK TWEETY AGENTS LOG] %(asctime)s - %(levelname)s - %(message)s')
...
def configure_tweety_agent_class(jclass_core_instance: 'MockJClassCore'):
    ...
    if class_name in _agent_class_configs:
        ...
        tweety_agents_logger.info(f"Applying specific configuration for agent class: {class_name}")
        config_func(jclass_core_instance)
    else:
        tweety_agents_logger.debug(f"No specific agent configuration found for {class_name}. Applying default mock behavior.")
