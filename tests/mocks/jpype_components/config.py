...
if not mock_logger.hasHandlers():
    ...
    formatter = logging.Formatter(
        "[MOCK JPYPE CONFIG LOG] %(asctime)s - %(levelname)s - %(message)s"
    )
...
class MockConfig:
    def __init__(self):
        ...
        mock_logger.debug(
            f"MockConfig instance créée. jvm_path: {self.jvm_path}, convertStrings: {self.convertStrings}, onexit: {self.onexit}, destroy_jvm: {self.destroy_jvm}"
        )
