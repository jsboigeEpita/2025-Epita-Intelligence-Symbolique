...
def is_module_available(
    module_name,
):  # Copié depuis conftest.py, pourrait être dans un utilitaire partagé
...
def setup_pandas():
    ...
    if (
    ...
    ) or not is_module_available("pandas"):
        if not is_module_available("pandas"):
            print(
                "Pandas non disponible, utilisation du mock (depuis pandas_setup.py)."
            )
        else:
            print(
                "Python 3.12+ détecté, utilisation du mock Pandas (depuis pandas_setup.py)."
            )

        mock_pandas_attrs = {
            attr: getattr(pandas_mock, attr)
            for attr in dir(pandas_mock)
            if not attr.startswith("__")
        }
        mock_pandas_attrs["__version__"] = (
            pandas_mock.__version__
            if hasattr(pandas_mock, "__version__")
            else "2.0.3.mock"
        )
        ...
        print(
            f"Utilisation de la vraie bibliothèque Pandas (version {getattr(pandas, '__version__', 'inconnue')}) (depuis pandas_setup.py)."
        )
        return pandas
