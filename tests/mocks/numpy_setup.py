...
def deep_delete_from_sys_modules(module_name_prefix, logger_instance):
    ...
    keys_to_delete = [
        ...
        if k == module_name_prefix or k.startswith(module_name_prefix + ".")
    ]
    if keys_to_delete:
        logger.info(
            f"Nettoyage des modules sys pour préfixe '{module_name_prefix}': {keys_to_delete}"
        )
    for key in keys_to_delete:
        try:
            ...
            logger.warning(
                f"Clé '{key}' non trouvée dans sys.modules lors de la suppression (deep_delete)."
            )
...
def setup_numpy_for_tests_fixture(request):
    ...
    logger.info(
        f"Fixture numpy_setup pour {request.node.name}: Nettoyage initial de numpy, pandas, etc."
    )
    ...
    if use_real_numpy:
        ...
        logger.info(
            f"Test {request.node.name} marqué 'use_real_numpy': Configuration pour VRAI NumPy."
        )
        original_numpy = None
        try:
            ...
            logger.info(
                f"Vrai NumPy (version {original_numpy.__version__}) importé pour {request.node.name}."
            )
            ...
            logger.info(
                f"Fin de la section 'use_real_numpy' pour {request.node.name}. Restauration de l'état pré-fixture."
            )
        ...
        try:
            ...
            if "matplotlib" not in sys.modules:
                ...
                matplotlib_mock = create_module_mock(
                    "matplotlib",
                    submodules={"pyplot": None, "backends": {"backend_agg": None}},
                )
            ...
            pytest.fail(
                f"Impossible d'importer create_numpy_mock depuis tests.mocks.numpy_mock: {e}"
            )
        finally:
            logger.info(
                f"Fin de la section MOCK pour {request.node.name}. Restauration de l'état pré-fixture."
            )
            ...
            if (
                numpy_state_before
            ):  # Si quelque chose existait avant (improbable), le restaurer
                sys.modules["numpy"] = numpy_state_before
                logger.info("État NumPy pré-fixture restauré.")
