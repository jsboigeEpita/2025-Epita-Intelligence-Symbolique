...
def setup_extract_definitions_mock():
    ...
    try:
        ...
        def dict(self, **kwargs) -> Dict[str, Any]:
            ...
            """
            return {"sources": self.to_dict_list()}
        ...
        def mock_save_extract_definitions(
            ...
            **kwargs,
        ):  # Garder kwargs au cas où
            """Mock simplifié de save_extract_definitions."""
            try:
                logger.info(
                    f"Mock save_extract_definitions appelé avec: config_file={config_file}, key_path={key_path}"
                )
                    ...
                    try:
                        ...
                        if not current_encryption_key:  # Clé vide
                            ...
                            current_encryption_key = (
                                None  # Forcer la sauvegarde non chiffrée
                            )
                    except Exception as e_key_load:
                        logger.error(
                            f"Erreur lors du chargement de la clé depuis {key_path}: {e_key_load}"
                        )
                        current_encryption_key = (
                            None  # Forcer la sauvegarde non chiffrée en cas d'erreur
                        )
                ...
                if isinstance(extract_definitions, list):
                    ...
                    extract_definitions, "to_dict_list"
                ):  # Pour l'objet ExtractDefinitions
                    ...
                    logger.warning(
                        f"Type inattendu pour extract_definitions: {type(extract_definitions)}. Tentative de sauvegarde directe."
                    )
                ...
                if config_file:
                    ...
                    if current_encryption_key:
                        ...
                        json_data = json_module.dumps(
                            data_to_save, indent=2, ensure_ascii=False
                        ).encode("utf-8")
                        ...
                        logger.info(
                            f"Définitions chiffrées avec Fernet sauvegardées dans {config_file}"
                        )
                    else:
                        # Sauvegarder en JSON non chiffré
                        with open(
                            str(file_path), "w", encoding="utf-8"
                        ) as f_file:  # Renommer f en f_file pour éviter conflit
                            json_module.dump(
                                data_to_save, f_file, indent=2, ensure_ascii=False
                            )
                        logger.info(
                            f"Définitions sauvegardées (non chiffrées) dans {config_file}"
                        )
        ...
        def mock_load_extract_definitions(config_file, key, app_config=None):
            ...
            try:
                logger.info(
                    f"Mock load_extract_definitions appelé avec: config_file={config_file}"
                )
                ...
                if not config_file_path.exists():
                    logger.info(
                        f"Fichier config chiffré '{config_file}' non trouvé. Utilisation définitions par défaut."
                    )
                ...
                if not key:
                    logger.warning(
                        "Clé chiffrement absente. Chargement config impossible. Utilisation définitions par défaut."
                    )
                    return fallback_definitions[:]
                ...
                if isinstance(
                    key, str
                ):  # Supposons que str est une passphrase à dériver
                    logger.info(
                        f"Dérivation de la clé Fernet à partir de la passphrase (str) pour {config_file_path}"
                    )
                    final_fernet_key = derive_key_from_passphrase(key)
                elif (
                    isinstance(key, bytes) and len(key) == 44 and key.endswith(b"=")
                ):  # Heuristique pour une clé Fernet b64
                    logger.info(
                        f"Utilisation de la clé Fernet (bytes) fournie directement pour {config_file_path}"
                    )
                    final_fernet_key = key
                else:  # Fallback si ce n'est ni une str ni une clé Fernet bytes reconnaissable
                    logger.warning(
                        f"Type de clé inattendu ou format incorrect ({type(key)}, len={len(key) if isinstance(key, bytes) else 'N/A'}). Tentative de dérivation comme passphrase."
                    )
                    final_fernet_key = derive_key_from_passphrase(key)
                ...
                compressed_data = f_cipher.decrypt(
                    encrypted_data
                )  # Peut lever InvalidToken
                logger.info(
                    f"Mock: Données déchiffrées avec succès pour {config_file_path}"
                )
                ...
                if hasattr(data, "get") and "sources" in data:
                    ...
                    logger.warning(
                        "Format de données invalide. Utilisation définitions par défaut."
                    )
                    return fallback_definitions[:]
                ...
                if not hasattr(definitions, "__iter__") or isinstance(definitions, str):
                    logger.warning(
                        "Format définitions invalide (pas itérable). Utilisation définitions par défaut."
                    )
                    return fallback_definitions[:]
                ...
                if result_definitions:
                    logger.info(
                        f"[OK] {len(result_definitions)} définitions chargées depuis fichier mock."
                    )
                    return result_definitions
                else:
                    logger.warning(
                        "Aucune définition valide trouvée. Utilisation définitions par défaut."
                    )
                    return fallback_definitions[:]

            except InvalidToken as e_token_outer:
                logger.error(
                    f"Mock: Échec du déchiffrement (InvalidToken) pour {config_file_path} lors de l'appel à decrypt. Clé dérivée de passphrase commençant par: {key[:5]}..."
                )
                raise e_token_outer  # S'assurer que InvalidToken est bien relancée
            except Exception as e_other:  # Renommer pour clarté
                logger.error(
                    f"Mock: [ERREUR] Autre erreur non-InvalidToken lors du chargement mock pour {config_file_path}: {e_other}"
                )
                return fallback_definitions[:]
        ...
        logger.error(
            f"Erreur lors de la configuration du mock ExtractDefinitions : {e}"
        )
        return False
