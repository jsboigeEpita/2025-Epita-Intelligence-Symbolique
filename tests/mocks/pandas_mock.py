...
class MockPandasIOFormatsConsole:
    ...
    def get_console_size(self):
        ...
        height = (
            get_option("display.max_rows") or 24
        )  # Ou une autre option pertinente pour la hauteur
        return (width, height)
...
class DataFrame:
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, str):
            ...
            return DataFrame(
                {k: self._data.get(k, []) for k in key}, columns=key, index=self.index
            )
    ...
    def to_dict(self, orient="records"):
        ...
        if orient == "records":
            return [
                {col: self._data[col][i] for col in self.columns}
            ...
            return {
                i: {col: self._data[col][idx] for col in self.columns}
    ...
    def groupby(self, by):
        ...
        for i in range(len(self.index)):
            key = (
                ...
                else tuple(self._data[col][i] for col in by)
            )
    ...
    def copy(self):
        ...
        return DataFrame(
            self._data.copy(), columns=self.columns.copy(), index=self.index.copy()
        )
...
class PandasMock:
    ...
    def __init__(self):
        ...
        self.__spec__ = type(
            "ModuleSpec",
            (),
            {"name": "pandas", "origin": "mock", "submodule_search_locations": None},
        )()
