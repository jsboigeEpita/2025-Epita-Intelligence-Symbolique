...
try:
    ...
    logger.warning(
        "NumpyMock: Real numpy not found. Some mock components might be less accurate (e.g., flatiter)."
    )
...
class dtype:
    ...
    def __init__(self, type_spec):
        ...
        if (
            ...
            and type_spec
            and isinstance(type_spec[0], tuple)
        ):
        ...
        if (
            "32" in self.name
            or "bool" in self.name
            or "byte" in self.name
            or "short" in self.name
        ):
            self.itemsize = 4
        ...
        if (
            "8" in self.name
            and "64" not in self.name
            and "32" not in self.name
            and "16" not in self.name
        ):  # int8, uint8, mais pas float16/32/64
            self.itemsize = 1
...
class ndarray:
    ...
    def __init__(
        self, shape=None, dtype=None, buffer=None, offset=0, strides=None, order=None
    ):
...
if _real_numpy_broadcast:
    broadcast = (
        _real_numpy_broadcast  # Utilise le vrai broadcast si numpy est disponible
    )
else:
...
class _NumPy_Core_Numeric_Mock:
    ...
    def __init__(self):
        ...
        self.normalize_axis_tuple = MagicMock(
            name="numpy.core.numeric.normalize_axis_tuple"
        )
        self.absolute = MagicMock(
            name="numpy.core.numeric.absolute"
        )  # np.absolute est souvent np.core.numeric.absolute
    ...
    def __getattr__(self, name):
        logger.info(
            f"NumpyMock: numpy.core.numeric.{name} accédé (retourne MagicMock)."
        )
    ...
    def __init__(self):
        ...
        class NumpyVersion:
            def __init__(self, version_string):
                ...
                try:
                    self.major, self.minor, self.patch = map(
                        int, version_string.split(".")[:3]
                    )
            ...
            def __ge__(self, other_version_string):
                ...
                try:
                    other_major, other_minor, other_patch = map(
                        int, other_version_string.split(".")[:3]
                    )
                    if self.major > other_major:
                        return True
                    if self.major == other_major and self.minor > other_minor:
                        return True
                    if (
                        self.major == other_major
                        and self.minor == other_minor
                        and self.patch >= other_patch
                    ):
                        return True
                    return False
                except ValueError:
...
def power(x1, x2, out=None):
    ...
    if isinstance(x1, ndarray) or isinstance(x2, ndarray):
        ...
        dtype_res = (
            x1.dtype
            if isinstance(x1, ndarray)
            else (x2.dtype if isinstance(x2, ndarray) else float)
        )
        return ndarray(shape=shape, dtype=dtype_res)
...
class _NumPy_Rec_Mock:
    ...
    def __init__(self):
        ...
        self.__path__ = (
            []
        )  # Indique qu'il peut être traité comme un package (pour les imports)
    ...
    class _recarray_impl(
        ndarray
    ):  # Renommée pour éviter confusion, ndarray est défini plus haut
        ...
        def __init__(
            ...
            if input_array is not None:
                ...
                if (
                    isinstance(input_array, tuple)
                    and all(isinstance(dim, int) for dim in input_array)
                    and _shape_arg is None
                ):
                ...
                if (
                    hasattr(input_array, "dtype") and _dtype_arg is None
                ):  # Ne pas écraser un dtype explicite
                    _dtype_arg = input_array.dtype

            if (
                _shape_arg is None
            ):  # Fallback si shape n'est toujours pas déductible ou fourni
                ...
                _shape_arg = (
                    (0,)
                    if _names_arg is None
                    else (
                        len(_names_arg) if isinstance(_names_arg, (list, tuple)) else 0,
                    )
                )
        ...
        def __getattr__(self, name):
            ...
            if name in self._names:
                ...
                field_shape = (
                    (self.shape[0],) if self.shape and len(self.shape) > 0 else (0,)
                )
                ...
                try:
                    ...
                    field_format = (
                        self._formats[idx]
                        if self._formats and idx < len(self._formats)
                        else object
                    )
                except ValueError:
                    field_format = object
                ...
                return ndarray(
                    shape=field_shape, dtype=globals()["dtype"](field_format)
                )
            ...
            try:
                ...
                raise AttributeError(
                    f"'{type(self).__name__}' object has no attribute '{name}' and it's not a field name"
                )
...
if (
    _real_numpy_inexact
):  # Garder la logique existante si _real_numpy_inexact est déjà défini
    ...
    class inexact_mock(
        number, metaclass=dtype_base
    ):  # Hérite de number (qui pourrait être le vrai ou le mock)
...
if _real_numpy_flexible:  # Garder existant
    ...
    class flexible_mock(
        generic, metaclass=dtype_base
    ):  # Hérite de generic (vrai ou mock)
...
if _real_numpy_character:  # Garder existant
    ...
    class character_mock(
        flexible, metaclass=dtype_base
    ):  # Hérite de flexible (vrai ou mock)
...
def _get_real_or_mock_type(type_name_str, base_classes=(), metaclass_type=dtype_base):
    if _actual_numpy_module and hasattr(_actual_numpy_module, type_name_str):
        ...
        for bc in base_classes:
            if isinstance(
                bc, str
            ):  # Si on a passé un nom de type de base (ex: 'number')
                ...
                if resolved_bc is None:  # Fallback sur generic_mock si non résolu
                    logger.warning(
                        f"Type de base '{bc}' non résolu pour {type_name_str}, fallback sur generic."
                    )
                    actual_base_classes.append(
                        generic
                    )  # generic est déjà défini (vrai ou mock)
                else:
...
int64 = _get_real_or_mock_type(
    "int64", (number,)
)  # number est déjà défini (vrai ou mock)
...
class byte(metaclass=dtype_base):
    pass  # byte est np.int8


byte = byte


class ubyte(metaclass=dtype_base):
    pass  # ubyte est np.uint8


ubyte = ubyte


class short(metaclass=dtype_base):
    pass  # short est np.int16


short = short


class ushort(metaclass=dtype_base):
    pass  # ushort est np.uint16
...
logger.info(
    f"Type de bool_: {type(bool_)}, Type de number: {type(number)}, Type de object_: {type(object_)}"
)
...
class _NumPy_Core_Multiarray_Umath_Mock:
    ...
    def __getattr__(self, name):
        logger.info(
            f"NumpyMock: numpy.core._multiarray_umath.{name} accédé (retourne MagicMock)."
        )
