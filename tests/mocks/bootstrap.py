...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s [BOOTSTRAP] [%(levelname)s] %(message)s"
)
...
if os.environ.get("PYTEST_DISABLE_JPYPE_MOCK") == "1":
    log.warning(
        "Le mock JPype est DÉSACTIVÉ via la variable d'environnement PYTEST_DISABLE_JPYPE_MOCK."
    )
else:
    if "jpype" not in sys.modules:
        try:
            ...
            log.info(
                "Mock 'jpype_mock' et ses sous-modules appliqués pour JPype et JPype1."
            )
        except ImportError:
            sys.modules["jpype"] = MagicMock()
            sys.modules["jpype1"] = MagicMock()
            log.warning(
                "tests.mocks.jpype_mock non trouvé, utilisation de MagicMock pour JPype."
            )
