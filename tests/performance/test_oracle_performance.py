...
PERFORMANCE_TESTS_ENABLED = (
    os.environ.get("ENABLE_PERFORMANCE_TESTS", "false").lower() == "true"
)
...
class PerformanceProfiler:
    ...
    def stop_profiling(self) -> Dict[str, Any]:
        ...
        if self.metrics["memory_usage"]:
            ...
            self.metrics["avg_memory"] = sum(self.metrics["memory_usage"]) / len(
                self.metrics["memory_usage"]
            )

        if self.metrics["cpu_usage"]:
            self.metrics["avg_cpu"] = sum(self.metrics["cpu_usage"]) / len(
                self.metrics["cpu_usage"]
            )
            self.metrics["peak_cpu"] = max(self.metrics["cpu_usage"])

        self.metrics["success_rate"] = len(
            [call for call in self.metrics["api_calls"] if call["success"]]
        ) / max(len(self.metrics["api_calls"]), 1)
        self.metrics["error_rate"] = len(self.metrics["errors"]) / max(
            len(self.metrics["api_calls"]), 1
        )
...
class TestOracleResponseTimePerformance:
    ...
    async def test_single_api_call_performance(
        self, real_gpt_kernel_performance, performance_profiler
    ):
        ...
        try:
            ...
            assert (
                metrics["avg_response_time"] < 15.0
            ), f"Temps moyen trop élevé: {metrics['avg_response_time']}s"

    @pytest.mark.asyncio
    async def test_multiple_sequential_calls_performance(
        self, real_gpt_kernel_performance, performance_profiler
    ):
        ...
        try:
            ...
            ), f"Échecs dans appels séquentiels: {metrics['error_rate']}"
            assert (
                metrics["avg_response_time"] < 20.0
            ), f"Temps moyen séquentiel trop élevé: {metrics['avg_response_time']}s"
            assert (
                metrics["max_response_time"] < 30.0
            ), f"Temps max trop élevé: {metrics['max_response_time']}s"

            # Vérification de la stabilité des temps de réponse
            response_times = metrics["response_times"]
            if len(response_times) > 2:
                variance = sum(
                    (t - metrics["avg_response_time"]) ** 2 for t in response_times
                ) / len(response_times)
                std_dev = variance**0.5
                assert (
                    std_dev < 10.0
                ), f"Variance temps de réponse trop élevée: {std_dev}s"
    ...
    @pytest.mark.asyncio
    async def test_oracle_workflow_end_to_end_performance(
        self,
        real_gpt_kernel_performance,
        performance_test_elements,
        ...
        try:
            ...
            for i in range(3):
                ...
                performance_profiler.record_api_call(
                    interaction_time, result is not None
                )
                performance_profiler.sample_resources()
            ...
            ), f"Durée totale excessive: {metrics['total_duration']}s"
            assert (
                metrics["success_rate"] >= 0.8
            ), f"Trop d'échecs: {metrics['success_rate']}"
...
class TestOracleLoadPerformance:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_oracle_agents_performance(
        self, performance_test_elements, performance_profiler
    ):
        ...
        try:
            ...
            async def concurrent_oracle_task(oracle_state, agent_id):
                ...
                for iteration in range(5):
                    ...
                    response_time = time.time() - start_time
                    performance_profiler.record_api_call(
                        response_time, result is not None
                    )
            ...
            ), f"Concurrence trop lente: {concurrent_duration}s"
            assert (
                max(task_times) < 20.0
            ), f"Tâche individuelle trop lente: {max(task_times)}s"
            assert (
                metrics["success_rate"] >= 0.9
            ), f"Trop d'échecs concurrents: {metrics['success_rate']}"
    ...
    async def test_memory_usage_under_load(
        ...
        try:
            ...
            for i in range(10):
                ...
                performance_profiler.sample_resources()
                current_memory = (
                    performance_profiler.process.memory_info().rss / 1024 / 1024
                )
            ...
            assert (
                memory_growth < 500
            ), f"Croissance mémoire excessive: {memory_growth}MB"
            ...
            assert (
                cleanup_efficiency > 0.3
            ), f"Nettoyage mémoire insuffisant: {cleanup_efficiency}"
...
@pytest.mark.performance
@pytest.mark.skipif(
    not REAL_GPT_AVAILABLE, reason="Nécessite OPENAI_API_KEY pour tests JVM"
)
class TestTweetyJVMPerformance:
    ...
    async def test_jvm_memory_management(self, performance_profiler):
        ...
        try:
            ...
            try:
                from argumentation_analysis.core.utils.jvm_utils import (
                    ensure_jvm_started,
                )
            ...
            if jvm_available:
                ...
                assert (
                    jvm_startup_time < 10.0
                ), f"Démarrage JVM trop lent: {jvm_startup_time}s"
                ...
                ), f"Opérations JVM trop lentes: {metrics['avg_response_time']}s"
                assert (
                    metrics["peak_memory"] < 2000
                ), f"Utilisation mémoire JVM excessive: {metrics['peak_memory']}MB"
    ...
    def test_jvm_garbage_collection_impact(self, performance_profiler):
        ...
        try:
            ...
            if metrics["avg_cpu"]:
                assert (
                    metrics["peak_cpu"] < 90
                ), f"CPU pic trop élevé lors GC: {metrics['peak_cpu']}%"
...
class TestPerformanceRegression:
    ...
    async def test_performance_baseline_comparison(self, performance_profiler):
        ...
        try:
            ...
            if (
                ...
                > baseline_metrics["max_response_time"]
            ):
                performance_issues.append(
                    f"Temps max dégradé: {metrics['max_response_time']}s > {baseline_metrics['max_response_time']}s"
                )
            ...
            if (
                ...
                performance_issues.append(
                    f"Temps moyen dégradé: {metrics['avg_response_time']}s > {baseline_metrics['avg_response_time']}s"
                )

            if metrics.get("peak_memory", 0) > baseline_metrics["peak_memory"]:
                performance_issues.append(
                    f"Mémoire pic dégradée: {metrics['peak_memory']}MB > {baseline_metrics['peak_memory']}MB"
                )

            if metrics.get("success_rate", 0) < baseline_metrics["success_rate"]:
                performance_issues.append(
                    f"Taux succès dégradé: {metrics['success_rate']} < {baseline_metrics['success_rate']}"
                )

            assert (
                not performance_issues
            ), f"Régressions de performance détectées: {performance_issues}"
    ...
    async def test_scalability_limits(self, performance_profiler):
        ...
        ), f"Dégradation throughput excessive: {throughput_degradation}"

        # Toutes les charges devraient être supportées
        for load, result in scalability_results.items():
            assert (
                result["success_count"] == load
            ), f"Échecs sous charge {load}: {result['success_count']}/{load}"
