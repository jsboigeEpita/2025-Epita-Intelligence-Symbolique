...
class TestFunctionalReplacement:
            ...
            try:
                ...
                checks = {
                    ...
                    "no_critical_errors": len(
                        [e for e in result.validation_errors if "critical" in e.lower()]
                    )
                    == 0,
                }
                ...
                if all(checks.values()):
                    ...
                    logger.info(
                        f"   Formules: {len(result.formulas)}, Confiance: {result.confidence_score:.2f}"
                    )
                else:
    ...
    async def test_error_handling_improvement(self):
        ...
        ), f"Gestion erreurs insuffisante: {error_handling_rate:.1%}"
...
class TestPerformanceComparison:
    ...
    async def test_performance_parity_or_improvement(self):
        ...
        for text in test_texts:
            ...
            logger.info(
                f"FOL: {analysis_time:.2f}s, confiance: {result.confidence_score:.2f}"
            )
        ...
        ), f"Performance FOL trop lente: {avg_fol_time:.2f}s"
        assert (
            confidence_acceptable
        ), f"Confiance FOL trop faible: {avg_fol_confidence:.2f}"
    ...
    async def test_stability_improvement(self):
        ...
        time_variance = (
            statistics.variance(analysis_times) if len(analysis_times) > 1 else 0
        )
...
class TestRegressionValidation:
    ...
    async def test_no_functionality_regression(self):
        ...
        for test in basic_tests:
            try:
                ...
                if (
                    not isinstance(result, FOLAnalysisResult)
                    or result.confidence_score <= 0.0
                    or len(result.validation_errors) > 3
                ):  # Trop d'erreurs
                    if test["must_work"]:
    ...
    async def test_improvement_metrics(self):
        ...
        improvement_metrics = {"stability": 0, "confidence": [], "error_recovery": 0}
        ...
        avg_confidence = (
            statistics.mean(improvement_metrics["confidence"])
            if improvement_metrics["confidence"]
            else 0.0
        )
