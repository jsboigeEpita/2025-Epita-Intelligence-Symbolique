...
class ExtendedBelief:
    ...
    def __init__(self, name: str, agent_source: str = "unknown", 
                 context: Dict[str, Any] = None, confidence: float = 0.0):
        self.name = name
...
class JTMSSession:
    ...
    def add_belief(self, name: str, agent_source: str, context: Dict = None, confidence: float = 0.0):
        ...
        if name in self.extended_beliefs:
            ...
            existing.record_modification("updated_by_agent", {
    ...
    def add_justification(self, in_list: List[str], out_list: List[str],
        ...
        if conclusion in self.extended_beliefs:
            self.extended_beliefs[conclusion].record_modification("justification_added", {
                "in_list": in_list,
                "out_list": out_list,
...
"""
    ...
    def check_consistency(self) -> Dict:
        ...
        return {
            ...
            "total_justifications": sum(len(b.justifications) for b in self.jtms.beliefs.values()),
...
class JTMSAgentBase(ABC):
    ...
    def __init__(self, kernel: Kernel, agent_name: str, session_id: str = None, 
        ...
        self._session_id = session_id or f"{agent_name}_{int(datetime.now().timestamp())}"
        self._jtms_session = JTMSSession(self._session_id, agent_name, strict_mode)
        
        self._logger.info(f"JTMSAgentBase initialisé pour {agent_name} (session: {self._session_id})")
    ...
    def add_belief(self, belief_name: str, context: Dict = None, confidence: float = 0.0) -> ExtendedBelief:
        """Ajoute une croyance dans le JTMS de l'agent"""
        self._logger.info(f"Ajout croyance '{belief_name}' par {self._agent_name}")
        return self._jtms_session.add_belief(belief_name, self._agent_name, context, confidence)
    
    def add_justification(self, in_list: List[str], out_list: List[str], conclusion: str) -> None:
        ...
        try:
            ...
            for belief_name in in_list + out_list:
                if belief_name not in self._jtms_session.extended_beliefs:
                    self._logger.warning(f"Croyance '{belief_name}' inconnue pour validation")
                    return False
    ...
    def update_belief_confidence(self, belief_name: str, new_confidence: float) -> None:
        ...
        if belief_name in self._jtms_session.extended_beliefs:
            ...
            self._logger.info(f"Confiance mise à jour pour '{belief_name}': {old_confidence} → {new_confidence}")
    ...
    def import_beliefs_from_agent(self, other_agent_state: Dict, 
                                 conflict_resolution: str = "merge") -> Dict:
                ...
                if belief_name in self._jtms_session.extended_beliefs:
                    ...
                    if conflict_resolution == "merge":
                        ...
                        if belief_data["confidence"] > existing.confidence:
                            self.update_belief_confidence(belief_name, belief_data["confidence"])
        ...
        self._logger.info(f"Import terminé: {len(import_report['imported_beliefs'])} importées, "
