...
class FormalValidator:
        ...
        try:
            ...
            if belief.justifications:
                for i, justification in enumerate(belief.justifications):
                    step_result = await self._validate_justification_formally(
                        justification, i
                    )
                    ...
                    if step_result["valid"]:
                        ...
                        proof_result["confidence"] = max(
                            proof_result["confidence"], step_result["confidence"]
                        )
    ...
    async def _validate_justification_formally(
        self, justification, step_index: int
    ) -> Dict:
        ...
        try:
            ...
            for premise in justification.in_list:
                ...
                if premise_name in self.jtms_session.jtms.beliefs:
                    in_beliefs_valid.append(
                        self.jtms_session.jtms.beliefs[premise_name].valid
                    )
                else:
            ...
            for negative in justification.out_list:
                ...
                if negative_name in self.jtms_session.jtms.beliefs:
                    out_beliefs_valid.append(
                        self.jtms_session.jtms.beliefs[negative_name].valid
                    )
                else:
            ...
            premises_ok = all(
                valid is True for valid in in_beliefs_valid if valid is not None
            )
            negatives_ok = all(
                valid is not True for valid in out_beliefs_valid if valid is not None
            )
            ...
            # Calcul de confiance
            valid_count = sum(
                1 for v in in_beliefs_valid + out_beliefs_valid if v is not None
            )
            if valid_count > 0:
    ...
    def _construct_formal_proof(
        self, belief_name: str, validation_steps: List[Dict]
    ) -> str:
        ...
        for i, step in enumerate(validation_steps):
            if step.get("valid", False):
                proof_lines.append(
                    f"Étape {i+1}: {step.get('logical_structure', 'unknown')}"
                )
                proof_lines.append(
                    f"  Prémisses valides: {step.get('premises_valid', False)}"
                )
                proof_lines.append(
                    f"  Négations invalides: {step.get('negatives_invalid', False)}"
                )
