...
class FOLLogicAgent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.warning(
                f"Impossible de créer l'agent FOL SK réel: {e}. Mode dégradé activé."
            )
            self._sk_agent = None
    ...
    def _init_tweety_bridge(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Impossible d'initialiser TweetyBridge: {e}. Mode dégradé activé."
            )
            self._tweety_bridge = None
    ...
    def text_to_belief_set(
        ...
        if self._sk_agent:
            try:
                ...
                self.logger.error(
                    f"Erreur avec l'agent FOL SK réel: {e}. Utilisation du mode dégradé."
                )
...
class LogicAgentFactory:
    ...
    @staticmethod
    def create_agent(
        logic_type: str, kernel: Any = None, service_id: Optional[str] = None, **kwargs
    ) -> Any:
        """
        ...
        try:
            if logic_type == "first_order":
                ...
                try:
                    ...
                    logger.error(
                        f"Impossible de créer l'agent propositionnel réel: {e}"
                    )
...
class PropositionalLogicAgentAdapter:
    ...
    def generate_queries(
        self, text: str, belief_set, context: Optional[Dict[str, Any]] = None
    ):
        return ["a", "b", "a => b"]
