...
class AFHandler:
    ...
    def __init__(self, initializer_instance: TweetyInitializer):
        ...
        self.SimplePreferredReasoner = jpype.JClass("org.tweetyproject.arg.dung.reasoner.SimplePreferredReasoner")
        self.Extension = jpype.JClass("org.tweetyproject.arg.dung.semantics.Extension")

    def analyze_dung_framework(self, arguments: List[str], attacks: List[List[str]], semantics: str = "preferred") -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Analyzing Dung framework with {len(arguments)} arguments and {len(attacks)} attacks for '{semantics}' semantics.")
        
        if semantics.lower() != "preferred":
            raise NotImplementedError("Only 'preferred' semantics are currently supported.")
        ...
        try:
            ...
            for source_name, target_name in attacks:
                if source_name in arg_map and target_name in arg_map:
                    ...
                    logger.warning(f"Skipping invalid attack from '{source_name}' to '{target_name}' as one or both arguments do not exist.")

            # 4. Initialize the reasoner and compute extensions
            reasoner = self.SimplePreferredReasoner()
            extensions_java = reasoner.getModels(dung_theory) # This returns a Collection<Extension>
            ...
            result = {
                "semantics": "preferred",
                "extensions": {
                    "preferred": sorted(preferred_extensions) # Sort for predictable output
                },
                "statistics": {
            ...
            logger.info(f"Analysis successful. Found {len(preferred_extensions)} preferred extensions.")
            return result

        except jpype.JException as e:
            logger.error(f"A Java exception occurred during Dung framework analysis: {e.stacktrace()}", exc_info=True)
