...
class TweetyBridgeSK(TweetyBridge):
    ...
    def text_to_modal_belief_set(self, text: str) -> str:
        ...
        try:
            ...
            is_valid, validation_msg = self.validate_modal_belief_set(belief_set_content)
    ...
    def _convert_text_to_modal_internal(self, text: str) -> str:
        ...
        for problematic, replacement in problematic_terms.items():
            if problematic in text_lower:
                ...
                raise ValueError(f"Error parsing Modal Logic formula 'constant {problematic}' for logic 'S4': Predicate 'constant{problematic}' has not been declared.")
        
        # Conversion réussie - version simplifiée
        # Dans un vrai système, ceci utiliserait NLP pour extraire les concepts
        safe_propositions = ["urgent_action", "world_peace", "climate_change", "cooperation"]
    ...
    def generate_modal_queries(self, text: str, belief_set: str) -> str:
        ...
        """
        self._logger.info(f"[SK] generate_modal_queries pour belief set de {len(belief_set)} caractères")
        ...
        try:
            ...
            if invalid_queries:
                result["warnings"] = f"{len(invalid_queries)} requêtes invalides ignorées"
...
"""
    ...
    def demonstrate_sk_retry_difference(self, problematic_text: str) -> Dict[str, Any]:
        ...
        """
        self._logger.info(f"[DEMO] Démonstration différence SK retry avec: '{problematic_text}'")
