...
class InformalAgent:
    ...
    def analyze_text(self, text: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        result = {"fallacies": fallacies, "analysis_timestamp": self._get_timestamp()}
    ...
    def perform_complete_analysis(
        ...
        if "contextual_analyzer" in self.tools and context:
            ...
            if hasattr(contextual_analyzer, "analyze_context"):
                try:
                    result["contextual_analysis"] = contextual_analyzer.analyze_context(
                        text, context
                    )
                except Exception as e:
