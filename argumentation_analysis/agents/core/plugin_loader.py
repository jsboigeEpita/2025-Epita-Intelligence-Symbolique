...
from argumentation_analysis.agents.core.exceptions import PluginManifestError, CircularDependencyError
...
class PluginLoader:
    ...
    def load_plugins_from_directory(self, plugins_directory: str) -> dict[str, BasePlugin]:
        """
        ...
        for potential_plugin_dir in plugins_dir.iterdir():
            if potential_plugin_dir.is_dir():
                ...
                if manifest_path.is_file():
                    ...
                    if plugin_name:
                         all_manifests[plugin_name] = {"manifest": manifest, "path": potential_plugin_dir}
        ...
        for plugin_name in sorted_plugins:
            ...
            plugin_instance = self._load_plugin_from_manifest(plugin_info["path"], plugin_info["manifest"])
    ...
    def _read_manifest(self, manifest_path: Path) -> dict:
        ...
        try:
            ...
            raise PluginManifestError(f"Malformed JSON in manifest file: {manifest_path}")

        required_fields = ["name", "entrypoint_module", "entrypoint_class"]
        for field in required_fields:
            if field not in manifest:
                raise PluginManifestError(f"Missing required field '{field}' in manifest: {manifest_path}")
        return manifest

    def _resolve_dependencies(self, dependency_graph: dict[str, list[str]]) -> list[str]:
        """
        ...
        def visit(plugin_name):
            ...
            for dependency in dependency_graph.get(plugin_name, []):
                if dependency in visiting:
                    raise CircularDependencyError(f"Circular dependency detected: {plugin_name} -> {dependency}")
        ...
        for plugin_name in list(dependency_graph): # Use list to allow modification during iteration if needed
    ...
    def _load_plugin_from_manifest(self, plugin_path: Path, manifest: dict) -> BasePlugin:
        ...
        try:
            ...
            raise PluginManifestError(f"Failed to load entrypoint for plugin {manifest['name']}: {e}")
        finally:
            # Restore sys.path
            sys.path[:] = original_sys_path

        if not issubclass(plugin_class, BasePlugin):
            raise TypeError(f"Plugin class {entrypoint_class_name} must inherit from BasePlugin.")
