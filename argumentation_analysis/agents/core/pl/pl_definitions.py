...
class PropositionalLogicPlugin:
    ...
    def __init__(self):
        ...
        self._logger.info(
            "Instance PropositionalLogicPlugin créée. Tentative init composants JVM..."
        )
    ...
    def _initialize_jvm_components(self):
        ...
        if not jpype.isJVMStarted():
            self._logger.critical(
                "Tentative d'init Plugin PL alors que la JVM n'est PAS démarrée ! Fonctions natives échoueront."
            )
            self._jvm_ok = False
            return
        ...
        try:
            ...
            self._SatReasoner = jpype.JClass(
                "org.tweetyproject.logics.pl.reasoner.SatReasoner"
            )
            self._PlFormula = jpype.JClass(
            ...
            self._jvm_ok = True
            self._logger.info(
                "✅ Classes et instances Java Tweety chargées avec succès."
            )
        except Exception as e:
            self._logger.critical(
                f"❌ Erreur chargement classes/instances Tweety (JVM démarrée): {e}",
                exc_info=True,
            )
    ...
    def _internal_parse_formula(self, formula_string: str) -> Optional[Any]:
        if not self._jvm_ok or not self._parser_instance:
            self._logger.error(
                f"Parse formula: JVM/Parser non prêt ('{formula_string[:60]}...')."
            )
            return None
        try:
            ...
            raise RuntimeError(
                f"Erreur Parsing Formule: {e_java.getMessage()}"
            ) from e_java
        except Exception as e:
            self._logger.error(
                f"Erreur Python parsing formule '{formula_string}': {e}", exc_info=True
            )
    ...
    def _internal_parse_belief_set(self, belief_set_string: str) -> Optional[Any]:
        if not self._jvm_ok or not self._parser_instance:
            self._logger.error(
                f"Parse BS: JVM/Parser non prêt (BS: '{belief_set_string[:60]}...')."
            )
            return None
        try:
            ...
            self._logger.debug(
                f"Parsing belief set (nettoyé): '{belief_set_string_cleaned[:100]}...'. Longueur: {len(belief_set_string_cleaned)}"
            )
            ...
            if not bs_str_repr or all(
                line.strip().startswith("#")
                for line in bs_str_repr.splitlines()
                if line.strip()
            ):
                self._logger.warning(
                    f" -> BS parsé semble vide ou ne contient que des commentaires: '{bs_str_repr[:100]}...'"
                )
            else:
                self._logger.debug(
                    f" -> BS parsé (type: {type(parsed_bs)}, size: {parsed_bs.size()}, repr: '{bs_str_repr[:100]}...')"
                )
            return parsed_bs
        except jpype.JException as e_java:
            ...
            if hasattr(e_java, "message") and "line " in e_java.message():
                try:
                    ...
                    error_context = (
                        f" (Probablement près de la ligne {line_info} du belief set)"
                    )
                    error_msg += error_context
                except Exception:
                    pass  # Ne pas planter si l'extraction échoue
            raise RuntimeError(
                f"Erreur Parsing Belief Set: {e_java.getMessage()}"
            ) from e_java
        except Exception as e:
    ...
    def _internal_execute_query(
        self, belief_set_obj: Any, formula_obj: Any
    ) -> Optional[bool]:
        ...
        try:
            ...
            if not jpype.JObject(formula_obj, self._PlFormula):
                raise TypeError(
                    f"Objet requête n'est pas un PlFormula (type: {type(formula_obj)})"
                )
            ...
            self._logger.debug(
                f"Exécution requête '{formula_str}' avec raisonneur '{self._reasoner_instance.getClass().getName()}'..."
            )

            # Exécuter la requête
            result_java_boolean = self._reasoner_instance.query(
                belief_set_obj, formula_obj
            )
            self._logger.debug(
                f" -> Résultat brut Java pour '{formula_str}': {result_java_boolean}"
            )

            # Conversion et retour
            if result_java_boolean is None:
                self._logger.warning(
                    f"Requête Tweety pour '{formula_str}' a retourné null (indéterminé?)."
                )
                return None
            else:
                result_python_bool = bool(result_java_boolean)
                self._logger.info(
                    f" -> Résultat requête Python pour '{formula_str}': {result_python_bool}"
                )
            ...
            self._logger.error(error_msg)
            raise RuntimeError(
                f"Erreur Exécution Requête Tweety: {e_java.getMessage()}"
            ) from e_java
        except Exception as e:
            self._logger.error(
                f"Erreur Python exécution requête '{str(formula_obj)}': {e}",
                exc_info=True,
            )
    ...
    def execute_pl_query(self, belief_set_content: str, query_string: str) -> str:
        ...
        self._logger.info(
            f"Appel execute_pl_query: Query='{query_string}' sur BS ('{belief_set_content[:60]}...')"
        )
        if not self._jvm_ok:
        ...
        try:
            ...
            result_bool: Optional[bool] = self._internal_execute_query(
                belief_set_obj, formula_obj
            )
            if result_bool is None:
                ...
                self._logger.info(
                    f" -> Résultat formaté requête '{query_string}': {result_label}"
                )
            return result_str
...
def setup_pl_kernel(kernel: sk.Kernel, llm_service):
    ...
    logger.info(
        f"Configuration Kernel pour {plugin_name} (V10.1 - Correction Indentation)..."
    )

    # Vérifier si la JVM est prête AVANT d'instancier/ajouter le plugin
    if not jpype.isJVMStarted():
        setup_logger.error(
            f"❌ ERREUR: Tentative de setup PL Kernel alors que la JVM n'est PAS démarrée. Plugin {plugin_name} ne sera PAS ajouté."
        )
    ...
    if not pl_plugin_instance._jvm_ok:
        setup_logger.error(
            f"❌ ERREUR: Échec de l'initialisation des composants Java dans {plugin_name}. Plugin ne sera PAS complètement fonctionnel."
        )
    ...
    if plugin_name in kernel.plugins:
        if native_facade not in kernel.plugins[plugin_name]:
            logger.error(
                f"ERREUR CRITIQUE: Fonction native {plugin_name}.{native_facade} non enregistrée!"
            )
        else:
...
# Log de chargement
logging.getLogger(__name__).debug("Module agents.core.pl.pl_definitions chargé.")
