...
**Agents**
- `oracle_base_agent.py` (697 l. ; 711 avant la fusion des jumelles #2345, 723 avant le retrait du `ClassVar` `BASE_ORACLE_SYSTEM_PROMPT`, #2137) — `OracleTools` :37, voie d'exécution unique `_run_oracle_query` :62 (#2340), corps unique des jumelles `_query_and_report` :232 (#2345) et 8 `@kernel_function` : `validate_query_permission` :83, `execute_authorized_query` :115, `get_available_query_types` :171, `reveal_information_controlled` :205, `query_oracle_dataset` :291, `execute_oracle_query` :311, `check_agent_permission` :331, `validate_agent_permissions` :370 ; `OracleBaseAgent` :382 (`BaseAgent`), point d'entrée `process_oracle_request` :505, `get_oracle_statistics` :575, `reset_oracle_state` :613 ;
...
3. **`OracleTools` : surface partiellement redondante — marquée (#2137), exécution unifiée (#2340), jumelles fusionnées (#2345)** — `query_oracle_dataset` :291 et `execute_oracle_query` :311 étaient déclarées « fonctionnellement identiques » (ici et dans la docstring de la seconde). **Mesuré faux (#2345)** : la seconde taisait `revealed_information`, donc un LLM passant par ce nom ne voyait jamais ce que l'Oracle venait de révéler. Le partage de `_run_oracle_query` (#2340) unifiait l'`await`, pas la mise en forme. Les deux noms délèguent désormais à un corps unique `_query_and_report` :232 ; garde `tests/unit/argumentation_analysis/agents/core/oracle/test_oracle_query_twins_2345.py` (même réponse sur toute la matrice, née rouge sur la ligne de la révélation). `execute_authorized_query` :115 reste une troisième surface, **distincte par contrat** : elle prend `agent_name` et répond par une chaîne sur un type invalide au lieu de lever. `reveal_information_controlled` :205 est un placeholder explicite. Les **noms** restent exposés au kernel (arbitrage #2137, ci-dessous). Grep des 8 noms hors `oracle_base_agent.py` : seuls des tests et des archives les citent — **0** référence dans un prompt, un plan d'agent, un orchestrateur ou un workflow. **Marquage #2137** : cette mesure est **statique et structurellement aveugle pour des outils LLM-face** — un `@kernel_function` est appelé par le LLM à l'exécution (comme `identify_vulnerabilities`, nommé dans le prompt de `conversational_orchestrator.py:416`), donc « 0 appelant Python » n'est pas « jamais appelé ». Le retrait au motif du grep vide est explicitement écarté par l'arbitrage #2137 ; ces outils restent exposés au kernel.
4. **Divergence interface/implémentation** — **partiellement résolue (#2340)** : `OracleBaseAgent.process_oracle_request` (`oracle_base_agent.py:505`) est désormais **async**, conforme à la déclaration `interfaces.py:26`, parce que son corps attendait une coroutine sans `await` (le défaut ci-dessous). Le type de retour diverge toujours (`OracleResponse` contre le `Dict[str, Any]` déclaré), et `DatasetManagerInterface.execute_query` :69 (sync) contre `DatasetAccessManager.execute_query` :229 (**async**) reste ouvert. Contrats jamais branchés, donc jamais démentis autrement que par lecture.
