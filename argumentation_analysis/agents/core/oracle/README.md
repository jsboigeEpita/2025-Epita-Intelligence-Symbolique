...
**Agents**
- `oracle_base_agent.py` (698 l. ; 697 avant l'héritage #2358, 711 avant la fusion des jumelles #2345, 723 avant le retrait du `ClassVar` `BASE_ORACLE_SYSTEM_PROMPT`, #2137) — `OracleTools` :38, voie d'exécution unique `_run_oracle_query` :63 (#2340), corps unique des jumelles `_query_and_report` :233 (#2345) et 8 `@kernel_function` : `validate_query_permission` :84, `execute_authorized_query` :116, `get_available_query_types` :172, `reveal_information_controlled` :206, `query_oracle_dataset` :292, `execute_oracle_query` :312, `check_agent_permission` :332, `validate_agent_permissions` :371 ; `OracleBaseAgent` :383 (`BaseAgent`, **`OracleAgentInterface` #2358**), point d'entrée `process_oracle_request` :506, `get_oracle_statistics` :576, `reset_oracle_state` :614 ;
...
- `dataset_access_manager.py` (632 l.) — `QueryCache` :32, `DatasetAccessManager` :186 (**`DatasetManagerInterface` #2358** ; `execute_query` :230, `execute_oracle_query` :457, `check_permission` :535), `CluedoDatasetManager` :594 ;
...
- `interfaces.py` (162 l.) — `OracleAgentInterface` :36, `DatasetManagerInterface` :78, `StandardOracleResponse` :122, `OracleResponseStatus` :150 ; les types `QueryType`/`QueryResult`/`OracleResponse` des signatures viennent de `permissions.py` (#2358) ;
...
| `dataset_access_manager.py` | 632 | **actif-critique** | `CluedoDatasetManager` instancié `cluedo_extended_orchestrator.py:248` et `core/cluedo_oracle_state.py:204` |
...
| `permissions.py` | 450 | **actif** | `core/cluedo_oracle_state.py:18,152` ; `CluedoDatasetManager` construit son `PermissionManager` — `dataset_access_manager.py:601-603` |
...
| `interfaces.py` | 162 | **actif** | **contrat armé (#2358)** : `OracleBaseAgent` :383 et `DatasetAccessManager` :186 en héritent ; garde de conformité signatures + async-ité (`test_interface_contract_2358.py`) — le contrat avait « licencié » le défaut #2340 (déclaration sync pour un `check_permission` async) |
