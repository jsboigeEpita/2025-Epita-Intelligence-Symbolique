...
6. **Collision de nom** — documenté, résolution séquencée : `oracle/hypothesis_tracker.py:38` définit un `HypothesisTracker` **ATMS**, et `agents/sherlock_jtms_agent.py:24` en définit un **autre** (basé JTMS). Le côté ATMS est listé par #2137 comme **0 importeur production** — son sort (retrait vs câblage) appartient à cette issue ; renommer maintenant serait du churn sur une classe dont le destin se tranche là. Le renommage du survivant suivra ce verdict.
