...
**Agents**
- `oracle_base_agent.py` (711 l. ; 723 avant le retrait du `ClassVar` `BASE_ORACLE_SYSTEM_PROMPT`, #2137) — `OracleTools` :37, voie d'exécution unique `_run_oracle_query` :62 (#2340) et 8 `@kernel_function` : `validate_query_permission` :83, `execute_authorized_query` :115, `get_available_query_types` :171, `reveal_information_controlled` :205, `query_oracle_dataset` :236, `execute_oracle_query` :292, `check_agent_permission` :345, `validate_agent_permissions` :384 ; `OracleBaseAgent` :396 (`BaseAgent`), point d'entrée `process_oracle_request` :519, `get_oracle_statistics` :589, `reset_oracle_state` :627 ;
