...
class OracleTools:
    ...
    def __init__(
        self, dataset_manager: DatasetAccessManager, agent_name: Optional[str] = None
    ):
        """
    ...
    @kernel_function(
        name="get_available_query_types",
        description="Récupère les types de requêtes autorisés pour un agent.",
    )
    ...
    @kernel_function(
        name="reveal_information_controlled",
        description="Révèle des informations selon la politique de révélation.",
    )
    def reveal_information_controlled(
        self, target_agent: str, information_type: str, context: str = ""
    ) -> str:
        """
    ...
    async def query_oracle_dataset(self, query_type: str, query_params: str) -> str:
        ...
        try:
            ...
            if response.authorized:
                ...
                if response.revealed_information:
                    result_msg += (
                        f" | Révélations: {', '.join(response.revealed_information)}"
                    )
                return result_msg
            else:
    ...
    async def check_agent_permission(
        self, query_type: str, target_agent: str = None
    ) -> str:
        """
        ...
        try:
            ...
            is_authorized = await self.dataset_manager.check_permission(
                agent_to_check, query_type_enum
            )
    ...
    @kernel_function(
        name="validate_agent_permissions",
        description="Valide les permissions d'un agent.",
    )
    async def validate_agent_permissions(
        self, target_agent: str, query_type: str
    ) -> str:
        """
...
Vous êtes un gardien impartial mais stratégique des données."""
    ...
    def _log_oracle_interaction(
        self, requesting_agent: str, query_type: QueryType, response: OracleResponse
    ) -> None:
