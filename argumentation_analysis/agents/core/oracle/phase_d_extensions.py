...
class PhaseDExtensions:
    ...
    def generate_progressive_revelation(
        self, agent_context: str, reveal_content: str, intensity: float = 0.7
    ) -> str:
        """
        ...
        return revelation_format.format(
            suspense=suspense, transition=transition, content=reveal_content
        )
    ...
    def generate_narrative_twist(
        self, context: Dict[str, Any]
    ) -> Optional[NarrativeMoment]:
        """
    ...
    def calculate_ideal_trace_metrics(
        ...
        natural_indicators = sum(
            ...
            if any(
                ...
                for word in ["brilliant", "exact", "précis", "géni"]
            )
        )
        metrics["naturalite_dialogue"]["naturalite_echanges"] = min(
            7.5 + (natural_indicators / total_messages) * 2.5, 10.0
        )

        transition_indicators = sum(
            1
            for msg in list(conversation_data.get("messages", []))[1:]
            if any(
                phrase in msg.get("content", "").lower()
                for phrase in ["suite à", "en réaction", "après", "comme dit"]
            )
        )
        transition_rate = transition_indicators / max(total_messages - 1, 1)
        metrics["naturalite_dialogue"]["qualite_enchainements"] = min(
            6.7 + transition_rate * 3.3, 10.0
        )
        metrics["naturalite_dialogue"]["score_global"] = (
            metrics["naturalite_dialogue"]["naturalite_echanges"] * 0.6
            + metrics["naturalite_dialogue"]["qualite_enchainements"] * 0.4
        )
        ...
        for msg in conversation_data.get("messages", []):
            ...
            if agent in personality_scores:
                ...
                if agent == "Watson" and any(
                    word in content for word in ["brilliant", "exact", "admira"]
                ):
                    score = 1
                elif agent == "Sherlock" and any(
                    word in content for word in ["précis", "observ", "dédu"]
                ):
                    score = 1
                elif agent == "Moriarty" and any(
                    word in content for word in ["révél", "mystèr", "théâtr"]
                ):
                    score = 1
        ...
        for agent, scores in personality_scores.items():
            if scores:
                ...
                metrics["personnalites_distinctes"][f"coherence_{agent.lower()}"] = min(
                    7.0 + avg_score * 3.0, 10.0
                )

        valid_scores = [
            s
            for s in metrics["personnalites_distinctes"].values()
            if isinstance(s, float) and s > 0
        ]
        metrics["personnalites_distinctes"]["score_global"] = (
            sum(valid_scores) / len(valid_scores) if valid_scores else 0.0
        )

        # --- 3. Progression logique ---
        logical_progression = sum(
            1
            for msg in conversation_data.get("messages", [])
            if any(
                word in msg.get("content", "").lower()
                for word in ["donc", "ainsi", "par conséquent", "logique"]
            )
        )
        progression_rate = logical_progression / total_messages
        metrics["progression_logique"]["progression_deductions"] = min(
            7.0 + progression_rate * 3.0, 10.0
        )
        # Placeholder for resolution efficiency
        metrics["progression_logique"]["efficacite_resolution"] = 8.0
        metrics["progression_logique"]["score_global"] = (
            metrics["progression_logique"]["progression_deductions"] * 0.7
            + metrics["progression_logique"]["efficacite_resolution"] * 0.3
        )

        # --- 4. Dosage révélations ---
        revelations_count = sum(
            1
            for msg in conversation_data.get("messages", [])
            if "révél" in msg.get("content", "").lower()
            or "**" in msg.get("content", "")
        )
        dramatic_elements = sum(
            1
            for msg in conversation_data.get("messages", [])
            if any(
                element in msg.get("content", "")
                for element in ["*pause*", "*regard*", "*silence*"]
            )
        )

        metrics["dosage_revelations"]["timing_revelations"] = (
            min(8.0 + (dramatic_elements / revelations_count) * 2.0, 10.0)
            if revelations_count > 0
            else 7.0
        )
        metrics["dosage_revelations"]["impact_revelations"] = 8.5  # Placeholder
        metrics["dosage_revelations"]["score_global"] = (
            metrics["dosage_revelations"]["timing_revelations"] * 0.5
            + metrics["dosage_revelations"]["impact_revelations"] * 0.5
        )
        ...
        metrics["engagement_global"]["engagement_collaboratif"] = min(
            7.0 + engagement_rate * 3.0, 10.0
        )
        metrics["engagement_global"]["engagement_moriarty"] = metrics[
            "dosage_revelations"
        ][
            "score_global"
        ]  # Link to revelations
        metrics["engagement_global"]["score_global"] = (
            metrics["engagement_global"]["engagement_collaboratif"] * 0.5
            + metrics["engagement_global"]["engagement_moriarty"] * 0.5
        )
        ...
        final_score = sum(
            metrics[key]["score_global"] * weight for key, weight in weights.items()
        )
...
def extend_oracle_state_phase_d(oracle_state_class):
    ...
    def add_dramatic_revelation(
        ...
        if use_false_lead and random.random() < 0.2:  # 20% de chance
            (
                ...
                misdirection,
                true_reveal,
            ) = self.phase_d_extensions.create_false_lead_sequence(content)
            return f"{false_setup}\n\n{misdirection}\n\n{true_reveal}"
        else:
            return self.phase_d_extensions.generate_progressive_revelation(
                "", content, intensity
            )
    ...
    def get_ideal_trace_metrics(self) -> Dict[str, float]:
        ...
        conversation_data = {"messages": getattr(self, "conversation_history", [])}
    ...
    oracle_state_class.apply_conversational_polish_to_message = (
