...
- Protocoles : `protocols.py` — `DialogueType` :18 (**6**), `SpeechAct` :29 (**9**), `Proposition` :43, `FormalArgument` :64, `DialogueMove` :87. Les classes `DialogueProtocol` / `InquiryProtocol` / `PersuasionProtocol` (ex-:106/:135/:197) ont été **retirées (#2137)** — jumeaux morts du `dialogue_handler` JVM vivant ;
...
- **`walton_krabbe_relations` n'a aucun producteur — MARQUÉ (#2137)** : `dung_arbitration_stage.py:123` l'accepte, `invoke_callables.py:8809` passe `context.get("walton_krabbe_relations")`, mais rien en production ne pose cette clé (seul `tests/.../test_dung_arbitration_wiring.py:45`). La docstring (`dung_arbitration_stage.py:144-145`) diffère le producteur à « PR2/PR3 » — **le module `debate` n'émet aucun `SpeechAct`** : l'import de production est un vocabulaire sans émetteur. **Décision #2137 : le lecteur RESTE** (lu par du code que personne n'alimente — l'inverse de #1625) ; câbler un producteur est le sujet de **#1649** (qualifier undercut/rebut/undermine) ;
