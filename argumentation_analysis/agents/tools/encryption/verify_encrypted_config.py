...
def verify_encrypted_config():
    ...
    if not settings.passphrase:
        logger.error(
            "❌ La variable d'environnement 'TEXT_CONFIG_PASSPHRASE' n'est pas définie dans votre .env ou configuration."
        )
        return False
    ...
    definition_service = DefinitionService(
        crypto_service=crypto_service, config_file=config_file
    )
    ...
    for i, source in enumerate(extract_definitions.sources):
        ...
        logger.info(
            f"     - URL: {source.schema}://{'.'.join(source.host_parts)}{source.path}"
        )
        ...
        if source.extracts:
            ...
            logger.info(
                f"       - Marqueur de début: {first_extract.start_marker[:50]}..."
            )
            logger.info(f"       - Marqueur de fin: {first_extract.end_marker[:50]}...")

    # Valider les définitions
    is_valid, validation_errors = definition_service.validate_definitions(
        extract_definitions
    )
...
def main():
    ...
    if success:
        ...
        print(
            "   Le fichier est correctement formaté et contient des définitions d'extraits valides."
        )
    else:
