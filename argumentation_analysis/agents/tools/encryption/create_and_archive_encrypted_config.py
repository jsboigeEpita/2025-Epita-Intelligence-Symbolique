...
def main():
    ...
    print(
        "Pour vérifier que le fichier encrypté complet est valide, nous allons le charger"
    )
    ...
    if temp_cache_dir.exists():
        if original_cache_dir.exists():
            print(
                f"⚠️ Le répertoire '{original_cache_dir}' existe déjà. Il ne sera pas écrasé."
            )
        else:
            try:
                temp_cache_dir.rename(original_cache_dir)
                print(
                    f"[OK] Répertoire de cache original sauvegardé dans '{original_cache_dir}'."
                )
            except Exception as e:
                print(
                    f"⚠️ Erreur lors de la sauvegarde du répertoire de cache original: {e}"
                )
    ...
    if confirmation.lower() in ["o", "oui", "y", "yes"]:
        ...
        if original_cache_dir.exists():
            try:
                ...
                print(
                    f"[OK] Répertoire de cache original '{original_cache_dir}' supprimé."
                )
            except Exception as e:
                print(
                    f"⚠️ Erreur lors de la suppression du répertoire de cache original: {e}"
                )
        ...
        if extract_sources_json.exists():
            try:
                ...
                print(
                    f"⚠️ Erreur lors de la suppression du fichier '{extract_sources_json}': {e}"
                )
    ...
    print(
        "Pour restaurer les fichiers de cache à partir du fichier encrypté, utilisez le script:"
    )
