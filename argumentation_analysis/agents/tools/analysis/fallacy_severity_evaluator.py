...
class FallacySeverityEvaluator:
    ...
    def __init__(self):
        ...
        config = ConfigManager.load_config(
            "fallacy_severity_config", _load_severity_config
        )
        self.base_severity = config["base_severity"]
        self.context_modifiers = config["context_modifiers"]
        self.logger.info(
            "Évaluateur de gravité des sophismes initialisé via les services partagés."
        )

    def evaluate_severity(
        self, fallacy_type: str, argument: str, context: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(
            f"Évaluation de la gravité du sophisme '{fallacy_type}' dans le contexte: {context}"
        )
        ...
        if (
            context_type in self.context_modifiers
            and fallacy_type in self.context_modifiers[context_type]
        ):
        ...
        results = {
            ...
            "explanation": self._generate_explanation(
                fallacy_type, context_type, final_score
            ),
        }
    ...
    def _calculate_visibility(self, fallacy_type: str, argument: str) -> float:
        ...
        keyword_count = sum(
            1
            for keyword in fallacy_keywords[fallacy_type]
    ...
    def _generate_explanation(
        self, fallacy_type: str, context_type: str, score: float
    ) -> str:
        """
            ...
            if "severity" not in fallacy:
                ...
                if "argument" in fallacy and "context" in fallacy:
                    severity_results = self.evaluate_severity(
                        fallacy["fallacy_type"], fallacy["argument"], fallacy["context"]
                    )
                    ...
                    fallacy["severity"] = self.base_severity.get(
                        fallacy["fallacy_type"], 0.5
                    )
                    fallacy["severity_level"] = self._determine_severity_level(
                        fallacy["severity"]
                    )

        # Trier les sophismes par gravité (du plus grave au moins grave)
        sorted_fallacies = sorted(
            fallacies, key=lambda x: x.get("severity", 0.0), reverse=True
        )
    ...
    def evaluate_impact(
        ...
        """
        self.logger.info(
            f"Évaluation de l'impact du sophisme '{fallacy_type}' sur l'argument"
        )
        ...
        impact_results = {
            ...
            "impact_on_validity": self._calculate_validity_impact(
                severity_results["final_score"]
            ),
    ...
    def _generate_correction_suggestions(
        self, fallacy_type: str, argument: str
    ) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"Résultats de l'évaluation de gravité: {json.dumps(severity_results, indent=2, ensure_ascii=False)}"
    )
    ...
    print(
        f"Sophismes classés par gravité: {json.dumps(ranked_fallacies, indent=2, ensure_ascii=False)}"
    )

    # Exemple d'évaluation de l'impact d'un sophisme
    impact_results = evaluator.evaluate_impact(fallacy_type, argument, context)
    print(
        f"Résultats de l'évaluation d'impact: {json.dumps(impact_results, indent=2, ensure_ascii=False)}"
    )
