...
class ComplexFallacyAnalyzer:
    ...
    def __init__(self):
        ...
        config = ConfigManager.load_config(
            "complex_fallacy_config", _load_complex_fallacy_config
        )
        self.fallacy_combinations = config["fallacy_combinations"]
        self.structural_fallacies = config["structural_fallacies"]

        self.logger.info(
            "Analyseur de sophismes complexes initialisé via les services partagés."
        )
    ...
    def identify_combined_fallacies(self, argument: str) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(
            f"Identification des combinaisons de sophismes dans l'argument (longueur: {len(argument)})"
        )
        ...
        for combination_name, combination_info in self.fallacy_combinations.items():
            ...
            components_present = all(
                any(
                    fallacy["fallacy_type"] == component
            ...
            if components_present:
                ...
                base_severity = max(
                    fallacy.get("confidence", 0.5) for fallacy in component_fallacies
                )
                combined_severity = min(
                    1.0, base_severity + combination_info["severity_modifier"]
                )
        ...
        self.logger.info(
            f"Combinaisons de sophismes identifiées: {len(combined_fallacies)}"
        )

        return combined_fallacies

    def analyze_structural_fallacies(
        ...
        """
        self.logger.info(
            f"Analyse des sophismes structurels dans {len(arguments)} arguments"
        )
        ...
        self.logger.info(
            f"Sophismes individuels identifiés dans tous les arguments: {len(all_fallacies)}"
        )
            ...
            structural_fallacies.append(
                {
                    ...
                    "description": self.structural_fallacies["Contradiction cachée"][
                        "description"
                    ],
        ...
        self.logger.info(
            f"Sophismes structurels identifiés: {len(structural_fallacies)}"
        )
                ...
                for pos_kw in positive_keywords:
                    for neg_kw in negative_keywords:
                        ...
                        if pos_kw in arg1 and neg_kw in arg2:
                            ...
                            pos_context = arg1[
                                max(0, pos_idx - 20) : min(
                                    len(arg1), pos_idx + len(pos_kw) + 20
                                )
                            ]
                            neg_context = arg2[
                                max(0, neg_idx - 20) : min(
                                    len(arg2), neg_idx + len(neg_kw) + 20
                                )
                            ]
                        ...
                        if pos_kw in arg2 and neg_kw in arg1:
                            ...
                            pos_context = arg2[
                                max(0, pos_idx - 20) : min(
                                    len(arg2), pos_idx + len(pos_kw) + 20
                                )
                            ]
                            neg_context = arg1[
                                max(0, neg_idx - 20) : min(
                                    len(arg1), neg_idx + len(neg_kw) + 20
                                )
                            ]
        ...
        for i in range(len(arguments)):
            for j in range(len(arguments)):
                ...
                for k in range(len(arguments)):
                    ...
                    if (
                        any(kw in arg1 for kw in ["donc", "ainsi", "par conséquent"])
                        and any(
                            kw in arg2 for kw in ["donc", "ainsi", "par conséquent"]
                        )
                        and any(
                            kw in arg3 for kw in ["donc", "ainsi", "par conséquent"]
                        )
                    ):
                        circles.append(
                            {
    ...
    def identify_fallacy_patterns(self, text: str) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(
            f"Identification des motifs de sophismes dans le texte (longueur: {len(text)})"
        )
        ...
        for i, paragraph in enumerate(paragraphs):
            ...
            paragraph_fallacies.append({"paragraph_index": i, "fallacies": fallacies})

        self.logger.info(
            f"Sophismes identifiés dans {len(paragraph_fallacies)} paragraphes"
        )
            ...
            patterns.append(
                {
                    ...
                    "fallacy_types": [
                        pattern["fallacy_type1"],
                        pattern["fallacy_type2"],
                    ],
                    "severity": 0.7,
                    "severity_level": "Élevé",
    ...
    def _detect_alternation_patterns(
        self, paragraph_fallacies: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for type1 in fallacy_types:
            for type2 in fallacy_types:
                ...
                for i in range(len(paragraph_fallacies) - 1):
                    fallacies1 = [
                        f["fallacy_type"] for f in paragraph_fallacies[i]["fallacies"]
                    ]
                    fallacies2 = [
                        f["fallacy_type"]
                        for f in paragraph_fallacies[i + 1]["fallacies"]
                    ]
    ...
    def _detect_escalation_patterns(
        ...
        for i in range(len(paragraph_fallacies) - 2):
            ...
            fallacies2 = [
                f["fallacy_type"] for f in paragraph_fallacies[i + 1]["fallacies"]
            ]
            fallacies3 = [
                f["fallacy_type"] for f in paragraph_fallacies[i + 2]["fallacies"]
            ]
            ...
            for f1 in fallacies1:
                for f2 in fallacies2:
                    for f3 in fallacies3:
                        if (
                            f1 in escalation_order
                            and f2 in escalation_order
                            and f3 in escalation_order
                        ):
