...
class FactClaimExtractor:
    ...
    def extract_factual_claims(
        ...
        try:
            self.logger.info(
                f"Extraction d'affirmations factuelles dans un texte de {len(text)} caractères"
            )
            ...
            self.logger.info(
                f"Extraction terminée: {len(claims)} affirmations factuelles trouvées"
            )
            return claims
    ...
    def _segment_sentences(self, text: str) -> List[Dict[str, Any]]:
        ...
        for i, part in enumerate(parts):
            if part.strip():
                ...
                sentences.append(
                    {"text": part.strip(), "start": start, "end": end, "index": i}
                )
    ...
    def _extract_claims_by_pattern(
        self, sentence: str, claim_type: ClaimType, patterns: List[str], start_pos: int
    ) -> List[FactualClaim]:
    ...
    def _assess_verifiability(self, sentence: str, claim: str) -> ClaimVerifiability:
        ...
        opinion_score = sum(
            1 for keyword in self.opinion_keywords if keyword in sentence_lower
        )

        # Vérifier les indicateurs de fait
        fact_score = sum(
            1 for keyword in self.fact_keywords if keyword in sentence_lower
        )
    ...
    def _calculate_confidence(
        self, sentence: str, claim_type: ClaimType, match
    ) -> float:
