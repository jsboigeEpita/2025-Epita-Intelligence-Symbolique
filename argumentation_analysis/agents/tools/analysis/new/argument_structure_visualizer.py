...
class ArgumentStructureVisualizer:
    ...
    def visualize_argument_structure(
        ...
        fallacy_heatmap = self._generate_fallacy_heatmap(
            arguments, argument_structure, output_format
        )
        visualizations["fallacy_heatmap"] = fallacy_heatmap

        # Graphe des relations entre arguments
        argument_graph = self._generate_argument_graph(
            arguments, argument_structure, output_format
        )
        ...
        if output_path:
            ...
            if fallacy_heatmap["format"] in ["png", "html"]:
                heatmap_path = (
                    output_dir
                    / f"fallacy_heatmap_{timestamp}.{fallacy_heatmap['format']}"
                )
                with open(
                    heatmap_path, "w" if fallacy_heatmap["format"] == "html" else "wb"
                ) as f:
            ...
            if argument_graph["format"] in ["png", "html"]:
                graph_path = (
                    output_dir
                    / f"argument_graph_{timestamp}.{argument_graph['format']}"
                )
                with open(
                    graph_path, "w" if argument_graph["format"] == "html" else "wb"
                ) as f:
    ...
    def _generate_argument_graph(
        ...
        if output_format == "json":
            ...
            nodes = [
                {"id": n, "label": G.nodes[n]["label"], "text": G.nodes[n]["text"]}
                for n in G.nodes
            ]
            edges = [
            ...
            nx.draw_networkx_labels(
                G, pos, labels={n: G.nodes[n]["label"] for n in G.nodes}, font_size=10
            )
            ...
            if output_format == "png":
                output_path = (
                    f"argument_graph_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                )
    ...
    def _generate_fallacy_heatmap(
        ...
        fallacies = argument_structure.get("vulnerability_analysis", {}).get(
        ...
        for arg_idx, arg_fallacies in fallacy_by_argument.items():
            total_severity = sum(
                fallacy.get("risk_level_score", 0.5) for fallacy in arg_fallacies
            )
        ...
        if output_format == "json":
            content = json.dumps(
                {
                    "fallacy_by_argument": {
                        str(k): v for k, v in fallacy_by_argument.items()
                    },
            ...
            if output_format == "png":
                output_path = (
                    f"fallacy_heatmap_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                )
                plt.savefig(output_path, bbox_inches="tight", dpi=300)
                plt.close()
