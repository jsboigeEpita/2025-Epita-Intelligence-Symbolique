...
try:
    ...
    from project_core.utils.shell import (
        run_async,
    ...
    print(
        "[WARNING] Gestionnaire centralisé des ports non disponible, utilisation des ports par défaut"
    )
    ...
    async def run_async(command, **kwargs):
        raise NotImplementedError("Shell utilities not found")

    async def run_in_activated_env_async(command, **kwargs):
        raise NotImplementedError("Shell utilities not found")
...
class MinimalProcessCleaner:
    ...
    async def cleanup_webapp_processes(self, ports_to_check: List[int] = None):
        ...
        self.logger.info(
            "[CLEANER] Le nettoyage par nom de processus est désactivé. Passage au nettoyage par port."
        )
        ...
        for i in range(max_retries):
            ...
            self.logger.info(
                f"[CLEANER] Tentative {i+1}/{max_retries}: PIDs {list(pids_on_ports.keys())} trouvés sur les ports {list(pids_on_ports.values())}. Terminaison..."
            )
...
class MinimalBackendManager:
    ...
    async def _log_stream_continuously(
        self,
        ...
        while not stream.at_eof():
            try:
                ...
                self.logger.error(
                    f"[{stream_name}] Erreur lors de la lecture du flux: {e}"
                )
    ...
    async def start(self, port_override=0):
        ...
        self.logger.info(
            f"[BACKEND] Tentative de démarrage du backend sur le port {self.port}..."
        )
        ...
        try:
            ...
            self.logger.info(
                f"[BACKEND] PYTHONPATH for subprocess: {env.get('PYTHONPATH')}"
            )

            env["RUNNING_VIA_ENV_MANAGER"] = "true"
            self.logger.info(
                "[BACKEND] Injection de RUNNING_VIA_ENV_MANAGER=true pour éviter la double validation de l'environnement."
            )

            # Correction pour l'erreur "OMP: Error #15" sur Windows avec certaines librairies (numpy, torch)
            env["KMP_DUPLICATE_LIB_OK"] = "TRUE"
            self.logger.info(
                "[BACKEND] Injection de KMP_DUPLICATE_LIB_OK=TRUE pour corriger le conflit OpenMP."
            )
            ...
            self.process = await run_in_activated_env_async(
                command=command, env_name="projet-is", env=env  # Nom symbolique
            )
            self.logger.info(
                f"[BACKEND] Processus backend (PID: {self.process.pid}) lancé."
            )
            ...
            self.log_tasks = [
                ...
                asyncio.create_task(
                    self._log_stream_continuously(
                        self.process.stderr, "SERVER STDERR", startup_event
                    )
                ),
            ]
            ...
            try:
                ...
                self.logger.error(
                    f"[BACKEND] Timeout de {timeout_seconds}s atteint. Le serveur backend n'a pas signalé 'Application startup complete'."
                )
                await self.stop()
                return {
                    "success": False,
            ...
            self.logger.info(
                "[BACKEND] Message de démarrage détecté. Démarrage du health check actif..."
            )
            ...
            if await self.health_check(health_check_url):
                self.logger.info(
                    f"[BACKEND] Health check réussi. Backend démarré et prêt sur {url}"
                )
            else:
                self.logger.error(
                ...
                return {
                    "success": False,
                    "error": "Le health check du backend a échoué après le démarrage.",
                }
            ...
            self.logger.error(
                f"[BACKEND] Erreur critique lors du lancement du processus backend: {e}",
                exc_info=True,
            )
    ...
    async def health_check(self, url: str, timeout: int = 30) -> bool:
        ...
        self.logger.info(
            f"[BACKEND HEALTH CHECK] Démarrage de la vérification sur {url} (timeout: {timeout}s)"
        )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, timeout=2) as response:
                        if response.status == 200:
                            self.logger.info(
                                f"[BACKEND HEALTH CHECK] Succès ! Le serveur a répondu avec le statut {response.status}."
                            )
                            return True
                        else:
                            self.logger.warning(
                                f"[BACKEND HEALTH CHECK] Le serveur a répondu avec le statut {response.status}. Nouvelle tentative..."
                            )
            except aiohttp.ClientConnectorError as e:
                self.logger.info(
                    f"[BACKEND HEALTH CHECK] Échec de la connexion ({e}). Le serveur n'est probablement pas encore prêt. Nouvelle tentative..."
                )
            except asyncio.TimeoutError:
                self.logger.info(
                    "[BACKEND HEALTH CHECK] Timeout de la requête. Nouvelle tentative..."
                )
            except Exception as e:
                self.logger.error(
                    f"[BACKEND HEALTH CHECK] Erreur inattendue: {e}", exc_info=True
                )

            await asyncio.sleep(1)  # Attendre 1 seconde avant la prochaine tentative

        self.logger.error(
            f"[BACKEND HEALTH CHECK] Échec final après {timeout}s. Le serveur ne répond pas correctement."
        )
        return False
    ...
    async def stop(self):
        if self.process and self.process.returncode is None:
            ...
            try:
                ...
                self.logger.info(
                    f"[BACKEND] Processus backend (PID: {self.process.pid}) arrêté."
                )
            except asyncio.TimeoutError:
                self.logger.warning(
                    f"[BACKEND] Le processus backend (PID: {self.process.pid}) n'a pas répondu à terminate. Utilisation de kill."
                )
                if self.process.returncode is None:
                    self.process.kill()
...
class MinimalFrontendManager:
    ...
    async def start(self):
        ...
        self.logger.info(
            f"[FRONTEND] Utilisation de l'environnement Conda '{conda_env_name}' pour le démarrage."
        )

        start_command = start_command_str.split()

        self.logger.info(
            f"[FRONTEND] Tentative de démarrage du frontend dans '{path}' sur le port {port}..."
        )
        ...
        if self.backend_url:
            ...
            self.logger.info(
                f"[FRONTEND] Variable d'environnement REACT_APP_BACKEND_URL définie sur: {self.backend_url}"
            )

        try:
            # Utilisation de la nouvelle fonction unifiée `run_async` avec l'environnement corrigé
            self.process = await run_async(command=start_command, cwd=path, env=env)
            self.logger.info(
                f"[FRONTEND] Processus frontend (PID: {self.process.pid}) lancé."
            )
            ...
            try:
                ...
                async def log_stream_and_set_event(stream, stream_name):
                    while not stream.at_eof():
                        ...
                        if (
                            "Compiled successfully!" in line
                            or "Compiled with warnings" in line
                        ) and not startup_event.is_set():
                            startup_event.set()
                ...
                log_tasks = [
                    asyncio.create_task(
                        log_stream_and_set_event(self.process.stdout, "STDOUT")
                    ),
                    asyncio.create_task(
                        log_stream_and_set_event(self.process.stderr, "STDERR")
                    ),
                ]
                ...
                log_output = "\n".join(output_lines)
                self.logger.error(
                    f"[FRONTEND] Timeout de {timeout_seconds}s atteint. Le serveur n'a pas signalé son démarrage. Logs:\n{log_output}"
                )
                await self.stop()
                return {
                    "success": False,
                    "error": "Timeout lors du démarrage du frontend.",
                }
            ...
            self.logger.error(
                f"[FRONTEND] Erreur critique lors du lancement du processus frontend: {e}",
                exc_info=True,
            )
            return {"success": False, "error": str(e)}
    ...
    async def stop(self):
        if self.process and self.process.returncode is None:
            self.logger.info(
                f"[FRONTEND] Arrêt du processus frontend (PID: {self.process.pid})..."
            )
            ...
            try:
                ...
                for child in children:
                    self.logger.info(
                        f"[FRONTEND] Arrêt du processus enfant (PID: {child.pid})..."
                    )
                    child.kill()
                self.logger.info(
                    f"[FRONTEND] Arrêt du processus parent (PID: {parent.pid})..."
                )
                parent.kill()
                # Attendre que le processus principal soit bien terminé
                await asyncio.wait_for(self.process.wait(), timeout=10.0)
                self.logger.info(
                    f"[FRONTEND] Processus frontend (PID: {self.process.pid}) et ses enfants ({len(children)}) arrêtés."
                )
            except (psutil.NoSuchProcess, asyncio.TimeoutError) as e:
                self.logger.error(
                    f"[FRONTEND] Erreur lors de l'arrêt du processus frontend (PID: {self.process.pid}): {e}"
                )
...
class UnifiedWebOrchestrator:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.process_cleaner.config = (
            self.config
        )  # Injection de la config au moment de l'initialisation
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not self.config_path.exists():
            ...
            print(
                f"INFO: Le fichier de configuration '{self.config_path}' n'existe pas. Création avec les valeurs par défaut."
            )
        ...
        try:
            ...
            if not isinstance(user_config, dict):
                print(
                    f"WARNING: Le contenu de {self.config_path} est vide ou invalide. Utilisation de la configuration par défaut."
                )
                return default_config
            ...
            print(
                "INFO: Configuration utilisateur chargée et fusionnée avec les valeurs par défaut."
            )
            return merged_config

        except Exception as e:
            print(
                f"ERROR: Erreur lors du chargement de la configuration {self.config_path}: {e}. Utilisation de la configuration par défaut."
            )
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        if CENTRAL_PORT_MANAGER_AVAILABLE:
            try:
                ...
                set_environment_variables()
                print(
                    f"[PORTS] Configuration centralisée chargée - Backend: {backend_port}, Frontend: {frontend_port}"
                )

            except Exception as e:
                print(
                    f"[PORTS] Erreur gestionnaire centralisé: {e}, utilisation des valeurs par défaut"
                )
                backend_port = 5003
                frontend_port = 8081
    ...
    async def start_webapp(
        self, headless: bool = True, frontend_enabled: bool = None
    ) -> bool:
        """
        ...
        try:
            ...
            frontend_config_enabled = self.config.get("frontend", {}).get(
                "enabled", False
            )
            frontend_enabled_effective = frontend_config_enabled or (
                frontend_enabled is True
            )

            self.logger.info(
                f"[FRONTEND_DECISION] Config 'frontend.enabled': {frontend_config_enabled}"
            )
            self.logger.info(
                f"[FRONTEND_DECISION] Argument CLI '--frontend' (via paramètre frontend_enabled): {frontend_enabled}"
            )
            self.logger.info(
                f"[FRONTEND_DECISION] Valeur effective pour démarrer le frontend: {frontend_enabled_effective}"
            )

            if frontend_enabled_effective:
                self.logger.info(
                    "[FRONTEND_DECISION] Condition de démarrage du frontend est VRAIE. Tentative de démarrage..."
                )
                await self._start_frontend()
            else:
                self.logger.info(
                    "[FRONTEND_DECISION] Condition de démarrage du frontend est FAUSSE. Frontend non démarré."
                )
    ...
    async def run_tests(self, test_paths: List[str] = None, **kwargs) -> bool:
        ...
        if not self.config.get("playwright", {}).get("enabled", False):
            self.add_trace(
                "[INFO] TESTS DESACTIVES",
                "Les tests sont désactivés dans la configuration ('playwright.enabled: false').",
                "Tests non exécutés",
            )
            return True
        ...
        if self.app_info.status != WebAppStatus.RUNNING:
            self.add_trace(
                ...
                "L'application doit être démarrée pour lancer les tests d'intégration.",
                "Démarrage requis avant tests",
                status="error",
            )
            return False
        ...
        self.logger.info(
            f"Construction de la commande de test directe pour l'environnement '{conda_env_name}'."
        )
        ...
        self.logger.info(
            f"Utilisation de l'exécutable Python de l'environnement courant : {python_executable}"
        )
        ...
        try:
            ...
            if return_code == 0:
                self.add_trace(
                    "[TEST] SUCCES",
                    f"Pytest a terminé avec succès (code {return_code}).",
                    "Tests passés",
                )
                return True
            else:
                ...
                raise subprocess.CalledProcessError(
                    return_code, command, "La sortie est dans les logs ci-dessus."
                )
            ...
            self.logger.error(
                f"Une erreur inattendue est survenue pendant l'exécution des tests: {e}",
                exc_info=True,
            )
            raise e
    ...
    async def stop_webapp(self):
        ...
        try:
            ...
            if backend_config.get("enabled"):
                ...
                if start_port:
                    ports_to_check.append(start_port)
                fallback_ports = backend_config.get("fallback_ports")
                if fallback_ports:
                    ports_to_check.extend(fallback_ports)
    ...
    async def full_integration_test(
        ...
        **kwargs,
        ...
        try:
            self.add_trace(
                "[TEST] INTEGRATION COMPLETE", "Démarrage orchestration complète"
            )
            ...
            try:
                ...
                self.add_trace(
                    "[TEST] Lancement avec timeout global", f"{test_timeout_s}s"
                )
    ...
    async def _launch_playwright_browser(self):
        ...
        self.add_trace(
            "[PLAYWRIGHT] LANCEMENT NAVIGATEUR",
            f"Browser: {playwright_config.get('browser', 'chromium')}",
        )
        try:
            self.playwright = await async_playwright().start()
            browser_type = getattr(
                self.playwright, playwright_config.get("browser", "chromium")
            )
    ...
    async def _start_backend(self) -> bool:
        ...
        self.add_trace(
            "[BACKEND] DEMARRAGE BACKEND", "Lancement avec failover de ports"
        )
        ...
        if self.frontend_manager and self.app_info.frontend_url:
            ...
            if not frontend_ok:
                self.add_trace(
                    ...
                    "L'interface utilisateur ne répond pas, mais le backend est OK.",
                    status="warning",
                )

        self.add_trace(
            "[OK] SERVICES VALIDES", "Tous les endpoints critiques répondent."
        )
        return True
    ...
    async def _check_all_api_endpoints(self) -> bool:
        ...
        if not self.app_info.backend_url:
            self.add_trace(
                "[ERROR] URL Backend non définie",
                "Impossible de valider les endpoints",
                status="error",
            )
            return False
...
"""
    ...
    def _save_urls_to_file(self):
        ...
        try:
            ...
            self.logger.error(
                f"[ERROR] Impossible de sauvegarder les URLs dans {urls_file}: {e}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrateur Unifié d'Application Web"
    )
    parser.add_argument(
        "--config",
        default="argumentation_analysis/webapp/config/webapp_config.yml",
    ...
    parser.add_argument(
        "--visible", action="store_true", help="Mode visible (override headless)"
    )
    parser.add_argument(
        "--frontend", action="store_true", help="Force activation frontend"
    )
    parser.add_argument(
        "--tests", nargs="*", help="Chemins spécifiques des tests à exécuter."
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    # Commandes
    parser.add_argument(
        "--start", action="store_true", help="Démarre seulement l'application."
    )
    parser.add_argument("--stop", action="store_true", help="Arrête l'application.")
    parser.add_argument(
        "--test",
        action="store_true",
        help="Exécute seulement les tests sur une app déjà démarrée ou en la démarrant.",
    )
    parser.add_argument(
        "--integration",
        action="store_true",
        ...
        try:
            ...
            if args.no_playwright:
                ...
                orchestrator.logger.info(
                    "Tests Playwright désactivés via l'argument --no-playwright."
                )

            if args.exit_after_start:
                success = await orchestrator.start_webapp(
                    orchestrator.headless, args.frontend
                )
                if success:
                    orchestrator.logger.info(
                        "Application démarrée avec succès. Arrêt immédiat comme demandé."
                    )
            ...
            orchestrator.logger.error(
                f"❌ Erreur inattendue dans l'orchestration : {e}", exc_info=True
            )
            success = False
        finally:
