...
class AsyncRequestTracker:
        ...
        if self.running:
            ...
            with self.lock:
                for request_id, event in self.request_events.items():
                    if not event.is_set():
                        logger.warning(f"Résolution forcée de la requête {request_id} lors de l'arrêt")
                        self.response_data[request_id] = {"status": "timeout", "error": "Arrêt forcé du tracker"}
                        event.set()
    ...
    async def wait_for_response(self, request_id: str, timeout: Optional[float] = None) -> Any:
        """
        ...
        with self.lock:
            if request_id not in self.request_events:
                logger.warning(f"Tentative d'attente pour une requête non enregistrée: {request_id}")
                return None
            
            event = self.request_events[request_id]
            effective_timeout = timeout or self.pending_requests.get(request_id, {}).get("timeout", 10.0)
        ...
        try:
            ...
            logger.warning(f"Timeout en attendant la réponse pour la requête {request_id}")
            with self.lock:
                if request_id in self.pending_requests:
                    del self.pending_requests[request_id]
                if request_id in self.request_events:
    ...
    def _cleanup_expired_requests(self):
        ...
        with self.lock:
            ...
            for request_id in expired_requests:
                ...
                if request_id in self.request_events:
                    self.response_data[request_id] = {"status": "timeout", "error": "Requête expirée"}
                    self.request_events[request_id].set()
                del self.pending_requests[request_id]
...
class TacticalAdapterWithTimeout(TacticalAdapter):
    ...
    def __init__(self, agent_id: str, middleware: MessageMiddleware, request_tracker: Optional[AsyncRequestTracker] = None):
        """
    ...
    async def request_strategic_guidance_async(self, request_type: str, parameters: dict, 
                                              recipient_id: str, timeout: float = 10.0,
        ...
        try:
            ...
            response_data = await self.request_tracker.wait_for_response(request_id, timeout)
            return response_data or {"status": "timeout", "error": "Pas de réponse reçue"}
...
class MessageMiddlewareWithTimeout(MessageMiddleware):
    ...
    def receive_message(self, recipient_id: str, channel_type: ChannelType, timeout: float = 5.0) -> Optional[Message]:
        """
...
def patch_middleware_for_tests(middleware: MessageMiddleware):
    ...
    async def patched_send_request_async(*args, **kwargs):
        ...
        try:
            ...
            return await asyncio.wait_for(original_send_request_async(*args, **kwargs), 
                                         timeout=kwargs["timeout"] + 1.0)
        except asyncio.TimeoutError:
            logger.warning(f"Timeout dans send_request_async: {kwargs.get('request_type')}")
            ...
            response = request.create_response(
                content={"status": "timeout", "info_type": "response", "data": {"error": "Timeout"}}
            )
...
def create_patched_tactical_adapter(agent_id: str, middleware: MessageMiddleware) -> TacticalAdapterWithTimeout:
    """
...
def create_mock_responder(middleware: MessageMiddleware, responder_id: str = "tactical-agent-2"):
    """
        ...
        while True:
            try:
                ...
                if request and request.type == MessageType.REQUEST:
                    ...
                    # Créer une réponse
                    response = request.create_response(
                        content={"status": "success", "info_type": "response", "data": {"solution": "Mock solution"}}
                    )
                    ...
                    logger.info(f"Mock responder sent response for request: {request.id}")
...
async def run_test_with_timeout():
    ...
    try:
        ...
        if response.get("status") == "success":
            ...
            logger.warning(f"⚠️ Test partiellement réussi: La requête a été traitée mais avec un statut {response.get('status')}")
