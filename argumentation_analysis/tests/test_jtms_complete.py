...
class TestJTMSService:
    ...
    async def test_session(self, session_manager):
        ...
        session_id = await session_manager.create_session(
            agent_id="test_agent", session_name="Test_Session", metadata={"test": True}
        )
        return session_id
    ...
    async def test_instance(self, jtms_service, test_session):
        ...
        instance_id = await jtms_service.create_jtms_instance(
            session_id=test_session, strict_mode=False
        )
        return instance_id
    ...
    async def test_create_belief(self, jtms_service, test_instance):
        ...
        result = await jtms_service.create_belief(
            instance_id=test_instance, belief_name="test_belief", initial_value=None
        )
        ...
        result2 = await jtms_service.create_belief(
            instance_id=test_instance, belief_name="true_belief", initial_value=True
        )
    ...
    async def test_add_justification(self, jtms_service, test_instance):
        ...
        assert (
            result["conclusion_status"] is None
        )  # C devrait être None car A et B ne sont pas définis
    ...
    async def test_explain_belief(self, jtms_service, test_instance):
        ...
        assert (
            explanation["current_status"] is True
        )  # Devrait être vraie grâce à la prémisse
    ...
    async def test_set_belief_validity(self, jtms_service, test_instance):
        ...
        result = await jtms_service.set_belief_validity(
            test_instance, "changeable", True
        )
        assert result["old_value"] is None
        assert result["new_value"] is True

        # Changer à False
        result2 = await jtms_service.set_belief_validity(
            test_instance, "changeable", False
        )
    ...
    async def test_export_import_jtms_state(self, jtms_service, test_session):
        ...
        instance2 = await jtms_service.import_jtms_state(
            test_session, exported_data, "json"
        )
        ...
        assert (
            state1["statistics"]["total_beliefs"]
            == state2["statistics"]["total_beliefs"]
        )
        assert (
            state1["statistics"]["total_justifications"]
            == state2["statistics"]["total_justifications"]
        )
...
class TestJTMSSessionManager:
    ...
    async def test_list_sessions(self, session_manager):
        ...
        assert (
            len(final_agent1) == len(initial_agent1) + 2
        )  # 2 nouvelles sessions pour agent1
        assert (
            len(final_agent2) == len(initial_agent2) + 1
        )  # 1 nouvelle session pour agent2
...
class TestJTMSSemanticKernelPlugin:
    ...
    async def test_create_belief_function(self, sk_plugin):
        ...
        result = await sk_plugin.create_belief(
            belief_name="sk_test_belief", initial_value="true", agent_id="test_sk_agent"
        )
    ...
    async def test_query_beliefs_function(self, sk_plugin):
        ...
        result = await sk_plugin.query_beliefs(
            filter_status="all", agent_id="query_agent"
        )
    ...
    async def test_explain_belief_function(self, sk_plugin):
        ...
        await sk_plugin.add_justification(
            "explainable", "", "derived", agent_id="explain_agent"
        )

        result = await sk_plugin.explain_belief(
            belief_name="derived", agent_id="explain_agent"
        )
    ...
    async def test_get_jtms_state_function(self, sk_plugin):
        ...
        result = await sk_plugin.get_jtms_state(
            include_graph="true", include_statistics="true", agent_id="state_agent"
        )
...
class TestJTMSIntegration:
    ...
    async def test_create_reasoning_session(self, integration):
        ...
        session_id, instance_id = await integration.create_reasoning_session(
            agent_id="reasoning_agent", session_name="Reasoning Test"
        )
    ...
    async def test_multi_agent_reasoning(self, integration):
        ...
        agents_info = [
            ...
            {
                "agent_id": "agent2",
                "initial_beliefs": [{"name": "belief3", "value": True}],
            },
        ]
    ...
    async def test_checkpoint_with_description(self, integration):
        ...
        # Ajouter du contenu
        await integration.jtms_service.create_belief(
            instance_id, "checkpoint_belief", True
        )
