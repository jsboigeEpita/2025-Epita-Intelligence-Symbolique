...
class MockEnhancedComplexFallacyAnalyzer:
    def detect_composite_fallacies(
        self, arguments: List[str], context: Any
    ) -> Dict[str, Any]:
        logger.debug(
            f"MockEnhancedComplexFallacyAnalyzer.detect_composite_fallacies appelée avec {len(arguments) if arguments else 0} arguments."
        )
        return {
...
class MockEnhancedContextualFallacyAnalyzer:
    def analyze_context(self, text: str, context: Any) -> Dict[str, Any]:
        logger.debug(
            f"MockEnhancedContextualFallacyAnalyzer.analyze_context appelée pour le texte: '{str(text)[:50]}...'"
        )
        return {
            "context_analysis": {
...
class MockEnhancedFallacySeverityEvaluator:
    def evaluate_fallacy_severity(
        ...
        logger.debug(
            f"MockEnhancedFallacySeverityEvaluator.evaluate_fallacy_severity appelée avec {len(arguments) if arguments else 0} arguments."
        )
        return {
...
class MockEnhancedRhetoricalResultAnalyzer:
    def analyze_rhetorical_results(
        self, results: Dict[str, Any], context: Any
    ) -> Dict[str, Any]:
        logger.debug(
            "MockEnhancedRhetoricalResultAnalyzer.analyze_rhetorical_results appelée."
        )
        return {
            "overall_analysis": {
...
def create_mock_advanced_rhetorical_tools() -> Dict[str, Any]:
    ...
    logger.warning(
        "[OK] Outils d'analyse rhétorique avancés simulés créés et retournés."
    )
    return tools
