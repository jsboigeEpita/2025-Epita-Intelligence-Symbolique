...
class MockClarityScorer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.jargon_list = self.config.get(
            "jargon_list", ["synergie", "paradigm shift", "holistique", "disruptif"]
        )
        self.ambiguity_keywords = self.config.get(
            "ambiguity_keywords", ["peut-être", "possiblement", "certains", "quelques"]
        )
        ...
        logger.info("MockClarityScorer initialisé avec config: %s", self.config)
    ...
    def score_clarity(self, text: str) -> Dict[str, Any]:
        ...
        clarity_score += (
            self.clarity_penalties["complex_words_ratio"] * complex_word_ratio
        )
        ...
        passive_indicators = len(
            re.findall(r"\b(est|sont|été|fut|furent)\s+\w+é(e?s?)\b", text_lower)
        )
        ...
        for amb_word in self.ambiguity_keywords:
            ambiguity_found_count += len(
                re.findall(rf"\b{re.escape(amb_word)}\b", text_lower)
            )
        if ambiguity_found_count > 0:
            clarity_score += (
                self.clarity_penalties["ambiguity_keywords"] * ambiguity_found_count
            )
        ...
        logger.info(
            f"MockClarityScorer score calculé: {final_score:.2f}, facteurs: {factors}"
        )
        return {
