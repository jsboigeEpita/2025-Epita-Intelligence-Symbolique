...
class MockFallacyDetector:
    ...
    def __init__(self, model_name: str = "mock_fallacy_model", language: str = "fr"):
        ...
        logger.info(
            f"MockFallacyDetector initialisé avec model='{model_name}', language='{language}'."
        )
    ...
    def detect(self, text: str, context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if not isinstance(text, str):
            logger.warning(
                "MockFallacyDetector.detect a reçu une entrée non textuelle."
            )
            return []

        logger.info(
            f"MockFallacyDetector.detect appelée pour le texte : '{text[:70]}...'"
        )
        ...
        if "personnellement" in text_lower and "attaque" in text_lower:
            ...
            context_slice = text[
                max(0, start_index - 20) : min(len(text), end_index + 20)
            ]
            detected_fallacies.append(
                {
        ...
        logger.info(
            f"VALIDATION_CORRECTIF: Recherche de sophisme Ad Hominem simulé pour le texte : '{text_lower}'"
        )
        if "auteur" in text_lower and (
            "condamné" in text_lower or "fraude" in text_lower
        ):
            detected_fallacies.append(
                {
                    "fallacy_type": "Ad Hominem (Simulé)",
        ...
        logger.info(
            f"{len(detected_fallacies)} sophismes simulés détectés par MockFallacyDetector."
        )
