...
class MockEmotionalToneAnalyzer:
    ...
    def analyze_tone(self, text: str) -> Dict[str, Any]:
        ...
        if not isinstance(text, str):
            logger.warning("MockEmotionalToneAnalyzer.analyze_tone a reçu une entrée non textuelle.")
            return {"error": "Entrée non textuelle", "emotions_detected": {}, "dominant_emotion": "Inconnue"}

        logger.info("MockEmotionalToneAnalyzer analyse le texte : %s...", text[:100])
        
        emotions_scores: Dict[str, float] = {emotion: 0.0 for emotion in self.emotion_keywords}
        ...
        for emotion, score in emotions_scores.items():
            if score > max_score and score >= 0.1: # Un minimum de score pour être considéré
                max_score = score
        ...
        logger.info(f"MockEmotionalToneAnalyzer a détecté les scores: {emotions_scores}, dominant: {dominant_emotion}")
        return {
