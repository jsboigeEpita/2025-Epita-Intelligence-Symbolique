...
class MockEvidenceDetector:
    ...
    def detect_evidence(
        self, text: str, claims: List[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for keyword in self.evidence_keywords:
            ...
            if (
                keyword.isalnum()
            ):  # Si le mot-clé est un mot simple, utiliser les frontières de mot
            ...
            for match in matches:
                ...
                if period_match != -1:
                    ...
                    evidence_text_end = min(
                        len(text), evidence_text_start + 150
                    )  # Portion plus longue pour les preuves
        ...
        logger.info(
            f"MockEvidenceDetector a trouvé {len(evidences)} éléments de preuve."
        )
        return evidences
