...
class MockContextualFallacyDetector:
    ...
    def detect_multiple_contextual_fallacies(self, arguments: List[str], context_description: str) -> Dict[str, Any]:
...
class MockArgumentCoherenceEvaluator:
    ...
    def evaluate_coherence(self, arguments: List[str], context: str = None) -> Dict[str, Any]:
        ...
        overall_score_numerator = sum(eval_data["score"] * eval_data["importance"] for eval_data in coherence_evaluations.values())
        overall_score_denominator = sum(eval_data["importance"] for eval_data in coherence_evaluations.values())
        overall_score = round(overall_score_numerator / overall_score_denominator, 2) if overall_score_denominator else 0.0
        
        if overall_score > 0.8: coherence_level = "Excellent"
        elif overall_score > 0.6: coherence_level = "Bon"
        elif overall_score > 0.4: coherence_level = "Moyen"
        else: coherence_level = "Faible"
        
        strengths = [f"Bonne cohérence {ct} ({ev['score']:.2f})" for ct, ev in coherence_evaluations.items() if ev["score"] > 0.7]
        weaknesses = [f"Faible cohérence {ct} ({ev['score']:.2f})" for ct, ev in coherence_evaluations.items() if ev["score"] < 0.5]
