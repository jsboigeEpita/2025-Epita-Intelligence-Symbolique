...
class MockCoherenceAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.contradiction_pairs = (
            self.config.get(  # Paires de phrases/mots-clés contradictoires (simplifié)
                "contradiction_pairs",
                [
        ...
        logger.info("MockCoherenceAnalyzer initialisé avec config: %s", self.config)

    def analyze_coherence(
        self, text: str, previous_text_summary: str = None
    ) -> Dict[str, Any]:
        """
        ...
        if not isinstance(text, str) or not text.strip():
            logger.warning(
                "MockCoherenceAnalyzer.analyze_coherence a reçu une entrée non textuelle ou vide."
            )
            return {
                "error": "Entrée non textuelle ou vide",
                "coherence_score": 0.0,
                "factors": {},
            }
        ...
        if num_words == 0:
            return {
                "coherence_score": 0.0,
                "factors": factors,
                "interpretation": self._interpret_score(0.0),
            }

        # Mots de transition
        transition_word_count = 0
        for tw in self.transition_words:
            transition_word_count += len(
                re.findall(rf"\b{re.escape(tw)}\b", text_lower)
            )
        ...
        if contradictions_found > 0:
            coherence_score += (
                self.coherence_factors["contradiction_penalty"] * contradictions_found
            )
        ...
        if previous_text_summary and isinstance(previous_text_summary, str):
            ...
            prev_summary_words = set(
                re.findall(r"\b\w+\b", previous_text_summary.lower()[:100])
            )
            current_text_start_words = set(
                words[:20]
            )  # 20 premiers mots du texte actuel
            common_words_ratio = len(
                prev_summary_words.intersection(current_text_start_words)
            ) / (len(prev_summary_words) + 1e-6)
            if (
                common_words_ratio < 0.1 and len(prev_summary_words) > 5
            ):  # Moins de 10% de mots communs
        ...
        coherence_score += self.coherence_factors.get(
            "consistent_pronoun_referencing", 0.1
        )
        ...
        logger.info(
            f"MockCoherenceAnalyzer score calculé: {final_score:.2f}, facteurs: {factors}"
        )
        return {
