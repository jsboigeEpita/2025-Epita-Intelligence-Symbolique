...
- DI lazy : `get_jtms_service` :67, `get_session_manager` :75, `get_sk_plugin` :85 ; erreur centralisée `jtms_http_exception` :114 — construit le `JTMSError` et classifie le statut **par provenance** (#2344 famille c) : le type nommé `JTMSClientInputError` (sous-classe de `ValueError`, levé par le service quand l'entité demandée n'existe pas = entrée client) → 400, tout le reste (défaut serveur, ex. `NotImplementedError` export graphml, ou `ValueError` de forme serveur comme un `ValidationError`) → 500. Les 18 handlers partagent cette définition unique ;
