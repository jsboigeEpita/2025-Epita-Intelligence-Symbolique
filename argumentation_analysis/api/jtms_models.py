...
class BeliefInfo(BaseModel):
    ...
    valid: Optional[bool] = Field(None, description="Statut de validité (true/false/null)")
    non_monotonic: bool = Field(False, description="Indique si la croyance est non-monotone")
...
class CreateBeliefRequest(BaseModel):
    ...
    initial_value: Optional[str] = Field("unknown", description="Valeur initiale: 'true', 'false', ou 'unknown'")
    session_id: Optional[str] = Field(None, description="ID de session (optionnel)")
    instance_id: Optional[str] = Field(None, description="ID d'instance JTMS (optionnel)")
...
class QueryBeliefsRequest(BaseModel):
    ...
    filter_status: str = Field("all", description="Filtre: 'valid', 'invalid', 'unknown', 'non_monotonic', 'all'")
    session_id: Optional[str] = Field(None, description="ID de session (optionnel)")
    instance_id: Optional[str] = Field(None, description="ID d'instance JTMS (optionnel)")
...
class CreateSessionRequest(BaseModel):
    ...
    session_name: Optional[str] = Field(None, description="Nom descriptif de la session")
    metadata: Optional[Dict[str, Any]] = Field({}, description="Métadonnées additionnelles")
...
class AddJustificationResponse(JTMSResponse):
    ...
    justification: JustificationInfo = Field(..., description="Informations sur la justification ajoutée")
    conclusion_status: Optional[bool] = Field(None, description="Nouveau statut de la conclusion")
...
class ExplainBeliefResponse(JTMSResponse):
    ...
    justifications: List[JustificationInfo] = Field([], description="Liste des justifications")
    explanation_text: str = Field("", description="Explication textuelle")
    natural_language_summary: Optional[str] = Field(None, description="Résumé en langage naturel")
...
class GetJTMSStateResponse(JTMSResponse):
    ...
    justifications_graph: Optional[List[JustificationInfo]] = Field(None, description="Graphe de justifications")
    statistics: Optional[JTMSStatistics] = Field(None, description="Statistiques du système")
    session_info: Optional[SessionInfo] = Field(None, description="Informations de session")
    natural_language_summary: Optional[str] = Field(None, description="Résumé en langage naturel")
...
class SetBeliefValidityResponse(JTMSResponse):
    ...
    propagation_occurred: bool = Field(True, description="Indique si propagation effectuée")
...
class JTMSError(BaseModel):
    ...
    error_details: Optional[Dict[str, Any]] = Field(None, description="Détails additionnels")
...
class ImportJTMSResponse(BaseModel):
    ...
    justifications_imported: int = Field(0, description="Nombre de justifications importées")
...
class PluginStatusResponse(BaseModel):
    ...
    semantic_kernel_available: bool = Field(..., description="Semantic Kernel disponible")
    ...
    session_manager_active: bool = Field(..., description="Gestionnaire de sessions actif")
