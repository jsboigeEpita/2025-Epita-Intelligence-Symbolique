...
def initialize_pyo3() -> bool:
    ...
    try:
        ...
        logger.error(
            f"❌ Erreur lors de l'initialisation de l'environnement PyO3: {e}",
            exc_info=True,
        )
        return False
...
def get_pyo3_module(
    module_name: str, init_func: Optional[Callable] = None
) -> Optional[Any]:
    """
    ...
    if module_name in _pyo3_modules:
        logger.debug(
            f"Module PyO3 '{module_name}' déjà chargé, réutilisation de l'instance existante."
        )
    ...
    if not _initialized and not initialize_pyo3():
        logger.error(
            f"❌ Impossible de charger le module PyO3 '{module_name}' car l'environnement PyO3 n'est pas initialisé."
        )
        return None
    ...
    try:
        ...
        logger.error(
            f"❌ Erreur inattendue lors du chargement du module PyO3 '{module_name}': {e}",
            exc_info=True,
        )
        return None
...
def unload_pyo3_module(module_name: str) -> bool:
    ...
    if module_name not in _pyo3_modules:
        logger.warning(
            f"Module PyO3 '{module_name}' non chargé, impossible de le décharger."
        )
        return False
    ...
    try:
        ...
        logger.error(
            f"❌ Erreur lors du déchargement du module PyO3 '{module_name}': {e}",
            exc_info=True,
        )
        return False
...
def reset_pyo3_environment() -> bool:
    ...
    try:
        ...
        logger.error(
            f"❌ Erreur lors de la réinitialisation de l'environnement PyO3: {e}",
            exc_info=True,
        )
        return False
