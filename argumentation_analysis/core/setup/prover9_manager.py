...
class Prover9Manager:
    ...
    def get_prover9_executable_path(self, force_reinstall=False):
        ...
        executable_path_base = (
            install_dir
            / Path(executable_relative_path).parent
            / Path(executable_relative_path).stem
        )
        ...
        if not force_reinstall:
            ...
            if original_executable_path.exists():
                self.logger.warning(
                    f"Prover9 original ({original_executable_path.name}) trouvé sans son wrapper. Recréation du wrapper..."
                )
            ...
            if standard_executable_path.exists():
                self.logger.info(
                    f"Exécutable Prover9 standard trouvé à : {standard_executable_path}. Création du wrapper..."
                )
                return str(self._create_prover9_wrapper(standard_executable_path))

        if force_reinstall and install_dir.exists():
            self.logger.warning(
                f"Réinstallation forcée de {tool_name}, suppression de {install_dir}..."
            )
    ...
    def _setup_prover9_unix(self, config, install_dir):
        ...
        self.logger.info(
            "L'installation de Prover9 sur les systèmes non-Windows nécessite une compilation manuelle."
        )
        ...
        try:
            ...
            self.logger.error(
                f"Échec de la compilation de Prover9. 'make' doit être installé. Erreur : {e}"
            )
            self.logger.error(
                "Veuillez compiler et installer Prover9 manuellement et vous assurer qu'il est dans le PATH système."
            )
            return None
        ...
        try:
            ...
            self.logger.info(
                f"Binaires de Prover9 copiés et rendus exécutables dans {dest_bin_dir}"
            )
        except Exception as e:
    ...
    def _create_prover9_wrapper(self, original_executable_path: Path):
        ...
        if original_executable_path.exists() and not original_renamed_path.exists():
            self.logger.info(
                f"Renommage de {original_executable_path} en {original_renamed_path}"
            )
...
def _download_file(
    url, dest_folder, file_name, logger, log_interval_seconds=5, force_download=False
):
