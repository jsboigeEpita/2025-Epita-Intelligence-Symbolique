...
def _download_file_async(url, dest_folder, file_name, logger_instance=None):
    ...
    thread = threading.Thread(
        ...
        args=(url, dest_folder, file_name, logger_instance),
        daemon=True,
    )
    thread.start()
    return thread
...
def _find_extracted_directory(base_dir, dir_pattern, logger_instance=None):
    ...
    logger.warning(
        f"Aucun répertoire correspondant au motif '{dir_pattern}' trouvé dans {base_dir}"
    )
    return None
...
def _is_tool_installed(env_var_name, expected_home_path=None, logger_instance=None):
    ...
    if env_var_value and os.path.isdir(env_var_value):
        logger.info(
            f"L'outil est déjà configuré via la variable d'environnement {env_var_name}={env_var_value}"
        )
        return env_var_value

    if expected_home_path and os.path.isdir(expected_home_path):
        logger.info(
            f"L'outil est déjà présent dans le répertoire attendu : {expected_home_path}"
        )
...
def _setup_multi_file_tool(tool_config, tools_base_dir, logger, force_reinstall):
    ...
    logger.info(
        f"Vérification du JAR pour {tool_name} v{version} dans {tweety_lib_dir}"
    )

    if force_reinstall and tweety_lib_dir.exists():
        logger.warning(
            f"Réinstallation forcée de {tool_name}, suppression de {tweety_lib_dir}..."
        )
        try:
    ...
    if _download_file(
        jar_url, tweety_lib_dir, jar_filename, logger, force_download=force_reinstall
    ):
...
def setup_single_tool(
    ...
    if tool_config.get("is_multi_file_download"):
        return _setup_multi_file_tool(
            tool_config, tools_base_dir, logger, force_reinstall
        )
    ...
    if not url or not file_name:
        logger.warning(
            f"URL de téléchargement non définie pour {tool_name} sur {platform.system().lower()}. Installation sautée."
        )
        return None

    expected_tool_path = _find_extracted_directory(
        tools_base_dir, dir_pattern, logger_instance=logger
    )

    # Vérification si l'outil est déjà installé
    installed_path = _is_tool_installed(
        env_var, expected_home_path=expected_tool_path, logger_instance=logger
    )
    if installed_path and not force_reinstall:
        logger.info(
            f"{tool_name} déjà configuré. Pour réinstaller, utilisez --force-reinstall."
        )
        return installed_path

    if force_reinstall and installed_path:
        logger.warning(
            f"Réinstallation forcée de {tool_name} demandée. Suppression de {installed_path}..."
        )
        try:
    ...
    zip_path = _download_file(
        ...
        force_download=force_reinstall,
    )
    if not zip_path:
        return None
    ...
    if not expected_tool_path:
        logger.error(
            f"Impossible de trouver le répertoire de {tool_name} après extraction."
        )
        return None
...
def setup_tools(
    ...
    logger = _get_logger_tools(logger_instance)
    logger.debug(
        f"setup_tools called with: tools_dir_base_path={tools_dir_base_path}, force_reinstall={force_reinstall}, interactive={interactive}, skip_jdk={skip_jdk}, skip_octave={skip_octave}, skip_node={skip_node}, skip_tweety={skip_tweety}"
    )
    ...
    tools_to_process = []
    if not skip_jdk:
        tools_to_process.append(JDK_CONFIG)
    if not skip_octave:
        tools_to_process.append(OCTAVE_CONFIG)
    if not skip_node:
        tools_to_process.append(NODE_CONFIG)
    if not skip_tweety:
        logger.warning(
            "La gestion de Tweety via ce script est obsolète. Les JARs doivent être placés manuellement."
        )
    else:
        logger.info("Skipping Tweety setup as per request (obsolete).")

    for config in tools_to_process:
        tool_home = setup_single_tool(
            config,
            tools_dir_base_path,
            temp_download_dir,
            logger_instance=logger,
    ...
    if os.path.isdir(temp_download_dir):
        logger.info(
            f"Temporary download directory {temp_download_dir} can be cleaned up manually for now."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Gestionnaire d'installation d'outils portables (JDK, Octave, Node.js)."
    )
    parser.add_argument(
        "--tools-dir",
        default=os.path.join(os.getcwd(), "libs"),
        help="Répertoire de base pour les outils portables.",
    )
    parser.add_argument(
        "--force-reinstall",
        action="store_true",
        help="Force le re-téléchargement et la réinstallation même si l'outil est déjà présent.",
    )
    parser.add_argument(
        "--skip-jdk", action="store_true", help="Saute l'installation du JDK."
    )
    parser.add_argument(
        "--skip-octave", action="store_true", help="Saute l'installation d'Octave."
    )
    parser.add_argument(
        "--skip-node", action="store_true", help="Saute l'installation de Node.js."
    )
    parser.add_argument(
        "--skip-tweety",
        action="store_true",
        help="Option obsolète. L'installation de Tweety est toujours sautée.",
    )
    parser.add_argument(
        "--install-prover9", action="store_true", help="Installe Prover9."
    )
    ...
    if args.install_prover9:
        ...
        prover9_path = prover9_manager.get_prover9_executable_path(
            force_reinstall=args.force_reinstall
        )
        if prover9_path:
            main_logger.info(
                f"Prover9 installé avec succès. Exécutable : {prover9_path}"
            )
        else:
            main_logger.error("Échec de l'installation de Prover9.")

    main_logger.info(
        "--- DÉBUT de la configuration des outils portables via le script principal ---"
    )
