...
class UnifiedSourceConfig:
    ...
    def to_legacy_config(self) -> SourceConfig:
        ...
        legacy_type = (
            LegacySourceType(self.source_type.value)
            if self.source_type.value in ["simple", "complex"]
            else LegacySourceType.SIMPLE
        )
        return SourceConfig(
...
class UnifiedSourceManager:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        if self.config.anonymize_logs and self.config.source_type in [
    ...
    def _get_passphrase(self) -> str:
        ...
        if not self.config.interactive_mode:
            ...
            raise ValueError(
                "Aucune phrase secrète disponible dans la configuration (config/settings) et le mode non-interactif est activé."
            )

        try:
            passphrase = getpass.getpass(
                "Veuillez entrer la phrase secrète pour déchiffrer les sources : "
            )
            if not passphrase:
    ...
    def _load_enc_file_sources(self) -> Tuple[Optional[ExtractDefinitions], str]:
        ...
        try:
            ...
            if not definitions:
                return (
                    None,
                    "Impossible de charger les définitions depuis le fichier .enc",
                )
    ...
    def select_text_for_analysis(
        self, extract_definitions: Optional[ExtractDefinitions]
    ) -> Tuple[str, str]:
        """
        ...
        if (
            ...
            in [UnifiedSourceType.SIMPLE, UnifiedSourceType.COMPLEX]
            and self._legacy_manager
        ):
        ...
        for source in extract_definitions.sources:
            if hasattr(source, "full_text") and source.full_text:
                description = (
                    f"Source {self.config.source_type.value}: {source.source_name}"
                )
    ...
    def cleanup_sensitive_data(self):
        ...
        for temp_file in self._temp_files:
            try:
                ...
                self.logger.warning(
                    f"Erreur lors de la suppression de {temp_file}: {e}"
                )
...
def create_unified_source_manager(
    ...
    try:
        ...
        raise ValueError(
            f"Type de source non supporté: {source_type}. Utilisez: {supported_types}"
        )
    ...
    def _load_complex_source(self) -> Tuple[str, str, str]:
        ...
        with create_unified_source_manager(
            ...
            while True:
                try:
                    choice = input(
                        f"Sélectionnez une source (1-{len(extract_definitions.sources)}): "
                    ).strip()
                    ...
                    if 0 <= choice_idx < len(extract_definitions.sources):
                        ...
                        selected_definitions = ExtractDefinitions(
                            sources=[selected_source]
                        )
                        text, desc = manager.select_text_for_analysis(
                        ...
                        print(
                            f"❌ Choix invalide. Entrez un nombre entre 1 et {len(extract_definitions.sources)}."
                        )
                except ValueError:
