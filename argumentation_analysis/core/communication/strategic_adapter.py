...
class StrategicAdapter:
    ...
    def issue_directive(
        ...
        message = Message(
            ...
            content={"command_type": directive_type, "parameters": parameters},
        ...
        if success:
            self.logger.info(
                f"Directive {directive_type} issued to {recipient_id or 'all tactical agents'}"
            )
        else:
    ...
    def broadcast_objective(
        ...
        message = Message(
            ...
            channel=ChannelType.HIERARCHICAL.value,
            priority=priority,
            metadata={"broadcast": True, **(metadata or {})},
        )
    ...
    def receive_report(
        ...
        while True:
            ...
            if timeout is not None:
                ...
                if remaining_timeout <= 0:
                    self.logger.warning(
                        f"Timeout ({timeout}s) expired while waiting for report for {self.agent_id}."
                    )
                    return None
            ...
            if message:
                is_report_type = (
                    ...
                    and message.content.get("info_type") == "report"
                )
                ...
                if is_report_type:
                    if filter_criteria:
                        ...
                        if not match:
                            self.logger.debug(
                                f"Message {message.id} (report) received by {self.agent_id} but did not match filter_criteria. Continuing to wait."
                            )
                            continue

                    self.logger.info(
                        f"Report {message.id} (type: {message.content.get('report_type')}) received from {message.sender} by {self.agent_id} and matches criteria."
                    )
                    return message
                else:
                    self.logger.debug(
                        f"Ignored message {message.id} (type: {message.type}, info_type: {message.content.get('info_type')}) by {self.agent_id} while waiting for report."
                    )
            ...
            if timeout is None and not message:
                ...
                self.logger.warning(
                    f"Timeout ({timeout}s) confirmed for {self.agent_id} as current_timeout was <=0 before receive_message (waiting for report)."
                )
                return None
        ...
        self.logger.error(
            f"Exited receive_report loop unexpectedly for {self.agent_id}."
        )
        return None
    ...
    def request_tactical_info(
        ...
        try:
            ...
            self.logger.error(
                f"Error requesting information from {recipient_id}: {str(e)}"
            )
            return None
    ...
    def share_strategic_data(
        ...
        self.logger.info(
            f"Strategic data {data_type} shared with {len(recipient_ids) if recipient_ids else 'all strategic agents'}"
        )
        return data_id
    ...
    def allocate_resources(
        ...
        message = Message(
            ...
            content={
                ...
                "parameters": {"resource_type": resource_type, "amount": amount},
            },
            recipient=recipient_id,
            channel=ChannelType.HIERARCHICAL.value,
            priority=priority,
        ...
        if success:
            ...
            self.logger.error(
                f"Failed to allocate resources {resource_type} to {recipient_id}"
            )
    ...
    def receive_guidance_request(
        ...
        if message:
            ...
            is_guidance_request = (
                ...
                and message.sender_level == AgentLevel.TACTICAL
                and message.content.get("request_type") == "guidance"
            )
    ...
    def provide_guidance(
        ...
        message = Message(
            ...
            channel=ChannelType.HIERARCHICAL.value,
            priority=priority,
            metadata={"reply_to": request_id, **(metadata or {})},
        )
