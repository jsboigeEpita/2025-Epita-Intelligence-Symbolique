...
class MessageMiddleware:
    ...
    def determine_channel(self, message: Message) -> ChannelType:
        ...
        if message.channel:
            try:
                ...
                self.logger.warning(
                    f"Invalid channel specified in message: {message.channel}"
                )
            ...
            if channel_type:
                ...
                if message:
                    ...
                    if message.type == MessageType.RESPONSE and self.request_response:
                        ...
                        if request_id:
                            self.logger.info(
                                f"Received response {message.id} for request {request_id}"
                            )
    ...
    def _handle_message(self, message: Message) -> None:
        ...
        if message.type == MessageType.RESPONSE and self.request_response:
            ...
            if request_id:
                self.logger.info(
                    f"Handling response {message.id} for request {request_id}"
                )
                try:
                    result = self.request_response.handle_response(message)
                    self.logger.info(f"Response handler result: {result}")
    ...
    def _debug_message_handler(self, message: Message) -> None:
        ...
        if message.type == MessageType.RESPONSE:
            request_id = message.metadata.get("reply_to")
            self.logger.info(
                f"DEBUG: Received response {message.id} for request {request_id}"
            )
