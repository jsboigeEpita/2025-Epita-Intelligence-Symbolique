...
class Topic:
    ...
    def publish_message(self, message: Message) -> List[str]:
        ...
        with self.lock:
            ...
            self.messages.append({"message": message, "published_at": datetime.now()})
    ...
    def get_recent_messages(
        ...
        with self.lock:
            ...
            filtered_messages = [
                ...
                or self._matches_filter(entry["message"], filter_criteria)
            ]
    ...
    def _matches_filter(
        self, message: Message, filter_criteria: Optional[Dict[str, Any]]
    ) -> bool:
        """
                ...
                with self.lock:
                    for topic_id, topic in self.topics.items():
                        with topic.lock:
                            ...
                            topic.messages = [
                                entry
                                for entry in topic.messages
                                if not entry["message"].metadata.get("ttl")
                                or entry["published_at"]
                                + timedelta(seconds=entry["message"].metadata["ttl"])
                                > now
                            ]
