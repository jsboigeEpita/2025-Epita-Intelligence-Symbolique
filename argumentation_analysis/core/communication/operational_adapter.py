...
class OperationalAdapter:
    ...
    def send_result(
        ...
        message = Message(
            ...
            priority=priority,
            metadata={"reply_to": task_id, **(metadata or {})},
        )
        ...
        if success:
            ...
            self.logger.error(
                f"Failed to send result for task {task_id} to {recipient_id}"
            )
    ...
    def request_task_clarification(
        ...
        try:
            ...
            response = self.middleware.send_request(
                ...
                content={"task_id": task_id, "question": question, "context": context},
                timeout=timeout,
                priority=priority,
                channel=ChannelType.HIERARCHICAL.value,
            )
            ...
            self.logger.error(
                f"Error requesting clarification for task {task_id}: {str(e)}"
            )
            return None
    ...
    def share_analysis_data(
        ...
        self.logger.info(
            f"Analysis data {data_type} shared with {len(recipient_ids) if recipient_ids else 'all operational agents'}"
        )
        return data_id
    ...
    def request_assistance(
        ...
        try:
            ...
            self.logger.error(
                f"Error requesting assistance for issue {issue_type}: {str(e)}"
            )
            return None
    ...
    def send_progress_update(
        ...
        if success:
            self.logger.info(
                f"Progress update for task {task_id} sent to {recipient_id}"
            )
        else:
            self.logger.error(
                f"Failed to send progress update for task {task_id} to {recipient_id}"
            )
    ...
    def get_data(
        self, data_id: str, version_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
