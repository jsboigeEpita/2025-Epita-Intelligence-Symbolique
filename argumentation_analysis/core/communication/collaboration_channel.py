...
class CollaborationGroup:
    ...
    def __init__(self, group_id: str, name: Optional[str] = None, 
                description: Optional[str] = None, members: Optional[List[str]] = None):
        """
...
class CollaborationChannel(Channel):
    ...
    def _send_group_message(self, message: Message, group_id: str) -> bool:
        ...
        with self.lock:
            ...
            if message.sender not in group.members:
                self.logger.error(f"Sender {message.sender} is not a member of group {group_id}")
                return False
    ...
    def receive_message(self, recipient_id: str, timeout: Optional[float] = None) -> Optional[Message]:
        ...
        with self.lock:
            ...
            if recipient_id in self.direct_messages and self.direct_messages[recipient_id]:
                for i, entry in enumerate(self.direct_messages[recipient_id]):
                    if not entry["read"]:
                        ...
                        self.logger.info(f"Direct message {entry['message'].id} received by {recipient_id}")
                ...
                if recipient_id in group.members:
                    for entry in group.messages:
                        ...
                        if message.sender != recipient_id:
                            ...
                            self.logger.info(f"Group message {message.id} received by {recipient_id} from group {group_id}")
                            return message
    ...
    def create_group(self, group_id: Optional[str] = None, name: Optional[str] = None,
                   description: Optional[str] = None, members: Optional[List[str]] = None) -> str:
        """
        ...
        with self.lock:
            ...
            self.logger.info(f"Group {group_id} created with {len(group.members)} members")
            return group_id
    ...
    def get_group_messages(self, group_id: str, count: Optional[int] = None) -> List[Message]:
        """
