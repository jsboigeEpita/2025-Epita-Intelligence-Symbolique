...
class TacticalAdapter:
    ...
    def receive_directive(
        ...
        if message:
            ...
            is_directive = (
                ...
                and message.sender_level == AgentLevel.STRATEGIC
            )
    ...
    def receive_task_result(
        ...
        while True:
            ...
            if timeout is not None:
                ...
                if remaining_timeout <= 0:
                    self.logger.warning(
                        f"Timeout ({timeout}s) expired while waiting for task result for {self.agent_id}."
                    )
                    return None
            ...
            if message:
                is_result_type = (
                    ...
                    and message.sender_level == AgentLevel.OPERATIONAL
                    and message.content.get("info_type") == "task_result"
                )
                ...
                if is_result_type:
                    ...
                    if filter_criteria:
                        ...
                        ):  # S'assurer que c'est un dict pour la recherche par clé
                        ...
                        if not match:
                            self.logger.debug(
                                f"Message {message.id} (task_result) received by {self.agent_id} but did not match filter_criteria. Continuing to wait."
                            )
                            continue  # Ne correspond pas au filtre, continuer à chercher

                    self.logger.info(
                        f"Task result {message.id} received from {message.sender} by {self.agent_id} and matches criteria."
                    )
                    return message
                else:
                    self.logger.debug(
                        f"Ignored message {message.id} (type: {message.type}, info_type: {message.content.get('info_type')}) by {self.agent_id} while waiting for task_result."
                    )
            ...
            if (
                timeout is None and not message
            ):  # Si attente indéfinie et rien reçu, petit sleep pour éviter busy-loop sur des messages non pertinents
                time.sleep(0.01)
            elif (
                timeout is not None
                and not message
                and current_timeout is not None
                and current_timeout <= 0
            ):  # Vérifier si le current_timeout calculé était déjà <=0
                self.logger.warning(
                    f"Timeout ({timeout}s) confirmed for {self.agent_id} as current_timeout was <=0 before receive_message."
                )
                return None
            elif (
                timeout is not None and not message
            ):  # Si receive_message a timeout (current_timeout > 0 initialement)
        ...
        self.logger.error(
            f"Exited receive_task_result loop unexpectedly for {self.agent_id}."
        )
        return None
    ...
    def request_strategic_guidance(
        ...
        try:
            ...
            self.logger.error(
                f"Error requesting guidance from {recipient_id}: {str(e)}"
            )
            return None
    ...
    def collaborate_with_tactical(
        ...
        if group_id is None:
            ...
            group_id = self.middleware.get_channel(
        ...
        message = Message(
            ...
            priority=priority,
            metadata={"group_id": group_id, **(metadata or {})},
        )
        ...
        if success:
            ...
            self.logger.error(
                f"Failed to send collaboration message to group {group_id}"
            )
    ...
    def share_task_data(
        ...
        self.logger.info(
            f"Tactical data {data_type} shared with {len(recipient_ids) if recipient_ids else 'all tactical agents'}"
        )
        return data_id
    ...
    def subscribe_to_operational_updates(
        ...
        subscription_id = self.middleware.get_channel(
        ...
        self.logger.info(
            f"Subscribed to operational updates: {', '.join(update_types)}"
        )
    ...
    def send_status_update(
        ...
        if recipient_id:
            ...
            if success:
                ...
                self.logger.error(
                    f"Failed to send status update {update_type} to {recipient_id}"
                )
        else:
            ...
            self.logger.info(
                f"Status update {update_type} published to all strategic agents"
            )
