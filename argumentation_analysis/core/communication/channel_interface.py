...
class LocalChannel(Channel):
    ...
    def send_message(self, message: Message) -> bool:
        logger_channel.debug(
            f"Canal '{self.id}': Envoi du message {message.id} de {message.sender} à {message.recipient}"
        )
        ...
        for sub_id, sub_info in list(
            self.subscribers.items()
        ):  # list() pour permettre la désinscription pendant l'itération
            ...
            if self.matches_filter(message, filter_criteria or {}):
                if callback:
                    try:
                        logger_channel.debug(
                            f"Canal '{self.id}': Notification de l'abonné {sub_id} pour le message {message.id}"
                        )
                        callback(message)
                    except Exception as e:
                        logger_channel.error(
                            f"Canal '{self.id}': Erreur lors de l'appel du callback pour {sub_id}: {e}"
                        )
    ...
    def receive_message(
        ...
        for i, msg in enumerate(self._message_queue):
            if (
                msg.recipient == recipient_id or recipient_id == "*"
            ):  # "*" pour écouter tous les messages
                logger_channel.debug(
                    f"Canal '{self.id}': Message {msg.id} reçu par {recipient_id}"
                )
    ...
    def subscribe(
        ...
        logger_channel.info(
            f"Canal '{self.id}': Abonné {subscriber_id} enregistré avec ID {subscription_id}"
        )
    ...
    def unsubscribe(self, subscription_id: str) -> bool:
        if subscription_id in self.subscribers:
            ...
            logger_channel.info(
                f"Canal '{self.id}': Abonnement {subscription_id} supprimé"
            )
            return True
        logger_channel.warning(
            f"Canal '{self.id}': Tentative de désabonnement pour ID inexistant {subscription_id}"
        )
        return False
    ...
    def get_pending_messages(
        ...
        logger_channel.debug(
            f"Canal '{self.id}': {len(pending)} messages en attente récupérés par {recipient_id}"
        )
        return pending
