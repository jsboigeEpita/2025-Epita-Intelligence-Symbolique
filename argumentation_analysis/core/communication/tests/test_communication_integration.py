...
class TestCommunicationIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.hierarchical_channel = HierarchicalChannel(
            channel_id="hierarchical-test-channel", config={}
        )
    ...
    def test_simple_communication_integration(self):
        ...
        def agent2_responder_thread_func():
            ...
            self.assertIsNotNone(
                request_msg, "Agent 2 n'a pas reçu de message de requête"
            )
            if not request_msg:
                return
            ...
            actual_response_msg = request_msg.create_response(
                content=response_content, sender_level=AgentLevel.OPERATIONAL
            )
        ...
        def agent1_listener_func():
            ...
            while (
                not agent1_listener_stop_event.is_set()
                and not send_request_finished_event.is_set()
            ):
                self.middleware.receive_message(
                    recipient_id=self.agent1_id, timeout=0.1
                )
        ...
        agent1_main_thread.join(
            timeout=2.0
        )  # Devrait être déjà terminé si l'événement est mis

        self.assertTrue(
            send_request_finished_event.is_set(),
            "send_request n'a pas terminé ou a expiré.",
        )
        response_message = response_from_send_request_store.get("response")

        self.assertIsNotNone(
            response_message, "Agent 1 n'a pas reçu de réponse via send_request"
        )
        ...
        self.assertIn(
            "id",
            original_request_id_store,
            "L'ID de la requête originale n'a pas été stocké par agent2.",
        )
        self.assertTrue(
            response_message.is_response_to(original_request_id_store["id"]),
    ...
    def test_multiple_messages_integration(self):
        ...
        for i in range(num_messages):
            ...
            self.assertTrue(
                send_success, f"L'envoi du message {i} par l'Agent 1 a échoué"
            )
        ...
        indices = sorted(
            [m.content.get("index") for m in received_messages_agent2 if m.content]
        )
    ...
    def test_timeout_integration(self):
        ...
        self.assertIsNone(
            message, "Un message a été reçu alors qu'aucun n'était attendu."
        )
        ...
        self.assertGreaterEqual(
            elapsed_time,
            0.1,
            "Le délai de réception était plus court que le timeout spécifié.",
        )
        self.assertLess(
            elapsed_time,
            0.5,
            "Le délai de réception était significativement plus long que le timeout.",
        )
    ...
    def test_concurrent_communication_integration(self):
        ...
        def agent1_listener_func_concurrent():
            ...
            while (
                not agent1_listener_stop_event.is_set()
                and processed_responses_count < num_concurrent_messages
            ):
                msg = self.middleware.receive_message(
                    recipient_id=self.agent1_id, timeout=0.1
                )
                if msg and msg.type == MessageType.RESPONSE:
                    ...
                    all_agent1_sends_finished_event.is_set()
                    and processed_responses_count >= num_concurrent_messages
                ):
                    break  # Tous les sends sont finis et on a traité assez de réponses

        agent1_listener_thread_concurrent = threading.Thread(
            target=agent1_listener_func_concurrent
        )
        ...
        def agent2_concurrent_responder_func(index):
            ...
            if request_msg:
                response_content = {
                    "status": "received_concurrently",
                    "original_index": request_msg.content.get("index"),
                }
                actual_response_msg = request_msg.create_response(
                    content=response_content, sender_level=AgentLevel.OPERATIONAL
                )
        ...
        for i in range(num_concurrent_messages):
            thread = threading.Thread(
                target=agent2_concurrent_responder_func, args=(i,)
            )
        ...
        def agent1_send_request_concurrent_func(index):
            ...
            if response:
                ...
                logger.warning(
                    f"Agent1 n'a pas reçu de réponse pour l'index {index} dans send_request."
                )

        for i in range(num_concurrent_messages):
            thread = threading.Thread(
                target=agent1_send_request_concurrent_func, args=(i,)
            )
        ...
        received_indices = sorted(
            [r.content.get("original_index") for r in valid_responses if r.content]
        )
