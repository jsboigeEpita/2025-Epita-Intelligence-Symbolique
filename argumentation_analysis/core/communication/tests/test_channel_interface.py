...
class TestChannel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.channel = MockChannel(
            "test-channel", ChannelType.HIERARCHICAL, {"buffer_size": 100}
        )
    ...
    def test_matches_filter_message_type(self):
        ...
        self.assertTrue(
            self.channel.matches_filter(message, {"message_type": "command"})
        )
        self.assertTrue(
            self.channel.matches_filter(
                message, {"message_type": ["command", "request"]}
            )
        )

        # Non-correspondance avec le type de message
        self.assertFalse(
            self.channel.matches_filter(message, {"message_type": "information"})
        )
        self.assertFalse(
            self.channel.matches_filter(
                message, {"message_type": ["information", "response"]}
            )
        )
    ...
    def test_matches_filter_sender(self):
        ...
        self.assertTrue(
            self.channel.matches_filter(message, {"sender": ["agent-1", "agent-3"]})
        )

        # Non-correspondance avec l'expéditeur
        self.assertFalse(self.channel.matches_filter(message, {"sender": "agent-2"}))
        self.assertFalse(
            self.channel.matches_filter(message, {"sender": ["agent-2", "agent-3"]})
        )
    ...
    def test_matches_filter_priority(self):
        ...
        self.assertTrue(
            self.channel.matches_filter(message, {"priority": ["high", "critical"]})
        )

        # Non-correspondance avec la priorité
        self.assertFalse(self.channel.matches_filter(message, {"priority": "normal"}))
        self.assertFalse(
            self.channel.matches_filter(message, {"priority": ["normal", "low"]})
        )
    ...
    def test_matches_filter_sender_level(self):
        ...
        self.assertTrue(
            self.channel.matches_filter(message, {"sender_level": "strategic"})
        )
        self.assertTrue(
            self.channel.matches_filter(
                message, {"sender_level": ["strategic", "tactical"]}
            )
        )

        # Non-correspondance avec le niveau d'expéditeur
        self.assertFalse(
            self.channel.matches_filter(message, {"sender_level": "operational"})
        )
        self.assertFalse(
            self.channel.matches_filter(
                message, {"sender_level": ["operational", "system"]}
            )
        )
    ...
    def test_matches_filter_content(self):
        ...
        self.assertTrue(
            self.channel.matches_filter(message, {"content": {"command": "test"}})
        )
        self.assertTrue(
            self.channel.matches_filter(message, {"content": {"priority": "high"}})
        )

        # Non-correspondance avec le contenu
        self.assertFalse(
            self.channel.matches_filter(message, {"content": {"command": "other"}})
        )
        self.assertFalse(
            self.channel.matches_filter(message, {"content": {"priority": "low"}})
        )
        self.assertFalse(
            self.channel.matches_filter(message, {"content": {"unknown": "value"}})
        )
