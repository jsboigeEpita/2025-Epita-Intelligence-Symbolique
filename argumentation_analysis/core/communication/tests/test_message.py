...
class TestMessage(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            ...
            {"command_type": "analyze_text", "parameters": {"text_id": "text-123"}},
        )
    ...
    def test_from_dict(self):
        ...
        self.assertEqual(
            message.timestamp, datetime.fromisoformat("2025-05-19T12:00:00")
        )
    ...
    def test_create_response(self):
        ...
        )  # Le destinataire du message original
        self.assertEqual(response.sender_level, AgentLevel.TACTICAL)
        self.assertEqual(
            response.content, {"status": "success", "result": "analysis complete"}
        )
        self.assertEqual(
            response.recipient, "strategic-agent-1"
        )  # L'expéditeur du message original
    ...
    def test_create_acknowledgement(self):
        ...
        self.assertEqual(
            ack.content, {"status": "acknowledged", "message_id": self.message.id}
        )
        self.assertEqual(
            ack.recipient, "strategic-agent-1"
        )  # L'expéditeur du message original
...
class TestCommandMessage(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            command.content["parameters"], {"text_id": "text-123", "depth": "full"}
        )
        self.assertEqual(
            command.content["constraints"], {"max_time": 60, "max_resources": "medium"}
        )
...
class TestRequestMessage(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            request.content["description"], "Besoin d'une analyse détaillée du texte"
        )
        self.assertEqual(
            request.content["context"],
            {"text_id": "text-123", "previous_analysis": "basic"},
        )
...
class TestEventMessage(unittest.TestCase):
    ...
    def test_init(self):
        ...
        )  # Les événements sont généralement diffusés
        ...
        self.assertEqual(
            event.content["description"], "Utilisation élevée de la mémoire"
        )
        self.assertEqual(
            event.content["details"],
            {"memory_usage": 85, "threshold": 80, "process_id": 1234},
        )
        self.assertEqual(
            event.content["recommended_action"], "Libérer des ressources non utilisées"
        )
