...
def check_text_file_readable_and_not_empty(file_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not content.strip():
            return (
                False,
                f"Le fichier est vide ou ne contient que des espaces: {file_path}",
            )

        return (
            True,
            f"Fichier texte valide et non vide ({len(content)} caractères): {file_path}",
        )
...
def check_json_file_valid(file_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not data:  # Ceci est True pour un dict vide {} ou une liste vide []
            return (
                False,
                f"Fichier JSON valide mais structure de données vide: {file_path}",
            )

        return (
            True,
            f"JSON valide et données non vides (longueur str: {len(str(data))}): {file_path}",
        )
...
def check_markdown_file_readable_and_basic_structure(
    file_path: Path,
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        return (
            True,
            f"Fichier Markdown valide avec titres ({len(content)} caractères): {file_path}",
        )
    except (
        Exception
    ) as e:  # Devrait être déjà attrapé par check_text_file_readable_and_not_empty
        return (
            False,
            f"Erreur inattendue lors de la vérification de la structure Markdown de {file_path}: {str(e)}",
        )
