...
def run_shell_command(
    ...
    """
    if work_dir:
        logger.info(f"Exécution de la commande : '{command}' dans le répertoire '{work_dir}' avec un timeout de {timeout_seconds}s.")
    else:
        logger.info(f"Exécution de la commande : '{command}' avec un timeout de {timeout_seconds}s.")
    ...
    try:
        ...
        logger.info(f"Commande '{command}' terminée avec le code de retour : {return_code}.")
        if stdout_str:
        ...
        logger.error(f"La commande '{command}' a expiré après {timeout_seconds} secondes.")
        # e.stdout et e.stderr sont des bytes, il faut les décoder.
        stdout_str = e.stdout.decode('utf-8', errors='replace').strip() if e.stdout else ""
        stderr_str = e.stderr.decode('utf-8', errors='replace').strip() if e.stderr else ""
        ...
        logger.error(f"Erreur : La commande ou l'exécutable '{cmd_name}' n'a pas été trouvé.")
        ...
        logger.error(f"Une erreur inattendue est survenue lors de l'exécution de '{command}': {type(e).__name__} - {e}")
        ...
        stdout_str = stdout_bytes.decode('utf-8', errors='replace').strip() if isinstance(stdout_bytes, bytes) else ""
        stderr_str = stderr_bytes.decode('utf-8', errors='replace').strip() if isinstance(stderr_bytes, bytes) else str(e)
        
        logger.debug(f"Stdout (sur erreur inattendue):\n{stdout_str}")
        logger.debug(f"Stderr (sur erreur inattendue):\n{stderr_str}")
        return -11, stdout_str, stderr_str # Code de retour générique pour autres erreurs
...
if __name__ == '__main__':
    ...
    assert out == "Hello World", f"Test 1 Échoué: Sortie attendue 'Hello World', obtenue '{out}'"
    ...
    logger.info(f"Code retour: {ret_code}, Sortie: '{out}', Erreur: '{err}'")
    assert ret_code == -9, f"Test 3 Échoué: Code retour attendu -9 (timeout), obtenu {ret_code}"
    ...
    path_to_test_file_str = str(temp_dir / test_file_in_temp_dir).replace('\\', '/') # Pour la chaîne de commande
    ...
    if temp_dir.exists():
        try:
            ...
            logger.warning(f"N'a pas pu supprimer {temp_dir}: {e}. Nettoyage manuel peut être requis.")
    ...
    assert file_content == str(temp_dir.resolve()), \
    ...
    logger.info(f"Code retour: {ret_code}, Sortie: '{out}', Erreur: '{err}'")
    assert ret_code == -10, f"Test 5 Échoué: Code retour attendu -10 (non trouvée), obtenu {ret_code}"
    assert non_existent_cmd in err, f"Test 5 Échoué: Nom de commande attendu dans stderr, obtenu '{err}'"
