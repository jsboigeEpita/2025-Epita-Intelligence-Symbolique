...
from typing import (
    ...
    Optional,
    Union,
)  # Ajouté pour la complétude, même si pas directement utilisé par les fonctions ici
...
def sanitize_filename(filename: str, max_len: int = 255) -> str:
    ...
    if not filename:
        path_ops_logger.warning(
            "Tentative de nettoyer un nom de fichier vide. Retour de 'empty_filename'."
        )
    ...
    if filename == "." or filename == "..":
        path_ops_logger.warning(
            f"Nom de fichier caché invalide '{original_filename_for_log}'. Utilisation de '_hidden_default'."
        )
    ...
    is_hidden_file_original = filename.startswith(
        "."
    )  # Basé sur le nom original pour la logique des fichiers cachés
    ...
    if is_hidden_file_original:
        ...
        if dot_temp:  # Une extension a été trouvée après le premier point
            name_to_sanitize = (
                "." + name_part_temp
            )  # Reconstituer le nom caché avec le point initial
            current_extension = ext_part_temp
        else:  # Pas d'extension trouvée après le premier point
            name_to_sanitize = (
                "." + base_for_partition
            )  # Tout est considéré comme nom (avec le point initial)
            current_extension = ""
    ...
    base_name_part_to_clean = (
        name_to_sanitize[1:]
        if name_to_sanitize.startswith(".") and is_hidden_file_original
        else name_to_sanitize
    )

    cleaned_base_name_part = re.sub(r"[\s_.-]+", "_", base_name_part_to_clean)
    cleaned_base_name_part = re.sub(
        r"[^\w_]", "", cleaned_base_name_part
    )  # Garde les underscores
    cleaned_base_name_part = cleaned_base_name_part.lower()
    cleaned_base_name_part = re.sub(
        r"^_+|_+$", "", cleaned_base_name_part
    ...
    if (
        is_hidden_file_original and not cleaned_base_name_part
    ):  # Ex: "...", "._-.", ". "
        sanitized_name = ".default_name"
        path_ops_logger.warning(
            f"Le nom de base du fichier caché '{original_filename_for_log}' est devenu vide après nettoyage. Utilisation de '{sanitized_name}'."
        )
    elif (
        not is_hidden_file_original and not cleaned_base_name_part
    ):  # Ex: "!@#$", "   "
        sanitized_name = "default_filename"
        path_ops_logger.warning(
            f"Le nom de base du fichier '{original_filename_for_log}' est devenu vide après nettoyage. Utilisation de '{sanitized_name}'."
        )
    ...
    if current_extension:
        ...
        current_extension = (
            sanitized_cleaned_extension if sanitized_cleaned_extension else ""
        )
        ...
        if current_extension:  # Si une extension est présente
            ...
            if len_ext_plus_dot >= max_len:
                ...
                path_ops_logger.debug(
                    f"Extension trop longue pour max_len={max_len}. Troncature brutale de '{original_filename_for_log}' en '{final_filename}'."
                )
            else:
    ...
    if not final_filename:
        ...
        if is_hidden_file_original:
            path_ops_logger.error(
                f"La sanitization du nom de fichier caché '{original_filename_for_log}' a résulté en une chaîne vide. Retour de '.error_empty_filename'."
            )
            return ".error_empty_filename"
        else:
            path_ops_logger.error(
                f"La sanitization du nom de fichier '{original_filename_for_log}' a résulté en une chaîne vide. Retour de 'error_empty_filename'."
            )
            return "error_empty_filename"

    path_ops_logger.debug(
        f"Nom de fichier original: '{original_filename_for_log}', nettoyé: '{final_filename}'"
    )
...
def check_path_exists(path: Path, path_type: str = "file") -> bool:
    ...
    """
    path_ops_logger.debug(
        f"Vérification de l'existence et du type pour le chemin : {path} (attendu: {path_type})"
    )
    if not path.exists():
    ...
    if path_type == PATH_TYPE_FILE:  # Utilisation de la constante
        if not path.is_file():
            path_ops_logger.critical(
                f"❌ ERREUR CRITIQUE: Le chemin {path} existe mais n'est pas un fichier (attendu: fichier)."
            )
            sys.exit(1)
        ...
        if not path.is_dir():
            path_ops_logger.critical(
                f"❌ ERREUR CRITIQUE: Le chemin {path} existe mais n'est pas un répertoire (attendu: répertoire)."
            )
            sys.exit(1)
        path_ops_logger.info(f"[OK] Le répertoire {path} existe et est un répertoire.")
    else:
        path_ops_logger.critical(
            f"❌ ERREUR CRITIQUE: Type de chemin non supporté '{path_type}'. Utilisez '{PATH_TYPE_FILE}' ou '{PATH_TYPE_DIRECTORY}'."
        )
        sys.exit(1)
    return True
...
def create_archive_path(
    base_archive_dir: Path, source_file_path: Path, preserve_levels: int = 2
) -> Path:
    """
    ...
    if preserve_levels < 0:
        path_ops_logger.warning(
            "preserve_levels ne peut pas être négatif. Utilisation de 0 à la place."
        )
        preserve_levels = 0
    ...
    if preserve_levels > 0 and source_file_path.parents:
        ...
        actual_parents = [
            p for p in source_file_path.parents if p.name
        ]  # Exclut Path('.') qui a un nom vide
...
def archive_file(source_path: Path, archive_path: Path) -> bool:
    ...
    """
    path_ops_logger.info(
        f"Tentative d'archivage du fichier {source_path} vers {archive_path}"
    )

    if not source_path.exists() or not source_path.is_file():
        path_ops_logger.error(
            f"❌ Le fichier source {source_path} n'existe pas ou n'est pas un fichier."
        )
        return False
    ...
    try:
        ...
        path_ops_logger.debug(
            f"Répertoire parent {archive_path.parent} pour l'archive vérifié/créé."
        )
        shutil.move(str(source_path), str(archive_path))
        path_ops_logger.info(
            f"[OK] Fichier {source_path} archivé avec succès vers {archive_path}"
        )
        return True
    except FileNotFoundError:
        path_ops_logger.error(
            f"❌ Erreur (FileNotFoundError) lors de la tentative d'archivage : {source_path} non trouvé."
        )
        return False
    except PermissionError:
        path_ops_logger.error(
            f"❌ Erreur de permission lors de la tentative d'archivage de {source_path} vers {archive_path}."
        )
        return False
    except Exception as e:
        path_ops_logger.error(
            f"❌ Erreur inattendue lors de l'archivage du fichier {source_path} vers {archive_path}: {e}",
            exc_info=True,
        )
        return False
...
path_ops_logger.info(
    "Utilitaires d'opérations sur les chemins (PathOperations) définis."
)
