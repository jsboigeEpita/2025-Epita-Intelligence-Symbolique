    ...
    if patterns_string:
        ...
        for p_name in pattern_names:
            ...
            if clean_p_name:  # S'assurer que le nom n'est pas vide après strip
                ...
                pattern_to_compile = (
                    re.escape(clean_p_name) if escape_pattern else clean_p_name
                )
                try:
                    compiled_patterns[dict_key] = re.compile(pattern_to_compile)
                    logger.debug(
                        f"Motif '{clean_p_name}' compilé en regex pour la clé '{dict_key}'."
                    )
                except re.error as e_re:
                    logger.error(
                        f"Erreur de compilation regex pour le motif '{pattern_to_compile}' (original: '{clean_p_name}'): {e_re}"
                    )

    if not compiled_patterns and default_patterns:
        logger.info(
            "Aucun motif valide fourni ou parsé, utilisation des motifs par défaut."
        )
        for name, pattern_str in default_patterns.items():
            pattern_to_compile_default = (
                re.escape(pattern_str) if escape_pattern else pattern_str
            )
            try:
                compiled_patterns[name] = re.compile(pattern_to_compile_default)
                logger.debug(
                    f"Motif par défaut '{pattern_str}' compilé pour la clé '{name}'."
                )
            except re.error as e_re_default:
                logger.error(
                    f"Erreur de compilation regex pour le motif par défaut '{pattern_to_compile_default}' (original: '{pattern_str}'): {e_re_default}"
                )
        return compiled_patterns

    if not compiled_patterns and not default_patterns:
        logger.warning(
            "Aucun motif fourni et aucun motif par défaut. Retour d'un dictionnaire de regex vide."
        )
