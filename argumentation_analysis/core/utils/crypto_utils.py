...
import logging
import os
from typing import Optional, Tuple, Union  # MODIFIÉ: Ajout de Union
...
from cryptography.fernet import (
    Fernet,
    InvalidToken,
)  # NOUVEAU: Pour encrypt/decrypt_data
...
# Causes nommées d'un déchiffrement manqué (#2344 famille d, miroir de
# `CryptoService`) : le contrat `None` des fonctions publiques est conservé,
# et la cause voyage À CÔTÉ des données via `decrypt_data_with_fernet_detailed`
# — le chargeur de dataset peut ainsi la nommer dans son log et dans sa
# décision de dégradation au lieu de confondre absence de clé, clé mal formée
# et mauvais jeton.
ERR_NO_KEY = "no-key"
...
def derive_encryption_key(passphrase: str) -> Optional[bytes]:
    ...
    if not passphrase:
        logger.warning(
            "Tentative de dérivation de clé avec une phrase secrète vide. Retour de None."
        )
        return None
    try:
        kdf = PBKDF2HMAC(
...
def load_encryption_key(passphrase_arg: Optional[str] = None) -> Optional[bytes]:
    ...
    if passphrase_arg:
        logger.info(
            "Utilisation de la phrase secrète fournie en argument pour dériver la clé."
        )
    ...
    # Priorité 2: La configuration centrale
        ...
        logger.info(
            "Utilisation de la phrase secrète depuis la configuration centrale `settings`."
        )
    ...
    if passphrase_to_use:
        ...
        if encryption_key:
            ...
            logger.error(
                "Échec de la dérivation de la clé à partir de la phrase secrète obtenue."
            )
            return None

    logger.warning(
        "Aucune phrase secrète disponible (ni en argument, ni dans la configuration centrale)."
    )
    return None
...
def encrypt_data_with_fernet(
    data: bytes, b64_encoded_key_str: Union[str, bytes]
) -> Optional[bytes]:
    """
    ...
    try:
        ...
        if isinstance(b64_encoded_key_str, str):
            ...
            logger.error(
                f"Erreur chiffrement Fernet: Type de clé inattendu {type(b64_encoded_key_str)}."
            )
            return None
        f = Fernet(key_bytes)
...
def decrypt_data_with_fernet_detailed(
    encrypted_data: bytes, b64_encoded_key_str: Union[str, bytes]
) -> Tuple[Optional[bytes], Optional[str]]:
    """
    Déchiffre des données binaires avec une clé Fernet, en rendant la cause
    nommée d'un échec à côté des données (#2344 famille d).
    ...
    Returns:
        Tuple[Optional[bytes], Optional[str]]: (données déchiffrées, None) en cas
        de succès ; (None, cause) sinon, où cause vaut ERR_NO_KEY, ERR_INVALID_KEY,
        ERR_BAD_TOKEN ou ERR_UNEXPECTED.
    """
    if not b64_encoded_key_str:
        logger.error("Erreur déchiffrement Fernet: Clé (str b64 ou bytes) manquante.")
        return None, ERR_NO_KEY
    ...
    if isinstance(b64_encoded_key_str, str):
        ...
        logger.error(
            f"Erreur déchiffrement Fernet: Type de clé inattendu {type(b64_encoded_key_str)}."
        )
        return None, ERR_INVALID_KEY

    try:
        f = Fernet(key_bytes)
    except Exception as e:
        # Clé mal formée (Fernet exige 32 octets base64url) : c'est un défaut
        # de configuration, distinct d'un jeton corrompu ou d'une mauvaise
        # phrase secrète.
        logger.error(f"Erreur déchiffrement Fernet (clé invalide): {e}")
        return None, ERR_INVALID_KEY

    try:
        return f.decrypt(encrypted_data), None
...
def decrypt_data_with_fernet(
    ...
    """
    decrypted, _cause = decrypt_data_with_fernet_detailed(
        encrypted_data, b64_encoded_key_str
    )
    return decrypted
...
def derive_key_aes(
    passphrase: str, salt: bytes, key_length: int = 32, iterations: int = 480000
) -> bytes:
    """
    ...
    if not passphrase:
        ...
        raise ValueError(
            "La phrase secrète ne peut pas être vide pour la dérivation de clé AES."
        )
    if not salt:
    ...
    try:
        ...
        logger.debug(
            f"Clé AES dérivée avec succès (longueur: {len(derived_key)} bytes)."
        )
        return derived_key
    except Exception as e:
...
def encrypt_data_aesgcm(
    data_bytes: bytes, passphrase: str, salt_len: int = 16, nonce_len: int = 12
) -> Optional[bytes]:
    """
    ...
    try:
        ...
        logger.debug(
            f"Données chiffrées avec AESGCM (sel: {salt_len}B, nonce: {nonce_len}B, payload: {len(encrypted_payload)}B)."
        )
        ...
        logger.error(
            f"Erreur inattendue lors du chiffrement AESGCM: {e}", exc_info=True
        )
        return None


def decrypt_data_aesgcm(
    encrypted_data_with_prefix: bytes,
    passphrase: str,
    salt_len: int = 16,
    nonce_len: int = 12,
) -> Optional[bytes]:
    ...
    if not passphrase:
        ...
        return None
    if (
        not encrypted_data_with_prefix
        or len(encrypted_data_with_prefix) < salt_len + nonce_len
    ):
        logger.error(
            f"Erreur déchiffrement AESGCM: Données chiffrées trop courtes (longueur: {len(encrypted_data_with_prefix)}B)."
        )
        return None
    ...
    try:
        ...
        logger.error(
            f"Erreur déchiffrement AESGCM (Token/Signature/Tag invalide): {e}. Cela peut indiquer une mauvaise phrase secrète ou des données corrompues."
        )
        return None
    except ValueError as ve:
        ...
        logger.error(
            f"Erreur inattendue lors du déchiffrement AESGCM: {e}", exc_info=True
        )
        return None
    ...
    if derived_b64_key:
        ...
        try:
            ...
            if encrypted:
                ...
                if decrypted:
                    logger.info(
                        f"Données déchiffrées: {decrypted.decode('utf-8', 'ignore')}"
                    )
                    if decrypted == test_data:
                        logger.info("(OK) Chiffrement et déchiffrement Fernet réussis!")
                    else:
                        logger.error(
                            "(ERROR) Les données déchiffrées ne correspondent pas aux originales."
                        )
                else:
        ...
        logger.error(
            "(ERROR) Impossible de tester Fernet, la dérivation de clé a échoué."
        )
    ...
    if derived_b64_key:
        try:
            ...
            if bad_b64_key and bad_b64_key != derived_b64_key:
                logger.info(
                    "Tentative de déchiffrement avec une mauvaise clé Fernet..."
                )
                decrypted_with_bad_key = decrypt_data_with_fernet(
                    encrypted, bad_b64_key
                )
                if decrypted_with_bad_key is None:
                    logger.info(
                        "(OK) Échec du déchiffrement avec mauvaise clé, comme attendu (InvalidToken/Signature)."
                    )
                else:
                    logger.error(
                        f"(ERROR) Le déchiffrement avec une mauvaise clé aurait dû échouer, mais a retourné: {decrypted_with_bad_key}"
                    )
            else:
                logger.warning(
                    "Impossible de générer une mauvaise clé distincte pour le test."
                )
        except Exception as e_bad_key_test:
            logger.error(
                f"(ERROR) lors du test de mauvaise clé Fernet: {e_bad_key_test}",
                exc_info=True,
            )
