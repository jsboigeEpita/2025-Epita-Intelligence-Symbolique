...
def run_shell_command(
    ...
    try:
        ...
        if result.stdout:
            stdout_str = result.stdout.strip() if isinstance(result.stdout, str) else result.stdout.decode('utf-8', 'replace').strip()
            ...
            logger.debug(f"Stdout (aperçu):\n{stdout_str[:500]}{'...' if len(stdout_str) > 500 else ''}")

        if result.stderr:
            stderr_str = result.stderr.strip() if isinstance(result.stderr, str) else result.stderr.decode('utf-8', 'replace').strip()
        ...
        logger.error(f"❌ Erreur lors de l'exécution de '{description}': Commande ou exécutable non trouvé.")
        return -1, "", "Commande non trouvée" # Code de retour personnalisé pour cette erreur
    except Exception as e:
        logger.error(f"❌ Erreur inattendue lors de l'exécution de '{description}': {e}", exc_info=True)
