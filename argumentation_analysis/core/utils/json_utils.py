...
from typing import (
    ...
    Callable,
    Tuple,
)  # Ajout de Union, Optional, Callable et Tuple
...
def load_json_from_file(file_path: Path) -> Optional[Union[List[Any], Dict[str, Any]]]:
    ...
    try:
        ...
        logger.error(
            f"Erreur de lecture du fichier JSON {file_path}: {e}", exc_info=True
        )
        return None
...
def save_json_to_file(
    ...
    try:
        ...
        logger.debug(
            f"Données JSON sauvegardées avec succès dans : {file_path.resolve()}"
        )
        return True
    except (TypeError, IOError) as e:
        logger.error(
            f"Échec de la sauvegarde JSON dans {file_path}: {e}", exc_info=True
        )
        return False
    ...
    if isinstance(json_data, list) and list_path_key is None:
        ...
        filtered_list = [
            ...
            if not (
                ...
                and item.get(filter_key) == filter_value_to_remove
            )
        ]
        ...
        if isinstance(json_data[list_path_key], list):
            ...
            filtered_list_in_dict = [
                item
                for item in original_list_in_dict
                if not (
                    isinstance(item, dict)
                    and item.get(filter_key) == filter_value_to_remove
                )
            ]
            items_removed_count = len(original_list_in_dict) - len(
                filtered_list_in_dict
            )
            json_data[list_path_key] = filtered_list_in_dict  # Modifier la copie
            logger.info(
                f"{items_removed_count} éléments retirés de la liste sous la clé '{list_path_key}'."
            )
            return json_data, items_removed_count
        else:
            logger.warning(
                f"La clé '{list_path_key}' dans les données JSON ne contient pas une liste. Aucun filtrage effectué."
            )
            return json_data, 0

    else:
        logger.warning(
            f"Structure de données JSON non prise en charge pour le filtrage (type: {type(json_data)}, list_path_key: {list_path_key}). Aucun filtrage effectué."
        )
        return json_data, 0
