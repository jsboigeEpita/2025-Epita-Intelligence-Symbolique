# `argumentation_analysis/core/utils/` — un assembleur d'utilitaires transverses à trois régimes : feuilles réellement consommées, façade de ré-export explicite, et fossiles test-only

20 modules feuilles plus un `__init__.py` : **21 fichiers `.py`, 4191 lignes** à la racine du paquet (re-mesuré le 2026-09-23 ; la suite colocée `tests/` a été retirée le 2026-09-14, #2105). Mesure des importeurs **résolue par `ast`** (`argumentation_analysis.core.utils.<module>`, dédupliquée par `(fichier, ligne)`) : **100 sites d'import de production**, dont **50 pour `crypto_utils` seul** (44 d'entre eux dans `scripts/`) et **20 pour `network_utils`**. **11 modules** ont un import de production direct, **1** n'est atteint qu'en transitif via la façade `file_utils.py`, **8** n'ont aucun consommateur de production. Ce que le paquet fait que ses feuilles ne font pas : `__init__.py` importe 17 modules d'un coup (coût d'import lourd et effets de bord), et `file_utils.py` ré-exporte 13 noms de quatre feuilles en une façade unique que deux consommateurs lisent.
...
2. **Façade d'agrégation** — `file_utils.py` ne définit **aucune** fonction (`ast` sur le corps du module : 0 `def`), elle ré-exporte **13 noms nommés** de quatre feuilles (`file_utils.py:31-46` : `file_loaders`, `file_savers`, `markdown_utils`, `path_operations`) et les fixe dans `__all__` (`:48-62`). Jusqu'à la PR #2152 (issue #2146), c'étaient quatre star-imports sur des feuilles sans `__all__` : tout nom public fuyait dans la façade, y compris le paquet PyPI `markdown` importé par `markdown_utils.py:10`. La fuite est fermée et gardée (`tests/unit/argumentation_analysis/utils/core_utils/test_file_utils.py:42`, `assert not hasattr(facade, "markdown")`).
3. **Fossiles et stub** — `error_management.py` (157 l.) est auto-déclaré « simplifiée pour les tests » (`:7`, `:59`) et réussit toute récupération par simulation en dur (`time.sleep(0.001); return True`, `:85-86`, `:103-104`, `:119-120`). Le paquet héberge aussi un module dont le littéral docstring précède ses imports. (Le doublon interne `run_shell_command` est résolu, voir Limites, item 1.)

La frontière avec le **paquet frère `argumentation_analysis/utils/`** (37 entrées à la racine) n'est **pas étanche** : deux noms de modules existent des deux côtés (`path_operations`, `reporting_utils` ; le `system_utils` de ce paquet a été retiré en #2345), et deux implémentations sont vérifiées dupliquées (`ensure_directory_exists`, `generate_performance_visualizations` — voir Frères et parent). Un comptage par `grep` du nom nu confond donc les deux paquets ; **tous les chiffres de ce README sont résolus par import**, pas par nom de fichier.

## Composants publics

`__init__.py` pèse **939 octets** : 17 instructions `from . import` (`:3-19`) et 17 entrées `__all__` (`:21-39`). **Trois modules sur 20 ne sont pas exportés** — `code_manipulation_utils`, `error_management`, `llm_completion_guard` — importables seulement par chemin explicite.
...
| `file_utils` | `file_utils.py` (66) | **façade vivante**, 0 `def` ; 13 imports nommés `:31-46`, `__all__` `:48-62` (#2152) |
| `file_validation_utils` | `file_validation_utils.py` (125) | test-only ; `check_json_file_valid:51` |
| `filesystem_utils` | `filesystem_utils.py` (185) | test-only (son unique import de production, mort, a été retiré en #2152) ; `ensure_directory_exists:14`, `check_files_existence:73`, `get_all_files_in_directory:127` |
...
| `network_utils` | `network_utils.py` (447) | **vivant, 20 sites** ; `network_breaker:34`, `retry_on_network_error:40`, `download_file:80`, `get_resilient_async_client:323`, `build_async_openai_client:344` (constructeur unique des clients `AsyncOpenAI`, #2394 : 18 des 20 sites) |
...
| `shell_utils` | `shell_utils.py` (144) | vivant ; `run_shell_command:14`, **seule** définition du paquet (le `timeout` de l'ancien doublon `system_utils` y est porté, #2345) |
...
3. **Suite canonique** (`pytest.ini:2` `testpaths = tests`) :
   ...
   ```
   20 fichiers, **361 fonctions `test_`** (comptées par `ast`, re-mesurées le 2026-09-23). Plus le fichier isolé `tests/unit/argumentation_analysis/core/utils/test_llm_completion_guard_1929.py` (6 tests).
...
Mesure résolue par `ast` sur tout le dépôt, hors `libs/` (vendored), en distinguant **production** (`argumentation_analysis/`, `scripts/`, `project_core/`, `examples/`) et **`tests/`**. **100** sites de production au total (re-mesuré le 2026-09-23).

**Amont (dépendances externes chargées à l'import du paquet)** : `cryptography`, `httpx` (`network_utils.py`), `markdown` (`markdown_utils.py:10`), `unidecode` (`path_operations.py`), `matplotlib.pyplot` + `pandas` (`visualization_utils.py`). Comme `__init__.py:3-19` importe 17 modules, **importer `core.utils` — même pour un seul utilitaire — tire matplotlib et pandas**.
...
**Modules sans aucun consommateur de production — c'est un fait, pas un oubli** (chacun n'a qu'un importeur de `tests/`) :
...
| `filesystem_utils` | `tests/.../test_filesystem_utils.py:7` (l'import mort de production a été retiré en #2152) |
...
La façade ré-exporte aussi `save_json_file` et `save_text_file` (`file_utils.py:39`) et quatre noms de `path_operations` (`:41-46`), sans qu'aucun de ces noms soit consommé hors tests.

## Statut d'intégration

Verdict mesuré, en trois régimes nets — **20 modules se partitionnent exactement** (re-mesuré le 2026-09-23) :

- **Vivants (11)** — ≥1 import de production direct : `crypto_utils` (pivot de flotte, 50 sites, consommé par `core/` lui-même), `network_utils` (20 sites), `logging_utils`, `cli_utils`, `shell_utils`, `file_utils` (façade), `file_loaders`, `reporting_utils`, `llm_completion_guard`, `code_manipulation_utils`, `visualization_utils`. (`filesystem_utils` n'y figure plus : son seul import de production était mort, et #2152 l'a retiré.)
...
- **Inertes (8)** — 0 consommateur de production, direct ou transitif : `error_management` (stub), `file_savers`, `file_validation_utils`, `filesystem_utils`, `parsing_utils`, `path_operations`, `string_utils`, `text_utils`. (`json_utils` a été retiré en #2152, `system_utils` en #2345.)

Autrement dit : **12 modules sur 20 sont réellement atteignables en production**, dont **1 seulement via une façade de compatibilité**. Les modules inertes sont néanmoins couverts par la suite canonique (**361** fonctions `test_`, re-mesurées le 2026-09-23 par `ast`) — ils sont *testés* sans être *appelés*, ce qui est le mode de panne propre à ce paquet : **le vert des tests ne mesure pas l'intégration**.
...
- **Effet de bord à l'import** : la façade attache un `StreamHandler` au logger du module (`file_utils.py:14-24`) et émet un `logger.info` **à chaque import** (`:64-66`).
- **Aucun artefact de données ou de journal n'est produit par le paquet lui-même.**

## Tests représentatifs

- **Suite canonique** : `tests/unit/argumentation_analysis/utils/core_utils/` — **20 fichiers, 361 fonctions `test_`** (comptées par `ast`, re-mesurées le 2026-09-23). Plus gros contributeurs : `test_file_utils.py` (40), `test_error_management.py` (32), `test_path_operations.py` (32).
...
- **Ce que les tests mesurent** : majoritairement le **contrat des feuilles** en isolation (mock, `tmp_path`), pas le chemin réel d'intégration. Le cas d'école était `test_file_utils.py:335`, qui patchait `file_utils.markdown.markdown`, c'est-à-dire le **nom du paquet PyPI qui avait fui** dans la façade : le test épinglait un détail d'implémentation de la fuite, pas la façade comme contrat. Depuis #2152, le patch vise le site de résolution réel (`markdown_utils.markdown.markdown`, `:375`).
...
- **Couverture par fichier** : 20 fichiers de test dans ce dossier (re-mesuré le 2026-09-23) pour 20 modules ; `visualization_utils` reste sans aucune couverture (`grep` = 0) et `llm_completion_guard` est testé hors dossier.
...
- **Frère homonyme et collisions vérifiées** : `argumentation_analysis/utils/` — 39 entrées hors `__pycache__` (35 `.py` à la racine, `README.md`, et 3 sous-dossiers : `core_utils/` le shim, `dev_tools/`, `extract_repair/`). Deux noms de modules sont **identiques des deux côtés** (`path_operations.py`, `reporting_utils.py` ; le `system_utils.py` de ce paquet a été retiré en #2345), et deux implémentations sont **dupliquées mot pour mot dans leur rôle** :
...
Relevées le 2026-09-11 ; celles corrigées depuis sont barrées, avec leur référence :
...
1. ~~**Doublon fonctionnel intra-paquet**~~ — **résolu le 2026-09-23 (#2345)**. `run_shell_command` était défini deux fois : `shell_utils.py:14` (consommé, 5 sites) et `system_utils.py:30` (orphelin, 0 site), avec des contrats qui divergeaient. Le seul apport de l'orphelin, un `timeout` rendu en code -9 avec la sortie partielle, a été porté dans `shell_utils` ; l'orphelin a ensuite été retiré avec sa suite. Ses codes -10 et -11 n'avaient aucun lecteur : les consommateurs ne testent que `== 0` / `!= 0`. Le retrait supprime aussi un effet de bord : `system_utils.py:23` appelait `logging.basicConfig` à l'import du paquet, ce qui configurait le logger racine et rendait sans effet le `basicConfig` qu'une application appelait ensuite. Garde : `tests/.../test_one_run_shell_command_2345.py`.
2. ~~**Doublon JSON inter-modules**~~ — **résolu (#2152, `json_utils` retiré)**. `json_utils.load_json_from_file:22` / `save_json_to_file:61` font le travail de `file_loaders.load_json_file:23` / `file_savers.save_json_file:24` ; les premiers n'ont aucun consommateur, les seconds sont vivants.
3. ~~**Import mort présenté comme consommation**~~ — **résolu (#2152)**. `project_core/core_from_scripts/organization_manager.py:5` importe `filesystem_utils` sans jamais l'utiliser. Un comptage par grep classerait ce module « consommé en production ».
...
7. ~~**Façade à fuite de noms**~~ — **résolu (#2152)** : `file_utils.py:31-46` importe 13 noms nommés et les fixe dans `__all__` (`:48-62`), et `test_file_utils.py:42` garde l'absence de `markdown`. Relevé d'origine : `file_utils.py:42-45` : quatre star-imports sans `__all__` côté feuilles. Tout nom public des quatre feuilles entre dans l'espace de noms `file_utils`, y compris `markdown` (paquet PyPI) et les constantes `PATH_TYPE_*` de `path_operations`. Fait mesuré : `test_file_utils.py:335` patche `file_utils.markdown.markdown`.
8. **Effets de bord et coût d'import** — importer `core.utils` (ou n'importe quelle feuille via `__init__`) tire `matplotlib.pyplot`, `pandas`, `httpx`, `cryptography`, `unidecode` ; `file_utils.py:14-24` ajoute un handler de logging et `:64-66` journalise à chaque import.
...
12. **Modules non exportés** — `code_manipulation_utils`, `error_management`, `llm_completion_guard` ne figurent ni dans les `from . import` (`__init__.py:3-19`) ni dans `__all__` (`:21-39`) : atteignables seulement par chemin explicite, ce qui les rend invisibles à un inventaire dérivé de `dir(core.utils)`.
...
*Re-mesure : 2026-09-23 (#2345), même méthode (`ast`, `utf-8-sig`, dédoublonnage `(fichier, ligne)`, production = fichiers suivis hors `tests/` et `libs/`, aucun fichier sauté au parsing). Portée : modules, lignes, `__init__.py`, façade, partition, tableau aval, suite canonique. Elle intègre les corrections de la PR #2152 (façade explicite, `json_utils` retiré, import mort retiré) et la croissance de `network_utils` (#2394), que ce README ne reflétait pas.*
