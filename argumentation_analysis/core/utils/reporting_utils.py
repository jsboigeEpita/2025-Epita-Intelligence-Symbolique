...
def save_json_report(
    data: Union[List[Dict[str, Any]], Dict[str, Any]], output_file: Path
) -> bool:
    """
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la sauvegarde des données JSON vers {output_file}: {e}",
            exc_info=True,
        )
        return False
...
def generate_json_report(
    analysis_results: List[Dict[str, Any]], output_file: Path
) -> None:
    """
    ...
    """
    logger.info(
        f"Génération du rapport JSON (via save_json_report) vers {output_file}..."
    )
...
def save_text_report(report_content: str, output_file: Path) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la sauvegarde du rapport textuel vers {output_file}: {e}",
            exc_info=True,
        )
        return False


def generate_specific_rhetorical_markdown_report(
    analysis_results: List[Dict[str, Any]], output_file: Path
) -> None:
    """
    ...
    """
    logger.info(
        f"Génération du rapport Markdown rhétorique spécifique vers {output_file}..."
    )
    ...
    for result_item in analysis_results:
        ...
        if not analysis:
            report_content_parts.append(
                "Aucune analyse disponible pour cette source (données d'analyse vides).\n"
            )
            report_content_parts.append("\n---\n")
            continue
        ...
        if fallacies:
            for i, fallacy in enumerate(fallacies):
                ...
                report_content_parts.append(
                    f"- Contexte : `{context_text[:200]}{'...' if len(context_text) > 200 else ''}`\n"
                )
        else:
        ...
        if categories:
            ...
            for category, types in categories.items():
                if types:  # S'assurer que la liste des types n'est pas vide
                    report_content_parts.append(
                        f"- **{category.replace('_', ' ').title()}**: {', '.join(types)}\n"
                    )
                    has_content = True
            if not has_content:
                report_content_parts.append(
                    "Aucune catégorie de sophisme identifiée.\n"
                )
        else:
    ...
    if not save_text_report(final_report_content, output_file):
        logger.error(
            f"Échec de la sauvegarde du rapport Markdown rhétorique spécifique vers {output_file}"
        )
    # save_text_report logue déjà le succès ou l'erreur spécifique.


def generate_performance_comparison_markdown_report(
    base_metrics: Dict[str, Any], advanced_metrics: Dict[str, Any], output_file: Path
) -> None:
    """
    ...
    """
    logger.info(
        f"Génération du rapport de comparaison de performance Markdown vers {output_file}..."
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    from datetime import (
        datetime,
    )  # Import local pour éviter dépendance au niveau module si non utilisée ailleurs
    ...
    report_parts.append("|-------|----------------------------|")
    report_parts.append(
        f"| Agent contextuel de base | {base_metrics.get('fallacy_counts', {}).get('base_contextual', 0)} |"
    )
    report_parts.append(
        f"| Agent contextuel avancé | {advanced_metrics.get('fallacy_counts', {}).get('advanced_contextual', 0)} |"
    )
    report_parts.append(
        f"| Agent de sophismes complexes | {advanced_metrics.get('fallacy_counts', {}).get('advanced_complex', 0)} |"
    )
    report_parts.append("")
    ...
    report_parts.append("|-------|--------------------------|")
    report_parts.append(
        f"| Agent de cohérence de base | {base_metrics.get('confidence_scores', {}).get('base_coherence', 0.0):.2f} |"
    )
    report_parts.append(
        f"| Agent rhétorique avancé | {advanced_metrics.get('confidence_scores', {}).get('advanced_rhetorical', 0.0):.2f} |"
    )
    report_parts.append(
        f"| Agent de cohérence avancé | {advanced_metrics.get('confidence_scores', {}).get('advanced_coherence', 0.0):.2f} |"
    )
    report_parts.append(
        f"| Agent d'évaluation de gravité | {advanced_metrics.get('confidence_scores', {}).get('advanced_severity', 0.0):.2f} |"
    )
    report_parts.append("")
    ...
    report_parts.append("|-------|-------------------------------------|")
    report_parts.append(
        f"| Agent contextuel de base | {base_metrics.get('richness_scores', {}).get('base_contextual', 0.0):.2f} |"
    )
    report_parts.append(
        f"| Agent contextuel avancé | {advanced_metrics.get('richness_scores', {}).get('advanced_contextual', 0.0):.2f} |"
    )
    report_parts.append(
        f"| Agent rhétorique avancé | {advanced_metrics.get('richness_scores', {}).get('advanced_rhetorical', 0.0):.2f} |"
    )
    report_parts.append("")
    ...
    base_fallacy_count = base_metrics.get("fallacy_counts", {}).get(
        "base_contextual", 0
    )
    advanced_contextual_fallacy_count = advanced_metrics.get("fallacy_counts", {}).get(
        "advanced_contextual", 0
    )
    advanced_complex_fallacy_count = advanced_metrics.get("fallacy_counts", {}).get(
        "advanced_complex", 0
    )

    if (
        advanced_complex_fallacy_count > base_fallacy_count
        and advanced_complex_fallacy_count > advanced_contextual_fallacy_count
    ):
        report_parts.append(
            "L'agent de sophismes complexes détecte le plus grand nombre de sophismes, suggérant une analyse plus approfondie."
        )
    elif advanced_contextual_fallacy_count > base_fallacy_count:
        report_parts.append(
            "L'agent contextuel avancé détecte plus de sophismes que l'agent contextuel de base, suggérant une meilleure précision."
        )
    else:
        report_parts.append(
            "L'agent contextuel de base détecte un nombre comparable de sophismes aux agents avancés, suggérant une bonne efficacité pour sa catégorie."
        )
    report_parts.append("")

    base_richness = base_metrics.get("richness_scores", {}).get("base_contextual", 0.0)
    advanced_contextual_richness = advanced_metrics.get("richness_scores", {}).get(
        "advanced_contextual", 0.0
    )
    advanced_rhetorical_richness = advanced_metrics.get("richness_scores", {}).get(
        "advanced_rhetorical", 0.0
    )

    if (
        advanced_rhetorical_richness > advanced_contextual_richness
        and advanced_rhetorical_richness > base_richness
    ):
        report_parts.append(
            "L'agent rhétorique avancé fournit l'analyse contextuelle la plus riche."
        )
    elif advanced_contextual_richness > base_richness:
        report_parts.append(
            "L'agent contextuel avancé fournit une analyse contextuelle plus riche que l'agent contextuel de base."
        )
    else:
        report_parts.append(
            "L'agent contextuel de base fournit une analyse contextuelle comparable aux agents avancés pour sa catégorie."
        )
    report_parts.append("")
    ...
    report_parts.append(
        "- **Analyse rapide et basique**: Agents de base (ContextualFallacyDetector, etc.)."
    )
    report_parts.append(
        "- **Analyse approfondie**: Agents avancés (EnhancedComplexFallacyAnalyzer, etc.)."
    )
    report_parts.append(
        "- **Détection de sophismes**: EnhancedComplexFallacyAnalyzer pour sophismes composites."
    )
    report_parts.append(
        "- **Analyse contextuelle**: EnhancedRhetoricalResultAnalyzer pour richesse contextuelle."
    )
    report_parts.append("")
    ...
    report_parts.append("")
    report_parts.append(
        "Les agents avancés offrent généralement une analyse plus détaillée et riche. Le choix dépend des besoins spécifiques de l'analyse (rapidité vs profondeur)."
    )

    final_report_content = "\n".join(report_parts)
    if not save_text_report(final_report_content, output_file):
        logger.error(
            f"Échec de la sauvegarde du rapport de comparaison de performance vers {output_file}"
        )
    # save_text_report logue déjà le succès ou l'erreur spécifique.


def generate_markdown_report_for_corpus(
    corpus_name: str, corpus_effectiveness_data: Dict[str, Any]
) -> List[str]:
    """
    ...
    if not isinstance(corpus_name, str):
        raise TypeError(
            "Le paramètre 'corpus_name' doit être une chaîne de caractères."
        )
    if not isinstance(corpus_effectiveness_data, dict):
        ...
        logger.warning(
            "La structure de 'corpus_effectiveness_data' pourrait ne pas être conforme."
        )
    ...
    if base_agents_data:
        ...
        for agent, metrics in base_agents_data.items():
            ...
            report_section_lines.append(
                f"| {agent} | {fallacy_count} | {effectiveness_score:.2f} |"
            )
...
def generate_overall_summary_markdown(
    ...
    if not all_average_scores:
        report_lines.append(
            "Aucune donnée de score moyen disponible pour générer le résumé global."
        )
        report_lines.append("")
        return report_lines
    ...
    header = (
        "| Corpus | "
        + " | ".join([name.replace("_", " ").title() for name in sorted_metric_names])
        + " |"
    )
    separator = (
        "|--------|-"
        + "-|-".join(
            ["-" * len(name.replace("_", " ").title()) for name in sorted_metric_names]
        )
        + "-|"
    )
    ...
    for corpus_name, scores in sorted(all_average_scores.items()):
        ...
        for metric_name in sorted_metric_names:
            score = scores.get(
                metric_name, 0.0
            )  # Mettre 0.0 si la métrique manque pour un corpus
