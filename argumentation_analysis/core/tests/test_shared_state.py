...
from argumentation_analysis.core.shared_state import RhetoricalAnalysisState, SharedState
...
class TestRhetoricalAnalysisState(unittest.TestCase):
    ...
    def test_add_fallacy(self):
        ...
        self.assertEqual(self.state.identified_fallacies[fallacy_id1]["type"], fallacy_type)
        self.assertEqual(self.state.identified_fallacies[fallacy_id1]["justification"], justification)
        self.assertNotIn("target_argument_id", self.state.identified_fallacies[fallacy_id1])
        ...
        self.assertEqual(self.state.identified_fallacies[fallacy_id2]["target_argument_id"], arg_id)
        
        # Test avec un argument cible inexistant
        with patch('logging.Logger.warning') as mock_warning:
            fallacy_id3 = self.state.add_fallacy(fallacy_type, justification, "arg_inexistant")
    ...
    def test_add_answer(self):
        ...
        with patch('logging.Logger.warning') as mock_warning:
            self.state.add_answer("task_inexistante", author_agent, answer_text, source_ids)
            mock_warning.assert_called_once()
    ...
    def test_from_dict(self):
        ...
        state_dict = {
            ...
            "identified_fallacies": {"fallacy_1": {"type": "ad_hominem", "justification": "Justification"}},
            "belief_sets": {"bs_1": {"logic_type": "Propositional", "content": "A -> B"}},
            "query_log": [{"log_id": "qlog_1", "belief_set_id": "bs_1", "query": "A -> B?", "raw_result": "True"}],
            "answers": {"task_1": {"author_agent": "agent_1", "answer_text": "Réponse", "source_ids": ["arg_1"]}},
            "extracts": [{"id": "extract_1", "name": "Extrait 1", "content": "Contenu"}],
            "errors": [{"id": "error_1", "agent_name": "agent_1", "message": "Erreur", "timestamp": None}],
