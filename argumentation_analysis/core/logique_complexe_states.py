...
class EinsteinsRiddleState(BaseWorkflowState):
    ...
    def executer_requete_logique(self, requete: str) -> Dict[str, Any]:
        ...
        self.requetes_executees.append(
            {"requete": requete, "etape": len(self.requetes_executees) + 1}
        )
        ...
        if "existe(" in requete.lower():
            ...
            return {
                "type": "universel",
                "resultat": False,
                "contre_exemple": "Position 2",
            }
        else:
            return {
                "type": "satisfiabilite",
                "resultat": True,
                "modele": "Modèle trouvé",
            }
    ...
    def verifier_progression_logique(self) -> Dict[str, Any]:
        ...
        progression = {
            ...
            and len(self.requetes_executees) >= 5,
        }
        return progression

    def proposer_solution_complexe(
        self, solution: Dict[int, Dict[str, str]]
    ) -> Dict[str, Any]:
        """
...
class LogiqueBridgeState(BaseWorkflowState):
    ...
    def etat_objectif(self) -> bool:
        ...
        return (
            self.cannibales_gauche == 0
            and self.missionnaires_gauche == 0
            and self.cannibales_droite == 5
            and self.missionnaires_droite == 5
        )
    ...
    def generer_actions_possibles(self) -> List[Tuple[int, int]]:
        ...
        for c in range(self.capacite_bateau + 1):
            for m in range(self.capacite_bateau + 1 - c):
                if (
                    1 <= c + m <= self.capacite_bateau
                ):  # Au moins 1 personne dans le bateau
                    actions.append((c, m))
        return actions
