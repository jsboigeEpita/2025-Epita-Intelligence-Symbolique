...
class GroupChatOrchestration:
    ...
    def coordinate_analysis(
        self, text: str, target_agents: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"Coordination d'analyse collaborative avec {len(target_agents)} agents"
        )
    ...
    def _consolidate_results(
        ...
        if individual_results:
            total_confidence = sum(
                result.get("confidence", 0) for result in individual_results.values()
            )
            consolidated["average_confidence"] = total_confidence / len(
                individual_results
            )
    ...
    def coordinate_analysis_async(
        ...
        self.logger.info(
            f"Coordination d'analyse asynchrone avec {len(target_agents)} agents"
        )
    ...
    def _analyze_with_agent(self, agent_id: str, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if agent_id not in self._agent_communication_locks:
                self._agent_communication_locks[agent_id] = (
                    asyncio.Lock()
                    if asyncio.iscoroutinefunction(getattr(agent, "analyze_text", None))
                    else None
                )
            ...
            analysis_result = {
                ...
                "agent_capabilities": getattr(
                    agent, "get_agent_capabilities", lambda: {}
                )(),
            }
    ...
    def _consolidate_results_robust(
        self, individual_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if valid_results:
            ...
            confidences = [
                result.get("confidence", 0) for result in valid_results.values()
            ]
            ...
            weights = {
                "formal_logic": 1.2,
                "informal_analysis": 1.0,
                "extraction": 0.8,
                "general_analysis": 0.9,
            }
            weighted_sum = 0
            total_weight = 0
            ...
            for finding_obj in all_findings:
                ...
                key_words = set(
                    finding_text.lower().split()[:3]
                )  # Premiers mots comme clé
        ...
        try:
            ...
            for agent_id, agent in self.active_agents.items():
                try:
                    ...
                    agents_health[agent_id] = {"status": "unhealthy", "error": str(e)}
            ...
            healthy_agents = len(
                [a for a in agents_health.values() if a.get("status") == "healthy"]
            )
