...
class Orchestrator:
    ...
    async def run_analysis_async(self, text_content: str):
        ...
        try:
            ...
            logger.info(
                "Agents created: %s, %s", fallacy_agent.name, manager_agent.name
            )
        ...
        chat_history.add_message(
            message=ChatMessageContent(role=AuthorRole.USER, content=text_content)
        )

        # 3. Create and configure Agent Group Chat
        chat = AgentGroupChat(
            agents=[manager_agent, fallacy_agent], chat_history=chat_history
        )
        ...
        for message in history:
            print(
                f"[{message.role.value.upper()}] - {message.name or 'User'}:\n---\n{message.content}\n---"
            )
