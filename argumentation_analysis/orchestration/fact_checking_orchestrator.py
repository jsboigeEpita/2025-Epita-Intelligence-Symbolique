...
class FactCheckingOrchestrator:
    ...
    def __init__(
        ...
        if not self.taxonomy_plugin or not self.verification_plugin:
            raise ValueError(
                "Les plugins 'taxonomy_explorer' et 'external_verification' sont requis."
            )

        self.family_analyzer = get_family_analyzer(
            taxonomy_plugin=self.taxonomy_plugin, api_config=api_config
        )
    ...
    async def analyze_with_fact_checking(
        self, request: FactCheckingRequest
        ...
        try:
            ...
            comprehensive_result = await self.family_analyzer.analyze_comprehensive(
                text=request.text, depth=request.analysis_depth
            )
    ...
    async def quick_fact_check(self, text: str, max_claims: int = 5) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"Fact-checking rapide pour un texte de {len(text)} caractères"
            )
            ...
            verification_results = await self.verification_plugin.verify_claims(
    ...
    async def analyze_fallacy_families_only(
        ...
        try:
            ...
            family_stats = await self.taxonomy_plugin.get_family_statistics(
    ...
    async def batch_analyze(
        self, texts: List[str], analysis_depth: AnalysisDepth = AnalysisDepth.STANDARD
    ) -> List[FactCheckingResponse]:
        """
        ...
        requests = [
            FactCheckingRequest(
                text=text, analysis_depth=analysis_depth, api_config=self.api_config
            )
            for text in texts
        ]
...
def get_fact_checking_orchestrator(
    ...
    if _global_fact_checking_orchestrator is None:
        _global_fact_checking_orchestrator = FactCheckingOrchestrator(
...
async def quick_analyze_text(
    text: str, api_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    request = FactCheckingRequest(
        text=text, analysis_depth=AnalysisDepth.STANDARD, api_config=api_config
    )
...
async def quick_fact_check_only(
    text: str, api_config: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
