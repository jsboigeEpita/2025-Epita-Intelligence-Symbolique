...
async def test_execute_logic_agent_with_chat_history():
    ...
    mock_logic_agent.invoke_single = AsyncMock(return_value=(mock_belief_set, "Processed with history"))
    mock_logic_agent.text_to_belief_set = AsyncMock()

    mock_kernel = MagicMock()
    
    with patch('argumentation_analysis.orchestration.operational.direct_executor.logger') as mock_logger:
        with patch.object(DirectOperationalExecutor, '__init__', lambda s, kernel: None):
        ...
        # Assert
        mock_logic_agent.invoke_single.assert_awaited_once_with(chat_history=chat_history)
        ...
        mock_logger.info.assert_any_call("Utilisation de l'historique de conversation pour l'analyse logique.")