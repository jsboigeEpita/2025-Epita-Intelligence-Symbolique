...
class TacticalState:
    ...
    def __init__(self):
        ...
        self.task_progress: Dict[str, float] = {}  # task_id -> completion_rate (0.0 to 1.0)
    ...
    def add_rhetorical_analysis_result(self, task_id: str, result_type: str, result: Any) -> bool:
        """
    ...
    def get_rhetorical_analysis_result(self, result_type: str, task_id: Optional[str] = None) -> Any:
        """
    ...
    def update_agent_utilization(self, agent_id: str, utilization: float) -> None:
        ...
        """
        self.tactical_metrics["agent_utilization"][agent_id] = max(0.0, min(1.0, utilization))
    ...
    def _update_conflict_resolution_rate(self) -> None:
        ...
        resolved_conflicts = sum(1 for conflict in self.identified_conflicts 
                                if conflict.get("resolved", False))
        
        self.tactical_metrics["conflict_resolution_rate"] = resolved_conflicts / len(self.identified_conflicts)
    ...
    def get_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tasks": {
                status: len(tasks) for status, tasks in self.tasks.items()
            },
            "task_assignments_count": len(self.task_assignments),
            "conflicts": {
                "total": len(self.identified_conflicts),
                "resolved": sum(1 for conflict in self.identified_conflicts 
                               if conflict.get("resolved", False))
            },
    ...
    def to_json(self) -> str:
        ...
        state_dict = {
            ...
            "intermediate_results": {k: str(v) if not isinstance(v, (dict, list, str, int, float, bool, type(None)))
                                    else v for k, v in self.intermediate_results.items()},
    ...
    def from_json(cls, json_str: str) -> 'TacticalState':
        ...
        if "rhetorical_analysis_results" in state_dict:
            state.rhetorical_analysis_results = state_dict["rhetorical_analysis_results"]
