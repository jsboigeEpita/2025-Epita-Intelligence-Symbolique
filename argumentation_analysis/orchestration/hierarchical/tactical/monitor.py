...
class ProgressMonitor:
    ...
    def update_task_progress(self, task_id: str, progress: float) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Mise à jour de la progression de la tâche {task_id}: {progress:.2f}"
        )
    ...
    def _check_task_anomalies(
        self, task_id: str, previous_progress: float, current_progress: float
    ) -> List[Dict[str, Any]]:
        """
        ...
        if (
            ...
            < self.thresholds["progress_stagnation"]
            and current_progress < 0.9
        ):
            anomalies.append(
                {
                    "type": "stagnation",
    ...
    def generate_progress_report(self) -> Dict[str, Any]:
        ...
        for objective in self.state.assigned_objectives:
            ...
            for status, tasks in self.state.tasks.items():
                for task in tasks:
                    if task.get("objective_id") == obj_id:
                        ...
                        task_with_status["progress"] = self.state.task_progress.get(
                            task["id"], 0.0
                        )
            ...
            obj_completed_tasks = sum(
                1 for t in obj_tasks if t["status"] == "completed"
            )
            obj_progress = 0.0
                ...
                issues.append(
                    {
                        ...
                        "description": conflict.get(
                            "description", "Conflit non résolu"
                        ),
    ...
    def detect_critical_issues(self) -> List[Dict[str, Any]]:
        ...
        if failed_task_ids:
            self.logger.debug(
                f"Détection de tâches bloquées. failed_task_ids: {failed_task_ids}"
            )
            # Vérifier les tâches "pending" ET "in_progress" pour les blocages
            tasks_to_check = (
                self.state.tasks["pending"] + self.state.tasks["in_progress"]
            )
            self.logger.debug(
                f"Nombre de tâches à vérifier pour blocage: {len(tasks_to_check)}"
            )
            for task in tasks_to_check:
                ...
                for status_key, task_list_for_status in self.state.tasks.items():
                    if any(t["id"] == task_id for t in task_list_for_status):
                        ...
                        break
                self.logger.debug(
                    f"Vérification blocage pour task_id: {task_id} (description: '{task.get('description', 'N/A')}', status: {current_task_status})"
                )
                ...
                self.logger.debug(
                    f"Dépendances de {task_id}: {current_task_dependencies}"
                )
                ...
                for failed_id in failed_task_ids:
                    ...
                    if failed_id in current_task_dependencies:
                        self.logger.debug(
                            f"Tâche {task_id} dépend de la tâche échouée {failed_id}."
                        )
                        blocked_by_failed.append(failed_id)

                self.logger.debug(
                    f"Pour {task_id}, blocked_by_failed après vérification des dépendances directes: {blocked_by_failed}"
                )
        ...
        for task in self.state.tasks["in_progress"]:
            ...
            progress = self.state.task_progress.get(task_id, 0.0)
            self.logger.debug(
                f"Vérification retard pour task_id: {task_id} (description: '{task.get('description', 'N/A')}', progress: {progress:.2f})"
            )

            start_time_str = task.get("start_time")
            estimated_duration_sec = task.get("estimated_duration")
            self.logger.debug(
                f"Task {task_id}: start_time_str='{start_time_str}', estimated_duration_sec={estimated_duration_sec}"
            )

            if (
                start_time_str
                and isinstance(estimated_duration_sec, (int, float))
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Task {task_id}: start_time_dt={start_time_dt}, elapsed_time_sec={elapsed_time_sec:.2f}s"
                    )

                    # Condition de retard: si le temps écoulé dépasse la durée estimée
                    # ET la progression n'est pas presque terminée (ex: < 90%).
                    is_overdue = (
                        elapsed_time_sec >= estimated_duration_sec
                    )  # Correction: >= au lieu de >
                    self.logger.debug(
                        f"Task {task_id}: is_overdue={is_overdue} (elapsed: {elapsed_time_sec:.2f}s >= estimated: {estimated_duration_sec}s)"
                    )  # Log mis à jour

                    if is_overdue and progress < 0.9:
                        self.logger.info(
                            f"Tâche {task_id} détectée comme EN RETARD. Progress: {progress}, Elapsed: {elapsed_time_sec:.0f}s, Estimated: {estimated_duration_sec}s"
                        )
                        critical_issues.append(
                            {
                                "type": "delayed_task",
                    ...
                    self.logger.warning(
                        f"Format de date invalide pour start_time de la tâche {task_id}: {start_time_str}"
                    )
            elif (
                progress < 0.1 and task.get("status") == "in_progress"
            ):  # Fallback: Stagnation si pas de dates/durées valides
                self.logger.info(
                    f"Tâche {task_id} détectée comme POTENTIELLEMENT BLOQUÉE/EN RETARD (fallback). Progress: {progress}"
                )
                critical_issues.append(
                    {
                ...
                self.logger.debug(
                    f"Task {task_id} non considérée en retard (pas de start_time/estimated_duration valides ou progression > 0.1)."
                )
    ...
    def suggest_corrective_actions(
        ...
        """
        self.logger.info(
            f"Suggestion d'actions correctives pour {len(issues)} problèmes"
        )
    ...
    def _evaluate_overall_coherence(
        ...
        weights = {"structure": 0.3, "thematic": 0.3, "logical": 0.4}
