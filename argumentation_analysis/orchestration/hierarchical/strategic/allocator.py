...
class ResourceAllocator:
    ...
    def allocate_initial_resources(
        ...
        computational_budget = self._allocate_computational_budget(
            agent_assignments, priority_levels
        )
    ...
    def _analyze_resource_needs(
        ...
        for phase in phases:
            ...
            duration_factor = {"short": 0.7, "medium": 1.0, "long": 1.3}.get(
                phase.get("estimated_duration", "medium"), 1.0
            )
    ...
    def _determine_agent_assignments(
        ...
        phase_resource_needs: Dict[str, Dict[str, float]],
    ) -> Dict[str, List[str]]:
        """
    ...
    def _define_agent_priorities(
        self, agent_assignments: Dict[str, List[str]], phase_priorities: Dict[str, str]
    ) -> Dict[str, str]:
        """
    ...
    def _allocate_computational_budget(
        self, agent_assignments: Dict[str, List[str]], priority_levels: Dict[str, str]
    ) -> Dict[str, float]:
        """
        ...
        priority_factors = {"high": 1.5, "medium": 1.0, "low": 0.5}
        ...
        for agent_id, assigned_phases in agent_assignments.items():
            ...
            priority_factor = priority_factors.get(
                priority_levels.get(agent_id, "medium"), 1.0
            )
            phase_count_factor = (
                min(len(assigned_phases), 3) / 3.0 if assigned_phases else 0.1
            )
        ...
        if total_score > 0:
            ...
            active_agents = [
                agent_id for agent_id, phases in agent_assignments.items() if phases
            ]
            if active_agents:
    ...
    def adjust_allocation(self, feedback: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for bottleneck in bottlenecks:
            ...
            if (
                agent_id
                and agent_id in self.state.resource_allocation["priority_levels"]
            ):
                # Augmenter la priorité
                allocation_adjustments["priority_levels"][agent_id] = "high"

                # Augmenter le budget computationnel
                current_budget = self.state.resource_allocation[
                    "computational_budget"
                ].get(agent_id, 0.0)
                increase_factor = 1.5 if severity == "high" else 1.2
                allocation_adjustments["computational_budget"][agent_id] = min(
                    current_budget * increase_factor, 0.5
                )
        ...
        for idle_resource in idle_resources:
            ...
            if (
                agent_id
                and agent_id in self.state.resource_allocation["computational_budget"]
            ):
                ...
                allocation_adjustments["computational_budget"][agent_id] = (
                    current_budget * reduction_factor
                )

        # Normaliser le budget computationnel ajusté
        if allocation_adjustments["computational_budget"]:
            # Copier le budget actuel
            adjusted_budget = dict(
                self.state.resource_allocation["computational_budget"]
            )

            # Appliquer les ajustements
            for agent_id, new_budget in allocation_adjustments[
                "computational_budget"
            ].items():
    ...
    def optimize_resource_utilization(
        ...
        for agent_id, metrics in agent_efficiency.items():
            ...
            efficiency_scores[agent_id] = (
                0.4 * processing_speed + 0.4 * accuracy + 0.2 * (1.0 - resource_usage)
            )
        ...
        if efficiency_scores:
            ...
            if total_score > 0:
                ...
                for agent_id, score in efficiency_scores.items():
                    ...
                    budget_adjustments[agent_id] = current_budget + 0.5 * (
                        ideal_budget - current_budget
                    )
    ...
    def get_resource_allocation_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "computational_budget": self.state.resource_allocation[
                "computational_budget"
            ],
        }
