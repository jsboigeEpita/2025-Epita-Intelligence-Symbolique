...
class StrategicPlanner:
    ...
    def create_analysis_plan(
        self, text_metadata: Dict[str, Any], objectives: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def _decompose_objectives_into_phases(
        self, objectives: List[Dict[str, Any]], text_complexity: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        if text_complexity == "high":
            intermediate_phase = {
                ...
                "objectives": [
                    obj for obj in phase2["objectives"] if "contexte" in obj.lower()
                ],
    ...
    def _establish_phase_dependencies(
    ...
    def _define_phase_priorities(
        self, phases: List[Dict[str, Any]], objectives: List[Dict[str, Any]]
    ) -> Dict[str, str]:
        """
        ...
        objective_priorities = {
            obj["id"]: obj.get("priority", "medium") for obj in objectives
        }
    ...
    def _define_success_criteria(
        ...
        success_criteria = {}
        objective_criteria = {
            obj["id"]: obj.get("success_criteria", "") for obj in objectives
        }
    ...
    def adjust_plan(self, feedback: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if plan_adjustments:
            ...
            plan_update = {"phases": [], "priorities": {}, "success_criteria": {}}
            ...
            for phase in self.state.strategic_plan["phases"]:
                ...
                if phase_id in plan_adjustments:
                    ...
                    adjusted_phase.update(
                        plan_adjustments[phase_id].get("phase_updates", {})
                    )
                    ...
                    if "priority" in plan_adjustments[phase_id]:
                        plan_update["priorities"][phase_id] = plan_adjustments[
                            phase_id
                        ]["priority"]

                    # Mettre à jour les critères de succès si nécessaire
                    if "success_criteria" in plan_adjustments[phase_id]:
                        plan_update["success_criteria"][phase_id] = plan_adjustments[
                            phase_id
                        ]["success_criteria"]
    ...
    def _identify_problematic_phases(
        self, progress: Dict[str, Any], issues: List[Dict[str, Any]]
        ...
        for phase_id, phase_progress in progress.items():
            if (
                phase_progress.get("completion_rate", 1.0) < 0.5
                and phase_progress.get("expected_completion_rate", 0.0) > 0.7
            ):
    ...
    def _create_phase_adjustment(
        self, phase_id: str, problems: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        return (
            adjustment
            if adjustment["phase_updates"]
            or "priority" in adjustment
            or "success_criteria" in adjustment
            else {}
        )
