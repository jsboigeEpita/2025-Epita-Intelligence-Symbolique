...
class OperationalState:
    ...
    def add_task(self, task: Dict[str, Any]) -> str:
        ...
        for existing_task in self.assigned_tasks:
            if existing_task.get("id") == task_id:
                self.logger.warning(
                    f"La tâche {task_id} existe déjà dans l'état opérationnel."
                )
                return task_id
    ...
    def update_task_status(
        ...
        for task in self.assigned_tasks:
            if task.get("id") == task_id:
                ...
                self.logger.info(
                    f"Statut de la tâche {task_id} mis à jour: {old_status} -> {status}"
                )
                return True

        self.logger.warning(
            f"Impossible de mettre à jour le statut de la tâche {task_id}: tâche non trouvée."
        )
        return False
    ...
    def add_text_extract(self, extract_id: str, extract_data: Dict[str, Any]) -> bool:
        ...
        if extract_id in self.text_extracts:
            self.logger.warning(
                f"L'extrait {extract_id} existe déjà dans l'état opérationnel."
            )
            return False
    ...
    def add_analysis_result(self, result_type: str, result_data: Dict[str, Any]) -> str:
        ...
        if result_type in self.analysis_results:
            ...
            self.logger.info(
                f"Résultat d'analyse {result_id} de type {result_type} ajouté à l'état opérationnel."
            )
        else:
    ...
    def update_metrics(self, task_id: str, metrics: Dict[str, Any]) -> bool:
        ...
        if "execution_time" in metrics:
            self.operational_metrics["processing_times"][task_id] = metrics[
                "execution_time"
            ]

        # Mettre à jour les scores de confiance
        if "confidence" in metrics:
            self.operational_metrics["confidence_scores"][task_id] = metrics[
                "confidence"
            ]
    ...
    def get_task_issues(self, task_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            issue
            for issue in self.encountered_issues
    ...
    def get_in_progress_tasks(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            task for task in self.assigned_tasks if task.get("status") == "in_progress"
        ]
    ...
    def get_completed_tasks(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            task for task in self.assigned_tasks if task.get("status") == "completed"
        ]
    ...
    def find_operational_task_by_tactical_id(
        self, tactical_task_id: str
    ) -> Optional[str]:
        """
