...
class RhetoricalToolsFeedback:
        ...
        if feedback_file.exists():
            try:
                with open(feedback_file, "r", encoding="utf-8") as f:
                    ...
                    self.feedback_stats = data.get(
                        "feedback_stats", self.feedback_stats
                    )

                self.logger.info(
                    f"Historique de feedback chargé: {len(self.feedback_history)} entrées"
                )
            except Exception as e:
                self.logger.error(
                    f"Erreur lors du chargement de l'historique de feedback: {e}"
                )
    ...
    def _save_feedback_history(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Historique de feedback sauvegardé: {len(self.feedback_history)} entrées"
            )
        except Exception as e:
            self.logger.error(
                f"Erreur lors de la sauvegarde de l'historique de feedback: {e}"
            )
    ...
    def add_feedback(
        ...
        self.logger.info(
            f"Feedback ajouté pour {tool_type} (ID: {result_id}): {feedback_type}"
        )
    ...
    def get_feedback_for_tool(self, tool_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            feedback
            for feedback in self.feedback_history
            if feedback["tool_type"] == tool_type
        ]
    ...
    def apply_feedback_to_tool_parameters(
        self, tool_type: str, current_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        positive_count = sum(
            1 for fb in tool_feedbacks if fb["feedback_type"] == "positive"
        )
        negative_count = sum(
            1 for fb in tool_feedbacks if fb["feedback_type"] == "negative"
        )
        ...
        if total_count > 0:
            ...
            if "confidence_threshold" in adjusted_parameters:
                ...
                adjusted_parameters["confidence_threshold"] = max(
                    0.5, min(0.95, current_threshold - adjustment)
                )

            # Ajuster d'autres paramètres selon les besoins

            self.logger.info(
                f"Paramètres ajustés pour {tool_type} en fonction de {total_count} feedbacks"
            )
    ...
    def generate_feedback_report(self) -> Dict[str, Any]:
        ...
        positive_feedbacks = sum(
            1 for fb in self.feedback_history if fb["feedback_type"] == "positive"
        )
        negative_feedbacks = sum(
            1 for fb in self.feedback_history if fb["feedback_type"] == "negative"
        )
        neutral_feedbacks = sum(
            1 for fb in self.feedback_history if fb["feedback_type"] == "neutral"
        )
        ...
        for tool_type in self.feedback_stats:
            ...
            if tool_feedbacks:
                tool_stats[tool_type] = {
                    ...
                    "neutral": sum(
                        1 for fb in tool_feedbacks if fb["feedback_type"] == "neutral"
                    ),
...
class FeedbackManager:
    ...
    def collect_feedback(
        ...
        if (
            level == "operational"
            and tool_type in self.rhetorical_tools_feedback.feedback_stats
        ):
    ...
    def apply_feedback(
        self, level: str, tool_type: str, current_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Applique les feedbacks accumulés pour ajuster les paramètres d'un outil.
