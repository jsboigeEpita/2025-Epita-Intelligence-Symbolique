...
class OperationalAgentRegistry:
    ...
    def __init__(
        ...
        if not self.kernel or not self.llm_service_id:
            self.logger.warning(
                "Kernel ou llm_service_id non fournis à OperationalAgentRegistry. L'initialisation des agents échouera."
            )
        if not self.project_context:
            self.logger.warning(
                "ProjectContext non fourni à OperationalAgentRegistry. L'initialisation des agents risque d'échouer."
            )
    ...
    async def get_agent(self, agent_type: str) -> Optional[OperationalAgent]:
        ...
        try:
            ...
            if not self.kernel or not self.llm_service_id or not self.project_context:
                self.logger.error(
                    f"Kernel, llm_service_id ou project_context manquant pour initialiser l'agent {agent_type}"
                )
                return None

            success = await agent.initialize(
                self.kernel, self.llm_service_id, self.project_context
            )
            if not success:
            ...
            self.logger.error(
                f"Erreur lors de la création de l'agent {agent_type}: {e}"
            )
            return None
    ...
    def register_agent_class(
        self, agent_type: str, agent_class: Type[OperationalAgent]
    ) -> bool:
        """
