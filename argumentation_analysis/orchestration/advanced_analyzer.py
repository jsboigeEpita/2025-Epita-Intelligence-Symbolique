...
def analyze_extract_advanced(
    ...
    try:
        ...
        if base_result:
            try:
                results["comparison_with_base"] = compare_rhetorical_analyses(results, base_result)
            except Exception as e:
                logger.error(f"Erreur lors de la comparaison des analyses pour '{extract_name}': {e}", exc_info=True)
                results["comparison_with_base"] = {"error": str(e)}

        return results

    except Exception as e:
        logger.error(f"Erreur lors de l'appel au AnalysisToolsPlugin pour l'extrait '{extract_name}': {e}", exc_info=True)
        return {
            "extract_name": extract_name,
            "source_name": source_name,
