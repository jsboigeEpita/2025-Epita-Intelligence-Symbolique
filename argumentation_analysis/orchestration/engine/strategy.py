...
async def _analyze_text_features_for_strategy(text: str) -> Dict[str, Any]:
    ...
    features = {
        ...
        "has_debate_markers": any(
            marker in text.lower()
            for marker in ["argument", "contre-argument", "objection", "réfutation"]
        ),
...
async def select_strategy(
    ...
    if source_info and source_info.get("type") == "monitoring":
        logger.info(
            "Selecting OPERATIONAL_DIRECT strategy for source type 'monitoring'."
        )
    ...
    if config.orchestration_mode and config.orchestration_mode.value != "auto_select":
        ...
        selected_strategy = mode_strategy_map.get(
            config.orchestration_mode, OrchestrationStrategy.HIERARCHICAL_FULL
        )
        logger.info(
            f"Manual mode selection: OrchestrationMode.{config.orchestration_mode.name} -> OrchestrationStrategy.{selected_strategy.name}"
        )
    ...
    if not config.auto_select_orchestrator_enabled:
        logger.info(
            "Auto-select orchestrator disabled. Defaulting to HIERARCHICAL_FULL strategy."
        )
    ...
    if config.analysis_type == AnalysisType.INVESTIGATIVE:
        logger.info(
            "AnalysisType is INVESTIGATIVE. Selecting SPECIALIZED_DIRECT strategy."
        )
        ...
        logger.info(
            "Hierarchical orchestration enabled and text is long. Selecting HIERARCHICAL_FULL strategy."
        )
        return OrchestrationStrategy.HIERARCHICAL_FULL
