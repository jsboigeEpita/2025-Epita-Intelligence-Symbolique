...
logger = logging.getLogger(
    __name__
)  # Déplacé ici pour être utilisé par les try/except d'import
...
try:
    ...
    AnalysisType = None
    logger.warning(
        "Could not import AnalysisType. Specialized orchestrator selection might be affected."
    )
...
class MainOrchestrator:
    ...
    def __init__(
        ...
        try:
            ...
            self.direct_operational_executor = DirectOperationalExecutor(
                kernel=self.kernel
            )
        except ImportError as e:
            self.direct_operational_executor = None
            logger.error(
                f"ECHEC de l'importation de DirectOperationalExecutor: {e}",
                exc_info=True,
            )

        # Initialiser la carte des orchestrateurs spécialisés
        self.specialized_orchestrators_map = (
            self._initialize_specialized_orchestrators()
        )
    ...
    async def run_analysis(
        ...
        """
        logger.info(
            f"Received analysis request for text_input (length: {len(text_input)})."
        )
        if source_info:
        ...
        strategy = await select_strategy(
            self.config, text_input, source_info, custom_config
        )
        logger.info(
            f"Stratégie sélectionnée : {strategy.value if strategy else 'None'}"
        )
    ...
    async def _execute_hierarchical_full(self, text_input: str) -> Dict[str, Any]:
        ...
        """
        logger.info(
            "[HIERARCHICAL] Exécution de l'orchestration hiérarchique complète..."
        )
        ...
        try:
            ...
            if self.strategic_manager:
                ...
                try:
                    strategic_results = self.strategic_manager.initialize_analysis(
                        text_input
                    )
                    ...
                    logger.error(
                        f"Erreur critique lors de l'appel à strategic_manager.initialize_analysis: {e}",
                        exc_info=True,
                    )
            ...
            if self.tactical_coordinator and self.strategic_manager:
                ...
                tactical_results = (
                    self.tactical_coordinator.process_strategic_objectives(objectives)
                )
            ...
            if self.operational_manager:
                ...
                if (
                    not operational_tasks_input and objectives
                ):  # Fallback si tactical n'a rien produit mais qu'on a des objectifs
                    logger.warning(
                        "[OPERATIONAL] Tactical coordination results are empty, attempting to use objectives for operational tasks."
                    )
                    operational_tasks_input = {
                        "objectives": objectives
                    }  # Adapter selon ce que _execute_operational_tasks attend

                operational_results = await self._execute_operational_tasks(
                    text_input, operational_tasks_input
                )  # Adaptation: text -> text_input
            ...
            logger.error(
                f"[HIERARCHICAL] Erreur dans l'orchestration hiérarchique: {e}",
                exc_info=True,
            )  # Ajout de exc_info pour la stack trace
            results["status"] = "error"
            results["error_message"] = str(e)
            if "strategic_analysis" not in results:  # S'assurer que la clé existe
                results["strategic_analysis"] = {}
            results["strategic_analysis"]["error"] = str(
                e
            )  # Maintenir la compatibilité si nécessaires

        return results

    async def _execute_operational_tasks(
        self, text_input: str, tactical_coordination_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            operational_results = (
                await self.direct_operational_executor.execute_operational_pipeline(
                    text_input=text_input,
                    tactical_results=tactical_coordination_results,
                )
            )
            logger.info(
                "[OPERATIONAL] Exécution via DirectOperationalExecutor terminée."
            )
            return operational_results

        except Exception as e:
            logger.error(
                f"[OPERATIONAL] Erreur durant l'exécution via DirectOperationalExecutor: {e}",
                exc_info=True,
            )
            return {
                "status": "error",
    ...
    async def _synthesize_hierarchical_results(
        ...
        try:
            ...
            if strategic_results:
                ...
                synthesis["strategic_alignment"] = min(
                    objectives_count / 4.0, 1.0
                )  # Max 4 objectifs

            # Efficacité tactique
            if tactical_results:
                tasks_created = tactical_results.get("tasks_created", 0)
                synthesis["tactical_efficiency"] = min(
                    tasks_created / 10.0, 1.0
                )  # Max 10 tâches

            # Succès opérationnel
            if operational_results:
                success_rate = operational_results.get("summary", {}).get(
                    "success_rate", 0.0
                )
            ...
            if synthesis["overall_score"] > 0.8:
                ...
                synthesis["recommendations"].append(
                    "Orchestration hiérarchique satisfaisante"
                )
            else:
                synthesis["recommendations"].append(
    ...
    async def _execute_strategic_only(self, text_input: str) -> Dict[str, Any]:
        ...
        logger.info(
            "[STRATEGIC_ONLY] Exécution de l'orchestration stratégique uniquement..."
        )
        results: Dict[str, Any] = {
            "status": "pending",
        ...
        try:
            ...
            if strategic_manager:
                logger.info(
                    "[STRATEGIC_ONLY] Initialisation de l'analyse stratégique..."
                )
                ...
                analysis_output = await strategic_manager.initialize_analysis(
                    text_input
                )
                ...
                if (
                    analysis_output and analysis_output.get("objectives") is not None
                    ...
                    logger.warning(
                        f"[STRATEGIC_ONLY] L'analyse stratégique n'a pas produit les résultats attendus: {analysis_output}"
                    )
                ...
                results["strategic_analysis"][
                    "error"
                ] = "StrategicManager non configuré."

        except Exception as e:
            logger.error(
                f"[STRATEGIC_ONLY] Erreur dans l'orchestration stratégique uniquement: {e}",
                exc_info=True,
            )
            results["status"] = "error"
            results["error_message"] = str(e)
    ...
    async def _execute_tactical_coordination(self, text_input: str) -> Dict[str, Any]:
        ...
        logger.info(
            "[TACTICAL_COORDINATION] Exécution de la stratégie de coordination tactique..."
        )
        results: Dict[str, Any] = {
            "status": "pending",
        ...
        try:
            strategic_manager = getattr(self.config, "strategic_manager_instance", None)
            tactical_coordinator = getattr(
                self.config, "tactical_coordinator_instance", None
            )
            objectives = []
            ...
            logger.info(
                "[TACTICAL_COORDINATION] Étape 1: Initialisation de l'analyse stratégique (interne)..."
            )
            strategic_analysis_output = await strategic_manager.initialize_analysis(
                text_input
            )

            if (
                not strategic_analysis_output
                or strategic_analysis_output.get("objectives") is None
            ):
                logger.warning(
                ...
                results["tactical_coordination_results"][
                    "details"
                ] = strategic_analysis_output  # Inclure la sortie stratégique pour le débogage
                return results
            ...
            logger.info(
                "[TACTICAL_COORDINATION] Étape 2: Coordination tactique basée sur les objectifs stratégiques..."
            )
            tactical_run_results = (
                await tactical_coordinator.process_strategic_objectives(objectives)
            )
            ...
            if (
                isinstance(tactical_run_results, dict)
                and "error" not in tactical_run_results
            ):
                results["status"] = "success"
            else:
                ...
                logger.warning(
                    f"[TACTICAL_COORDINATION] La coordination tactique a rencontré un problème ou n'a pas produit les résultats attendus: {tactical_run_results}"
                )
                if (
                    isinstance(tactical_run_results, dict)
                    and "error" not in tactical_run_results
                ):  # Ajouter un message d'erreur générique si non présent
                    results["tactical_coordination_results"][
                        "error"
                    ] = "La coordination tactique n'a pas produit les résultats attendus."

        except Exception as e:
            logger.error(
                f"[TACTICAL_COORDINATION] Erreur majeure dans la stratégie de coordination tactique: {e}",
                exc_info=True,
            )
            results["status"] = "error"
            results["error_message"] = str(e)
    ...
    async def _execute_operational_direct(self, text_input: str) -> Dict[str, Any]:
        ...
        logger.info(
            "[OPERATIONAL_DIRECT] Exécution de la stratégie opérationnelle directe..."
        )
        ...
        try:
            ...
            # Étape 1 (Interne) - Analyse Stratégique
            logger.info(
                "[OPERATIONAL_DIRECT] Étape 1 (Interne): Analyse Stratégique..."
            )
            strategic_results = strategic_manager.initialize_analysis(text_input)
            objectives = strategic_results.get("objectives", [])
            if not objectives:
                logger.warning(
                    "[OPERATIONAL_DIRECT] L'analyse stratégique n'a pas produit d'objectifs. L'exécution opérationnelle pourrait être limitée."
                )

            # Étape 2 (Interne) - Coordination Tactique
            logger.info(
                "[OPERATIONAL_DIRECT] Étape 2 (Interne): Coordination Tactique..."
            )
            tactical_results = tactical_coordinator.process_strategic_objectives(
                objectives
            )

            # Étape 3 - Exécution Opérationnelle
            logger.info("[OPERATIONAL_DIRECT] Étape 3: Exécution Opérationnelle...")
            operational_execution_output = await self._execute_operational_tasks(
            ...
            if (
                isinstance(operational_execution_output, dict)
                and "error" in operational_execution_output
            ):
                current_status = "error"
                error_message_content = operational_execution_output.get(
                    "error", "Erreur inconnue lors de l'exécution opérationnelle."
                )
            elif (
                not isinstance(operational_execution_output, dict)
                or not operational_execution_output
            ):
                current_status = "error"
                ...
                operational_execution_output = {
                    "error": error_message_content,
                    "details": operational_execution_output,
                }
            ...
            logger.error(
                f"[OPERATIONAL_DIRECT] Erreur majeure dans la stratégie opérationnelle directe: {e}",
                exc_info=True,
            )
            return {
                "status": "error",
    ...
    async def _execute_specialized_direct(self, text_input: str) -> Dict[str, Any]:
        ...
        logger.info(
            "[SPECIALIZED_DIRECT] Exécution de l'orchestration spécialisée directe..."
        )
        ...
        try:
            ...
            if selected_orchestrator_info:
                ...
                if not orchestrator_instance:
                    logger.error(
                        f"Orchestrator instance for '{orchestrator_name}' is missing in config map."
                    )
                ...
                logger.info(
                    f"[SPECIALIZED_DIRECT] Utilisation de l'orchestrateur: {orchestrator_name} (type: {type(orchestrator_instance)})"
                )

                specialized_run_results: Dict[str, Any] = {}
                # Assurer que les orchestrateurs et run_cluedo_game sont importés
                if (
                    orchestrator_name == "cluedo"
                    and CluedoExtendedOrchestrator
                    and run_cluedo_game
                    and isinstance(orchestrator_instance, CluedoExtendedOrchestrator)
                ):
                    specialized_run_results = await self._run_cluedo_investigation(
                        text_input, orchestrator_instance
                    )
                elif (
                    orchestrator_name == "conversation"
                    and ConversationOrchestrator
                    and isinstance(orchestrator_instance, ConversationOrchestrator)
                ):
                    if hasattr(orchestrator_instance, "run_conversation"):
                        specialized_run_results = (
                            await orchestrator_instance.run_conversation(text_input)
                        )
                    else:
                        logger.warning(
                            f"ConversationOrchestrator '{orchestrator_name}' lacks 'run_conversation' method."
                        )
                        specialized_run_results = {
                            "status": "error",
                ...
                output_results["status"] = specialized_run_results.get(
                    "status", "unknown_specialized_status"
                )
            else:
                logger.info(
                    "[SPECIALIZED_DIRECT] Aucun orchestrateur spécialisé n'a pu être sélectionné."
                )
                output_results["specialized_orchestration"] = {
            ...
            logger.error(
                f"[SPECIALIZED_DIRECT] Erreur majeure dans l'orchestration spécialisée directe: {e}",
                exc_info=True,
            )
    ...
    async def _select_specialized_orchestrator(
        self,
        ...
        if not specialized_orchestrators_map:
            logger.warning(
                "No specialized orchestrators map found in config (self.config.specialized_orchestrators_map)."
            )
            return None
        ...
        if not current_analysis_type or not AnalysisType:
            logger.warning(
                "Analysis type enum (self.config.analysis_type_enum) or AnalysisType import not found. Considering all orchestrators."
            )
            ...
            if not compatible_orchestrators:
                logger.info(
                    f"No specialized orchestrator directly compatible with {current_analysis_type}. Considering all available."
                )
                compatible_orchestrators = list(specialized_orchestrators_map.items())
        ...
        logger.debug(
            f"Selected specialized orchestrator: {selected_name} with priority {selected_data.get('priority')}"
        )
        return selected_name, selected_data

    async def _run_cluedo_investigation(
        self, text_input: str, orchestrator_instance: Any
    ) -> Dict[str, Any]:
        """Exécute une investigation de type Cluedo."""
        if (
            not CluedoExtendedOrchestrator
            or not run_cluedo_game
            or not isinstance(orchestrator_instance, CluedoExtendedOrchestrator)
        ):
            logger.warning(
                "Cluedo investigation prerequisites not met (CluedoExtendedOrchestrator, run_cluedo_game, or instance type mismatch)."
            )
            return {
                "status": "limited",
                "message": "Prérequis pour l'investigation Cluedo non remplis.",
            }
        try:
            if hasattr(orchestrator_instance, "kernel"):
                logger.info(
                    f"Running Cluedo investigation with orchestrator: {type(orchestrator_instance)}"
                )
                ...
                conversation_history = (
                    results.get("conversation_history", []) if results else []
                )
                final_state = results.get("final_state", {}) if results else {}
                final_solution = (
                    final_state.get("final_solution") if final_state else None
                )
                ...
                logger.warning(
                    "Cluedo orchestrator instance does not have 'kernel' attribute."
                )
                return {
                    "status": "limited",
                    "message": "Instance CluedoOrchestrator sans attribut 'kernel'.",
                }
        except Exception as e:
    ...
    async def _execute_hybrid(self, text_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if tactical_results.get("status") == "error":
                errors.append(
                    f"Tactical coordination failed: {tactical_results.get('error_message', 'Unknown error')}"
                )
        except Exception as e:
            logger.error(
                f"Error during tactical coordination in hybrid strategy: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            if specialized_results.get("status") == "error":
                errors.append(
                    f"Specialized direct execution failed: {specialized_results.get('error_message', 'Unknown error')}"
                )
        except Exception as e:
            logger.error(
                f"Error during specialized direct execution in hybrid strategy: {e}",
                exc_info=True,
            )
        ...
        if errors:
            if (
                tactical_results.get("status") == "error"
                and specialized_results.get("status") == "error"
            ):
                final_status = "error"
            else:
    ...
    async def _execute_service_manager(self, text_input: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Executing SERVICE_MANAGER strategy for input: {text_input[:50]}..."
        )
        try:
            ...
            logger.error(
                f"ServiceManager instance is missing 'get_services_status' method or is not correctly configured: {e}"
            )
            return {
                "status": "error",
                "strategy_used": OrchestrationStrategy.SERVICE_MANAGER.value,
            ...
            logger.error(
                f"Error during Service Manager strategy execution: {e}", exc_info=True
            )
            return {
                "status": "error",
                "strategy_used": OrchestrationStrategy.SERVICE_MANAGER.value,
    ...
    async def _execute_fallback(self, text_input: str) -> Dict[str, Any]:
        ...
        input_snippet = (
            text_input[:snippet_length] + "..."
            if len(text_input) > snippet_length
            else text_input
        )
...
if __name__ == "__main__":
    ...
    async def main(text_to_analyze: str):
        ...
        logger.info(
            "Initialisation de l'orchestrateur principal en mode script exemple."
        )
    ...
    if __name__ == "__main__":
        parser = argparse.ArgumentParser(
            description="Exécute l'orchestrateur d'analyse d'argumentation."
        )
        parser.add_argument(
            "--text", type=str, required=True, help="Le texte à analyser."
        )
