...
async def main():
    ...
    logger.info(f"Début de l'invocation du chat pour l'analyse du texte.")
    
    # 5. Exécution du scénario d'analyse et récupération de l'historique
    final_history: List[ChatMessageContent] = [
        message async for message in chat.invoke()
    ]
    
    logger.info(f"Invocation du chat terminée. {len(final_history)} messages dans l'historique final.")
    
    # 6. Affichage des résultats
    # L'historique initial est déjà inclus dans le chat, donc on affiche tout.
