...
try:
    ...
    from argumentation_analysis.orchestration.hierarchical.tactical.manager import (
        TacticalManager,
    )
    ...
    logging.warning(
        f"Certains gestionnaires hiérarchiques ne sont pas disponibles: {e}"
    )
...
try:
    ...
    logging.error(
        f"Échec critique de l'importation des modules de communication: {e}",
        exc_info=True,
    )
...
class OrchestrationServiceManager:
        ...
        try:
            ...
            if not self.project_context:
                raise ServiceManagerError(
                    "Échec de l'initialisation du contexte du projet (bootstrap)."
                )
    ...
    async def _initialize_hierarchical_managers(self):
        ...
        if not self.middleware:
            self.logger.warning(
                "Middleware non initialisé. Impossible d'initialiser les gestionnaires hiérarchiques."
            )
            return
        ...
        try:
            ...
            if OperationalManager:
                ...
                self.logger.info(
                    "OperationalManager initialisé et abonné au middleware."
                )

        except Exception as e:
            self.logger.error(
                f"Erreur critique lors de l'initialisation des gestionnaires hiérarchiques: {e}",
                exc_info=True,
            )
    ...
    async def _initialize_specialized_orchestrators(self):
        ...
        try:
            ...
            self.logger.error(
                f"Erreur critique lors de l'initialisation des orchestrateurs spécialisés: {e}"
            )
    ...
    async def analyze_text(
        ...
        if not self._initialized:
            raise ServiceManagerError(
                "ServiceManager non initialisé. Appelez initialize() d'abord."
            )
    ...
    async def _run_specialized_analysis(
        ...
        try:
            ...
            if hasattr(orchestrator, "analyze_with_fact_checking"):
                from .fact_checking_orchestrator import (
                    FactCheckingRequest,
                    AnalysisDepth,
                )
            ...
            # Interface générique pour autres orchestrateurs
                ...
                raise Exception(
                    f"Orchestrateur {orchestrator.__class__.__name__} sans méthode d'analyse supportée"
                )
        except Exception as e:
    ...
    async def _run_hierarchical_analysis(
        self, text: str, analysis_type: str, options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.operational_manager:
                try:
                    operational_result = await self._run_operational_analysis(
                        text, options
                    )
    ...
    async def _run_strategic_analysis(
        self, text: str, options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Lance l'analyse stratégique."""
        self.logger.info(
            f"ServiceManager._run_strategic_analysis appelée pour le texte : '{text[:50]}...'"
        )
        if not self.strategic_manager:
            self.logger.warning(
                "StrategicManager non disponible pour _run_strategic_analysis."
            )
        ...
        try:
            ...
            self.logger.info(
                f"strategic_manager.initialize_analysis retourné : {initial_strategic_data}"
            )

            objectives = initial_strategic_data.get("objectives", [])
            if not objectives:
                self.logger.warning(
                    "Aucun objectif stratégique retourné par initialize_analysis."
                )
                return {
                    "level": "strategic",
            ...
            self.logger.info(
                f"Définition de {len(objectives)} objectifs stratégiques..."
            )
            ...
            for objective in list(objectives):
                self.logger.info(
                    f"Appel de strategic_manager.define_strategic_goal pour l'objectif : {objective.get('id')}"
                )
                self.strategic_manager.define_strategic_goal(objective)  # Synchrone
                self.logger.info(
                    f"Objectif {objective.get('id')} transmis au niveau tactique (via define_strategic_goal)."
                )
            ...
            self.logger.error(
                f"Erreur dans _run_strategic_analysis: {e}", exc_info=True
            )
    ...
    async def _run_tactical_analysis(
        ...
        self.logger.info(
            f"ServiceManager._run_tactical_analysis appelée pour le texte : '{text[:50]}...'. Options: {options}"
        )
...
Réponds au format JSON avec les clés: arguments, sophismes, structure_rhetorique, coherence, recommandations."""
    ...
    async def _run_operational_analysis(
        self, text: str, options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Lance l'analyse opérationnelle avec de vrais appels LLM."""
        self.logger.info(
            f"ServiceManager._run_operational_analysis appelée pour le texte : '{text[:50]}...'. Options: {options}"
        )
...
Réponds au format JSON avec les clés: entites, relations, patterns, persuasion, premisses, conclusions, biais, validite_logique."""
            ...
            self.logger.error(
                f"Erreur dans l'analyse opérationnelle: {e}", exc_info=True
            )
            return {
                "level": "operational",
                "status": "error",
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["checks"]["analysis"] = {"status": "error", "error": str(e)}
    ...
    def get_status_details(self) -> Dict[str, Any]:
        ...
        if settings.service_manager.enable_hierarchical:
            service_details["active_components"]["strategic_manager"] = (
                self.strategic_manager is not None
            )
            service_details["active_components"]["tactical_manager"] = (
                self.tactical_manager is not None
            )
            service_details["active_components"]["operational_manager"] = (
                self.operational_manager is not None
            )

            if self.strategic_manager and hasattr(self.strategic_manager, "get_status"):
                try:
                    service_details["component_specific_status"][
                        "strategic_manager"
                    ] = self.strategic_manager.get_status()
                except Exception as e:
                    service_details["component_specific_status"][
                        "strategic_manager"
                    ] = {"error": str(e)}
            ...
            if self.tactical_manager and hasattr(self.tactical_manager, "get_status"):
                try:
                    service_details["component_specific_status"][
                        "tactical_manager"
                    ] = self.tactical_manager.get_status()
                except Exception as e:
                    service_details["component_specific_status"]["tactical_manager"] = {
                        "error": str(e)
                    }

            if self.operational_manager and hasattr(
                self.operational_manager, "get_status"
            ):
                try:
                    service_details["component_specific_status"][
                        "operational_manager"
        ...
        if settings.service_manager.enable_specialized_orchestrators:
            service_details["active_components"]["cluedo_orchestrator"] = (
                self.cluedo_orchestrator is not None
            )
            service_details["active_components"]["conversation_orchestrator"] = (
                self.conversation_orchestrator is not None
            )
            service_details["active_components"]["llm_orchestrator"] = (
                self.llm_orchestrator is not None
            )
            service_details["active_components"]["fact_checking_orchestrator"] = (
                self.fact_checking_orchestrator is not None
            )

            if self.cluedo_orchestrator and hasattr(
                self.cluedo_orchestrator, "get_status"
            ):
                try:
                    service_details["component_specific_status"][
                        "cluedo_orchestrator"
            ...
            if self.conversation_orchestrator and hasattr(
                self.conversation_orchestrator, "get_status"
            ):
                try:
                    service_details["component_specific_status"][
            ...
            if self.llm_orchestrator and hasattr(self.llm_orchestrator, "get_status"):
                try:
                    ...
                    service_details["component_specific_status"]["llm_orchestrator"] = {
                        "error": str(e)
                    }

            if self.fact_checking_orchestrator and hasattr(
                self.fact_checking_orchestrator, "get_performance_metrics"
            ):
                try:
                    service_details["component_specific_status"][
        ...
        if settings.service_manager.enable_communication_middleware:
            service_details["active_components"]["middleware"] = (
            ...
            if self.middleware and hasattr(self.middleware, "get_status"):
                try:
                    service_details["component_specific_status"][
                        "middleware"
                    ] = self.middleware.get_status()
                except Exception as e:
                    service_details["component_specific_status"]["middleware"] = {
                        "error": str(e)
                    }
    ...
    def __repr__(self) -> str:
        return (
            f"ServiceManager(session_id='{self.state.session_id}', "
