...
AGENT_CONFIG = {
    ...
    "FormalAgent": {
        ...
        "instructions": (
            ...
            "ETAPE 0 — Consult Shared Atoms/Signature (#545) :\n"
            "1. Lis l'etat via get_current_state_snapshot()\n"
            "2. Verifie si atomic_propositions ou fol_shared_signature existent deja\n"
            "   - Si atomic_propositions present: utilise ces atomes comme base pour PL\n"
            "   - Si fol_shared_signature present: utilise sorts/predicates/constants pour FOL\n"
            "3. Les atomes/signatures partages garantissent coherence inter-arguments\n\n"
            "ETAPE 1 — NL → Traduction Formelle :\n"
            "1. Lis les arguments identifies dans l'etat via get_current_state_snapshot()\n"
            "2. Pour chaque argument cle, commence par translate_to_fol(text='argument text') "
            "   — FOL est la logique par defaut car elle capture relations, quantificateurs "
            "   et predicats que PL ne peut pas exprimer\n"
            "3. Si FOL echoue ou si l'argument est purement propositionnel, "
            "   alors utilise translate_to_pl(text='argument text') comme fallback\n"
            "4. IMPORTANT: Si des atomes partages existent (ETAPE 0), inclus-les dans "
            "   le texte passe au translateur pour guider la generation vers les memes symboles\n"
