...
class EnqueteStateManagerPlugin:
    ...
    def __init__(self, state: BaseWorkflowState):
        ...
        self._logger.info(
            f"EnqueteStateManagerPlugin initialisé avec l'instance d'état (id: {id(self._state)}, type: {type(self._state).__name__})."
        )

    # --- Méthodes héritées de BaseWorkflowState ---

    @kernel_function(
        description="Ajoute une nouvelle tâche au workflow de l'enquête.",
        name="add_task",
    )
    def add_task(
        self, description: str, assignee: str, task_id: Optional[str] = None
    ) -> str:
        self._logger.info(
            f"Appel add_task: desc='{description[:50]}...', assignee='{assignee}'"
        )
        try:
    ...
    @kernel_function(
        description="Récupère les détails d'une tâche spécifique.", name="get_task"
    )
    ...
    @kernel_function(
        description="Met à jour le statut d'une tâche.", name="update_task_status"
    )
    def update_task_status(self, task_id: str, status: str) -> str:
        self._logger.info(
            f"Appel update_task_status: task_id='{task_id}', status='{status}'"
        )
        try:
    ...
    @kernel_function(
        description="Récupère une liste de tâches, potentiellement filtrée par assigné ou statut.",
        name="get_tasks",
    )
    def get_tasks(
        self, assignee: Optional[str] = None, status: Optional[str] = None
    ) -> str:
    ...
    def designate_next_agent(self, agent_name: str) -> str:
        ...
        try:
            ...
            return json.dumps(
                {"message": f"Agent '{agent_name}' désigné pour le prochain tour."}
            )
        except Exception as e:
    ...
    @kernel_function(
        description="Récupère l'agent désigné pour le prochain tour.",
    ...
    def get_designated_next_agent(self) -> str:
        ...
        try:
            ...
            self._logger.error(
                f"Erreur dans get_designated_next_agent: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})
    ...
    @kernel_function(
        description="Récupère la description principale du cas d'enquête.",
    ...
    def get_case_description(self) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode get_case_description n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
    ...
    @kernel_function(
        description="Met à jour la description principale du cas d'enquête.",
        name="update_case_description",
    )
    def update_case_description(self, new_description: str) -> str:
        self._logger.info(
            f"Appel update_case_description: new_desc='{new_description[:50]}...'"
        )
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode update_case_description n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            ...
            self._logger.error(
                f"Erreur dans update_case_description: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Ajoute un élément identifié (preuve, témoignage, etc.) à l'enquête.",
        name="add_identified_element",
    )
    def add_identified_element(
        self, element_type: str, description: str, source: str
    ) -> str:
        self._logger.info(
            f"Appel add_identified_element: type='{element_type}', desc='{description[:50]}...'"
        )
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode add_identified_element n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            element = self._state.add_identified_element(
                element_type, description, source
            )
            return json.dumps(element)
        except Exception as e:
            self._logger.error(
                f"Erreur dans add_identified_element: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Récupère les éléments identifiés, potentiellement filtrés par type.",
        name="get_identified_elements",
    )
    def get_identified_elements(self, element_type: Optional[str] = None) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode get_identified_elements n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            ...
            self._logger.error(
                f"Erreur dans get_identified_elements: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Ajoute ou met à jour un Belief Set (ensemble de croyances/formules logiques).",
        name="add_or_update_belief_set",
    )
    def add_or_update_belief_set(self, bs_id: str, content: str) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode add_or_update_belief_set n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            ...
            self._logger.error(
                f"Erreur dans add_or_update_belief_set: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Récupère le contenu d'un Belief Set spécifique.",
        name="get_belief_set_content",
    )
    def get_belief_set_content(self, bs_id: str) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode get_belief_set_content n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            content = self._state.get_belief_set_content(bs_id)
            return (
                json.dumps({"belief_set_id": bs_id, "content": content})
                if content is not None
                else json.dumps(None)
            )
        except Exception as e:
            self._logger.error(
                f"Erreur dans get_belief_set_content: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Liste les IDs de tous les Belief Sets existants.",
    ...
    def list_belief_sets(self) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode list_belief_sets n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
    ...
    @kernel_function(
        description="Ajoute une nouvelle hypothèse à l'enquête.", name="add_hypothesis"
    )
    def add_hypothesis(
        self, text: str, confidence_score: float, hypothesis_id: Optional[str] = None
    ) -> str:
        self._logger.info(
            f"Appel add_hypothesis: text='{text[:50]}...', confidence={confidence_score}"
        )
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode add_hypothesis n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            hypothesis = self._state.add_hypothesis(
                text, confidence_score, hypothesis_id
            )
    ...
    @kernel_function(
        description="Récupère une hypothèse spécifique par son ID.",
        name="get_hypothesis",
    )
    def get_hypothesis(self, hypothesis_id: str) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode get_hypothesis n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
    ...
    @kernel_function(
        description="Met à jour une hypothèse existante.", name="update_hypothesis"
    )
    def update_hypothesis(
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode update_hypothesis n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
            success = self._state.update_hypothesis(
                ...
                new_confidence,
                new_status,
                add_supporting_evidence_id,
                add_contradicting_evidence_id,
            )
            return json.dumps({"success": success})
        except Exception as e:
    ...
    @kernel_function(
        description="Récupère les hypothèses, potentiellement filtrées par statut.",
        name="get_hypotheses",
    )
    def get_hypotheses(self, status: Optional[str] = None) -> str:
        ...
        if not isinstance(self._state, EnquetePoliciereState):
            return json.dumps(
                {
                    "error": "La méthode get_hypotheses n'est disponible que pour EnquetePoliciereState."
                }
            )
        try:
    ...
    @kernel_function(
        description="Récupère les éléments du jeu Cluedo (suspects, armes, lieux).",
        name="get_cluedo_game_elements",
    )
    def get_cluedo_game_elements(self) -> str:
        ...
        if not isinstance(self._state, EnqueteCluedoState):
            return json.dumps(
                {
                    "error": "La méthode get_cluedo_game_elements n'est disponible que pour EnqueteCluedoState."
                }
            )
        try:
            ...
            self._logger.error(
                f"Erreur dans get_cluedo_game_elements: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Récupère l'ID du Belief Set principal pour l'enquête Cluedo.",
        name="get_cluedo_main_belief_set_id",
    )
    def get_cluedo_main_belief_set_id(self) -> str:
        ...
        if not isinstance(self._state, EnqueteCluedoState):
            return json.dumps(
                {
                    "error": "La méthode get_cluedo_main_belief_set_id n'est disponible que pour EnqueteCluedoState."
                }
            )
        try:
            ...
            self._logger.error(
                f"Erreur dans get_cluedo_main_belief_set_id: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Propose une solution finale à l'enquête Cluedo.",
    ...
    def propose_final_solution(self, solution: Dict[str, str]) -> str:
        ...
        if not isinstance(self._state, EnqueteCluedoState):
            return json.dumps(
                {
                    "error": "La méthode propose_final_solution n'est disponible que pour EnqueteCluedoState."
                }
            )
        try:
            self._state.propose_final_solution(solution)
            return json.dumps(
                {"message": "Solution finale enregistrée.", "solution_proposed": True}
            )
        except Exception as e:
            self._logger.error(
                f"Erreur dans propose_final_solution: {e}", exc_info=True
            )
            return json.dumps({"error": str(e)})

    @kernel_function(
        description="Permet à un agent de faire une suggestion (suspect, arme, lieu) pour obtenir un indice en retour.",
        name="faire_suggestion",
    )
    def faire_suggestion(self, suspect: str, arme: str, lieu: str) -> str:
        self._logger.info(
            f"Appel faire_suggestion: suspect='{suspect}', arme='{arme}', lieu='{lieu}'"
        )
        if not isinstance(self._state, EnqueteCluedoState):
            return json.dumps(
                {
                    "error": "La méthode faire_suggestion n'est disponible que pour EnqueteCluedoState."
                }
            )
        ...
        try:
            ...
            if indice_trouve:
                ...
                response = {
                    "indice": "Aucun des éléments de votre suggestion n'est connu par Watson. Continuez."
                }
...
module_logger.debug(
    "Module argumentation_analysis.orchestration.plugins.enquete_state_manager_plugin chargé."
)
