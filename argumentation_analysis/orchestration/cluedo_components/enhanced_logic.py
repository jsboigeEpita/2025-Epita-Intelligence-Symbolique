...
class EnhancedLogicHandler:
    ...
    def analyze_suggestion_quality(self, suggestion: str) -> Dict[str, Any]:
        ...
        trivial_keywords = [
            "je ne sais pas", "peut-être", "il faut chercher", "hmm", 
            "c'est difficile", "vraiment qui", "des indices", "quelqu'un avec", "à dire"
        ]
        
        for keyword in trivial_keywords:
            if keyword in suggestion_lower:
                return {"is_trivial": True, "reason": f"trivial_keyword_detected: {keyword}"}
        
        return {"is_trivial": False, "reason": "substantive_suggestion"}

    def trigger_auto_revelation(self, trigger_reason: str, context: str) -> Dict[str, Any]:
        """Déclenche une révélation automatique Enhanced."""
        if not self.oracle_state:
            return {"type": "auto_revelation", "success": False, "reason": "oracle_state_not_available"}
        
        moriarty_cards = self.oracle_state.get_moriarty_cards()
        if not moriarty_cards:
            return {"type": "auto_revelation", "success": False, "reason": "no_cards_available"}
        
        revealed_card = moriarty_cards[0]
        revelation_text = f"Révélation automatique Enhanced: Moriarty possède '{revealed_card}'"
        
        revelation = {
            "type": "auto_revelation", "success": True, "trigger_reason": trigger_reason,
            "context": context, "revealed_card": revealed_card, "revelation_text": revelation_text,
            "content": revelation_text, "auto_triggered": True, "oracle_strategy": self.oracle_strategy
        }
        
        revelation_record = RevelationRecord(
            card_revealed=revealed_card, revelation_type="auto_revelation",
            message=f"Auto-révélation: {revealed_card}", strategic_value=0.9,
            revealed_to="Enhanced_System", metadata={"trigger_reason": trigger_reason, "context": context}
        )
        self.oracle_state.add_revelation(revelation=revelation_record, revealing_agent="Enhanced_System")
        
        return revelation

    def handle_enhanced_state_transition(self, current_state: str, target_state: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Gère les transitions d'état pour le mode Enhanced."""
        valid_states = ["idle", "investigation_active", "suggestion_analysis", "auto_revelation_triggered", "solution_approaching"]
        if target_state not in valid_states:
            return {"success": False, "new_state": current_state, "enhanced_features_active": False, "error": f"Invalid target state: {target_state}"}
        ...
        return {
            "success": True, "new_state": target_state,
    ...
    async def execute_optimized_agent_turn(self, agent_name: str, turn_number: int, context: str) -> Dict[str, Any]:
        """Exécute un tour d'agent optimisé avec des rôles spécialisés."""
        role_mapping = {"Sherlock": "investigator", "Watson": "analyzer", "Moriarty": "oracle_revealer"}
        ...
        return {
            "role": agent_role, "action_type": agent_role.replace('_', ' '),
            "performance": {
            ...
            "turn_number": turn_number, "context": context, "success": True
        }