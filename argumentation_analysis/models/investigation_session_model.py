...
class SessionCheckpoint:
    ...
    def __post_init__(self):
        ...
        if not self.checkpoint_id:
            self.checkpoint_id = f"checkpoint_{uuid.uuid4().hex[:8]}_{int(self.timestamp.timestamp())}"
...
class SessionSummary:
    ...
    def calculate_success_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "evidence_utilization": len(self.evidence_processed) / max(1, len(self.evidence_processed)),
            "hypothesis_testing_rate": self.hypotheses_tested and len(self.hypotheses_tested) / max(1, self.validation_cycles)
        }
        ...
        overall_score = sum(metrics[metric] * weights.get(metric, 0) for metric in metrics)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "participating_agents": {agent: role.value for agent, role in self.participating_agents.items()},
...
class InvestigationSessionModel:
    ...
    auto_checkpoint_interval: timedelta = field(default_factory=lambda: timedelta(minutes=10))
    ...
    def get_active_investigators(self) -> List[str]:
        ...
        return [
            agent_id for agent_id in self.active_agents
    ...
    def start_session(self) -> bool:
        ...
        if self.status == SessionStatus.INITIALIZING:
            ...
            self.create_checkpoint("session_start", "Début de la session d'investigation")
    ...
    def start_phase(self, phase_name: str, description: str = "", 
                   phase_data: Dict[str, Any] = None) -> str:
    ...
    def add_session_belief(self, belief_id: str, belief_data: Dict[str, Any], 
                          agent_id: str) -> bool:
    ...
    def update_session_belief(self, belief_id: str, updates: Dict[str, Any], 
                            agent_id: str) -> bool:
        """Met à jour une croyance de session"""
        if belief_id in self.session_beliefs:
            self.session_beliefs[belief_id].update(updates)
            self.session_beliefs[belief_id]["last_modified"] = datetime.now().isoformat()
    ...
    def share_belief_with_agents(self, belief_id: str, target_agents: List[str]) -> bool:
    ...
    def add_hypothesis(self, hypothesis_id: str, hypothesis_data: Dict[str, Any], 
                      formulated_by: str) -> bool:
    ...
    def validate_hypothesis(self, hypothesis_id: str, validation_result: Dict[str, Any], 
    ...
    def link_evidence_to_hypothesis(self, evidence_id: str, hypothesis_id: str, 
                                   relationship: str = "supports") -> bool:
        """Lie une évidence à une hypothèse"""
        if evidence_id in self.available_evidence and hypothesis_id in self.hypotheses_under_test:
            
            # Mettre à jour l'évidence
            if hypothesis_id not in self.available_evidence[evidence_id]["related_hypotheses"]:
                self.available_evidence[evidence_id]["related_hypotheses"].append(hypothesis_id)
    ...
    def create_checkpoint(self, name: str, description: str = "", 
                         force: bool = False) -> Optional[SessionCheckpoint]:
    ...
    def calculate_session_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "beliefs_per_hour": self.session_statistics["beliefs_created"] / max(0.1, duration_hours),
            "validation_rate": (
            ...
            "agent_collaboration_score": len(self.active_agents) / max(1, len(self.registered_agents)),
            "evidence_utilization": len(self.available_evidence) / max(1, self.session_statistics["evidence_analyzed"]),
        ...
        normalized_beliefs_per_hour = min(1.0, metrics["beliefs_per_hour"] / 10.0)  # Assume 10/h max
        
        overall_score = (
            metrics["validation_rate"] * performance_weights["validation_rate"] +
            metrics["conflict_resolution_rate"] * performance_weights["conflict_resolution_rate"] +
            metrics["agent_collaboration_score"] * performance_weights["agent_collaboration_score"] +
            metrics["hypothesis_success_rate"] * performance_weights["hypothesis_success_rate"] +
            metrics["evidence_utilization"] * performance_weights["evidence_utilization"] +
    ...
    def complete_session(self, final_results: Dict[str, Any] = None) -> SessionSummary:
        ...
        summary = SessionSummary(
            ...
            total_synchronizations=self.session_statistics["synchronizations_performed"],
            average_confidence=final_metrics.get("validation_rate", 0.0),
            consistency_maintained=self.global_consistency_state.get("is_consistent", True),
            collaboration_efficiency=final_metrics.get("agent_collaboration_score", 0.0),
            evidence_processed=list(self.available_evidence.keys()),
            hypotheses_tested=list(self.hypotheses_under_test.keys()) + list(self.validated_conclusions.keys()),
    ...
    def to_dict(self, include_checkpoints: bool = False) -> Dict[str, Any]:
        ...
        result = {
            ...
            "registered_agents": {agent: role.value for agent, role in self.registered_agents.items()},
            "active_agents": list(self.active_agents),
            "lead_investigator": self.lead_investigator,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'InvestigationSessionModel':
        ...
        session = cls(
            ...
            end_time=end_time,
            max_duration=timedelta(seconds=data["max_duration"]) if data.get("max_duration") else None,
            auto_checkpoint_interval=timedelta(seconds=data.get("auto_checkpoint_interval", 600)),
        ...
        if "checkpoints" in data:
            session.checkpoints = [
                SessionCheckpoint.from_dict(cp_data) 
