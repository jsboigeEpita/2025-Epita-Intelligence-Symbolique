...
class AgentMessage:
    ...
    def _validate_payload(self):
        ...
        if message_type in required_fields:
            ...
            if missing_fields:
                raise ValueError(
                    f"Message {message_type.value} manque les champs: {missing_fields}"
                )
    ...
    def is_response_to(self, other_message: "AgentMessage") -> bool:
        ...
        return (
            self.metadata.reply_to == other_message.metadata.message_id
            or self.metadata.correlation_id == other_message.metadata.message_id
        )

    def create_response(
        self, response_payload: Dict[str, Any], response_type: MessageType = None
    ) -> "AgentMessage":
        ...
        if not response_type:
            ...
            response_type = response_mapping.get(
                self.metadata.message_type, MessageType.STATUS_UPDATE
            )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"AgentMessage({self.metadata.message_type.value}: "
...
class CommunicationProtocol:
    ...
    message_routing: Dict[MessageType, str] = field(
    ...
    def can_send_message(
        self, sender_id: str, message_type: MessageType
    ) -> Tuple[bool, str]:
    ...
    def validate_message(self, message: AgentMessage) -> Tuple[bool, List[str]]:
        ...
        if message_size > self.max_message_size:
            errors.append(
                f"Message trop volumineux: {message_size} > {self.max_message_size}"
            )
        ...
        can_send, reason = self.can_send_message(
            message.metadata.sender_id, message.metadata.message_type
        )
        if not can_send:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "message_routing": {
                mt.value: strategy for mt, strategy in self.message_routing.items()
            },
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CommunicationProtocol":
        """Création depuis un dictionnaire"""
        supported_message_types = [
            MessageType(mt) for mt in data.get("supported_message_types", [])
        ]
        message_routing = {
...
class SyncOperation:
    ...
    def fail(self, error_reason: str):
        ...
        self.log_event(
            "operation_failed",
            {"error_reason": error_reason, "progress_at_failure": self.progress},
        )
    ...
    def cancel(self, reason: str = ""):
        ...
        self.log_event(
            "operation_cancelled",
            {"reason": reason, "progress_at_cancellation": self.progress},
        )
    ...
    def update_progress(self, new_progress: float, message: str = ""):
        ...
        if message:
            self.log_event(
                "progress_update", {"progress": self.progress, "message": message}
            )
...
def create_critique_request_message(
    sender_id: str,
    receiver_id: str,
