...
class BeliefMetadata:
    ...
    def set_validation_result(
        self, agent_id: str, status: str, confidence: float = None
    ):
...
class ExtendedBeliefModel:
    ...
    def __post_init__(self):
        ...
        self.record_modification(
            "created", self.metadata.source_agent, "Croyance créée"
        )
    ...
    def validate(
        self, agent_id: str, validation_confidence: float = None, notes: str = ""
    ):
        """Valide la croyance"""
        self.valid = True
        self.metadata.set_validation_result(
            agent_id, "validated", validation_confidence
        )
    ...
    def add_dependency(self, belief_id: str, agent_id: str):
        ...
        if belief_id not in self.depends_on:
            ...
            self.record_modification(
                "dependency_added", agent_id, f"Dépendance ajoutée: {belief_id}"
            )
    ...
    def add_support(self, belief_id: str, agent_id: str):
        ...
        if belief_id not in self.supports:
            ...
            self.record_modification(
                "support_added", agent_id, f"Support ajouté: {belief_id}"
            )

    def derive_from_rule(
        self, rule_name: str, source_beliefs: List[str], agent_id: str
    ):
    ...
    def get_modification_summary(self) -> Dict[str, Any]:
        ...
        for mod in self.modification_history:
            ...
            summary["modification_types"][mod_type] = (
                summary["modification_types"].get(mod_type, 0) + 1
            )
    ...
    def is_consistent_with(self, other_belief: "ExtendedBeliefModel") -> Dict[str, Any]:
        ...
        if (
            self.belief_name.startswith("not_")
            and other_belief.belief_name == self.belief_name[4:]
        ) or (
            other_belief.belief_name.startswith("not_")
            and self.belief_name == other_belief.belief_name[4:]
        ):
        ...
        if self.valid is not None and other_belief.valid is not None:
            if (
                self.belief_name == other_belief.belief_name
                and self.valid != other_belief.valid
            ):
                consistency_report["is_consistent"] = False
    ...
    def to_dict(self, include_history: bool = True) -> Dict[str, Any]:
        ...
        if include_history:
            result["modification_history"] = [
                mod.to_dict() for mod in self.modification_history
            ]
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ExtendedBeliefModel(id='{self.belief_id}', "
