...
- `DungStudentProvider` (`dung_student_provider.py:37`) — la bibliothèque étudiant `abs_arg_dung/` comme fournisseur alternatif de la capability `dung_extensions` (#893) ; 4 sémantiques seulement (grounded/preferred/stable/complete, :29-34), `quality_score` 0.6 (:53-59), dispo conditionnée JVM (:65-90). (Les fonctions d'adaptateur registre `invoke_dung_student`/`register_dung_student_provider` ont été retirées, #2116 A2.)
...
- **Retirés (#2116 A2)** : `register_dung_student_provider` et `invoke_dung_student` — 0 appelant production (grep plein dépôt) ; l'accès réel instancie la classe directement. La classe reste active.
...
| `register_dung_student_provider` / `invoke_dung_student` | **retiré (#2116 A2)** | 0 appelant production, garde dédiée |
...
- ~~`register_dung_student_provider`/`invoke_dung_student`~~ retirés (#2116 A2) — la classe seule reste, accédée par instanciation directe ;
