...
class LLMFallacyDetector:
    ...
    SYSTEM_PROMPT = (
        ...
        "- Attaque personnelle (Ad Hominem)\n"
        "- Appel a la popularite (Ad Populum) — y compris la variante elitiste: "
        "'seuls les personnes eclairees/competentes pensent ainsi', "
        "'ceux qui comprennent vraiment savent que...'\n"
        "- Appel a l'emotion (Appeal to Emotion) — y compris le schema "
        "affectif 'drive→relief': creer une tension emotionnelle puis "
        "offrir la solution comme soulagement (peur→espoir, angoisse→reconfort, "
        "crise→salut)\n"
        "- Generalisation hative (Hasty Generalization)\n"
        "- Fausse causalite (False Cause)\n"
        "- Faux dilemme (False Dilemma)\n"
        "- Raisonnement circulaire (Circular Reasoning) — y compris la variante "
        "theorie-critique: la these se justifie en disqualifiant toute objection "
        "comme preuve de la these elle-meme (ex: 'si vous n'etes pas d'accord, "
        "c'est que vous etes sous l'emprise du systeme')\n"
        "- Pente glissante (Slippery Slope)\n"
