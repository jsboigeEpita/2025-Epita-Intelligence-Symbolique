...
class ConversationSimulator:
    ...
    def __init__(
        self, agent_names: List[str], default_agent: str = "ProjectManagerAgent"
    ):
        """
        ...
        self.state = RhetoricalAnalysisState(
            "Texte de simulation pour l'analyse rhétorique."
        )
        self.history = []

        logger.info(
            f"Simulateur initialisé avec {len(agent_names)} agents: {', '.join(agent_names)}"
        )
    ...
    def _create_real_agents(self, agent_names: List[str]) -> List[BaseAgent]:
        ...
        for name in agent_names:
            ...
            if (
                "informal" in name.lower()
                or "rhetorical" in name.lower()
                or "projectmanager" in name.lower()
            ):
                agent = InformalAnalysisAgent(kernel=mock_kernel, agent_name=name)
            elif "extract" in name.lower():
                agent = ExtractAgent(kernel=mock_kernel, agent_name=name)
            else:
    ...
    async def run_simulation(
        ...
        designation_bias: Dict[str, float] = None,
    ) -> Dict[str, List[float]]:
        """
        ...
        if designation_bias is None:
            designation_bias = {
                name: 1.0 / len(self.agent_names) for name in self.agent_names
            }

        # Normaliser le biais
        total_bias = sum(designation_bias.values())
        designation_bias = {
            name: bias / total_bias for name, bias in designation_bias.items()
        }

        logger.info(f"Démarrage de la simulation sur {num_turns} tours")
        logger.info(
            f"Probabilité de désignation explicite: {designation_probability:.2f}"
        )
        ...
        for turn in range(1, num_turns + 1):
            ...
            if np.random.random() < designation_probability:
                ...
                logger.debug(
                    f"Tour {turn}: Désignation explicite de {designated_agent}"
                )
            ...
            if turn % 10 == 0 or turn == num_turns:
                logger.info(
                    f"Tour {turn}/{num_turns} - Agent sélectionné: {selected_agent.name}"
                )
                current_rates = {
                    name: count / turn for name, count in participation_counts.items()
                }
    ...
    def plot_participation_history(
        self,
        ...
        output_path = os.path.join(
            os.path.dirname(__file__), "balanced_participation_simulation.png"
        )
...
async def run_standard_simulation():
    ...
    # Créer la stratégie
    strategy = BalancedParticipationStrategy(
        simulator.agents, simulator.state, target_participation=target_participation
    )
...
async def run_comparison_simulation():
    ...
    strategy1 = BalancedParticipationStrategy(
        simulator.agents, simulator.state, target_participation=equal_targets
    )
    ...
    strategy2 = BalancedParticipationStrategy(
        simulator.agents, simulator.state, target_participation=pm_dominant
    )

    # Exécuter les simulations
    logger.info("=== Simulation 1: Distribution équitable ===")
    history1 = await simulator.run_simulation(
        strategy1, num_turns=100, designation_probability=0.2
    )
    ...
    history2 = await simulator.run_simulation(
        strategy2, num_turns=100, designation_probability=0.2
    )

    # Afficher les résultats
    simulator.plot_participation_history(
        history1, equal_targets, title="Simulation avec distribution équitable"
    )

    simulator.plot_participation_history(
        history2, pm_dominant, title="Simulation avec PM dominant"
    )
