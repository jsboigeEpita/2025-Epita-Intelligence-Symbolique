...
from argumentation_analysis.ui.config import (
    ...
    CONFIG_FILE,
    CONFIG_FILE_JSON,
)  # Nécessaire pour load_extract_definitions_safely
...
async def main():
    ...
    fallback_file_path = (
        Path(CONFIG_FILE_JSON) if Path(CONFIG_FILE_JSON).exists() else None
    )

    # Vérifier que le fichier d'entrée existe
    if not input_file_path.exists() and not (
        fallback_file_path and fallback_file_path.exists()
    ):
        logger.error(
            f"Le fichier d'entrée {input_file_path_str} (et son fallback {CONFIG_FILE_JSON} si applicable) n'existe pas."
        )
        return

    # Charger les définitions d'extraits
    logger.info(
        f"Chargement des définitions d'extraits depuis {input_file_path_str}..."
    )
    try:
        ...
        if error_message:
            logger.error(
                f"Erreur lors du chargement des définitions d'extraits: {error_message}"
            )
            return
    ...
    if args.limit is not None and args.limit > 0:
        total_extracts_count = sum(
            len(source.get("extracts", [])) for source in extract_definitions_data
        )
        logger.info(
            f"Limitation du nombre d'extraits: {args.limit}/{total_extracts_count} extraits maximum."
        )
    ...
    final_invalid_count = sum(
        1 for result in results if result.get("status") == "invalid"
    )
    ...
    final_avg_coherence = (
        final_total_coherence / final_valid_count_for_avg
        if final_valid_count_for_avg > 0
        else 0
    )
    final_avg_relevance = (
        final_total_relevance / final_valid_count_for_avg
        if final_valid_count_for_avg > 0
        else 0
    )
    final_avg_integrity = (
        final_total_integrity / final_valid_count_for_avg
        if final_valid_count_for_avg > 0
        else 0
    )
    ...
    if final_invalid_count > 0:
        ...
        for result in results:
            if result.get("status") == "invalid":
                logger.info(
                    f"  - {result.get('source_name')} - {result.get('extract_name')}: {result.get('comments')}"
                )
    ...
    print(
        f"Résumé: {final_valid_count} valides, {final_invalid_count} invalides, {final_error_count} erreurs"
    )
    print(
        f"Scores moyens: Cohérence {final_avg_coherence:.2f}/5, Pertinence {final_avg_relevance:.2f}/5, Intégrité {final_avg_integrity:.2f}/5"
    )
