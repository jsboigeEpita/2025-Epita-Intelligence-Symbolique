...
1. **Phase de workflow via CLI (opt-in).** `run_orchestration.py:447-451` déclare `--shield-preset` avec `choices=["off","basic","advanced","output_only","strict"]`, défaut `off` (`:451`). Le `shield_config` n'est construit que si le preset diffère de `off` (`:214-218`), avec `"fail_open": shield_preset != "strict"` (`:217`). `unified_pipeline.py:378` teste `context.get("shield_config")` et appelle alors `_inject_shield_phase` (`unified_pipeline.py:89-119`), qui barre **réellement** le DAG : chaque phase **racine** du workflow reçoit `depends_on=["shield"]` (copie via `dataclasses.replace`, jamais de mutation), donc `shield` est seul au niveau 0 et toute racine originale part un niveau plus tard — la phase est `optional=True` (`:103`), résolue par capacité `input_validation` vers `ai_shield_service` (`registry_setup.py:740-753`) et exécutée par `_invoke_ai_shield` (`invoke_callables.py:10369`). La valeur est réellement transmise par `run_orchestration.py:879`.
   ```bash
...
| capacité `input_validation` | **demandée** (1 demandeur) | `unified_pipeline.py:111` |
...
| `ai_shield_results` | **lu en production depuis #2095** (lecture seule) | écrit `invoke_callables.py:10440`, lu `unified_pipeline.py:56`, déclaré `core/shared_state.py:613` |
| verdict `blocked` dans le pipeline | **décide** : phase terminale, dépendantes SKIPPED | `workflow_dsl.py:987` (helper `:730`, motif nommé `:543`) |
...
- **Artefact produit (phase)** : le dict de verdict — `shield_available`, `blocked`, `overall_score`, `passed`, `reason`, `layer_results` (`invoke_callables.py:10418-10435`) — empilé dans `state.ai_shield_results`, d'où `unified_pipeline._shield_verdict` le relit en lecture seule et le rend sous `shield_verdict` (`unified_pipeline.py:462`).
...
> dépendantes passent SKIPPED avec un motif qui nomme le bouclier (`:543`) — et
> l'injection barre **réellement** le DAG (`_inject_shield_phase`,
> `unified_pipeline.py:89-119` : chaque racine reçoit `depends_on=["shield"]`, le
> bouclier est seul au niveau 0) ; côté REST la
...
> (`unified_pipeline._shield_verdict`, `unified_pipeline.py:56`), qui n'ajoute jamais.
> Les items **6, 12, 13** restent ouverts tels que décrits. **Attribution des ancres,
> mesurée** : #2095 a déplacé `llm_validator.py` (+8), `unified_pipeline.py` (+33 puis
> +25 au reprise R995), `workflow_dsl.py` et le corps d'`invoke_callables.py` (+5) —
> celles-ci sont re-mesurées ; en revanche l'ancre du `def _invoke_ai_shield`
> (`:10339`→`:10369`) et les ancres `shield.py` (`:146`→`:154`, etc.) étaient
> **périmées avant #2095** (déplacées par des PR antérieures) : elles sont **rafraîchies**,
> pas déplacées par cette PR. Le résidu d'`ImportError` des deux portes est nommé
...
10. **Verdict sans effet** *(**fermé par #2095** dans le pipeline ; inchangé côté REST)* — un texte bloqué n'interrompait ni le pipeline (phase `optional=True`, `unified_pipeline.py:103` ; et avant la reprise R995, aucune arête ne reliait le bouclier aux racines : il était **frère** de elles au niveau 0) ni le flux REST (la réponse décrit le blocage, l'appelant décide). **Corrigé pour le pipeline** : l'injection barre le DAG (`_inject_shield_phase`, `unified_pipeline.py:89-119` — chaque racine reçoit `depends_on=["shield"]`), la phase porte `terminal` (`workflow_dsl.py:987`, helper `:730`) et ses dépendantes passent SKIPPED avec un motif qui nomme le bouclier (`:543`) au lieu de la classification #1909 qui emprunte le même chemin. Le flux REST garde son contrat : il *décrit*, l'appelant décide.
11. **`ai_shield_results` déclaré-jamais-lu** *(**fermé par #2095**)* (`core/shared_state.py:613`) — le rapport d'exécution ne pouvait pas relire un blocage depuis l'état ; les 9 dumps qui portent le champ le portent vide. **Corrigé** : `unified_pipeline._shield_verdict` (`unified_pipeline.py:56`) est un lecteur de production en **lecture seule** — il n'ajoute jamais, car une phase bouclier peut tourner deux fois dans un même workflow et un lecteur qui écrirait doublerait la liste.
12. **Docs d'intégration périmées ou incomplètes** :
    ...
    - `docs/architecture/NORTHSTAR_GAP_ANALYSIS.md:124` et `:290`, ainsi que `docs/architecture/design_specs/TRACK-03_shield_preset_selector.md:38-40`, citent des **ancres de ligne périmées** (`invoke_callables.py:6393` et `:6118` pour un callable qui est à `:10369` ; `registry_setup.py:660` pour une déclaration à `:740` ; `shared_state.py:452` pour un champ à `:613`). Dérive préexistante, non introduite ici.
    - `docs/reports/integration_audit/A-13_ai_shield.md:69` affirme le service « complètement inutilisable dans le pipeline normal » — démenti par `unified_pipeline.py:378-390` et `run_orchestration.py:447-451`.
