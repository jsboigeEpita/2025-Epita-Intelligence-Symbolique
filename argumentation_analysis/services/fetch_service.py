...
class FetchService:
    ...
    def __init__(
        ...
        self.logger.info(
            f"FetchService initialisé avec Tika URL: {self.tika_server_url}, timeout: {self.tika_server_timeout}s"
        )
        ...
        if self.temp_download_dir:
            ...
            self.logger.info(
                f"Répertoire temporaire initialisé: {self.temp_download_dir}"
            )

    def reconstruct_url(
        self, schema: str, host_parts: List[str], path: str
    ) -> Optional[str]:
        """
    ...
    def fetch_text(
        ...
        try:
            if source_type == "jina":
                ...
                is_plaintext = any(
                    path.lower().endswith(ext) for ext in self.plaintext_extensions
                )
                if is_plaintext:
    ...
    def fetch_direct_text(self, url: str, timeout: int = 60) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(
                f"Erreur persistante de téléchargement direct pour {url}: {e}"
            )
    ...
    def fetch_with_jina(self, url: str, timeout: int = 90) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(
                f"Disjoncteur ouvert pour fetch_with_jina sur {jina_url}."
            )
            return None
        except requests.exceptions.RequestException as e:
    ...
    def fetch_with_tika(
        ...
        if url:
            ...
            if any(url.lower().endswith(ext) for ext in self.plaintext_extensions):
                self.logger.info(
                    f"URL détectée comme texte simple ({url}). Fetch direct."
                )
            ...
            if self.temp_download_dir:
                effective_raw_cache_path = (
                    ...
                    else self.temp_download_dir / f"{url_hash}{file_extension}"
                )
            ...
            if content_to_send is None:
                ...
                try:
                    response_dl = self._robust_get_request(
                        url, stream=True, timeout=timeout_dl
                    )
                    if response_dl is None:
                        return None
            ...
            # Vérifier si c'est un fichier texte
            if any(
                file_name.lower().endswith(ext) for ext in self.plaintext_extensions
            ):
                self.logger.info(
            ...
            self.logger.error(
                "fetch_with_tika: Il faut soit source_url soit file_content."
            )
            return None
        ...
        # Envoyer à Tika
        self.logger.info(
            f"Envoi contenu à Tika ({self.tika_server_url})... (Timeout={effective_timeout}s)"
        )
        ...
        try:
            response_tika = self._robust_put_request(
                self.tika_server_url,
            ...
            self.logger.error(
                f"Échec de l'envoi à Tika après plusieurs tentatives: {e}"
            )
            return None
        except Exception as e:
            # Catchall pour toute autre erreur inattendue
            self.logger.error(
                f"Erreur inattendue lors de la communication avec Tika: {e}",
                exc_info=True,
            )
            return None
