...
class JTMSSessionManager:
    ...
    def __init__(self, jtms_service: JTMSService, storage_path: str = "./logs/sessions"):
    ...
    async def create_session(self, agent_id: str, session_name: str = None, 
    ...
    async def list_sessions(self, agent_id: Optional[str] = None, 
    ...
    async def create_checkpoint(self, session_id: str, description: str = None, 
                              auto_generated: bool = False) -> str:
        """
        ...
        checkpoint_data = {
            ...
            "session_id": session_id,
            "created_at": datetime.now().isoformat(),
            "description": description or f"Checkpoint automatique {datetime.now().strftime('%H:%M:%S')}",
        ...
        if len(self.checkpoints[session_id]) > self.max_checkpoints_per_session:
            ...
            non_initial_checkpoints = [cp for cp in self.checkpoints[session_id] 
                                     if not cp.get("auto_generated") or cp["description"] != "session_created"]
            if len(non_initial_checkpoints) > self.max_checkpoints_per_session - 1:
                non_initial_checkpoints.sort(key=lambda x: x["created_at"])
                to_remove = non_initial_checkpoints[:(len(non_initial_checkpoints) - self.max_checkpoints_per_session + 1)]
                for cp_to_remove in to_remove:
                    self.checkpoints[session_id].remove(cp_to_remove)
        
        # Mettre à jour la session
        self.sessions[session_id]["checkpoint_count"] = len(self.checkpoints[session_id])
        self.sessions[session_id]["last_accessed"] = datetime.now().isoformat()
    ...
    async def restore_checkpoint(self, session_id: str, checkpoint_id: str) -> bool:
        ...
        for old_instance_id, state_data in target_checkpoint["jtms_states"].items():
            ...
            new_instance_id = await self.jtms_service.import_jtms_state(session_id, state_json)
        ...
        self.sessions[session_id]["metadata"] = target_checkpoint["session_metadata"].copy()
        self.sessions[session_id]["last_accessed"] = datetime.now().isoformat()
        
        # Créer un checkpoint de restauration
        await self.create_checkpoint(
            session_id, 
            f"Restauré depuis {checkpoint_id}", 
            auto_generated=True
        )
    ...
    async def add_jtms_instance_to_session(self, session_id: str, instance_id: str) -> bool:
        """
    ...
    async def _load_checkpoints_for_session(self, session_id: str):
        ...
        for checkpoint_file in checkpoint_files:
            ...
            if not any(cp["checkpoint_id"] == checkpoint_data["checkpoint_id"] 
