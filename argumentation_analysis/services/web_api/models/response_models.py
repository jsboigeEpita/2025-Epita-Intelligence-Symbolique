...
class FallacyDetection(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confiance de la détection (0-1)"
    )
    location: Optional[Dict[str, int]] = Field(
        default=None, description="Position dans le texte"
    )
    context: Optional[str] = Field(default=None, description="Contexte du sophisme")
    explanation: Optional[str] = Field(
        default=None, description="Explication détaillée"
    )
...
class ArgumentStructure(BaseModel):
    ...
    premises: List[str] = Field(
        default_factory=list, description="Prémisses identifiées"
    )
    ...
    strength: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Force de l'argument"
    )
    coherence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Cohérence de l'argument"
    )
...
class AnalysisResponse(BaseModel):
    ...
    analysis_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de l'analyse"
    )

    # Résultats de l'analyse
    fallacies: List[FallacyDetection] = Field(
        default_factory=list, description="Sophismes détectés"
    )
    argument_structure: Optional[ArgumentStructure] = Field(
        default=None, description="Structure argumentative"
    )

    # Métriques globales
    overall_quality: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Qualité globale"
    )
    coherence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Score de cohérence"
    )
    fallacy_count: int = Field(default=0, ge=0, description="Nombre de sophismes")

    # Métadonnées
    processing_time: float = Field(
        default=0.0, ge=0.0, description="Temps de traitement (secondes)"
    )
...
class ValidationResult(BaseModel):
    ...
    premise_analysis: List[Dict[str, Any]] = Field(
        default_factory=list, description="Analyse des prémisses"
    )
    conclusion_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="Analyse de la conclusion"
    )
    logical_structure: Dict[str, Any] = Field(
        default_factory=dict, description="Structure logique"
    )
...
class ValidationResponse(BaseModel):
    ...
    validation_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la validation"
    )
...
class FallacyResponse(BaseModel):
    ...
    detection_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la détection"
    )
    ...
    severity_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Distribution par sévérité"
    )
    category_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Distribution par catégorie"
    )
...
class Extension(BaseModel):
    ...
    arguments: List[str] = Field(
        default_factory=list, description="Arguments dans l'extension"
    )
...
class FrameworkVisualization(BaseModel):
    ...
    nodes: List[Dict[str, Any]] = Field(
        default_factory=list, description="Nœuds pour la visualisation"
    )
    edges: List[Dict[str, Any]] = Field(
        default_factory=list, description="Arêtes pour la visualisation"
    )
    layout: Dict[str, Any] = Field(
        default_factory=dict, description="Configuration du layout"
    )
...
class FrameworkResponse(BaseModel):
    ...
    framework_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la construction"
    )

    # Structure du framework
    arguments: List[ArgumentNode] = Field(
        default_factory=list, description="Arguments du framework"
    )
    attack_relations: List[Dict[str, str]] = Field(
        default_factory=list, description="Relations d'attaque"
    )
    support_relations: List[Dict[str, str]] = Field(
        default_factory=list, description="Relations de support"
    )

    # Extensions calculées
    extensions: List[Extension] = Field(
        default_factory=list, description="Extensions calculées"
    )
    ...
    # Visualisation
    visualization: Optional[FrameworkVisualization] = Field(
        default=None, description="Données de visualisation"
    )
...
class ErrorResponse(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        default=None, description="Détails supplémentaires"
    )
...
class LogicQueryResult(BaseModel):
    ...
    result: Optional[bool] = Field(
        ..., description="Résultat de la requête (True, False ou None)"
    )
    formatted_result: str = Field(..., description="Résultat formaté")
    explanation: Optional[str] = Field(
        default=None, description="Explication du résultat"
    )
...
class LogicBeliefSet(BaseModel):
    ...
    creation_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de création"
    )
...
class LogicBeliefSetResponse(BaseModel):
    ...
    conversion_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la conversion"
    )
...
class LogicQueryResponse(BaseModel):
    ...
    query_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de l'exécution"
    )
    ...
    query_options: Dict[str, Any] = Field(
        default_factory=dict, description="Options utilisées"
    )
...
class LogicGenerateQueriesResponse(BaseModel):
    ...
    generation_timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la génération"
    )
...
class SuccessResponse(BaseModel):
    ...
    message: str = Field(
        default="Operation successful", description="Message de succès"
    )
    data: Optional[Any] = Field(
        default=None, description="Données retournées par l'opération"
    )
    status_code: int = Field(default=200, description="Code de statut HTTP")
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de la réponse"
    )
