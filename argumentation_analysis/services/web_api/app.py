    ...
    def before_request_hook():
        ...
        if not getattr(g, 'services_initialized', False):
            if not flask_app_instance.services_initialized:
                ...
                try:
                    logger.info("[DEADLOCK_DEBUG] ÉTAPE 1: Appel de initialize_project_environment...")
                    project_context = initialize_project_environment()
                    logger.info("[DEADLOCK_DEBUG] ÉTAPE 1: ... initialize_project_environment TERMINÉ.")
                    flask_app_instance.project_context = project_context
                    
                    logger.info("[DEADLOCK_DEBUG] ÉTAPE 2: Imports locaux des services...")
                    ...
                    logger.info("[DEADLOCK_DEBUG] ÉTAPE 2: ... Imports locaux TERMINÉS.")

                    class AppServices:
                        """Conteneur pour les instances de service, initialisé paresseusement."""
                        def __init__(self, proj_context):
                            logger.info("[DEADLOCK_DEBUG] Initializing AppServices container...")
                            self.project_context = proj_context
                            is_integration_test = os.environ.get("INTEGRATION_TEST_MODE") == "true"
                            
                            logger.info("[DEADLOCK_DEBUG] Création du logic_llm_service...")
                            logic_llm = create_llm_service(service_id="logic_service", model_id="gpt-4o-mini", force_mock=is_integration_test)
                            logger.info("[DEADLOCK_DEBUG] ... logic_llm_service créé.")
                            
                            logger.info("[DEADLOCK_DEBUG] Création du analysis_llm_service...")
                            analysis_llm = create_llm_service(service_id="analysis_service", model_id="gpt-4o-mini", force_mock=is_integration_test)
                    ...
                    logger.info("[DEADLOCK_DEBUG] Hook 'before_request': Initialisation terminée avec succès.")
                except Exception as e:
                    logger.critical(f"Échec critique de l'initialisation des services: {e}", exc_info=True)
                    return jsonify({"error": "Service Unavailable", "message": "Critical components failed to initialize."}), 503
