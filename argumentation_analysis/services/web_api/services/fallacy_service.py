...
class FallacyService:
        ...
        try:
            ...
            if EnhancedContextualAnalyzer:
                self.enhanced_analyzer = EnhancedContextualAnalyzer(
                    fallacy_detector=self.base_fallacy_detector
                )
    ...
    def detect_fallacies(self, request: FallacyRequest) -> FallacyResponse:
        ...
        try:
            ...
            if self.contextual_analyzer and (
                not request.options or getattr(request.options, "use_contextual", True)
            ):
                fallacies.extend(
                    self._detect_with_contextual_analyzer(request.text, request.options)
                )

            if self.enhanced_analyzer and (
                not request.options or getattr(request.options, "use_enhanced", True)
            ):
                fallacies.extend(
                    self._detect_with_enhanced_analyzer(request.text, request.options)
                )

            # Détection avec les patterns intégrés
            if not request.options or getattr(request.options, "use_patterns", True):
                fallacies.extend(
                    self._detect_with_patterns(request.text, request.options)
                )
    ...
    def _detect_with_contextual_analyzer(
        self, text: str, options: Optional[FallacyOptions]
        ...
        try:
            if self.contextual_analyzer:
                ...
                results = self.contextual_analyzer.identify_contextual_fallacies(
                    text, "general"
                )
    ...
    def _detect_with_enhanced_analyzer(
        self, text: str, options: Optional[FallacyOptions]
    ) -> List[FallacyDetection]:
        ...
        try:
            if self.enhanced_analyzer:
                ...
                if hasattr(self.enhanced_analyzer, "analyze_fallacies"):
                    ...
                    results = self.enhanced_analyzer.identify_contextual_fallacies(
                        text, "general"
                    )
                else:
                    self.logger.warning(
                        "Enhanced analyzer n'a pas de méthode d'analyse appropriée"
                    )
                    results = []
    ...
    def _detect_with_patterns(
        ...
        print(
            f"[DEBUG] Nombre de patterns dans fallacy_patterns: {len(self.fallacy_patterns)}"
        )
        print(
            f"[DEBUG] Nombre de patterns dans fallacy_patterns: {len(self.fallacy_patterns)}"
        )
            ...
            for fallacy_type, fallacy_info in patterns_source.items():
                ...
                for i, pattern in enumerate(fallacy_info["patterns"]):
                    ...
                    if self._pattern_matches(pattern, text_lower):
                        print(
                            f"[DEBUG] MATCH trouvé pour {fallacy_type} avec pattern: {pattern}"
                        )
                        position = text_lower.find(
                            pattern.split(".*")[0] if ".*" in pattern else pattern
                        )
    ...
    def _extract_context(
        self, text: str, position: int, context_size: int = 50
    ) -> Optional[str]:
    ...
    def _filter_and_deduplicate(
        self, fallacies: List[FallacyDetection], options: Optional[FallacyOptions]
    ) -> List[FallacyDetection]:
        ...
        threshold = (
            ...
            if options and options.severity_threshold is not None
            else 0.5
        )
        ...
        if options and options.categories:
            filtered = [
                f
                for f in filtered
                if f.type in options.categories or f.name in options.categories
            ]
        ...
        for fallacy in filtered:
            key = (
                fallacy.type,
                fallacy.location.get("start", 0) if fallacy.location else 0,
            )
            if key not in seen:
                seen.add(key)
        ...
        max_fallacies = (
            options.max_fallacies
            if options and options.max_fallacies is not None
            else 10
        )
