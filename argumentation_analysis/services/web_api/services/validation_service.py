...
class ValidationService:
    ...
    async def validate_argument(self, request: ValidationRequest) -> ValidationResponse:
        ...
        try:
            ...
            if not request.premises:
                raise ValueError(
                    "Aucune prémisse fournie. Un argument valide nécessite au moins une prémisse."
                )

            if not request.conclusion or not request.conclusion.strip():
                raise ValueError(
                    "Aucune conclusion fournie. Un argument valide doit avoir une conclusion claire."
                )
            ...
            result = ValidationResult(
                ...
                issues=[str(ve)],
                suggestions=[
                    "Vérifiez que vous avez fourni au moins une prémisse et une conclusion claire."
                ],
            )
            ...
            result = ValidationResult(
                ...
                logical_structure={},
                issues=[
                    f"Une erreur s'est produite lors de l'analyse de votre argument : {str(e)}"
                ],
                suggestions=[
    ...
    def _analyze_logical_structure(
        self, premises: List[str], conclusion: str, argument_type: Optional[str]
    ) -> Dict[str, Any]:
    ...
    def _calculate_validity_score(
        self,
        premise_analysis: List[Dict[str, Any]],
        conclusion_analysis: Dict[str, Any],
        ...
        try:
            if not premise_analysis:
                return 0.0  # Eviter division par zéro
            # Score basé sur la force des prémisses
            premise_strength = sum(p["strength"] for p in premise_analysis) / len(
                premise_analysis
            )
    ...
    def _calculate_soundness_score(
        self, premise_analysis: List[Dict[str, Any]], validity_score: float
    ) -> float:
        ...
        try:
            ...
            credibility_avg = sum(
                p["credibility_score"] for p in premise_analysis
            ) / len(premise_analysis)
    ...
    def _is_factual_claim(self, text: str) -> bool:
        ...
        """
        return not any(
            word in text.lower() for word in ["devrait", "pourrait", "opinion", "crois"]
        )
    ...
    def _assess_conclusion_strength(self, conclusion: str) -> float:
        ...
        """
        return (
            self._assess_clarity(conclusion) + self._assess_specificity(conclusion)
        ) / 2
    ...
    def _identify_issues(
        ...
        if not premise_analysis:
            ...
            if unclear_premises:
                issues.append(
                    f"{len(unclear_premises)} prémisse(s) manque(nt) de clarté. Reformulez-les pour les rendre plus explicites."
                )

            low_credibility = [
                p for p in premise_analysis if p["credibility_score"] < 0.4
            ]
            if low_credibility:
                issues.append(
                    f"{len(low_credibility)} prémisse(s) manque(nt) de crédibilité. Ajoutez des sources ou des preuves pour les renforcer."
                )
        ...
        if structure.get("logical_flow", 0.0) < 0.4:
            issues.append(
                "Le raisonnement manque de fluidité logique. Utilisez des connecteurs logiques appropriés pour lier vos prémisses à votre conclusion."
            )
        ...
        if structure.get("consistency", 0.0) < 0.4:
            issues.append(
                "Les prémisses sont contradictoires entre elles. Assurez-vous que vos prémisses ne se contredisent pas."
            )
    ...
    def _generate_suggestions(
        ...
        if not issues:
            suggestions.append(
                "Votre argument est bien structuré. Pour le renforcer davantage, vous pourriez ajouter des exemples concrets ou des contre-arguments."
            )
            return suggestions
        ...
        for issue in issues:
            if "prémisse" in issue.lower():
                if "clarté" in issue.lower():
                    suggestions.append(
                        "Pour améliorer la clarté de vos prémisses : utilisez des phrases courtes et directes, évitez le jargon inutile, et définissez les termes techniques."
                    )
                elif "crédibilité" in issue.lower():
                    suggestions.append(
                        "Pour renforcer la crédibilité de vos prémisses : citez des sources fiables, utilisez des statistiques vérifiables, ou appuyez-vous sur des faits établis."
                    )
                elif "pertinence" in issue.lower():
                    suggestions.append(
                        "Pour améliorer la pertinence de vos prémisses : assurez-vous que chaque prémisse contribue directement à votre conclusion, et éliminez les informations non essentielles."
                    )

            elif "conclusion" in issue.lower():
                if "clarté" in issue.lower():
                    suggestions.append(
                        "Pour clarifier votre conclusion : utilisez des termes précis, évitez les ambiguïtés, et assurez-vous qu'elle découle logiquement de vos prémisses."
                    )
                else:
                    suggestions.append(
                        "Votre conclusion devrait être clairement liée à vos prémisses. Utilisez des connecteurs logiques comme 'donc', 'par conséquent', ou 'ainsi'."
                    )

            elif "logique" in issue.lower():
                if "fluidité" in issue.lower():
                    suggestions.append(
                        "Pour améliorer la fluidité logique : utilisez des connecteurs logiques appropriés, structurez vos idées de manière progressive, et assurez-vous que chaque étape découle naturellement de la précédente."
                    )
                elif "incomplet" in issue.lower():
                    suggestions.append(
                        "Pour compléter votre argument : ajoutez des prémisses intermédiaires qui renforcent le lien entre vos prémisses principales et votre conclusion."
                    )
                elif "contradictoire" in issue.lower():
                    suggestions.append(
                        "Pour résoudre les contradictions : revoyez vos prémisses pour vous assurer qu'elles sont cohérentes entre elles, et reformulez-les si nécessaire."
                    )

        argument_type = structure.get("argument_type")
        if argument_type == "deductive":
            suggestions.append(
                "Pour un argument déductif, assurez-vous que vos prémisses sont universellement vraies et que votre conclusion en découle nécessairement."
            )
        elif argument_type == "inductive":
            suggestions.append(
                "Pour un argument inductif, renforcez vos prémisses avec des exemples variés et représentatifs pour augmenter la probabilité de votre conclusion."
            )

        # S'assurer qu'il y a toujours au moins une suggestion si des problèmes ont été trouvés
        if issues and not suggestions:
            suggestions.append(
                "Examinez les problèmes identifiés pour améliorer la structure et la clarté de votre argument."
            )
