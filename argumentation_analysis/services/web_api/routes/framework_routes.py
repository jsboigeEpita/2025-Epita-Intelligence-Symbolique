...
from argumentation_analysis.services.web_api.models.request_models import FrameworkAnalysisRequest
from argumentation_analysis.services.web_api.models.response_models import SuccessResponse, ErrorResponse
...
def analyze_framework_route():
    ...
    if not request.is_json:
        return jsonify(ErrorResponse(error="Invalid input", message="Request must be JSON", status_code=400).dict()), 400
    ...
    try:
        ...
        return jsonify(ErrorResponse(error="Framework analysis failed", message=str(e), status_code=500).dict()), 500