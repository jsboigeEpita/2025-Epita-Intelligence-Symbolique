...
from argumentation_analysis.core.communication.channel_interface import ChannelInterface
from argumentation_analysis.core.communication.agent_adapter import AgentAdapter
...
class AlertChannel(ChannelInterface):
    ...
    def __init__(self, name, config=None):
        ...
        # Statistiques
        self.stats = {"alerts_sent": 0, "alerts_received": 0, "active_subscriptions": 0}
    ...
    def send_message(self, message):
        ...
        with self.lock:
            ...
            if message.type != ExtendedMessageType.ALERT.value:
                self.logger.warning(
                    f"Message de type {message.type} reçu sur le canal d'alertes"
                )
                return False
            ...
            self.logger.info(
                f"Alerte de niveau {alert_level} de type {alert_type} envoyée"
            )
        ...
        with self.lock:
            ...
            if recipient_id in self.alerts:
                pending_alerts.extend(
                    [alert["message"] for alert in self.alerts[recipient_id]]
                )
    ...
    def subscribe(self, subscriber_id, filter_criteria=None):
        ...
        with self.lock:
            ...
            self.logger.info(
                f"Agent {subscriber_id} abonné aux alertes avec ID {subscription_id}"
            )
    ...
    def get_statistics(self):
        ...
        with self.lock:
            ...
            stats["pending_alerts"] = sum(
                len(alerts) for alerts in self.alerts.values()
            )
...
def main():
    ...
    alert_channel = AlertChannel(
        name="alert", config={"retention_period": 7200}  # 2 heures
    )
    ...
    tactical_adapter._get_agent_level = (
        lambda: AgentLevel.TACTICAL
    )  # Méthode temporaire pour l'exemple
    ...
    def visualization_agent():
        ...
        subscription_id = alert_channel.subscribe(
            ...
            filter_criteria={"alert_level": ["critical", "warning"]},
        )
        ...
        graph_data = {
            "nodes": [
                {
                    "id": "p1",
                    "label": "Tous les hommes sont mortels",
                    "type": "premise",
                },
    ...
    def tactical_agent():
        ...
        visualization = tactical_adapter.receive_message(
            timeout=5.0, filter_criteria={"content.info_type": "visualization"}
        )
        ...
        alert = tactical_adapter.receive_message(
            timeout=5.0, filter_criteria={"type": ExtendedMessageType.ALERT.value}
        )
