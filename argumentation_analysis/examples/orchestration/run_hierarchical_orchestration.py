...
class HierarchicalOrchestrator:
    ...
    def __init__(self):
        ...
        if not self.llm_service:
            self.logger.error(
                "Échec de la création du service LLM pour HierarchicalOrchestrator."
            )
        ...
        self.logger.info(
            f"Kernel initialisé avec le service LLM: {self.llm_service_id}"
        )
        ...
        self.strategic_tactical_interface = StrategicTacticalInterface(
            strategic_state=self.strategic_state, tactical_state=self.tactical_state
        )

        self.tactical_operational_interface = TacticalOperationalInterface(
            tactical_state=self.tactical_state, operational_state=self.operational_state
        )
    ...
    async def initialize_all_agents(
        self,
    ):  # Cette méthode va maintenant appeler get_agent pour chaque type
        """Initialise tous les types d'agents connus via le registre."""
        self.logger.info(
            "Pré-initialisation de tous les types d'agents via le registre..."
        )
        ...
        for agent_type in agent_types:
            ...
            if agent:
                self.logger.info(
                    f"Agent de type '{agent_type}' initialisé ou récupéré."
                )
            else:
                self.logger.error(
                    f"Échec de l'initialisation de l'agent de type '{agent_type}'."
                )

        self.logger.info(
            "Tous les types d'agents ont été sollicités pour initialisation."
        )

    async def analyze_text(
        self, text: str, analysis_type: str = "complete"
    ) -> Dict[str, Any]:
        """
        ...
        tactical_directives = self.strategic_tactical_interface.translate_objectives(
            objectives
        )
        ...
        strategic_report = self.strategic_tactical_interface.process_tactical_report(
            tactical_report
        )
    ...
    def _create_final_report(
        self, strategic_report: Dict[str, Any], tactical_report: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        final_report = {
            ...
            "quality_score": strategic_report["metrics"]["quality_indicators"][
                "quality_score"
            ],
            RESULTS_DIR: {
                ...
                "coherence_evaluation": self._extract_coherence_evaluation(
                    task_results
                ),
            },
            "metrics": {
    ...
    def _extract_coherence_evaluation(
        self, task_results: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyse rhétorique utilisant l'architecture hiérarchique"
    )
    ...
    parser.add_argument(
        "--type",
        "-y",
        choices=["complete", "fallacies", "arguments", "formal"],
        default="complete",
        help="Type d'analyse à réaliser",
    )
    parser.add_argument(
    ...
    if args.output:
        ...
        for arg in results[RESULTS_DIR]["arguments"]:
            print(
                f"- {arg['id']}: {arg['conclusion']} (confiance: {arg['confidence'] * 100:.1f}%)"
            )

        print("\nSophismes détectés:")
        for fallacy in results[RESULTS_DIR]["fallacies"]:
            print(
                f"- {fallacy['id']}: {fallacy['type']} (confiance: {fallacy['confidence'] * 100:.1f}%)"
            )
