...
async def run_rhetorical_analysis_example():
    ...
    strategic_tactical_interface = StrategicTacticalInterface(
        strategic_state=strategic_state, tactical_state=tactical_state
    )

    tactical_operational_interface = TacticalOperationalInterface(
        tactical_state=tactical_state, operational_state=operational_state
    )
    ...
    if result["status"] == "completed" or result["status"] == "completed_with_issues":
        ...
        if complex_fallacy_results:
            ...
            for result in complex_fallacy_results:
                ...
                logger.info(
                    f"  - Sophismes individuels: {analysis_results.get('individual_fallacies_count', 0)}"
                )
                logger.info(
                    f"  - Combinaisons de base: {len(analysis_results.get('basic_combinations', []))}"
                )
                logger.info(
                    f"  - Combinaisons avancées: {len(analysis_results.get('advanced_combinations', []))}"
                )

        # Afficher les résultats de l'analyse des sophismes contextuels
        contextual_fallacy_results = [
            output
            for output in outputs
            if output.get("type") == "contextual_fallacy_analysis"
        ]
        if contextual_fallacy_results:
            ...
            for result in contextual_fallacy_results:
                analysis_results = result.get("analysis_results", {})
                logger.info(
                    f"  - Sophismes identifiés: {len(analysis_results.get('identified_fallacies', []))}"
                )
                logger.info(
                    f"  - Facteurs contextuels: {analysis_results.get('context_factors', {})}"
                )
        ...
        strategic_state.update_rhetorical_analysis_summary(
            {
                "complex_fallacy_summary": {
                    "total_fallacies": sum(
                        len(
                            output.get("analysis_results", {}).get(
        ...
        logger.error(
            f"Erreur lors du traitement de la tâche: {result.get('issues', [])}"
        )
