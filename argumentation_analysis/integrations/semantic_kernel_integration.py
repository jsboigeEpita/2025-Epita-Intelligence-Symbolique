...
""",
    ...
    def _add_reasoning_function(self, name: str, template: str):
        ...
        reasoning_function = self.kernel.create_function_from_prompt(
            prompt_config=prompt_config, plugin_name="jtms_reasoning"
        )

    async def create_reasoning_session(
        self, agent_id: str = None, session_name: str = None
    ) -> tuple[str, str]:
        """
    ...
    async def analyze_argument_with_llm(
        self, argument: str, session_id: str = None, instance_id: str = None
    ) -> Dict[str, Any]:
        """
    ...
    async def resolve_contradiction_with_llm(
        ...
        non_monotonic_beliefs = [
            belief_name
            for belief_name, belief_data in current_state["beliefs"].items()
    ...
    async def explain_belief_with_llm(
        self, belief_name: str, session_id: str, instance_id: str
    ) -> Dict[str, Any]:
        """
        ...
        technical_explanation = await self.jtms_service.explain_belief(
            instance_id, belief_name
        )
    ...
    async def update_beliefs_with_llm(
        self, new_information: str, session_id: str, instance_id: str
    ) -> Dict[str, Any]:
        """
    ...
    async def multi_agent_reasoning(
        self, agents_info: List[Dict[str, Any]], shared_session_id: str = None
    ) -> Dict[str, Any]:
        """
        ...
        if not shared_session_id:
            ...
            if not shared_instance_id:
                shared_instance_id = await self.jtms_service.create_jtms_instance(
                    session_id=shared_session_id, strict_mode=False
                )
                await self.session_manager.add_jtms_instance_to_session(
                    shared_session_id, shared_instance_id
                )
        ...
        for agent_info in agents_info:
            ...
            for belief_info in initial_beliefs:
                if isinstance(belief_info, dict):
                    ...
                    await self.jtms_service.create_belief(
                        instance_id=shared_instance_id, belief_name=str(belief_info)
                    )
    ...
    async def create_checkpoint_with_description(
        self, session_id: str, description: str = None
    ) -> str:
        """
        ...
        if not description:
            ...
            if session_info["jtms_instances"]:
                state = await self.jtms_service.get_jtms_state(
                    session_info["jtms_instances"][0]
                )
                stats = state["statistics"]
                description = (
                    f"Auto-checkpoint: {stats['total_beliefs']} croyances, "
...
def create_jtms_kernel(
    openai_api_key: str = None, model_name: str = "gpt-4"
) -> JTMSKernelIntegration:
    """
    ...
    if openai_api_key:
        chat_service = OpenAIChatCompletion(
            ai_model_id=model_name, api_key=openai_api_key
        )
...
async def save_reasoning_session(
    integration: JTMSKernelIntegration, session_id: str, filepath: str
):
    """
...
async def load_reasoning_session(
    integration: JTMSKernelIntegration, filepath: str
) -> str:
    """
    ...
    for instance_id, state in instances_states.items():
        ...
        new_instance_id = await integration.jtms_service.import_jtms_state(
            session_id=new_session_id, state_data=state_json
        )
