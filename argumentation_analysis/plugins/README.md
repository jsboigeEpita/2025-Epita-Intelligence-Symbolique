...
- **Homonymes à ne pas confondre — un `grep plugin` les mesure indifféremment :**
  - **`argumentation_analysis/plugin_framework/`** — framework de plugins **séparé**, prédécesseur abandonné, **sans chargeur** : ses trois mécanismes de découverte ont été retirés (#2099, aucun appelant de production). Sans rapport avec `agents/core/plugin_loader.py`, qui cherche **`manifest.json`**.
