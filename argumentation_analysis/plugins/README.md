...
**Une entrée qui ne mène nulle part** : ~~`agents/core/plugin_loader.py` ne peut charger aucun module de ce paquet~~ — le chargeur a été **retiré** (#2145, *Limites connues* §4) : 0 appelant de production, et l'unique `manifest.json` du paquet était irrecevable par son format.
...
- **Enfants** : `semantic_kernel/` (fiche propre, 1 plugin, 5 `@kernel_function`, route API dédiée — **ne pas confondre** avec `argumentation_analysis/integrations/semantic_kernel_integration.py`, autre montage, résiduel) et `analysis_tools/` (fiche propre : `logic/` + `plugin.py` + `tests/` — le `manifest.json` a été retiré avec son chargeur, #2145).
- **Homonymes à ne pas confondre — un `grep plugin` les mesure indifféremment :**
  - **`argumentation_analysis/plugin_framework/`** — framework de plugins **séparé**, prédécesseur abandonné, **sans chargeur** : ses **deux** chargeurs de plugins (`core/plugin_loader.py`, `core/plugins/plugin_loader.py`, qui cherchaient des fichiers **`plugin_manifest.json`**) et son chargeur d'agents (`agents/agent_loader.py`) ont été retirés (#2099, aucun appelant de production). Sans rapport avec l'ancien `agents/core/plugin_loader.py` (retiré #2145, lui aussi sans appelant), qui cherchait **`manifest.json`**.
...
Relevées pour la plupart **non corrigées** (lot documentaire initial). Cinq d'entre elles — **1, 2, 4, 5 et 6** — ont été **corrigées par #2145** (le 2 en deux grains) ; elles restent listées ici, avec leur statut, parce qu'un item retiré en silence serait indistinguable d'un item jamais relevé.

**Suivi** : les items **1, 2, 4, 5, 6** sont **corrigés** par **#2145** (le 2 en deux grains : fabrique puis analyseur). L'item **7** (homonymie de vocabulaire) est consigné comme dette **#1604** avec l'item **3** (capacités déclarées sans phase demandeuse) — le garde `test_one_capability_surface_1842.py:24-25` s'en exclut explicitement par design.
...
4. ~~**`agents/core/plugin_loader.py` ne peut charger aucun plugin de ce paquet.**~~ → **CORRIGÉ par retrait (#2145, grain finition)**. La voie était doublement inerte — l'unique `manifest.json` du paquet (`plugins/analysis_tools/manifest.json`, format `entry_point`) était irrecevable (`entrypoint_module`/`entrypoint_class` exigés), et le lecteur n'avait **aucun appelant de production** (2 fichiers de `tests/` seulement). Retirés avec lui : le manifeste, `agents/core/abc/plugin.py` (son seul importeur production était ce chargeur ; `LegoPlugin`/`ParameterSpec` en avaient zéro), `agents/core/exceptions.py` (leur seul éleveur) et les 3 fichiers de test de l'île. `AnalysisToolsPlugin` reste inchangé — 4 importateurs production directs (voir sa fiche).
...
6. ~~**Trois répertoires de prompts SK natifs sont résiduels.**~~ → **CORRIGÉ par archivage (#2145, grain finition)**. `ExplorationPlugin/Explore/`, `GuidingPlugin/GuidingPlugin/`, `SynthesisPlugin/Synthesize/` (chacun `config.json` + `skprompt.txt`) sont déplacés vers `docs/archives/plugins_overflow/` — le traitement du précédent #321 pour les prompts SK orphelins. Non-usage mesuré avant le déplacement : aucun `.py` ne les charge (aucune API SK de chargement par répertoire ne les vise), occurrences documentaires only ; origine `5f0422b81` (feat — plan de workflow parallèle jamais câblé ; l'analyse parallèle vivante est `fallacy_workflow_plugin.py`). L'homonymie avec les classes vivantes (`exploration_plugin.py:21`, `narrative_synthesis_plugin.py:660`) disparaît du paquet vivant. Les docs vivants qui citaient les chemins ont été mis à jour (l'inventaire daté `docs/reports/INVENTORY_…_LOT0_2026-09-09.md` reste gelé).
