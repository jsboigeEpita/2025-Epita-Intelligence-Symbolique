...
- **Enfants** : `semantic_kernel/` (fiche propre, 1 plugin, 5 `@kernel_function`, route API dédiée — l'ancien second montage `argumentation_analysis/integrations/semantic_kernel_integration.py` a été **retiré** #2116) et `analysis_tools/` (fiche propre : `logic/` + `plugin.py` + `tests/` — le `manifest.json` a été retiré avec son chargeur, #2145).
