...
class EnhancedFallacySeverityEvaluator:
    ...
    def evaluate_fallacy_severity(
        ...
        """
        self.logger.info(
            f"Évaluation de la gravité des sophismes dans {len(arguments)} arguments"
        )
        ...
        for i, argument in enumerate(arguments):
            ...
            if "expert" in argument.lower() or "autorité" in argument.lower():
                ...
                "million" in argument.lower()
                or "populaire" in argument.lower()
                or "tout le monde" in argument.lower()
            ):
                fallacies.append(
                    {
                ...
                "peur" in argument.lower()
                or "risque" in argument.lower()
                or "danger" in argument.lower()
            ):
                fallacies.append(
                    {
        ...
        for fallacy in fallacies:
            severity_evaluation = self._calculate_fallacy_severity(
                fallacy, context_analysis
            )
            fallacy_evaluations.append(severity_evaluation)

        # Calculer la gravité globale
        overall_severity, severity_level = self._calculate_overall_severity(
            fallacy_evaluations
        )
    ...
    def evaluate_fallacy_list(
        self, fallacies: List[Dict[str, Any]], context: str = "général"
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_context_impact(self, context: str) -> Dict[str, Any]:
        ...
        context_type = (
            ...
            and context.lower() in self.context_severity_modifiers
            else "général"
        )
        ...
        context_severity_modifier = self.context_severity_modifiers.get(
            context_type, 0.0
        )
        audience_severity_modifier = self.audience_severity_modifiers.get(
            audience_type, 0.0
        )
    ...
    def _calculate_fallacy_severity(
        self, fallacy: Dict[str, Any], context_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        final_severity = (
            base_severity + context_modifier + audience_modifier + domain_modifier
        )
    ...
    def _calculate_overall_severity(
        self, fallacy_evaluations: List[Dict[str, Any]]
    ) -> Tuple[float, str]:
        """
        ...
        severities = [
            evaluation.get("final_severity", 0.0) for evaluation in fallacy_evaluations
        ]
