    ...
    def generate(
        ...
        if fallacy_analysis.get("overall_severity", 0.5) > 0.7:
            ...
            for fallacy in most_common_fallacies:
                if fallacy == "Appel à l'émotion":
                    fallacy_recommendations.append(
                        "Réduire les appels émotionnels excessifs"
                    )
                elif fallacy == "Ad hominem":
                    fallacy_recommendations.append("Éviter les attaques personnelles")
                elif fallacy == "Appel à l'autorité":
                    fallacy_recommendations.append(
                        "Citer des sources plus crédibles et diversifiées"
                    )

        if fallacy_analysis.get("contextual_ratio", 0) < 0.3:
            fallacy_recommendations.append(
                "Adapter les sophismes au contexte spécifique de l'argumentation"
            )

        # Recommandations basées sur la cohérence
        if coherence_analysis.get("overall_coherence", 0.5) < 0.6:
            coherence_recommendations.append(
                "Améliorer la cohérence globale de l'argumentation"
            )

        if coherence_analysis.get("contradiction_count", 0) > 0:
            coherence_recommendations.append(
                f"Résoudre les {coherence_analysis.get('contradiction_count')} contradictions identifiées"
            )
        ...
        for issue in main_coherence_issues:
            if issue == "Thematic shifts":
                coherence_recommendations.append(
                    "Maintenir une cohérence thématique plus forte"
                )
        ...
        if persuasion_analysis.get("persuasion_score", 0.5) < 0.6:
            persuasion_recommendations.append(
                "Renforcer l'efficacité persuasive globale"
            )
        ...
        if rhetorical_appeals:
            ...
            if rhetorical_appeals[min_appeal] < 0.4:
                if min_appeal == "ethos":
                    persuasion_recommendations.append(
                        "Renforcer l'appel à la crédibilité"
                    )
                elif min_appeal == "pathos":
                    persuasion_recommendations.append("Renforcer l'appel émotionnel")
                elif min_appeal == "logos":
                    persuasion_recommendations.append(
                        "Renforcer l'appel logique avec des preuves et des raisonnements solides"
                    )

        # Recommandations spécifiques au contexte
        if context == "politique":
            if (
                persuasion_analysis.get("emotional_appeal", 0.5) > 0.8
                and persuasion_analysis.get("logical_appeal", 0.5) < 0.4
            ):
                context_specific_recommendations.append(
                    "Équilibrer les appels émotionnels avec des arguments logiques"
                )
        elif context == "scientifique":
            if persuasion_analysis.get("logical_appeal", 0.5) < 0.6:
                context_specific_recommendations.append(
                    "Renforcer la rigueur logique et les preuves empiriques"
                )
        elif context == "commercial":
            if persuasion_analysis.get("credibility_appeal", 0.5) < 0.5:
                context_specific_recommendations.append(
                    "Renforcer la crédibilité de la marque ou du produit"
                )

        # Ajouter des recommandations générales basées sur l'ensemble des analyses
        if (
            fallacy_analysis.get("overall_severity", 0.5) > 0.6
            and coherence_analysis.get("overall_coherence", 0.5) < 0.6
        ):
            general_recommendations.append(
                "Restructurer l'argumentation pour réduire les sophismes et améliorer la cohérence"
            )
...
class EnhancedRhetoricalResultAnalyzer:
    ...
    def analyze_rhetorical_results(
        self, results: Dict[str, Any], context: str = "général"
    ) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(
            f"Analyse avancée des résultats rhétoriques dans le contexte: {context}"
        )
        ...
        (
            rhetorical_quality,
            rhetorical_quality_level,
        ) = self._calculate_overall_rhetorical_quality(
    ...
    def _analyze_fallacy_distribution(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        individual_fallacies_count = complex_fallacy_analysis.get(
            "individual_fallacies_count", 0
        )
        basic_combinations = complex_fallacy_analysis.get("basic_combinations", [])
        advanced_combinations = complex_fallacy_analysis.get(
            "advanced_combinations", []
        )

        contextual_fallacies = contextual_fallacy_analysis.get(
            "contextual_fallacies", []
        )

        # Calculer le nombre total de sophismes
        total_fallacies = (
            individual_fallacies_count
        ...
        most_common_fallacies = [
            fallacy_type for fallacy_type, _ in fallacy_types.most_common(3)
        ]

        # Identifier les sophismes les plus graves
        fallacies_by_severity = sorted(
            contextual_fallacies, key=lambda f: f.get("confidence", 0.5), reverse=True
        )
    ...
    def _analyze_coherence_quality(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        thematic_coherence = (
            0.7 if len(thematic_clusters) <= 2 else 0.5
    ...
    def _analyze_persuasion_effectiveness(
        self, results: Dict[str, Any], context: str
    ) -> Dict[str, Any]:
        """
        ...
        rhetorical_appeals = self._identify_rhetorical_appeals(
        ...
        audience_characteristics = context_analysis.get(
    ...
    def _evaluate_context_appropriateness(
        self, rhetorical_appeals: Dict[str, float], context_type: str
    ) -> float:
        """
        ...
        weights = context_appeal_weights.get(
            context_type, context_appeal_weights["général"]
        )
    ...
    def _evaluate_audience_alignment(
        self, rhetorical_appeals: Dict[str, float], audience_characteristics: List[str]
    ) -> float:
        """
        ...
        if audience_characteristics:
            for characteristic in audience_characteristics:
                if characteristic in audience_appeal_weights:
                    for appeal, weight in audience_appeal_weights[
                        characteristic
                    ].items():
    ...
    def _calculate_overall_rhetorical_quality(
        ...
        rhetorical_quality = (
            (fallacy_quality * 0.3) + (coherence_score * 0.3) + (persuasion_score * 0.4)
        )
    ...
    def _identify_persuasion_strategies(
        self, contextual_fallacy_analysis: Dict[str, Any]
    ) -> Dict[str, float]:
        """
        ...
        if total_score > 0:
            strategies = {
                ...
                for strategy, score in strategy_counts.items()
            }
        else:
            strategies = {}
    ...
    def _identify_rhetorical_appeals(
        self, contextual_fallacy_analysis: Dict[str, Any]
    ) -> Dict[str, float]:
        """
        ...
        appeals = {"ethos": 0.0, "pathos": 0.0, "logos": 0.0}
