...
class EnhancedContextualFallacyAnalyzer:
    ...
    def _load_learning_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if learning_data_path.exists():
                ...
                self.logger.info(
                    f"Données d'apprentissage chargées: {len(learning_data['feedback_history'])} entrées d'historique"
                )
        except Exception as e:
            self.logger.error(
                f"Erreur lors du chargement des données d'apprentissage: {e}"
            )
    ...
    def _save_learning_data(self):
        ...
        try:
            ...
            self.logger.info(
                f"Données d'apprentissage sauvegardées: {len(self.learning_data['feedback_history'])} entrées d'historique"
            )
        except Exception as e:
            self.logger.error(
                f"Erreur lors de la sauvegarde des données d'apprentissage: {e}"
            )
    ...
    def analyze_context(self, text: str, context: str) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Analyse contextuelle améliorée du texte (longueur: {len(text)}) dans le contexte: {context}"
        )

        # 1. Analyse approfondie du contexte fourni
        context_analysis = self._analyze_context_deeply(context)
        self.logger.info(
            f"Analyse contextuelle approfondie: {context_analysis['context_type']} (confiance: {context_analysis['confidence']:.2f})"
        )

        # 2. Identification des sophismes potentiels (base + NLP)
        potential_fallacies = self._identify_potential_fallacies_with_nlp(text)
        self.logger.info(
            f"Sophismes potentiels identifiés avec NLP: {len(potential_fallacies)}"
        )

        # 3. Filtrage et ajustement de la confiance en fonction du contexte
        contextual_fallacies = self._filter_by_context_semantic(
            potential_fallacies, context_analysis
        )
        self.logger.info(
            f"Sophismes contextuels identifiés après analyse sémantique: {len(contextual_fallacies)}"
        )
        ...
        self.last_analysis_fallacies = {
            f"fallacy_{i}": fallacy for i, fallacy in enumerate(contextual_fallacies)
        }
            ...
            try:
                ...
                if "politique" in context_type:
                    if "élection" in context.lower():
                        ...
                        "parlement" in context.lower() or "assemblée" in context.lower()
                    ):
                ...
                confidence = min(
                    0.9,
                    0.7
                    + len(context_subtypes) * 0.05
                    + len(audience_characteristics) * 0.05,
                )

            except Exception as e:
                self.logger.error(
                    f"Erreur lors de l'analyse approfondie du contexte: {e}"
                )
            ...
            try:
                ...
                for i, sentence in enumerate(sentences):
                    ...
                    if sentiment_score > 0.8:
                        ...
                        if not any(
                            fallacy["fallacy_type"] == "Appel à l'émotion"
                            and sentence in fallacy["context_text"]
                    ...
                    if person_entities and (
                        "expert" in sentence.lower()
                        or "autorité" in sentence.lower()
                        or "scientifique" in sentence.lower()
                    ):
                        # Vérifier si ce n'est pas déjà identifié comme un appel à l'autorité
                        if not any(
                            fallacy["fallacy_type"] == "Appel à l'autorité"
                            and sentence in fallacy["context_text"]
                            for fallacy in potential_fallacies
                        ):
                            potential_fallacies.append(
                ...
                for fallacy in potential_fallacies:
                    ...
                    if fallacy_type in self.learning_data["confidence_adjustments"]:
                        adjustment = self.learning_data["confidence_adjustments"][
                            fallacy_type
                        ]
                        fallacy["confidence"] = min(
                            1.0, max(0.1, fallacy["confidence"] + adjustment)
                        )
                        fallacy["confidence_adjusted"] = True

            except Exception as e:
                self.logger.error(
                    f"Erreur lors de l'identification des sophismes avec NLP: {e}"
                )
    ...
    def _filter_by_context_semantic(
        ...
        subtype_adjustments = {
            "électoral": {"Appel à l'émotion": 0.1, "Appel à la peur": 0.1},
            "parlementaire": {"Ad hominem": 0.1, "Homme de paille": 0.1},
            "diplomatique": {"Faux dilemme": 0.1, "Pente glissante": 0.1},
        }

        # Ajustements basés sur l'audience
        audience_adjustments = {
            "généraliste": {"Appel à la popularité": 0.1, "Appel à l'émotion": 0.1},
            "expert": {"Appel à l'autorité": 0.1, "Argument circulaire": 0.1},
            "académique": {"Appel à l'autorité": 0.1, "Généralisation hâtive": 0.1},
        }

        # Ajustements basés sur le niveau de formalité
        formality_adjustments = {
            "élevé": {"Ad hominem": 0.1, "Appel à l'émotion": 0.1},
            "faible": {"Appel à la popularité": 0.1, "Appel à la tradition": 0.1},
        }
        ...
        for fallacy in potential_fallacies:
            ...
            if fallacy_type in context_fallacy_mapping[context_type]:
                context_adjustment += context_fallacy_mapping[context_type][
                    fallacy_type
                ]
            ...
            for subtype in context_subtypes:
                if (
                    subtype in subtype_adjustments
                    and fallacy_type in subtype_adjustments[subtype]
                ):
                    context_adjustment += subtype_adjustments[subtype][fallacy_type]

            for aud in audience:
                if (
                    aud in audience_adjustments
                    and fallacy_type in audience_adjustments[aud]
                ):
                    context_adjustment += audience_adjustments[aud][fallacy_type]

            if (
                formality in formality_adjustments
                and fallacy_type in formality_adjustments[formality]
            ):
    ...
    def _analyze_fallacy_relations(
        ...
        for i in range(len(fallacies)):
            for j in range(i + 1, len(fallacies)):
                ...
                elif self._are_complementary_fallacies(
                    fallacy1["fallacy_type"], fallacy2["fallacy_type"]
                ):
                    relations.append(
                        {
    ...
    def identify_contextual_fallacies(
        ...
        """
        self.logger.info(
            f"Identification améliorée des sophismes contextuels dans l'argument (longueur: {len(argument)}) dans le contexte: {context}"
        )
    ...
    def provide_feedback(
        self, fallacy_id: str, is_correct: bool, feedback_text: Optional[str] = None
    ) -> None:
        """
        ...
        """
        self.logger.info(
            f"Réception de feedback pour le sophisme {fallacy_id}: {'correct' if is_correct else 'incorrect'}"
        )
        ...
        if fallacy_id in self.last_analysis_fallacies:
            ...
            self.learning_data["confidence_adjustments"][fallacy_type] = max(
                -0.5,
                min(0.5, self.learning_data["confidence_adjustments"][fallacy_type]),
            )

            self.logger.info(
                f"Ajustement de confiance pour le type de sophisme '{fallacy_type}': {self.learning_data['confidence_adjustments'][fallacy_type]:.2f}"
            )
    ...
    def get_contextual_fallacy_examples(
        ...
        for example in basic_examples:
            ...
            explanation = self._generate_fallacy_explanation(
                fallacy_type, context_type, example
            )
    ...
    def _generate_fallacy_explanation(
        self, fallacy_type: str, context_type: str, example: str
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"Résultats de l'analyse contextuelle améliorée: {json.dumps(results, indent=2, ensure_ascii=False)}"
    )

    # Exemple d'identification de sophismes contextuels
    fallacies = analyzer.identify_contextual_fallacies(text, context)
    print(
        f"Sophismes contextuels identifiés avec l'analyseur amélioré: {json.dumps(fallacies, indent=2, ensure_ascii=False)}"
    )
    ...
    print(
        f"Exemples enrichis de sophismes: {json.dumps(examples, indent=2, ensure_ascii=False)}"
    )
