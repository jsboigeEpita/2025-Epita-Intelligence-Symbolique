...
- **Faux frères, tous deux retirés** : l'ancien `agents/core/plugin_loader.py` (`PluginLoader`) — seul lecteur du format `manifest.json`, sans appelant de production — a été **retiré** (#2145) en même temps que le descripteur ; son homonyme `plugin_framework/core/plugins/plugin_loader.py`, qui cherchait `plugin_manifest.json`, l'a été aussi (#2099, aucun appelant de production).
