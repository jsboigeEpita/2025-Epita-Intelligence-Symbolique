...
class JTMSSemanticKernelPlugin:
    ...
    def __init__(
        self,
        jtms_service: JTMSService = None,
        session_manager: JTMSSessionManager = None,
    ):
        """
    ...
    async def _ensure_session_and_instance(
        self,
        session_id: str = None,
        ...
        if not session_id:
            if self.default_session_id:
                ...
                session_id = await self.session_manager.create_session(
                    ...
                    metadata={
                        "created_by": "semantic_kernel_plugin",
                        "auto_created": True,
                    },
                )
    ...
    async def create_belief(
        ...
        initial_value: Annotated[
            str, "Valeur initiale: 'true', 'false', ou 'unknown'"
        ] = "unknown",
    ...
    async def add_justification(
        self,
        in_beliefs: Annotated[
            str, "Liste des croyances positives (séparées par des virgules)"
        ],
        out_beliefs: Annotated[
            str, "Liste des croyances négatives (séparées par des virgules)"
        ],
        ...
        try:
            ...
            in_beliefs_list = (
                [b.strip() for b in in_beliefs.split(",") if b.strip()]
                if in_beliefs
                else []
            )
            out_beliefs_list = (
                [b.strip() for b in out_beliefs.split(",") if b.strip()]
                if out_beliefs
                else []
            )
    ...
    async def explain_belief(
        ...
        instance_id: Annotated[str, "Identifiant de l'instance JTMS"] = "",
        agent_id: Annotated[
            str, "Identifiant de l'agent demandeur"
        ] = "semantic_kernel",
    ) -> str:
        """
        ...
        try:
            ...
            result = await self.jtms_service.explain_belief(
                instance_id=instance_id, belief_name=belief_name
            )
            ...
            result["natural_language_summary"] = (
                f"La croyance '{belief_name}' est actuellement {status_text}. "
    ...
    async def query_beliefs(
        ...
        filter_status: Annotated[
            str,
            "Filtre par statut: 'valid', 'invalid', 'unknown', 'non_monotonic', ou 'all'",
        ] = "all",
        session_id: Annotated[str, "Identifiant de la session JTMS"] = "",
        instance_id: Annotated[str, "Identifiant de l'instance JTMS"] = "",
        agent_id: Annotated[
        ...
        try:
            ...
            result = await self.jtms_service.query_beliefs(
                instance_id=instance_id, filter_status=filter_param
            )
            ...
            if filter_status == "all":
                ...
                summary = (
                    f"Trouvé {result['filtered_count']} croyances avec le statut '{filter_status}' "
    ...
    async def get_jtms_state(
        ...
        include_graph: Annotated[
            str, "Inclure le graphe de justifications: 'true' ou 'false'"
        ] = "true",
        include_statistics: Annotated[
            str, "Inclure les statistiques détaillées: 'true' ou 'false'"
        ] = "true",
        session_id: Annotated[str, "Identifiant de la session JTMS"] = "",
        instance_id: Annotated[str, "Identifiant de l'instance JTMS"] = "",
        agent_id: Annotated[
        ...
        try:
            ...
            summary = (
                f"Système JTMS de la session '{session_info['session_name']}' "
    ...
    def configure_auto_creation(
        self, auto_session: bool = True, auto_instance: bool = True
    ):
...
def create_jtms_plugin(
    jtms_service: JTMSService = None, session_manager: JTMSSessionManager = None
