...
def _import_matplotlib():
    ...
    try:
        ...
        raise ImportError(
            f"matplotlib et seaborn sont requis pour la génération de rapports: {e}"
        )
    ...
    for corpus in all_corpora:
        ...
        if corpus in base_by_corpus:
            ...
            for result in corpus_results:
                contextual_fallacies = result.get("analyses", {}).get(
                    "contextual_fallacies", {}
                )
                for arg_result in contextual_fallacies.get("argument_results", []):
                    base_contextual_fallacies += len(
                        arg_result.get("detected_fallacies", [])
                    )
        ...
        if corpus in advanced_by_corpus:
            ...
            for result in corpus_results:
                ...
                advanced_contextual_fallacies += contextual_fallacies.get(
                    "contextual_fallacies_count", 0
                )

                complex_fallacies = result.get("analyses", {}).get(
                    "complex_fallacies", {}
                )
                advanced_complex_fallacies += complex_fallacies.get(
                    "individual_fallacies_count", 0
                )
                advanced_complex_fallacies += len(
                    complex_fallacies.get("basic_combinations", [])
                )
                advanced_complex_fallacies += len(
                    complex_fallacies.get("advanced_combinations", [])
                )
                advanced_complex_fallacies += len(
                    complex_fallacies.get("fallacy_patterns", [])
                )

                fallacy_severity = result.get("analyses", {}).get(
                    "fallacy_severity", {}
                )
                advanced_severity_score += fallacy_severity.get("overall_severity", 0.0)

                rhetorical_results = result.get("analyses", {}).get(
                    "rhetorical_results", {}
                )
                overall_analysis = rhetorical_results.get("overall_analysis", {})
                advanced_rhetorical_quality += overall_analysis.get(
                    "rhetorical_quality", 0.0
                )
            ...
            effectiveness[corpus]["advanced_agents"][
...
def _generate_visualizations(
    ...
    base_fallacies = [
        sum(
            metrics.get("fallacy_count", 0)
            for agent_type in effectiveness[c].get("base_agents", {})
            for agent, metrics in effectiveness[c]["base_agents"].items()
        )
        for c in corpora
    ]
    advanced_fallacies = [
        sum(
            metrics.get("fallacy_count", 0)
            for agent_type in effectiveness[c].get("advanced_agents", {})
            for agent, metrics in effectiveness[c]["advanced_agents"].items()
        )
        for c in corpora
    ]
...
def _generate_markdown_report(
    ...
    report_content.append(
        "Ce rapport présente une synthèse complète des résultats des analyses rhétoriques..."
    )
    # ... (contenu du résumé)
    report_content.append("\n### Points clés")
    report_content.append(
        "- Les agents d'analyse rhétorique ont été évalués sur plusieurs corpus..."
    )
    report_content.append(
        "- Les agents avancés fournissent généralement une analyse plus riche..."
    )
    report_content.append(
        "- Chaque agent présente des forces et des faiblesses spécifiques..."
    )
    ...
    advanced_by_corpus = group_results_by_corpus(advanced_results)
    all_corpora = sorted(
        list(set(base_by_corpus.keys()).union(set(advanced_by_corpus.keys())))
    )
    for corpus in all_corpora:
        base_count = len(base_by_corpus.get(corpus, []))
        advanced_count = len(advanced_by_corpus.get(corpus, []))
        report_content.append(
            f"- **{corpus}**: {base_count} extraits (base), {advanced_count} extraits (avancé)"
        )
    report_content.append("\n### Agents évalués")
    # ... (description des agents)
    report_content.append("\n#### Agents de base")
    report_content.append(
        "- **ContextualFallacyDetector**: Détecte les sophismes contextuels."
    )
    ...
    report_content.append("\n#### Agents avancés")
    report_content.append(
        "- **EnhancedComplexFallacyAnalyzer**: Détecte les sophismes complexes."
    )
    ...
    for corpus_name_sorted in all_corpora:
        ...
        report_content.extend(
            generate_markdown_report_for_corpus(
                corpus_name_sorted, corpus_effectiveness_data
            )
        )
    ...
    for name, path in visualization_paths.items():
        ...
        try:
            ...
            rel_path = path.name
            logger.warning(
                f"Impossible de créer un chemin relatif pour {path} par rapport à {output_file.parent}. Utilisation de {rel_path}."
            )

        report_content.append(f"\n### {name.replace('_', ' ').title()}")
        report_content.append(
            f"![{name.replace('_', ' ').title()}]({rel_path})"
        )  # Utiliser des slashes pour Markdown
...
def _generate_html_report(
    markdown_file: Path, output_file: Path, visualization_dir: Path
) -> None:
    """
    ...
    """
    logger.info(
        f"Génération du rapport HTML à partir de {markdown_file} vers {output_file}"
    )
    try:
        markdown_content = load_text_file(markdown_file)
        if markdown_content is None:
            logger.error(
                f"❌ Impossible de lire le contenu Markdown depuis {markdown_file}"
            )
            return

        if save_markdown_to_html(markdown_content, output_file):
            logger.info(
                f"[OK] Rapport HTML généré avec succès via l'utilitaire: {output_file}"
            )
        else:
            logger.error(
                f"❌ Échec de la génération du rapport HTML via l'utilitaire pour {output_file}"
            )
    except Exception as e:
        logger.error(
            f"❌ Erreur lors de la génération du rapport HTML ({output_file}): {e}",
            exc_info=True,
        )
...
def run_comprehensive_report_pipeline(
    ...
    if verbose:
        ...
        logger.setLevel(
            logging.DEBUG
        )  # Assurez-vous que le logger de ce module est aussi en DEBUG
    ...
    logger.info(
        "Démarrage de la génération du rapport d'analyse complet via le pipeline..."
    )
    ...
    if not base_results_path.exists():
        logger.error(
            f"Le fichier de résultats de base {base_results_path} n'existe pas."
        )
        return False
    if not advanced_results_path.exists():
        logger.error(
            f"Le fichier de résultats avancés {advanced_results_path} n'existe pas."
        )
        return False
    ...
    if performance_report_file_path:
        ...
        if perf_report_path.exists():
            ...
            logger.warning(
                f"Fichier de rapport de performance {perf_report_path} non trouvé."
            )
    else:
    ...
    if performance_metrics_file_path:
        ...
        if perf_metrics_path.exists():
            ...
            performance_metrics = (
                df_metrics if df_metrics is not None else pd.DataFrame()
            )
        else:
            logger.warning(
                f"Fichier de métriques de performance {perf_metrics_path} non trouvé."
            )
    else:
    ...
    if isinstance(base_results_data, list):
        ...
        logger.warning(
            f"Les résultats de base chargés depuis {base_results_path} ne sont pas une liste. Type: {type(base_results_data)}. Traitement comme liste vide."
        )
    ...
    if isinstance(advanced_results_data, list):
        ...
        logger.warning(
            f"Les résultats avancés chargés depuis {advanced_results_path} ne sont pas une liste. Type: {type(advanced_results_data)}. Traitement comme liste vide."
        )
    ...
    combined_average_scores: Dict[str, Dict[str, float]] = {}
    all_corpora_for_scores = set(average_scores_base.keys()) | set(
        average_scores_advanced.keys()
    )
    for corpus_name in all_corpora_for_scores:
        ...
        all_metrics_for_corpus = set(base_corpus_scores.keys()) | set(
            advanced_corpus_scores.keys()
        )
    ...
    effectiveness = _analyze_agent_effectiveness(
        base_results, advanced_results, performance_metrics
    )
    ...
    visualization_paths = _generate_visualizations(
        base_results, advanced_results, effectiveness, visualization_dir
    )
    ...
    logger.info(
        f"[OK] Pipeline de génération du rapport complet terminé. Résultats dans {output_dir}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Exécute le pipeline de génération de rapport complet."
    )
    parser.add_argument(
        "--base-results", required=True, help="Chemin des résultats de base (JSON)."
    )
    parser.add_argument(
        "--advanced-results", required=True, help="Chemin des résultats avancés (JSON)."
    )
    parser.add_argument(
        "--performance-report", help="Chemin du rapport de performance (Markdown)."
    )
    parser.add_argument(
        "--performance-metrics", help="Chemin des métriques de performance (CSV)."
    )
    parser.add_argument(
        "--output-dir",
        default="results/comprehensive_report_pipeline_test",
