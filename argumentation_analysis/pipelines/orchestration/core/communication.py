...
try:
    ...
    logging.getLogger(__name__).warning(
        f"Could not import core communication components: {e}"
    )
...
def initialize_communication_middleware(
    ...
    if (
        service_manager
        and hasattr(service_manager, "middleware")
        and service_manager.middleware
    ):
        logger.info(
            "[COMMUNICATION] Communication middleware linked from ServiceManager."
        )
    ...
    if enable_communication:
        logger.warning(
            "[COMMUNICATION] Creating a new, isolated middleware instance. "
            "This may lead to unsynchronized communication if a ServiceManager is used elsewhere."
        )
        return MessageMiddleware()

    logger.info(
        "[COMMUNICATION] Middleware not initialized (not enabled or no source available)."
    )
    return None
