...
N'est **pas** un jeu de « briques de traitement atomiques » (annonce du README parent, corrigée en #2110) : aucun processeur d'extraction/normalisation n'a jamais été écrit ici. Les docstrings qui annonçaient 5 processeurs chacune — `ExtractProcessor`, `ResultFormattingProcessor`… — **aucun n'existe** (grep 0 match) et elles sont retirées (#2110). À distinguer de `pipelines/analysis_pipeline.py` et de [`orchestration/`](../../../orchestration/README.md) racine.

## Composants publics

| Fonction | Ligne | Ce qu'elle fait réellement |
|---|---|---|
| `post_process_orchestration_results` | `post_processors.py:24` | injecte `results["recommendations"]` (seuils `overall_score > 0.7`, :32-47) + `communication_log` (:50) |
| `execute_operational_tasks` | `processors.py:29` | fabrique **jusqu'à 5 tâches factices** `"Résultat de la tâche opérationnelle {i+1}"`, `execution_time: 0.5` codé dur (:40-46) |
| `synthesize_hierarchical_results` | `processors.py:63` | moyenne de 3 scores heuristiques (:77-83) |
...
- `execution/engine.py:51-52`, appels réels :102 (`post_process_orchestration_results`) et :115 (`save_orchestration_trace`) — **mais engine.py lui-même n'a aucun appelant production** (voir [`../execution/`](../execution/README.md)) ;
- hors du paquet : **zéro importeur production** (grep `pipelines.orchestration.analysis` : tests uniquement).

Attention : `execution/strategies.py:182,193` appellent `pipeline._execute_operational_tasks` / `._synthesize_hierarchical_results` — **méthodes du pipeline, pas ces fonctions libres**, et ces méthodes ne sont définies dans aucun fichier du dépôt (elles n'existent que sur les mocks des tests).
...
- `post_processors.py:50` appelle `pipeline._get_communication_log()` — méthode définie nulle part (la fonction libre `traces.py:36` est distincte) ;
