...
class UnifiedTextAnalysisPipeline:
    ...
    async def initialize(self) -> bool:
            ...
            try:
                ...
                if self.jvm_ready:
                    ...
                    if self.conversation_logger and hasattr(
                        ...
                        self.conversation_logger.info(
                            "System: JVM initialisee pour analyse formelle"
                        )
                else:
                    logger.warning(
                        "[JVM] JVM non disponible - analyse formelle limitee"
                    )
        ...
        try:
            ...
            if self.llm_service:
                logger.info(
                    f"[LLM] Service LLM cree (ID: {self.llm_service.service_id})"
                )
                if self.conversation_logger and hasattr(
                    self.conversation_logger, "info"
                ):
                    self.conversation_logger.info(
                        f"System: Service LLM initialise (ID: {self.llm_service.service_id})"
                    )
        except Exception as e:
    ...
    async def _initialize_analysis_tools(self):
        ...
        if self.config.use_mocks:
            logger.warning(
                "use_mocks=True demandé mais les mocks ont été éliminés en Phase 2"
            )
    ...
    async def analyze_text_unified(
        self, text: str, source_info: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"[ANALYZE] Debut analyse unifiee (modes: {self.config.analysis_modes})"
        )
        ...
        logger.info(
            f"[ANALYZE] Analyse unifiee terminee en {results['execution_time']:.2f}s"
        )
        return results
    ...
    async def _perform_informal_analysis(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result = analysis_plugin.analyze_text(
                text, "general_pipeline_context"
            )
    ...
    async def _perform_formal_analysis(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if belief_set:
                ...
                is_consistent, consistency_details = await logic_agent.is_consistent(
                    belief_set
                )
                ...
                for query in queries[:3]:  # Limite pour performance
                    result, raw_output = await logic_agent.execute_query(
                        belief_set, query
                    )
                ...
                orchestration_results.update(
                    {
                        ...
                        "conversation_summary": orch_result.get(
                            "conversation_summary", {}
                        ),
    ...
    async def _perform_informal_analysis_orchestrated(
        self, text: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.orchestrator, "analyze_text_comprehensive"):
                ...
                logger.warning(
                    "Méthode d'orchestration non reconnue, fallback vers analyse standard"
                )
                return await self._perform_informal_analysis(text)
                ...
                for i, tool_call in enumerate(conversation_log["tool_calls"]):
                    if isinstance(tool_call, dict):
                        ...
                        if isinstance(result, dict):
                            ...
                            sophismes_data = (
                                result.get("fallacies")
                                or result.get("sophismes")
                                or result.get("detected_fallacies")
                                or result.get("fallacy_analysis", {}).get(
                                    "fallacies", []
                                )
                            )
                            ...
                            logger.info(
                                f"🔍 DEBUG: {len(sophismes_data)} fallacies trouvés dans le tool call {i}"
                            )
                            ...
                            for sophisme_data in sophismes_data:
                                if isinstance(sophisme_data, dict):
                                    ...
                                    confidence_value = (
                                        sophisme_data.get("confidence", 0)
                                        or sophisme_data.get("match_score", 0)
                                        or sophisme_data.get("detection_confidence", 0)
                                        or 0.7  # Valeur par défaut raisonnable au lieu de 0.0
                                    )

                                    sophisme = {
                                        "type": sophisme_data.get(
                                            "type",
                                            sophisme_data.get(
                                                "name",
                                                sophisme_data.get(
                                        ...
                                        "text_fragment": sophisme_data.get(
                                            "fragment",
                                            sophisme_data.get(
                                                "citation", "Fragment non disponible"
                                            ),
                                        ),
                                        "explanation": sophisme_data.get(
                                            "explanation",
                                            sophisme_data.get(
                                                "explication",
                                        ...
                                        "severity": sophisme_data.get(
                                            "severity", "Moyen"
                                        ),
                                        "confidence": confidence_value,
                                        "reformulation": sophisme_data.get(
                                            "reformulation", ""
                                        ),
                                    }
                            ...
                            if sophismes_data:
                                logger.info(
                                    f"✅ PRIORITÉ 1 réussie : {len(sophismes_data)} sophismes extraits depuis tool call {i}"
                                )
                        ...
                        # Si le résultat est une string, chercher le JSON brut de GPT-4o-mini
                            ...
                            logger.info(
                                f"🔍 DEBUG: Tool call {i} contient JSON brut, tentative d'extraction..."
                            )
                            try:
                                import json
                                ...
                                if json_start != -1:
                                    ...
                                    for sophisme_data in sophismes_json.get(
                                        "sophismes", []
                                    ):
                                        sophisme = {
                                            "type": sophisme_data.get(
                                    ...
                                    logger.info(
                                        f"✅ PRIORITÉ 1 JSON réussie : {len(sophismes_json.get('sophismes', []))} sophismes extraits depuis tool call {i}"
                                    )
                                    break

                            except (json.JSONDecodeError, Exception) as e:
                                logger.warning(
                                    f"Erreur lors du parsing JSON des sophismes: {e}"
                                )
                        ...
                        try:
                            ...
                            if "sophismes" in content.lower():
                                ...
                                if json_start != -1:
                                    ...
                                    logger.info(
                                        f"✅ {len(sophismes_json.get('sophismes', []))} sophismes extraits depuis JSON brut"
                                    )
                                    break
            ...
            if (
                not informal_results["fallacies"]
                and isinstance(conversation_log, dict)
                and "messages" in conversation_log
            ):
                for msg in conversation_log["messages"]:
                    if isinstance(msg, dict):
                        ...
                        if (
                            ...
                            and "sophismes détectés" in content
                        ):
                            ...
                            try:
                                ...
                                sophismes_match = re.search(
                                    r"(\d+) sophismes détectés: (.+?)(?:\. Score|$)",
                                    content,
                                )
                                if sophismes_match:
                                    ...
                                    matches = re.findall(
                                        sophisme_pattern, sophismes_text
                                    )
                                    ...
                                    logger.info(
                                        f"✅ {len(matches)} sophismes extraits de l'orchestrateur (fallback)"
                                    )
                                    break

                            except Exception as e:
                                logger.warning(
                                    f"Erreur lors du parsing des sophismes (fallback): {e}"
                                )
            ...
            logger.info(
                f"📊 Résumé extraction: {len(fallacies)} sophismes, confiance moyenne: {informal_results['summary']['average_confidence']:.2%}"
            )
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if len(fallacies) > 3:
            recommendations.append(
                "Nombreux sophismes détectés - révision argumentative recommandée"
            )

        critical_fallacies = [f for f in fallacies if f.get("severity") == "Critique"]
        if critical_fallacies:
            recommendations.append(
                "Sophismes critiques présents - attention particulière requise"
            )
        ...
        if formal.get("consistency_check") is False:
            recommendations.append(
                "Incohérences logiques détectées - clarification nécessaire"
            )
        ...
        if orchestration.get("status") == "success":
            recommendations.append(
                "Analyse orchestrée complétée - examen des insights avancés recommandé"
            )
    ...
    def get_conversation_log(self) -> List[Dict[str, Any]]:
        ...
        if self.conversation_logger:
            return {
                ...
                "state_snapshots": getattr(
                    self.conversation_logger, "state_snapshots", []
                ),
            }
        return {}
