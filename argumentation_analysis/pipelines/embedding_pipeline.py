...
try:
    ...
    NameError
):  # __file__ n'est pas défini si exécuté dans un interpréteur interactif par exemple
...
from argumentation_analysis.nlp.embedding_utils import (
    get_embeddings_for_chunks,
...
def run_embedding_generation_pipeline(
    ...
    if input_json_file_path:
        logger.info(
            f"Utilisation des définitions JSON fournies via fichier: {input_json_file_path}"
        )
    elif json_string:
        ...
        logger.error(
            "Aucune source de configuration d'entrée spécifiée (ni --input-config, ni --json-string, ni --input-json-file). Arrêt."
        )
    ...
    if input_json_file_path and not input_json_file_path.exists():
        logger.error(
            f"Le fichier d'entrée JSON {input_json_file_path} n'existe pas. Arrêt."
        )
        sys.exit(1)  # Erreur critique: fichier d'entrée JSON manquant
    elif (
        input_config_path
        and not input_config_path.exists()
        and not json_string
        and not input_json_file_path
    ):
        logger.error(
            f"Le fichier d'entrée chiffré {input_config_path} n'existe pas et aucune autre source (chaîne JSON ou fichier JSON) n'est fournie. Arrêt."
        )
        sys.exit(
            1
        )  # Erreur critique: fichier d'entrée chiffré manquant et pas d'alternative
    ...
    if output_config_path.exists() and not force_overwrite:
        ...
        logger.warning(
            f"Le fichier de sortie {output_config_path} existe et sera écrasé (--force activé)."
        )
    ...
    encryption_key_to_use = (
        CONFIG_UI_ENCRYPTION_KEY  # Clé de chiffrement depuis la configuration de l'UI
    )
    if not encryption_key_to_use:
        logger.error(
            "ENCRYPTION_KEY n'est pas disponible depuis argumentation_analysis.ui.config. Impossible de continuer pour les opérations chiffrées. Arrêt."
        )
        sys.exit(1)  # Erreur critique: clé de chiffrement manquante

    # Log informatif sur la clé utilisée (tronquée pour la sécurité)
    logger.info(
        f"Utilisation de ENCRYPTION_KEY (commençant par '{encryption_key_to_use[:10].decode('utf-8', 'ignore')}') pour les opérations de chiffrement/déchiffrement."
    )
    ...
    if input_json_file_path:
        try:
            logger.info(
                f"Chargement des définitions d'extraits depuis le fichier JSON non chiffré: {input_json_file_path}..."
            )
            ...
            if isinstance(loaded_data, list):
                ...
                loaded_data is None
            ):  # load_json_file retourne None et logue en cas d'erreur
                logger.error(
                    f"Échec du chargement du fichier JSON {input_json_file_path} (détails ci-dessus). Arrêt."
                )
                sys.exit(1)  # Erreur critique: échec du chargement JSON
            else:  # Le JSON est valide mais n'est pas une liste
                logger.error(
                    f"Le contenu du fichier JSON {input_json_file_path} n'est pas une liste de définitions. Type trouvé: {type(loaded_data)}. Arrêt."
                )
                sys.exit(1)  # Erreur critique: format JSON incorrect
            logger.info(
                f"{len(extract_definitions)} définitions d'extraits chargées depuis {input_json_file_path}."
            )
        except Exception as e:  # Capture d'autres exceptions potentielles
            logger.error(
                f"Erreur inattendue lors du traitement du fichier JSON {input_json_file_path}: {e}. Arrêt."
            )
            sys.exit(1)  # Erreur critique
    elif json_string:
        try:
            logger.info(
                "Chargement des définitions d'extraits depuis la chaîne JSON fournie..."
            )
            loaded_data = json.loads(json_string)
            if not isinstance(loaded_data, list):
                logger.error(
                    "La chaîne JSON fournie ne contient pas une liste de définitions. Arrêt."
                )
                sys.exit(1)  # Erreur critique: format JSON incorrect
            extract_definitions = loaded_data
            logger.info(
                f"{len(extract_definitions)} définitions d'extraits chargées depuis la chaîne JSON."
            )
        ...
        try:
            logger.info(
                f"Chargement et déchiffrement des définitions d'extraits depuis: {input_config_path}..."
            )
            ...
            logger.info(
                f"{len(extract_definitions)} définitions d'extraits chargées et déchiffrées depuis {input_config_path}."
            )
        except Exception as e:  # Erreur pendant le chargement/déchiffrement
            logger.error(
                f"Erreur majeure lors du chargement ou du déchiffrement de {input_config_path}: {e}. Arrêt."
            )
            sys.exit(1)  # Erreur critique
    else:
        # Ce cas ne devrait pas être atteint grâce aux vérifications initiales, mais par sécurité :
        logger.error(
            "Logique de chargement incohérente: aucune source de configuration n'a été traitée. Arrêt."
        )
        sys.exit(1)
    ...
    for i, source_info in enumerate(extract_definitions):
        ...
        if source_info.get("full_text") and source_info["full_text"].strip():
            logger.info(
                f"  Le texte complet est déjà présent pour la source {source_id}."
            )
        else:
            logger.info(
                f"  Texte complet manquant pour la source {source_id}. Tentative de récupération..."
            )
            try:
                # Déterminer la méthode de récupération (compatibilité avec 'source_type')
                fetch_method = source_info.get(
                    "fetch_method", source_info.get("source_type", "unknown")
                )
                ...
                if fetch_method == "file":
                    ...
                    if file_path_str:
                        ...
                        logger.info(
                            f"  Récupération du contenu du fichier local: {document_path}"
                        )
                        full_text_content_retrieved_this_run = load_document_content(
                            document_path
                        )
                        if (
                            full_text_content_retrieved_this_run is None
                        ):  # load_document_content logue déjà l'erreur
                            logger.warning(
                                f"  load_document_content n'a pas pu lire {document_path} pour la source {source_id}."
                            )
                    else:
                        logger.error(
                            f"  Champ 'path' manquant pour la source locale de type 'file': {source_id}."
                        )
                elif fetch_method in [
                    "url",
                    "api",
                    "web_page",
                ]:  # Autres types gérés par get_full_text_for_source
                    logger.info(
                        f"  Utilisation de get_full_text_for_source pour la source {source_id} (méthode: {fetch_method})."
                    )
                    ...
                    full_text_content_retrieved_this_run = get_full_text_for_source(
                        source_info, app_config=None
                    )
                else:
                    logger.warning(
                        f"  Méthode de récupération '{fetch_method}' non reconnue pour la source {source_id}."
                    )
                ...
                if full_text_content_retrieved_this_run:
                    ...
                    logger.info(
                        f"  Texte complet récupéré et mis à jour pour la source {source_id} (longueur: {len(full_text_content_retrieved_this_run)})."
                    )
                    updated_sources_count += 1
                elif not (
                    source_info.get("full_text") and source_info["full_text"].strip()
                ):  # Si toujours pas de texte
                    logger.warning(
                        f"  Impossible de récupérer le texte complet pour la source {source_id} (méthode: {fetch_method}). 'full_text' reste vide ou inchangé."
                    )
                    sources_with_errors_count += 1
            except FileNotFoundError as fnf_err:
                logger.error(
                    f"  Fichier non trouvé lors de la récupération du texte pour la source {source_id}: {fnf_err}"
                )
                sources_with_errors_count += 1
            except (
                Exception
            ) as e:  # Capturer les autres erreurs pendant la récupération
                logger.error(
                    f"  Erreur générique lors de la récupération du texte pour la source {source_id}: {e}"
                )
                sources_with_errors_count += 1
        ...
        if generate_embeddings_model and (source_info.get("full_text") or "").strip():
            ...
            logger.info(
                f"  Tentative de génération d'embeddings pour la source {source_id} avec le modèle '{generate_embeddings_model}'..."
            )
            try:
                ...
                embeddings = get_embeddings_for_chunks(
                    text_chunks, generate_embeddings_model
                )

                if (
                    embeddings and embeddings[0] is not None
                ):  # Vérifier si des embeddings ont été retournés et ne sont pas None
                    logger.info(
                        f"    Embeddings générés: {len(embeddings)} vecteur(s). Dimension du premier: {len(embeddings[0])}."
                    )
                    ...
                    embeddings_output_dir = (
                        base_output_dir / "embeddings_data"
                    )  # Répertoire dédié pour les embeddings
                    ...
                    sanitized_model_name = sanitize_filename(
                        str(generate_embeddings_model)
                    )
                    ...
                    logger.info(
                        f"    Sauvegarde des embeddings pour la source {source_id} dans: {output_path_embeddings}"
                    )
                    if save_embeddings_data(
                        embeddings_data_to_save, output_path_embeddings
                    ):
                        logger.info(
                            f"    Embeddings pour {source_id} sauvegardés avec succès."
                        )
                    else:  # save_embeddings_data logue déjà l'échec
                        logger.error(
                            f"    Échec de la sauvegarde des embeddings pour {source_id} (détails ci-dessus)."
                        )
                else:
                    logger.warning(
                        f"    Aucun embedding valide n'a été généré ou retourné pour la source {source_id} avec le modèle '{generate_embeddings_model}'."
                    )
            except (
                Exception
            ) as emb_exc:  # Erreur pendant la génération ou sauvegarde des embeddings
                logger.error(
                    f"    Erreur lors de la génération/sauvegarde des embeddings pour la source {source_id} (modèle '{generate_embeddings_model}'): {emb_exc}"
                )
    ...
    logger.info(
        f"Traitement des sources terminé. {updated_sources_count} sources ont eu leur texte complet mis à jour/récupéré. {sources_with_errors_count} erreurs lors de la récupération de texte."
    )
    ...
    try:
        logger.info(
            f"Sauvegarde des définitions d'extraits (potentiellement mises à jour) dans {output_config_path}..."
        )
        ...
        if save_success:
            logger.info(
                f"Définitions d'extraits sauvegardées avec succès dans {output_config_path}."
            )
        else:  # save_extract_definitions logue déjà l'échec
            logger.error(
                f"Échec de la sauvegarde des définitions dans {output_config_path} (détails ci-dessus)."
            )
        ...
        logger.error(
            f"Erreur majeure lors de la tentative de sauvegarde des définitions dans {output_config_path}: {e}"
        )
        sys.exit(1)  # Erreur critique
