...
def surface_restitution(
    results: Dict[str, Any], restitution_output: Optional[str] = None
) -> None:
    """Surface the restitution report in the CLI summary + optional Markdown write.

    CONV-D #1335. If ``results["restitution_report"]`` is present (a
    ``RenderedReport``), print the gate-lisibilité band + report length. If
    ``restitution_output`` is set, write the Markdown there (caller picks a
    gitignored path for real corpora). No-op when the report is absent (the run
    was started without ``--render-restitution``, or rendering failed fail-loud).
    """
    report = results.get("restitution_report")
    if report is None:
        return
    markdown = getattr(report, "markdown", "") or ""
    verdict = getattr(report, "verdict", None)
    band = getattr(verdict, "band", "?") if verdict is not None else "?"
    print(f"  Restitution : gate={band}, {len(markdown)} chars")
    if restitution_output:
        out_path = Path(restitution_output)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(markdown, encoding="utf-8")
        logging.info(f"Restitution Markdown écrite dans {out_path}")
...
async def run_modern_analysis(
    ...
    restitution_output: Optional[str] = None,
) -> Dict[str, Any]:
    ...
    # CONV-D #1335: surface the restitution report if one was rendered.
    surface_restitution(results, restitution_output)
...
Exemples:
  ...
  %(prog)s --text "Mon argument" --mode conversational  # Mode conversationnel (agents dialoguent)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Render the readable 3-act restitution report from the completed state "
        "(CONV-D #1335). Works for both --mode pipeline (acts already generated by "
        "the spectacular phases, assembled) and --mode conversational (acts generated "
        "from the completed state, LLM-costly). Off by default.",
    )
    parser.add_argument(
        "--restitution-output",
        type=str,
        default=None,
        help="Path to write the rendered restitution Markdown (only with "
        "--render-restitution). For real corpora, pick a gitignored local path "
        "(privacy: never commit corpus-derived content).",
    )
    ...
    if mode == "sherlock_modern":
        ...
        results = await run_conversational_analysis(
            ...
            render_restitution=args.render_restitution,
        )
        ...
        # CONV-D #1335: surface the restitution report if one was rendered.
        surface_restitution(results, getattr(args, "restitution_output", None))
        ...
        await run_modern_analysis(
            ...
            render_restitution=args.render_restitution,
            restitution_output=args.restitution_output,
        )
