...
def create_marker_editor_ui():
    ...
    extract_definitions, error_message = load_extract_definitions_safely(
        CONFIG_FILE, ENCRYPTION_KEY, CONFIG_FILE_JSON
    )
    ...
    source_dropdown = widgets.Dropdown(
        options=[
            (s.get("source_name", f"Source #{i}"), i)
            for i, s in enumerate(extract_definitions)
        ],
        description="Source:",
    ...
    search_button = widgets.Button(
        description="Rechercher", button_style="info", icon="search"
    )

    # Boutons d'action
    verify_button = widgets.Button(
        description="Vérifier", button_style="info", icon="check"
    )

    suggest_button = widgets.Button(
        description="Suggérer corrections", button_style="warning", icon="search"
    )

    extract_auto_button = widgets.Button(
        description="Extraire automatiquement", button_style="primary", icon="magic"
    )

    save_button = widgets.Button(
        description="Sauvegarder", button_style="success", icon="save"
    )

    # Boutons d'export/import
    export_button = widgets.Button(
        description="Exporter JSON", button_style="info", icon="download"
    )

    import_button = widgets.Button(
        description="Importer JSON", button_style="info", icon="upload"
    )
    ...
    def update_extract_list(change):
        ...
        if source_idx is not None and 0 <= source_idx < len(extract_definitions):
            ...
            extract_dropdown.options = [
                (e.get("extract_name", f"Extrait #{i}"), i)
    ...
    def load_extract_details(change):
        ...
        if source_text:
            ...
            # Affichage du texte extrait
            if extracted_text:
                extract_display.value = (
                    f"<h4>Texte extrait:</h4><p>{extracted_text[:1000]}</p>"
                )
                if len(extracted_text) > 1000:
                    extract_display.value += "<p>[...]</p>"
            else:
                extract_display.value = (
                    "<p>Aucun texte extrait avec les marqueurs actuels.</p>"
                )
            ...
            if end_marker:
                ...
                if similar_end:
                    for i, (context, pos, match) in enumerate(similar_end):
                        ...
                        def create_end_apply_handler(marker):
                            def apply_handler(b):
                        ...
                        apply_button.on_click(create_end_apply_handler(match))
                        display(apply_button)
                else:
    ...
    def search_text(b):
        ...
        with search_output:
            ...
            html_results, count = highlight_search_results(
                source_text, search_term, case_sensitive_checkbox.value
            )
    ...
    def save_modifications(b):
        ...
        extract_definitions[source_idx]["extracts"][extract_idx][
            "start_marker"
        ] = start_marker
        extract_definitions[source_idx]["extracts"][extract_idx][
            "end_marker"
        ...
        if template_start:
            ...
            "template_start" in extract_definitions[source_idx]["extracts"][extract_idx]
        ):
            del extract_definitions[source_idx]["extracts"][extract_idx][
                "template_start"
            ]
    ...
    async def extract_automatically(b):
        ...
        try:
            ...
            llm_service = create_llm_service(service_id="extract_editor_llm")
            if not llm_service:
                with status_output:
                    clear_output(wait=True)
            ...
            if not llm_service_instance:
                with status_output:
                    clear_output(wait=True)
                    print(
                        "❌ Impossible de créer l'instance du service LLM pour l'éditeur."
                    )
                return
            ...
            if (
                not extract_agent
            ):  # Ce test n'est plus vraiment pertinent, l'instanciation lèverait une erreur
                with status_output:
                    clear_output(wait=True)
    ...
    extract_auto_button.on_click(
        lambda b: asyncio.create_task(extract_automatically(b))
    )
    ...
    editor_ui = widgets.VBox(
        [
            ...
            widgets.HTML(
                "<p>Cet outil permet d'ajuster les bornes des extraits dans l'application d'analyse d'argumentation.</p>"
            ),
            ...
            widgets.HBox(
                [verify_button, suggest_button, extract_auto_button, save_button]
            ),
            status_output,
