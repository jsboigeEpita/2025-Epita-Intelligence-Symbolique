...
class OrchestrationResponse(BaseModel):
    ...
    error_message: Optional[str] = Field(
        None, description="Un message décrivant l'erreur en cas d'échec."
    )
...
class Capability(BaseModel):
    ...
    input_schema: Dict[str, Any] = Field(
        ..., alias="input", description="Schéma JSON de l'objet d'entrée attendu."
    )
    output_schema: Dict[str, Any] = Field(
        ..., alias="output", description="Schéma JSON de l'objet de sortie produit."
    )
...
class PluginManifest(BaseModel):
    ...
    plugin_type: Literal["WORKFLOW", "STANDARD"] = Field(
        ...,
        description="Type de plugin, 'WORKFLOW' pour orchestrer d'autres plugins, 'STANDARD' pour une tâche atomique.",
    )
    ...
    entrypoint: str = Field(
        ...,
        description="Chemin d'importation vers la classe principale du plugin (ex: 'my_plugin.main.MyPlugin').",
    )
    dependencies: List[str] = Field(
        default_factory=list,
        description="Liste des noms de plugins requis par ce plugin.",
    )
    capabilities: List[Capability] = Field(
        ..., description="Liste des capacités fournies par ce plugin."
    )
...
class BenchmarkResult(BaseModel):
    ...
    request_id: str = Field(
        ..., description="ID de la requête d'orchestration associée."
    )
    is_success: bool = Field(
        ..., description="True si l'exécution a réussi, False sinon."
    )
    duration_ms: float = Field(
        ..., description="Temps d'exécution de la requête en millisecondes."
    )
    output: Optional[Dict[str, Any]] = Field(
        None, description="Sortie de l'exécution en cas de succès."
    )
    error: Optional[str] = Field(None, description="Message d'erreur en cas d'échec.")
    custom_metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Métrique personnalisée pour cette exécution, ex: nombre de tokens.",
    )
...
class BenchmarkSuiteResult(BaseModel):
    ...
    total_duration_ms: float = Field(
        ..., description="Durée totale de toutes les exécutions en millisecondes."
    )
    average_duration_ms: float = Field(
        ..., description="Durée moyenne d'une exécution réussie en millisecondes."
    )
    min_duration_ms: float = Field(
        ..., description="Durée minimale d'une exécution réussie en millisecondes."
    )
    max_duration_ms: float = Field(
        ..., description="Durée maximale d'une exécution réussie en millisecondes."
    )

    results: List[BenchmarkResult] = Field(
        ..., description="Liste détaillée des résultats de chaque exécution."
    )
    aggregated_custom_metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Agrégation des métriques personnalisées sur toute la suite.",
    )
