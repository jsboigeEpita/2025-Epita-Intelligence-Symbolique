...
def count_fallacies(results: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    for result in results:
        ...
        base_analysis_part = result.get(
            "analyses", result
        )  # Pour compatibilité avec différentes structures de résultats
        ...
        for arg_result in argument_results:
            if isinstance(arg_result, dict):  # Ajout de vérification
                fallacy_counts["base_contextual"] += len(
        ...
        if isinstance(complex_fallacies, dict):  # Ajout de vérification
            fallacy_counts["advanced_complex"] += complex_fallacies.get(
                "individual_fallacies_count", 0
            )
            fallacy_counts["advanced_complex"] += len(
        ...
        advanced_contextual_data = base_analysis_part.get(
            "contextual_fallacies", {}
        )  # Peut être redondant si déjà lu
        if isinstance(advanced_contextual_data, dict):  # Ajout de vérification
            fallacy_counts["advanced_contextual"] += advanced_contextual_data.get(
...
def extract_confidence_scores(results: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    for result in results:
        ...
        if not isinstance(analyses_data, dict):
            continue  # Ignorer si pas un dict
        ...
        if isinstance(base_coherence, dict):
            base_coherence_score = base_coherence.get("overall_coherence", {}).get(
                "score", 0.0
            )
            confidence_scores_lists["base_coherence"].append(
                float(base_coherence_score)
            )
        ...
        if isinstance(rhetorical_results, dict):
            ...
            if isinstance(overall_analysis, dict):
                ...
                confidence_scores_lists["advanced_rhetorical"].append(
                    float(rhetorical_quality)
                )
            ...
            if isinstance(coherence_analysis, dict):
                overall_coherence_data = coherence_analysis.get(
                    "overall_coherence", {}
                )  # C'était 0.0 avant
                ...
                if isinstance(overall_coherence_data, dict):
                    ...
                    overall_coherence_data, (float, int)
                ):  # Cas où c'est directement un score
                    overall_coherence_score = float(overall_coherence_data)
                confidence_scores_lists["advanced_coherence"].append(
                    float(overall_coherence_score)
                )
...
def analyze_contextual_richness(results: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    for result in results:
        ...
        if isinstance(base_contextual_data, dict):
            ...
            base_richness = float(
                len(contextual_factors) if isinstance(contextual_factors, dict) else 0
            )
        ...
        if isinstance(rhetorical_results, dict):
            ...
            if isinstance(overall_analysis, dict):
                rhetorical_richness_score += len(
                    overall_analysis.get("main_strengths", [])
                )
                rhetorical_richness_score += len(
                    overall_analysis.get("main_weaknesses", [])
                )
                if overall_analysis.get("context_relevance"):
                    relevance_value = overall_analysis.get("context_relevance")
                    if (
                        isinstance(relevance_value, (int, float))
                        and relevance_value > 0
                    ):
            ...
            richness_scores_lists["advanced_rhetorical"].append(
                rhetorical_richness_score
            )
