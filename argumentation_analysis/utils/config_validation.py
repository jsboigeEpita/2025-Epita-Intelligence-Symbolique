...
def identify_missing_full_text_segments(
    ...
    """
    logger.info(f"Analyse du fichier de configuration pour les segments manquants: {config_file_path}")
    ...
    for i, source in enumerate(sources_data):
        ...
        if not isinstance(extracts, list):
            logger.warning(f"La source '{source_name}' (ID: {source_id}) a un champ 'extracts' qui n'est pas une liste. Ignorée.")
            continue

        for j, extract in enumerate(extracts):
            total_extracts_count += 1
            if not isinstance(extract, dict):
                logger.warning(f"Extrait {j+1} de la source '{source_name}' n'est pas un dictionnaire, ignoré.")
                continue
            ...
            if segment is None or segment == "":
                ...
                has_full_text_in_source = "PRÉSENT" if source.get("full_text") else "ABSENT"
    ...
    if missing_segments_count == 0:
        ...
        logger.info(f"Nombre total d'extraits avec segment manquant ou vide : {missing_segments_count}")
