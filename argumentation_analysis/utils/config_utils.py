...
from typing import (
    ...
    Union,
    Tuple,
)  # Ajout de Set, Union, et Tuple

logger = logging.getLogger(__name__)


def find_sources_in_config_by_ids(
    config_data: Union[List[Dict[str, Any]], Dict[str, Any]], ids_to_find: List[str]
    ...
    if isinstance(config_data, list):
        ...
        logger.debug(
            "Configuration fournie sous forme de dictionnaire. Tentative de localisation de la liste des sources."
        )
            ...
            if id_to_check in config_data and id_to_check not in found_ids_set:
                ...
                if isinstance(source_config, dict):
                    ...
                    if "id" not in source_config:
                        ...
                        logger.info(
                            f"Source trouvée par clé ID (ID aussi dans le dict): {id_to_check}"
                        )
                        found_sources_list.append(source_config)
                    found_ids_set.add(id_to_check)
                else:
                    logger.warning(
                        f"La clé ID '{id_to_check}' trouvée dans le dictionnaire de config, mais sa valeur n'est pas un dictionnaire de source."
                    )
        ...
        if (
            not source_list_found_in_dict and not found_sources_list
        ):  # Si rien n'a été trouvé par les méthodes précédentes
            process_source_item(
                config_data
            )  # Traiter le dictionnaire racine comme une source potentielle

    else:
        logger.warning(
            f"Format de configuration non supporté (type: {type(config_data)}). Attendu: liste ou dictionnaire."
        )
