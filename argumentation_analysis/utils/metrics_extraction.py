...
def extract_execution_time_from_results(
    ...
    for result in results:
        ...
        )  # Assurer un nom unique si manquant
        main_timestamp_str = result.get("timestamp")

        if not main_timestamp_str:
            logger.debug(
                f"Timestamp principal manquant pour l'extrait '{extract_name}'. Temps d'exécution non calculables."
            )
            continue
        ...
        try:
            ...
            logger.warning(
                f"Format de timestamp principal invalide pour '{extract_name}': {main_timestamp_str}. Erreur: {e}"
            )
            continue
        ...
        if not isinstance(analyses_data, dict):
            logger.debug(
                f"Aucune donnée d'analyse (ou format incorrect) pour '{extract_name}'."
            )
            continue
        ...
        for analysis_type, analysis_details in analyses_data.items():
            if not isinstance(analysis_details, dict):
                logger.debug(
                    f"Détails d'analyse pour '{analysis_type}' dans '{extract_name}' n'est pas un dictionnaire."
                )
                continue
            ...
            if not analysis_timestamp_str:
                logger.debug(
                    f"Timestamp d'analyse manquant pour '{analysis_type}' dans '{extract_name}'."
                )
                continue
            ...
            try:
                ...
                logger.warning(
                    f"Format de timestamp d'analyse invalide pour '{analysis_type}' dans '{extract_name}': {analysis_timestamp_str}. Erreur: {e}"
                )
...
def count_fallacies_in_results(
    ...
    for result in results:
        ...
        if isinstance(complex_fallacies_analysis, dict):
            adv_complex_total += complex_fallacies_analysis.get(
                "individual_fallacies_count", 0
            )
            adv_complex_total += len(
                complex_fallacies_analysis.get("basic_combinations", [])
            )
            adv_complex_total += len(
                complex_fallacies_analysis.get("advanced_combinations", [])
            )
            adv_complex_total += len(
                complex_fallacies_analysis.get("fallacy_patterns", [])
            )
...
def extract_confidence_scores_from_results(
    ...
    for result in results:
        ...
        if analyses_data is None or not isinstance(analyses_data, dict):
            logger.debug(
                f"Clé 'analyses' absente ou format incorrect pour '{extract_name}' lors de l'extraction des scores de confiance. Retour d'un dict vide."
            )
        ...
        if isinstance(base_coherence_analysis, dict):
            overall_coherence_data = base_coherence_analysis.get(
                "overall_coherence", {}
            )
            if isinstance(overall_coherence_data, dict):
                current_extract_scores["base_coherence"] = overall_coherence_data.get(
                    "score", 0.0
                )
            else:
        ...
        if isinstance(rhetorical_results_analysis, dict):
            overall_rhetorical_analysis = rhetorical_results_analysis.get(
                "overall_analysis", {}
            )
            ...
            coherence_sub_analysis = rhetorical_results_analysis.get(
                "coherence_analysis", {}
            )
        ...
        if isinstance(fallacy_severity_analysis, dict):
            current_extract_scores["advanced_severity"] = fallacy_severity_analysis.get(
    ...
    logger.info(
        f"Extraction des scores de confiance terminée pour {len(confidence_scores)} extraits."
    )
...
def analyze_contextual_richness_from_results(
    ...
    for result in results:
        ...
        if analyses_data is None or not isinstance(analyses_data, dict):
            logger.debug(
                f"Clé 'analyses' absente ou format incorrect pour '{extract_name}' lors de l'analyse de richesse contextuelle. Retour d'un dict vide."
            )
        ...
        if isinstance(advanced_contextual_data, dict):
            ...
            if isinstance(context_analysis, dict):
                if context_analysis.get("context_type"):
                    adv_contextual_richness += 1
                adv_contextual_richness += len(
                    context_analysis.get("context_subtypes", [])
                )
                adv_contextual_richness += len(
                    context_analysis.get("audience_characteristics", [])
                )
        ...
        if isinstance(rhetorical_results_data, dict):
            ...
            if isinstance(overall_analysis_data, dict):
                adv_rhetorical_richness += len(
                    overall_analysis_data.get("main_strengths", [])
                )
                adv_rhetorical_richness += len(
                    overall_analysis_data.get("main_weaknesses", [])
                )
                if overall_analysis_data.get("context_relevance"):
                    adv_rhetorical_richness += 1
    ...
    logger.info(
        f"Analyse de la richesse contextuelle terminée pour {len(richness_scores)} extraits."
    )
...
def evaluate_coherence_relevance_from_results(
    ...
    for result in results:
        ...
        if analyses_data is None or not isinstance(analyses_data, dict):
            logger.debug(
                f"Clé 'analyses' absente ou format incorrect pour '{extract_name}' lors de l'évaluation de pertinence/cohérence. Retour d'un dict vide."
            )
        ...
        if isinstance(rhetorical_results_data, dict):
            coherence_analysis_data = rhetorical_results_data.get(
                "coherence_analysis", {}
            )
            if isinstance(coherence_analysis_data, dict):
                if coherence_analysis_data.get("overall_coherence"):
                    adv_coherence_relevance += 1
                if coherence_analysis_data.get("coherence_level"):
                    adv_coherence_relevance += 1
                if coherence_analysis_data.get("thematic_coherence"):
                    adv_coherence_relevance += 1
                if coherence_analysis_data.get("logical_coherence"):
                    adv_coherence_relevance += 1

            recommendations_data = rhetorical_results_data.get("recommendations", {})
            if isinstance(recommendations_data, dict):
                coherence_recommendations_list = recommendations_data.get(
        ...
        current_extract_scores["advanced_recommendations"] = float(
            adv_recommendations_relevance
        )

        relevance_scores[extract_name] = current_extract_scores

    logger.info(
        f"Évaluation de la pertinence de cohérence terminée pour {len(relevance_scores)} extraits."
    )
...
def _calculate_obj_complexity(obj: Any, depth: int = 0) -> float:
    ...
    if isinstance(obj, dict):
        if not obj:
            return float(depth)
        return depth + sum(
            _calculate_obj_complexity(v, depth + 1) for v in obj.values()
        ) / len(obj)
    elif isinstance(obj, list):
        if not obj:
            return float(depth)
        return depth + sum(_calculate_obj_complexity(v, depth + 1) for v in obj) / len(
            obj
        )
    else:
        return float(depth)
...
def analyze_result_complexity_from_results(
    ...
    for result in results:
        ...
        if not isinstance(analyses_data, dict):
            logger.debug(
                f"Aucune donnée d'analyse pour '{extract_name}' lors de l'analyse de complexité."
            )
        ...
        for analysis_type, analysis_content in analyses_data.items():
            if isinstance(analysis_content, dict) or isinstance(analysis_content, list):
                current_extract_scores[analysis_type] = _calculate_obj_complexity(
                    analysis_content
                )
            else:
    ...
    logger.info(
        f"Analyse de la complexité des résultats terminée pour {len(complexity_scores)} extraits."
    )
