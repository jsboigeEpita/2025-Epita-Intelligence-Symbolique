...
class UnifiedAnalysisPipeline:
    ...
    async def analyze_text(
        ...
        try:
            ...
            self.logger.info(
                f"[OK] Analyse {analysis_id} terminée en {result.execution_time:.2f}s"
            )
    ...
    async def analyze_batch(
        self, texts: List[str], source_type: SourceType = SourceType.BATCH
    ) -> List[AnalysisResult]:
        """
    ...
    async def _execute_analysis_mode(
        self, text: str, mode: AnalysisMode
    ) -> Dict[str, Any]:
    ...
    async def _real_llm_analysis(self, text: str, mode: AnalysisMode) -> Dict[str, Any]:
        ...
        try:
            ...
            mock_level = (
                MockLevel.NONE
                if self.config.mock_level == "none"
                else MockLevel.MINIMAL
            )
            ...
            agent = InformalAnalysisAgent(
                kernel=kernel, agent_name="PipelineInformalAgent"
            )
    ...
    async def _analyze_batch_parallel(
        self, texts: List[str], source_type: SourceType
    ) -> List[AnalysisResult]:
    ...
    async def _analyze_batch_sequential(
        self, texts: List[str], source_type: SourceType
    ) -> List[AnalysisResult]:
