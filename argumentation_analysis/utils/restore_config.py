...
def restore_config_files():
    ...
    """
    logger.info(
        f"Restauration des fichiers de configuration à partir de {CONFIG_FILE_ENC}..."
    )
    ...
    if (
        not extract_definitions
        or not isinstance(extract_definitions, list)
        or len(extract_definitions) == 0
    ):
        logger.error("Les définitions d'extraits n'ont pas été chargées correctement.")
        return False

    logger.info(
        f"[OK] Définitions d'extraits chargées avec succès: {len(extract_definitions)} sources trouvées."
    )
