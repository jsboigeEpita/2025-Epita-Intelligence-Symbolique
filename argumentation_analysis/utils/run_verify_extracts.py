...
def main():
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="verify_extracts_report.html",
    ...
    if invalid_count > 0:
        ...
        for result in results:
            if result.get("status") == "invalid":
                logger.info(
                    f"  - {result.get('source_name')} - {result.get('extract_name')}: {result.get('message')}"
                )
    ...
    print(
        f"Résumé: {valid_count} valides, {warning_count} avec avertissements, {invalid_count} invalides, {error_count} erreurs"
    )
