...
def generate_markdown_performance_report(
    ...
    report_content.append("\n## 2. Données Analysées")
    report_content.append(
        f"- Nombre d'extraits pour l'analyse de base: {base_results_summary.get('count', 'N/A')}"
    )
    sources_base = base_results_summary.get("sources", [])
    report_content.append(
        f"- Sources pour l'analyse de base: {', '.join(sources_base) if sources_base else 'N/A'}"
    )
    report_content.append(
        f"- Nombre d'extraits pour l'analyse avancée: {advanced_results_summary.get('count', 'N/A')}"
    )
    sources_advanced = advanced_results_summary.get("sources", [])
    report_content.append(
        f"- Sources pour l'analyse avancée: {', '.join(sources_advanced) if sources_advanced else 'N/A'}"
    )
    ...
    if metrics:
        try:
            ...
            report_content.append("\n### Détection de Sophismes")
            report_content.append(
                "- Données insuffisantes ou format incorrect pour déterminer le meilleur agent pour la détection de sophismes."
            )

        try:
            richest_context_agent = max(
                metrics.items(),
            ...
            report_content.append("\n### Richesse Contextuelle")
            report_content.append(
                "- Données insuffisantes ou format incorrect pour déterminer l'agent avec la meilleure richesse contextuelle."
            )
    ...
    report_content.append("\n## 5. Conclusion")
    report_content.append(
        "Le choix de l'agent optimal dépendra des priorités spécifiques de l'analyse (précision, rapidité, profondeur, etc.). Ce rapport fournit une base quantitative pour guider cette sélection."
    )
    report_content.append(
    ...
    try:
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("\n".join(report_content))
        logger.info(
            f"Rapport de performance Markdown généré avec succès : {output_file}"
        )
    except Exception as e:
        logger.error(
            f"Erreur lors de l'écriture du rapport Markdown {output_file}: {e}",
            exc_info=True,
        )
