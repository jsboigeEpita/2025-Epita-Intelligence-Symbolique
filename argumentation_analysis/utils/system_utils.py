...
def get_project_root():
    ...
    for _ in range(10):  # Limite de 10 niveaux de recherche vers le haut
        ...
        ):  # On a atteint la racine du système de fichiers (ex: C:\)
            break
...
def check_and_install(package_import_name: str, package_install_name: str):
    ...
    try:
        ...
        logger.warning(
            f"⚠️ Dépendance '{package_import_name}' manquante (package: {package_install_name}). Tentative d'installation..."
        )
        try:
            ...
            logger.error(
                f"❌ Échec de l'installation/import de {package_install_name}: {e}"
            )
            logger.warning(
                "‼️ Un redémarrage du noyau (Kernel -> Restart Kernel) peut être nécessaire si l'import échoue toujours."
            )
            return False
