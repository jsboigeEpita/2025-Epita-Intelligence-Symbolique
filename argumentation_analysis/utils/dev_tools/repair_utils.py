...
async def setup_agents(
    llm_service, kernel_instance: sk.Kernel
):  # -> Tuple[ChatCompletionAgent, ChatCompletionAgent]: # Type de retour modifié
    """
...
async def repair_extract_markers(
    ...
    for source_idx, source_info in enumerate(extract_definitions.sources):
        ...
        for extract_idx, extract_info in enumerate(source_info.extracts):
            ...
            if template_start and "{0}" in template_start:
                prefix_to_add = template_start.split("{0}", 1)[
                    0
                ]  # Extrait la partie avant {0}
                if start_marker and not start_marker.startswith(
                    prefix_to_add
                ):  # Vérifie si le start_marker commence déjà par le préfixe
                    ...
                    logger.info(
                        f"Correction de l'extrait '{extract_name}': '{old_start}' -> '{new_start}'"
                    )
                    ...
                    message = (
                        f"Marqueur de début corrigé: '{old_start}' -> '{new_start}'"
                    )
            ...
            results.append(
                {
                    ...
                    "explanation": (
                        f"Première lettre manquante ajoutée selon le template '{template_start}'"
                        if current_status == "repaired"
                        else "Aucune correction appliquée."
                    ),
                }
            )
    ...
    logger.info(
        f"{sum(1 for r in results if r['status'] == 'repaired')} extraits corrigés (logique simplifiée)."
    )
...
async def run_extract_repair_pipeline(
    ...
    try:
        llm_service = create_llm_service(
            service_id="repair_utils", model_id="gpt-4o-mini"
        )
        if not llm_service:
        ...
        if not extract_definitions.sources:
            logger.warning(
                "Aucune source restante après filtrage. Arrêt du pipeline de réparation."
            )
            return
        ...
        logger.info(
            f"Réparation terminée (pipeline). {len(results)} résultats obtenus."
        )
        ...
        if save_changes:
            ...
            success, error_msg_save = await definition_service.save_definitions(
                updated_definitions
            )
            if success:
                ...
                logger.error(
                    f"❌ Erreur lors de la sauvegarde (pipeline): {error_msg_save}"
                )
        ...
        if output_json_path_str:
            ...
            logger.info(
                f"Exportation des définitions JSON mises à jour vers {output_json_file} (pipeline)..."
            )
            (
                success_export,
                msg_export,
            ) = await definition_service.export_definitions_to_json(
        ...
        logger.error(
            f"Exception non gérée dans le pipeline de réparation: {e}", exc_info=True
        )
