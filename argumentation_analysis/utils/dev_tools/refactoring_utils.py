...
DEFAULT_IMPORT_PATTERNS: List[Tuple[str, str]] = [
    ...
    (
        r"import\s+orchestration(?!\s*\.)",
        r"import argumentation_analysis.orchestration",
    ),
    (
    ...
    ),  # Attention, peut être trop générique
...
def update_imports_in_file_content(
    content: str, import_patterns: List[Tuple[str, str]] = DEFAULT_IMPORT_PATTERNS
) -> Tuple[str, int]:
    """
...
def update_imports_in_file(
    ...
    try:
        ...
        updated_content, total_replacements = update_imports_in_file_content(
            content, import_patterns
        )

        if total_replacements > 0:
            logger.info(
                f"Fichier {file_path}: {total_replacements} importation(s) potentiellement à mettre à jour."
            )
            if not dry_run:
        ...
        logger.error(
            f"Erreur lors de la mise à jour du fichier {file_path}: {e}", exc_info=True
        )
        return 0, None
...
def update_imports_in_directory(
    ...
    logger.info(
        f"Mise à jour des importations dans le répertoire: {directory} (dry_run: {dry_run})"
    )
    ...
    for file_path in directory.rglob("*"):
        if file_path.is_file():
            ...
            if file_path.suffix not in extensions:
                logger.debug(
                    f"Fichier ignoré (extension non correspondante): {file_path}"
                )
                continue

            stats["total_files_scanned"] += 1

            replacements, _ = update_imports_in_file(
                file_path, dry_run, import_patterns
            )

            if replacements > 0:
                stats["files_with_potential_changes"] += 1
                stats["total_potential_replacements"] += replacements
                stats["modified_files_list_details"].append(
                    {"path": str(file_path), "replacements": replacements}
                )
                # Le logging détaillé est déjà fait par update_imports_in_file

    logger.info(
        f"Scan terminé. {stats['total_potential_replacements']} remplacement(s) potentiel(s) dans {stats['files_with_potential_changes']} fichier(s) sur {stats['total_files_scanned']} scanné(s)."
    )
    return stats
...
DEFAULT_PATHS_MODULE_IMPORT_PATTERN: str = (
    r"from\s+argumentation_analysis\.paths\s+import\s+([A-Z_0-9,\s]+)"
)
...
def update_paths_in_file_content(
    ...
    for pattern, replacement_str_format in path_patterns:
        ...
        match_const = re.search(
            r"([A-Z_]+(?:_DIR|_FILE))\s*(?:/)?", replacement_str_format
        )
        if match_const:
            ...
            if replacement_str_format.isupper() and (
                "_DIR" in replacement_str_format or "_FILE" in replacement_str_format
            ):
                if re.search(pattern, temp_content_for_scan):
    ...
    if total_path_replacements > 0 and referenced_path_constants:
        ...
        new_import_statement_str = (
            f"from {paths_module_qualifier} import {', '.join(paths_to_import_list)}"
        )
        ...
        if match_import:
            ...
            existing_constants = set(
                c.strip() for c in existing_imported_constants_str.split(",")
            )

            missing_constants = referenced_path_constants - existing_constants
            if missing_constants:
                all_needed_constants = sorted(
                    list(existing_constants | referenced_path_constants)
                )
                updated_import_line = f"from {paths_module_qualifier} import {', '.join(all_needed_constants)}"
                updated_content = import_pattern_re.sub(
                    updated_import_line, updated_content, count=1
                )
                import_modified_or_added = True
                logger.debug(
                    f"Importation de chemins mise à jour : {updated_import_line}"
                )
        else:
...
def update_paths_in_file(
    ...
    try:
        ...
        if total_replacements > 0 or import_changed:
            logger.info(
                f"Fichier {file_path}: {total_replacements} référence(s) de chemin potentiellement à mettre à jour. Import modifié/ajouté: {import_changed}."
            )
            if not dry_run:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(updated_content)
            ...
            logger.debug(
                f"Aucune référence de chemin à mettre à jour ou import à modifier dans {file_path}."
            )
        ...
        logger.error(
            f"Erreur lors de la mise à jour des chemins dans le fichier {file_path}: {e}",
            exc_info=True,
        )
        return 0, False, None
...
def update_paths_in_directory(
    ...
    logger.info(
        f"Mise à jour des chemins dans le répertoire: {directory} (dry_run: {dry_run})"
    )
    ...
    for file_path in directory.rglob("*"):
        if file_path.is_file():
            ...
            replacements, import_changed, _ = update_paths_in_file(
                ...
                dry_run,
                path_patterns,
                paths_module_import_pattern,
                paths_module_qualifier,
            )
    ...
    logger.info(
        f"Scan des chemins terminé. {stats['total_path_replacements']} remplacement(s) de chemin dans {stats['files_with_path_changes']} fichier(s)."
    )
    logger.info(
        f"{stats['files_with_import_changes']} fichier(s) avec importations de chemin modifiées/ajoutées sur {stats['total_files_scanned']} scanné(s)."
    )
    return stats
