...
from argumentation_analysis.models.extract_definition import (
    ...
    SourceDefinition,
    Extract,
)  # Pour typer les objets DefinitionService
...
def verify_extract(
    source_info: Dict[str, Any], extract_info: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Vérification de l'extrait '{extract_name}' de la source '{source_name}'..."
    )
    ...
    if not source_text:
        logger.error(
            f"Impossible de charger le texte source pour '{source_name}': {url}"
        )
        return {
    ...
    if start_found and end_found:
        ...
        if len(extracted_text) < 10:
            logger.warning(
                f"Extrait '{extract_name}' valide mais très court ({len(extracted_text)} caractères)."
            )
            current_status = "warning"
            result_details["messages"].append(
                f"Extrait valide mais très court ({len(extracted_text)} caractères)."
            )
        ...
        if (
            template_prefix
            and start_marker
            and not start_marker.startswith(template_prefix)
        ):
        ...
        has_encoding_issues = any(
            char_seq in extracted_text
            for char_seq in special_chars_to_check
            if char_seq
        )
        ...
        if not result_details["messages"] and current_status == "valid":
            result_details["messages"].append(
                "Extrait valide. Les deux marqueurs ont été trouvés."
            )
        elif (
            not result_details["messages"] and current_status == "warning"
        ):  # Fallback si un warning a été mis sans message
...
def verify_all_extracts(
    extract_definitions_list: List[
        Dict[str, Any]
    ],  # Attend une liste de dictionnaires (sources)
...
def generate_verification_report(
    results: List[Dict[str, Any]], output_file_str: str
) -> None:
    """
    ...
    for result in results:
        ...
        if status == "invalid":
            ...
            if result.get("extracted_length", 100) < 10:
...
def run_extract_verification_pipeline(
    ...
    try:
        config_file_to_use = (
            Path(custom_input_path_str) if custom_input_path_str else None
        )
        ...
        current_config_file = (
            config_file_to_use if config_file_to_use else settings.config_file_enc
        )
        ...
        if not current_fallback_file.exists():
            logger.warning(
                f"Fichier de fallback JSON {current_fallback_file.resolve()} non trouvé."
            )
        ...
        fetch_service = FetchService(
            cache_service=cache_service, temp_download_dir=temp_download_dir
        )
        ...
        (
            extract_definitions_obj,
            error_message,
        ) = definition_service.load_definitions()  # C'est un objet ExtractDefinitions
        if error_message:
            logger.warning(
                f"Avertissement lors du chargement des définitions (pipeline): {error_message}"
            )
        ...
        logger.info(
            f"{len(extract_definitions_obj.sources)} sources chargées dans le pipeline."
        )
        ...
        if hasattr(extract_definitions_obj, "sources"):
            for source_def_obj in extract_definitions_obj.sources:
                if hasattr(source_def_obj, "to_dict"):  # Vérifier si la méthode existe
                    ...
                    logger.warning(
                        f"Source {source_def_obj.source_name} n'a pas de méthode to_dict()."
                    )
        ...
        if not definitions_as_list_of_dicts:
            logger.warning(
                "Aucune source restante après filtrage. Arrêt du pipeline de vérification."
            )
            return
        ...
        logger.info(
            f"Vérification terminée (pipeline). {len(results)} résultats obtenus."
        )
        ...
        logger.error(
            f"Exception non gérée dans le pipeline de vérification: {e}", exc_info=True
        )
