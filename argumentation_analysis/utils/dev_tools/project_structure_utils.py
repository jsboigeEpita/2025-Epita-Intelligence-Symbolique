...
def map_package_to_module(
    ...
    for key in sorted_mapping_keys:
        if key == '.' and package_name == '.': # Cas spécial pour le package racine parfois noté '.'
             return current_mapping[key]
        if key != '.' and package_name.startswith(key): # Vérifier si le nom du package commence par une clé de mapping
            return current_mapping[key]
    
    logger.debug(f"Aucun mappage de module trouvé pour le package '{package_name}'. Retour de 'Autre'.")
    return 'Autre'