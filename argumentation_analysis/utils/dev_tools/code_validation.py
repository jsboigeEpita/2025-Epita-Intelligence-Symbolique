...
def check_python_syntax(file_path: str) -> Tuple[bool, str, List[str]]:
    ...
    try:
        ...
        logger.warning(
            f"   Ligne {e.lineno}, colonne {e.offset}: {e.text.strip() if e.text else ''}"
        )

        lines = content.split("\n")
        start_line_ctx = max(0, e.lineno - 3)  # 2 lignes avant
        end_line_ctx = min(
            len(lines), e.lineno + 2
        )  # 2 lignes après (inclusif de la ligne d'erreur)
    ...
    try:
        ...
        for token in tokens:
            if token.type == tokenize.ERRORTOKEN:
                ...
                logger.warning(
                    f"❌ Token d'erreur détecté à la ligne {token.start[0]}, colonne {token.start[1]} dans {file_path}: {token.string!r}"
                )
...
def analyze_directory_references(
    directory_to_scan: str,
    patterns_to_find: Dict[str, Any],
    ...
    """
    results = {
        pattern_name: {"count": 0, "files": {}, "examples": []}
    ...
    logger.info(
        f"Analyse des références dans {directory_to_scan} pour les motifs: {list(patterns_to_find.keys())}"
    )
    ...
    for root, dirs, files in os.walk(directory_to_scan):
        ...
        for file_name in files:
            ...
            try:
                ...
                logger.warning(
                    f"Fichier non trouvé pendant l'analyse des références: {file_path}"
                )
            except UnicodeDecodeError:
                logger.warning(
                    f"Erreur de décodage (non UTF-8) pour le fichier: {file_path}"
                )
            except Exception as e:
                logger.error(
                    f"Erreur lors de la lecture ou de l'analyse du fichier {file_path}: {e}",
                    exc_info=True,
                )

    for pattern_name, data in results.items():
        logger.info(
            f"Motif '{pattern_name}': {data['count']} références trouvées dans {len(data['files'])} fichier(s)."
        )
        if data["examples"]:
...
if __name__ == "__main__":
    ...
    logger.info(
        "--- DÉBUT DES TESTS POUR check_python_syntax ET check_python_tokens ---"
    )
    ...
    if not tokens_ok:
        logger.info(f"Tokens d'erreur: {err_tokens}")
    ...
    if is_valid_syntax_for_token_test:  # Devrait être False à cause de l'indentation
        ...
        logger.info(
            "Syntaxe déjà invalide pour le test de token, ce qui est attendu pour une mauvaise indentation."
        )
    ...
    with open(
        os.path.join(temp_dir_analyze, "subdir", "file2.py"), "w", encoding="utf-8"
    ) as f:
        f.write("import os\n")
    ...
    if (
        analysis_results["config_refs"]["count"] == 2
        and len(analysis_results["config_refs"]["files"]) == 1
        and analysis_results["data_refs"]["count"] == 2
        and len(analysis_results["data_refs"]["files"]) == 2
        and analysis_results["non_existent_refs"]["count"] == 0
    ):
    ...
    if os.path.exists(valid_test_file):
        os.remove(valid_test_file)
    if os.path.exists(invalid_syntax_file):
        os.remove(invalid_syntax_file)
