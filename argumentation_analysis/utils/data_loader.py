...
def load_results_from_json(file_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(results, list):
            logger.warning(
                f"Les données dans {file_path} ne sont pas une liste JSON. Type trouvé: {type(results)}. Retourne une liste vide."
            )
            return []

        logger.info(
            f"[OK] {len(results)} résultats chargés avec succès depuis {file_path}"
        )
        return results
