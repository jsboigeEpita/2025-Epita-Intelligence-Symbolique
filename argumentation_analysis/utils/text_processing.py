...
def split_text_into_arguments(text: str, min_arg_length: int = 10) -> List[str]:
    ...
    if not text or not isinstance(text, str):
        logger.warning(
            "split_text_into_arguments a reçu une entrée non valide (None ou pas une chaîne)."
        )
        return []
    ...
    for delimiter in delimiters:
        ...
        replacement = delimiter.rstrip(
            " "
        )  # Enlève l'espace de fin s'il y en a un ("! " -> "!")
    ...
    for arg in raw_arguments:
        cleaned_arg = (
            arg.strip()
        )  # Nettoyer les espaces au début/fin de chaque segment potentiel
    ...
    if (
        ...
        and text.strip()
        and len(text.strip()) >= min_arg_length
    ):
        ...
        logger.debug(
            f"Aucun délimiteur standard n'a permis de scinder le texte ou les segments étaient trop courts, et le texte original est un argument valide. Texte original considéré comme un seul argument: '{text[:70]}...'"
        )
