...
def generate_sample_text(
    extract_name: str, source_name: str, style: str = "generic"
) -> str:
    """
    ...
    disponible, par exemple pour des tests ou des démonstrations. Le style est
    un identifiant de configuration opaque (#2362) : aucun contenu de nom ne
    sélectionne de branche.

    :param extract_name: Nom de l'extrait (journalisé uniquement).
    :type extract_name: str
    :param source_name: Nom de la source (journalisé uniquement).
    :type source_name: str
    :param style: Style opaque du texte — « civil_address », « formal_oratory »,
                 défaut « generic ».
    :type style: str
    ...
    """
    logger.debug(
        f"Génération de texte d'exemple (style='{style}') pour extract_name='{extract_name}', source_name='{source_name}'"
    )

    # Sélection par identifiant de style opaque
    if style == "civil_address":
        return """
