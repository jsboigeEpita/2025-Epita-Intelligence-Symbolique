...
def save_json_file(file_path: Path, data: Any, indent: int = 4) -> bool:
    ...
    """
    savers_logger.info(f"Tentative de sauvegarde des données JSON dans {file_path} avec une indentation de {indent}")
    try:
        file_path.parent.mkdir(parents=True, exist_ok=True)
        savers_logger.debug(f"Répertoire parent {file_path.parent} pour la sauvegarde JSON vérifié/créé.")
        ...
        savers_logger.error(f"❌ Erreur de type lors de la sérialisation JSON pour le fichier {file_path} (données non sérialisables?): {e}", exc_info=True)
        return False
    except Exception as e:
        savers_logger.error(f"❌ Erreur inattendue lors de la sauvegarde du fichier JSON {file_path}: {e}", exc_info=True)
        return False
...
def save_text_file(file_path: Path, content: str, encoding: str = "utf-8") -> bool:
    ...
    Crée les répertoires parents si nécessaire.
    """
    savers_logger.info(f"Tentative de sauvegarde du contenu dans {file_path} avec l'encodage {encoding}")
    try:
        file_path.parent.mkdir(parents=True, exist_ok=True)
        savers_logger.debug(f"Répertoire parent {file_path.parent} pour la sauvegarde vérifié/créé.")
        ...
        savers_logger.error(f"❌ Erreur d'encodage Unicode lors de la sauvegarde du fichier {file_path} avec l'encodage {encoding}: {e}", exc_info=True)
        return False
    except Exception as e:
        savers_logger.error(f"❌ Erreur inattendue lors de la sauvegarde du fichier {file_path}: {e}", exc_info=True)
        return False
...
def save_temp_extracts_json(
    ...
    if not isinstance(extract_definitions, list):
        savers_logger.error("Les définitions d'extraits fournies ne sont pas une liste.")
        return None

    try:
        current_temp_dir = Path.cwd() / base_temp_dir_name
        current_temp_dir.mkdir(parents=True, exist_ok=True)
        savers_logger.debug(f"Répertoire temporaire pour les extraits: {current_temp_dir.resolve()}")
        ...
        savers_logger.error(f"❌ Erreur inattendue lors de la création ou sauvegarde du fichier d'extraits temporaire: {e}", exc_info=True)
        return None
