...
class CryptoWorkflowManager:
    ...
    def __init__(self, passphrase: Optional[str] = None):
        ...
        if not passphrase_to_use:
            self.logger.warning(
                "Aucune passphrase fournie ou configurée dans les settings. Le déchiffrement échouera probablement."
            )
    ...
    def derive_encryption_key(self) -> bytes:
        ...
        if self._encryption_key is None:
            if not self.passphrase:
                raise ValueError(
                    "Impossible de dériver la clé: aucune passphrase n'est définie."
                )
    ...
    async def load_encrypted_corpus(
        ...
        if not corpus_files:
            self.logger.info(
                "La liste des fichiers de corpus est vide, aucune action requise."
            )
        ...
        try:
            ...
            self.logger.info(
                f"🔓 Déchiffrement de {len(corpus_files)} fichiers de corpus"
            )
            ...
            for corpus_file in corpus_files:
                ...
                try:
                    ...
                    if definitions:
                        ...
                        self.logger.info(
                            f"✅ {corpus_path.name}: {len(definitions)} définitions"
                        )
                    else:
                    ...
                    self.logger.error(error_msg)
                    result.success = (
                        False  # En cas d'erreur d'import, l'opération globale échoue
                    )
            ...
            if not result.errors:
                ...
                result.success = len(result.loaded_files) > 0 and not any(
                    "non disponibles" in err for err in result.errors
                )
            result.processing_time = time.time() - start_time

            self.logger.info(
                f"🎯 Déchiffrement terminé: {len(result.loaded_files)} fichiers, {result.total_definitions} définitions"
            )
    ...
    def validate_corpus_integrity(
        ...
        for file_data in corpus_data["loaded_files"]:
            if not file_data.get("definitions"):
                errors.append(
                    f"Fichier sans définitions: {file_data.get('file', 'inconnu')}"
                )

            for definition in file_data.get("definitions", []):
                if not definition.get("content"):
                    errors.append(
                        f"Définition sans contenu dans {file_data.get('file', 'inconnu')}"
                    )
