...
async def evaluate_extract(
    ...
    logger.info(
        f"Évaluation de l'extrait '{extract_name}' de la source '{source_name}'..."
    )
    ...
    try:
        logger.info(
            f"Invocation de l'agent d'évaluation pour l'extrait '{extract_name}'..."
        )
    ...
    logger.info(
        f"Évaluation terminée pour l'extrait '{extract_name}': {result['status']}"
    )
    return result


async def verify_extracts_with_llm(
    extract_definitions: List[Dict[str, Any]], llm_service
) -> List[Dict[str, Any]]:
...
def generate_report(
    ...
    status_counts = {"valid": 0, "invalid": 0, "error": 0}
