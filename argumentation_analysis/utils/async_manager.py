...
class AsyncManager:
    ...
    def _run_coroutine(self, coro: Coroutine, timeout: float) -> Any:
        ...
        try:
            ...
            self.logger.warning(
                f"Timeout lors de l'exécution de la coroutine après {timeout}s"
            )
            raise
        except Exception as e:
    ...
    def _run_sync_function(
        ...
        try:
            ...
            self.logger.warning(
                f"Timeout lors de l'exécution de la fonction après {timeout}s"
            )
            raise
        except Exception as e:
    ...
    def cleanup_completed_tasks(self, max_age_hours: int = 1) -> int:
        ...
        for task_id, task_info in self.active_tasks.items():
            if (
                task_info.get("status") in ["completed", "error"]
                and task_info.get("end_time", datetime.now()) < cutoff
            ):
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_tasks": len(
                [t for t in self.active_tasks.values() if t.get("status") == "error"]
            ),
...
def run_hybrid_safe(
    ...
    **kwargs,
    ...
    """
    return async_manager.run_hybrid(
        func_or_coro, *args, timeout=timeout, fallback_result=fallback_result, **kwargs
    )
