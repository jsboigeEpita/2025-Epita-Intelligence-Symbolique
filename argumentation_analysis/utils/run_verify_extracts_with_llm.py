...
async def main():
    ...
    try:
        ...
        logger.error(
            "Vérifiez que les modules nécessaires sont présents dans le projet."
        )
    ...
    extract_definitions, error_message = load_extract_definitions_safely(
        input_file, ENCRYPTION_KEY, CONFIG_FILE_JSON
    )
    ...
    if args.limit is not None and args.limit > 0:
        ...
        logger.info(
            f"Limitation du nombre d'extraits: {args.limit}/{total_extracts} extraits maximum."
        )
    ...
    llm_service = create_llm_service(
        service_id="verify_extracts", model_id="gpt-4o-mini"
    )
    if not llm_service:
    ...
    avg_coherence = (
        total_coherence / valid_count_for_avg if valid_count_for_avg > 0 else 0
    )
    avg_relevance = (
        total_relevance / valid_count_for_avg if valid_count_for_avg > 0 else 0
    )
    avg_integrity = (
        total_integrity / valid_count_for_avg if valid_count_for_avg > 0 else 0
    )
    ...
    print(
        f"Résumé: {valid_count} valides, {invalid_count} invalides, {error_count} erreurs"
    )
    print(
        f"Scores moyens: Cohérence {avg_coherence:.2f}/5, Pertinence {avg_relevance:.2f}/5, Intégrité {avg_integrity:.2f}/5"
    )
