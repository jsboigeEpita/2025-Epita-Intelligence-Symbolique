...
def display_extract_sources_details(
    ...
    if not isinstance(extract_sources_data, list):
        logger.error(
            "Les données fournies ne sont pas une liste. Impossible d'afficher les détails."
        )
        return

    logger.info(
        f"Début de l'affichage des détails pour {len(extract_sources_data)} sources."
    )
    ...
    if source_id_to_inspect:
        ...
        for item in extract_sources_data:
            if isinstance(item, dict) and item.get("id") == source_id_to_inspect:
                ...
                if "full_text" in item:
                    ...
                    logger.info(
                        f"--- Fin Full Text pour source ID: {source_id_to_inspect} ---"
                    )
                else:
                    logger.info(
                        f"Pas de champ 'full_text' pour la source ID: {source_id_to_inspect}"
                    )
                found_source = True
                break
        if not found_source:
        ...
        for i, item in enumerate(extract_sources_data):
            ...
            is_french_by_keyword = (
                any(keyword in source_name for keyword in keywords_fr)
                or any(keyword in source_path for keyword in keywords_fr)
                or any(keyword in domain_str for keyword in keywords_fr)
            )
            ...
            if show_all or (show_all_french and is_french_by_keyword):
                ...
                logger.info(
                    f"  Type: {item.get('source_type', 'N/A')}, Fetch: {item.get('fetch_method', 'N/A')}"
                )
                ...
                if extracts:
                    ...
                    for j, extract in enumerate(extracts):
                        logger.info(
                            f"    {j+1}. Nom: {extract.get('extract_name', 'N/A')}"
                        )
                        logger.info(
                            f"       Start: {extract.get('start_marker', 'N/A')}"
                        )
                ...
                if "full_text" in item:
                    full_text_preview = (
                        ...
                        else (item["full_text"] if item["full_text"] else "[VIDE]")
                    )
                    logger.info(
                        f"  Aperçu Full Text (200 premiers caractères): {full_text_preview}"
                    )
                else:
        ...
        if displayed_count == 0:
            if show_all_french and not show_all:
                logger.info(
                    "Aucune source française identifiée par les mots-clés actuels."
                )
            else:
                logger.info(
                    "Aucune source à afficher (la liste est peut-être vide ou les filtres n'ont rien retourné)."
                )

    else:
        # Comportement par défaut si aucun filtre spécifique: afficher les N premiers
        logger.info(
            "Affichage des 2 premiers éléments par défaut (aucun filtre spécifique appliqué)."
        )
        for i, item in enumerate(extract_sources_data[:2]):
            ...
            if isinstance(item, dict):
                ...
                if "full_text" in item and item["full_text"]:
                    logger.info(
                        f"Aperçu Full Text (500 premiers caractères): {item['full_text'][:500]}"
                    )
    ...
    if output_json_path:
        ...
        try:
            ...
            logger.info(
                f"Données complètes sauvegardées dans : {output_file.resolve()}"
            )
        except Exception as e_write:
            logger.error(
                f"Erreur lors de la sauvegarde du JSON dans {output_file}: {e_write}",
                exc_info=True,
            )
