...
def prepare_manual_correction_data(
    ...
    """
    logger.info(
        f"Préparation des données de correction manuelle pour source '{target_source_id}', extrait '{target_extract_name}'."
    )
    ...
    if not found_source_dict:
        ...
        available_ids = [
            s.get("id", "ID_MANQUANT") for s in sources_data if isinstance(s, dict)
        ]
    ...
    if not isinstance(extracts_list, list):
        logger.error(
            f"Le champ 'extracts' pour la source ID '{target_source_id}' n'est pas une liste."
        )
        return None
    ...
    for extract in extracts_list:
        if (
            ...
            and extract.get("extract_name") == target_extract_name
        ):
    ...
    if not found_extract_dict:
        logger.error(
            f"Extrait nom '{target_extract_name}' non trouvé dans la source ID '{target_source_id}'."
        )
        available_extract_names = [
            e.get("extract_name", "NOM_MANQUANT")
            for e in extracts_list
            if isinstance(e, dict)
        ]
        logger.info(
            f"Noms d'extraits disponibles pour la source '{target_source_id}': {available_extract_names}"
        )
        return None
    ...
    if output_debug_file_path:
        ...
        if source_full_text:
            debug_report_lines.append(
                f"Full_text de la source (longueur: {len(source_full_text)} caractères) :\n--- DEBUT FULL TEXT ---"
            )
            debug_report_lines.append(source_full_text)
            debug_report_lines.append("--- FIN FULL TEXT ---")
        else:
            debug_report_lines.append(
                "ATTENTION: Full_text de la source est ABSENT ou VIDE."
            )
        ...
        try:
            ...
            logger.info(
                f"Les informations de débogage pour correction ont été écrites dans : {output_debug_file_path.resolve()}"
            )
        except Exception as e_write_debug:
            logger.error(
                f"Impossible d'écrire le fichier de débogage '{output_debug_file_path}': {e_write_debug}",
                exc_info=True,
            )

    if not source_full_text:
        logger.warning(
            f"Le full_text pour la source {target_source_id} est manquant. La correction manuelle des marqueurs sera difficile."
        )
