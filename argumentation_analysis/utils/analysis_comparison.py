...
def compare_rhetorical_analyses(
    ...
    if not isinstance(advanced_results, dict):
        logger.error(f"Résultats avancés invalides pour '{extract_name}'. Attendu: dict, Reçu: {type(advanced_results)}")
        return {
    ...
    if base_results is not None and not isinstance(base_results, dict):
        logger.error(f"Résultats de base invalides pour '{extract_name}'. Attendu: dict, Reçu: {type(base_results)}")
    ...
    adv_fallacies_data = adv_analyses.get("contextual_fallacies", adv_analyses.get("fallacy_detection", {}))
    adv_fallacies_list = adv_fallacies_data.get("fallacies", []) if isinstance(adv_fallacies_data, dict) else \
                         (adv_fallacies_data if isinstance(adv_fallacies_data, list) else []) # Cas où la liste est directe
    ...
    if adv_fallacy_count is None: # Si toujours None, compter depuis la liste
         adv_arg_results = adv_fallacies_data.get("argument_results", []) # Structure plus ancienne
         if isinstance(adv_arg_results, list) and adv_arg_results:
             ...
             adv_fallacy_count = len(adv_fallacies_list) if isinstance(adv_fallacies_list, list) else 0
    adv_fallacy_count = adv_fallacy_count if isinstance(adv_fallacy_count, int) else 0
    
    adv_fallacy_types = {f.get("type") for f in adv_fallacies_list if isinstance(f, dict) and f.get("type")}
    ...
    if base_results:
        base_fallacies_data = base_analyses.get("contextual_fallacies", base_analyses.get("fallacy_detection", {}))
        base_fallacies_list = base_fallacies_data.get("fallacies", []) if isinstance(base_fallacies_data, dict) else \
                              (base_fallacies_data if isinstance(base_fallacies_data, list) else [])
        ...
        if base_fallacy_count is None:
            ...
            if isinstance(base_arg_results, list) and base_arg_results:
                ...
                base_fallacy_count = len(base_fallacies_list) if isinstance(base_fallacies_list, list) else 0
        base_fallacy_count = base_fallacy_count if isinstance(base_fallacy_count, int) else 0
        
        base_fallacy_types = {f.get("type") for f in base_fallacies_list if isinstance(f, dict) and f.get("type")}
        
        newly_identified_fallacies = sorted(list(adv_fallacy_types - base_fallacy_types))
        missing_in_advanced = sorted(list(base_fallacy_types - adv_fallacy_types))
        common_fallacies = sorted(list(adv_fallacy_types.intersection(base_fallacy_types)))
    ...
    if base_results:
        if adv_fallacy_count > base_fallacy_count:
            summary_parts.append(f"L'analyse avancée a identifié plus de sophismes ({adv_fallacy_count} vs {base_fallacy_count}).")
        elif adv_fallacy_count < base_fallacy_count:
            summary_parts.append(f"L'analyse de base a identifié plus de sophismes ({base_fallacy_count} vs {adv_fallacy_count}).")
        else:
            summary_parts.append(f"Les deux analyses ont identifié le même nombre de sophismes ({adv_fallacy_count}).")
        
        if newly_identified_fallacies:
            summary_parts.append(f"Nouveaux sophismes dans l'avancée: {', '.join(newly_identified_fallacies)}.")
        if missing_in_advanced:
            summary_parts.append(f"Sophismes de base non trouvés dans l'avancée: {', '.join(missing_in_advanced)}.")
    else:
        summary_parts.append(f"Analyse avancée: {adv_fallacy_count} sophisme(s) identifié(s) ({', '.join(newly_identified_fallacies) if newly_identified_fallacies else 'aucun type spécifique listé'}).")
        summary_parts.append("Résultats de base non fournis pour comparaison directe des sophismes.")

    comparison["comparison_summary"] = " ".join(summary_parts) if summary_parts else "Pas de différences notables ou données insuffisantes pour un résumé."
