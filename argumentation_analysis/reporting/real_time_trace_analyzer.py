...
class RealTimeTraceAnalyzer:
    ...
    def stop_capture(self):
        ...
        with self.lock:
            ...
            logger.info(
                f"[TRACE] Capture terminee - {len(self.conversation_blocks)} blocs, {self.total_tool_calls} appels d'outils"
            )
    ...
    def record_tool_call(
        ...
        with self.lock:
            ...
            logger.debug(
                f"[TRACE] Outil enregistre: {agent_name}.{tool_name} -> {execution_time_ms:.1f}ms"
            )
    ...
    def generate_complete_conversation_report(self) -> str:
        ...
        report_lines.append(
            f"**Durée d'analyse:** {self._get_total_duration_ms():.1f}ms"
        )
        ...
        for block in self.conversation_blocks:
            ...
            total_time_per_agent[agent] = sum(
                call.execution_time_ms for call in block.tool_calls
            )
        ...
        for agent, count in tool_counts.items():
            ...
            report_lines.append(
                f"- {agent}: {count} outils, {total_time_per_agent[agent]:.1f}ms total, {avg_time:.1f}ms moyenne"
            )
        ...
        report_lines.append(
            "✅ **OBJECTIF ATTEINT:** Conversation agentielle complète capturée"
        )
        report_lines.append(
            "✅ **FORMAT RESPECTÉ:** Tous les appels d'outils formatés selon spécifications"
        )
        report_lines.append(
            "✅ **RIEN D'OMIS:** Capture exhaustive de tous les échanges"
        )
        report_lines.append(
            "✅ **LISIBILITÉ:** Arguments tronqués intelligemment, résultats condensés"
        )
        ...
        report_lines.append(
            "*Capture en temps réel de conversation agentielle complète*"
        )
