...
def generate_fallacy_detection(
    argument: str, common_fallacies: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
...
def generate_rhetorical_analysis_for_extract(
    ...
    extract_text = generate_sample_text(
        extract_name=extract_name, source_name=source_name
    )
    ...
    for i, argument_text in enumerate(arguments):
        detected_fallacies = generate_fallacy_detection(
            argument_text, common_fallacies_for_simulation
        )
...
def generate_markdown_summary_for_analysis(
    ...
    fallacies_found_in_extract = any(
        arg_res["detected_fallacies"] for arg_res in fallacy_results
    )
        ...
        for arg_result in fallacy_results:
            if arg_result["detected_fallacies"]:
                ...
                for fallacy in arg_result["detected_fallacies"]:
                    ...
                    content += (
                        f"- **Sévérité**: {fallacy.get('severity', 'Non spécifiée')}\n"
                    )
                    content += (
                        f"- **Confiance**: {fallacy.get('confidence', 0.0):.2f}\n"
                    )
                    content += f"- **Contexte**:\n  > {argument}\n\n"
    else:
...
def generate_global_summary_report(
    all_analyses: List[Dict[str, Any]],
    output_dir: Path,
    rhetorical_agents_config: List[Dict[str, Any]],
) -> Path:
    """
    ...
    for analysis in all_analyses:
        ...
        analyses_by_agent.setdefault(agent_name, {}).setdefault(source_name, []).append(
            analysis
        )
    ...
    for agent_name_key, agent_data in analyses_by_agent.items():
        ...
        total_fallacies_agent = sum(
            an["analyses"]["fallacy_count"]
            for src_analyses in agent_data.values()
            for an in src_analyses
        )
        ...
        agent_info_cfg = next(
            (
                agent
                for agent in rhetorical_agents_config
                if agent["name"] == agent_name_key
            ),
            None,
        )
        strengths = (
            ", ".join(agent_info_cfg["strengths"][:2]) if agent_info_cfg else "N/A"
        )
        weaknesses = (
            ", ".join(agent_info_cfg["weaknesses"][:2]) if agent_info_cfg else "N/A"
        )
    ...
    for source_name_key in sorted(list(sources_set)):
        ...
        content += (
            "| Agent | Extraits | Sophismes | Cohérence Moy. | Qualité Glob. Moy. |\n"
        )
        content += (
            "|-------|----------|-----------|----------------|--------------------|\n"
        )
        for agent_name_key, agent_data_for_source in analyses_by_agent.items():
            if source_name_key in agent_data_for_source:
                ...
                total_fallacies_src = sum(
                    an["analyses"]["fallacy_count"] for an in source_specific_analyses
                )

                coherence_scores_src = [
                    an["analyses"]["coherence_evaluation"]["overall_coherence"]["score"]
                    for an in source_specific_analyses
                ]
                avg_coherence_src = (
                    sum(coherence_scores_src) / len(coherence_scores_src)
                    if coherence_scores_src
                    else 0
                )

                quality_scores_src = [
                    an["analyses"]["metrics"]["overall_quality"]
                    for an in source_specific_analyses
                ]
                avg_quality_src = (
                    sum(quality_scores_src) / len(quality_scores_src)
                    if quality_scores_src
                    else 0
                )
...
def run_summary_generation_pipeline(
    ...
    """
    logger.info(
        "Démarrage du pipeline de génération de synthèses d'analyses rhétoriques..."
    )
    ...
    for agent_config_item in rhetorical_agents_data:
        logger.info(
            f"Pipeline: Traitement avec l'agent '{agent_config_item['name']}'..."
        )
        for source_item in simulated_sources_data:
            ...
            for extract_def_item in extracts_list:
                ...
                generate_markdown_summary_for_analysis(
                    analysis_result_item, summaries_subdir
                )
    ...
    full_json_output_path = (
        output_reports_dir / f"all_rhetorical_analyses_simulated_{json_timestamp}.json"
    )
    ...
    try:
        ...
        logger.info(
            f"Toutes les analyses simulées sauvegardées en JSON: {full_json_output_path}"
        )
    except Exception as e:
        logger.error(
            f"Erreur lors de la sauvegarde du JSON global des analyses: {e}",
            exc_info=True,
        )

    logger.info(
        f"Pipeline de génération de synthèses terminé. Rapport global: {global_summary_file_path}"
    )
