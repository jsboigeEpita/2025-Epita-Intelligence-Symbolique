...
class EnhancedRealTimeTraceAnalyzer:
    ...
    def start_capture(self):
        ...
        with self.lock:
            ...
            logger.info("[ENHANCED-TRACE] Capture d'orchestration PM avec gestion d'état démarrée")
    ...
    def stop_capture(self):
        ...
        with self.lock:
            ...
            logger.info(f"[ENHANCED-TRACE] Capture terminée - {len(self.orchestration_phases)} phases, "
    ...
    def capture_state_snapshot(self, phase_id: str, tour_number: int, agent_active: str,
        ...
        with self.lock:
            ...
            logger.debug(f"[ENHANCED-TRACE] État capturé: Phase {phase_id}, Tour {tour_number}, Agent {agent_active}")
    ...
    def record_enhanced_tool_call(self, agent_name: str, tool_name: str, arguments: Dict[str, Any],
        ...
        with self.lock:
            ...
            logger.debug(f"[ENHANCED-TRACE] Outil enregistré (amélioré): {agent_name}.{tool_name} -> {execution_time_ms:.1f}ms")
    ...
    def capture_conversation_message(self, agent_name: str, content: str,
        ...
        with self.lock:
            ...
            logger.debug(f"[ENHANCED-CONV] Message capturé: {agent_name} (tour {tour_number})")
    ...
    def generate_enhanced_pm_orchestration_report(self) -> str:
        ...
        objectives_met = [
            ("Format conversation lisible", "✅", "Headers spéciaux et markup élégant implémentés"),
            ("PM opérationnel", "✅", "Orchestration multi-phases coordonnée"),
            ("Gestion d'état partagé", "✅", f"{self.total_state_snapshots} snapshots capturés"),
            ("Trace enrichie", "✅", "Évolution d'état documentée à chaque phase"),
            ("Démo complète", "✅", f"{len(self.orchestration_phases)} phases d'orchestration")
        ]
    ...
    def save_enhanced_report(self, filepath: str) -> bool:
        ...
        try:
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(report)
            logger.info(f"[ENHANCED-SAVE] Rapport d'orchestration PM sauvegardé: {filepath}")
            return True
