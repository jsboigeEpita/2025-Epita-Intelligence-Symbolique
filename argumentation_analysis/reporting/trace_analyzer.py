...
class ToolCall:
    ...
    def to_compact_string(self) -> str:
        ...
        args_str = (
            json.dumps(self.arguments, ensure_ascii=False)[:100] + "..."
            if len(str(self.arguments)) > 100
            else json.dumps(self.arguments, ensure_ascii=False)
        )
        status = "✓" if self.success else "✗"
        return (
            f"{status} {self.tool_name}({args_str}) -> {self.execution_time_ms:.1f}ms"
        )
        ...
        for key, value in self.arguments.items():
            if isinstance(value, str) and len(value) > 100:
                ...
                if isinstance(value, list):
                    summary = (
                        f"[{len(value)} items: {str(value[:2])[:-1]}...]"
                        if len(value) > 2
                        else str(value)
                    )
                else:
...
class ConversationCapture:
    ...
    def to_compact_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tool_summary": [
                call.to_compact_string() for call in self.tool_calls[-5:]
            ],  # 5 derniers appels
            "conversation_flow": [
                f"{msg['role']}: {msg['content'][:50]}..." for msg in self.messages[-3:]
            ],  # 3 derniers messages
        }
...
class TraceAnalyzer:
    ...
    def finalize_conversation_capture(self):
        ...
        if self.capture_enabled and self.conversation_capture:
            ...
            if self.current_agent_step:
                self.current_agent_step.complete()
            logger.info(
                f"Capture finalisée - {len(self.conversation_capture.messages)} messages, {len(self.conversation_capture.tool_calls)} appels d'outils"
            )
    ...
    def load_traces(
        self, conversation_log_path: str = None, report_json_path: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            if not conversation_log_path or not os.path.exists(conversation_log_path):
                ...
                if log_files:
                    conversation_log_path = os.path.join(
                        self.logs_directory, log_files[0]
                    )

            if not report_json_path or not os.path.exists(report_json_path):
                json_files = [
                    f for f in os.listdir(self.logs_directory) if f.endswith(".json")
                ]
                if json_files:
            ...
            if os.path.exists(conversation_log_path):
                ...
                logger.warning(
                    f"Fichier log conversation non trouvé: {conversation_log_path}"
                )
            ...
            return (
                self.raw_conversation_data is not None
                or self.raw_report_data is not None
            )
        ...
        try:
            ...
            if self.raw_conversation_data:
                ...
                source_match = re.search(
                    r"Fichier source analysé : (.+)", self.raw_conversation_data
                )
                if source_match:
                ...
                length_match = re.search(
                    r"Longueur du texte: (\d+) caractères", self.raw_conversation_data
                )
                if length_match:
                ...
                timestamp_match = re.search(
                    r"Horodatage de l'analyse\s*:\s*(.+)", self.raw_conversation_data
                )
                if timestamp_match:
        ...
        try:
            if self.raw_conversation_data:
                ...
                orchestration_lines = re.findall(
                    r"\[ETAPE \d+/\d+\] (.+)", self.raw_conversation_data
                )
                ...
                for pattern in coordination_patterns:
                    matches = re.findall(
                        f".*{pattern}.*", self.raw_conversation_data, re.IGNORECASE
                    )
                ...
                time_pattern = re.compile(
                    r"(?:termin\w*.*en|Temps total:)\s+([\d.]+)\s*ms", re.IGNORECASE
                )
                ...
                if re.search(
                    r"(échec|erreur|failed|error)",
                    self.raw_conversation_data,
                    re.IGNORECASE,
                ):
                    flow.success_status = "partial_failure"
                elif re.search(
                    r"analyse terminée avec succès",
                    self.raw_conversation_data,
                    re.IGNORECASE,
                ):
        ...
        try:
            if self.raw_conversation_data:
                ...
                for indicator in belief_indicators:
                    matches = re.findall(
                        f".*{indicator}.*", self.raw_conversation_data, re.IGNORECASE
                    )
                    for match in matches:
                        evolution.belief_state_construction.append(
                            {"phase": indicator, "details": match.strip()}
                        )
                ...
                for pattern in enrichment_patterns:
                    matches = re.findall(
                        pattern, self.raw_conversation_data, re.IGNORECASE
                    )
                    ...
                    for match in matches:
                        if isinstance(match, str):
                            ...
                            evolution.progressive_enrichment.extend(
                ...
                for start_pattern, end_pattern in transition_patterns:
                    if re.search(
                        start_pattern, self.raw_conversation_data, re.IGNORECASE
                    ) and re.search(
                        end_pattern, self.raw_conversation_data, re.IGNORECASE
                    ):
        ...
        try:
                        ...
                        if "formulas" in kb:
                            for formula in kb["formulas"]:
                                results.formalizations.append(
                                    {"formula": formula, "type": "extracted_formula"}
                                )
            ...
            if self.raw_conversation_data:
                ...
                logic_types = re.findall(
                    r"logique (\w+)", self.raw_conversation_data, re.IGNORECASE
                )
                ...
                kb_status_match = re.search(
                    r"Création de la KB : (\w+)", self.raw_conversation_data
                )
                if kb_status_match:
        ...
        try:
                    ...
                    if "severity_distribution" in informal:
                        ...
                        for level, count in severity.items():
                            exploration.severity_analysis.append(
                                {"severity_level": level, "count": count}
                            )
            ...
            if self.raw_conversation_data:
                ...
                if fallacy_match:
                    ...
                    exploration.fallacy_detection.append(
                        {"total_detected": count, "source": "conversation_log"}
                    )
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        try:
            ...
            report.append(
                f"- **Temps d'exécution total:** {orchestration.total_execution_time} ms"
            )
            report.append(
                f"- **Nombre d'agents appelés:** {len(orchestration.agents_called)}"
            )
            ...
            if orchestration.sequence_order:
                ...
                for i, (step, description) in enumerate(
                    orchestration.sequence_order, 1
                ):
            ...
            if state_evolution.shared_state_changes:
                report.append(
                    f"- **Modifications d'état partagé:** {len(state_evolution.shared_state_changes)} détectées"
                )

            if state_evolution.belief_state_construction:
                report.append(
                    f"- **Construction belief state:** {len(state_evolution.belief_state_construction)} phases"
                )
            report.append("")
            ...
            report.append("-" * 28)
            report.append(
                f"- **Types de logique utilisés:** {', '.join(query_results.logic_types_used) if query_results.logic_types_used else 'Aucun'}"
            )
            report.append(
                f"- **Statut base de connaissances:** {query_results.knowledge_base_status}"
            )
            report.append(
                f"- **Nombre de formalisations:** {len(query_results.formalizations)}"
            )
            report.append(
                f"- **Requêtes d'inférence:** {len(query_results.inference_queries)}"
            )
            report.append(
                f"- **Résultats de validation:** {len(query_results.validation_results)}"
            )
            ...
            if query_results.validation_results:
                ...
                for result in query_results.validation_results[:3]:  # Limite à 3
                    ...
                    report.append(
                        f"    Résultat: {result.get('result', 'N/A')}, Confiance: {confidence}"
                    )
            report.append("")
            ...
            if informal_exploration.fallacy_detection:
                ...
                for detection in informal_exploration.fallacy_detection:
                    if "total_detected" in detection:
                        report.append(
                            f"  - Total détecté: {detection['total_detected']}"
                        )
                    else:
                        report.append(
                            f"  - Type: {detection.get('type', 'N/A')}, "
            ...
            if informal_exploration.severity_analysis:
                ...
                for analysis in informal_exploration.severity_analysis:
                    report.append(
                        f"  - {analysis['severity_level']}: {analysis['count']}"
                    )
            ...
            report.append(
                f"- **Richesse des traces:** {total_components} composants analysés"
            )
            report.append(
            ...
            if not query_results.validation_results:
                report.append(
                    "- Améliorer la génération et l'exécution des requêtes d'inférence"
                )

            if len(informal_exploration.fallacy_detection) == 0:
                report.append(
                    "- Enrichir la détection de sophismes avec des textes plus complexes"
                )
            ...
            report.append("=" * 60)
            report.append(
                "*Rapport généré par TraceAnalyzer v2.0 - Système d'Argumentation Unifié*"
            )
