...
### Flaky Test: `test_invoke_fact_extraction[_short_text]` family (ATT-1 residual, latent)

- **Scope**: **Two sibling tests** in `tests/unit/argumentation_analysis/workflows/test_formal_verification.py::TestInvokeCallables`, both real-LLM-call floaters on the same function `_invoke_fact_extraction`, flaking in **opposite directions**:
  - `test_invoke_fact_extraction_short_text` — input `"Hi."`, asserts `claim_count == 0`; flakes **up** to `>= 1` when the model hallucinates a claim on trivial input.
  - `test_invoke_fact_extraction` — input `"This is a long claim about logic. Another important assertion here. A third point."`, asserts `claim_count >= 1`; flakes **down** to `0` when the real LLM call intermittently returns/parses no claim (empty response, transient parse, ambient degradation).
- **Symptom**: either sibling can intermittently fail when the suite runs with an OpenAI key present. Observed CI-RED runs `81c5d2b4`/`ad2f03cf` (2026-07-10) = the `>= 1` sibling flaked to `0`; unrelated to the concurrent I5/I1 merges (those touch `_invoke_dung_extensions`/`neuro_symbolic_arbitrator`, not `_invoke_fact_extraction`).
- **Root cause**: `_invoke_fact_extraction` (`invoke_callables.py`) issues a **real LLM call** regardless of input length. This is **intrinsic LLM non-determinism**, not a code bug. The no-key path (heuristic fallback) is deterministic — `"Hi."` is below the per-sentence `len > 20` threshold, so `claims == []` and the `== 0` test passes cleanly; the 3-sentence input clears the threshold so the `>= 1` test passes cleanly.
- **Proof**: `_short_text` **PASSED** in green run #1420 (run `28893501286`); both are *latent* floaters, not a persistent CI-RED. Count stays ~1–2 F+E, the failing **set rotates** run-to-run (hallmark of LLM/timing flake, not regression).
