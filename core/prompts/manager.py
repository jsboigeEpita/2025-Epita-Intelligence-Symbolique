...
class PromptTemplate:
    ...
    def __init__(self, name: str, path: Path, config: Dict[str, Any], parameters: Dict[str, Any], prompt_text: str):
        self.name = name
        self.path = path
        self.config = config
...
class PromptRenderer:
    ...
    def _process_optimization_blocks(self, template: PromptTemplate, context: Dict[str, Any]) -> str:
        # This regex now consumes surrounding whitespace to avoid blank lines
        pattern = re.compile(
            r'\s*\{\#optimize_axis\s+(?P<params>.*?)\}(?P<body>.*?)\{\/optimize_axis\}\s*',
            re.DOTALL
        )
        template_axes = {axis['name']: axis for axis in template.parameters.get('optimization_axes', [])}

        def replace_block(match):
            params = dict(re.findall(r'(\w+)=["\']?([^"\']+)["\']?', match.group('params')))
            ...
            axis_value = context.get('optimization_params', {}).get(axis_name, axis_config.get('default'))
            ...
            if 'value' in params:
                ...
                if axis_value is not None and int(axis_value or 0) >= int(params['min']):
                    should_render = True
            
            return self._process_loops(body, context, axis_value, axis_name) if should_render else ""

        return pattern.sub(replace_block, template.prompt_text)

    def _process_loops(self, body: str, context: Dict[str, Any], axis_value: Any, axis_name: str) -> str:
        ...
        def replace_loop(match):
            ...
            loop_params = dict(re.findall(r'(\w+)=["\']?([^"\']+)["\']?', match.group('params')))
...
class PromptManager:
    ...
    def discover_templates(self):
        if not self.templates_path.is_dir(): return
        for template_dir in self.templates_path.iterdir():
            if template_dir.is_dir():
                try:
                    self.templates[template_dir.name] = self._load_template(template_dir)
    ...
    def _load_template(self, template_path: Path) -> PromptTemplate:
        ...
        return PromptTemplate(
            name=template_path.name, path=template_path,
...
if __name__ == "__main__":
    ...
    if "sample_analysis_v1" in manager.templates:
        ...
        test_context_2 = {
            ...
            "examples" : [
                {"text": "Cats are better than dogs.", "fallacy_type": "Hasty Generalization"},
                {"text": "You are either with us, or against us.", "fallacy_type": "False Dichotomy"}
            ]
        }
