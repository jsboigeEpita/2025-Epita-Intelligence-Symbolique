...
Mesure du 2026-09-23 sur ai-01 (`projet-is-roo-new`, `main` `742d1a0e` plus les correctifs de #2447, items 4, 5, 7 et 8) : `test_worker_fol_tweety.py` rend 18 passed, 0 skipped, 4 deselected, avec la JVM et le filtre de la CI (`-m "not slow and not requires_api"`), pour 0 requête LLM au compteur d'egress. Les 4 désélectionnés portent `requires_api` : ils convertissent par le vrai LLM (`test_end_to_end_fol_syllogism_with_llm`, `test_fol_multilingual_support`, `test_fol_syntax_error_recovery`, `test_fol_memory_usage_stability`). Lancés seuls (`-m requires_api`), ils rendent 4 passed pour 16 requêtes LLM. `tests/unit/agents/test_fol_logic_agent.py` rend 17 passed.

Mesure du 2026-09-23 sur ai-01, `main` `fc023bb5` plus le correctif de #2468, même filtre : 23 passed, 0 skipped, 4 deselected, 0 requête LLM. Les 5 tests ajoutés (noms que le parseur refuse, dans `TestAnalyzeReachesTheSolver` et `TestPipelineFolPhaseNames`) échouent sur le code de `main`.
...
- `FOLLogicAgent.text_to_belief_set()` convertit par le LLM (`_convert_to_fol`), et le belief set porte la signature que le parseur exige (#2447). Quand seule l'heuristique peut tourner (pas de service, ou conversion en échec), elle ne rend **pas** de belief set : ses formules de substitution ne traduisent rien, et le message dit pourquoi. `TestAnalyzeReachesTheSolver::test_text_to_belief_set_answers_the_syllogism_query` le vérifie contre le vrai solveur, avec un faux service de chat : la requête `Mortel(socrate)` rend `True`. Avant, la méthode ne demandait rien au LLM, et `test_end_to_end_fol_syllogism_with_llm` passait sans aucune requête.
...
- Les noms que le parseur refuse sont renommés, dans la signature et dans les formules (#2468). Mesure sur la vraie JVM : une déclaration de prédicat suit `[A-Za-z][A-Za-z0-9]*` (`A_Fait` est refusé), une constante commence par une lettre (`_t_` est refusé, `jean_paul` accepté). `FOLLogicAgent.extract_fol_metadata` rend la signature et, sous `"formulas"`, les formules renommées : l'agent, la phase FOL du pipeline et la phase du solveur externe lisent ce même appel. Les accents sont repliés (`Évalue` → `Evalue`, `été` → `ete`), le `_` d'un prédicat disparaît en PascalCase (`A_Fait` → `AFait`), et un nom déjà légal garde le sien, si bien qu'un renommage ne fusionne jamais deux noms. Avant, `A_Fait` était déclaré tel quel, `Évalue` devenait `_value`, et les formules de l'agent gardaient leurs noms d'origine : aucun verdict.
