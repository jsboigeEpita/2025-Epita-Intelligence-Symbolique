...
| 6 | **EAF (Epistemic)** | `state.dung_frameworks` | `_write_eaf_to_state` (`state_writers.py:1243-1252`) | `_invoke_eaf` (`invoke_callables.py:4317-4342`) → `handler.analyze_epistemic_framework` (`agents/core/logic/eaf_handler.py:71-130`) | **Epistemic beliefs** — the per-agent `Dict[agent, List[arg]]` mapping. Computed at `invoke_callables.py:4326` (`_eaf_beliefs_from_context`) and returned by the handler (`eaf_handler.py:118`). The writer discards them; only `extensions={"eaf_extensions": extensions}` survives. `statistics.agents_count` is computed (`eaf_handler.py:124`) and dropped. **[Corrected 2026-09-07 #2, #1648 sonde — MESURÉ sur Tweety 1.31, coord ai-01]** Le verdict d'origine « the writer discards them » est **FAUX**. Le blocage 1.28 était réel *sur la machine worker* — `libs/tweety/` est **gitignoré** (`.gitignore:152`), donc le jeu modulaire 1.31 n'y était pas ; il est présent sur ai-01, où la sonde a donc pu tourner. Vrai `_invoke_eaf` → vrai `EAFHandler` (log `EAF classes loaded. Reasoners: ['grounded','preferred','stable','complete','admissible']`, et `statistics.handler == "EAFHandler"` — donc le handler Java, **pas** `_python_eaf_fallback`) → `_write_eaf_to_state` → état relu : `formalism_specific.epistemic_beliefs` **value-equal** à ce que le handler produit, `attacks` préservés. Seule perte réelle : **`statistics`** (produit `{arguments_count, attacks_count, agents_count, handler}`, absent de l'état) — le sinistre systématique du finding 4. | Reader-visible shape: `attacks` preserved as `[src, tgt]` (good), but the EAF-specific epistemic dimension is invisible to readers that read `extensions["eaf_extensions"]` as a flat list of extensions. |
...
| # | Module | Container | Writer (file:line) | Invoke (file:line) | Distinct info lost | Source vs re-read gap |
|---|--------|-----------|--------------------|--------------------|--------------------|-----------------------|
| 11 | **Conditional Logic (CL)** | `state.propositional_analysis_results` | `_write_cl_to_state` (`state_writers.py:1163-1174`) | `_invoke_cl` (`invoke_callables.py:4003-4034`) → `handler.query` (`agents/core/logic/cl_handler.py:178-212`) | **The conditionals themselves** (the `ClBeliefSet` of `(conclusion | premise)` formulas). The invoke receives them at `invoke_callables.py:4005` (`conditionals = context.get("conditionals", [])`) and the handler parses them via `kb = handler.create_knowledge_base(conditionals)` (line 4017), but the writer stores only `[f"CL({num_conditionals} conditionals): {message}"]`. The count is in the formula string; the conditionals themselves are gone. CL's distinctive semantics (non-material conditionals, system-P/System-Z ranking) is invisible. **[Corrected 2026-09-07 #2, #1648 sonde — MESURÉ sur Tweety 1.31, coord ai-01]** Deux corrections. **(a) L'attribution « 1.28-vs-1.31 API shape difference » est RÉFUTÉE** : la levée `too many values to unpack (expected 2)` se reproduit **à l'identique sous 1.31**. Sa cause n'est pas la version mais la **forme d'entrée de la sonde** — `create_knowledge_base` attend `List[Tuple[conclusion, premise]]` (`cl_handler.py:132-149`, `for conclusion_str, premise_str in conditionals`), si bien qu'une liste de *chaînes* déplie chaque chaîne caractère par caractère. Aucun défaut de production ici ; le contrôle qui tranche est la reproduction sur l'autre version, pas la lecture du message. **(b) Avec la forme correcte** `[("b","a"), ("c","b")]`, le site est **MESURÉ** et le verdict d'origine « the conditionals themselves are gone » est **FAUX** : l'état relu porte `formalism_specific.conditionals == [["b","a"],["c","b"]]` (tuple→list), et `entailed=True` est préservé sous la forme `satisfiable: true`. Seule perte réelle : **`statistics`**. | Writer synthesises a label `"CL(N conditionals): {msg}"` — the verdict (`entailed` → `satisfiable`) is preserved; structure is dropped. |
...
**Measured verdicts (13 of 13 — sites 6 et 11 mesurés le 2026-09-07 par le coordinateur sur Tweety 1.31, cf. §1.6):**
...
**NON MESURÉ (0 of 13) — ~~2 of 13~~, résolu le 2026-09-07. Historique conservé ci-dessous ; les deux sites sont désormais mesurés (§1.6) :**
...
1. **Per-module comparison (invoke vs state):** the inventory above IS that
   ...
   **[Corrected 2026-09-07, #1648 sonde]** That one-shot diff has now been run
   — 13 of 13 sites executed end-to-end (produce → write → re-read); verdicts
