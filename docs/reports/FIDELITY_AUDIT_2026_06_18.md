...
- **G6 (real)**: TweetyProject formal-validation layer (`logic/validator.py` + `tweety_bridge.py` → `ValidationResult`) MISSING — unified kept the `ValidationResult` dataclass (definitions.py lines 94-101, anchor fossilized #2137: the dataclass was withdrawn — never instantiated, the delivered contract is the dict built by `invoke_callables._build_counter_argument_validation`) but never populated it. If formal validation is in-scope for the unified agent, this is a regression.
