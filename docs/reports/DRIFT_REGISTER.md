...
| O-2 | `HypothesisTracker` (ATMS-based) in `hypothesis_tracker.py`: exported from `__init__.py`, never instantiated. Redundant with JTMS HypothesisTracker in `sherlock_jtms_agent.py` which IS active. **Résolu (#2137)** : retiré avec ses tests — le suivi d'hypothèses vivant reste `services/jtms/atms_core.ATMS`. | 4 | Present since initial | **Withdrawn (#2137)** — redundant |
...
| D-3 | Walton-Krabbe protocol classes (`DialogueProtocol`, `InquiryProtocol`, `PersuasionProtocol`, `DialogueMove`, `FormalArgument`, `KnowledgeBase`): well-designed formal dialogue specifications, never instantiated. **Tranché (#2137)** : les 3 classes de protocole (jumeaux morts du `dialogue_handler` JVM vivant) et `KnowledgeBase` ont été retirés ; le vocabulaire (`DialogueMove`, `FormalArgument`, enums, `Proposition`) reste. | 3 | Present since initial | **Partially withdrawn (#2137)** — vocabulary stays |
