# SCDA State Export

**Generated:** 2026-05-19 07:46 UTC

## Analysis Tasks (1)

- **task_1:** <scrubbed>

## Arguments (2)

- **arg_1:** <scrubbed>
- **arg_2:** <scrubbed>

## Fallacies (2)

### fallacy_1
- **type:** appel à la tradition / appel à l'histoire (appeal to tradition)
- **justification:** <scrubbed>
- **target_argument_id:** arg_arg1_conclusion

### fallacy_2
- **type:** genéralisation hâtive / essentialisation
- **justification:** <scrubbed>
- **target_argument_id:** arg_arg1_conclusion


## Belief Sets (0)

```
{}
```

## Query Log (0)

```
[]
```

## Answers (1)

### task_1
- **author_agent:** ProjectManager
- **answer_text:** <scrubbed>
- **source_ids:** []


## Extracts (0)

```
[]
```

## Errors (0)

```
[]
```

## Final Conclusion (0)

##  Next Agent Designated (1)

```
InformalAgent
```

## Counter-Arguments (1)

1. **id:** ca_1 | **original_argument:** <scrubbed> | **counter_content:** <scrubbed> | **strategy:** demander preuves empiriques + fact-check historique | **score:** 0.9

## Quality Scores (0)

```
{}
```

## JTMS Beliefs (0)

```
{}
```

## Jtms Retraction Chain (0)

```
[]
```

## Dung Frameworks (0)

```
{}
```

## Governance Decisions (0)

```
[]
```

## Debate Transcripts (0)

```
[]
```

## Transcription Segments (0)

```
[]
```

## Semantic Index Refs (0)

```
[]
```

## Neural Fallacy Scores (0)

```
[]
```

## Ranking Results (0)

```
[]
```

## ASPIC Results (0)

```
[]
```

## Belief Revision Results (0)

```
[]
```

## Dialogue Results (0)

```
[]
```

## Probabilistic Results (0)

```
[]
```

## Bipolar Results (0)

```
[]
```

## FOL Analysis (0)

```
[]
```

## Fol Signature (0)

```
[]
```

## PL Analysis (0)

```
[]
```

## Modal Analysis (0)

```
[]
```

## Formal Synthesis (0)

```
[]
```

## Nl To Logic Translations (0)

```
[]
```

## Atms Contexts (0)

```
[]
```

## Workflow Results (0)

```
{}
```

## Narrative Synthesis (1)

```

```

## Atomic Propositions (0)

```
{}
```

## FOL Shared Signature (0)

```
{}
```

##  Jtms Session (1)

```
<argumentation_analysis.services.jtms.extended_belief.JTMSSession object at 0x0000024B3507C8B0>
```
