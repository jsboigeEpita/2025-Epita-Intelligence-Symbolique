...
class TestWebAppIntegration(unittest.TestCase):
    ...
    def test_02_service_manager_integration(self):
        ...
        self.assertTrue(
            self.service_manager_available,
            "OrchestrationServiceManager doit être disponible",
        )
        ...
        if self.service_manager_available and self.app_available:
            ...
            self.assertIsNotNone(
                sm, "OrchestrationServiceManager doit être instanciable"
            )
    ...
    def test_03_fallacy_analyzers_availability(self):
        ...
        if self.fallacy_analyzers_available:
            ...
            self.assertIsNotNone(
                complex_analyzer, "ComplexFallacyAnalyzer doit être instanciable"
            )
            self.assertIsNotNone(
                contextual_analyzer, "ContextualFallacyAnalyzer doit être instanciable"
            )
    ...
    def test_04_routes_basic_functionality(self):
        ...
        self.assertEqual(
            response.status_code, 200, "La route principale doit être accessible"
        )
        self.assertIn(
            b"Argumentation Analysis", response.data, "Le titre doit être présent"
        )

        # Test route status
        response = self.client.get("/status")
        self.assertEqual(
            response.status_code, 200, "La route status doit être accessible"
        )

        status_data = json.loads(response.data)
        self.assertIn(
            "status", status_data, "La réponse status doit contenir le champ 'status'"
        )
        self.assertIn(
            "webapp", status_data, "La réponse status doit contenir le champ 'webapp'"
        )
    ...
    def test_05_analyze_with_real_servicemanager(self):
        ...
        response = self.client.post(
            "/analyze", data=json.dumps(test_payload), content_type="application/json"
        )

        self.assertEqual(
            response.status_code, 200, "La route analyze doit être accessible"
        )
        ...
        if result["status"] == "success":
            self.assertIn(
                "results", result, "Une analyse réussie doit contenir des résultats"
            )
    ...
    def test_06_fallacy_detection_endpoint(self):
        ...
        self.assertEqual(
            ...
            200,
            "L'endpoint de détection de sophismes doit être accessible",
        )
        ...
        if result["status"] == "success" and "results" in result:
            ...
            if "fallacies" in results:
                self.assertIsInstance(
                    results["fallacies"],
                    list,
                    "Les sophismes détectés doivent être dans une liste",
                )
    ...
    def test_07_api_examples_endpoint(self):
        ...
        self.assertEqual(
            response.status_code, 200, "L'endpoint des exemples doit être accessible"
        )

        examples = json.loads(response.data)
        self.assertIn("examples", examples, "La réponse doit contenir des exemples")
        self.assertIsInstance(
            examples["examples"], list, "Les exemples doivent être dans une liste"
        )
    ...
    def test_08_performance_basic(self):
        ...
        self.assertLess(
            response_time, 2.0, "La route status doit répondre en moins de 2 secondes"
        )
