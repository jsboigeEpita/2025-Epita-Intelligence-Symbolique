        ...
        <div className="multiple-extensions">
          {extension.map((ext: string[], index: number) => (
            <div key={index} className="extension-set">
              <strong>Extension {index + 1}:</strong> {ext.join(', ') || 'Ø'}
            </div>
          ))}
        </div>
      );
              ...
              >
                ...
                <div className="step-info">
                  ...
                  <div className="step-status">
                    ...
                    {step.error && '❌'}
                    {!step.completed && !step.error && index === currentStep && isValidating && '⏳'}
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="step-controls">
              ...
              >
                {isValidating ? 'Validation...' : 
                 currentStep === validationSteps.length - 1 ? 'Terminer' : 'Suivant →'}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="final-result">
          ...
          <div className={`result-summary ${finalResult.isValid ? 'valid' : 'invalid'}`}>
            <div className="result-status">
              {finalResult.isValid ? '✅ Argument Valide' : '❌ Argument Invalide'}
            </div>
      ...
      <div className="premises-list">
        ...
        {premises.map((premise, index) => (
          <div key={index} className="premise-item">
            <span>{premise}</span>
            <button onClick={() => removePremise(index)}>×</button>
          </div>
        ))}
      </div>
  ...
  const createTooltip = (step: TutorialStep): HTMLElement => {
    ...
    tooltip.innerHTML = `
      <div class="tooltip-header">
        <h4>${step.title}</h4>
        <button class="close-btn" onclick="this.closest('.tutorial-tooltip').remove()">×</button>
      </div>
  ...
  private showUpdateBanner(updateInfo: UpdateInfo): void {
    ...
    banner.innerHTML = `
      <div class="update-content">
        <span>🚀 Nouvelle version ${updateInfo.version} disponible</span>
