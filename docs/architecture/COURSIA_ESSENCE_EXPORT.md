...
| [`orchestration_modes_compared.ipynb`](../coursia_contrib/orchestration_modes_compared.ipynb) | orchestration (7 modes comparés : le budget fabrique le classement) | ✓ livré, exécuté firsthand | instrument `scripts/compare_orchestration_modes.py` (2 runs BYOK mesurés : 180 s défaut vs 600 s calibré, `gpt-5.6-luna`) |
...
- **orchestration_modes_compared** (CI-4 #1835) — un **instrument**, pas un axe : comment arbitrer entre 7 architectures qui font « la même chose ». Au budget par défaut (180 s), la même exécution rend **7/7 `terminates`, 4/7 tués par le filet, 3/7 `success`, 2/7 complets** — quatre chiffres pour une exécution, et un classement par « qui meurt le plus tard ». Au budget calibré (600 s, dérivé du ~500 s documenté pour `pipeline_standard` 15/15) : **0 tué, 5/7 complets**, et les 2 restants sont des partiels honnêtes *par politique propre* (`conversational` reste borné en wall-time par conception ; `hierarchical_delegation` assume un verdict sur 3/5 tâches). `success` répond à « le mode assume-t-il son verdict ? », pas à « a-t-il tout fait ? ». Corpus-free et key-free : textes synthétiques du script, mesures committées, un seul appel déterministe (`--depth-parity`, sans LLM ni JVM). `conversation_deterministic` = témoin déterministe, jamais « mode de référence ».
