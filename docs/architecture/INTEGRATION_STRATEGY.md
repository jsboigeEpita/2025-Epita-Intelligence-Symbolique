...
| ~~`agents/core/abc/plugin.py`~~ | **Removed (#2145)** — the extended `LegoPlugin` interface was declared, never wired (its only importer, `core/plugin_loader.py`, had no production caller); the plugin surface production actually loads is Semantic Kernel's (`orchestration/registry_setup.py`) |
