...
class ValidationService:
    ...
    def _analyze_logical_structure(
        self, premises: List[str], conclusion: str, argument_type: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_validity_score(
        self, premise_analysis: List[Dict], conclusion_analysis: Dict, structure: Dict
    ) -> float:
    ...
    def _calculate_soundness_score(
        self, premise_analysis: List[Dict], validity_score: float
    ) -> float:
    ...
    def _assess_logical_flow(self, premises: List[str], conclusion: str) -> float:
        ...
        premise_quality = (
            len(premises) >= 2
        )  # Au moins 2 prémisses pour un bon argument
    ...
    def _assess_completeness(self, premises: List[str], conclusion: str) -> float:
        ...
        premise_count_score = min(
            1.0, len(premises) / 3
        )  # Optimal autour de 3 prémisses
        conclusion_length_score = min(
            1.0, len(conclusion.split()) / 10
        )  # Conclusion substantielle
    ...
    def _generate_suggestions(self, issues: List[str], structure: Dict) -> List[str]:
        ...
        if "prémisse(s) faible(s)" in str(issues):
            suggestions.append(
                "Renforcez vos prémisses avec des données ou des sources fiables"
            )

        if "Conclusion peu claire" in str(issues):
            suggestions.append(
                "Reformulez votre conclusion de manière plus précise et spécifique"
            )

        if "peu pertinentes" in str(issues):
            suggestions.append(
                "Assurez-vous que vos prémisses sont directement liées à votre conclusion"
            )

        if "Flux logique déficient" in str(issues):
            suggestions.append(
                "Utilisez des connecteurs logiques (donc, par conséquent, ainsi...)"
            )

        if "Nombre insuffisant de prémisses" in str(issues):
            suggestions.append(
                "Ajoutez des prémisses supplémentaires pour renforcer votre argument"
            )
