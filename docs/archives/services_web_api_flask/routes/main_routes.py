...
def health_check():
    ...
    try:
        ...
        response_data = {
            ...
            "services": {
                "jvm": {
                    "running": is_jvm_started,
                    "status": "OK" if is_jvm_started else "Not Running",
                },
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Erreur de health check", message=str(e), status_code=500
                ).dict()
            ),
            500,
        )
...
async def analyze_text():
    ...
    try:
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Données invalides", message=str(e), status_code=400
                ).dict()
            ),
            400,
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Erreur d'analyse", message=str(e), status_code=500
                ).dict()
            ),
            500,
        )
...
async def validate_argument():
    ...
    try:
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Erreur de validation", message=str(e), status_code=500
                ).dict()
            ),
            500,
        )
...
def detect_fallacies():
    ...
    try:
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Erreur de détection", message=str(e), status_code=500
                ).dict()
            ),
            500,
        )
    ...
    #         return jsonify(result)
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Erreur de framework", message=str(e), status_code=500
                ).dict()
            ),
            500,
        )
...
async def logic_graph():
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la génération du graphe logique: {str(e)}", exc_info=True
        )
        return (
            jsonify(
                ErrorResponse(
